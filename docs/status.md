...
| 6 | <img src="images/scenes/scene6.png" alt="Scene 6 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.284}}$ / $\color{#1a7f37}{\textsf{0.022}}$ | $\color{#1a7f37}{\textsf{0.284}}$ / $\color{#1a7f37}{\textsf{0.021}}$ | specular-glossiness gold sphere, solid textures, requested ground, and finite DDS skybox |
...
| 13 | <img src="images/scenes/scene13.png" alt="Scene 13 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.010}}$ / $\color{#1a7f37}{\textsf{0.081}}$ | $\color{#1a7f37}{\textsf{0.010}}$ / $\color{#1a7f37}{\textsf{0.081}}$ | material grid, requested ground, and explicit occlusion semantics |
...
| 28 | <img src="images/scenes/scene28.png" alt="Scene 28 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.004}}$ / $\color{#1a7f37}{\textsf{0.049}}$ | $\color{#1a7f37}{\textsf{0.004}}$ / $\color{#1a7f37}{\textsf{0.049}}$ | `KHR_materials_clearcoat` intensity, roughness, and coat-normal textures |
...
| 31 | <img src="images/scenes/scene31.png" alt="Scene 31 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.003}}$ / $\color{#1a7f37}{\textsf{0.018}}$ | $\color{#1a7f37}{\textsf{0.003}}$ / $\color{#1a7f37}{\textsf{0.018}}$ | `KHR_materials_emissive_strength` and factor-only emissive materials |
...
| 168 | <img src="images/scenes/scene168.png" alt="Scene 168 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.068}}$ / $\color{#1a7f37}{\textsf{0.388}}$ | $\color{#1a7f37}{\textsf{0.068}}$ / $\color{#1a7f37}{\textsf{0.389}}$ | mirrored double-sided winding through a clockwise front-face pipeline; 100% within one byte |
| 176 | <img src="images/scenes/scene176.png" alt="Mosquito in Amber" width="120"> | $\color{#1a7f37}{\textsf{0.064}}$ / $\color{#1a7f37}{\textsf{0.064}}$ | — | integrated linear transmission, authored alpha state, IOR, volume, and multisampled scene-color copy<br><em>Babylon's pinned normal-normalization order and repeat-addressing scene-color sampler cut the former 0.263 residual to 0.064; the remainder is bounded by the recorded resolve-then-tone-map adaptation in [fidelity.md](fidelity.md).</em> |
...
| 210 | <img src="images/scenes/scene210.png" alt="Scene 210 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.037}}$ / $\color{#1a7f37}{\textsf{0.210}}$ | $\color{#1a7f37}{\textsf{0.037}}$ / $\color{#1a7f37}{\textsf{0.210}}$ | `KHR_xmp_json_ld` metadata on a rounded cube<br><em>Every pixel is within one byte; a sub-LSB shading offset on the smooth gray cube flips rounding boundaries, matching upstream's accepted same-browser floor.</em> |
...
| 243 | <img src="images/scenes/scene243.png" alt="Scene 243 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.046}}$ / $\color{#cf222e}{\textsf{1.043}}$ | $\color{#1a7f37}{\textsf{0.046}}$ / $\color{#cf222e}{\textsf{1.043}}$ | deterministic MorphStressTest glTF animation with Babylon-compatible overlapping clip precedence and pinned uncapped storage-buffer morphing<br><em>The storage-buffer GPU morph path renders bit-identically to the former CPU fallback, and the Dawn backend — the browser's own compiler and raster stack — reproduces SDL_GPU within one LSB, ruling out evaluation place, shader codegen, and rasterization; the residual is MSAA coverage stepping from float32-versus-float64 deformation input math on morphed silhouettes, with the interior within 2 LSB.</em> |
...
| 247 | <img src="images/scenes/scene247.png" alt="Scene 247 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.035}}$ / $\color{#1a7f37}{\textsf{0.405}}$ | $\color{#1a7f37}{\textsf{0.035}}$ / $\color{#1a7f37}{\textsf{0.406}}$ | `EXT_mesh_gpu_instancing` with local extension T/R/S, separate node-world composition, and one native instanced draw<br><em>Normals follow Babylon's pinned order — object-space normalize, unnormalized varying, fragment renormalize — which cut silhouette error by a third; the remainder is MSAA coverage stepping traced to float32-versus-float64 instance/parent world composition (Dawn reproduces SDL_GPU within one LSB, exonerating codegen and raster).</em> |
...
| 255 | <img src="images/scenes/scene255.png" alt="Scene 255 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.011}}$ / $\color{#1a7f37}{\textsf{0.101}}$ | $\color{#1a7f37}{\textsf{0.011}}$ / $\color{#1a7f37}{\textsf{0.101}}$ | normalized integer skin-weight accessors |
...
| 265 | <img src="images/scenes/scene265.png" alt="Scene 265 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.002}}$ / $\color{#1a7f37}{\textsf{0.037}}$ | $\color{#1a7f37}{\textsf{0.002}}$ / $\color{#1a7f37}{\textsf{0.037}}$ | `EXT_lights_image_based` half-float RGBD cubemap, generated 1024-sample BRDF LUT, unclamped SH irradiance, rotation, and raw-vector horizon occlusion |
| 266 | <img src="images/scenes/scene266.png" alt="Scene 266 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.130}}$ / $\color{#1a7f37}{\textsf{0.247}}$ | $\color{#1a7f37}{\textsf{0.130}}$ / $\color{#1a7f37}{\textsf{0.246}}$ | mirrored spheres with source-derived clockwise front-face state; 99.46% within one byte |
...
| transmission-skybox | <img src="images/scenes/transmission-skybox.png" alt="PBR skybox rendering" width="120"> | $\color{#1a7f37}{\textsf{0.013}}$ / $\color{#1a7f37}{\textsf{0.013}}$ | — | independent PBR skybox-mode gate |
