...
| Scene | Preview | SDL_GPU | Dawn | Coverage |
| ---: | :---: | ---: | ---: | --- |
| light-setters | <img src="images/scenes/regression-light-setters.png" alt="Light setters rendering" width="160"> | 0.000 / 0.000 | 0.000 / 0.000 | a light's properties written after creation: a spot moved onto the ground by its two `ObservableVec3` setters, its cone narrowed, sharpened and range-limited by its three scalar writes -- `angle` through the accessor that recomputes the cone cosine, `exponent` and `range` as record stores -- and a directional position that reaches no pixel but has to compile and link |
