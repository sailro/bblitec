...
| 4 | <img src="images/scenes/scene4.png" alt="Scene 4 rendering" width="160"> | 0.000 / 0.000 | 0.000 / 0.000 | two shadow filters over one receiver: an exponential-shadow-map directional light whose caster pass writes its own exponential depth into a `rgba16float` map and blurs it through the pin’s generated separable kernel, and a PCF spot beside it — the composed receiver declares a float-texture triple and a depth-texture triple in one group. The ground is displaced from a greyscale heightmap by the pin’s own CPU builder
...
| 18 | <img src="images/scenes/scene18.png" alt="Scene 18 rendering" width="160"> | 0.000 / 0.000 | 0.000 / 0.000 | percentage-closer-filtered spot shadows: a depth-only caster pass through the box material's own no-colour view under the pin's standard-Z exception, and the ground's composed receiver fragment sampling that map with the pin's nine-tap comparison filter |
...
| 22 | <img src="images/scenes/scene22.png" alt="Scene 22 rendering" width="160"> | 0.000 / 0.000 | 0.000 / 0.000 | a PBR shadow receiver: the ground samples the same two filters scene 4 composes for the Standard family, through the pin's own `createPbrShadowFragment`, and takes the multi-light arm because a receiver never resolves the single-light one. Its albedo is a linear-format image the material decodes itself under `setPbrGammaAlbedo`, and its punctual falloff is the Standard-style range and spot exponent |
...
| sprite-layer-arms | <img src="images/scenes/regression-sprite-layer-arms.png" alt="Sprite layer arms rendering" width="160"> | 0.000 / 0.000 | 0.000 / 0.000 | the Sprite2D arms upstream's own demos use and no corpus scene reaches: `updateSprite2DIndex` once per preserve rule (position, colour, a frame change keeping its flip, a hide/show restoring size from the shadow, flip alone, rotation alone), `clearSprite2DLayer`, and the three renderer-list entry points fired from a zero-delay timeout so both backends rebuild their per-layer GPU records |
