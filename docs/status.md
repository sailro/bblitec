...
| 243 | <img src="images/scenes/scene243.png" alt="Scene 243 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.002}}$ / $\color{#1a7f37}{\textsf{0.052}}$ | $\color{#1a7f37}{\textsf{0.002}}$ / $\color{#1a7f37}{\textsf{0.052}}$ | deterministic MorphStressTest glTF animation with Babylon-compatible overlapping clip precedence, pinned uncapped storage-buffer morphing, and the dedicated TEXCOORD_1 occlusion pair<br><em>The former 1.043 foreground floor was never deformation math: instrumented browser captures showed bit-identical weights, deltas, and geometry, and isolating the draws localized the whole band to the platform slab's baked-AO occlusion texture on TEXCOORD_1, which the native material pipeline silently dropped. Porting Babylon's dedicated uv2 occlusion pair closed it on both backends.</em> |
...
| 247 | <img src="images/scenes/scene247.png" alt="Scene 247 rendering" width="120"> | $\color{#1a7f37}{\textsf{0.035}}$ / $\color{#1a7f37}{\textsf{0.405}}$ | $\color{#1a7f37}{\textsf{0.035}}$ / $\color{#1a7f37}{\textsf{0.406}}$ | `EXT_mesh_gpu_instancing` with local extension T/R/S, separate node-world composition, and one native instanced draw<br><em>Normals follow Babylon's pinned order — object-space normalize, unnormalized varying, fragment renormalize — which cut silhouette error by a third; the remainder is achromatic MSAA coverage stepping on instanced silhouettes (Dawn reproduces SDL_GPU within one LSB, exonerating codegen and raster). Scene 243's floor proved to be an unrelated occlusion-texture gap, so this residual awaits its own instrumented differential capture rather than a shared arithmetic theory.</em> |
