...
| --- | --- |
| TypeScript modules/functions | classes as compile-time instances (private fields become locals, the constructor and void command methods inline with `this` bound to those fields; inheritance, accessors, statics, and value-returning methods are rejected), records carrying methods and single-return getters that close over the scope which built them (a factory can hand one back and the frame loop can drive it), named local imports and re-exports, module constants, typed non-generic function parameters/defaults, lexical scopes, if/else, numeric for/while with native break/continue, switch over numeric discriminants, static-array for-of unrolling plus runtime range-for over data containers, recursion rejection; fully data-typed functions emit once as real C++ functions with early returns, while handle-touching helpers keep the inline path with one final return |
| Plain-data model | resource handles stored inside data (a mesh held in a struct or array drives its transforms, materials, and scene membership like a mesh local), const locals bound to a container element or member as aliases whose writes reach the container, interface-typed structs by value, `T \| null` optionals with checker narrowing, dynamic arrays (`new Array`, fill, multi-element push, pop, single-element splice, index writes, length truncation), `Float32Array`/`Uint32Array` with storage-exact element semantics, string-literal-union enum tags that a `let` local can be reassigned to, `Record<Union, T>` as one slot per union member indexed at runtime by the tag (slots sit in tag order while their initializers evaluate in written order), deep readonly numeric tables with runtime indexing, constant arrays of numbers or tags materialized as shared namespace constants once a runtime index or an `indexOf` reaches them, `indexOf` over any array whose elements compare by value (numbers, booleans, tags, handles), mutable tuple locals with runtime index writes, tuple/struct destructuring in for-of and declarations, function-valued parameters inlined at their call sites (early bare returns lower through breakable wrappers; object and typed-array parameters alias their arguments on the inline path exactly like the native-function reference passing), numeric `\|\|` fallbacks, object spread in declarations and assignments, runtime `Math` calls, and the pinned seeded `Math.random` contract |
...
| tetris-board | <img src="images/scenes/tetris-board.png" alt="Tetris board rendering" width="120"> | $\color{#1a7f37}{\textsf{0.016}}$ / $\color{#1a7f37}{\textsf{0.016}}$ | $\color{#1a7f37}{\textsf{0.016}}$ / $\color{#1a7f37}{\textsf{0.016}}$ | the stage-2 capstone: the demo's board drawn the way its own renderer draws it (`lab/lite/src/demos/tetris/renderer.ts`) at a board the pinned rules played — studio IBL, a blurred PBR skybox box, the dark backboard, glossy emissive chips over the pinned chamfered generator, a translucent ghost, and per-colour fixed-capacity thin-instance pools<br><em>Every pixel is within one step of the golden on both backends, the usual environment residual (~5% differ by exactly one). Building it found the environment gate bug: `materialFactors.w` carries `environmentIntensity`, and a `< 0.5` early-out was dropping the environment from any material asking for less than half intensity.</em> |
...
| tetris-modes | <img src="images/scenes/tetris-modes.png" alt="Tetris modes rendering" width="120"> | $\color{#1a7f37}{\textsf{0.000}}$ / $\color{#1a7f37}{\textsf{0.000}}$ | $\color{#1a7f37}{\textsf{0.000}}$ / $\color{#1a7f37}{\textsf{0.000}}$ | the demo renderer's block-style switch (`lab/lite/src/demos/tetris/renderer.ts`): three render sets keyed by the demo's `TetrisMode` union in a `Record`, reached through the demo's own renderer record: a factory returns `{ setMode, toggleMode, get mode() }` closing over the mode it holds, and the frame loop drives it. Every set occupies the same cells and only the selected one is scaled into view, so the slot the tag picks is the whole picture, while a marker cube reports its height from the getter<br><em>Byte-identical to the golden on both backends (maximum channel delta 0). Slots are laid out in the union's tag order while their initializers evaluate in written order, so the rows enter the scene in source order. The record outlives the factory that built it, so it carries that scope: its methods and getter still reach the mode local.</em> |
