...
| Engine/scene | creation, registration, fixed delta, reached before-render callbacks, runtime material-family append, `setFog` linear/exp/exp2 fog on PBR-only scenes |
...
- scene fog is ported for PBR-only scenes; fog composed with Standard,
  Grid, custom-shader, background, transmission, geometry-output, or
  diagnostic surfaces fails explicitly until those fog fragments are
  ported
