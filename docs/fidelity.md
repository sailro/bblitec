...
| Unicode/collation | Normalization and collation use the host ICU C API (Windows system ICU, packaged ICU elsewhere). Locale data and default language follow that host; sort order can differ across ICU versions. JavaScript UTF-16 strings preserve lone surrogates in native WTF-8 storage. |
| Language runtime | A thrown Error travels as its message: a caught value's `name` is `Error` and a `{ cause }` option is dropped. Error string conversion combines the represented name and message; the optional JavaScript `stack` field is undefined. WeakMap/WeakSet hold their keys strongly. `Date.now` reads the system clock. Generic bodies are spelled per instantiation and rest arguments are packed per call. |
| Storage/files | Host preference storage, native URL tokens and synchronized picker completion |
| Runtime HTTP | WinHTTP on Windows, libcurl elsewhere; requests execute outside the realm thread. The complete body is buffered before fulfillment, with a 32 MiB request/response limit. Native transport uses system TLS validation and has no browser cookie jar or CORS policy. Windows operations time out after 5 seconds without progress; libcurl uses a 5-second connection and 30-second request limit. Closing a realm cancels pending work and waits for its transport threads. |
| Environment | Navigator identifies the native runtime and SDL platform; processor count and language come from the host. `onLine` remains true and does not probe network reachability. Native Window contexts report `isSecureContext` as true. Browser client hints, device-memory estimates and `navigator.gpu` are absent. |
