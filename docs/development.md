...
| `BBLITE_RUNTIME_TRACE=1` | print input dispatch, camera changes, scene-membership rebuilds and native window create/reuse/destroy identity to stderr; the trace observes generated applications without modifying their source |
...
| `BBLITE_INPUT_REPLAY=<actions>` | dispatch one comma-separated DOM `KeyboardEvent.code` (optionally `Ctrl+`), mouse button, exact `MouseMove@x:y` / `MouseLeft@x:y` client coordinate, retained-UI `UiClick@x:y`, `MouseLeftOutsideCanvas`, `MouseMoveRight`, wheel, or `WindowClose` action per frame through the application's ordinary callbacks (`-` is an idle frame), for deterministic source-independent interaction diagnostics |
