...
test("scroll behavior retains axis longhands, shorthand order and live style writes",()=>{
    const result=compileSheet('.panel{overflow:auto;overscroll-behavior:auto contain;overscroll-behavior-x:none}@media(max-width:480px){.panel{overscroll-behavior-y:none}}');
    assert.match(result.cpp,/overscroll-behavior:auto contain;overscroll-behavior-x:none/);
    const live=compileSource(`import {createEngine} from '@babylonjs/lite';await createEngine({});const panel=document.createElement('div');document.body.appendChild(panel);panel.style.overscrollBehaviorY='contain';panel.style.removeProperty('overscroll-behavior-y');`);
    assert.match(live.cpp,/"overscroll-behavior-y", "contain"/);
    for(const css of ['overscroll-behavior:contain none auto','overscroll-behavior-y:auto none','overscroll-behavior-x:scroll'])
        assert.throws(()=>compileSheet(`.panel{${css}}`),/style property/);
});

test("native scroll containment and media cascades retain layout and axis behavior",t=>runRmlUiFixture(t,"ui-scroll-media"));

test("stable scrollbar gutters share static, media and live style admission",()=>{
    const result=compileSheet('.panel{overflow-y:auto;scrollbar-gutter:stable;scrollbar-width:thin}@media(max-width:480px){.panel{scrollbar-gutter:auto}}');
    assert.match(result.cpp,/scrollbar-gutter:stable/);
    assert.match(result.cpp,/480\.0, "scrollbar-gutter:auto"/);
    assert.throws(()=>compileSheet('.panel{scrollbar-gutter:stable both-edges}'),/style property/);
});
