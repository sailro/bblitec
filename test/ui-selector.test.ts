...
test("compound selector parsing preserves relations, quoted values and specificity", () => {
    ...
    assert.deepEqual(
        sequence.map((step) => step.relation),
        ["self", "child", "next"],
    );
    assert.equal(
    ...
    for (const source of [
        ...
        assert.deepEqual(
            parseUiSelectorSequence(
                uiSelectorSequenceCss(parseUiSelectorSequence(source)!),
            ),
            parseUiSelectorSequence(source),
        );
});
...
test("structural and negated selector terms normalize formulas and retain specificity", () => {
    ...
    assert.equal(
        uiSelectorSequenceCss(parse(".item:nth-child(+n - 2)")),
        ".item:nth-child(1n-2)",
    );
    assert.equal(
    ...
    for (const source of [
        ".item:not(:not(.muted))",
        '.item:not([title="a)b"], .panel > .entry)',
        ".item:nth-child(-2n + 5)",
        ':is(.entry, [role="dialog"]) > img',
        ".item:where(.a,.b)",
        ".item:has(> :is(.child, .other):hover, + .neighbor span)",
        ".item:not(:has(.child))",
    ])
        assert.deepEqual(
            parse(uiSelectorSequenceCss(parse(source))),
            parse(source),
        );
    for (const source of [
        ".item:not()",
        ".item:not(.a,)",
        ".item:not(.a",
        ".item:nth-child(2n of .entry)",
        ".item:nth-child(2 n)",
        ".item:nth-child(2147483648)",
        ".item:empty()",
        ".item:is()",
        ".item:where(.a,)",
        ".item:has(>)",
...
test("conditional grid geometry retains authored selector relationships", () => {
    ...
    `);
    assert.match(
        compile('.cell[data-mode="large"]{width:60px}').cpp,
        /width:60px/,
    );
    assert.match(
        compile(".grid > .cell{color:red}").cpp,
        /UiSelectorRelation::Child/,
    );
});
