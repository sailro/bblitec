...
test("value kinds reject unrelated payloads and narrow their own metadata", () => {
    ...
    const material: Value = {
        kind: "material",
        cpp: "material",
        materialUboArrayFields: new Map(),
    };
...
test("native metadata round trips preserve shared resource identity", () => {
    ...
    const transported: Value = {
        ...nativeDataMetadata(texture),
        kind: "data",
        cpp: "slot",
    };
    const restored = withNativeMetadata(
        { kind: "texture", cpp: "(*slot)" },
        transported,
    );
...
test("materialization favors leaf metadata and strips generation payloads", () => {
    ...
    const group = withNativeMetadata(
        { kind: "animation-group", cpp: "group" },
        source,
    );
    ...
    assert.equal(
        Object.hasOwn(nativeDataMetadata(promise), "promiseResult"),
        false,
    );
    assert.equal(
        Object.hasOwn(valueForKind("data", promise), "promiseType"),
        false,
    );
});
...
test("runtime choices retain only common resource metadata", () => {
    ...
    const loaded: Value = {
        kind: "asset",
        cpp: "first",
        asset: { kind: "gltf", source: "first.glb", output: "first.glb" },
    };
    const otherAsset: Value = {
        kind: "asset",
        cpp: "second",
        asset: { kind: "gltf", source: "second.glb", output: "second.glb" },
    };
    ...
    const mixed = commonResourceValue(assets, [
        assets,
        { kind: "asset", cpp: "third", assetKind: "babylon" },
    ]);
