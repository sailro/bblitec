...
} from "../src/lowering/physics-event-lowerer.js";
...
test("collision fields and separation are read from the pinned event object", () => {
    const changed = lowerPhysicsCollisionInfo(
        doctoredContext(
            "src/physics/havok-collision.ts",
            "(pointB.x - pointA.x) * normal.x",
            "(pointB.x - pointA.x) * normal.x * 2",
        ),
    );
    ...
    assert.throws(
        ...
        /finally lowering does not admit early returns/,
    );
...
for (const compiler of ["msvc", "clangcl"] as const)
    for (const thin of [false, true])
        test(
            `collision callbacks preserve identities, deferred release and finally semantics (thin=${thin}, compiler=${compiler})`,
            { skip: !tools },
            () => {
                const nativeTools = discoverWindowsBuildTools(compiler);
                const output = resolve(
                    `artifacts/physics-events-${thin ? "thin" : "ordinary"}-${compiler}`,
                );
                ...
                emitUpstreamGenerated(output, [
                    ...
                    "renderer:scene",
                    "physics:world",
                    ...(thin ? ["physics:thin-instances" as const] : []),
                ]);
                ...
                runNativeFixtureCompiler(nativeTools, [
                    ...
                    "/external:W0",
                    "test/fixtures/physics-events-check.cpp",
