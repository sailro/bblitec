...
test(
    "Bullet preserves Havok speed limits and damping, convex mass frames, filters and contact activation",
    { skip: !tools },
    async () => {
        ...
        const actual = {
            limits: jsonNumbers(captured.limits),
        ...
        const velocities = (body: HP_BodyId): number[] => [
        ...
        try {
            ...
            for (const [row, values] of expected.entries())
                for (const [lane, value] of values.entries())
                    assert.ok(
                        Math.abs(actual.impulses[row]![lane]! - value) < 1e-4,
                        `impulse ${row}, lane ${lane}: ${actual.impulses[row]![lane]} vs ${value}`,
                    );
            damped = create();
            ...
            for (const [lane, value] of velocities(damped).entries())
                assert.ok(
                    Math.abs(actual.damped[lane]! - value) < 1e-4,
                    `damping lane ${lane}`,
                );
            ...
            try {
                for (const mass of [1, 12]) {
                    ...
                    try {
                        ...
                        const properties =
                            hp.HP_Shape_BuildMassProperties(hull)[1];
                        properties[1] = mass;
                        ...
                        hp.HP_World_RemoveBody(world, moving);
                        hp.HP_Body_Release(moving);
                    }
                }
            } finally {
            ...
            writeFileSync(
                ...
                JSON.stringify(
                    { actual, expected: { impulses: expected, convex } },
                    null,
                    2,
                ) + "\n",
            );
            ...
            for (const [row, values] of convex.entries())
                for (const [lane, value] of values.entries()) {
                    const start = lane < 3 ? 0 : 3;
                    const scale = Math.hypot(...values.slice(start, start + 3));
                    assert.ok(
                        Math.abs(actual.convex[row]![lane]! - value) <
                            0.005 * Math.max(0.001, scale),
                        `convex impulse ${row}, lane ${lane}: ${actual.convex[row]![lane]} vs ${value}`,
                    );
                }
        } finally {
            hp.HP_World_RemoveBody(migrated, body);
            hp.HP_Body_Release(body);
            if (damped) {
                hp.HP_World_RemoveBody(world, damped);
                hp.HP_Body_Release(damped);
            }
            hp.HP_Shape_Release(shape);
            hp.HP_World_Release(world);
            hp.HP_World_Release(migrated);
        }
    },
);
