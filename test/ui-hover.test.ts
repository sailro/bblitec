...
test("source stylesheet hover uses the existing selector kinds and state flag", () => {
    ...
    for (const kind of [
        ...
        assert.ok(
            rules.some((rule) =>
                rule.includes(`bbl::UiStyleSelectorKind::${kind},`),
            ),
            kind,
        );
});
...
test("source CSS shares active and keyboard-focus states with host rules", () => {
    ...
    const result = compile(
        ".entry:active{color:red}.entry:focus-visible{color:blue}.entry:active:hover:focus-visible{color:green}",
    );
    const rules = result.cpp
        .split("\n")
        .filter((line) => line.includes("bbl::ui_add_style_rule("));
    ...
    for (const state of ["active", "focus-visible"])
        assert.match(
            compile(
                `.entry:${state}{display:grid;grid-template-columns:repeat(2,20px);}`,
            ).cpp,
            /grid-template-columns:repeat\(2,20px\)/,
        );
    assert.throws(
        () => compile(".entry:active:active{color:red}"),
