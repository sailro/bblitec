...
test("scene morph attachment records its exact PBR row and keeps weight updates live", async () => {
    ...
    assert.deepEqual(composed.renderableMeshFeatures, [
        pin.MSH_HAS_MORPH_TARGETS,
        0,
    ]);
    const morphVariants = composed.pinnedVariants.filter((variant) =>
        variant.fragmentKey.split("|").includes("morph"),
    );
    assert.ok(
        morphVariants.length > 0,
        "the actual pipeline must compose the morph stage",
    );
...
test("procedural and data meshes carry independent pinned skeleton and morph bits", async () => {
    ...
    for (const kind of ["from-data", "box", "sphere"]) {
        assert.equal(
            await pinnedSceneMeshFeatures({ kind, gltfAssetsBefore: 0 }),
            0,
        );
        assert.equal(
            await pinnedSceneMeshFeatures({
                kind,
                gltfAssetsBefore: 0,
                skinned: true,
            }),
            pin.MSH_HAS_SKELETON,
        );
        assert.equal(
            await pinnedSceneMeshFeatures({
                kind,
        ...
        assert.equal(
            ...
            pin.MSH_HAS_SKELETON | pin.MSH_HAS_MORPH_TARGETS,
        );
    }
    await assert.rejects(
        pinnedSceneMeshFeatures({
            ...
            gltfAssetsBefore: 0,
            morphTargets: true,
            thinInstances: "possible",
        }),
        /native-coordinate instance stream/,
    );
});
...
test("deformation attachment refuses uncertain or already-running composition", () => {
    for (const statement of [
        ...
        assert.throws(
            () => compileSource(prefix + statement),
...
interface PinMorph {
    deltasBuffer: PinBuffer;
    weightsBuffer: PinBuffer;
    count: number;
        ...
        _device: {
            ...
            queue: {
                writeBuffer(
                    ...
                    new Uint8Array(buffer.data, offset, values.byteLength).set(
                        new Uint8Array(
                            values.buffer,
                            values.byteOffset,
                            values.byteLength,
...
test("direct morph replacement refuses the pin's independently retained detached-resource shape", async () => {
    const pin = await importPinnedModule<PinMorphModule>(
    ...
    assert.equal(
        new Float32Array(mesh.morphTargets.weightsBuffer.data, 16)[0],
        0,
    );
    ...
    for (const material of [
        ...
        const source = prefix
            .replace(
                "createPbrMaterial, createMorphTargets",
                "createPbrMaterial, createStandardMaterial, createMorphTargets",
            )
            .replace(
                "mesh.material = createPbrMaterial({});",
        ...
        assert.equal(
            admitted.cpp.match(/bbl::attach_morph_target\(/g)?.length,
            1,
        );
        assert.equal(
            admitted.cpp.match(/bbl::set_morph_target_weights\(/g)?.length,
            2,
        );
        ...
        assert.equal(
            separate.cpp.match(/bbl::attach_morph_target\(/g)?.length,
            2,
        );
    }
});
...
test(
    "native direct morph storage matches pin bytes and composes deformation under the mesh world",
    { skip: !tools },
    async () => {
        const pin = await importPinnedModule<PinMorphModule>(
            "morph/create-morph-targets.js",
        ...
        const engine = pinMorphEngine();
        const checks: string[] = [];
        const positionDeltas = new Float32Array([
            1.35, -0, 2.3, -4.1, 0.125, -1e-20,
        ]);
        const normalDeltas = new Float32Array([-0, 0.3, -0.2, 0.7, -0.8, 0.9]);
        const cppFloats = (values: Float32Array): string =>
            [...new Uint32Array(values.buffer)]
                .map((bits) => `std::bit_cast<float>(${bits}u)`)
                .join(", ");
        for (const normal of [null, normalDeltas]) {
            const morph = pin.createMorphTargets(
                engine,
                [{ positions: positionDeltas, normals: normal }],
                2,
                [0.75],
            );
            checks.push(`{
        ...
        writeFileSync(
            join(output, "picking_projection.hpp"),
            deformPickingHeader(
...
${[
    ...
    .map((name) => cppDefinition(renderPlan, name))
    .join("\n")}
}
namespace bbl {
...
namespace bbl::pal {
${cppRecord(pal, "struct GpuVertex {")}
${[
    "std::array<float, 16> mesh_block_world(",
    "int pick_mesh_projection(",
...
${checks.join("\n")}
    ...
    bbl::Scene scene;
    // A deformed draw uploads the source lanes and carries the mesh world;
    // the palette and the morph compose inside the vertex stage, as the
    // pin's finalWorld does.
    for (const bool skinned : {false, true}) {
        ...
        const auto packed = bbl::pal::mesh_gpu_vertices(engine.geometries[0], record);
        ...
        assert(bbl::pal::mesh_block_world(scene, engine, record) == expected);
