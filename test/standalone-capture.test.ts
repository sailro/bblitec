...
test("standalone renderers synchronize live contexts, batch uploads and capture frame targets", (t) => {
    ...
    writeFileSync(
        ...
        [
            ...
            cppRecord(shared, "struct FrameOptions {"),
    ...
    for (const [name, file, signatures] of [
        ...
        [
            "DawnEffect",
            "pal_dawn_effect.cpp",
            ["bool acquire()", "void encode()", "void present()"],
        ],
        [
            "DawnSprite",
            "pal_dawn_sprite.cpp",
        ...
        writeFileSync(
            join(directory, `${name}.hpp`),
            `struct ${name} : ${backend}Context {\n` +
                signatures
    ...
    runNativeFixtureCompiler(tools, [
        ...
        `/Fo:${directory}/`,
        `/Fe:${executable}`,
        "test/fixtures/standalone-capture-check.cpp",
    ]);
