...
test("glTF group initialization follows actual factory values and source mutations", (t) => {
    ...
    const cases = variants.map((context) => {
        const body = context
            .functionDeclaration(module, "createAnimationGroups")
            .declaration.getText()
            .replace(/^export /, "");
        const frameRate = context
            .moduleScopeConstant(
        ...
        )() as (data: object) => Array<Record<string, unknown>>;
        return run({
            clips,
            nodes: [],
            skeletons: [],
            ...
            expected: {
                ...
                frame_rate: group.frameRate,
                playing: group.isPlaying,
                time: group.currentTime,
                speed_ratio: group.speedRatio,
    ...
    const directory = resolve("artifacts/test-gltf-animation-group-factory");
...
test("new group state cannot silently bypass the factory lowering", () => {
    assert.throws(
        () =>
            lowerGltfAnimationGroupFactory(
                doctoredContext(
                    module,
                    "weight: 1,",
                    "weight: 1, extraState: 2,",
                ),
            ),
        /Unbound animation group factory field/,
    );
});
