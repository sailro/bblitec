...
} from "../src/pinned-local-cubemap.js";
...
test("resource members execute once and flattened typed-array lanes preserve source width", (t) => {
    ...
    const result =
        compileSource(`import {createEngine,createBox} from "@babylonjs/lite";
async function main() {
...
test("retained surface admission follows actual canvas tags and registration boundaries", () => {
    const nativeHostUi = {
        ...
        elements: [
            ...
            { tag: "canvas", attributes: { id: "secondary" } },
            { tag: "button", attributes: { id: "button" } },
        ],
    };
    const source = (
        canvas: string,
    ) => `import {createEngine,createSurface,createSceneContext,registerScene} from "@babylonjs/lite";
    ...
    const independentSamples = compileSource(
        source("secondary").replace(
            "createEngine(primary)",
            "createEngine(primary,{msaaSamples:1})",
        ),
        { nativeHostUi },
    );
    assert.doesNotMatch(
        independentSamples.cpp,
        /configure_scene_render_defaults\(bbl::create_scene_context\(v_surface\), true, 1u\)/,
    );
    assert.throws(
        () => compileSource(source("button"), { nativeHostUi }),
        /canvas elements/,
    );
    assert.throws(
        () =>
            compileSource(`import {createEngine,createSceneContext,registerScene,enablePbrLocalCubemap} from "@babylonjs/lite";
        async function main(){const engine=await createEngine({});const scene=createSceneContext(engine);await registerScene(scene);await enablePbrLocalCubemap();}`),
        /before scene registration/,
    );
    assert.throws(
        () =>
            compileSource(`import {createEngine,createSceneContext,registerScene,createPbrMaterial,createSolidTexture2D} from "@babylonjs/lite";
        ...
        await registerScene(scene);material.ormTexture=createSolidTexture2D(engine,1,1,1,1);}`),
        /before scene registration/,
    );
});
...
test("selected conditional material records retain static local-cubemap setup", () => {
    const fileName = resolve(
        "corpus/babylon-lite/lab/lite/src/lite/scene186.ts",
    );
    const source = readFileSync(fileName, "utf8");
    for (const [search, probes] of [
        ["", 4],
        ["?compare=0", 2],
        ["?blend=0", 2],
    ] as const) {
    ...
    const runtimeSelection = source.replace(
...
test("pinned local probes retain distinct cube layers, grid bytes, writer fields and debug state", async () => {
    ...
    const environments = ["left", "right"].map(
        (name) =>
            `https://raw.githubusercontent.com/BabylonJS/Babylon-Lite/${pin.sourceVersion}/lab/public/textures/scene186/${name}.env`,
    );
    const plan: LocalCubemapPlan = {
        kind: "probes",
        maxCandidates: 2,
        entryFileName: resolve("test/local-probes.ts"),
        environments,
        options: {
            probes: [0, 1].map((environment) => ({
                environment,
                capturePosition: [environment * 6 - 3, 0, 1.5],
                projectionPosition: [environment * 6 - 3, 0, 2],
                projectionSize: [6, 5, 5],
                influencePosition: [environment * 6 - 3, 0, 2],
                influenceInnerSize: [0.075, 5, 5],
                influenceOuterPosition: [0, 0, 2],
                influenceOuterSize: [24, 6, 6],
            })),
            voxelGrid: {
                minimum: [-6, -2.5, -0.5],
                maximum: [6, 2.5, 4.5],
    ...
    assert.deepEqual(
        debug.uniform
            .map((lane, index) => (lane === packet.uniform[index] ? -1 : index))
            .filter((index) => index >= 0),
        [3],
    );
    for (const shape of ["box", "sphere"]) {
        const single = await packLocalCubemap({
            ...plan,
            kind: "single",
    ...
    const unprojected = await packLocalCubemap({
        ...plan,
        kind: "environment",
    ...
    assert.deepEqual(
        ...
        packet,
        "Independent repeated packing changed bytes",
    );
});
