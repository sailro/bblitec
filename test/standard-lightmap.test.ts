...
test("Standard lightmap setup lowers texture replacement, removal and material inputs", () => {
    ...
    assert.match(
        result.cpp,
        /set_standard_lightmap_texture\([^;]+bbl::FileTexture\{\}\)/,
    );
    ...
    assert.throws(
        ...
        /written after this texture was bound/,
    );
    assert.throws(
        () =>
            compileSource(
                program.replace(
                    "setStandardLightmapTexture(material,null);",
                    "registerScene(scene);setStandardLightmapTexture(material,null);",
                ),
            ),
            ...
            for (const uAng of [0, Math.PI]) {
                ...
                assert(
                    variant.fragmentWgsl.includes(
                        `textureSample(lT,lS,${uAng ? `vec2<f32>(${uv}.x,1.0- ${uv}.y)` : uv}).rgb*mat.lmLvl`,
                    ),
                );
                assert(
                    variant.fragmentWgsl.includes(
...
test(
    "native Standard lightmap features, material uniforms and UV lanes match the pin",
    { skip: !tools },
    async () => {
        const store = new UpstreamSourceStore(),
            context = new LoweringContext(store),
            factory = new FactoryLowerer(context);
        ...
        const variant = pinnedStandardVariantManifestEntry(
            await composePinnedStandardVariant({
                lightmapTexture: {},
                lightmapCoordIndex: 1,
            }),
        );
        writeFileSync(
            join(directory, "bblite/upstream/pinned_variant_bindings.hpp"),
            pinnedSharedVariantDecls(context, "lightmap control"),
        );
        writeFileSync(
            join(directory, "bblite/upstream/material_texture_slots.hpp"),
            inlineCpp(
                materialTextureSlotsHeader(
                    {
                        transmission: false,
        ...
        writeFileSync(
            join(directory, "standard.hpp"),
            inlineCpp(
                pinnedStandardVariantsHeader(context, "lightmap control", [
                    variant,
                ]),
            ) +
                inlineCpp(
        ...
        const { createStandardMaterial } = await importPinnedModule<{
            createStandardMaterial(this: void): LightmapMaterial;
        }>("material/standard/create-standard-material.js");
        const { setStandardLightmapTexture } = await importPinnedModule<{
            setStandardLightmapTexture(
                this: void,
                material: LightmapMaterial,
                texture: LightmapTexture | null,
            ): void;
        }>("material/standard/set-std-lightmap.js");
        const { writeStdMaterialData } = await importPinnedModule<{
        ...
        for (const coord of [0, 1])
            for (const shadowmap of [false, true])
                for (const angle of [0, 0.37, Math.PI])
                    for (const inverted of [false, true]) {
                        ...
                        Object.assign(material, {
                            lightmapLevel: 3.2,
                            lightmapCoordIndex: coord,
                            useLightmapAsShadowmap: shadowmap,
                            uvScale: [2, 0.5],
                            uvOffset: [0.3, -0.7],
                        });
                        const data = new Float32Array(24),
                            uv = new Float32Array(56);
                        writeStdMaterialData(data, material, 1);
                        writeUvTransformData(uv, material);
                        expected.push([
...
test("Standard lightmap setter rejects changed target and extension registration", () => {
    class EditedStore extends UpstreamSourceStore {
        ...
        public override getSourceFile(module: string): ts.SourceFile {
            ...
            return ts.createSourceFile(
                module,
                module.endsWith("/set-std-lightmap.ts")
    ...
    for (const [from, to] of [
        ["mat._lightmapTexture = texture", "mat._emissiveTexture = texture"],
        ["_registerStdExt(stdLightmapExt)", "_registerStdExt(otherExt)"],
    ])
        assert.throws(
            () =>
                new FactoryLowerer(
                    new LoweringContext(new EditedStore(from!, to!)),
                ).lowerStandardMaterialSetters(setterFeatures),
            /./,
        );
});
