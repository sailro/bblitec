...
import { jsonArray, jsonObject, jsonRecords, jsonString } from "./json.js";
...
test("acceptance progress requires evidence, complete scope and final integration gates", async (t) => {
    ...
    const evidence = {
        path: "pass.log",
        sha256: hash("native checks passed\n"),
    };
    const inventory = JSON.stringify({
        requirements: [{ id: "required-api" }],
    });
    ...
    const original = {
        schemaVersion: 1,
        baseline: "neutral-fixture",
        baselinePackageIds: ids,
        integrationGateIds: ids.slice(1),
        endToEnd: {
            generate: "generate",
            build: "build",
            run: "run",
        ...
        packages: ids.map((id, index) => ({
            id,
            title: id,
            status: index === 0 ? "complete" : "open",
            dependencies: index ? [ids[index - 1]!] : [],
            blockers: [] as string[],
    ...
    const run = (ledger: typeof original) => {
        ...
        return spawnSync(
            process.execPath,
            ["tools/project-progress.mjs", ledgerPath, reportPath],
            { encoding: "utf8" },
        );
    };
    const baseline = run(original);
    assert.equal(baseline.status, 0, baseline.stderr);
    assert.equal(jsonObject(JSON.parse(baseline.stdout)).percent, 20);
    assert.match(
        readFileSync(reportPath, "utf8"),
        /1\/5 acceptance groups complete/,
    );
    const partial = structuredClone(original);
    partial.packages[0]!.status = "open";
    assert.equal(
        jsonObject(JSON.parse(run(partial).stdout)).percent,
        0,
        "passing partial checks cannot close a group",
    );
    const complete = structuredClone(original);
    complete.packages.forEach((item) => {
        item.status = "complete";
    });
    assert.equal(jsonObject(JSON.parse(run(complete).stdout)).percent, 100);

    const rejects: [string, (ledger: typeof original) => void, RegExp][] = [
        [
            "missing evidence",
            (ledger) => {
                ledger.packages[0]!.acceptance[0]!.evidence = [];
            },
        ...
        [
            "failed acceptance",
            (ledger) => {
                ledger.packages[0]!.acceptance[0]!.status = "failed";
            },
            /lacks passing evidence/,
        ],
        [
            "changed evidence",
            (ledger) => {
                ledger.packages[0]!.acceptance[0]!.evidence[0]!.sha256 =
                    "invalid";
            },
            /evidence changed/,
        ],
        [
            "open blocker",
            (ledger) => {
                ledger.packages[0]!.blockers.push("required case fails");
            },
            /open blockers/,
        ],
        [
            "removed scope",
            (ledger) => {
                ledger.packages.shift();
            },
            /baseline package\/gate removed/,
        ],
        [
            "missing launch gate",
            (ledger) => {
            ...
            /missing run integration gate/,
        ],
        [
            "merged final gates",
            (ledger) => {
                ledger.endToEnd.run = "build";
            },
            /distinct gates/,
        ],
        [
            "open dependency",
            (ledger) => {
                ledger.packages[2]!.status = "complete";
            },
            /dependency generate remains open/,
        ],
        [
            "changed inventory",
            (ledger) => {
                ledger.inventory.sha256 = "invalid";
            },
            /inventory changed/,
        ],
        [
            "omitted requirement",
            (ledger) => {
                ledger.packages[0]!.requirementIds = [];
            },
            /not fully assigned/,
        ],
        [
            ...
            (ledger) => {
                ledger.packages[1]!.requirementIds = ["required-api"];
            },
            /assigned twice/,
        ],
        [
            "dependency cycle",
            (ledger) => {
                ledger.packages[1]!.dependencies = ["build"];
            },
            /dependency cycle/,
        ],
        [
            ...
            (ledger) => {
                ledger.packages.push({
                    ...structuredClone(ledger.packages[0]!),
                    id: "extra",
                });
            },
...
`;
    ...
    execFileSync(
        process.execPath,
        ["tools/project-requirements.mjs", entry, output],
        { stdio: "pipe" },
    );
    const report = jsonObject(JSON.parse(readFileSync(output, "utf8")));
    const entryDetails = jsonRecords(report.files).find(
        (file) => file.path === "entry.ts",
    );
    assert.ok(entryDetails);
    const requirements = jsonRecords(report.requirements);
    const sites = (id: string) =>
        jsonRecords(requirements.find((row) => row.id === id)?.sites ?? []);
    assert.equal(jsonObject(report.totals).files, 2);
    assert.equal(entryDetails.sha256, hash(source));
    const imports = jsonRecords(entryDetails.imports);
    assert.deepEqual(
        imports.map((item) => item.typeOnly),
        [true, false, true],
    );
    assert.ok(imports.every((item) => item.target === "helper.ts"));
    assert.equal(
        jsonArray(entryDetails.workers).length,
        1,
        "local Worker classes are not platform entries",
    );
    assert.equal(jsonArray(entryDetails.dynamicImports).length, 1);
    for (const id of [
        "syntax:class-extends",
        "syntax:class-static-block",
        "syntax:generator",
        "syntax:for-await",
        "syntax:await-in-catch",
        "syntax:await-in-finally",
        "tuple:dynamic-write",
        "syntax:rest-binding",
        "unresolved:property",
    ]) {
    ...
    const listeners = requirements
        .filter((row) => jsonString(row.id).endsWith(".addEventListener"))
        .flatMap((row) => jsonRecords(row.sites));
    assert.equal(listeners.length, 1);
    const detail = jsonObject(listeners[0]!.detail);
    assert.equal(detail.event, "pointermove");
    assert.deepEqual(detail.arguments, ["StringLiteral", "callback", "true"]);
});
