...
test(
    "the complete Babylon camera parser matches the pin for absent, null and populated options",
    { skip: !nativeTools },
    async () => {
        interface Camera {
        ...
        const pin = await importPinnedModule<{
            parseBabylonCamera(this: void, data: object): Camera;
        }>("loader-babylon/parse-camera.js");
        const inputs = [
            { position: [1, 2, 3] },
            {
                position: [-1.234567890123, 0, 16777217],
                rotation: [],
                fov: null,
                minZ: null,
                maxZ: null,
        ...
        const guard = lowerBabylonCamera(
            doctoredContext(
                modulePath,
                "if (cd.fov != null) {\n        cam.fov = cd.fov;",
                "if (cd.fov == null) {\n        cam.fov = 0.75;",
            ),
        ).replace("parse_babylon_camera(", "parse_changed_guard(");
        const fallback = lowerBabylonCamera(
            doctoredContext(
                modulePath,
                "cd.rotation?.[0] ?? 0",
                "cd.rotation?.[0] ?? 0.5",
            ),
        ).replace("parse_babylon_camera(", "parse_changed_fallback(");
