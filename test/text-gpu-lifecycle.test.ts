...
function pinnedFunctions(context: LoweringContext): string {
    ...
    return ts.transpileModule(
        [
            ...
            module("src/text/_gpu/text-textures.ts", [
                "nextPow2Rows",
                "rowsForTexels",
                "createMetaBuffer",
                "createAtlasTexture",
...
function statementProbe(source: string, adapt = true): string {
    ...
    const statement: NonNullable<PinnedNumericScope["statement"]> = (
        ...
        if (
            ...
            node.expression.expression.getText() !== "retain"
        )
            return undefined;
        if (node.expression.arguments.length !== 1)
            throw new Error("Unrepresented retained statement shape.");
        return [
            `${indent}ops.event("hook", ${lowerer.expression(node.expression.arguments[0]!)});`,
        ];
    };
...
test("text GPU helpers preserve pinned identities, byte uploads, growth, failures and draw order", async (t) => {
    ...
    writeFileSync(
        resolve(directory, "statement-probe.hpp"),
        `void statement_probe(Ops& ops){\n${statementProbe(probe)}\n}`,
    );
    const events: string[] = [],
        writes: Buffer[] = [];
    ...
    const write = (
        ...
        failure("write:" + target.label);
        event("write", target.id, offset, count);
        writes.push(Buffer.from(buffer.slice(begin, begin + count)));
    };
    const makeDevice = () => ({
        ...
        }) => resource(label, size.width * size.height * 16),
        createBindGroup: ({
            layout,
            entries,
        }: {
            layout: { id: number };
            entries: Array<{ resource: Resource | { buffer: Resource } }>;
        }) => {
            failure("group");
            const group = { id: next++ };
            event(
                ...
                group.id,
                layout.id,
                ...entries.map(({ resource: r }) =>
                    "buffer" in r ? r.buffer.id : r.id,
                ),
            );
            return group;
        },
        queue: {
            ...
            writeTexture: (
                ...
                buffer: ArrayBuffer,
                options: { offset: number; bytesPerRow: number },
                size: { height: number },
            ) =>
                write(
                    texture,
                    ...
                    buffer,
                    options.offset,
                    options.bytesPerRow * size.height,
                ),
        },
    });
    const deviceA = makeDevice(),
        deviceB = makeDevice(),
        engine = { _device: deviceA };
    const pattern = (count: number, seed: number) =>
        new Float32Array(
            Uint8Array.from({ length: count }, (_, i) => (i * 37 + seed) & 255)
                .buffer,
        );
    ...
    interface GroupObservation {
        _curveSet: { _atlas: AtlasObservation };
        _curveSetId: string;
        _groupKey: string;
        _slotStart: number;
        _slotCount: number;
        _liveCount: number;
        _bindGroup: { id: number } | null;
    ...
    const atlas: AtlasObservation = {
        _curveTexData: pattern(131072, 3),
        _bandTexData: pattern(131072, 4),
        _metaData: pattern(384, 5),
        _curveTexelsUsed: 3,
        _bandTexelsUsed: 7,
        _slotCount: 3,
        _version: 1,
    ...
    const data = {
        _instances: pattern(192, 1),
        _styles: pattern(128, 2),
        _instanceCount: 3,
        _styleCount: 1,
        _version: 1,
        _styleVersion: 1,
    ...
    const constants = (path: string, name: string) => {
        const f = context.sourceFile(path);
        return context.numericValue(context.variableInitializer(f, name), f);
    };
    ...
    const pass = {
        ...
        setPipeline: (p: { id: number }) => event("pipeline", p.id),
        setBindGroup: (slot: number, g: { id: number }) => {
            assert.equal(slot, 0);
            event("bind", g.id);
        },
    ...
    const draw = () =>
        event("draws", pin.drawTextRenderable(gpu, data, quad, pass));
    const record = () => {
        assert(atlas._gpu);
        event(
            "state",
            gpu._instanceCap,
            gpu._styleBuf.size,
            gpu._uploadedDataVersion,
            gpu._uploadedStyleVersion,
            data._dirtyStart,
            data._dirtyEnd,
            atlas._gpu._curveTexRows,
            atlas._gpu._bandTexRows,
            atlas._gpu._metaCap,
            atlas._gpu._uploadedVersion,
            data._groups[0]!._bindGroup?.id ?? 0,
            data._groups[0]!._bindGroupVersion,
        );
    };
    const actions: string[] = [];
    const act = (cpp: string, run: () => void) => {
        actions.push(cpp);
        run();
    };
    act("statement_probe(ops);", () =>
        createJavaScriptFunction(
            "retain",
            probe,
        )((value: number) => event("hook", value)),
    );
    const alpha = await importPinnedModule<{
        setAlphaToCoverage(this: void, target: object, enabled: boolean): void;
    ...
    act(
        ...
        () =>
            event(
                "alpha",
                +alpha.getAlphaToCoverage(r),
                +alpha.getAlphaToCoverage(second),
            ),
    );
    act(
    ...
    act(
        "data->version=2;data->dirty_start=1;data->dirty_end=2;update();record();",
        () => {
            data._version = 2;
            data._dirtyStart = 1;
            data._dirtyEnd = 2;
    ...
    act(
        'data->version=3;data->dirty_start=1;data->dirty_end=3;ops.fail="write:text-instance";try{update();return 11;}catch(const std::runtime_error&){}record();',
        () => {
            data._version = 3;
            data._dirtyStart = 1;
            data._dirtyEnd = 3;
            fail = "write:text-instance";
            assert.throws(update, /write:text-instance/);
            record();
        },
    );
    assert.equal(gpu._uploadedDataVersion, 2);
    assert.equal(data._dirtyEnd, 3);
    act("update();record();", () => {
        update();
        record();
    });
    act(
        ...
        () => {
            ...
            data._version = 4;
            data._styleCount = 3;
            data._styleVersion = 2;
            atlas._curveTexelsUsed = 4097;
            atlas._bandTexelsUsed = 4098;
            atlas._slotCount = 6;
            atlas._version = 2;
            update();
            record();
    ...
    act(
        ...
        () => {
            ...
            assert.throws(ensure, /create:text-instance/);
            assert.equal(r._gpu, oldGpu);
            record();
        },
    );
    act("ensure();update();record();", () => {
        ensure();
        update();
        record();
    });
    act(
        "data->instance_count=0;data->version=5;update();draw();record();",
        () => {
    ...
    assert.equal(
        readFileSync(resolve(directory, "events.txt"), "utf8").replaceAll(
            "\r\n",
            "\n",
        ),
        events.join("\n") + "\n",
...
test("resource adapters refuse changed device shapes and leave unknown statements to the shared lowerer", () => {
    ...
    assert.throws(
        () => statementProbe("retain(1,2);"),
        /Unrepresented retained statement shape/,
    );
    assert.throws(
