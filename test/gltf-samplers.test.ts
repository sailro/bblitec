...
import {
    ...
    lowerGltfSamplers,
} from "../src/lowering/gltf/sampler-resolver.js";
...
test("glTF sampler lookup, descriptors and sharing execute the pinned source", (t) => {
    ...
    const contexts = [
        ...
        doctoredContext(
            module,
            "json.samplers?.[json.textures[texInfo.index].sampler]",
            "json.samplers?.[0]",
        ),
        doctoredContext(
    ...
    const inputs: { textures?: object[] | null; samplers?: object[] | null }[] =
        [
            {},
            { textures: null, samplers: null },
            { textures: [{}, { sampler: null }, { sampler: 4 }], samplers: [] },
            {
                textures: [{ sampler: 0 }, { sampler: 1 }, { sampler: 2 }],
                samplers: [{}, { minFilter: 9728 }, { wrapS: 33071 }],
            },
        ];
    for (const minFilter of [
        ...
        inputs.push({
            ...
            samplers: [
                { minFilter, magFilter: 9729 },
                { minFilter, magFilter: 9728, wrapS: 33071, wrapT: 33648 },
    ...
    const outputs = contexts.map((context) => {
        ...
        const sampler = context.findNodes(
            declaration,
            (node): node is ts.VariableDeclaration =>
                ts.isVariableDeclaration(node) &&
                ts.isIdentifier(node.name) &&
                node.name.text === "sampler",
        )[0];
        assert.ok(
            sampler?.initializer && ts.isCallExpression(sampler.initializer),
        );
        const source = [
            context
                .functionDeclaration(module, "gltfTexSamplerDesc")
                .declaration.getText(),
            context
        ...
        return inputs.map((input) => {
            ...
            return {
                ...
                descriptors: infos
                    .slice(1)
                    .map((info) => execute.gltfTexSamplerDesc(input, info!)),
                samplers,
                serial,
