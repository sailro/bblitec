...
interface Group {
    id: number;
    _gltfMixer: object[] | undefined;
    weight: number;
    _additive: object | undefined;
    _stopped: boolean;
}
...
function sourceResult(context: LoweringContext): unknown[] {
    ...
    for (const group of groups)
        if (group.key !== null && !keys.has(group.key)) keys.set(group.key, {});
    const groupValues: Group[] = groups.map((group) => ({
        id: group.id,
        weight: group.weight,
        _stopped: group.stopped,
        _gltfMixer:
            group.key === null ? undefined : [{}, keys.get(group.key)!, []],
        _additive: group.additive ? {} : undefined,
    }));
    const scratch = {
        keys: new Set<object>(),
        targets: new Map<object, Target>(),
    };
    const events: unknown[] = [];
    const reset = (target: Target) => {
        events.push(["reset", target.key]);
        target.active = false;
        target.value = 0;
    };
    const getTarget = (_scratch: unknown, mixer: object[]) => {
        const key = mixer[1]!;
        let target = scratch.targets.get(key);
        if (!target) {
            target = {
                key: [...keys].find((entry) => entry[1] === key)![0],
                active: false,
                value: 0,
            };
            scratch.targets.set(key, target);
            reset(target);
        }
        return target;
    };
    const body = context
        .functionDeclaration(module, "updateWeightedGltfAnimations")
        .declaration.getText();
    const update = createJavaScriptFunction(
        "getScratch",
        "getAnimationGroups",
        ...
        reset,
        getTarget,
        (group: Group, _mixer: object[], delta: number) => {
            events.push(["advance", group.id, delta]);
            return 0;
        },
        (
            _manager: unknown,
            ...
            events.push(["base", group.id, delta]);
            const target = getTarget(scratch, mixer);
            target.active = true;
            target.value += group.weight;
        },
        (group: Group, delta: number, engine: object) => {
            assert.ok(engine);
            events.push(["tick", group.id, delta]);
        },
        (_scratch: unknown, group: Group, mixer: object[]) => {
            events.push(["add", group.id]);
            getTarget(scratch, mixer).value += group.weight * 10;
        },
    ...
    const run = () => ({
        ...
        targets: [...scratch.targets.values()].map((target) => ({ ...target })),
    });
    const first = run();
    for (const group of groupValues) {
        group.weight = 1;
        group._additive = undefined;
    }
    const second = run();
...
function contexts(): LoweringContext[] {
    ...
    const { file, declaration } = base.functionDeclaration(
        module,
    ...
    const reordered =
        file.text.slice(0, start) +
        upload.getText(file) +
        "\n" +
        additive.getText(file) +
        file.text.slice(end);
    return [
        base,
        doctoredContext(
            module,
            "group.weight === 1 && !group._additive",
            "group.weight === 0.5 && !group._additive",
        ),
...
test("weighted source traversal preserves shared-node keys, empty passes and publication order", () => {
    ...
    for (const context of variants)
        assert.ok(lowerGltfWeightedAnimationPasses(context));
    assert.throws(
        () =>
            lowerGltfWeightedAnimationPasses(
                doctoredContext(
                    module,
                    "tickAnimationCore(group, deltaMs, manager.engine)",
                    "tickAnimationCore(group, deltaMs, undefined)",
                ),
            ),
        /changed|identity/,
    );
