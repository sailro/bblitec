...
} from "../src/lowering/physics-heightfield-lowerer.js";
...
test("heightfield and gravity contracts reject unrepresented constructor or dispatch changes", () => {
    for (const [module, before, after, lower] of [
        ...
        [
            ...
            "return { _hkShape: hkShape, _type: PhysicsShapeType.MESH };",
            lowerPhysicsHeightfield,
        ],
        [
            "src/physics/havok.ts",
            "[gravity.x, gravity.y, gravity.z], worldPosition",
            "[gravity.z, gravity.y, gravity.x], worldPosition",
...
test(
    "heightfields preserve pinned ground samples, rays, contacts and region gravity",
    { skip: !tools },
    async () => {
        ...
        matrices.push(
            Float32Array.from([
                0.8, 0.2, 0.4, 0, 0.3, 1.1, -0.2, 0, -0.4, 0.1, 0.9, 0, -2.25,
                4.5, 0.75, 1,
            ]),
        );
        for (const count of [2, 3, 5])
            for (const matrix of matrices) {
                ...
                const expected = pin.optionsFromGroundMesh({
        ...
        writeFileSync(
            join(output, "heightfield-ground-cases.inc"),
            cases.join("\n"),
        );
        ...
        runNativeFixtureCompiler(tools!, [
            ...
            "/O2",
            "/Gy",
            "/DBBLITE_HAS_PHYSICS_HEIGHTFIELD=1",
            ...
            "/external:W0",
            "test/fixtures/physics-heightfield-check.cpp",
