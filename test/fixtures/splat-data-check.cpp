...
namespace bbl::pal {
std::vector<std::uint8_t> read_binary_file(const std::string& path) { return read(path); }
} // namespace bbl::pal
...
static void expect_geometry(const bbl::SplatMeshRecord& mesh, const std::string& path) {
    ...
    const auto compare = [&](const void* data, std::size_t length) {
        ...
        require(std::memcmp(expected.data() + offset, data, length) == 0,
                "geometry differs from pin");
        offset += length;
    };
    const std::array<std::uint32_t, 3> shape{mesh.vertex_count, mesh.texture_width,
                                             mesh.texture_height};
...
int main(int argc, char** argv) {
    ...
    require(std::memcmp(original.data(), mutated.data(), mutated.size()) == 0,
            "numeric view mutation differs from pin");
    expect_geometry(mesh, path("stage0"));
    require(mesh.data_version == 0, "buffer write published data");
    bbl::update_splat_data(engine, handle, original);
    require(mesh.data_version == 1 && mesh.splats_data->data() == original.data(),
            "same-buffer update identity/version");
    expect_geometry(mesh, path("stage1"));
    const bbl::js::ArrayBuffer replacement(
        std::vector<std::uint8_t>(original.data(), original.data() + original.byte_length()));
    ...
    require(mesh.data_version == 2 && mesh.splats_data->data() == replacement.data(),
            "replacement identity/version");
    require(original.byte_length() == mutated.size() &&
                std::memcmp(original.data(), mutated.data(), mutated.size()) == 0,
            "old alias changed");
    require(original_view.load(8) == -4.0f && replacement_view.load(7) == 7.0f,
            "replacement changed old numeric view");
        ...
        try {
            ...
            caught =
                std::string(error.what())
                    .find("updateData requires retained ArrayBuffer storage") != std::string::npos;
        }
    ...
    require(std::memcmp(mesh.splats_data->data(), replacement_bytes.data(),
                        replacement_bytes.size()) == 0,
    ...
    for (const auto length : {96u, 0u, 65u}) {
        bool caught = false;
        try {
            bbl::update_splat_data(engine, handle,
                                   bbl::js::ArrayBuffer(std::vector<std::uint8_t>(length)));
        } catch (const std::runtime_error& error) {
            caught = true;
            if (length == 96u)
                require(std::string(error.what()) == "GS vertex count mismatch",
                        "count diagnostic");
            if (length == 0u)
                require(std::string(error.what()) == "splat buffer is empty", "empty diagnostic");
        }
        require(caught, "invalid update accepted");
        require(mesh.data_version == 2 && mesh.splats_data->data() == replacement.data(),
                "rejection published state");
        expect_geometry(mesh, path("stage2"));
    }
    {
    ...
    require(mesh.data_version == 3 && mesh.splats_data->byte_length() == 68,
            "aligned trailing bytes rejected");
    expect_geometry(mesh, path("stage3"));
    const auto before_bake = bbl::splat_data(engine, handle);
    const auto before_bytes = std::vector<std::uint8_t>(
        before_bake.data(), before_bake.data() + before_bake.byte_length());
    mesh.position = bbl::Vec3{3, 4, 5};
    mesh.scaling = bbl::Vec3{2, 2, 2};
    bbl::bake_current_transform_into_vertices(engine, handle);
    require(mesh.data_version == 4 && mesh.splats_data->data() != before_bake.data(),
            "bake did not replace buffer");
    require(std::memcmp(before_bake.data(), before_bytes.data(), before_bytes.size()) == 0,
            "bake changed retained alias");
    const auto baked = read(path("baked"));
    require(mesh.splats_data->byte_length() == baked.size() &&
                std::memcmp(mesh.splats_data->data(), baked.data(), baked.size()) == 0,
            "bake bytes differ from pin");
    require(mesh.position.x == 0 && mesh.position.y == 0 && mesh.position.z == 0 &&
    ...
    require(std::memcmp(original.data(), mutated.data(), mutated.size()) == 0,
            "alias lost its owner");
    require(original_view.load(1) == 0.0f && replacement_view.load(7) == 7.0f,
            "numeric aliases lost disposed cloud backing");
