...
int main() {
    ...
    assert(physics_shape_debug_descriptor(capsule).parameters ==
           physics_shape_debug_descriptor(cylinder).parameters);
    auto mesh = physics_shape_create_mesh({{0, 0, 0}, {1, 0, 0}, {0, 1, 0}}, {0, 1, 2});
    assert((physics_shape_debug_descriptor(mesh) ==
    ...
    assert((physics_shape_debug_descriptor(hull).parameters ==
    ...
    assert((physics_shape_debug_descriptor(container) ==
    ...
    assert(
        (physics_shape_debug_descriptor(sphere).parameters == std::vector<float>{1, 2, 3, 0.75f}));
