...
namespace bbl {
static int mesh_dirty = 0, node_dirty = 0, asset_writes = 0;
...
void set_mesh_rotation_quaternion(Engine& e, MeshHandle h, Vec4 q) {
    e.meshes[h.value].rotation_quaternion = q;
    e.meshes[h.value].has_rotation_quaternion = true;
    ++mesh_dirty;
}
void set_transform_node_position(Engine& e, TransformNodeHandle h, Vec3d p) {
    e.transform_nodes[h.value].position = p;
    ++node_dirty;
}
void set_transform_node_rotation(Engine& e, TransformNodeHandle h, Vec3 p) {
    e.transform_nodes[h.value].rotation = p;
    ++node_dirty;
}
void set_transform_node_scaling(Engine& e, TransformNodeHandle h, Vec3 p) {
    e.transform_nodes[h.value].scaling = p;
    ++node_dirty;
}
void set_transform_node_rotation_quaternion(Engine& e, TransformNodeHandle h, Vec4 p) {
...
int main() {
    ...
    for (const auto& handle : {mesh, node}) {
        set_scene_node_position(engine, handle, {10000000000.25, 2, 3});
        assert(scene_node_position(engine, handle).x == 10000000000.25);
        set_scene_node_rotation(engine, handle, {1, 2, 3});
        assert(scene_node_rotation(engine, handle).y == 2);
        set_scene_node_scaling(engine, handle, {2, 3, 4});
        assert(scene_node_scaling(engine, handle).z == 4);
        set_scene_node_rotation_quaternion(engine, handle, {0, 1, 0, 0});
        assert(scene_node_rotation_quaternion(engine, handle).y == 1);
    }
    assert(mesh_dirty == 4 && node_dirty == 4);
    for (const auto& handle : {mesh, node}) {
        set_scene_node_position_component(engine, handle, 1, 7);
        assert(scene_node_position(engine, handle).x == 10000000000.25);
        assert(scene_node_position(engine, handle).y == 7);
        set_scene_node_rotation_component(engine, handle, 2, 8);
        assert(scene_node_rotation(engine, handle).x == 1);
        assert(scene_node_rotation(engine, handle).z == 8);
        set_scene_node_scaling_component(engine, handle, 0, 9);
        assert(scene_node_scaling(engine, handle).x == 9);
        assert(scene_node_scaling(engine, handle).y == 3);
        set_scene_node_rotation_quaternion_component(engine, handle, 3, 1);
        assert(scene_node_rotation_quaternion(engine, handle).y == 1);
        assert(scene_node_rotation_quaternion(engine, handle).w == 1);
    }
    assert(mesh_dirty == 8 && node_dirty == 8);
    set_scene_node_position(engine, asset, {4, 5, 6});
    set_scene_node_rotation(engine, asset, {1, 2, 3});
    assert(asset_writes == 2 && scene_node_position(engine, asset).y == 5);
    assert(scene_node_rotation(engine, asset).z == 3);
    set_scene_node_position_component(engine, asset, 0, 12);
    ...
    set_scene_node_scaling(engine, asset, {1, 1, 1});
    assert(scene_node_scaling(engine, asset).x == 1);
    set_scene_node_rotation_quaternion(engine, asset, {0, 0, 0, 1});
    set_scene_node_scaling(engine, asset, {2, 1, 1});
