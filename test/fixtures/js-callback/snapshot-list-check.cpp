...
int main() {
    ...
    callbacks.push_back([&]() {
        ...
        for (const auto& callback : nested)
            callback();
    });
    ...
    for (const auto& callback : selected)
        callback();
    ...
    for (const auto& callback : before_insert)
        callback();
    assert(first == 1 && later == 0 && prepended.size() == 2);
    const auto after_insert = prepended;
    for (const auto& callback : after_insert)
        callback();
    ...
    membership.erase(
        std::remove_if(membership.begin(), membership.end(), [](int value) { return value == 2; }),
        membership.end());
