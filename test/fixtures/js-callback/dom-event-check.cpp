...
void phases() {
    ...
    const auto listen = [&](Target target, bool capture, double phase, char label) {
        registry.add(
            target, "pointermove", 1,
            [&, target, phase, label](const Event& event) {
                require(event.dom->target == child && event.dom->current_target == target,
                        "target identities");
    ...
    listen(window, true, 1, 'A');
    listen(document, true, 1, 'B');
    listen(parent, true, 1, 'C');
    listen(child, true, 2, 'D');
    listen(child, false, 2, 'E');
    listen(parent, false, 3, 'F');
    listen(document, false, 3, 'G');
    listen(window, false, 3, 'H');
    auto event = pointer();
    registry.dispatch(event);
    require(calls == "ABCDEFGH", "capture, target and bubble order");
    require(event.dom->phase == 0 && !event.dom->current_target && event.dom->path.empty(),
            "dispatch cleanup");
    calls.clear();
...
void stopping() {
    ...
    Registry immediate;
    int visits = 0;
    immediate.add(child, "pointermove", 1,
                  [](const Event& event) { event.stop_immediate_propagation(); });
...
void cancellation() {
    Registry registry;
    registry.add(
        child, "pointermove", 1, [](const Event& event) { event.prevent_default(); }, false, false,
        true);
    ...
    failures.add(
        child, "pointermove", 1, [](const Event&) { throw std::runtime_error("listener"); }, false,
        false, true);
...
void keyboard() {
    ...
    registry.add(
        window, "keydown", 1,
        [&](const bbl::PlatformKeyboardEvent& key) {
            require(key.shift_key && key.code == "Escape" && key.dom->phase == 1,
                    "keyboard uses same target traversal");
            ++calls;
            key.prevent_default();
        },
        true);
...
void engine_ownership() {
    ...
    bbl::dispatch_dom_pointer(display,
                              bbl::dom_event(Event{}, "pointermove", {child, document, window}));
    require(calls == 0 && mailbox.has_value(),
            "display queues native payload without invoking script");
...
void physical_input_transactions() {
    ...
    bbl::on_dom_pointer(application, child, "pointerout", 1, [&](const Event& event) {
        require(&bbl::dom_event_owner(event) == &application,
                "target values use the receiving realm's document");
        require(event.dom->related_target == sibling, "out carries the new hit target");
        visits.push_back("out");
    });
    bbl::on_dom_pointer(application, parent, "pointerleave", 2,
                        [&](const Event&) { visits.push_back("parent-leave"); });
    bbl::on_dom_pointer(application, sibling, "pointerenter", 3, [&](const Event& event) {
        require(event.dom->related_target == child && !event.dom->bubbles,
                "enter carries the previous hit target");
        visits.push_back("enter");
    });
    bbl::dom_input(display).hover_path = {child, parent, document, window};
    auto move = bbl::dom_pointer_input(display, bbl::DomPointerAction::Move, Event{},
                                       {sibling, parent, document, window});
    ...
    require(!move->ready() && visits.empty(),
            "display never runs application listeners or waits for completion");
    mailbox->dispatch(application);
    require(move->ready() && visits == std::vector<std::string>{"out", "enter"},
            "sibling crossing preserves the common parent's hover");
    for (const auto& entry : move->events)
        std::visit(
            [](const auto& event) {
                require(event.dom->dispatch_engine == nullptr,
                        "completed packets retain no realm pointer");
            },
            entry.payload);

    int mouse_down = 0, mouse_up = 0, clicks = 0;
    bbl::on_dom_pointer(application, sibling, "pointerdown", 4,
                        [](const Event& event) { event.prevent_default(); });
    ...
    auto down = bbl::dom_pointer_input(display, bbl::DomPointerAction::Down,
                                       Event{.button = 0, .buttons = 1}, path);
    down->dispatch(application);
    require(down->ready() && down->default_prevented && mouse_down == 0,
            "pointerdown cancellation suppresses native default and compatibility mousedown");
    auto up = bbl::dom_pointer_input(display, bbl::DomPointerAction::Up,
                                     Event{.button = 0, .buttons = 0}, path);
    ...
    bbl::dom_pointer_input(display, bbl::DomPointerAction::Down, Event{.button = 0, .buttons = 1},
                           path)
        ->dispatch(application);
    require(mouse_down == 1, "released pointer clears compatibility suppression");

    bbl::on_dom_keyboard(application, window, "keydown", 8,
                         [](const bbl::PlatformKeyboardEvent& event) { event.prevent_default(); });
    bbl::DomEventBatch keys;
    keys.add(bbl::dom_event(bbl::PlatformKeyboardEvent{}, "keydown", path), true);
    keys.dispatch(application);
    require(keys.ready() && keys.default_prevented,
            "keyboard native default waits for source cancellation");
}
} // namespace
