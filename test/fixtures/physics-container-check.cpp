...
int main() {
    ...
    const auto ray = [&](double x) {
        return pal::physics_world_raycast(solver_world, {x, 0, -2}, {x, 0, 2}, 0xffffffffu,
                                          0xffffffffu, false);
    };
    ...
    pal::physics_world_step(solver_world, 1.0 / 60);
    const auto scaled_hit = pal::physics_world_raycast(solver_world, {4, 0, 5}, {4, 0, 15},
                                                       0xffffffffu, 0xffffffffu, false);
    assert(scaled_hit.has_hit && std::abs(scaled_hit.point[2] - 8) < 0.001);
    const auto scaled_miss = pal::physics_world_raycast(solver_world, {5.1, 0, 5}, {5.1, 0, 15},
                                                        0xffffffffu, 0xffffffffu, false);
    ...
    pal::physics_world_step(solver_world, 1.0 / 60);
    const auto shared_hit = pal::physics_world_raycast(solver_world, {1, 0, 18}, {1, 0, 22},
                                                       0xffffffffu, 0xffffffffu, false);
