...
std::string text_content(Rml::Element& element) {
    ...
    for (int i = 0; i < element.GetNumChildren(); ++i)
        result += text_content(*element.GetChild(i));
    return result;
}
...
int main() {
    ...
    SDL_SetAssertionHandler(
        [](const SDL_AssertData* data, void*) {
            std::cerr << "SDL assertion: " << data->condition << " at " << data->filename << ':'
                      << data->linenum << '\n';
    ...
    assert(window);
    {
        ...
        assert(projected);
        assert(text_content(*projected) == "a\xE2\x80\x99"
                                           "b\xE2\x80\x94"
                                           "c");
        ...
        assert(text_content(*projected) ==
               "\xE2\x80\x9Cnew\xE2\x80\x9D\xC2\xA0\xC2\xA9\xE2\x80\xA6");
    ...
    }
    {
        ...
        const auto thumb = [&](Rml::Element* element) {
            ...
            for (int i = 0; i < bar->GetNumChildren(true); ++i) {
                if (bar->GetChild(i)->GetTagName() == "sliderbar")
                    return bar->GetChild(i);
            }
            return static_cast<Rml::Element*>(nullptr);
        };
        const auto expect_size = [](Rml::Element* element, float vertical, float horizontal) {
            assert(std::abs(
                       element->GetElementScroll()->GetScrollbarSize(Rml::ElementScroll::VERTICAL) -
                       vertical) < .01f);
            assert(std::abs(element->GetElementScroll()->GetScrollbarSize(
                                Rml::ElementScroll::HORIZONTAL) -
                            horizontal) < .01f);
        };
    ...
    }
    {
        ...
        for (const auto& [clip, area] : std::array{std::pair{"border-box", Rml::BoxArea::Border},
            ...
            for (const auto& vertex : frame.vertices) {
                if (vertex.red == 0xf0 && vertex.green == 0x12 && vertex.blue == 0x34 &&
                    vertex.alpha == 255) {
