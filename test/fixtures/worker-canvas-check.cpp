...
std::shared_ptr<pal::CanvasEndpoint> endpoint() {
    return std::make_shared<pal::CanvasEndpoint>(std::make_shared<pal::OffscreenSurface>(40, 30),
                                                 std::make_shared<pal::OffscreenDevice>());
}
...
void canvas_transfer() {
    ...
    require(canvas->detached() && !canvas->width() && !canvas->height(),
            "Transfer left a usable sender canvas");
    ...
    std::jthread worker([message = std::move(message), &failure, native]() mutable {
        try {
            ...
            require(copies[0] == copies[1] && copies[0]->width() == 40,
                    "Receiver lost canvas identity or dimensions");
            copies[0]->set_width(80.9);
            auto context = copies[0]->rendering_context();
            require(native->surface->extent().width == 80,
                    "Receiver did not own the transferred surface");
            require(context == copies[1]->rendering_context(),
                    "Canvas created two rendering contexts");
...
void ordered_transfer_failure() {
    ...
    require(first->detached() && !busy->detached(),
            "Later failure rolled back an earlier transfer");
}

void dimensions_and_native_lifetime() {
    const js::RealmScope realm;
    require(pal::OffscreenCanvas::dimension(-0.9) == 0, "EnforceRange checked before truncation");
    for (double value : {-1.0, 0x1p64, std::numeric_limits<double>::quiet_NaN(),
    ...
    require(canvas->width() == 40 && native->surface->extent().width == 40,
            "Failed dimension conversion mutated the canvas");
    canvas.reset();
    native.reset();
    require(!device.expired() && context->extent().width == 40,
            "Renderer lost its native resources with the source wrapper");
    context.reset();
...
int main() {
    try {
        canvas_transfer();
        ordered_transfer_failure();
        dimensions_and_native_lifetime();
