...
int main() {
    ...
    assert(window);
    {
        ...
        ui_add_style_rule(engine, sheet, UiStyleSelectorKind::Class, "menu_entry", "", "", false,
                          -1, "background-color:#edab12;", UiScrollbarPart::None, false, true);
        ui_add_style_rule(engine, sheet, UiStyleSelectorKind::Class, "menu_entry", "", "", false,
                          -1, "background-color:#1122ff;", UiScrollbarPart::None, true, false);
        ...
        ui_set_attribute(
            engine, panel, "style",
            "position:absolute;left:20px;top:20px;width:100px;height:40px;pointer-events:auto;");
        ...
        const auto color = [&](uint8_t red, uint8_t green, uint8_t blue) {
            ...
            const bool found =
                std::any_of(frame.vertices.begin(), frame.vertices.end(), [&](const auto& vertex) {
                    return vertex.red == red && vertex.green == green && vertex.blue == blue &&
                           vertex.alpha == 255;
                });
            if (!found) {
                auto* raw = runtime.projected_elements.at(panel.value).element;
                const auto actual =
                    raw->GetProperty(Rml::PropertyId::BackgroundColor)->Get<Rml::Colourb>();
                std::fprintf(
                    stderr,
                    "Expected color %02x%02x%02x; actual %02x%02x%02x; hover %d active %d focus %d focus-visible %d\n",
                    red, green, blue, actual.red, actual.green, actual.blue,
                    raw->IsPseudoClassSet("hover"), raw->IsPseudoClassSet("active"),
                    raw->IsPseudoClassSet("focus"), raw->IsPseudoClassSet("focus-visible"));
            }
            assert(found);
        };
