...
struct Recorder final : Rml::RenderInterface {
    ...
    void ReleaseTexture(Rml::TextureHandle id) override {
        require(textures.erase(id) == 1, "Texture released twice.");
    }
...
int main() try {
    ...
    const std::array<Rml::String, 4> symbols{"\xF0\x9F\x8F\x81", "\xF0\x9F\x8E\xAE",
                                             "\xF0\x9F\x9B\xA0", "\xF0\x9F\x93\xBA"};
    for (const int size : {16, 24, 32}) {
        const auto handle = engine.GetFontFaceHandle("bbl-emoji", Rml::Style::FontStyle::Normal,
                                                     Rml::Style::FontWeight::Normal, size);
        require(handle != 0, "Emoji handle creation failed.");
        for (const auto& symbol : symbols) {
            const int advance =
                engine.GetStringWidth(handle, symbol, shaping, Rml::Character::Null);
            ...
            require(engine.GenerateString(layout->GetRenderManager(), handle, 0, symbol, {0, 40},
                                          Rml::Colourb{255, 255, 255}.ToPremultiplied(), 1, shaping,
                                          meshes) == advance,
                    "Layout/render advances differ.");
            require(meshes.size() == 1 && meshes[0].mesh.vertices.size() == 4,
                    "Emoji has no glyph quad.");
            ...
            for (std::size_t offset = 0; offset < pixels.size(); offset += 4) {
                ...
                if (alpha &&
                    ...
                    ++colored;
                require(pixels[offset] <= alpha && pixels[offset + 1] <= alpha &&
                            pixels[offset + 2] <= alpha,
                        "Emoji pixels are not premultiplied.");
            }
            require(visible > static_cast<std::size_t>(size), "Emoji bitmap is empty.");
            if (symbol != symbols[0])
                require(colored > 0, "Color emoji became monochrome.");
            ...
            std::cout << "emoji=" << symbol << " size=" << size << " advance=" << advance
                      << " visible=" << visible << " colored=" << colored << '\n';
        }
    }
    Rml::Shutdown();
