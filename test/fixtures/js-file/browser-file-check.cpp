...
int main(int argc, char** argv) {
    ...
    require(blob.bytes() == std::vector<std::uint8_t>({'A', 0u, 255u, 'Z'}),
    ...
    const bbl::UiElementHandle anchor_handle{static_cast<std::uint32_t>(engine.ui_elements.size())};
    ...
    require(last_save_options.suggested_name == "map.json" &&
    ...
    save_dialog_hook = [first](bbl::Engine& current) {
        const bbl::js::Blob extra({bbl::js::blob_part_string("growth")}, "text/plain");
    ...
    bbl::js::click_download_anchor(engine, anchor_handle);
    save_dialog_hook = {};
    require(writes == 1 && written == blob.bytes(),
            "download snapshots payload before dialog callbacks");
    require_throws([&]() { bbl::js::click_download_anchor(engine, anchor_handle); },
    ...
    require_throws([&]() { bbl::js::click_download_anchor(engine, anchor_handle); },
                   "revoked URL click");
    const bbl::ObjectUrlHandle reused = bbl::js::create_object_url(engine, blob);
    ...
    const bbl::UiElementHandle input_handle{static_cast<std::uint32_t>(engine.ui_elements.size())};
    ...
    engine.ui_elements[input_handle.value].file_change_callbacks.push_back([&]() {
        ...
        const bbl::BrowserFileHandle file = bbl::js::file_at(files, 0u);
    ...
    open_file = bbl::pal::SelectedFileSnapshot{
        .bytes = std::vector<std::uint8_t>(readable.begin(), readable.end()),
    ...
    require_throws([&]() { bbl::js::click_file_input(engine, input_handle); },
    ...
    require(engine.browser_file_storage->snapshot_count() == 2u &&
                engine.browser_file_storage->retained_bytes() == readable.size() + 3u,
            "retained old File and replacement own exactly two snapshots");

    for (int index = 0; index < 32; ++index) {
        const std::string replacement = "replacement-" + std::to_string(index);
        open_file = bbl::pal::SelectedFileSnapshot{
            .bytes = std::vector<std::uint8_t>(replacement.begin(), replacement.end()),
    ...
    retained.first = {};
    require(engine.browser_file_storage->snapshot_count() == 0u &&
                engine.browser_file_storage->retained_bytes() == 0u,
            "last FileList handle reclaims the old payload");
    require_throws(
        [&]() { static_cast<void>(bbl::js::file_text(engine, bbl::BrowserFileHandle{})); },
        "absent File handle surfaces without an aliasable slot");

    bbl::Engine capped_engine;
    capped_engine.browser_file_storage = std::make_shared<bbl::BrowserFileStorage>(6u);
    ...
    bbl::js::FileList capped_retained = bbl::js::input_files(capped_engine, capped_input_handle);
    open_file = bbl::pal::SelectedFileSnapshot{
        .bytes = {'n', 'e', 'w'},
        .display_name = "new.txt",
    };
    require_throws([&]() { bbl::js::click_file_input(capped_engine, capped_input_handle); },
                   "aggregate cap refuses a replacement while the old File is retained");
    require(capped_changes == 1 &&
                bbl::js::file_text(
                    capped_engine,
                    bbl::js::file_at(bbl::js::input_files(capped_engine, capped_input_handle),
                                     0u)) == "four",
            "cap refusal preserves the current selection and dispatch count");
    capped_retained.first = {};
    bbl::js::click_file_input(capped_engine, capped_input_handle);
    require(capped_changes == 2 && capped_engine.browser_file_storage->snapshot_count() == 1u &&
    ...
    require(capped_engine.browser_file_storage->snapshot_count() == 0u &&
    ...
    const std::filesystem::path forced_staging = root / "forced-stage.tmp";
    const std::string forced_replacement = "forced";
    bbl::pal::detail::write_file_atomically_with_staging_paths(
        target,
        std::span<const std::uint8_t>{
            reinterpret_cast<const std::uint8_t*>(forced_replacement.data()),
            forced_replacement.size()},
        16u, "collision test", [&](const std::filesystem::path&, std::size_t attempt) {
            return attempt == 0u ? predictable_collision : forced_staging;
        });
    require(bbl::pal::detail::read_text_file_bounded(target, 16u, "collision result") ==
                forced_replacement,
            "exclusive staging retries a colliding name");
    require(!std::filesystem::exists(forced_staging),
            "successful collision retry commits its staging file");
    const std::string replacement = "new";
    bbl::pal::detail::write_file_atomically(target, replacement, 16u, "test file");
    require(bbl::pal::detail::read_text_file_bounded(target, 16u, "test file") == replacement,
            "atomic replacement");
    require(bbl::pal::detail::read_text_file_bounded(predictable_collision, 16u,
                                                     "collision sentinel") == "attacker",
            "predictable staging collision is untouched");

    const std::filesystem::path staging_victim = root / "staging-victim.json";
    const std::filesystem::path staging_symlink = root / "forced-stage-link.tmp";
    const std::filesystem::path staging_after_symlink = root / "forced-stage-after-link.tmp";
    {
        std::ofstream victim(staging_victim, std::ios::binary);
        victim << "victim";
    }
    std::error_code staging_symlink_error;
    std::filesystem::create_symlink(staging_victim, staging_symlink, staging_symlink_error);
    if (!staging_symlink_error) {
        bbl::pal::detail::write_file_atomically_with_staging_paths(
            target,
            std::span<const std::uint8_t>{reinterpret_cast<const std::uint8_t*>(replacement.data()),
                                          replacement.size()},
            16u, "staging symlink test", [&](const std::filesystem::path&, std::size_t attempt) {
                return attempt == 0u ? staging_symlink : staging_after_symlink;
            });
        require(bbl::pal::detail::read_text_file_bounded(staging_victim, 16u, "staging victim") ==
                    "victim",
                "exclusive staging never follows an existing symlink");
        require(std::filesystem::is_symlink(staging_symlink),
    ...
    require_throws(
        [&]() {
            bbl::pal::detail::write_file_atomically(directory_target, replacement, 16u,
                                                    "test file");
        },
    ...
    for (const auto& entry : std::filesystem::directory_iterator(root)) {
        require(!entry.path().filename().string().starts_with(".bblite-write-"),
    ...
    if (!symlink_error) {
        bbl::pal::detail::write_file_atomically(linked_target, replacement, 16u,
                                                "symlink destination");
        require(bbl::pal::detail::read_text_file_bounded(outside, 16u, "outside file") == "outside",
                "atomic replacement never follows a destination symlink");
        require(bbl::pal::detail::read_text_file_bounded(linked_target, 16u, "replaced symlink") ==
                    replacement,
                "atomic replacement replaces the symlink itself");
    }
    require_throws(
        [&]() {
            static_cast<void>(bbl::pal::detail::read_text_file_bounded(target, 2u, "test file"));
        },
