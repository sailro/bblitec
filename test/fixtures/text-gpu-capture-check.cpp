...
int main() {
    try {
        ...
        disabled.destroy(999);
        disabled.begin_frame(90);
        disabled.draw({});
        disabled.stop();
        ...
        require(first == 1 && capture.resources()[0].role == "text-ubo",
                "resource role must be copied");
        require(capture.resources()[0].uploaded_bytes.empty() &&
                    capture.resources()[0].written_ranges.empty(),
        ...
        require(capture.resources()[0].uploaded_bytes[4] == 11 &&
                    capture.resources()[0].uploaded_bytes[6] == 13,
        ...
        require(partial.written_ranges.size() == 2 && partial.written_ranges[0].offset == 4 &&
                    partial.written_ranges[0].bytes == 6 &&
                    partial.written_ranges[1].offset == 12 && partial.written_ranges[1].bytes == 3,
                "overlap must merge covered bytes without claiming the gap");
        require(partial.uploaded_bytes[5] == 12 && partial.uploaded_bytes[6] == 21 &&
                    partial.uploaded_bytes[9] == 24 && partial.uploaded_bytes[12] == 41 &&
                    !observed(partial, 10) && !observed(partial, 11),
        ...
        require(capture.resources()[0].written_ranges.size() == 1 &&
                    capture.resources()[0].written_ranges[0].offset == 4 &&
                    capture.resources()[0].written_ranges[0].bytes == 11 &&
                    capture.resources()[0].uploaded_bytes.size() == 15,
                "adjacent writes merge, while a zero-byte receipt does not fill unknown memory");
        require(capture.resources()[0].writes.size() == 5 &&
                    capture.resources()[0].writes[4].sequence == 5 &&
                    capture.resources()[0].writes[4].frame == 7 &&
                    capture.resources()[0].writes[4].offset == 20 &&
                    !capture.resources()[0].writes[4].bytes,
        ...
        require(capture.resources()[0].writes.size() == 5 &&
                    capture.resources()[0].uploaded_bytes == beforeInvalid,
        ...
        require(capture.resources()[1].writes[0].sequence == 6 &&
                    capture.resources()[1].writes[0].frame == 7,
                "write ordering must span allocations and exclude failed receipts");
        require(capture.resources()[4].id == atlas && capture.resources()[4].width == 4096 &&
                    capture.resources()[4].rows == 1,
                "texture allocation extents and IDs must survive independent buffer creation");

        TextGpuDrawCapture draw;
        draw.pipeline = 0x100000065ULL;
        draw.group = 201;
        draw.quad = 301;
        draw.instances = instances;
        draw.color_format = "rgba8unorm";
        draw.depth_format = "depth24plus";
        draw.depth_compare = "greater_equal";
        draw.topology = "triangle-list";
        draw.cull_mode = "none";
        draw.front_face = "ccw";
        draw.samples = 4;
        draw.depth_write = true;
        draw.alpha_to_coverage = true;
        ...
        draw.instances = otherInstances;
        draw.pushed_uniform_bytes[0] = 8;
        capture.draw(draw);
        draw.bindings[0].resource = second;
        draw.fragment_constants[0].value = 0;
        draw.color_format.clear();
        draw.pushed_uniform_bytes.clear();
        require(
            capture.draws().size() == 2 && capture.draws()[0].bindings[0].resource == first &&
                capture.draws()[1].bindings[0].resource == first &&
                capture.draws()[1].instances == otherInstances,
            "draw snapshots must retain the actual first group owner, not infer it from the later renderable");
        require(capture.draws()[0].pushed_uniform_bytes[0] == 1 &&
                    capture.draws()[1].pushed_uniform_bytes[0] == 8 &&
                    capture.draws()[0].fragment_constants[0].value == 1.0000000000000002 &&
                    capture.draws()[0].vertex_constants[0].id == 3 &&
                    capture.draws()[0].color_format == "rgba8unorm" &&
                    capture.draws()[0].depth_write && !capture.draws()[0].blend_enabled &&
                    capture.draws()[0].alpha_to_coverage && capture.draws()[0].samples == 4 &&
                    capture.draws()[0].first_instance == 2,
                "pipeline, range and uniform observations must be independent immutable snapshots");
        serialize("frame7.json", capture);
        require(
            capture.enabled() && capture.frame() == 7 && capture.draws().size() == 2,
            "JSON serialization must observe receipts without rerunning or mutating collection");
        const auto savedFrame = capture.draws();
        capture.destroy(first);
        capture.destroy(first);
        const auto replacement = capture.create_resource("text-ubo", 20);
        require(replacement == 6 && capture.resources()[0].id == first &&
                    capture.resources()[0].destroyed &&
                    capture.resources()[0].uploaded_bytes == beforeInvalid &&
                    !capture.resources().back().destroyed &&
                    capture.resources().back().written_ranges.empty(),
                "destruction/replacement must preserve old IDs and observed state");
        capture.begin_frame(8);
        require(capture.draws().empty() && savedFrame.size() == 2 &&
                    savedFrame[1].bindings[0].resource == first &&
                    capture.resources()[0].writes.size() == 5,
                "begin_frame replaces only draw receipts, not allocation/write history");
        draw.bindings[0].resource = replacement;
        draw.pipeline = 102;
        draw.color_format = "bgra8unorm";
        draw.samples = 1;
        draw.alpha_to_coverage = false;
        draw.blend_enabled = true;
        ...
        capture.draw(draw);
        capture.write(replacement, 0, bridge);
        require(capture.resources().back().writes[0].sequence == 7 &&
                    capture.resources().back().writes[0].frame == 8,
                "later-frame writes must keep global receipt order");
        capture.stop();
        capture.stop();
        require(!capture.enabled() && capture.create_resource("ignored", 100) == 0,
                "stop prevents later allocations");
        capture.begin_frame(9);
        capture.write(999, 999, tail);
        capture.destroy(replacement);
        capture.draw({});
        require(
            capture.frame() == 8 && capture.draws().size() == 1 &&
                capture.resources().size() == 6 && !capture.resources().back().destroyed &&
                capture.resources().back().writes.size() == 1,
