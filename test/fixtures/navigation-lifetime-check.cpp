...
int main() try {
    ...
    const NavSoloBuild solo = bbl::upstream::solo_nav_mesh_build(ground, params);
    {
        ...
        navigation_create_solo_nav_mesh(keep, ground, solo, defaults);
        ...
        for (int iteration = 0; iteration < 100; ++iteration) {
            std::weak_ptr<NavigationPluginState> weak;
            {
                ...
                require(navigation_agent_goto(crowd, agent, {2, 0, 2}),
                        "crowd lost its retained mesh");
                navigation_update_crowd(crowd, 0.1f);
                require(navigation_agent_position(crowd, agent).has_value(),
                        "crowd query lost its mesh");
            }
            require(allocations.size() == stable_count,
                    "completed navigation session retained library allocations");
        }
        require(!navigation_compute_path(keep, {-2, 0, -2}, {2, 0, 2}).empty(),
        ...
        try {
            (void)navigation_create_crowd(keep, 4, 0.5f);
        } catch (const std::runtime_error&) {
            crowd_failed = true;
        }
        fail_boundary = 0;
        require(crowd_failed && allocations.size() == before_crowd,
                "failed crowd creation leaked or succeeded");
    }
    require(allocations.empty(), "last plugin did not release all navigation allocations");
    {
        ...
        for (unsigned failure = 1; failure <= build_allocations; ++failure) {
            ...
            fail_boundary = 0;
            if (failed)
                require(original == plugin.ownership->mesh,
                        "failed tile-cache build replaced a valid mesh");
