...
int main() {
    ...
    const auto quad = [&](float x, float y, float width, float height, Rml::ColourbPremultiplied color) {
        Rml::Mesh mesh;
    ...
    assert(saved && recorder.frame.draws.empty() && recorder.frame.operations.empty() && recorder.frame.layer_count == 0);
    ...
    const auto pixel = [&](const UiRenderTexture& texture, int x, int y) {
        ...
        std::copy_n(texture.rgba->data() + (static_cast<std::size_t>(y) * texture.width + x) * 4, 4, result.begin());
        return result;
    };
    ...
    recorder.begin_frame(64, 64);
    recorder.RenderGeometry(full, {}, saved);
    assert(recorder.frame.textures[0].id == first_texture.id && recorder.frame.textures[0].rgba == first_texture.rgba);
    ...
    for (const auto geometry : {full, red, blue, hole}) recorder.ReleaseGeometry(geometry);
    ...
    assert(window);
    {
        ...
        ui_set_attribute(engine, panel, "style", "position:absolute;left:40px;top:40px;width:64px;height:48px;border-radius:12px;background-color:white;box-shadow:inset 0 0 0 4px red, 6px 8px 0 2px blue;");
        ...
        for (int y = 12; y < 36; ++y) for (int x = 12; x < 52; ++x)
        ...
        for (std::size_t index = 3; index < texture.rgba->size(); index += 4)
            covered_edge = covered_edge || (texture.rgba->at(index) > 0 && texture.rgba->at(index) < 255);
        assert(covered_edge);
        ...
        for (std::size_t index = 0; index < bytes.size(); index += 4)
            soft_blue = soft_blue || (bytes[index] == 0 && bytes[index + 2] > 0 && bytes[index + 3] < 128);
        assert(soft_blue);
