...
extern "C" bool SDLCALL SDL_SetWindowRelativeMouseMode(SDL_Window*, bool enabled) {
...
std::string environment_variable(const char* name) {
    return std::string_view(name) == "BBLITE_INPUT_REPLAY" ? replay_source : "";
}
double monotonic_milliseconds() { return 0; }
void apply_canvas_cursor(Engine&) { ++cursor_updates; }
bool handle_ui_rml_event(int&, SDL_Event&) {
    calls.push_back("ui");
    return !consume_ui;
}
...
bool request_renderer_restart_if_scene_set_changed(Engine&,
                                                   const std::vector<std::shared_ptr<Scene>>&) {
    return false;
}
struct InputDriver {
    Engine engine;
...
template <typename Driver> void exercise(int index) {
    ...
    engine.key_down_callbacks.add(1, [&](const PlatformKeyboardEvent& event) {
        calls.push_back("key");
        keys.push_back(event.code + ":" + event.key);
        if (event.code != "Escape")
            assert(event.ctrl_key && event.repeat);
        if (prevent_key)
    ...
    engine.mouse_move_callbacks.add(1, [&](const PlatformMouseEvent& event) {
        moves.push_back(event.buttons);
        assert(event.client_x == 20 && event.client_y == 30);
        assert(event.movement_x == 4 && event.movement_y == -5);
    });
    engine.mouse_wheel_callbacks.add(
        1, [&](const PlatformMouseEvent& event) { wheels.push_back(event.delta_y); });
    ...
    queue_key(SDL_SCANCODE_SPACE, true);
    queue_key(SDL_SCANCODE_F3, false);
    assert(driver.prepare() == FramePreparation::ready);
    std::vector<std::string> expected{"key"};
    if (ui)
        expected.push_back("ui");
    if (scene)
        expected.push_back("camera");
    ...
    queue_key(SDL_SCANCODE_SPACE, true);
    queue_key(SDL_SCANCODE_F3, true, true);
    assert(driver.prepare() == FramePreparation::ready);
    ...
    if (ui) {
        assert(calls == std::vector<std::string>{"ui"});
        assert(cursor_updates == 0);
    }
    consume_ui = false;
    moves.clear();
    ...
    replay_source = "WheelUp,WheelDown";
    state.input_replay = PlatformInputReplay{};
    ++state.frame;
    assert(driver.prepare() == FramePreparation::ready);
    ++state.frame;
    assert(driver.prepare() == FramePreparation::ready);
    ...
    replay_source = "UiClick@20:30";
    state.input_replay = PlatformInputReplay{};
    engine.input_replay_next_frame = 0;
    engine.input_replay_pointer_x = 16;
    engine.input_replay_pointer_y = 35;
    state.frame_options.test_pass = true;
    ++state.frame;
    assert(driver.prepare() == FramePreparation::ready);
    ++state.frame;
    ...
    for (const Uint32 type : {SDL_EVENT_FINGER_DOWN, SDL_EVENT_FINGER_UP}) {
        SDL_Event touch{};
        touch.type = type;
        touch.tfinger.windowID = SDL_GetWindowID(window);
        touch.tfinger.touchID = 3;
        touch.tfinger.fingerID = 9;
        touch.tfinger.x = .25f;
        touch.tfinger.y = .5f;
    ...
    assert(driver.prepare() == FramePreparation::ready);
    assert(!engine.pointer_locked && !engine.pointer_lock_requested && !relative_mouse &&
           cursor_visible);
    lock();
    replay_source = "Escape";
    state.input_replay = PlatformInputReplay{};
    engine.input_replay_next_frame = 0;
    ++state.frame;
    assert(driver.prepare() == FramePreparation::ready);
    ...
    on_dom_pointer(engine, DomEventTarget::canvas(), "pointerdown", 90,
                   [&](const PlatformMouseEvent& event) {
                       if (event.button == 2)
                           engine.pointer_lock_requested = true;
                   });
    on_dom_pointer(engine, DomEventTarget::canvas(), "pointerup", 90,
                   [&](const PlatformMouseEvent& event) {
                       if (event.button == 2)
    ...
    int locked_moves = 0;
    on_dom_pointer(
        engine, DomEventTarget::canvas(), "pointermove", 90, [&](const PlatformMouseEvent& event) {
    ...
    on_dom_pointer(engine, DomEventTarget::canvas(), "contextmenu", 90,
                   [](const PlatformMouseEvent& event) { event.prevent_default(); });
    consume_ui = true;
    for (const auto type : {SDL_EVENT_MOUSE_BUTTON_DOWN, SDL_EVENT_MOUSE_BUTTON_UP}) {
        queue_pointer(type, 20, SDL_BUTTON_RIGHT);
        assert(driver.prepare() == FramePreparation::ready);
        assert(engine.pointer_locked == (type == SDL_EVENT_MOUSE_BUTTON_DOWN) &&
               relative_mouse == engine.pointer_locked);
...
int main() {
    SDL_SetAssertionHandler(
        [](const SDL_AssertData* data, void*) {
