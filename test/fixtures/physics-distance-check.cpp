...
namespace p = bbl::pal;
struct Pair {
    p::PhysicsWorldHandle world;
    p::PhysicsBodyHandle a, b;
};
Pair pair(double parent, double child, double y, bool dynamic_parent = false,
          bool cartesian = false, const btTransform& placement = btTransform::getIdentity()) {
    const auto world = p::physics_world_create();
    p::physics_world_set_gravity(world, {0, 0, 0});
    ...
    const auto pose = [&](const btVector3& position) {
        const auto point = placement * position;
        const auto q = placement.getRotation();
        return p::PhysicsTransform{{point.x(), point.y(), point.z()}, {q.x(), q.y(), q.z(), q.w()}};
    };
    p::physics_body_set_transform(a, pose({0, 0, 0}));
    p::physics_body_set_transform(b, pose({0, btScalar(y), -.2f}));
    p::physics_world_add_body(world, a, false);
    p::physics_world_add_body(world, b, false);
    p::PhysicsConstraintAxes axes{};
    axes[6] = {p::PhysicsConstraintAxisMode::limited, 1, 2};
    if (cartesian)
        axes[0] = {p::PhysicsConstraintAxisMode::locked};
    p::physics_world_create_constraint(world, a, b, {{0, parent, 0}, {1, 0, 0}, {0, 1, 0}},
                                       {{0, child, 0}, {1, 0, 0}, {0, 1, 0}}, axes, false);
    return {world, a, b};
}
void check(double parent, double child, double y, double dt, const p::PhysicsTransform& expected,
           const std::array<double, 3>& expected_velocity) {
    auto fixture = pair(parent, child, y);
    p::physics_world_step(fixture.world, dt);
    const auto actual = p::physics_body_get_transform(fixture.b);
    double position_error = 0, rotation_error = 0;
    for (int axis = 0; axis < 3; ++axis)
        position_error =
            std::max(position_error, std::abs(actual.position[axis] - expected.position[axis]));
    for (int axis = 0; axis < 4; ++axis)
        rotation_error =
            std::max(rotation_error, std::abs(actual.rotation[axis] - expected.rotation[axis]));
    ...
    if (child == 0) {
        ...
        for (int axis = 0; axis < 3; ++axis)
            assert(std::abs(velocity[axis] - expected_velocity[axis]) < .001);
    }
    std::cout << parent << ' ' << child << ' ' << y << ' ' << dt
              << " positionError=" << position_error << " rotationError=" << rotation_error << '\n';
}
int main() {
...
#include "radial-cases.inc"
    ...
    p::physics_world_step(base.world, 1. / 60);
    p::physics_world_step(rotated.world, 1. / 60);
    ...
    for (int step = 0; step < 120; ++step)
        p::physics_world_step(mixed.world, 1. / 60);
    const auto& joint =
        static_cast<btGeneric6DofSpring2Constraint&>(*mixed.world.ownership->hinges.front().joint);
    ...
    for (int step = 0; step < 120; ++step)
        p::physics_world_step(dynamic.world, 1. / 60);
    const auto sum = dynamic.a.ownership->body->getWorldTransform().getOrigin() +
                     dynamic.b.ownership->body->getWorldTransform().getOrigin();
    ...
    p::physics_world_add_body(mixed.world, mixed.b, false);
    p::physics_world_step(mixed.world, 1. / 60);
    assert(mixed.world.ownership->world->getNumConstraints() == 1);
    std::cout
        << "physics-distance: ok; mixed Cartesian/radial, rigid-frame invariance, momentum and membership\n";
}
