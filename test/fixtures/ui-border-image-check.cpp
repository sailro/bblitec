...
int main() {
    ...
    assert(window);
    {
        ...
        const auto frame_for = [&](const char* slices,
                                   const char* widths) -> const pal::UiRenderFrame& {
            ui_set_style_property(engine, box, "border-image",
                                  std::string("url(\"frame4px.png\") ") + slices + " / " + widths +
                                      " / 0 0 0 0 stretch");
        ...
        const auto quads = [](const pal::UiRenderFrame& frame) {
            ...
            for (const auto& draw : frame.draws) {
                if (draw.texture_id == 0)
                    continue;
                for (std::uint32_t i = draw.first_index; i < draw.first_index + draw.index_count;
                     i += 6) {
                    // MeshUtilities' quad triangles refer to four consecutive vertices.
                    const auto begin =
                        *std::min_element(frame.indices.begin() + i, frame.indices.begin() + i + 6);
                    result.push_back({frame.vertices[begin], frame.vertices[begin + 1],
                                      frame.vertices[begin + 2], frame.vertices[begin + 3]});
                }
            }
            return result;
        };
        ...
        assert((*texture.rgba)[0] == 120 && (*texture.rgba)[1] == 60 && (*texture.rgba)[2] == 30 &&
               (*texture.rgba)[3] == 128);
        ...
        expect_near(tiles.front()[0].x, 20);
        expect_near(tiles.front()[0].y, 30);
        float max_u = 0, max_v = 0;
        for (const auto& v : tiles.front()) {
            max_u = std::max(max_u, v.u);
            max_v = std::max(max_v, v.v);
        }
        expect_near(max_u, .25f);
        expect_near(max_v, .25f);
        for (const auto& v : first.vertices)
            assert(!(v.red == 255 && v.green == 0 && v.blue == 0));
        ...
        raw->SetProperty("width", "100px");
        raw->SetProperty("height", "80px");
        raw->SetProperty("border-width", "0px");
        raw->SetProperty("border-image",
                         "url(\"frame4px.png\") 3 3 3 3 / auto auto auto auto / 0 0 0 0 stretch");
        runtime.context->Update();
        tiles = quads(pal::record_ui_rml_frame(runtime, 640, 480));
        expect_near(tiles.front()[1].x - tiles.front()[0].x, 3);
        runtime.context->SetDensityIndependentPixelRatio(2);
        ...
        raw->SetProperty(
            "border-image",
            "url(\"frame4px.png\") 3 3 3 3 / 3e38dp 3e38dp 3e38dp 3e38dp / 0 0 0 0 stretch");
        runtime.context->Update();
        tiles = quads(pal::record_ui_rml_frame(runtime, 640, 480));
        assert(tiles.size() == 6);
