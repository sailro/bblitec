...
namespace bbl {
void flush_thin_instances(Engine& engine, MeshHandle mesh) { ++engine.meshes.at(mesh.value).instance_version; }
}
namespace p = bbl::pal;
...
int main() {
    ...
    assert(!world.events);
    const auto ordinary = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 0}, u::PhysicsMotionType::STATIC, false);
#if TEST_THIN
    std::vector<float> matrices(48);
    for (std::size_t i = 0; i < 3; ++i) u::thin_compose_matrix(matrices, static_cast<double>(i * 16), static_cast<double>(i * 10), 0, 0, 0, 0, 0, 1, 1, 1, 1);
...
#endif
    const auto target = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 1}, u::PhysicsMotionType::DYNAMIC, false);
    ...
    for (const auto& native : native_handles) assert(native.ownership->collision_events_enabled);
    ...
    for (const auto& native : native_handles) stream.push_back(p::PhysicsCollisionEvent{
        p::PhysicsCollisionEventType::started, {1, 2, 3}, {0, 1, 0}, 4,
        native.value, ordinary.handle.value, {1, 1.75, 3}});
    stream.push_back(p::PhysicsCollisionEvent{p::PhysicsCollisionEventType::started, {}, {}, 0, 0, ordinary.handle.value});
    ...
    u::on_physics_collision(handle, [&](const u::PhysicsCollisionInfo& info) {
        ...
        assert(info.collider_index == static_cast<double>(first) && info.collided_against_index == 0);
        assert(info.type == u::PhysicsCollisionType::STARTED && info.impulse == 4 && info.distance == -0.25);
        ...
        for (const auto& native : native_handles) assert(native.ownership->body && native.ownership->world == 0);
        assert(ordinary.handle.ownership->body);
    });
    u::on_physics_collision(handle, [&](const u::PhysicsCollisionInfo& info) {
        assert(info.collider == target && info.collided_against == ordinary);
        assert(info.collider_index == static_cast<double>(second++));
        assert(info.collider.handle.ownership->body && info.collided_against.handle.ownership->body);
    });
    ...
    const auto throwing = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 2}, u::PhysicsMotionType::STATIC, false);
    ...
    bool threw = false;
    try { u::physics_dispatch_after_step(world, 1.0 / 60); }
    catch (const std::runtime_error& error) { threw = std::string(error.what()) == "callback"; }
    assert(threw && !throwing.handle.ownership->body && !world.events->draining && !world.events->removed);
    world.after_step.clear();

    // Real Bullet contact records retain both native IDs, signed separation and FINISHED data.
    const auto ground = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 2}, u::PhysicsMotionType::STATIC, false);
    engine.meshes[3].position = {0, 1.5, 0};
    const auto ball = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 3}, u::PhysicsMotionType::DYNAMIC, false);
    ...
    assert((contact.collider_identity == ground.handle.value && contact.collided_against_identity == ball.handle.value) ||
           (contact.collider_identity == ball.handle.value && contact.collided_against_identity == ground.handle.value));
    double distance = 0;
    for (std::size_t i = 0; i < 3; ++i) distance += (contact.point_other[i] - contact.point[i]) * contact.normal[i];
