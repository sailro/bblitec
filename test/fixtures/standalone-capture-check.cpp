...
extern "C" SDL_GPURenderPass* SDLCALL SDL_BeginGPURenderPass(SDL_GPUCommandBuffer*,
    ...
    assert(count == 1);
    sdl_targets.push_back(*targets);
    return &sdl_pass;
}
extern "C" void SDLCALL SDL_EndGPURenderPass(SDL_GPURenderPass*) {}
extern "C" void SDLCALL SDL_BlitGPUTexture(SDL_GPUCommandBuffer*, const SDL_GPUBlitInfo* blit) {
...
extern "C" void SDLCALL SDL_ReleaseGPUTransferBuffer(SDL_GPUDevice*,
    ...
    assert(!buffer->mapped && !buffer->released);
...
extern "C" SDL_GPUCopyPass* SDLCALL SDL_BeginGPUCopyPass(SDL_GPUCommandBuffer*) {
    return &sdl_copy;
}
extern "C" void SDLCALL SDL_EndGPUCopyPass(SDL_GPUCopyPass*) {}
extern "C" void SDLCALL SDL_UploadToGPUBuffer(SDL_GPUCopyPass*,
                                              const SDL_GPUTransferBufferLocation* source,
                                              const SDL_GPUBufferRegion* destination, bool cycle) {
    assert(!source->transfer_buffer->mapped && !source->transfer_buffer->released &&
           source->offset % 4 == 0);
    if (cycle)
        std::fill(destination->buffer->bytes.begin(), destination->buffer->bytes.end(), 0);
...
extern "C" WGPUTexture wgpuDeviceCreateTexture(WGPUDevice,
    ...
    dawn_texture_descriptors.push_back(*descriptor);
    dawn_render_textures.emplace_back();
    return &dawn_render_textures.back();
}
extern "C" void wgpuSurfaceGetCurrentTexture(WGPUSurface, WGPUSurfaceTexture* target) {
    ...
    target->status = surface_available ? WGPUSurfaceGetCurrentTextureStatus_SuccessOptimal
...
wgpuCommandEncoderBeginRenderPass(WGPUCommandEncoder, const WGPURenderPassDescriptor* descriptor) {
    assert(descriptor->colorAttachmentCount == 1);
    dawn_targets.push_back(*descriptor->colorAttachments);
    return &dawn_pass;
}
extern "C" void wgpuRenderPassEncoderEnd(WGPURenderPassEncoder) {}
extern "C" WGPUCommandBuffer wgpuCommandEncoderFinish(WGPUCommandEncoder,
...
extern "C" void wgpuQueueSubmit(WGPUQueue, std::size_t count, const WGPUCommandBuffer*) {
    assert(count == 1);
    ++submissions;
}
extern "C" WGPUStatus wgpuSurfacePresent(WGPUSurface) {
    ++presentations;
    return WGPUStatus_Success;
}
extern "C" void wgpuTextureRelease(WGPUTexture texture) {
    dawn_released_textures.push_back(texture);
}
...
void record_effect_pass(SDL_GPUCommandBuffer*, SDL_GPURenderPass*, Engine&, Pass&,
                        EffectWrapperHandle) {}
void record_dawn_effect_pass(WGPURenderPassEncoder, const Pass&) {}
void record_sprite_pass(SDL_GPUCommandBuffer*, SDL_GPURenderPass*, Engine&, Pass&, std::uint32_t,
                        std::uint32_t) {}
...
Pass create_sprite_pass(SDL_GPUDevice*, Engine&, SpriteRendererHandle handle,
                        const std::vector<SDL_GPUTexture*>&, SDL_GPUTextureFormat) {
    ++pass_creations;
    return {handle};
}
Pass create_dawn_sprite_pass(WGPUDevice, WGPUQueue, int&, Engine&, SpriteRendererHandle handle,
                             const std::vector<WGPUTexture>&, const std::vector<WGPUTextureView>&,
                             WGPUTextureFormat) {
    ++pass_creations;
    return {handle};
}
...
void upload_sprite_pass(SDL_GPUDevice*, Engine&, Pass& pass, double,
...
void save_texture_png(SDL_GPUDevice*, SdlGpuCommand& command, SDL_GPUTexture* texture,
...
DawnSurfaceCapture begin_dawn_surface_capture(WGPUDevice, WGPUCommandEncoder, WGPUTexture texture,
                                              std::uint32_t, std::uint32_t) {
    assert(texture == &dawn_texture);
    ++readbacks;
    return {};
}
void finish_dawn_surface_capture(DawnDevice&, DawnSurfaceCapture&, std::uint32_t, std::uint32_t,
                                 const std::string&) {}
...
struct SdlContext : Context {
    ...
    std::unique_ptr<GpuBufferUploadBatch> buffer_uploads =
        std::make_unique<GpuBufferUploadBatch>(device);
};
...
void reset_observations() {
    ...
    dawn_targets.clear();
    blits.clear();
    readbacks = submissions = presentations = 0;
    readback_texture = nullptr;
}

template <typename Renderer> void configure(Renderer& renderer, bool capture) {
    renderer.frame_options.screenshot_frame = 1;
    if (capture)
        renderer.frame_options.screenshot_path = "capture.png";
...
void check_sdl() {
    for (const bool capture : {false, true})
        for (const unsigned samples : {1u, 4u}) {
            ...
            renderer.capture_run = renderer.captures.requested();
            ...
            for (long frame = 0; frame < 3; ++frame) {
                ...
                renderer.encode();
                renderer.present();
                assert(textures.size() - allocated ==
                       static_cast<std::size_t>(capture) + (samples > 1));
                ...
                assert(target.store_op ==
                       (samples > 1 ? SDL_GPU_STOREOP_RESOLVE : SDL_GPU_STOREOP_STORE));
                assert(blits.size() == static_cast<std::size_t>(capture));
                if (capture)
                    assert(blits[0].source.texture == renderer.resolve &&
                           blits[0].destination.texture == &swapchain_texture);
                assert(readbacks == static_cast<unsigned>(capture && frame == 1) &&
                       submissions == 1);
                if (readbacks)
                    assert(readback_texture == renderer.resolve);
            }
            if (renderer.color)
                SDL_ReleaseGPUTexture(nullptr, renderer.color);
            if (renderer.resolve)
                SDL_ReleaseGPUTexture(nullptr, renderer.resolve);
        }
    for (bool capture : {false, true}) {
        ...
        for (long frame = 0; frame < 3; ++frame) {
            ...
            for (const auto& target : sdl_targets)
                assert(target.texture == (capture ? renderer.color : &swapchain_texture));
            assert(blits.size() == static_cast<std::size_t>(capture));
            assert(readbacks == static_cast<unsigned>(capture && frame == 1) && submissions == 1);
            if (readbacks)
                assert(readback_texture == renderer.color);
        }
        ...
        surface_width += 1;
        assert(renderer.acquire());
        renderer.command.submit();
        assert((renderer.color != previous) == capture);
        if (capture)
            assert(previous->released && renderer.color->info.width == surface_width);
        surface_width -= 1;
        if (renderer.color)
            SDL_ReleaseGPUTexture(nullptr, renderer.color);
    }
...
template <typename Renderer> void check_dawn(bool effect) {
    for (bool capture : {false, true})
            ...
            for (bool ui : {false, true}) {
                ...
                for (long frame = 0; frame < 3; ++frame) {
                    ...
                    for (const auto& target : dawn_targets) {
                        assert(target.view ==
                               (effect && samples > 1 ? &dawn_msaa_view : &dawn_view));
                        assert(target.resolveTarget ==
                               (effect && samples > 1 ? &dawn_view : nullptr));
                    }
                    assert(readbacks == static_cast<unsigned>(capture && frame == 1));
...
template <typename Renderer> void check_registration() {
    Renderer renderer;
    ...
    if constexpr (std::is_same_v<Renderer, SdlSprite>)
        ...
        assert(!renderer.render_texture_views[0] && dawn_released_textures.back() == texture);
}
...
int main() {
    ...
    DawnEffect dawn_effect;
    DawnSprite dawn_sprite;
    assert(!sdl_effect.acquire() && !sdl_sprite.acquire() && !dawn_effect.acquire() &&
           !dawn_sprite.acquire());
}
