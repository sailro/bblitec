...
#include "heightfield-ground-cases.inc"
    ...
    for (double x : {-1.6, -0.4, 0.4, 1.6})
        for (double z : {-2.4, -0.8, 0.8, 2.4}) {
            const auto hit =
                p::physics_world_raycast(world, {x, 100, z}, {x, -100, z}, ~0u, ~0u, false);
            assert(hit.has_hit);
    ...
    for (double x : {-0.5, 0.5})
        for (double z : {-0.5, 0.5}) {
            const auto hit =
                p::physics_world_raycast(world, {10 + x, 10, z}, {10 + x, -10, z}, ~0u, ~0u, false);
    ...
    assert(refuses([&] {
        static_cast<void>(p::physics_shape_create_heightfield(3, 2, {1, 1, 1}, {0, 1, 2, 3, 4, 5}));
    }));
    assert(refuses([&] {
        static_cast<void>(
            p::physics_shape_create_heightfield(2, 2, {1, 1, 1}, {0, 1, 2, INFINITY}));
    }));
    assert(refuses([&] {
        static_cast<void>(p::physics_shape_create_heightfield(2, 2, {0, 1, 1}, {0, 1, 2, 3}));
    }));
    assert(
        refuses([&] { p::physics_body_set_motion_type(body, p::PhysicsMotionType::simulated); }));
    ...
    for (int step = 0; step < 180; ++step)
        p::physics_world_step(fall_world, 1.0 / 60);
    ...
    const auto gravity = [](p::PhysicsWorldHandle handle) {
        return handle.ownership->world->getGravity();
    };
