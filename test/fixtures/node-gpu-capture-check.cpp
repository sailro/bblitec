...
int main() {
    ...
    for (std::uint32_t view = 0; view < 3; ++view) {
        ...
        pipeline.attributes = {{"position", "float32x3", 2, 0, view ? 48u : 0u, 128},
                               {"normal", "float32x3", 3, 0, view ? 80u : 12u, 128}};
        capture.pipeline(pipeline);
        pipeline.attributes.clear();
        const auto uniform = capture.create_resource("mesh-ubo", 160);
        std::array<std::uint8_t, 4> bytes{};
        for (std::uint32_t lane = 0; lane < 4; ++lane)
            bytes[lane] = static_cast<std::uint8_t>(view * 4 + lane + 1);
        capture.write(uniform, 0, bytes);
        NodeGpuDrawCapture draw;
        draw.pipeline = 200 + view;
        draw.group = 100 + view;
        draw.vertices = vertices;
        draw.indices = indices;
        draw.mesh_uniform = uniform;
        draw.mesh = 12;
        draw.material = 79;
        draw.vertex_offset = 16;
        draw.index_offset = 4;
        draw.index_count = 6;
        draw.first_index = 2;
        draw.base_vertex = -3;
        draw.bindings = {{4, "albedo-view", 0x100000001ULL, 0x100000002ULL},
                         {5, "albedo-sampler", 0x100000003ULL, 0}};
        if (view == 2)
            draw.pushed_uniform_bytes = {21, 22, 23, 24};
        capture.draw(draw);
        draw.bindings.clear();
        bytes.fill(255);
    }
