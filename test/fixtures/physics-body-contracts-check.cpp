...
PhysicsBodyHandle make_body(PhysicsShapeHandle shape, PhysicsMotionType motion,
...
void check_mass_frame() {
    ...
    for (int x = -4; x <= 4; ++x)
        for (int y = -4; y <= 4; ++y)
            for (int z = -4; z <= 4; ++z)
                vertices.push_back({3 + .28 * x * .2 - .96 * y * .4,
                                    -2 + .96 * x * .2 + .28 * y * .4, 5 + z * .6});
    ...
    const btMatrix3x3 diagonal(static_cast<btScalar>(mass.inertia[0]), 0, 0, 0,
                               static_cast<btScalar>(mass.inertia[1]), 0, 0, 0,
                               static_cast<btScalar>(mass.inertia[2]));
    ...
    for (const PhysicsTransform pose :
         {PhysicsTransform{{10, 20, 30}}, PhysicsTransform{{-4, 7, 11}, {0, .6, 0, .8}}}) {
        ...
        for (const auto& vertex : corners) {
            ...
            for (int i = 0; i < hull->getNumPoints(); ++i)
                nearest =
                    std::min(nearest, (rigid.getWorldTransform() * hull->getUnscaledPoints()[i] -
                                       node * to_bt(vertex))
                                          .length());
            assert(nearest < 1e-4);
        }
...
void check_filters() {
    for (bool custom : {false, true}) {
        ...
        for (const auto& member : world_at(world).members) {
            ...
            assert(proxy->m_collisionFilterMask ==
                   (custom ? 8 : btBroadphaseProxy::AllFilter ^ btBroadphaseProxy::StaticFilter));
        }
...
void velocities(PhysicsBodyHandle body) {
    const auto linear = physics_body_get_linear_velocity(body),
               angular = physics_body_get_angular_velocity(body);
    std::cout << '[';
    for (std::size_t i = 0; i < 3; ++i)
        std::cout << (i ? "," : "") << linear[i];
    for (const double value : angular)
        std::cout << ',' << value;
    std::cout << ']';
}
...
int main() {
    ...
    std::cout << std::setprecision(17) << "{\"limits\":[" << limits.max_linear << ','
              << limits.max_angular << "],\"impulses\":[";
    const auto sphere = physics_shape_create_sphere({0, 0, 0}, .5);
