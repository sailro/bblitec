...
int main() {
    ...
    payload->instances = {pattern(192, 1), 3, 12, 192};
    ...
    plain.slot_count = 3;
    plain.live_count = 2;
    TextDrawGroup variant = plain;
    variant.group_key = "variant";
    variant.slot_start = 3;
    variant.slot_count = 2;
    TextDrawGroup empty = plain;
    empty.slot_count = 0;
    payload->groups = {plain, variant, empty};
    payload->version = 1;
    payload->style_version = 1;
    ...
    auto record = [&] {
        ...
        ops.event("state", gpu->instance_capacity, gpu->style_buffer_bytes,
                  ...
                  a.uploaded_version, Ops::id(data->groups.at(0).bind_group),
                  data->groups.at(0).bind_group_version);
    };
#include "actions.hpp"
    return 0;
}
