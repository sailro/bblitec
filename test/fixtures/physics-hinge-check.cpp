...
#include "constraint-axis-cases.inc"
    ...
    const auto pivot_error = [&]() {
        ...
        maximum_pivot_error = std::max(
            maximum_pivot_error, static_cast<double>((aw.getOrigin() - bw.getOrigin()).length()));
    ...
    a = p::physics_body_create();
    b = p::physics_body_create();
    p::physics_body_set_shape(a, shape);
    p::physics_body_set_shape(b, shape);
    ...
    try {
        ...
        same_body_refused = true;
    }
    assert(same_body_refused);
    const auto different_world = p::physics_world_create();
    bool foreign_body_refused = false;
    try {
        p::physics_world_create_hinge(different_world, a, b, anchor, anchor, false);
    } catch (const std::runtime_error&) {
        foreign_body_refused = true;
    }
