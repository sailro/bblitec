...
void print_pose(PhysicsBodyHandle body) {
    ...
    values(pose.rotation);
    values(physics_body_get_linear_velocity(body));
    values(physics_body_get_angular_velocity(body));
    std::cout << ']';
}
...
int main() {
    ...
    std::cout << "],\"boxQueries\":[";
    first = true;
    for (const double size : {.2, 1.0, 2.0, 10.0})
        for (const double y : {0.0, .3, .6, 1.1}) {
            const auto world = physics_world_create();
            const auto body = physics_body_create();
            ...
            for (const bool cast : {false, true}) {
                ...
                const auto hits =
                    cast ? physics_world_collect_shape_cast(world, capsule, {0, 0, 0, 1}, from,
                                                            {size / 2 - .15, y, 0}, false, {}, 8)
                         : physics_world_collect_shape_proximity(
                               world, capsule, {from, {0, 0, 0, 1}}, .2, false, {}, 8);
                if (!first)
                    std::cout << ',';
                first = false;
                std::cout << '[';
                ...
                for (const auto& hit : hits) {
                    ...
                    for (const auto& vector :
                         {hit.input_point, hit.point, hit.input_normal, hit.normal})
