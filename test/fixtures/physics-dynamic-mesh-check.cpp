...
int main() {
    ...
    auto mesh = physics_shape_create_mesh({{-0.5, -0.5, -0.5},
                                           {0.5, -0.5, -0.5},
                                           {0.5, 0.5, -0.5},
                                           {-0.5, 0.5, -0.5},
                                           {-0.5, -0.5, 0.5},
    ...
    for (const double inertia : mass.inertia)
        assert(std::isfinite(inertia) && inertia > 0);
    ...
    physics_body_set_shape(floor,
                           physics_shape_create_box({0, -0.5, 0}, {0, 0, 0, 1}, {20, 1, 20}));
    ...
    for (int i = 0; i < 3; ++i)
        assert(std::abs(before.position[i] - after.position[i]) < 0.00001);
    physics_world_step(world, 1.0 / 60);
