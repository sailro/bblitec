...
bblscene::WorldJson sample_world() {
    ...
    world.parts.push_back(
        bbl::js::make_ref<bblscene::WorldPartDataData>(bblscene::WorldPartDataData{
            bbl::js::Tuple<3>{1.0, 2.0, 3.0}, bbl::js::Tuple<3>{4.0, 5.0, 6.0},
            bbl::js::Tuple<4>{0.0, 0.0, 0.0, 1.0}, bbl::js::Tuple<3>{1.0, 1.0, 1.0},
            bbl::js::Nullable<double>{}}));
    world.parts.push_back(
        bbl::js::make_ref<bblscene::WorldPartDataData>(bblscene::WorldPartDataData{
            bbl::js::Tuple<3>{2.0, 2.0, 2.0}, bbl::js::Tuple<3>{0.0, 1.5, 0.0},
            bbl::js::Tuple<4>{0.0, 0.0, 0.0, 1.0}, bbl::js::Tuple<3>{0.5, 0.25, 0.0},
...
void check_stringify() {
    ...
    check_equal(bbl::js::json_stringify(world),
                "{\"version\":1,\"parts\":["
    ...
    single.parts.push_back(
        bbl::js::make_ref<bblscene::WorldPartDataData>(bblscene::WorldPartDataData{
            bbl::js::Tuple<3>{1.0, 1.0, 1.0}, bbl::js::Tuple<3>{0.0, 0.0, 0.0},
            bbl::js::Tuple<4>{0.0, 0.0, 0.0, 1.0}, bbl::js::Tuple<3>{1.0, 0.0, 0.0},
            bbl::js::Nullable<double>{}}));
    check_equal(bbl::js::json_stringify(single, 2),
                "{\n"
                "  \"version\": 1,\n"
                "  \"parts\": [\n"
                "    {\n"
    ...
    check_equal(bbl::js::json_stringify(std::string("a\"b\\c\nd\te\x01")),
                "\"a\\\"b\\\\c\\nd\\te\\u0001\"", "string escaping");
    check_equal(bbl::js::json_stringify(std::numeric_limits<double>::quiet_NaN()), "null",
                "NaN writes as null");
    check_equal(bbl::js::json_stringify(std::numeric_limits<double>::infinity()), "null",
                "Infinity writes as null");
    check_equal(bbl::js::json_stringify(true), "true", "boolean");
    check_equal(bbl::js::json_stringify(bbl::js::Array<double>{}), "[]", "empty array");
    check_equal(bbl::js::json_stringify(bbl::js::Array<double>{}, 2), "[]",
    ...
    check_equal(bbl::js::json_stringify(holes), "[null,4]",
                "an absent array slot is null, as JavaScript writes undefined");
    bbl::js::Map<std::string, double> record;
    record.set("z", 1.0);
    record.set("a", 2.0);
    check_equal(bbl::js::json_stringify(record), "{\"z\":1,\"a\":2}",
...
void check_parse() {
    ...
    check(!document.get("version").strict_equals(std::string_view("1")),
    ...
    check(document.get("missing").is_undefined(), "a missing property reads undefined");
    check(!document.get("missing").truthy(), "a missing property is falsy");
    check(document.get("missing").type_of() == "undefined", "typeof a missing property");
    check(document.get("parts").at(0.0).get("s").at(1.0).strict_equals(2.0), "nested index");
    check(document.get("parts").at(9.0).is_undefined(), "an index past the end reads undefined");
    check(document.get("parts").at(0.0).get("s").every(
              [](const bbl::js::JsonValue& element) { return element.is_number(); }),
          "every over an array of numbers");
    check(bbl::js::json_parse("null").is_null(), "a null document");
    check(!bbl::js::json_parse("null").truthy(), "a null document is falsy");
    check(bbl::js::json_parse("\"\"").type_of() == "string" &&
              !bbl::js::json_parse("\"\"").truthy(),
          "an empty string is falsy");
    check(bbl::js::json_parse("0").type_of() == "number" && !bbl::js::json_parse("0").truthy(),
          "zero is falsy");
    // A wrong-shaped document fails its guards rather than the program.
    const bbl::js::JsonValue wrong = bbl::js::json_parse("{\"parts\":7}");
    check(!wrong.get("parts").is_array(), "a wrong-typed field is not an array");
    check(!wrong.get("version").strict_equals(1.0), "a missing version fails the version guard");
    check(std::isnan(wrong.get("version").to_number()), "a missing number coerces to NaN");
    bool threw = false;
    try {
...
void check_storage() {
    ...
    check(!bbl::pal::read_local_storage(key).has_value(), "an unset key reads as absent");
    ...
    check_equal(stored.value_or(""), "{\"version\":1}", "the stored value round-trips");
    bbl::pal::write_local_storage(key, "");
    const auto empty = bbl::pal::read_local_storage(key);
    check(empty.has_value() && empty->empty(), "an empty value is present, not absent");
    const auto js_empty = bbl::js::local_storage_get_item(key);
    check(js_empty.has_value() && js_empty.value().empty(),
          "the JavaScript shape carries the empty value too");
    bbl::pal::remove_local_storage(key);
    check(!bbl::js::local_storage_get_item(key).has_value(), "a removed key reads as absent");
