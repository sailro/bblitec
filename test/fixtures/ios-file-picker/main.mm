...
- (void)performPending {
    ...
    if (operation == FileOperation::None) return;
    try {
        if (operation == FileOperation::Save) {
            const bbl::pal::FileDialogOptions options{
                "Export fixture", "bblite-fixture.json", "JSON", "*.json"};
            const bool saved = bbl::pal::save_file(engine, options, std::string_view("{\"value\":\"UIKit document export\"}\n"));
            ...
            if (!selected) self.status.text = @"cancelled";
            else {
        ...
        self.status.text = [@"error: " stringByAppendingString:[NSString stringWithUTF8String:error.what()]];
    }
}
@end
...
int main(int, char**) {
    ...
    for (NSUInteger index = 0; index < names.count; ++index) {
        ...
        [button addTarget:actions action:index == 0 ? @selector(save:) : @selector(open:) forControlEvents:UIControlEventTouchUpInside];
