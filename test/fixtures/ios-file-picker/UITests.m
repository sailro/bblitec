...
- (void)waitForPicker:(XCUIApplication*)app {
    XCTAssertTrue([app.navigationBars.firstMatch waitForExistenceWithTimeout:60], @"%@",
                  app.debugDescription);
    NSString* documents =
        NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).firstObject;
    XCTAssertTrue([app.screenshot.PNGRepresentation
        writeToFile:[documents stringByAppendingPathComponent:@"picker.png"]
         atomically:YES]);
}
- (void)waitForStatus:(NSString*)value application:(XCUIApplication*)app {
    ...
    [self expectationForPredicate:predicate
              evaluatedWithObject:app.staticTexts[@"file-status"]
                          handler:nil];
...
- (XCUIElement*)entry:(NSString*)name application:(XCUIApplication*)app {
    ...
    if ([cell waitForExistenceWithTimeout:3])
        return cell;
...
- (void)localFolder:(XCUIApplication*)app {
    ...
    XCTAssertTrue([app.buttons[@"Files, Actions Menu"] waitForExistenceWithTimeout:15], @"%@",
                  app.debugDescription);
}
...
- (void)testCancellation {
    ...
    XCUIApplication* app =
        [[XCUIApplication alloc] initWithBundleIdentifier:@"org.bblite.filetest"];
    [app launch];
...
- (void)testExportImport {
    ...
    [save tap];
    if ([app.buttons[@"Replace"] waitForExistenceWithTimeout:2])
