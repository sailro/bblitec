...
extern "C" SDL_GPURenderPass* SDLCALL
SDL_BeginGPURenderPass(SDL_GPUCommandBuffer*, const SDL_GPUColorTargetInfo* colors, Uint32 count,
                       const SDL_GPUDepthStencilTargetInfo* depth) {
    assert(count == 1 && !depth && colors->store_op == SDL_GPU_STOREOP_STORE);
    attachments.push_back(
        {colors->texture->id, colors->load_op == SDL_GPU_LOADOP_CLEAR, colors->clear_color.r});
    return &sdl_pass;
}
extern "C" void SDLCALL SDL_EndGPURenderPass(SDL_GPURenderPass* pass) {
    assert(pass == &sdl_pass);
    draws.push_back("end");
}
...
extern "C" void wgpuRenderPassEncoderEnd(WGPURenderPassEncoder pass) {
    assert(pass == &dawn_pass);
    draws.push_back("end");
}
extern "C" void wgpuRenderPassEncoderRelease(WGPURenderPassEncoder pass) {
    assert(pass == &dawn_pass);
}
...
extern "C" void wgpuRenderPassEncoderSetVertexBuffer(WGPURenderPassEncoder, uint32_t, WGPUBuffer,
                                                     uint64_t, uint64_t) {}
...
extern "C" void wgpuRenderPassEncoderDrawIndexed(WGPURenderPassEncoder, uint32_t count,
                                                 uint32_t instances, uint32_t, int32_t, uint32_t) {
...
void record_sprite_pass(SDL_GPUCommandBuffer*, SDL_GPURenderPass*, Engine&, int pass, unsigned,
                        unsigned) {
...
struct Stages {
    ...
    struct Plan {
        std::vector<upstream::RenderStage> stages{
...
int main() {
    ...
    SdlStages sdl;
    sdl.state.msaa_color = &msaa;
    sdl.state.color = &resolved;
    sdl.swapchain = &swapchain;
    for (bool sampled : {false, true})
        for (bool transmission : {false, true})
            for (bool captured : {false, true})
                for (bool overlay : {false, true}) {
                    sdl.state.sample_count =
                        sampled ? SDL_GPU_SAMPLECOUNT_4 : SDL_GPU_SAMPLECOUNT_1;
                    sdl.transmission_enabled = transmission;
                    sdl.capture_frame = captured;
    ...
    sdl.visible_color = &swapchain;
    sdl.sprite_render_textures = {&offscreen};
    dawn.surface_view = &surface;
    dawn.state.sprite_render_texture_views = {&target};
    ...
    const auto setup = [](Engine& engine) {
        ...
        engine.sprite_renderers[0].has_target = true;
        engine.sprite_renderers[0].target = SpriteRenderTextureHandle{0};
    ...
    for (bool is_dawn : {false, true}) {
        ...
        assert(attachments.size() == 3 && attachments[0].id == 3 && attachments[1].id == 4 &&
               attachments[2].id == 3);
        assert(!attachments[0].clear && attachments[1].clear && !attachments[2].clear &&
               attachments[1].red == .25);
    }
    assert(dawn.capture_source == &screenshot);
    for (bool scene_stages : {false, true})
        for (bool ground : {false, true}) {
            ...
            assert(draws == expected);
            if (scene_stages)
                expected.insert(expected.begin(), "skybox");
            for (bool depth : {false, true})
                for (const auto samples : {SDL_GPU_SAMPLECOUNT_1, SDL_GPU_SAMPLECOUNT_4}) {
