...
void check_transform(const std::vector<float>& matrices, const p::PhysicsTransform& expected) {
    ...
    for (std::size_t i = 0; i < 3; ++i) assert(std::abs(transform.position[i] - expected.position[i]) < 1e-13);
    for (std::size_t i = 0; i < 4; ++i) assert(std::abs(transform.rotation[i] - expected.rotation[i]) < 1e-13);
}
int main() {
    using namespace bbl;
    ...
    for (std::size_t i = 0; i < 3; ++i) u::thin_compose_matrix(matrices, static_cast<double>(i * 16), static_cast<double>(i * 10), 10, 0, 0, 0, 0, 1, 1, 1, 1);
    ...
    const auto ordinary = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 1}, u::PhysicsMotionType::STATIC, false);
    assert(u::get_physics_body_instance_count(ordinary) == 1);
    u::enable_havok_thin_instance_physics(handle);
    u::enable_havok_thin_instance_physics(handle);
    const auto body = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 0}, u::PhysicsMotionType::DYNAMIC, false);
    ...
    for (const auto& native : state->handles) assert(p::physics_body_get_mass_properties(native).mass == 2);
    u::apply_physics_impulse(handle, body, {2,0,0}, {});
    for (const auto& native : state->handles) assert(std::abs(p::physics_body_get_linear_velocity(native)[0] - 1) < 1e-6);
    ...
    for (std::size_t i = 0; i < 3; ++i) {
        ...
        assert(hit.has_hit && hit.body && *hit.body == body && hit.body_index == static_cast<double>(i));
    }
    ...
    for (const auto& native : state->handles) native_lifetimes.push_back(native.ownership);
    ...
    const auto refuses = [](auto action) { try { action(); } catch (const std::runtime_error&) { return true; } return false; };
    carrier.instance_count = 0;
    assert(refuses([&] { static_cast<void>(u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 0}, u::PhysicsMotionType::DYNAMIC, false)); }));
    assert(refuses([&] { static_cast<void>(u::create_physics_aggregate(handle, {0}, u::PhysicsShapeType::BOX, {})); }));
    carrier.instance_count = 3;
    {
        namespace c = bbl::character;
        const auto instances = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 0}, u::PhysicsMotionType::DYNAMIC, false);
        u::set_physics_body_shape(handle, instances, shape);
        u::set_physics_body_mass(handle, instances, 2);
        const auto control = u::create_physics_body(handle, {u::PhysicsNodeKind::mesh, 1}, u::PhysicsMotionType::STATIC, false);
        c::PhysicsCharacterController controller(engine);
        controller._world = js::make_ref<c::PhysicsWorld>(); controller._world->value = handle;
        ...
        for (std::size_t i = 1; i < handles.size(); ++i) assert(p::physics_body_get_linear_velocity(handles[i])[0] == 0);
        const auto first_identity = alias.weak_identity();
        const auto second_identity = std::get<1>(*second).weak_identity();
        alias = {}; first.reset(); second.reset();
        controller.dispose();
