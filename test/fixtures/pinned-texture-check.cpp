    ...
    for (unsigned alpha = 0; alpha < 256; ++alpha) {
        for (unsigned color = 0; color < 256; ++color) {
            const std::uint8_t rgba[] = {
                static_cast<std::uint8_t>(color), static_cast<std::uint8_t>(255 - color),
                static_cast<std::uint8_t>(color / 2), static_cast<std::uint8_t>(alpha)};
            ...
            for (unsigned lane = 0; lane < 3; ++lane) {
                ...
                if (std::bit_cast<std::uint32_t>(pixel[lane]) !=
                    std::bit_cast<std::uint32_t>(baseline))
                    throw std::runtime_error("RGBD f32 result changed before half packing");
            }
            if (pixel[3] != 1.0f)
                throw std::runtime_error("RGBD opacity changed");
        }
    }
    for (unsigned width = 1; width <= 8192; width = width * 2 + 1) {
        for (unsigned height = 1; height <= 8192; height = height * 2 + 1) {
            ...
            if (bbl::upstream::mip_level_count(width, height) != full ||
                bbl::upstream::transmission_mip_level_count(width, height) !=
                    std::max(1.0, full - 4))
