...
int main() {
    ...
    auto body =
        create_physics_body(world, physics_node(MeshHandle{0}), PhysicsMotionType::STATIC, false);
    auto viewer = create_physics_viewer(scene, world, 7);
    assert(!show_physics_body(viewer, body, 3)); // No assigned shape.
    PhysicsShape shape;
    shape.handle = pal::physics_shape_create_sphere({0, 0, 0}, 1);
