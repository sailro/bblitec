...
static SDL_GPUTexture* texture(SDL_GPUDevice* device, SDL_GPUTextureFormat format,
                               SDL_GPUTextureUsageFlags usage, Uint32 width) {
...
static SDL_GPUShader* shader(SDL_GPUDevice* device, const std::filesystem::path& path,
...
static void download_texture(SDL_GPUCopyPass* copy, SDL_GPUTexture* image,
                             SDL_GPUTransferBuffer* target, Uint32 width, Uint32 stride) {
    ...
    destination.transfer_buffer = target;
    destination.pixels_per_row = stride;
...
int main(int argc, char** argv) {
    ...
    for (int stage = 0; stage < (phase == "mixed" ? 3 : 1); ++stage) {
        if (phase != "compute" && (phase != "mixed" || stage != 1)) {
            ...
            auto* fragment =
                shader(device, directory / "fragment.dxil", SDL_GPU_SHADERSTAGE_FRAGMENT, 2);
            ...
            assert(copy);
            download_texture(copy, image, result, 1, 64);
            SDL_EndGPUCopyPass(copy);
            checks.push_back([=] {
                const auto* rgba = static_cast<const unsigned char*>(
                    SDL_MapGPUTransferBuffer(device, result, false));
                assert(rgba && rgba[0] == 85 && rgba[1] == 85 && rgba[2] == 85 && rgba[3] == 255);
                SDL_UnmapGPUTransferBuffer(device, result);
                SDL_ReleaseGPUTransferBuffer(device, result);
            ...
            info.threadcount_x = 1;
            info.threadcount_y = 1;
            info.threadcount_z = 1;
            auto* pipeline = SDL_CreateGPUComputePipeline(device, &info);
            assert(pipeline);
            auto* image = texture(device, SDL_GPU_TEXTUREFORMAT_R32G32B32A32_FLOAT,
                                  SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_WRITE, operations);
            auto* unused_image = texture(device, SDL_GPU_TEXTUREFORMAT_R32G32B32A32_FLOAT,
                                         SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_WRITE, 1);
            SDL_GPUBufferCreateInfo buffer_info{};
            buffer_info.usage = SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE;
            buffer_info.size = operations * 16;
            ...
            image_bindings[0].texture = image;
            image_bindings[1].texture = unused_image;
            std::array<SDL_GPUStorageBufferReadWriteBinding, 2> buffer_bindings{};
            buffer_bindings[0].buffer = buffer;
            buffer_bindings[1].buffer = unused_buffer;
            auto* pass = SDL_BeginGPUComputePass(command, image_bindings.data(), 2,
                                                 buffer_bindings.data(), 2);
            assert(pass);
            ...
            for (Uint32 i = 0; i < operations; ++i) {
                const std::array<SDL_GPUTextureSamplerBinding, 3> bindings{
                    {{inputs[i % 3], sampler},
                     {inputs[(i + 1) % 3], sampler},
                     {inputs[(i + 2) % 3], sampler}}};
            ...
            auto* copy = SDL_BeginGPUCopyPass(command);
            assert(copy);
            download_texture(copy, image, image_result, operations, 1104);
            ...
            checks.push_back([=] {
                for (auto* result : {image_result, buffer_result}) {
                    const auto* values =
                        static_cast<const float*>(SDL_MapGPUTransferBuffer(device, result, false));
                    assert(values);
                    for (Uint32 i = 0; i < operations * 4; ++i) {
                        const float expected = i % 4 == 3   ? 1.0f
                                               : i % 4 == 0 ? 2.0f / 3.0f
                                                            : 1.0f / 6.0f;
                ...
                SDL_ReleaseGPUTexture(device, unused_image);
                SDL_ReleaseGPUBuffer(device, unused_buffer);
            });
        }
    }
    wait(device, command);
    ...
    for (auto* input : inputs)
        SDL_ReleaseGPUTexture(device, input);
