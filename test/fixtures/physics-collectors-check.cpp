...
int main() {
    ...
    const auto floor = physics_shape_create_mesh({{-5, 0, -5}, {5, 0, -5}, {5, 0, 5}, {-5, 0, 5}},
                                                 {0, 2, 1, 0, 3, 2});
    std::vector<PhysicsBodyHandle> bodies;
    for (const auto& [shape, transform] :
         std::vector<std::pair<PhysicsShapeHandle, PhysicsTransform>>{
             {floor, {}},
             {box, {{.75, 1, 0}, {0, 0, 0, 1}}},
             {box, {{-.75, 1, 0}, {0, 0, 0, 1}}},
             {capsule, {{0, .9, 0}, {0, 0, 0, 1}}}}) {
    ...
    const auto proximity =
        physics_world_collect_shape_proximity(world, capsule, pose, .15, false, bodies[3], 16);
    const auto cast = physics_world_collect_shape_cast(world, capsule, pose.rotation, pose.position,
                                                       {2, .9, 0}, false, bodies[3], 16);
    assert(proximity.size() >= 3);
    assert(!cast.empty());
    assert(physics_world_collect_shape_proximity(world, capsule, pose, .15, false, bodies[3], 1)
               .size() == 1);
    assert(physics_world_collect_shape_cast(world, capsule, pose.rotation, pose.position,
                                            {2, .9, 0}, false, bodies[3], 1)
               .size() == 1);
    std::cout << "{\"queries\":[";
    ...
    for (const auto& hits : {proximity, cast}) {
        ...
        for (const auto& hit : hits) {
            ...
            auto body = std::find_if(bodies.begin(), bodies.end(), [&](const auto& value) {
                return value.value == hit.body_identity;
            });
            assert(body != bodies.end());
            if (!first_hit)
                std::cout << ',';
            first_hit = false;
            std::cout << '[' << std::distance(bodies.begin(), body) << ','
                      << hit.distance_or_fraction;
            for (const auto& vector : {hit.input_point, hit.point, hit.input_normal, hit.normal})
    ...
    for (const auto& shape : {capsule, physics_shape_create_box({0, 0, 0}, {0, 0, 0, 1}, {2, 3, 4}),
    ...
    try {
        static_cast<void>(physics_shape_default_mass(
            physics_shape_create_box({0, 0, 0}, {0, 0, 0, 1}, {2, 0, 4})));
    } catch (const std::runtime_error&) {
        flat_default_rejected = true;
    }
    assert(flat_default_rejected);
    physics_shape_set_trigger(box, true);
    const auto no_triggers =
        physics_world_collect_shape_proximity(world, capsule, pose, .15, false, bodies[3], 16);
    assert(std::all_of(no_triggers.begin(), no_triggers.end(),
                       [&](const auto& hit) { return hit.body_identity == bodies[0].value; }));
    assert(physics_world_collect_shape_proximity(world, capsule, pose, .15, true, bodies[3], 16)
               .size() == proximity.size());
    ...
    try {
        static_cast<void>(physics_body_get_mass_properties(bodies[3]));
    } catch (const std::runtime_error&) {
        stale_rejected = true;
    }
