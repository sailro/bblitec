...
int main() {
    ...
    assert(window);
    {
        ...
        ui_set_attribute(engine, image, "style",
                         "position:absolute;left:10px;top:20px;width:100px;height:100px;");
        ui_append_to_root(engine, image);
        pal::UiRmlRuntime runtime(engine, window, 640, 480);
        const auto expect_quad = [&](float left, float top, float right, float bottom, float u0 = 0,
            ...
            for (auto i = image_draw->first_index;
                 i < image_draw->first_index + image_draw->index_count; ++i) {
            ...
            expect_near(min_x, left);
            expect_near(min_y, top);
            expect_near(max_x, right);
            expect_near(max_y, bottom);
            expect_near(min_u, u0);
            expect_near(min_v, v0);
            expect_near(max_u, u1);
            expect_near(max_v, v1);
        };
        ...
        refused = false;
        try {
            pal::record_ui_rml_frame(runtime, 640, 480);
        } catch (const std::runtime_error&) {
            refused = true;
        }
        assert(refused);
