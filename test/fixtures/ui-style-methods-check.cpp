...
int main() {
    ...
    assert(window && generated_main() == 0);
    {
        ...
        assert(runtime.projected_elements.at(panel.value)
                   .element->GetProperty<Rml::String>("--span") == "5dp");
        ...
        ui_set_attribute(engine, panel, "style",
                         "--Case:first;--Case:second;--case:lower;height:10px;height:30px");
        ...
        pal::update_ui_rml_runtime(runtime, 640, 480);
        const auto projected_text =
            runtime.projected_elements.at(panel.value).element->GetProperty<Rml::String>("--Text");
