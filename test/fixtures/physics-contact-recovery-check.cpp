            ...
            for (double dt : {1.0 / 60, 1.0 / 120, 1.0 / 240}) {
                for (double depth : {0.02, 0.04, 0.1, 0.3, 0.5}) {
                    ...
                    for (int i = 0; i < 2; ++i) {
                        ...
                        physics_body_set_motion_type(body,
                                                     i ? PhysicsMotionType::simulated
                                                       : static_cast<PhysicsMotionType>(motion));
                        if (i || motion == 2)
                            physics_body_set_mass_properties(
                                body, physics_shape_build_mass_properties(shape, mass));
