...
int main() try {
    ...
    check(window != nullptr, "Window creation");
    {
        ...
        ui_set_attribute(
            engine, panel, "style",
            "width:100px;height:30px;border-top:2px #ff0000;border-right:3px #0000ff;border-bottom:4px #00ff00;border-left:5px #112233;font-style:italic;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transform-origin:left bottom;");
        ...
        check(box.GetEdge(Rml::BoxArea::Border, Rml::BoxEdge::Top) == 2.f &&
                  ...
                  box.GetEdge(Rml::BoxArea::Border, Rml::BoxEdge::Bottom) == 4.f &&
                  box.GetEdge(Rml::BoxArea::Border, Rml::BoxEdge::Left) == 5.f,
              "independent border edges");
        check(raw->GetComputedValues().font_style() == Rml::Style::FontStyle::Italic &&
                  raw->GetComputedValues().text_transform() ==
                      Rml::Style::TextTransform::Uppercase &&
                  raw->GetComputedValues().text_overflow() == Rml::Style::TextOverflow::Ellipsis,
              "text presentation");
        ...
        ui_set_style_property(engine, panel, "text-overflow", "clip");
        update();
        check(raw->GetBox().GetEdge(Rml::BoxArea::Border, Rml::BoxEdge::Right) == 6.f &&
                  raw->GetBox().GetEdge(Rml::BoxArea::Border, Rml::BoxEdge::Left) == 8.f,
              "border width replacement");
        ...
        check(raw->GetBox().GetEdge(Rml::BoxArea::Border, Rml::BoxEdge::Right) == 0.f,
              "border shorthand removal clears its width longhands");
    }
    SDL_DestroyWindow(window);
    SDL_Quit();
    return 0;
} catch (const std::exception& error) {
    std::fprintf(stderr, "UI presentation failure: %s\n", error.what());
    return 1;
}
