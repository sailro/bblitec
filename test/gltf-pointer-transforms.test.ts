...
function runFixture(name: string, source: string): void {
    ...
    assert.ok(
        execFileSync(executable, { encoding: "utf8" }).includes(`${name}: ok`),
    );
}
...
for (const layer of [
    ...
    test(
        `the emitted ${layer.name} writer reads every live texture transform`,
        {
            skip: !tools,
        },
        async () => {
            ...
            const material = {
                [layer.property]: {
                    isEnabled: true,
                    ...Object.fromEntries(
                        layer.textureKeys.map((key, i) => [key, transforms[i]]),
                    ),
                },
            };
            ...
            const structEnd = header.indexOf(
                "inline void write_PbrTest_material",
                start,
            );
            const writerStart = header.indexOf(
                `inline void write_PbrTest_${layer.baseField}`,
                structEnd,
            );
            ...
            assert.ok(
                start >= 0 &&
                    structEnd > start &&
                    writerStart > structEnd &&
                    writerEnd > writerStart,
            );
            const writers = await importPinnedModule<
                Record<
                    string,
                    (
                        data: Float32Array,
            ...
            runFixture(
                `gltf-${layer.name}-transforms-check`,
...
int main() {
    ...
    ${transforms
        ...
        .map((texture, index) =>
            Object.entries({
                u_scale: texture.uScale,
                v_scale: texture.vScale,
                rotation: texture.uAng,
                u_offset: texture.uOffset,
                v_offset: texture.vOffset,
            })
                .map(
                    ([property, value]) =>
                        `material.${layer.fields[index]}_transform.${property} = static_cast<float>(${value});`,
                )
                .join("\n    "),
        )
        .join("\n    ")}
    ...
    ${layer.uvFields
        .flatMap((field) => [field + "m", field + "t"])
        .map((field) => {
            const offset = spec._offsets.get(field)! / 4;
            return [0, 1, 2, 3]
                .map(
                    (lane) =>
                        `assert(std::abs(actual.${field}[${lane}] - static_cast<float>(${expected[offset + lane]})) < 0.000001f);`,
                )
                .join("\n    ");
        })
        .join("\n    ")}
...
test("source pointer transforms resolve shared occlusion and extension owners", async () => {
    const { resolveAnimationPointer } = await importPinnedModule<{
        resolveAnimationPointer: (
            pointer: string,
            context: { materials: object[] },
        ) => {
    ...
    for (const independent of [false, true]) {
        ...
        const mat = {
            ...
            ...(independent
                ? { occlusionTexture: { uOffset: 21, vOffset: 22 } }
                : {}),
        };
        ...
        assert.equal(
            mat.occlusionTexture?.uOffset,
            independent ? 0.25 : undefined,
        );
        assert.equal(
            sharedImage.uOffset,
            11,
            "the pointer owns a private transform wrapper",
        );
    }
    ...
    for (const [path, owners] of [
        ...
        [
            ...
            ["_subsurface", "translucency", "colorTexture"],
        ],
        [
            "KHR_materials_diffuse_transmission/diffuseTransmissionTexture",
            ["_subsurface", "translucency", "intensityTexture"],
        ],
        [
        ...
        const sharedTexture = { uOffset: 11, vOffset: 12 };
        owner[key] = sharedTexture;
        const target = resolveAnimationPointer(
            `/materials/0/extensions/${path}/extensions/KHR_texture_transform/offset`,
            { materials: [mat] },
        );
        assert.ok(target);
        target.writer(new Float32Array([0.25, 0.75]), 0);
        assert.equal(sharedTexture.uOffset, 11);
        assert.deepEqual(owner[key], {
            uOffset: 0.25,
            vOffset: 0.75,
            _animPriv: true,
        });
    ...
    for (const path of [
        ...
        "extensions/KHR_materials_unknown/unknownTexture",
    ])
        assert.equal(
            resolveAnimationPointer(
                `/materials/0/${path}/extensions/KHR_texture_transform/offset`,
                { materials: [{}] },
            ),
            null,
        );
