...
test("a method outside the producer's contract refuses naming both", () => {
    ...
    const queue = (device as unknown as { queue: { writeTexture: unknown } })
        .queue;
    assert.throws(
        () => queue.writeTexture,
...
test("compute recording preserves uploads and per-dispatch bindings when scratch textures are reused", () => {
    ...
    const descriptor = {
        layout: "auto",
        compute: { module: shader, entryPoint: "main", constants },
    };
    ...
    const commands = encoder as {
        ...
        copyTextureToTexture(
            source: { texture: RecordedTexture },
            destination: { texture: RecordedTexture; origin: { z: number } },
            size: number[],
        ): void;
    };
    const queue = (
        device as unknown as {
            queue: {
                copyExternalImageToTexture(
                    source: unknown,
                    destination: { texture: RecordedTexture },
                    size: number[],
                ): void;
            };
        }
    ...
    for (let face = 0; face < images.length; face++) {
        queue.copyExternalImageToTexture(
            { source: images[face], flipY: false },
            { texture: input },
            [16, 16],
        );
        ...
        commands.copyTextureToTexture(
            { texture: output },
            { texture: cube, origin: { z: face } },
            [16, 16],
        );
    }
    assert.deepEqual(
        recorder.textureOperations.map((operation) => operation.kind),
    ...
    for (const operation of recorder.textureOperations) {
        if (operation.kind === "upload") {
            ...
            if (operation.upload.kind === "external")
                assert.deepEqual(operation.upload.source, {
                    source: images[uploadIndex++],
                    flipY: false,
                });
...
test("compute passes refuse uncontracted calls, foreign resources and missing pipelines", () => {
    ...
    const other = createRecordingDevice({
        producer: "other",
        device: ["createComputePipeline", "createBindGroup"],
    });
    ...
    assert.throws(
        ...
        /compute dispatch has no pipeline/,
    );
    const foreign = other.device.createComputePipeline(descriptor);
    assert.equal(recorder.kindOf(foreign), undefined);
    assert.throws(
        () => pass.setPipeline(foreign),
        /pipeline this device did not create/,
    );
    assert.throws(
        () => pass.setPipeline(new RecordedComputePipeline(descriptor)),
        /pipeline this device did not create/,
    );
    assert.throws(
        () => pass.setBindGroup(0, other.device.createBindGroup({})),
        /group this device did not create/,
    );
    assert.throws(
        () => pass.end(),
        /compute pass for 'compute-refusals' does not answer 'end'/,
    );
...
test("submitted texture recording follows queue order and rejects command-buffer reuse", () => {
    const { device, recorder } = createRecordingDevice({
        producer: "submitted-order",
        device: ["createTexture", "createCommandEncoder"],
    ...
    const textures = Array.from({ length: 3 }, () =>
        device.createTexture({ size: [1, 1], format: "rgba8unorm", usage: 3 }),
    );
    ...
    assert.deepEqual(
        recorder.textureOperations
            .filter((operation) => operation.kind === "copy")
            .map((operation) => operation.copy.source.texture),
        [textures[1], textures[0]],
    );
    assert.throws(
        () => queue.submit([commands[0]!]),
        /unknown or already submitted/,
    );
...
test("textures carry their descriptor in both extent spellings and their uploads", () => {
    ...
    assert.deepEqual(
        upload.kind === "write" ? [...upload.bytes] : [],
        [1, 2, 3, 4],
    );
    assert.deepEqual(
        cube.uploads.map((entry) => entry.kind),
        ["external"],
    );
    assert.throws(
        () =>
...
test("buffers keep the mapped range and every queue write, by WebGPU's byte rules", () => {
    ...
    scratch.fill(0);
    assert.deepEqual(
        recorder.bufferWrites.map((write) => [
            write.buffer,
            write.offset,
            writtenFloats(write),
        ]),
    ...
    assert.deepEqual(
        [...new Float32Array(uniform.bytes)].slice(0, 4),
        [9, 10, 6, 7],
    );
    assert.throws(
...
test("copied-descriptor handles keep the descriptor's members and their kind", () => {
    interface Shapes {
        ...
        pipelineLayout: {
            bindGroupLayouts: readonly {
                entries: readonly { binding: number }[];
            }[];
        };
        renderPipeline: {
            layout: object;
            vertex: { module: { code: string } };
        };
        bindGroup: {
    ...
    assert.deepEqual(
        [module, layout, pipelineLayout, pipeline, sampler, group].map(
            (handle) => recorder.kindOf(handle),
        ),
        [
            "shaderModule",
            "bindGroupLayout",
...
test("the encoder records texture copies, render passes and submits", () => {
    ...
    assert.throws(
        () => pass.setViewport(),
        /Recording render pass for 'encoding' does not answer 'setViewport'/,
    );
    assert.throws(
        () => pass.setPipeline({}),
    ...
    assert.throws(
        () =>
            commands.beginRenderPass({
                label: "bad",
                colorAttachments: [{ view: {}, loadOp: "load" }],
            }),
        /attached a view this device did not create/,
    );
    const queue = (
        device as unknown as { queue: { submit(buffers: unknown[]): void } }
    ).queue;
