...
function doubleArray(values: Float64Array): string {
    ...
    return `std::array<double, ${values.length}>{${Array.from(
        values,
        (_, i) =>
            `std::bit_cast<double>(std::uint64_t{0x${bytes.readBigUInt64LE(i * 8).toString(16)}})`,
    ).join(",")}}`;
}
...
test("whole pinned scene pack matches bytes, optional contributors and getter ordering", async (t) => {
    ...
    const extras = await importPinnedModule<{
        setFog(this: void, scene: Scene, fog: Fog): void;
        setClipPlane(this: void, scene: Scene, plane: number[]): void;
        registerEnvSceneUniforms(this: void, scene: Scene): void;
    ...
    writeFileSync(
        join(directory, "pack.hpp"),
        new SceneUboLowerer(new LoweringContext()).packingHeader(),
    );

    const projection = Float64Array.from({ length: 16 }, (_, i) =>
        i === 0 ? -0 : i / 3,
    );
    const view = Float64Array.from({ length: 16 }, (_, i) =>
        i === 12 ? -0 : 1 + i * 2 ** -24,
    );
    const world = Float64Array.from({ length: 16 }, (_, i) =>
        i === 12 ? -0 : 1e9 + i / 7,
    );
    ...
    const camera = {
        ...
        _viewVer: 0,
        get _vpCache() {
            return observe("projection", projection);
        },
        get _viewCache() {
            return observe("view", view);
        },
        get worldMatrix() {
            return observe("world", world);
        },
    };
    const engine = {
    ...
    function sample(label: string, cpp: string, change: () => void = () => {}) {
        ...
        if (fail)
            ...
            _packSceneUniforms(data, engine, scene, camera, 2);
            for (const contributor of scene._sceneUboContributors ?? [])
    ...
    assert.equal(data[35], 0);
    assert.equal(data[40], 0);
    assert.equal(data[84], 0);
    assert.ok(Object.is(data[0], -0) && Object.is(data[32], -0));
    sample(
        "retained matrix alias read before later getter mutation",
        "mutate_alias = true;",
        () => {
            mutateAlias = true;
        },
    ...
    sample(
        "floating origin still reads world, then zeros the eye",
        "engine.use_floating_origin = true;",
        () => {
    ...
    sample(
        "base image values and environment without contributors",
        `scene.exposure = 1.25; scene.contrast = .75;
    scene.tone_mapping_enabled = true; scene.environment = Environment{.375};`,
        () => {
            ...
            scene._envTextures = { lodGenerationScale: 0.375 };
        },
    );
    sample(
        "registered contributors, with source order and padding preserved",
        `
    ...
    contributors = true;`,
        () => {
            extras.setFog(scene, {
                ...
                density: 0.03125,
                color: [0.125, 0.25, 0.375],
            });
            extras.setClipPlane(scene, [1, -2, 0.5, -0]);
            scene._environmentRotation = -0.25;
            scene._envTextures!.sphericalHarmonics = Float32Array.from(
                { length: 36 },
                (_, i) => (i - 18) / 7,
            );
    ...
    sample(
        "registered writers with cleared optional state",
        `scene.fog.reset(); scene.clip_plane.reset();
    scene.environment_rotation.reset(); scene.environment.reset();`,
        () => {
            delete scene.fog;
            delete scene.clipPlane;
            delete scene._environmentRotation;
            delete scene._envTextures;
        },
    );
    for (const name of ["projection", "view", "world"]) {
        sample(
            `failure in ${name} after fill and before any matrix store`,
            `fail = "${name}";`,
            () => {
                fail = name;
            },
    ...
    sample("recovery retains caller buffer", "fail.clear();", () => {
        fail = "";
    });
    ...
    const slab = Float64Array.from({ length: 48 }, (_, i) =>
        i === 20 ? -0 : (i - 10) / 9,
    );
    data.fill(7);
    packMat4IntoF32(data, slab, 19, 16);
    expected.push(Buffer.from(new Uint8Array(data.buffer)));
    data.fill(8);
    packMat4IntoF32(data, new Float32Array(view));
    expected.push(Buffer.from(new Uint8Array(data.buffer)));
    ...
    const aliasLowerer = new PinnedNumericLowerer(aliasSource, {
        bindings: new Map([
            [
                "camera.worldMatrix",
                { cpp: "borrow()", type: "f64-buffer", materializeAlias: true },
            ],
        ]),
        calls: new Map([
            ["matrix", () => "borrow()"],
            ["freshMatrix", () => "fresh()"],
            ["observe", (args: readonly string[]) => `observed = ${args[0]}`],
        ]),
        callShapes: new Map([
            ["matrix", "f64-buffer"],
            ["freshMatrix", "f32"],
    ...
    const aliasBody = aliasSource.statements
        .flatMap((statement) => aliasLowerer.statement(statement, "        "))
        .join("\n");
    writeFileSync(
        source,
        `#include "pack.hpp"
...
`,
    ...
    assert.deepEqual(
        execFileSync(executable, [bytes], { encoding: "utf8" })
            .trim()
            .split(/\r?\n/),
        trace,
    );
    const actual = readFileSync(bytes),
        allExpected = Buffer.concat(expected);
    assert.equal(actual.length, allExpected.length);
    for (let offset = 0; offset < actual.length; offset += 4)
        assert.equal(
            actual.readUInt32LE(offset),
            allExpected.readUInt32LE(offset),
            `snapshot ${Math.floor(offset / 368)}, lane ${(offset / 4) % 92}`,
        );
});
...
test("scene pack refuses newly reached unknown pin behavior", () => {
        ...
        override getSourceFile(module: string): ts.SourceFile {
            ...
            return module === "src/frame-graph/scene-uniforms-pack.ts"
                ? ts.createSourceFile(
                      ...
                      file.text.replace(
                          "data.fill(0);",
                          "data.fill(0); unknownContributor(data);",
                      ),
                      ts.ScriptTarget.Latest,
                      true,
                  )
    ...
    assert.throws(
        () =>
            new SceneUboLowerer(
                new LoweringContext(new ChangedStore()),
            ).packingHeader(),
        /unknownContributor/,
    );
});
...
test("borrowed buffer calls refuse unrepresented local rebinding", () => {
    const file = ts.createSourceFile(
        "buffer-rebinding.ts",
        "let value = matrix(); value = other();",
        ts.ScriptTarget.Latest,
        true,
    );
    const lowerer = new PinnedNumericLowerer(file, {
        bindings: new Map(),
        calls: new Map([["matrix", () => "matrix()"]]),
        callShapes: new Map([["matrix", "f32"]]),
    });
    assert.throws(
        () => lowerer.statement(file.statements[0]!, ""),
        /mutable buffer call binding/,
    );
});
