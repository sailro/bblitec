...
struct Allocation final:bbl::pal::StorageBufferAllocation {
 ...
 void write(std::size_t offset,std::span<const std::uint8_t> data)override{++writes;std::copy(data.begin(),data.end(),bytes.begin()+static_cast<std::ptrdiff_t>(offset));}
};
