...
const vatModule = "src/vat/vat-baker.ts",
...
function sourceResult(context: LoweringContext): unknown[] {
    ...
    const constants = paths.map((name) =>
    ...
    const api = createJavaScriptFunction(
        ...
        stopAnimation(this: void, group: Group): void;
    };
    const functions = [
        "bindingOf",
        "clipFrameCount",
        ...
        .map((name) =>
            context
                .functionDeclaration(vatModule, name)
                .declaration.getText()
                .replace(/^export /, ""),
        )
        .join("\n");
    ...
    return scenarios.map((scenario) => {
        const palettes = [0, 1, 2].map((resource) =>
            Float32Array.from(
                { length: 16 },
                (_, index) => resource * 20 + index,
            ),
        );
        const skeletons = palettes.map(() => ({
            boneTexture: {},
            boneCount: 1,
        }));
        const bindings = palettes.map((boneMatrices, index) => ({
            runtimeSkeleton: skeletons[index],
            boneTexture: skeletons[index]!.boneTexture,
            boneCount: scenario.mismatched ? 2 : 1,
            boneMatrices,
        }));
        let current = 0;
        ...
        const groups = scenario.owners.map((owner, index) => {
            const group = api.createAnimationGroups({
                clips: [
                    {
                        name: `clip${index}`,
                        duration: 0.0625,
                ...
                nodes: [node],
                skeletons: [],
                morphBindings: [{ nodeIdx: 0 }],
                nodeTargets: [],
                nodeNames: [],
            })[0]!;
            group._gltfMixer[2] =
                owner === 0 ? bindings.slice(0, 2) : bindings.slice(2);
            group._stopped = true;
            group.speedRatio = 2;
            const mask = group._ctrl._setMask;
            group._ctrl._setMask = (value) => {
                current = index;
                events.push(["mask", index]);
                mask(value);
            };
            return group;
        });
        pose = () => {
            const group = groups[current]!,
                drive = scenario.drives[current]!;
            events.push(["pose", current, group._ctrl.time]);
            palettes[drive]!.set(
                Float32Array.from(
                    { length: 16 },
                    (_, lane) => drive * 20 + lane + group._ctrl.time * 100,
                ),
            );
        };
        const live = Array.from(palettes[scenario.target]!);
        const targetSkeleton =
            scenario.absent || scenario.attached
                ? null
                : scenario.alias
                  ? { ...skeletons[scenario.target]! }
                  : skeletons[scenario.target];
        let error: string | null = null,
            baked: object | null = null;
        try {
            const result = prepare(
                [{ mesh: { name: "target", skeleton: targetSkeleton } }],
                groups,
            )[0]!;
            baked = {
                data: Array.from(result.data),
                frame_count: result.frameCount,
                bone_count: result.boneCount,
                clips: result.clips,
            };
        } catch (caught) {
...
test("VAT source CPU seeking preserves binding validation order and uploaded palette isolation", (t) => {
    ...
    const contexts = variants(),
        expected = contexts.map(sourceResult);
    assert.notDeepEqual(expected[0], expected[1]);
    assert.notDeepEqual(expected[0], expected[2]);
    const directory = resolve("artifacts/test-gltf-vat-cpu");
...
${new VatLowerer(context)
    .lower({ instances: false })
    .source.replace(/^#include .*$/gm, "")
    .replace("namespace bbl {", "")
    .replace(/} \/\/ namespace bbl\s*$/, "")
    .replace(/\b(stop_animation|vat_play|bake_vat)\b/g, "fixture_$1")}
    ...
    assert.throws(
        () =>
            new VatLowerer(
                doctoredContext(
                    vatModule,
                    "binding.boneTexture === skeleton.boneTexture",
                    "binding.boneCount === skeleton.boneCount",
                ),
            ).lower({ instances: false }),
        /identity lookup/,
    );
