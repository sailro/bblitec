...
test("native animation rest nodes follow the complete pinned parser and changed source", async (t) => {
    ...
    const contexts = [
        ...
        doctoredContext(
            ...
            "parentIdx: findParent(parentMap, i + 1)",
        ),
        doctoredContext(
            module,
            "const n = json.nodes[i];",
            "const n = json.nodes[nodeCount - i - 1];",
        ),
        doctoredContext(
            module,
            "n.scale ?? [1, 1, 1]",
            "n.scale ?? [findParent(parentMap, i), 1, 1]",
        ),
    ];
    const inputNodes = [
        {},
        {
            translation: [0.1234567890123, -2, 4],
    ...
    const constants =
        await importPinnedModule<Record<string, number>>("animation/types.js");
    const expected = contexts.map((context) => {
        ...
        const source =
            ["INTERP_MAP", "PATH_MAP"]
                .map(
                    (name) =>
                        `const ${name} = ${context.moduleScopeConstant(file, name)!.getText()};`,
                )
                .join("\n") +
            ["toSamplerFloat32", "hasWritableNodeChannel", "parseAnimationData"]
                .map((name) =>
                    context
                ...
                .join("\n");
        const reads: number[] = [];
        const parse = createJavaScriptFunction(
            "exports",
            "F32",
            "_convertSampler",
            "_parsePointerChannel",
            "resolveAccessor",
            "findParent",
            ...Object.keys(constants),
            transpileCommonJs(source, module) + "\nreturn parseAnimationData;",
        )(
            {},
            Float32Array,
            null,
            null,
            (_json: object, _bin: object, index: number) => ({
                _data: new Float32Array(index ? [0, 0, 0] : [0]),
                _count: 1,
                _componentCount: index ? 3 : 1,
            }),
            (_map: object, index: number) => {
                reads.push(index);
    ...
    const output = resolve("artifacts/gltf-animation-node-rest");
    mkdirSync(output, { recursive: true });
    writeFileSync(
        join(output, "cases.json"),
        JSON.stringify({ input: { nodes: inputNodes }, parents, expected }),
    );
    ...
    assert.equal(
        execFileSync(executable, { cwd: output, encoding: "utf8" }),
        "",
    );
});
...
test("unrepresented animation rest storage and parent ownership refuse", () => {
    assert.throws(
        () =>
            lowerGltfAnimationNodeRest(
                doctoredContext(module, "sx: s[0],", "sx: s[0], extra: 2,"),
            ),
        /Unsupported node-rest field/,
    );
    assert.throws(
        () =>
            lowerGltfAnimationNodeRest(
                doctoredContext(
                    module,
                    "parentIdx: findParent(parentMap, i)",
                    "parentIdx: findParent(otherMap, i)",
                ),
            ),
