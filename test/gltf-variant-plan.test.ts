...
const document = (): Promise<JsonObject> =>
    withMeshPlan({
        materials: [
            { name: "base" },
            { name: "red" },
        ...
        meshes: [
            { primitives: [{ material: 0, attributes: {} }] },
            {
                primitives: [
                    {
                        material: 1,
                        ...
                        extensions: {
                            KHR_materials_variants: {
                                mappings: [
                                    ...
                                    { material: 2, variants: [0, 2] },
                                    { material: 3, variants: [99] },
                                ],
                            },
                        },
                    },
                    {
                        ...
                        extensions: {
                            KHR_materials_variants: {
                                mappings: [{ material: 0, variants: [1] }],
                            },
                        },
                    },
                ],
...
async function sourcePlan(json: JsonObject, context: LoweringContext) {
        ...
        (visitorContext) => (root) => {
            const visit: ts.Visitor = (node) =>
                ...
                node.expression.kind === ts.SyntaxKind.ImportKeyword
                    ? ts.factory.createIdentifier("mipmapModule")
    ...
    const meshes = base.meshes.map((mesh) => ({
        material: baseMaterials[mesh.material]!,
    }));
    const leaves = {
        mipmapModule: { generateMipmaps() {} },
        getOrCreateSampler: () => ({}),
        makeImageFetcher: () => () => undefined,
        assembleMaterial: async (
            _json: JsonObject,
            _bin: DataView,
            index: number,
        ) => ({ index }),
        uploadTex() {},
        identityTexWrap: (value: unknown) => value,
        ...
        applyGltfUvTransform() {},
        applyGltfOptInPbrFeatures() {},
    };
    const load = createJavaScriptFunction(
        ...Object.keys(leaves),
        transpileCommonJs(`${text}\nreturn loadVariantMaterials;`, module),
    )(...Object.values(leaves)) as (
        json: JsonObject,
        bin: DataView,
        base: string,
        names: string[],
    ...
    const { declaration: selection } = context.functionDeclaration(
        "src/loader-gltf/material-variants.ts",
        "selectVariant",
    );
    const select = createJavaScriptFunction(
        transpileCommonJs(
            `${selection.getText().replace(/^export\s+/, "")}\nreturn selectVariant;`,
            "selection.ts",
        ),
    )() as (container: { materialVariants: VariantData }, name: string) => void;
    const selections: Record<string, number[]> = Object.create(null) as Record<
        string,
        number[]
    >;
    ...
    for (const name of names) {
        meshes.forEach((mesh, index) => {
            mesh.material = initialMaterials[index]!;
        });
...
for (const [name, context] of [
    ...
    [
        ...
        doctoredContext(
            module,
            "variantExt.mappings as",
            "[...variantExt.mappings].reverse() as",
        ),
    ],
    [
        "mapping argument",
        doctoredContext(
            module,
            "getMat(mapping.material)",
            "getMat((mapping.material + 1) % 5)",
        ),
    ],
    ["name gate", doctoredContext(module, "if (name)", 'if (name === "B")')],
    [
        "cache key",
        doctoredContext(module, "matCache[matIdx]", "matCache[matIdx % 2]"),
    ],
    [
        "selection order",
        doctoredContext(
    ...
    [
        "selection without source reset",
        doctoredContext(
            "src/loader-gltf/material-variants.ts",
    ...
    test(`variant material schedule follows ${name} source`, async () => {
        assert.deepEqual(
            await gltfVariantPlan(await document(), context),
            await sourcePlan(await document(), context),
        );
    });
...
test("variant slots retain separate base identity and every source-built mapping", async () => {
    ...
    for (const name of ["A", "B"]) {
        ...
        assert.deepEqual(
            renderables.map((renderable) => renderable.material),
            plan.selections[name],
        );
    }
});
...
test("variant planning refuses invalid resources and metadata collisions", async () => {
    ...
    assert.deepEqual(
        packed[GLTF_VARIANT_PLAN],
        await gltfVariantPlan(await document()),
    );
    const cached = await gltfVariantPlan(packed);
    assert.equal(
        cached.materials,
        (packed[GLTF_VARIANT_PLAN] as { materials: number[] }).materials,
    );
    ...
    await assert.rejects(
        gltfVariantPlan(invalid),
        /Invalid or missing packaged/,
    );
    await assert.rejects(
        gltfVariantPlan(
            await document(),
            doctoredContext(
                module,
                "assembleMaterial(json, binChunk, matIdx, baseUrl, imageCache)",
                "assembleMaterial(json, binChunk, matIdx, 'other', imageCache)",
            ),
        ),
        /resource ownership/,
    );
    for (const plan of [
        ...
        {},
        { ...cached, baseCount: 0 },
        { ...cached, materials: [100] },
        { ...cached, selections: {} },
        { ...cached, selections: { A: [0], B: [0] } },
        {
            ...cached,
            selections: { A: [99, 99, 99, 99, 99], B: [0, 0, 0, 0, 0] },
        },
    ]) {
        await assert.rejects(
            gltfVariantPlan({
                ...(await document()),
                [GLTF_VARIANT_PLAN]: plan,
            }),
            /Invalid packaged/,
        );
    }
