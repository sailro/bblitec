...
test("range parts retain originating selectors, part states and authored decorations", () => {
    ...
    assert.throws(
        () =>
            compileSheet(
                ".level::-webkit-slider-thumb:horizontal{background:red}",
            ),
        /selector.*not lowered/,
    );
    assert.throws(
        () => compileSheet('.level::-webkit-slider-thumb::before{content:"x"}'),
        /selector.*not lowered/,
    );
    assert.throws(
        () => compileSheet('.level::-webkit-slider-thumb{content:"x"}'),
...
test("Gecko range rules follow Chromium selector-list rejection without changing the live cascade", () => {
    ...
    assert.throws(
        () => compileSheet(".level::-moz-range-unrepresented{width:20px}"),
        /selector.*not lowered/,
    );
    assert.throws(
        () => compileSheet('[data-kind="::-moz-range-thumb"]{unrepresented:1}'),
        /style property/,
    );
});

test("native custom range parts preserve dimensions, cascade, values and control input", (t) =>
    runRmlUiFixture(t, "ui-range-parts"));

test("host range rules share target validation and native emission", () => {
    const source =
        "import {createEngine} from '@babylonjs/lite';await createEngine({});";
    const host = {
        ...
        styleRules: [
            {
                kind: "class" as const,
                primary: "level",
                range: "thumb" as const,
                style: "appearance:none;width:18px",
            },
        ],
    };
    assert.match(
        compileSource(source, { nativeHostUi: host }).cpp,
        ...
        () =>
            compileSource(source, {
                nativeHostUi: {
                    ...host,
                    styleRules: [
                        { ...host.styleRules[0]!, scrollbar: "thumb" },
                    ],
                },
            }),
        /exclusive thumb or track/,
    );
    assert.throws(
        () =>
            compileSource(source, {
        ...
        /requires an authored retained element/,
    );
});
