...
const minimal = {
    scene: "scene1",
    phases: [
        { id: "baseline", frame: 10 },
        { id: "orbit", frame: 10, tape: ["-*3", "+UiMouseLeft@1:2"] },
    ],
    expect: [
...
test("parses a declared check and expands tape shorthands", () => {
    ...
    assert.deepEqual(expandTape(spec.phases[1]!.tape!), [
        "-",
        "-",
        "-",
        "+UiMouseLeft@1:2",
                ...
                steps: [
                    {
                        id: "s",
                        actions: [
                            { drag: [1, 2, 3, 4], steps: 8 },
                            { fill: { selector: "#t", text: "x" } },
                        ],
                        screenshot: "canvas",
                    },
                ],
    ...
    assert.deepEqual(observed.observe?.steps[0]?.actions?.[0], {
        drag: [1, 2, 3, 4],
        steps: 8,
    });
});
...
test("refuses unknown keys, kinds and undeclared phases by name", () => {
        ...
        () =>
            parseCheckSpec(
                JSON.stringify({
                    ...minimal,
                    expect: [
                        { kind: "viewport", phase: "resize", equals: [1, 2] },
                    ],
                }),
                "test",
            ),
    ...
    assert.throws(
        () => readCheckSpec("no-such-check"),
        /No check is declared for 'no-such-check'/,
    );
});
...
test("reads a capture through the path language", () => {
    const capture = {
        ...
        draws: [
            { pipeline: "shader", order: 1000 },
            { pipeline: "standard", order: 0 },
            { pipeline: "shader", order: 1000 },
        ],
        materials: [
            {
                directIntensity: 0,
                textures: [{ slot: "metallicRoughness", byteLength: 4 }],
            },
            { directIntensity: 1, textures: [] },
        ],
    };
    ...
    assert.deepEqual(readCapturePath(capture, "meshes[*].position"), [
        [1, 2, 3],
        [4, 5, 6],
    ]);
    assert.equal(readCapturePath(capture, "draws[pipeline=shader].length"), 2);
    assert.deepEqual(
        readCapturePath(capture, "draws[pipeline=shader].order"),
        [1000, 1000],
    );
    assert.equal(
        readCapturePath(capture, "draws[pipeline=shader][1].order"),
        1000,
    );
    assert.equal(
        readCapturePath(capture, "draws[pipeline=shader][2]"),
        undefined,
    );
    assert.throws(
        () => readCapturePath(capture, "draws[pipeline=shader]x[0]"),
        /cannot parse segment/,
    );
    assert.deepEqual(
        readCapturePath(
            capture,
            "materials[directIntensity=0].textures[slot=metallicRoughness].byteLength",
        ),
        [[4]],
    );
    assert.equal(readCapturePath(capture, "camera.missing.deeper"), undefined);
    assert.throws(
        () => readCapturePath(capture, "draws[bad]"),
        /must be an index/,
    );
});
...
test("applies observe hooks once and refuses ambiguous markers", () => {
    ...
    assert.equal(
        applyObserveHooks(source, [
            { marker: "await registerScene(scene);", inject: "hook();" },
        ]),
        "a();\nawait registerScene(scene);\nhook();\nb();\n",
    );
    assert.equal(
        applyObserveHooks(source, [
            { marker: "b();", inject: "hook();", position: "before" },
        ]),
        "a();\nawait registerScene(scene);\nhook();\nb();\n",
    );
    assert.throws(
        () =>
            applyObserveHooks(`${source}b();`, [
                { marker: "b();", inject: "hook();" },
            ]),
...
test("every declared check names a registry scene, existing plugins and unique hook markers", () => {
    ...
    for (const id of ids) {
        ...
        for (const expectation of spec.expect) {
            if (expectation.kind === "plugin") {
                assert.ok(
                    existsSync(resolve(expectation.module)),
                    `${id}: plugin ${expectation.module}`,
                );
            }
        }
        if (spec.observe?.initScriptFile !== undefined) {
            assert.ok(
                existsSync(resolve(spec.observe.initScriptFile)),
                `${id}: init script ${spec.observe.initScriptFile}`,
            );
        }
        ...
        for (const hook of spec.observe?.hooks ?? []) {
            assert.equal(
                source.split(hook.marker).length,
                2,
                `${id}: hook marker '${hook.marker}' in ${scene.source}`,
            );
        }
    }
    ...
    const named = new Set(
        ids.flatMap((id) =>
            readCheckSpec(id).expect.flatMap((expectation) =>
                expectation.kind === "plugin"
                    ? [resolve(expectation.module)]
                    : [],
            ),
        ),
    );
    const initScripts = new Set(
        ids.flatMap((id) => {
            const file = readCheckSpec(id).observe?.initScriptFile;
            return file === undefined ? [] : [resolve(file)];
        }),
    );
