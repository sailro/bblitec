...
test("compiled material colors retain source identity, double width, fallback and replacement", async (t) => {
    ...
    const { createPbrMaterial } = await importPinnedModule<{
        createPbrMaterial(
            this: void,
            props: { baseColorFactor?: number[] },
        ): { baseColorFactor?: number[] };
    }>("material/pbr/pbr-material.js");
    const { createStandardMaterial } = await importPinnedModule<{
        createStandardMaterial(this: void): { diffuseColor: number[] };
    }>("material/standard/create-standard-material.js");
    const values = [0.123456789012345, 0.2, 0.3, 1];
    const pbr = createPbrMaterial({ baseColorFactor: values });
    assert.equal(pbr.baseColorFactor, values);
    values[0] = 0.875;
    assert.equal(pbr.baseColorFactor[0], 0.875);
    assert.equal(createPbrMaterial({}).baseColorFactor, undefined);
    assert.deepEqual(
        createPbrMaterial({ baseColorFactor: [1, 1, 1, 1] }).baseColorFactor,
        [1, 1, 1, 1],
    );
    const standard = createStandardMaterial(),
        another = createStandardMaterial();
    ...
    const variants = await composeScenePbrVariants(
        compiled.manifest.scenePbrMaterials,
        arms,
    );
    assert.ok(
        variants.some((variant) =>
            variant.fragmentWgsl.includes("material.baseColorFactor"),
        ),
    );
    ...
    const sceneSource = new SceneLowerer(new LoweringContext()).lowerCore()
        .source;
    const registration = [
        ...
        .map((name) => cppFunction(sceneSource, name))
        .join("\n");
...
test("numeric material reads refuse a co-reached tuple color producer in either source order", () => {
    for (const statements of [
        ...
        assert.throws(
            () =>
                compileSource(
                    program.slice(0, program.indexOf("async function main")) +
                        `async function main(){const engine=await createEngine({});${statements}}`,
                ),
...
test("material-color transport refuses unsupported widths and later material-group snapshots", () => {
    ...
    for (const statement of [
        ...
        assert.throws(
            ...
            /per-group UBO snapshots|independent material-group UBO snapshots/,
        );
    assert.throws(
        () =>
            compileSource(
                prefix + `createPbrMaterial({baseColorFactor:[1,2,3]});}`,
            ),
        /four-channel numeric array/,
    );
    assert.throws(
        () => compileSource(prefix + `p.diffuseColor=[1,2];}`),
    ...
    assert.throws(
        () =>
            compileSource(
                prefix +
                    `const values:readonly number[]=[1,1,1,1];createPbrMaterial({baseColorFactor:values});}`,
            ),
        /static readonly tuple cannot retain material color identity/,
    );
    const writeOnly = prefix.slice(0, prefix.indexOf("const color="));
    assert.match(
        compileSource(
            writeOnly + `registerScene(scene);p.diffuseColor=[.2,.4,.6];}`,
        ).cpp,
    ...
    assert.doesNotThrow(() =>
        compileSource(
            writeOnly +
                `p.diffuseColor=[.2,.4,.6];rebuildSceneRenderables(scene);}`,
        ),
    );
    assert.throws(
        () =>
            compileSource(
                writeOnly +
                    `const channels=[.2,.4,.6];p.diffuseColor=channels;rebuildSceneRenderables(scene);}`,
            ),
        /per-group UBO snapshots/,
    );
    assert.throws(
        () =>
            compileSource(
                writeOnly +
                    `const channels=[.2,.4,.6,1];createPbrMaterial({baseColorFactor:channels});rebuildSceneRenderables(scene);}`,
            ),
        /per-group UBO snapshots/,
    );
});
...
test("glTF public factor presence comes from the pinned conditional and retains its array", async () => {
    ...
    for (const image of [null, {}])
        for (const factor of [
            [1, 1, 1, 1],
            [0.123456789012345, 0.4, 0.7, 0.8],
        ]) {
            const mat = assemblePbrProps(
                {
                    _baseColorImage: image,
            ...
            assert.equal(
                mat.baseColorFactor,
                image && factor[0] !== 1 ? factor : undefined,
            );
            const extended = assemblePbrPropsExt(
                {
                    _baseColorImage: image,
                    _baseColorFactor: factor,
                    _normalScale: 1,
