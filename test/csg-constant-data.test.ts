...
test("CSG binary transport preserves float bits, integer lanes and empty streams", () => {
    ...
    assert.throws(
        () => unpackBakedCsgMesh(bytes.subarray(0, bytes.length - 1)),
        /lengths/,
    );
...
test("repeated CSG geometry shares one packaged asset and constructs distinct meshes", () => {
    ...
    `);
    assert.equal(
        result.manifest.assets.filter((asset) => asset.kind === "binary")
            .length,
        1,
    );
...
test("unchanged Scene 90 packages its baked streams outside the C++ source", () => {
    ...
    assert.ok(
        result.cpp.length < 100000,
        "baked geometry must not expand into C++ literals",
    );
    assert.ok(
        main.length < 100000,
        "immutable geometry data must not be expanded inside the loop body",
    );
...
test(
    "native baked-mesh transport matches generation bytes and rejects invalid lengths",
    { skip: !nativeTools },
    () => {
        const bytes = packBakedCsgMesh({
            positions: Float32Array.of(-0, 1 / 3),
            normals: Float32Array.of(1),
            uvs: new Float32Array(),
...
test(
    "packaged Scene 90 data compiles warning-clean",
    { skip: !nativeTools },
    () => {
        const path = "corpus/babylon-lite/lab/lite/src/lite/scene90.ts";
        ...
        writeFileSync(
            join(includes, "camera_math.hpp"),
            new CameraLowerer(new LoweringContext()).lowerArcRotateFactory()
                .header,
        );
...
test(
    "shared literal tables preserve distinct typed-array storage on every evaluation",
    { skip: !nativeTools },
    () => {
