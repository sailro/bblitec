...
test("JSON record replacement tolerates a transient Windows reader", { skip: process.platform !== "win32", timeout: 15000 }, async () => {
    ...
    const child = spawn("pwsh", ["-NoProfile", "-Command", script], { windowsHide: true, stdio: "ignore" });
    ...
    try {
        ...
        while (!existsSync(ready) && Date.now() < deadline && child.exitCode === null) await setTimeout(10);
        assert.ok(existsSync(ready), "reader acquired the file");
        writeJsonRecord(path, { completed: 2 });
        assert.deepEqual(JSON.parse(readFileSync(path, "utf8")), { completed: 2 });
