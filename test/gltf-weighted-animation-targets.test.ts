...
function contexts(): LoweringContext[] {
    const base = new LoweringContext(),
        file = base.sourceFile(module);
    const moved = file.text
        .replace("scratch.targets.set(nodes, target);", "")
        .replace(
            "resetWeightedGltfTarget(target);",
            "resetWeightedGltfTarget(target); scratch.targets.set(nodes, target);",
        );
    return [
        base,
        doctoredContext(module, "sample: new F32(16)", "sample: new F32(20)"),
        doctoredContext(
            module,
            "order[cursor++] = idx;",
            "order[cursor++] = idx + 1;",
...
interface Target {
    nodes: Node[];
    skeletons: unknown[];
    overrides: { id: number } | undefined;
    baseRot: Float32Array | undefined;
    trs: Float32Array;
...
function sourceResult(context: LoweringContext): unknown {
    ...
    const reset = (target: Target) => {
        events.push(["reset", scratch.targets.has(target.nodes)]);
        if (fail) {
            fail = false;
    ...
    const source =
        "let scratchByManager;\n" +
        ["getScratch", "getTarget", "computeTopoOrder"]
            .map((name) =>
                context.functionDeclaration(module, name).declaration.getText(),
            )
            .join("\n");
    const api = createJavaScriptFunction(
        "F32",
        "I32",
        "U8",
        "GLTF_NODES",
    ...
    )(Float32Array, Int32Array, Uint8Array, 1, 2, 12, reset) as {
        getScratch(manager: object): Scratch;
        getTarget(scratch: Scratch, mixer: unknown[]): Target;
    };
    const manager = {};
    scratch = api.getScratch(manager);
    scratch.sample[0] = 99;
    const nodesA: Node[] = [
            { parentIdx: 1 },
            { parentIdx: -1 },
        ...
        nodesB: Node[] = [{ parentIdx: -1 }, { parentIdx: 0 }];
    const mixerA = [
            {},
            nodesA,
            [{ runtimeSkeleton: { _overrides: { id: 1 } } }],
        ],
        mixerSecond = [
            {},
            nodesA,
            [{ runtimeSkeleton: { _overrides: { id: 2 } } }],
        ],
        mixerB = [{}, nodesB, []];
    try {
        api.getTarget(scratch, mixerA);
    } catch (error) {
        events.push((error as Error).message);
    }
    const first = api.getTarget(scratch, mixerA),
        again = api.getTarget(scratch, mixerSecond),
        second = api.getTarget(scratch, mixerB);
    const row = (target: Target) => ({
        key: target.nodes === nodesA ? 0 : 1,
        overrides: target.overrides?.id ?? null,
        baseRot: target.baseRot?.length ?? null,
        lengths: [
            target.trs.length,
            target.localMat.length,
            target.worldMat.length,
            target.tWeight.length,
            target.rWeight.length,
            target.sWeight.length,
        ],
        topology: [...target.topoOrder],
        ...
        zero: [
            ...
            target.rWeight,
            target.sWeight,
        ].every((values) => values.every((value) => value === 0)),
    });
    return {
        events,
        same: first === again,
        distinct: first !== second,
        scratchSame: scratch === api.getScratch(manager),
        scratchLengths: [
            scratch.sample.length,
            scratch.reference.length,
            scratch.delta.length,
        ],
        sample: scratch.sample[0],
        targets: [...scratch.targets.values()].map(row),
    };
}
test("source weighted target cache retains identity, allocation widths and publication on reset failure", () => {
    const variants = contexts(),
        outputs = variants.map(sourceResult);
    for (const output of outputs.slice(1))
        assert.notDeepEqual(output, outputs[0]);
    for (const context of variants)
        assert.ok(lowerGltfWeightedAnimationTargets(context));
    assert.throws(
        () =>
            lowerGltfWeightedAnimationTargets(
                doctoredContext(
                    module,
                    "scratch.targets.get(nodes)",
                    "scratch.targets.get(skeletons)",
                ),
            ),
...
test("native weighted target allocation follows source mutations and failed initial reset", (t) => {
    ...
    const variants = contexts(),
        directory = resolve("artifacts/test-gltf-weighted-animation-targets");
