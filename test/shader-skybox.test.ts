...
test("skybox specialization transforms typed bindings, stage interfaces and fog expressions", () => {
    ...
    assert.deepEqual(
        result.vertex.bindings?.map(({ group, binding }) => [group, binding]),
        [[1, 0]],
    );
    assert.deepEqual(
        result.fragment.bindings?.map(({ group, binding }) => [group, binding]),
        [
            [2, 0],
            [2, 1],
            [3, 0],
    ...
    assert.deepEqual(
        result.vertex.structs[0]?.members,
        result.fragment.structs[0]?.members,
    );
    for (const stage of [result.vertex, result.fragment]) {
        assert.ok(
            !stage.entryPoint.statements.some((statement) =>
                statementUsesPath(
                    statement,
                    (parts) =>
                        ["mesh", "scene"].includes(parts[0]!) ||
                        parts.includes("vFogDistance"),
                ),
            ),
        );
        assert.deepEqual(
            parseWgslModule(emitWgslModule(stage), stage.entryPoint.stage),
            stage,
        );
    }
    const branch = result.fragment.entryPoint.statements.find(
            ...
            arguments: [
                {
                    ...
                    expression: {
                        kind: "binary",
                        operator: "*",
                        left: { kind: "path", parts: ["uniforms", "view"] },
                        right: {
                            kind: "construct",
                            type: "vec4<f32>",
                            arguments: [
                                { kind: "path", parts: ["b", "vPositionW"] },
...
test("formatting and minifier renaming preserve skybox specialization", () => {
    const format = (source: string): string =>
        source.replace(/([{}:;,])/g, " /* outer /* nested */ */ $1 \n");
    assert.deepEqual(
        specializeImageSkybox(format(vertex), format(fragment)),
        specializeImageSkybox(vertex, fragment),
    );
    const rename = (source: string): string =>
        source
            .replace(/\b[a-g]\b/g, (name) => `renamed_${name}`)
            .replaceAll("mesh", "meshBinding")
            .replaceAll("normal", "normalInput");
    ...
    assert.deepEqual(
        renamed.vertex.structs[0]?.members,
        renamed.fragment.structs[0]?.members,
    );
...
test("skybox specialization refuses semantic changes its native interface cannot carry", () => {
    for (const [from, to, reason] of [
        ...
        assert.throws(
            () => specializeImageSkybox(vertex.replace(from, to), fragment),
            reason,
        );
    }
    assert.throws(
        () =>
            specializeImageSkybox(
                vertex,
                fragment.replace(
                    "scene.vFogColor.rgb",
                    "scene.unmappedColor.rgb",
                ),
            ),
        /unmapped/,
    );
