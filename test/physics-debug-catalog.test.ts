...
import { jsonNumbers, jsonRecords } from "./json.js";
import {
    materializePhysicsDebugCatalog,
    physicsDebugDescriptor,
    renderPhysicsDebugCatalog,
} from "../src/physics-debug-catalog.js";
import {
...
const box = {
    type: "BOX",
    parameters: [0, 0, 0, 0, 0, 0, 1, 2, 3, 4],
    indices: [],
    children: [],
};
const sphere = {
...
test("constructor descriptors refuse unrepresented fields, widths and parameter order/arity", () => {
    ...
    for (const value of [
        { ...box, bodyPose: [0, 1, 0] },
        { ...box, parameters: [0, 0, 0] },
        { ...box, indices: [0] },
        { ...box, children: [sphere] },
        { ...box, parameters: [...box.parameters.slice(0, -1), Infinity] },
        { ...box, type: "UNSUPPORTED" },
        { ...box, parameters: [...box.parameters, 1] },
    ])
        assert.throws(
            () => physicsDebugDescriptor(value),
            /constructor inputs/,
        );
});
...
test("catalog identity depends on complete descriptors and is independent of construction order", async () => {
    ...
    assert.equal(
        ...
        renderPhysicsDebugCatalog(
            await materializePhysicsDebugCatalog([sphere, box]),
        ),
    );
    const changed = await materializePhysicsDebugCatalog([
        { ...box, parameters: [...box.parameters.slice(0, -1), 4.5] },
    ]);
    assert(
        !entries.some(
            (entry) => entry.shapeIdentity === changed[0]!.shapeIdentity,
        ),
    );
});
...
test(
    "native catalog selects exact inputs, refuses drift and guards construction execution",
    { skip: !tools },
    async () => {
        ...
        writeFileSync(
            ...
            renderPhysicsDebugCatalog(
                await materializePhysicsDebugCatalog([box, sphere]),
            ),
        );
        writeFileSync(
    ...
    `,
        ...
        assert.match(
            execFileSync(executable, [inputs], {
                encoding: "utf8",
                env: tools!.environment,
            }),
            /physics-debug-catalog: ok/,
        );
        const captured = jsonRecords(JSON.parse(readFileSync(inputs, "utf8")));
        assert.equal(captured.length, 3);
        assert.equal(jsonNumbers(captured[1]!.parameters)[9], 4.125);
