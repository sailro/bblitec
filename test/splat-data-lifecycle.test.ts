...
class EditedStore extends UpstreamSourceStore {
    constructor(private readonly edit: (text: string) => string) {
        super();
    }
...
test("splat CPU lifecycle anchors the whole handoff", () => {
    for (const edit of [
        (s: string) =>
            s.replace(
                ...
                "return; const newGeom = buildSplatGeometry(newBuffer);",
            ),
        (s: string) =>
            s.replace(
                "mesh._sortDepthTransform.fill(0);",
                "mesh._sortDepthTransform.fill(1);",
            ),
        (s: string) =>
            s.replace(
                "retainedSplatsData = newBuffer;",
                "retainedSplatsData = newBuffer.slice(0);",
            ),
        (s: string) =>
            s.replace("newGeom.centersRGBA);", "newGeom.colorsRGBA);"),
    ]) {
        const context = new LoweringContext(new EditedStore(edit));
        assert.throws(
            () =>
                new SplatLowerer(context).lowerLoader({
                    retainRows: true,
                    containers: new Map(),
                }),
            /updateData/,
    ...
    assert.doesNotMatch(
        lowerer.lowerLoader({ retainRows: false, containers: new Map() })
            .source,
        /void update_splat_data/,
    );
    const changed = new SplatLowerer(
        new LoweringContext(
            new EditedStore((s) =>
                s.replace("GS vertex count mismatch", "changed pinned message"),
            ),
        ),
    ).lowerLoader({ retainRows: true, containers: new Map() });
    assert.match(
        changed.source,
        /throw std::runtime_error\("changed pinned message"\)/,
    );
});
...
test("either public splat row API retains loader data without reaching transform baking", () => {
    for (const [name, operation] of [
        [
            "getter",
            "const data=cloud.splatsData; if(data.byteLength!==64) throw new Error('rows');",
        ],
        [
            "update",
            "const bytes=new Uint8Array(64); cloud.updateData(bytes.buffer);",
        ],
        ["unused", ""],
    ] as const) {
        const compiled = compileSource(splatProgram(operation));
        assert.equal(
            compiled.manifest.features.includes("loader:splat-data"),
            name !== "unused",
        );
        assert.equal(
            compiled.manifest.features.includes("loader:splat-bake"),
            false,
        );
        const output = resolve(`artifacts/splat-data-exposure-${name}`);
        emitUpstreamGenerated(output, compiled.manifest.features);
        const loader = readFileSync(
            join(output, "upstream/src/splat_loader.cpp"),
            "utf8",
        );
        assert.equal(
            loader.includes("void update_splat_data("),
            name !== "unused",
        );
        assert.equal(
            loader.includes("record.splats_data ="),
            name !== "unused",
        );
    }
});
...
test("getter-only splat rows refuse writes through direct, alias and nested source paths", () => {
    for (const write of [
        ...
        assert.throws(
            () =>
                compileSource(
                    splatProgram(`const bytes=new Uint8Array(64); ${write}`),
                ),
...
async function pinnedLifecycle(output: string): Promise<void> {
    const { buildSplatGeometry } = await importPinnedModule<{
        buildSplatGeometry(this: void, buffer: ArrayBuffer): Geometry;
    }>("loader-splat/splat-data.js");
    const { createGaussianSplattingMesh } = await importPinnedModule<{
        createGaussianSplattingMesh(
            ...
            geometry: Geometry,
            worker: unknown,
            parsed: { data: ArrayBuffer },
        ): Cloud;
    }>("mesh/GaussianSplatting/gaussian-splatting-mesh.js");
    const { bakeCurrentTransformIntoVertices } = await importPinnedModule<{
        bakeCurrentTransformIntoVertices(this: void, mesh: Cloud): void;
    }>("mesh/GaussianSplatting/gaussian-splatting-bake.js");
    ...
    const queue = {
        writeBuffer() {},
        writeTexture(
            destination: { texture: { index: number } },
            bytes: ArrayBuffer,
        ) {
            events.push(`texture${destination.texture.index}`);
            textures[destination.texture.index] = new Float32Array(
                bytes.slice(0),
            );
        },
    };
    const engine = {
        _device: {
            ...
            createTexture() {
                ...
                textures.push(new Float32Array());
                return { index, createView: () => ({}) };
            },
            createSampler: () => ({}),
    ...
    const worker = {
        postMessage(message: { p: Float32Array }) {
            events.push("positions");
            positions = message.p.slice();
        },
    };
    ...
    const cloud = createGaussianSplattingMesh(
        engine,
        "cloud",
        geometry,
        worker,
    ...
    const capture = (name: string) => {
        const chunks = [
            ...
            new Float32Array([...cloud.boundMin, ...cloud.boundMax]),
            positions,
            ...textures,
        ];
        writeFileSync(
            join(output, `${name}.bin`),
            Buffer.concat(
                chunks.map((v) =>
    ...
    assert.throws(() => {
        cloud.splatsData = new ArrayBuffer(64);
    }, TypeError);
    assert.equal(
        cloud.splatsData,
        original,
        "getter-only rows cannot be replaced by assignment",
    );
    ...
    assert.deepEqual(
        textures,
        oldPayload,
        "a buffer write is not an updateData upload",
    );
    ...
    capture("stage2");
    assert.equal(
        new Float32Array(original)[8],
        -4,
        "replacement preserves old aliases",
    );
    for (const [length, message] of [
        [96, "GS vertex count mismatch"],
        [0, "splat buffer is empty"],
        [65, "multiple of 4"],
    ] as const) {
        events.length = 0;
        assert.throws(
            () => cloud.updateData(new ArrayBuffer(length)),
            new RegExp(message),
        );
        ...
        assert.deepEqual(
            events,
            [],
            "rejection precedes uploads and worker messages",
        );
    }
...
test(
    "splat getter equality alone emits complete native ArrayBuffer support",
    { skip: !nativeTools },
    () => {
        ...
        const compiled = compileSource(
            splatProgram(
                'if(cloud.splatsData!==cloud.splatsData) throw new Error("identity");',
            ),
        );
...
test(
    "native splat CPU updates and bake match the pin across shared and replaced buffers",
    { skip: !nativeTools },
    async () => {
        ...
        for (const [name, lowered] of [
            ["splat_geometry", lowerer.lowerGeometry()],
            ["splat_sort", lowerer.lowerSort()],
            [
                "splat_loader",
                lowerer.lowerLoader({
                    retainRows: true,
            ...
            if (lowered.header)
                writeFileSync(join(headers, `${name}.hpp`), lowered.header);
            writeFileSync(join(output, `${name}.cpp`), lowered.source);
        }
        const source = join(output, "check.cpp");
        writeFileSync(
            source,
            readFileSync("test/fixtures/splat-data-check.cpp"),
        );
