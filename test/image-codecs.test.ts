...
test("image codecs follow packaged content independently of capture", (t) => {
    ...
    const asset = (
        output: string,
        kind: CompileAsset["kind"] = "texture",
    ): CompileAsset => ({ source: output, output, kind });
    assert.deepEqual(reachedImageCodecs(root, []), []);
    assert.deepEqual(
        reachedImageCodecs(root, [asset("pixels.rgba16f", "pixels")]),
        [],
    );
    assert.deepEqual(
        reachedImageCodecs(root, [asset("environment-data", "environment")]),
        ["png"],
    );
    assert.deepEqual(
        reachedImageCodecs(root, [
            asset("face.PNG"),
            asset("photo.jpg"),
            asset("photo.JPEG"),
            asset("icon.webp"),
    ...
    writeFileSync(
        join(root, "assets", "opaque"),
        Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]),
    );
    assert.deepEqual(reachedImageCodecs(root, [asset("opaque", "binary")]), [
        "png",
    ]);
    writeFileSync(
        join(root, "assets", "opaque"),
...
test("codec metadata admits a new format without a second codec table", () => {
    ...
    assert.deepEqual(
        parseImageCodecs(JSON.parse(readFileSync("native/vcpkg.json", "utf8"))),
        imageCodecs,
    );
});
...
test("codec metadata refuses malformed signatures and extensions", () => {
    ...
    for (const metadata of [
        ...
        assert.throws(
            () =>
                parseImageCodecs({
                    features: { future: { "$bblite-image": metadata } },
                }),
            /Invalid image codec metadata/,
        );
    }
    assert.throws(() => parseImageCodecs({}), /must declare features/);
    assert.throws(
        () => parseImageCodecs({ features: {} }),
        /no image features/,
    );
});
...
test("container image references select codecs without scanning unrelated glTF text", (t) => {
    ...
    assert.deepEqual(
        reachedImageCodecs(root, [{ ...asset, output: "model.glb" }]),
        ["jpeg", "webp"],
    );
    writeFileSync(
        join(root, "assets", "model.babylon"),
        JSON.stringify({
            textures: [
                { name: "face.png?version=2" },
    ...
    assert.deepEqual(
        reachedImageCodecs(root, [
            { ...asset, output: "model.babylon", kind: "babylon" },
        ]),
        ["png", "jpeg"],
    );
});
