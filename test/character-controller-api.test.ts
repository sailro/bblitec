...
import {
    characterCollisionObservableSource,
    characterControllerHeader,
} from "../src/lowering/character-controller-runtime.js";
...
test(
    "character observers match pinned live iteration and duplicate callback removal",
    { skip: !tools },
    async () => {
        ...
        let first = true;
        const removeA = observable.add(() => {
            expected.push(1);
            if (first) {
                first = false;
                observable.add(() => expected.push(4));
            }
        });
        const removeB = observable.add(() => {
            expected.push(2);
            removeB();
        });
        ...
        const removeDuplicate = observable.add(duplicate);
        observable.add(duplicate);
        removeDuplicate();
        observable.notify({});
        removeDuplicate();
        observable.notify({});
...
test("character lifecycle PAL substitutions and observable contracts reject unrepresented source drift", () => {
    ...
    for (const [from, to] of [
        ...
        ["if (i >= 0)", "if (i > 0)"],
        [
            "hknp.HP_QueryCollector_Create(16)[1]",
            "hknp.HP_QueryCollector_Create(16)[0]",
        ],
        [
            "hknp.HP_Shape_Release(this._shape._hkShape)",
            "hknp.HP_Shape_Release(this._body._hkBody)",
        ],
        [
            "hknp.HP_QueryCollector_Release(this._startCollector)",
            "hknp.HP_QueryCollector_Release(this._body)",
        ],
