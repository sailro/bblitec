...
test("HINGE contracts retain anchor ordering, defaults, perpendicular math and axis locks", () => {
    ...
    for (const [before, after] of [
        ...
        [
            "vec3Array(pivotA), vec3Array(axisA), vec3Array(perpAxisA)",
            "vec3Array(pivotA), vec3Array(perpAxisA), vec3Array(axisA)",
        ],
        [
            "hknp.HP_Constraint_SetEnabled(joint, true)",
            "hknp.HP_Constraint_SetEnabled(joint, false)",
        ],
        [
        ...
        class Changed extends LoweringContext {
            public override sourceFile(module: string): ts.SourceFile {
                ...
                if (module !== "src/physics/havok.ts")
                    return super.sourceFile(module);
        ...
        assert.throws(
            () => lowerPhysicsConstraints(new Changed(store)),
            /changed|exactly/,
        );
    }
...
test("constraint admission refuses untyped axes, partial bounds and unsupported springs", () => {
    ...
    for (const [limits, error] of [
        ...
        [
            "[{axis:PhysicsConstraintAxis.LINEAR_DISTANCE,minLimit:1}]",
            /minLimit and maxLimit/,
        ],
        [
            "[{axis:PhysicsConstraintAxis.LINEAR_DISTANCE,minLimit:1,maxLimit:2,stiffness:1}]",
        ...
        assert.throws(
            () =>
                compileSource(
                    `${prefix} createPhysicsConstraint(world,a.body,b.body,PhysicsConstraintType.SIX_DOF,{},${limits});`,
                ),
            error,
        );
    }
...
test(
    "native constraint configurations match all pinned factories and hinge lifetime stays valid",
    { skip: !tools },
    async () => {
        ...
        const pin = await importPinnedModuleWithExports<{
            PhysicsConstraintType: Record<string, number>;
            PhysicsConstraintAxis: Record<string, number>;
        ...
        const cases: string[] = [
            `constexpr double hinge_type = ${pin.PhysicsConstraintType.HINGE};`,
        ];
        for (const type of Object.values(pin.PhysicsConstraintType))
            for (const distance of [undefined, 2]) {
                const axes = names.map(() => ({
                    mode: "free",
                    minimum: 0,
                    maximum: 0,
                }));
                const limits = names.map((name, index) => ({
                    axis: pin.PhysicsConstraintAxis[name]!,
                    minLimit: index / 10,
                    maxLimit: 1 + index / 10,
                }));
                pin.configureConstraintAxes(
                    {
                        ConstraintAxis: Object.fromEntries(
                        ...
                        HP_Constraint_SetAxisMode: (_joint, axis, mode) => {
                            axes[axis]!.mode = mode;
                        },
                        HP_Constraint_SetAxisMinLimit: (
                            _joint,
                        ...
                        HP_Constraint_SetAxisMaxLimit: (
                            ...
                            axes[axis]!.maximum = value;
                        },
                    },
                    {},
                    type,
                    distance === undefined ? {} : { maxDistance: distance },
                    limits,
                );
                cases.push(
                    `{ const auto axes = u::physics_constraint_axes(${type}, ${distance === undefined ? "{}" : "{.max_distance = 2.0}"}, {${limits.map((row) => `{${row.axis},${row.minLimit},${row.maxLimit}}`).join(",")}});\n` +
                        axes
                            .map(
                                (row, index) =>
                                    `assert(axes[${index}].mode == p::PhysicsConstraintAxisMode::${row.mode} && axes[${index}].minimum == ${row.minimum} && axes[${index}].maximum == ${row.maximum});`,
                            )
                            .join("\n") +
                        "\n}",
                );
        ...
        writeFileSync(
            join(output, "constraint-axis-cases.inc"),
            cases.join("\n"),
        );
        ...
        runNativeFixtureCompiler(tools!, [
            ...
            "/external:W0",
            "test/fixtures/physics-hinge-check.cpp",
