...
test("iOS publishing selects device SDL packaging without conflating Android or Simulator options", () => {
    const args = mobilePackageArguments("ios", "tetris", new Map([
        ["--jobs", "3"], ["--workers", "1"], ["--output", "artifacts/releases with spaces"],
    ]));
    for (const [flag, expected] of [["-Platform", "ios"], ["-Scene", "tetris"], ["-Jobs", "3"],
    ...
    for (const flag of ["--sdk", "--device", "--abi"]) {
        assert.throws(() => mobilePackageArguments("ios", "tetris", new Map([[flag, "value"]])), /ARM64 devices/);
    }
    assert.throws(() => mobilePackageArguments("ios", "tetris", new Map([["--workers", "2"]])), /share dependencies/);
    for (const jobs of ["0", "-1", "1.5", "three"]) {
        assert.throws(() => mobilePackageArguments("ios", "tetris", new Map([["--jobs", jobs]])), /positive integer/);
    }
    const result = spawnSync(process.execPath, ["dist/src/shipping-demos.js", "--platform", "ios",
        "--scene", "tetris", "--plan"], { encoding: "utf8", windowsHide: true });
    assert.equal(result.status, 0, result.stdout + result.stderr);
    assert.deepEqual(JSON.parse(result.stdout), [{ scene: "tetris", args: mobilePackageArguments("ios", "tetris", new Map()) }]);
});

test("iOS shipping scripts parse and preserve the per-feature trimmed SDL policy", { skip: !tools.powershell }, t => {
    const directory = mkdtempSync(join(tmpdir(), "bblite-ios-shipping-policy-"));
    t.after(() => rmSync(directory, { recursive: true, force: true }));
    const sdl = readFileSync("tools/build-sdl-min.ps1", "utf8");
    const flags = cppSection(sdl, "$audioSetting =", "# Keep in lockstep");
    const options = cppSection(sdl, "$sdlOptions =", "$configureArguments =")
        .replaceAll("$IsMacOS", "$macHost").replaceAll("$IsLinux", "$linuxHost");
...
test("iOS packaging refuses incompatible caches, device identity and external libraries", { skip: !tools.powershell }, t => {
    const directory = mkdtempSync(join(tmpdir(), "bblite-ios-shipping-admission-"));
    t.after(() => rmSync(directory, { recursive: true, force: true }));
    const source = readFileSync("tools/package-ios.ps1", "utf8");
    const configurationGuard = cppSection(source, "$required =", "$sourceBundle =");
    const binaryGuard = cppSection(source, "$architecture =", "$dependencies | Set-Content");
    ...
    const base = {
        cache: {
            ...
            BBLITE_IOS_BUNDLE_IDENTIFIER: "org.bblite.demo.tetris", BBLITE_SDL_DIR: "/trimmed",
            BBLITE_GENERATED_DIR: "/generated", CMAKE_OSX_DEPLOYMENT_TARGET: "16.0",
        },
        configuration: { minSize: true, sdk: "iphoneos", sdkVersion: "16.4", signed: false, generatedDirectory: "/generated" },
        architecture: "arm64", platform: "IOS", minos: "16.0", sdk: "16.4",
        dependencies: ["/usr/lib/libc++.1.dylib", "/System/Library/Frameworks/Metal.framework/Metal"],
        plist: { CFBundleExecutable: "bblite_native", CFBundleIdentifier: "org.bblite.demo.tetris",
            CFBundleSupportedPlatforms: ["iPhoneOS"], UIDeviceFamily: [1, 2], MinimumOSVersion: "16.0" },
    };
    writeFileSync(script, `
$ErrorActionPreference = 'Stop'
...
`);
    const run = (value: typeof base) => {
        ...
        return spawnSync(tools.powershell!, ["-NoProfile", "-File", script], { encoding: "utf8", windowsHide: true });
    };
    const valid = run(base);
    assert.equal(valid.status, 0, valid.stdout + valid.stderr);
    const cases: Array<{ change: (value: typeof base) => void; error: RegExp }> = [
        ...(["BBLITE_MINSIZE", "BBLITE_PCH", "BBLITE_VISUAL_CAPTURE", "BBLITE_AUDIO_CAPTURE"] as const).map(key => ({
            change: (value: typeof base) => {
                value.cache[key] = value.cache[key] === "ON" ? "OFF" : "ON";
            }, error: /iOS shipping requires/,
        })),
        { change: value => { value.cache.BBLITE_BACKEND = "DAWN"; }, error: /BBLITE_BACKEND/ },
        { change: value => { value.cache.BBLITE_SDL_DIR = ""; }, error: /trimmed device/ },
        { change: value => { value.cache.BBLITE_GENERATED_DIR = "/other"; }, error: /generated tree/ },
        { change: value => { value.configuration.signed = true; }, error: /unsigned/ },
        { change: value => { value.configuration.sdk = "iphonesimulator"; }, error: /device bundle/ },
        { change: value => { value.platform = "IOSSIMULATOR"; }, error: /device executable/ },
        { change: value => { value.platform = "MACOS"; }, error: /device executable/ },
        { change: value => { value.architecture = "x86_64 arm64"; }, error: /thin ARM64/ },
        { change: value => { value.sdk = "16.2"; }, error: /SDK disagrees/ },
        { change: value => { value.minos = "17.0"; }, error: /deployment target/ },
        { change: value => { value.plist.UIDeviceFamily = [1]; }, error: /iPhone and iPad/ },
        { change: value => { value.plist.CFBundleIdentifier = "other.application"; }, error: /identify this executable/ },
        { change: value => { value.dependencies.push("@rpath/libSDL3.dylib"); }, error: /external import/ },
    ];
...
test("iOS publication retains a previous unsigned package without touching comparison baselines", { skip: !tools.powershell }, t => {
    const directory = mkdtempSync(join(tmpdir(), "bblite-ios-shipping-publish-"));
    t.after(() => rmSync(directory, { recursive: true, force: true }));
