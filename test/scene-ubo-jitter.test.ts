...
test("pinned jitter preserves clean data, writes one matrix after history, and wraps its sequence", async (t) => {
    ...
    task._blend.updateUniforms = () => {
        events.push("uniform");
    };
    for (const [name, child] of [
        ["blend", task._blend],
        ["present", task._present],
        ["history", task._historyUpdate],
    ] as const)
        child.execute = () => {
            events.push(name);
            return 1;
...
`,
    ...
    assert.match(
        execFileSync(executable, [join(directory, "clean.bin"), actual], {
            encoding: "utf8",
        }),
        /scene-ubo-jitter: ok/,
    );
