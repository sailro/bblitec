...
test(
    "shape collectors preserve Havok capsule features, mesh edges, body identity and filters",
    { skip: !tools },
    async () => {
        ...
        const actual = {
            queries: jsonArray(captured.queries).map((row) =>
                jsonArray(row).map(jsonNumbers),
            ),
            defaultMasses: jsonNumbers(captured.defaultMasses),
        };
        ...
        const vertices = hp._malloc(48),
            indices = hp._malloc(24);
        ...
        const massShapes = [
            capsule,
            hp.HP_Shape_CreateBox([0, 0, 0], [0, 0, 0, 1], [2, 3, 4])[1],
            hp.HP_Shape_CreateSphere([0, 0, 0], 0.5)[1],
        ];
        const referenceMasses = massShapes.map(
            (shape) => hp.HP_Shape_BuildMassProperties(shape)[1][1],
        );
        try {
            ...
            for (const shape of [capsule, box, floor])
                hp.HP_Shape_Release(shape);
            for (const shape of massShapes.slice(1)) hp.HP_Shape_Release(shape);
            hp.HP_World_Release(world);
        }
        assert.deepEqual(
            native.map((hits) => hits.length),
            reference.map((hits) => hits.length),
        );
        const errors = native.map((hits, query) =>
            hits.map((hit, i) =>
                hit.map((value, lane) =>
                    Math.abs(value - reference[query]![i]![lane]!),
                ),
            ),
        );
        ...
        writeFileSync(
            ...
            JSON.stringify({ native, reference, maxError }, null, 2) + "\n",
        );
        assert(maxError < 1e-5, `Collector feature error ${maxError}`);
        const massErrors = actual.defaultMasses.map(
            (mass, i) =>
                Math.abs(mass - referenceMasses[i]!) /
                Math.max(1, referenceMasses[i]!),
        );
        writeFileSync(
            join(output, "mass-properties.json"),
            JSON.stringify(
                {
                    actual: actual.defaultMasses,
                    reference: referenceMasses,
                    relativeErrors: massErrors,
                },
                null,
                2,
            ) + "\n",
        ...
        assert(
            Math.max(...massErrors) < 1e-6,
            "Default shape mass must preserve Havok density and authored volume.",
        );
    },
);
