...
interface Kernel {
    ...
    _simplexSolverSolve(
        ...
        velocity: Vector,
        deltaTime: number,
        minimumDeltaTime: number,
        maximumVelocity: Vector,
    ): SolverOutput;
    calculateMovement(
        deltaTime: number,
        forward: Vector,
...
function pinnedKernel() {
    ...
    const exports: {
        PhysicsCharacterController?: new (
            world: object,
            position: Vector,
            options: { capsuleHeight: number; capsuleRadius: number },
        ) => Kernel;
    } = {};
    const transport = {
    ...
    return new exports.PhysicsCharacterController(
        { _hknp: { HP_QueryCollector_Create: () => [0, {}] } },
        vector(),
        { capsuleHeight: 1.8, capsuleRadius: 0.6 },
    );
}
...
test(
    "reference-preserving simplex, friction and movement kernels match unchanged pinned TypeScript",
    { skip: !nativeTools },
    () => {
        const directory = resolve("artifacts/character-controller-kernel");
        mkdirSync(directory, { recursive: true });
        writeFileSync(
            join(directory, "kernel.hpp"),
            lowerCharacterControllerKernel(new LoweringContext()),
        );
        const kernel = pinnedKernel();
        const expected: number[][] = [];
        const runs: string[] = [];
        const cppVector = (value: Vector): string =>
            `v(${[value.x, value.y, value.z].map(doubleLiteral).join(", ")})`;
        const cppPlane = (value: Constraint): string =>
            `plane(${cppVector(value.planeNormal)}, ${cppVector(value.velocity)}, ${[value.priority, value.staticFriction, value.dynamicFriction, value.extraUpStaticFriction, value.extraDownStaticFriction].map(doubleLiteral).join(", ")})`;
        const normals = [
            vector(0, 1, 0),
            vector(1, 0, 0),
            vector(0, 0, 1),
        ...
        for (let i = 0; i < 30; i++) {
            const constraint = plane(normals[i % normals.length]!, {
                velocity: vector((i % 3) / 5, 0, -0.2),
                staticFriction: (i % 4) / 3,
                dynamicFriction: (i % 5) / 4,
                extraUpStaticFriction: i % 2,
                extraDownStaticFriction: (i % 3) / 2,
            });
            ...
            runs.push(
                `{ auto output = v(); kernel._solve1d(${cppPlane(constraint)}, ${cppVector(velocity)}, output); print(output); }`,
            );
        }
        const constraintSets = [
            [],
            [plane(vector(0, 1, 0))],
            [plane(vector(0, 1, 0)), plane(vector(1, 0, 0))],
            [
                plane(vector(0, 1, 0)),
                plane(vector(1, 0, 0)),
                plane(vector(0, 0, 1)),
            ...
            [
                plane(vector(0, 1, 0), { velocity: vector(1, 0, 0) }),
                plane(vector(1, 0, 0), { priority: 2 }),
            ],
            [
                plane(vector(0.6, 0.8, 0), { staticFriction: 0.5 }),
                plane(vector(0, 0, 1), { dynamicFriction: 0.25 }),
            ],
        ];
        for (const constraints of constraintSets) {
            for (const velocity of [
                ...
                const output = kernel._simplexSolverSolve(
                    constraints,
                    velocity,
                    0.1,
                    0.03,
                ...
                expected.push([
                    ...
                    output.velocity.z,
                    output.deltaTime,
                    ...output.planeInteractions.flatMap((interaction) => [
                        Number(interaction.touched),
                        Number(interaction.stopped),
                        interaction.surfaceTime,
                        interaction.penaltyDistance,
                        interaction.status,
                    ]),
                ]);
                runs.push(
                    `{ auto output = kernel._simplexSolverSolve({${constraints.map(cppPlane).join(", ")}}, ${cppVector(velocity)}, 0.1, 0.03, v(10, 10, 10)); print(output); }`,
                );
            }
        }
        for (const normal of normals) {
            const output = kernel.calculateMovement(
                1 / 60,
                vector(0, 0, 1),
                normal,
                vector(1, 2, 3),
            ...
            expected.push([output.x, output.y, output.z]);
            runs.push(
                `print(kernel.calculateMovement(${doubleLiteral(1 / 60)}, v(0, 0, 1), ${cppVector(normal)}, v(1, 2, 3), v(0.1, 0, 0.3), v(1, 0, -1), v(0, 1, 0)));`,
            );
        }
        writeFileSync(
...
`,
        ...
        for (const [row, values] of expected.entries()) {
            assert.equal(
                actual[row]!.length,
                values.length,
                `case ${row} output shape`,
            );
            for (const [column, value] of values.entries()) {
                const error = Math.abs(actual[row]![column]! - value);
                assert.ok(
                    error <= 1e-12,
                    `case ${row}, component ${column}: ${actual[row]![column]} != ${value}`,
                );
        ...
        writeFileSync(
            ...
            JSON.stringify(
                { cases: expected.length, maximumError, expected, actual },
                null,
                2,
            ),
        );
