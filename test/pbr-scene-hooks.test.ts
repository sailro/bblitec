...
import {
    lowerPbrSceneHookRegistry,
    lowerPbrTransmissionSelection,
} from "../src/lowering/pbr-scene-hooks.js";
...
const scenarios: Scenario[] = [
    ...
    ...[-1, 0, 0.25, 1, 1e-100].map((intensity) => ({
        materials: [{ transmissive: true, intensity }],
        meshes: [0],
    })),
    {
        materials: [
            { transmissive: true, intensity: 1 },
            { transmissive: false },
        ],
        meshes: [1],
    },
...
function selectionOracle(
    ...
    return (input) => {
        ...
        const execute = createJavaScriptFunction(
            ...
            () => ({}),
            undefined,
        ) as (scene: object, engine: object, meshes: unknown[]) => void;
        const materials = input.materials.map(
            (material) =>
                material && {
                    _transmissive: material.transmissive,
                    _subsurface: {
                        refraction: { intensity: material.intensity },
                    },
                },
        );
        const meshes = input.meshes.map((index) =>
            index < 0 ? undefined : { material: materials[index] },
        );
        try {
            execute({}, {}, meshes);
        } catch (error) {
...
async function registryOracle(
    context: LoweringContext,
    fail: boolean,
): Promise<object> {
    const registry = ["_registerPbrSceneHook", "_getPbrSceneHooks"]
        .map((name) =>
            context
                .functionDeclaration(flagsModule, name)
                .declaration.getText()
                .replace("export ", ""),
        )
        .join("\n");
    ...
    const loop = builder.body!.statements.find(
        (statement) =>
    ...
    const code = transpileCommonJs(
        ...
        return {register: _registerPbrSceneHook, get: _getPbrSceneHooks, run};`,
        flagsModule,
    );
    type Hook = () => void;
    const executed = createJavaScriptFunction(code)() as {
        register(hook: Hook): void;
        get(): Iterable<Hook>;
        run(...args: object[]): Promise<void>;
    };
    const empty = executed.get(),
        events: string[] = [];
    const c = () => {
        events.push("c");
    };
    ...
    const b = () => {
        events.push("b");
        if (fail) throw new Error("hook");
    };
    executed.register(a);
    const live = executed.get();
    executed.register(a);
    executed.register(b);
    ...
    for (let i = 0; i < 2; i++) {
        try {
            ...
            if (!(error instanceof Error) || error.message !== "hook")
                throw error;
            failures.push(true);
        }
    }
...
test("native PBR hook registry and transmission selection agree with source execution", async (t) => {
    ...
    const registryCases = await Promise.all(
        [false, true].map(async (fail) => ({
            fail,
            expected: await registryOracle(contexts[0]!, fail),
        })),
    );
    const directory = resolve("artifacts/test-pbr-scene-hooks");
    mkdirSync(directory, { recursive: true });
    writeFileSync(
        resolve(directory, "cases.json"),
        JSON.stringify({ selection: cases, registry: registryCases }),
    );
...
test("PBR hook lowering refuses unrepresented activation, material reads and registrations", () => {
    for (const [before, after] of [
        [
            ...
            "enableSceneTransmission(scene, otherEngine)",
        ],
        ...
        assert.throws(
            () =>
                lowerPbrTransmissionSelection(
                    doctoredContext(transmissionModule, before!, after!),
                ),
            /adapter|Unsupported|Expected|changed/,
        );
    const module = "src/material/pbr/set-transmission.ts",
        name = "setPbrTransmission";
    const context = doctoredContext(
        module,
        "_registerPbrSceneHook(registerPbrTransmission)",
        "_registerPbrSceneHook(unknownHook)",
    );
    assert.throws(
        () =>
            lowerGltfMaterialObjectFunction(
                context,
                {
                    ...
                    name,
                    cpp: name,
                    declaration: context.functionDeclaration(module, name)
                        .declaration,
                },
                () => undefined,
            ),
