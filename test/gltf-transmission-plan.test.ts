...
import {
    ...
    gltfTransmissionPlan,
    materialSubjects,
    packageGltfTransmissionPlan,
} from "../src/pinned-material-arms.js";
import {
    recordingTransmissionSetter,
    transmissionRegistrationMarker,
} from "../src/pinned-pbr-transmission.js";
import { doctoredContext } from "./doctored-store.js";
import { writeGlbFixture } from "./glb-fixture.js";
import { meshPlanFixture } from "./gltf-mesh-fixture.js";

const transmission = (factor: number): JsonObject => ({
    extensions: { KHR_materials_transmission: { transmissionFactor: factor } },
});

async function prepare(
    materials: JsonObject[],
    primitive: JsonObject = { material: 0 },
    extra: JsonObject = {},
) {
    const { document, bin } = meshPlanFixture({
        asset: { version: "2.0" },
        extensionsUsed: [
...
test("source construction and hook predicate select used materials, including empty and dispersion-only setters", async () => {
    ...
    for (const row of cases) {
        ...
        assert.deepEqual(await gltfTransmissionPlan(document), {
    ...
    assert.equal(
        (await materialSubjects(document))[0]!.transmissionRegistered,
        true,
    );
...
test("variant transmission receipts follow source-selected scene mesh slots", async () => {
    const { document } = await prepare(
        ...
        },
        {
            extensions: {
                KHR_materials_variants: {
                    variants: [{ name: "Glass" }, { name: "Opaque" }],
                },
            },
        },
    );
    await packageGltfTransmissionPlan(document);
    assert.deepEqual(packagedGltfTransmissionPlan(document), {
        registered: true,
        initial: false,
        variants: { Glass: true, Opaque: false },
    });
    assert.equal(await gltfLinearImageProcessing(document), false);
    ...
    await assert.rejects(
        gltfLinearImageProcessing(document, "missing"),
        /Missing glTF transmission selection/,
    );
    await assert.rejects(
        packageGltfTransmissionPlan(document),
        /already carries/,
    );
    assert.throws(
        () =>
...
test("specialization reports unknown before source construction and requires its receipt for emission", async () => {
    const directory = resolve(
        "artifacts/test-gltf-transmission-specialization",
    );
    mkdirSync(resolve(directory, "assets"), { recursive: true });
    const { document, binary } = await prepare([transmission(0.5)]);
...
test("recorded transmission registration follows the actual source call", async () => {
    type Setter = {
        setPbrTransmission(material: JsonObject, options: JsonObject): void;
    };
    const contexts = [
        undefined,
        doctoredContext(
            ...
            "if (refraction.intensity > 0) _registerPbrSceneHook(registerPbrTransmission);",
        ),
    ];
    for (const [index, context] of contexts.entries()) {
        const setter = (await import(
            recordingTransmissionSetter("transmission", context)
        )) as Setter;
        const material: JsonObject = {};
        setter.setPbrTransmission(material, { intensity: 0 });
        assert.equal(
            material[transmissionRegistrationMarker] === true,
            index === 0,
        );
        assert.equal(material._transmissive, true);
        assert.equal(
            Object.keys(material).includes(transmissionRegistrationMarker),
            false,
        );
    }
});
