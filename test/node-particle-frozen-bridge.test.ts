...
test(
    "frozen sprite bridges keep sheet aliases live and preserve pinned buffer precision",
    { skip: !tools },
    async () => {
        interface Buffer {
            capacity: number;
        ...
        const { createParticleSystem } = await importPinnedModule<{
            createParticleSystem(this: void, capacity: number): System;
        }>("particle/particle-system.js");
        const { createParticleSprite2DBridge, syncParticleSprite2DBridge } =
        ...
        const options = {
            pixelsPerUnit: 177.3,
            originPx: [96, 96] as [number, number],
        };
        ...
        const spriteApi = await importPinnedModule<{
            clearSprite2DLayer(this: void, layer: Layer): void;
            addSprite2DIndex(
                this: void,
                layer: Layer,
                props: {
        ...
        syncParticleSprite2DBridge(bridge);
        assert.equal(bridge.layer.count, 1);
        spriteApi.addSprite2DIndex(bridge.layer, {
            positionPx: [0, 0],
            sizePx: [12, 13],
        });
        ...
        assert.equal(bridge.layer.count, 1);
        assert.deepEqual(
            Array.from(bridge.layer._instanceData.slice(0, 13)),
            snapshots[1],
        );
        assert.deepEqual(
            Array.from(bridge.layer._savedSize.slice(2, 4)),
            [0, 0],
        );
        ...
        system.blendMode = 4;
        const exact = exactApi.createParticleSprite2DBridgeWithBlendModes(
            system,
            options,
        );
        ...
        const presentation = (layer: Layer) => [
            layer.opacity,
            layer.visible,
            layer.order,
            layer.view,
        ...
        assert.deepEqual(
            presentation(exact.layers[1]!),
            presentation(exact.layer),
        );
        const entry: NodeParticleSystemEmit = {
            bake: {
                ...
                sizes: [
                    buffer.size[0] * buffer.scaleX[0],
                    buffer.size[0] * buffer.scaleY[0],
                ],
                colors: [
                    buffer.colorR[0],
                    buffer.colorG[0],
        ...
        const particle = new NodeParticleLowerer(context).lower(
            [
                entry,
                { ...entry, bake: { ...entry.bake, system: 1, blendMode: 4 } },
                {
                    ...entry,
                    bake: {
                        ...entry.bake,
