...
const meshFeatures = [
    0,
    bits.MSH_HAS_SKELETON,
    bits.MSH_HAS_MORPH_TARGETS,
...
test("regular picking composes each pin deformation arm for both pass modes", async () => {
    ...
    const basic = await importPinnedModule<{
        pickingShaderSource(
            this: void,
            options: { _vertexProjection: object },
        ): string;
    }>("picking/picking-shader.js");
    const detailed = await importPinnedModuleWithExports<{
        shader(this: void, rule: null, projection: object): string;
    }>("picking/picking-detailed-pipeline.js", ["shader"]);
    const variants = await composeDeformPickingShaders({
        meshFeatures,
        skeleton: true,
        morph: true,
        detailed: true,
    ...
    assert.deepEqual(
        variants.map(({ skeleton, morph }) => [skeleton, morph]),
        [
            [true, false],
            [false, true],
            [true, true],
    ...
    for (const variant of variants) {
        ...
        const engine = {
            _device: {
                createBindGroupLayout: (layout: { entries: unknown[] }) => {
                    layouts.push(layout);
                    return layout;
                },
            },
        ...
        assert.equal(
            variant.mesh,
            basic.pickingShaderSource({ _vertexProjection: projection.shader }),
        );
        assert.equal(
            variant.detailed,
            detailed.shader(null, projection.shader),
        );
        assert.equal(
            layouts.at(-1)!.entries.length,
            Number(variant.skeleton) + Number(variant.morph) * 2,
        );
        ...
        assert.equal(
            pin.getDeformPickingProjection(engine, { ...mesh, vat: {} }),
            null,
        );
    }
    ...
    assert.equal(
        separate.length,
        2,
        "independent attachments do not compose an unused combined arm",
    );
    assert.ok(
        separate.every((variant) => !(variant.skeleton && variant.morph)),
    );
});
...
test("picking does not deploy unreached projection families or detailed stages", async () => {
    ...
    for (const skeleton of [false, true]) {
        ...
        assert.match(
            deformPickingHeader(variants),
            /"picking-deform-0.vert", nullptr/,
        );
    }
});
