...
function fixture(channels: JsonObject[], pointers = true) {
    ...
    const scalar = append(new Float32Array([0, 0.75]), "SCALAR", 1);
    const vector = append(
        new Float32Array([0, 0, 0, 1.123456789, 2, 3]),
        "VEC3",
        3,
    );
    const color = append(
        new Float32Array([0, 0, 0, 1, 1, 0.5, 0.25, 1]),
        "VEC4",
        4,
    );
    const document: JsonObject = {
        ...
        meshes: [
            {
                primitives: [
                    { attributes: { POSITION: position }, material: 0 },
                ],
            },
        ],
        nodes: [{ mesh: 0, name: "Animated node" }],
        scenes: [{ nodes: [0] }],
        animations: [
            {
                name: "clip",
                channels,
                samplers: [
                    { input: time, output: vector },
                    { input: time, output: scalar },
                    { input: time, output: color },
                ],
            },
        ],
...
const pointer = (path: string, sampler = 1): JsonObject => ({
    sampler,
    target: {
        path: "pointer",
        extensions: { KHR_animation_pointer: { pointer: path } },
    },
});
const trs = (path = "translation", node = 0): JsonObject => ({
    sampler: 0,
    target: { node, path },
});
function closure(
    value: CaptureValue | undefined,
): Extract<CaptureValue, { kind: "closure" }> {
...
test("source terminal null and writable-node gates determine accepted animation data", async () => {
    ...
    assert.deepEqual(plan.animation.clips[0]!.controller.nodeTrsBindings, [
        { target: 0, off: 0, mask: 1 },
    ]);
    assert.deepEqual(plan.animation.nodeNames, ["Animated node"]);
    const absent = fixture([
        pointer("/nodes/9/extensions/KHR_node_visibility/visible"),
    ]);
    assert.deepEqual(
        (await gltfMeshPlan(absent.document, absent.bin)).animation,
        { accepted: false, materialState: [], nodeNames: [], clips: [] },
    );
    const rejected = await gltfMeshPlan(
        input.document,
        input.bin,
        doctoredContext(
            animationModule,
            "!hasWritableNodeChannel(clips, nodeTargets, excludedNodeIndices)",
            "true",
        ),
    );
...
test("source clip order, converted sampler bytes, controller bindings and group names are transported", async () => {
    const input = fixture([
        trs(),
        pointer("/materials/0/pbrMetallicRoughness/baseColorFactor", 2),
        pointer("/nodes/0/scale", 0),
        pointer("/nodes/0/extensions/KHR_node_visibility/visible"),
    ]);
    const bytes = await packageGltfMeshPlan(input.document, input.bin);
    const receipt = packagedGltfMeshPlan(input.document).animation!,
    ...
    assert.deepEqual(
        clip.channels.map((channel) => channel.samplerIdx),
        [0, 2, 0, 1],
    );
    assert.deepEqual(clip.targetedAnimations, [
        {
            target: 0,
            targetName: "Animated node",
            nodeIndex: 0,
            path: "translation",
        },
        { path: "pointer" },
        { target: 0, targetName: "Animated node", nodeIndex: 0, path: "scale" },
        { path: "pointer" },
    ]);
    assert.deepEqual(clip.controller.nodeTrsBindings, [
        { target: 0, off: 0, mask: 5 },
    ]);
    ...
    assert.deepEqual(
        readPackedGltfAttribute(
            input.document,
            bytes,
            clip.samplers[0]!.output,
        ),
        [0, 0, 0, Math.fround(1.123456789), 2, 3],
    );
    const named = fixture([trs()]);
    const moved = await gltfMeshPlan(
        named.document,
        named.bin,
        doctoredContext(
    ...
    assert.equal(
        moved.animation!.clips[0]!.targetedAnimations[0]!.targetName,
        "source name",
    );
});
...
test("pointer resolution records physical material owners after the source map's later replay overwrite", async () => {
    ...
    assert.deepEqual(
        closure(ordinary.animation!.clips[0]!.channels[0]!.writer).values.mat,
        { kind: "material", index: 0, path: [] },
    );
    const distinct = await gltfMeshPlan(
        input.document,
        input.bin,
        doctoredContext(
    ...
    assert.deepEqual(
        distinct.meshes.map((mesh) => mesh.material),
        [0, 1],
    );
    assert.deepEqual(
        closure(distinct.animation!.clips[0]!.channels[0]!.writer).values.mat,
        { kind: "material", index: 1, path: [] },
    );
    const nullWriter = await gltfMeshPlan(
        input.document,
        input.bin,
        doctoredContext(
            pointerModule,
    ...
    assert.deepEqual(
        nullWriter.baseColorDefinitions,
        [0],
        "source material preparation remains selected when the terminal writer is rejected",
    );
...
test("actual pointer preparation supplies material initialization and base-color definition identities", async () => {
    ...
    assert.ok(
        patches.some(
            (patch) =>
                patch.path.join(".") === "baseColorFactor" &&
                patch.operation === "set",
        ),
    );
    assert.ok(
    ...
    assert.deepEqual(ordinary.baseColorDefinitions, []);
    assert.equal(ordinary.baseColorModule, false);
});

test("source private texture selection and late light lookup captures remain distinct", async () => {
    const input = fixture([
        pointer(
            "/materials/0/pbrMetallicRoughness/baseColorTexture/extensions/KHR_texture_transform/rotation",
        ),
        pointer("/extensions/KHR_lights_punctual/lights/7/intensity"),
    ]);
    input.document.textures = [{ source: 0 }];
    input.document.images = [{}];
    asRecords(input.document.materials)[0]!.pbrMetallicRoughness = {
        baseColorTexture: { index: 0 },
    };
    const plan = await gltfMeshPlan(input.document, input.bin),
        channels = plan.animation!.clips[0]!.channels;
    assert.equal(
        channels.length,
        2,
        "an absent light still has a source writer and counts toward terminal acceptance",
    );
    const uv = closure(channels[0]!.writer);
    assert.deepEqual(uv.values.mat, { kind: "material", index: 0, path: [] });
    assert.deepEqual(uv.values.tex, {
        kind: "material",
        index: 0,
        path: ["baseColorTexture"],
    });
    const light = closure(channels[1]!.writer),
        lookup = closure(light.values.getLight);
    assert.deepEqual(lookup.lightLookups, [7]);
    assert.deepEqual(lookup.values.lightIdx, { kind: "literal", value: 7 });
    assert.deepEqual(light.values.field, {
        kind: "literal",
        value: "intensity",
    });
});
...
test("source resolver arity/quaternion and controller construction mutations move receipts", async () => {
    ...
    const altered = await gltfMeshPlan(
        input.document,
        input.bin,
        doctoredContext(
            pointerModule,
    ...
    const quaternion = await gltfMeshPlan(
        input.document,
        input.bin,
        doctoredContext(
            "src/loader-gltf/gltf-feature-animation-pointer.ts",
            "pointerArity: resolved.arity,",
            "pointerArity: resolved.arity, pointerQuaternion: true,",
        ),
    );
    assert.equal(
        quaternion.animation!.clips[0]!.channels[0]!.pointerQuaternion,
        true,
    );
    ...
    const changed = await gltfMeshPlan(
        ...
        standard.bin,
        doctoredContext(
            "src/skeleton/skeleton-updater.ts",
            "maskByNode.set(ni, (maskByNode.get(ni) ?? 0) | bit);",
            "maskByNode.set(ni, 7);",
        ),
    );
    assert.equal(
        changed.animation!.clips[0]!.controller.nodeTrsBindings[0]!.mask,
        7,
    );
    assert.throws(
        () =>
            readAnimationReceipt(
                {
                    ...
                    clips: [
                        {
                            ...changed.animation!.clips[0],
                            channels: [{ samplerIdx: 99, nodeIdx: 0, path: 0 }],
                        },
                    ],
                },
                {
...
test("source normalized sampler converter and variant construction share the completed material schedule", async () => {
    ...
    const binary = new BinaryBuilder(
        Buffer.from(
            input.bin.buffer,
            input.bin.byteOffset,
            input.bin.byteLength,
        ),
    );
    const data = new Uint8Array([0, 255]);
    const views = asRecords(input.document.bufferViews),
        accessors = asRecords(input.document.accessors);
    views.push({
        buffer: 0,
        byteOffset: binary.append(data),
        byteLength: data.byteLength,
    ...
    input.document.bufferViews = views;
    input.document.accessors = accessors;
    asRecords(asRecords(input.document.animations)[0]!.samplers)[1]!.output =
        accessors.length - 1;
    input.document.extensionsUsed = [
    ...
    const plan = packagedGltfMeshPlan(input.document),
        clip = plan.animation!.clips[0]!;
    assert.deepEqual(
        readPackedGltfAttribute(
            input.document,
            bytes,
            clip.samplers[1]!.output,
        ),
        [0, 1],
    );
    assert.deepEqual(
        closure(clip.channels[0]!.writer).values.mat,
        { kind: "material", index: 0, path: [] },
...
test("each source controller selects only skeletons reached by that clip's joint ancestors", async () => {
    ...
    const append = (data: Uint8Array | Float32Array) => {
        ...
        accessors.push({
            bufferView: views.length - 1,
            componentType: data instanceof Uint8Array ? 5121 : 5126,
            count: 3,
            type: "VEC4",
        });
        return accessors.length - 1;
    };
    const primitive = asRecords(
        asRecords(input.document.meshes)[0]!.primitives,
    )[0]!;
    primitive.attributes = {
        ...(primitive.attributes as object),
        JOINTS_0: append(new Uint8Array(12)),
        WEIGHTS_0: append(
    ...
    input.document.scenes = [{ nodes: [0, 1, 2] }];
    input.document.skins = [{ joints: [1] }];
    const first = asRecords(input.document.animations)[0]!;
    input.document.animations = [
        first,
        { ...first, name: "plain", channels: [trs("translation", 2)] },
    ];
    const bytes = binary.build(),
        bin = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const plan = await gltfMeshPlan(input.document, bin);
    assert.deepEqual(
        plan.animation!.clips.map((clip) => clip.controller.clipSkeletons),
        [[0], []],
    );
    assert.deepEqual(
        plan.animation!.clips.map((clip) => clip.controller.requiresEngine),
        [true, false],
    );
    assert.deepEqual(
        plan.animation!.clips.map((clip) => clip.controller.nodeTrsBindings),
        [[], [{ target: 2, off: 24, mask: 1 }]],
    );
});
