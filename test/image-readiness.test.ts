...
test("image readiness compiles owned startup and packages both valid and broken images", ()=>{
    const directory=resolve("artifacts/image-readiness");mkdirSync(directory,{recursive:true});
    ...
    const source='const worker=new Worker(new URL("./worker.ts",import.meta.url),{type:"module"});worker.terminate();'+fixture+
        '(async()=>{await checkImageReadiness();globalThis.close();})();';
    const result=compileSource(source,{fileName:join(directory,"entry.ts")});
    assert.ok(result.manifest.assets.some(asset=>asset.output==="pixel.png"));
    assert.ok(result.manifest.assets.some(asset=>asset.output==="broken.png"));
