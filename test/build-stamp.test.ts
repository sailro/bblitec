...
test("uses CMake's generation marker rather than the cache mtime", (t) => {
    const root = scratchRepository();
    t.after(() => rmSync(root, { recursive: true, force: true }));
    const build = resolve(root, "native/build-scene-release");
    const generated = resolve(root, "generated/scene");
    const marker = resolve(build, "CMakeFiles/cmake.check_cache");
    const cache = resolve(build, "CMakeCache.txt");
    mkdirSync(resolve(build, "CMakeFiles"), { recursive: true });
    writeFileSync(resolve(root, "native/vcpkg.json"), "{}\n");
    writeFileSync(cache, "BBLITE_BACKEND:STRING=BOTH\n");
    writeFileSync(marker, "# generated by CMake\n");

    const old = new Date("2025-01-01T00:00:00Z");
    const configured = new Date("2025-01-01T00:00:10Z");
    const changed = new Date("2025-01-01T00:00:20Z");
    utimesSync(cache, old, old);
    utimesSync(resolve(root, "native/CMakeLists.txt"), old, old);
    utimesSync(resolve(root, "native/vcpkg.json"), old, old);
    utimesSync(resolve(generated, "features.cmake"), old, old);
    utimesSync(marker, configured, configured);

    assert.equal(
        generatorWouldReconfigure(build, generated, root),
        false,
        "an unchanged cache mtime does not erase a successful configure",
    );
    utimesSync(resolve(generated, "features.cmake"), changed, changed);
    assert.equal(generatorWouldReconfigure(build, generated, root), true);
    rmSync(marker);
    assert.equal(generatorWouldReconfigure(build, generated, root), true);
});
