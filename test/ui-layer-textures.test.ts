...
test("literal box shadows retain complete layers on elements and generated boxes", () => {
    const compile = (shadow: string) => compileSource(`import {createEngine} from "@babylonjs/lite";
    ...
    for (const shadow of ["none", "inset 0 0 0 2px blue", "0 0 var(--ink, rgba(20,30,40,.5))", "0 0 var(--ink)", "0 0 var(--2Ink, var(--Fallback, red))", "#1234 -2px 3px", "1px 2px 3px -4px red inset",
        ...
        assert.doesNotMatch(result.manifest.adaptations.find(item => item.id === "substituted-ui-runtime")?.nativeSemantics ?? "", /degradation[^.]*box-shadow/);
    }
    for (const shadow of ["", "inset inset 0 0 red", "red blue 0 0", "0 0", "0 0 -1px red", "0 0 1% blue",
        "red 0 0,", "0 red 0", "0 0 url(mask.svg)", "0 0 var(--shadow, url(mask.svg))", "none, 0 0 red", "0 0 currentColor"]) {
