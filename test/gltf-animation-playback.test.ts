...
interface Controller {
    ...
    _setMask(this: void, mask: unknown): void;
}
interface Group {
    currentTime: number;
    isPlaying: boolean;
...
const scenarios: Array<{
...
}> = [
    {
        ...
        ops: [
            { op: "tick", value: 250 },
            { op: "tick", value: 250, engine: true },
            { op: "time", value: 3.75 },
            { op: "tick", value: 250 },
            { op: "speed", value: -2 },
            { op: "time", value: 0.25 },
            { op: "tick", value: 500 },
            { op: "time", value: 4.75, playing: false },
            { op: "tick", value: 0 },
            { op: "time", value: 1.25, stopped: true },
            { op: "tick", value: 700, engine: true },
            { op: "seek", value: 540 },
            { op: "seek", value: 540, engine: true },
            {
                op: "time",
                value: 1,
                stopped: false,
    ...
    },
    {
        ...
        ops: [
            ...
            { op: "time", value: 4.75 },
            { op: "tick", value: 0 },
            { op: "time", value: 0.25, playing: true },
            { op: "speed", value: -2 },
            { op: "weighted", value: 500 },
            { op: "time", value: -2, loop: false },
            { op: "core", value: 0 },
            { op: "time", value: 6, playing: false },
            { op: "core", value: 0 },
        ],
    },
];
...
function sourceResult(context: LoweringContext): unknown[] {
    ...
    const body =
        ...
        "\n" +
        context
            .functionDeclaration(mixerModule, "advanceGroupTime")
            .declaration.getText();
    ...
    return scenarios.map((scenario) => {
        ...
        const group = runtime.createAnimationGroups({
            ...
            nodes: [node],
            skeletons: [],
            morphBindings: scenario.requiresEngine ? [{ nodeIdx: 0 }] : [],
            nodeTargets: [],
            nodeNames: [],
        })[0]!;
        current = group;
        ...
        group._ctrl._setMask = (value) => {
            events.push(["mask"]);
            mask(value);
        };
        return scenario.ops.map((operation) => {
            if (operation.playing !== undefined)
                group.isPlaying = operation.playing;
            if (operation.stopped !== undefined)
                group._stopped = operation.stopped;
            if (operation.loop !== undefined)
                group.loopAnimation = operation.loop;
            if (operation.managed === true) group._animationManager = {};
            else if (operation.managed === false)
                delete group._animationManager;
            ...
            try {
                switch (operation.op) {
                    case "time":
                        group.currentTime = operation.value!;
                        break;
                    case "speed":
                        group.speedRatio = operation.value!;
                        break;
                    case "frameRate":
                        group.frameRate = operation.value!;
                        break;
                    case "tick":
                        runtime.tickAnimationImpl(
                            group,
                            operation.value!,
                            engine,
                        );
                    ...
                    case "core":
                        ...
                        break;
                    case "seek":
                        runtime.goToFrame(group, operation.value!, engine);
                        break;
                    case "weighted":
                        result = runtime.advanceGroupTime(
                            group,
                            group._gltfMixer,
                            operation.value!,
                        );
                        break;
                    default:
                        throw new Error("Unknown playback fixture operation.");
                }
            } catch (caught) {
...
function contexts(): LoweringContext[] {
    return [
        ...
        doctoredContext(
            ...
            "ctrl.time += (deltaMs / 500) * ctrl.speedRatio",
        ),
        doctoredContext(
            groupModule,
        ...
        doctoredContext(
            ...
            "group.currentTime += (deltaMs / 2000) * group.speedRatio",
        ),
        doctoredContext(
            groupModule,
...
test("playback clocks and source mutation cases exercise different controller and weighted rules", () => {
    ...
    for (const mutated of sources.slice(1))
        assert.notDeepEqual(mutated, sources[0]);
    for (const context of contexts())
        assert.ok(lowerGltfAnimationPlayback(context, true));
    assert.throws(
        () =>
        ...
        () =>
            lowerGltfAnimationPlayback(
                doctoredContext(
                    ...
                    "syncControllerFromGroup(group, group)",
                ),
            ),
        /identity|changed/,
    );
