...
import { androidCaptureSettings, verifyAndroidNativeRun } from "../src/android-capture.js";
...
test("Android canvas-only capture changes pixels, not the registered pose or frame budget", () => {
    ...
    assert.deepEqual(androidCaptureSettings(undefined), {
        captureFrame: "5", captureEnvironment: { BBLITE_MAX_FRAMES: "8", BBLITE_TEST_PASS: "1", BBLITE_GPU_BACKEND: "sdl_gpu" },
    });
    assert.deepEqual(androidCaptureSettings(undefined, { canvasOnly: true }), {
        captureFrame: "5", captureEnvironment: { BBLITE_MAX_FRAMES: "8", BBLITE_TEST_PASS: "1", BBLITE_GPU_BACKEND: "sdl_gpu", BBLITE_CAPTURE_UI: "0" },
    });
...
test("Android capture selects either backend without changing registered poses", () => {
    for (const id of ["scene1", "torus-states", "offscreen"]) {
        ...
        assert.deepEqual(dawn, { captureFrame: sdl.captureFrame,
            captureEnvironment: { ...sdl.captureEnvironment, BBLITE_GPU_BACKEND: "dawn" } });
    }
});
...
test("Android smoke requires the requested backend and one successful exit for the current run", () => {
    for (const backend of ["sdl_gpu", "dawn"] as const) {
        ...
        verifyAndroidNativeRun(`GPU backend: other run=old\nNative exit: 1 run=old\n${selected}${exit}`, "current", backend);
        for (const log of [exit, selected + selected + exit,
            selected.replace(backend, "other") + exit, selected.replace("current", "stale") + exit]) {
            assert.throws(() => verifyAndroidNativeRun(log, "current", backend), /did not select/);
        }
        for (const log of [selected, selected + exit + exit, selected + exit.replace("current", "stale")]) {
            assert.throws(() => verifyAndroidNativeRun(log, "current", backend), /one native exit/);
        }
        for (const status of ["1", "-1"]) {
            assert.throws(() => verifyAndroidNativeRun(selected + exit.replace(": 0", `: ${status}`), "current", backend), /status/);
        }
    }
});
...
test("Android packaging passes the requested target and keeps shared work serialized", () => {
    const values = new Map([
        ...
        ["--device", "emulator-5554"], ["--jobs", "3"], ["--workers", "1"], ["--backend", "dawn"],
    ]);
    ...
    for (const [flag, expected] of [["-Platform", "android"], ["-Scene", "torus-states"],
        ["-Abi", "x86_64"], ["-Sdk", "C:/SDK with spaces"], ["-Device", "emulator-5554"], ["-Jobs", "3"], ["-ExpectBackend", "DAWN"]]) {
    ...
    const defaults = mobilePackageArguments("android", "torus-states", new Map());
    assert.equal(defaults[defaults.indexOf("-ExpectBackend") + 1], "SDL_GPU");
    for (const backend of ["both", "vulkan", ""]) {
        assert.throws(() => mobilePackageArguments("android", "torus-states", new Map([["--backend", backend]])), /backend/);
    }
    assert.throws(() => mobilePackageArguments("ios", "torus-states", new Map([["--backend", "dawn"]])), /backend/);
});
