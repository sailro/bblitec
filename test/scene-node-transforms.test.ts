...
test("retained SceneNode transforms and narrowed optional handles use concrete dispatch", () => {
    ...
    assert.doesNotMatch(
        cpp,
        /scene_node_rotation_quaternion\([^;\n]+\)\.[xyzw] =/,
    );
...
test(
    "SceneNode dispatch retains concrete dirty writers and imported root bounds",
    { skip: !nativeTools },
    () => {
        const output = resolve("artifacts/scene-node-transform-check");
        mkdirSync(output, { recursive: true });
        writeFileSync(
            join(output, "scene-node-transforms.hpp"),
