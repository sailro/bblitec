...
test("pinned source cache preserves identity, projection polling, clean bytes and failure stores", async (t) => {
    ...
    const { _writePassSceneUBO } = await importPinnedModule<{
        _writePassSceneUBO(
            ...
            engine: object,
            scene: object,
            camera: PinCamera | null,
        ): void;
    }>("frame-graph/render-task.js");
    const directory = resolve("artifacts/scene-ubo-cache-check");
    mkdirSync(directory, { recursive: true });
    writeFileSync(
        join(directory, "cache.hpp"),
        new SceneUboLowerer(new LoweringContext()).cacheHeader(),
    );
    writeFileSync(
        join(directory, "camera.hpp"),
        cameraChangeKeyHeader(new LoweringContext()),
    );
    const cameraA = createArcRotateCamera(-1, 1, 10, { x: 0, y: 0, z: 0 });
    const cameraB = createArcRotateCamera(-1, 1, 10, { x: 0, y: 0, z: 0 });
    const initialProjection = {
        fov: cameraB.fov,
        near: cameraB.nearPlane,
        far: cameraB.farPlane,
    };
        ...
        _device: {
            queue: {
                writeBuffer(
                    ...
                    assert.equal(buffer, source._sceneUBO);
    ...
    const scene = {
        fog: null as { density: number } | null,
    ...
    function sample(
        ...
        const cameraVersion = key[2] ?? -1;
        const sceneVersion = key[3] ?? -1;
        assert.ok(
            typeof cameraVersion === "number" &&
        ...
        bytes.push(
            Buffer.from(new Uint8Array(source._suData.buffer)),
            Buffer.from(new Uint8Array(source._sceneUBO.buffer)),
        );
        steps.push(
    ...
    sample("first source pack", "camera = &camera_a;", () => {
        camera = cameraA;
    });
    sample("unchanged source");
    sample(
        "drawn matrix mutation remains while clean scratch is unchanged",
        "source.drawn[0] = 99.0f;",
        () => {
            source._sceneUBO[0] = 99;
        },
    );
    sample(
        "transform away and back",
        "camera_a.world_matrix_version += 2;",
        () => {
            cameraA.alpha += 0.5;
            cameraA.alpha -= 0.5;
        },
    ...
    sample(
        "projection away and back before polling",
        "camera_a.fov += .1; camera_a.fov -= .1;",
        () => {
            const fov = cameraA.fov;
            cameraA.fov += 0.1;
            cameraA.fov = fov;
        },
    );
    sample("same component value", "", () => {
        // eslint-disable-next-line no-self-assign -- Equal writes exercise the pinned setter's version policy.
        cameraA.alpha = cameraA.alpha;
        // eslint-disable-next-line no-self-assign -- Equal component writes must not dirty the camera.
        cameraA.target.x = cameraA.target.x;
    });
    sample(
        "bulk vector write dirties even equal values",
        "camera_a.world_matrix_version += 1;",
        () => {
            cameraA.target.set(0, 0, 0);
        },
    );
    sample("camera replacement", "camera = &camera_b;", () => {
        camera = cameraB;
    });
    sample(
        "same camera object projection changes",
        "camera_b.fov = .4;",
        () => {
            cameraB.fov = 0.4;
        },
    );
    sample("viewport aspect", "camera_b.viewport = Viewport{.5, 1};", () => {
        cameraB.viewport = { x: 0, y: 0, width: 0.5, height: 1 };
    });
    sample("viewport position alone does not invalidate", "", () => {
        cameraB.viewport!.x = 0.25;
    });
    sample("same effective canvas aspect", "source.canvas_size = true;", () => {
        source._config.cs = true;
    });
    sample("canvas aspect change", "engine.height = 64;", () => {
        engine.canvas.height = 64;
    });
    sample("fog replacement", "scene.fog_identity = 1;", () => {
        scene.fog = { density: 1 };
    });
    sample("fog same object mutation", "", () => {
        scene.fog!.density = 2;
    });
    sample("fog equal value replacement", "scene.fog_identity = 2;", () => {
        scene.fog = { density: 2 };
    });
    sample("environment replacement", "scene.environment_identity = 1;", () => {
        scene._envTextures = { lodGenerationScale: 0.8 };
    });
    sample("environment same object mutation", "", () => {
        scene._envTextures!.lodGenerationScale = 0.7;
    });
    sample("exposure value", "scene.exposure = 2;", () => {
        scene.imageProcessing.exposure = 2;
    });
    sample("contrast value", "scene.contrast = 2;", () => {
        scene.imageProcessing.contrast = 2;
    });
    sample("untracked image setting", "", () => {
        scene.imageProcessing.toneMappingEnabled = false;
    });
    sample(
        "pack failure after cache stores",
        "scene.exposure = 3; fail_pack = true;",
        () => {
    ...
    sample(
        "failed pack is not retried for identical key",
        "fail_pack = false;",
        () => {
            failPack = false;
        },
    ...
    sample(
        "upload failure after clean pack",
        "scene.exposure = 4; fail_write = true;",
        () => {
            scene.imageProcessing.exposure = 4;
            failWrite = true;
        },
        /write failed/,
    );
    sample(
        "failed upload is not retried for identical key",
        "fail_write = false;",
        () => {
            failWrite = false;
        },
...
`,
    ...
    const actual = execFileSync(executable, [nativeBytes], {
        encoding: "utf8",
    });
    assert.deepEqual(actual.trim().split(/\r?\n/), expected);
    assert.ok(
        readFileSync(nativeBytes).equals(Buffer.concat(bytes)),
        "Native clean/drawn byte history differs from the pin.",
    );
});
