...
function cases(context: LoweringContext) {
    ...
    return [0, 1, 2].flatMap((interpolation) =>
        [0, 1, 3].flatMap((count) =>
            [1, 3, 4, 20].flatMap((stride) =>
                [-1, 0, 0.25, 0.5, 0.75, 1.5, 3].map((time) => {
                    const input = new Float32Array(
                        [0, 0.5, 1.5].slice(0, count),
                    );
                    const values: number[] = [];
                    for (let key = 0; key < count; key++)
                        for (
                            let part = 0;
                            part < (interpolation === constants[1] ? 3 : 1);
                            part++
                        )
                            for (
                                let component = 0;
...
test("complete sampler lowering responds to source branch and cubic coefficient mutations", () => {
    const variants = contexts().map((context) => {
        assert.ok(lowerGltfAnimationEvaluator(context));
