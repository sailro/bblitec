...
test("texture headers follow changed pinned arithmetic and constants", () => {
    ...
    store.edits.set("src/frame-graph/transmission.ts", (text) =>
        text
            .replaceAll("1024", "512")
            .replace("REFRACTION_LOD_BIAS = 4", "REFRACTION_LOD_BIAS = 3"),
    );
    store.edits.set("src/resource/trilinear-anisotropic-sampler.ts", (text) =>
        text.replace("maxAnisotropy: 4", "maxAnisotropy: 2"),
    );
    store.edits.set("src/loader-env/rgbd-decode.ts", (text) =>
        text.replace("vec3f(2.2)", "vec3f(2.4)"),
    );
...
test("RGBD typed selection tolerates formatting and refuses unbound output dependencies", () => {
    ...
    const store = new EditedStore();
    store.edits.set("src/loader-env/rgbd-decode.ts", (text) =>
        text.replaceAll(";", "; /* nested /* comment */ */\n"),
    );
    assert.equal(pinnedRgbdHeader(new LoweringContext(store)), baseline);
    store.edits.set("src/loader-env/rgbd-decode.ts", (text) =>
        text.replace("c.rgb", "other.rgb"),
    );
    assert.throws(
        () => pinnedRgbdHeader(new LoweringContext(store)),
        /Unbound WGSL/,
    );
    store.edits.set("src/loader-env/rgbd-decode.ts", (text) =>
        text
            .replace("textureStore(o", "if(c.a > 0.5){textureStore(o")
            .replace(",1));}`", ",1));}}`"),
    );
    assert.throws(
        () => pinnedRgbdHeader(new LoweringContext(store)),
    ...
    store.edits.set("src/loader-env/rgbd-decode.ts", (text) =>
        text.replace("vec2u(g.x", "vec2u(g.y"),
    );
    assert.throws(
        () => pinnedRgbdHeader(new LoweringContext(store)),
        /texel mapping/,
    ...
    store.edits.set("src/loader-env/rgbd-decode.ts", (text) =>
        text.replaceAll('format: "rgba8unorm"', 'format: "rgba16float"'),
    );
    assert.throws(
        () => pinnedRgbdHeader(new LoweringContext(store)),
        /rgba8unorm/,
    ...
    assert.throws(
        () =>
            emitShaderCppExpression(
                { kind: "call", name: "unknown", arguments: [] },
                new Map(),
            ),
        /Unsupported WGSL call/,
    );
});
...
test("WGSL abstract arithmetic materializes only at a concrete float sink", () => {
    const emit = (expression: string): string[] =>
        emitShaderCppExpression(parseWgslExpression(expression), new Map())
            .components;
    assert.deepEqual(emit("vec3f(16777216.0 + 1.0 - 16777216.0)"), [
        "1.0f",
        "1.0f",
        "1.0f",
    ]);
    assert.deepEqual(emit("vec2f(1 / 255, 1.0 / 255.0)"), [
        "0.0f",
...
test(
    "generated RGBD preserves every byte-pair result and transmission mip counts",
    { skip: !tools },
    () => {
        ...
        writeFileSync(
            join(output, "pinned_rgbd.hpp"),
            pinnedRgbdHeader(context),
        );
        writeFileSync(
            join(output, "pinned_texture.hpp"),
