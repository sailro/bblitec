...
test("unknown texture descriptor and side effects refuse at their source", () => {
    assert.throws(
        ...
        /Unsupported Babylon texture property 'unknownLevel'/,
    );
    assert.throws(
        () =>
            lowerBabylonTextureSlots(
                doctoredContext(
                    ...
                    "m.unknownProperty = true;",
                ),
            ),
        /Unsupported Babylon texture property 'unknownProperty'/,
    );
});
...
test("Babylon texture slots preserve pinned defaults, guards, assignments and factory calls", async (t) => {
    ...
    const populated = {
        ...
        bumpTexture: { name: "bump.png", level: 0.27 },
    ...
    const materials = [
        populated,
        {
            ...
            specularTexture: {
                name: "default-specular.png",
                coordinatesIndex: null,
            },
            ambientTexture: {
                name: "default-ambient.png",
                level: null,
                coordinatesIndex: 0,
            },
            lightmapTexture: {
                name: "default-lightmap.png",
                level: null,
                coordinatesIndex: null,
            },
            opacityTexture: {
                name: "default-opacity.png",
                level: null,
                getAlphaFromRGB: false,
            },
            reflectionTexture: {
                name: "default-reflection.png",
                level: null,
        ...
        {
            id: "cube",
            reflectionTexture: { name: "cube", isCube: true, level: 0.87 },
        },
        {
            id: "repeated-url",
            diffuseTexture: { name: "diffuse.png", wrapU: 0, wrapV: 2 },
        },
    ];
    const document = {
        materials,
    ...
    const imported = await importPinnedModuleFetching<{
        loadBabylon(
            engine: object,
            url: string,
        ): Promise<{ entities: Array<{ material: PinMaterial }> }>;
    }>("loader-babylon/load-babylon.js", () =>
        Buffer.from(JSON.stringify(document)),
    );
    const bitmapProperty = Object.getOwnPropertyDescriptor(
        globalThis,
        "createImageBitmap",
    );
    ...
    try {
        ...
        const loaded = await imported.module.loadBabylon(
            engine,
            "https://fixture/textures.babylon",
        );
        expected = loaded.entities.map(({ material }) => [
            ...properties.map((name) => Number(material[name])),
            ...material.uvScale,
        ...
        assert.equal(
            loaded.entities[0]!.material.diffuseTexture,
            loaded.entities[5]!.material.diffuseTexture,
        );
    } finally {
        imported.release();
        fetch.mock.restore();
    ...
    assert.deepEqual(
        composed.map((material) => [
            ...properties.map((name) => Number(material[name])),
            ...(material.uvScale as number[]),
            ...textures.map((name) => (material[name] ? 1 : 0)),
        ]),
        expected,
    );
    ...
    const helpers = [
        ...
        .join("\n");
    const changed = lowerBabylonTextureSlots(
        doctoredContext(
            "src/loader-babylon/load-babylon.ts",
            "if (t.hasAlpha) {\n                m.alphaCutOff = 0.4;",
            "if (!t.hasAlpha) {\n                m.alphaCutOff = 0.6;",
