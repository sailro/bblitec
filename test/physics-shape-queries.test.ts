...
test(
    "shape queries preserve live cylinder rotation, closest-feature ties and filters",
    { skip: !tools },
    async () => {
        ...
        runNativeFixtureCompiler(tools!, [
            ...
            "/external:W0",
            "test/fixtures/physics-queries-check.cpp",
            "/link",
        ...
        const cylinder = hknp.HP_Shape_CreateCylinder(
            [0, -1, 0],
            [0, 1, 0],
            0.5,
        )[1];
        const capsule = hknp.HP_Shape_CreateCapsule(
            [0, -0.5, 0],
            [0, 0.5, 0],
            0.5,
        )[1];
        ...
        try {
            ...
            for (const angle of [0, 0.2, 0.6, 1.1]) {
                ...
                hknp.HP_World_ShapeProximityWithCollector(world, collector, [
                    cylinder,
                    [-1, 2.5, 0],
                    rotation,
                    10,
                ...
                const proximity =
                    hknp.HP_QueryCollector_GetShapeProximityResult(
                        collector,
                        0,
                    )[1];
                hknp.HP_World_ShapeCastWithCollector(world, collector, [
                    cylinder,
                    rotation,
                    [-1, 2.5, 0],
                    [4, 2.5, 0],
                ...
                const cast = hknp.HP_QueryCollector_GetShapeCastResult(
                    collector,
                    0,
                )[1];
                reference.push(
                    [proximity, cast].map((hit) => [
                        1,
                        hit[0],
                        ...hit[1][3],
                        ...hit[2][3],
        ...
        const errors = native.map((queries, pose) =>
            queries.map((result, query) =>
                result.map((value, lane) =>
                    Math.abs(value - reference[pose]![query]![lane]!),
                ),
            ),
        );
        writeFileSync(
        ...
        assert(
            Math.max(...errors.flat(2)) < 0.005,
            `Physics query error ${Math.max(...errors.flat(2))}`,
        );
    },
);
...
test("scene49 reaches shape queries without replacing authored query and picking flow", () => {
    ...
    const result = compileSource(readFileSync(path, "utf8"), {
        fileName: path,
        search: "?capture",
    });
