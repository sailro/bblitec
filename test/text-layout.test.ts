...
test("native live layout matches the pinned shaper and layout over editing, wrapping and paragraph batches", async (t) => {
    ...
    const { layoutText } = await importPinnedModule<{
        layoutText(
            this: void,
            font: unknown,
            text: string,
            ...
            _glyphs: { glyphId: number; x: number; y: number }[];
            _width: number;
            _height: number;
            _pixelsPerFontUnit: number;
        };
    }>("text/layout.js");
    ...
    const cases = inputs.flatMap((text) =>
    ...
    const expected = cases.map((input) => {
        ...
        return [
            result._width,
            result._height,
            result._pixelsPerFontUnit,
            ...result._glyphs.flatMap((glyph) => [
                glyph.glyphId,
                glyph.x,
                glyph.y,
            ]),
...
`,
    ...
    for (const [i, row] of expected.entries()) {
        assert.equal(
            actual[i]!.length,
            row.length,
            `${JSON.stringify(cases[i])}: glyph count`,
        );
        for (const [j, value] of row.entries())
            assert.ok(
                Math.abs(actual[i]![j]! - value) < 1e-12,
                `${JSON.stringify(cases[i])} lane ${j}: ${actual[i]![j]} versus ${value}`,
            );
    }
});
