...
test("standalone text source and native agree on affine uploads, layer order, shared data and bundle invalidation", (t) => {
    ...
    for (const [name, text] of Object.entries(headers)) {
        ...
        writeFileSync(resolve(directory, "bblite", `${name}.hpp`), text);
    }
    ...
    const constants = (path: string, name: string) => {
        const f = c.sourceFile(path);
        return c.numericValue(c.variableInitializer(f, name), f);
    };
    ...
    const write = (
        ...
        event("write", r.id, offset, count);
        writes.push(Buffer.from(buffer.slice(begin, begin + count)));
    };
    const drawCommands = () => {
        const commands: unknown[][] = [];
        return {
            setPipeline: (p: { id: number }) =>
                commands.push(["pipeline", p.id]),
            setVertexBuffer: (slot: number, p: { id: number }) =>
                commands.push(["vertex", slot, p.id]),
            setBindGroup: (slot: number, p: { id: number }) => {
                assert.equal(slot, 0);
                commands.push(["bind", p.id]);
            },
            draw: (...args: number[]) => commands.push(["draw", ...args]),
            finish: () => commands,
        };
    };
    let bundles = 0;
    ...
    const engine = {
        ...
        _currentEncoder: {
            beginRenderPass: ({
                ...
                assert.equal(color.storeOp, "store");
                event(
                    "pass",
                    +(color.loadOp === "clear"),
                    ...Object.values(color.clearValue),
                );
                return {
                    executeBundles: (bundles: unknown[][][]) =>
                        bundles.forEach((b) => b.forEach((e) => event(...e))),
                    end: () => event("end"),
                };
            },
        },
    ...
    const pipelines = {
        ...
        _cache: {
            _bindGroupLayout: { id: 1003 },
            _quadVertexBuffer: { id: 1004 },
        },
    };
    ...
    interface Gpu {
        ...
        _bindGroupCache: unknown[];
    }
    interface Renderer {
        _update(): void;
        _record(): number;
        _layers: Layer[];
        _layerGpu: Map<Layer, Gpu>;
        _disposed: boolean;
    }
    interface PinnedFunctions {
        createTextLayer(input: typeof data): Layer;
    ...
    const group = (key: string, start: number, count: number) => ({
    ...
    const a = api.createTextLayer(data),
        b = api.createTextLayer(data);
    a.positionPx = { x: 10, y: 20 };
    a.order = 2;
    b.positionPx = { x: 250, y: 50 };
    b.scale = 0.75;
    b.order = 1;
    const canvas = { width: 1280, height: 720 },
        rr = api.createTextRenderer(
            { engine, canvas, format: "bgra8unorm", scRT: { _colorView: {} } },
            { layers: [a, b], clearValue: { r: 0.1, g: 0.2, b: 0.3, a: 1 } },
        );
    const update = () => rr._update(),
        draw = () => event("draws", rr._record());
    const record = () =>
        rr._layers.forEach((l) => {
            const g = rr._layerGpu.get(l)!;
            event(
                "state",
                l === a ? "a" : "b",
                g._instanceCap,
                g._uploadedDataVersion,
                g._uploadedStyleVersion,
                g._uploadedViewportW,
                g._uploadedViewportH,
                g._bundleLayoutVersion,
                g._bundleDrawCalls,
                g._bindGroupCache.length,
            );
        });
    ...
    assert.equal(bundles, 2, "steady state reuses bundles");
    act(
        "a->position_px={17.125,-21};a->rotation_rad=std::numbers::pi/7;a->scale=1.25;a->opacity=.7;a->coverage_gamma=2.2;update();draw();record();",
        () => {
            a.positionPx = { x: 17.125, y: -21 };
            a.rotationRad = Math.PI / 7;
            a.scale = 1.25;
            a.opacity = 0.7;
            a.coverageGamma = 2.2;
            update();
    ...
    act("a->visible=true;a->order=0;update();draw();record();", () => {
        a.visible = true;
        a.order = 0;
        update();
        draw();
    ...
    assert.equal(bundles, 10);
    act("data->instance_count=0;update();draw();record();", () => {
