...
test("live text specializations refuse changed structure and retain source numeric growth", () => {
        ...
        () =>
            new TextDataUpdateLowerer(
                new ChangedContext(
                    ...
                    "function applyReplaceRun",
                    "function changedApplyReplaceRun",
                ),
            ).header(),
        /applyReplaceRun/,
    );
    assert.throws(
        ...
        /native option default/,
    );
    const changed = new TextDataUpdateLowerer(
        new ChangedContext(
            "src/text/text-data.ts",
    ...
    for (const [path, before, after, diagnostic] of [
        [
            ...
            "sampleCount: 1,",
            "sampleCount: 4,",
            /bundle descriptor|sample|shape/i,
        ],
        [
            "src/text/text-renderer.ts",
            "opts.layers.slice()",
            "opts.layers",
            /surface and layer ownership/,
        ],
        [
            "src/text/text-renderer.ts",
            "_version: 0,",
    ...
    for (const [path, before, after, diagnostic] of [
        ...
        [
            ...
            "key = {};",
            "key = {changed:true};",
            /interned group identity/,
        ],
        [
            "src/text/text-data.ts",
            "data._runRecords.has(ref)",
            "data._runRecords.has(data)",
            /run ownership/,
        ],
    ] as const)
        assert.throws(
...
test("live default text allocator matches pinned bytes, capacity, slot reuse and versions through edits", async (t) => {
    ...
    const baked = materializePinnedText(bytes, {
        live: true,
        text: "",
        fontSizePx: 48,
    ...
    const observe = () => {
        ...
        const sourceIds = new Map(
            [...group._curveSet._atlas._glyphSlots].map(([id, slot]) => [
                slot._index,
                id,
            ]),
        );
        const words = new Uint32Array(
            ...
            data._instanceCount * 3,
        );
        const instances = Array.from(words, (word, i) =>
            i % 3 !== 2 || word === 0xffffffff
                ? word
                : (baked.live!.glyphSlots[sourceIds.get(word & 0xffff)!]! |
                      (word & 0xffff0000)) >>>
                  0,
        );
        return {
            ...
            height: data.height,
            instances,
            styles: Array.from(new Uint32Array(data._styles.buffer)),
            capacity: data._instances.length,
            count: data._instanceCount,
            styleCount: data._styleCount,
            version: data._version,
            styleVersion: data._styleVersion,
            layoutVersion: data._layoutVersion,
            ...
            free: group._freeSlots.slice(),
            weighted: typeof group._groupKey !== "string",
        };
    };
    const expected = inputs.map((text) => {
        if (data) {
            data._dirtyStart = data._dirtyEnd = 0;
            updateDefaultTextData(data, text);
        } else
    ...
    const act = (cpp: string, action: () => void) => {
        ...
        actions.push(`data->dirty_start=0;data->dirty_end=0;${cpp};record();`);
    };
    act('bbl::update_default_text_data(data,"AV ffi")', () =>
        updateDefaultTextData(data!, "AV ffi"),
    );
    for (const offset of [0, 20, 20])
        act(`bbl::set_font_weight_offset(data,0.0,${offset})`, () =>
            setFontWeightOffset(data!, 0, offset),
        );
    act(
        "bbl::replace_default_text_run(data,data->live->runs->front(),bbl::clone_text_run(data->live->runs->front(),bbl::js::Tuple<4>{0.1,0.6,0.3,1}))",
        () => {
            const previous = data!.runs[0]!;
            updateTextData(data!, {
                update: "replaceRun",
                previous,
                run: { ...previous, defaultColor: [0.1, 0.6, 0.3, 1] },
            });
        },
    );
    act("bbl::set_font_weight_offset(data,data->live->runs->front(),33)", () =>
        setFontWeightOffset(data!, data!.runs[0]!, 33),
    );
    act('bbl::update_default_text_data(data,"Another wrapped line")', () =>
        updateDefaultTextData(data!, "Another wrapped line"),
    );
    for (const offset of [1e300, -10, NaN, 40, 0])
        act(
            `bbl::set_font_weight_offset(data,0.0,${Number.isNaN(offset) ? "std::numeric_limits<double>::quiet_NaN()" : offset})`,
            () => setFontWeightOffset(data!, 0, offset),
        );
...
`,
    ...
    for (const [i, row] of expected.entries())
        assert.deepEqual(
            actual[i],
            row,
            `Edit ${i}: ${JSON.stringify(inputs[i])}`,
        );
});
