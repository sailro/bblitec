...
test("appearance and logical spacing preserve stylesheet and inline declarations",()=>{
    ...
    const styleWrites=result.cpp.split('\n').filter(line=>line.includes('bbl::ui_set_attribute')||line.includes('bbl::ui_set_style_property')).join('\n');
    assert.doesNotMatch(styleWrites,/-webkit-appearance/);
    for(const css of ['appearance:menulist','padding-inline:-1px','padding-inline:auto','margin-inline:1px 2px 3px','padding-block-start:1px 2px'])
        assert.throws(()=>compileSource(`import {createEngine} from '@babylonjs/lite';await createEngine({});const panel=document.createElement('div');panel.style.cssText=${JSON.stringify(css)};document.body.appendChild(panel);`),/Retained UI style property/);
});

test("constructed text, password and range controls share native type assignment",()=>{
    for(const type of ['text','password','range']) {
        const result=compileSource(`import {createEngine} from '@babylonjs/lite';await createEngine({});const input=document.createElement('input');input.type=${JSON.stringify(type.toUpperCase())};document.body.appendChild(input);`);
        assert.match(result.cpp,new RegExp(`ui_set_attribute[^\\n]+"type", "${type}"`));
