...
test("the same ownership and blending assertions pass on the pinned implementation", async () => {
    ...
    const javascript = ts.transpileModule(
        program.replace(/import\s*\{[^}]+\}\s*from\s*"@babylonjs\/lite";/, ""),
        {
            compilerOptions: {
    ...
    await run(
        ...names.map((name) =>
            name === "createEngine" ? async () => ({}) : pin[name],
        ),
    );
});
...
test("data bindings refuse missing, readonly and nonnumeric leaves", () => {
    for (const [declaration, path, diagnostic] of [
        [
            "const target: { position: { y: number } } = { position: { y: 0 } };",
            "position.x",
            /has no field 'x'/,
        ],
        [
            "const target: { readonly value: number } = { value: 0 };",
            "value",
            /mutable numeric data field/,
        ],
        [
...
test(
    "native seeking and ticking write the retained data owner",
    { skip: !tools },
    () => {
        ...
        const lowered = new AnimationLowerer(
            new LoweringContext(),
        ).lowerPropertyAnimation({ managedGroups: true, blending: true });
        ...
        assert.match(
            ...
            /property-animation-data-check: ok/,
        );
    },
);
