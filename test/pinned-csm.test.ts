...
class ChangedCsmContext extends LoweringContext {
    ...
    public override sourceFile(modulePath: string): ts.SourceFile {
        ...
        this.changed ??= ts.createSourceFile(
            modulePath,
            this.change(source.text),
...
test("CSM emits the pinned fitting, coordinate, caster and receiver bodies", () => {
    ...
    assert.match(
        header,
        /csm_compute_cascades\(\s*aspect, camera, light, cfg, casters, scratch,/,
    );
    assert.match(
        header,
    ...
    assert.doesNotMatch(
        header,
        /const auto transform_point|caster_min_x|previous_split/,
    );
});
...
test("CSM lowers changed arithmetic and renamed aliases instead of asserting a transcript", () => {
    ...
    const changed = pinnedCsmFunctions(
        new ChangedCsmContext((source) =>
            source
                .replace(
    ...
    const geometry = pinnedCsmFunctions(
        new ChangedCsmContext((source) =>
            source.replace(
                "const ix = mats[o]! * lx",
                "const ix = 1.125 * mats[o]! * lx",
            ),
        ),
    );
        ...
        () =>
            pinnedCsmFunctions(
                new ChangedCsmContext((source) =>
                    source.replace(
                        "const d = cfg._lambda * (log - uniform) + uniform;",
                        "const d = unloweredSplit(log, uniform);",
                    ),
                ),
            ),
        /csm-shadow-task-hooks\.ts:\d+:\d+:.*unloweredSplit/,
    );
});
            ...
            pinnedCsmFunctions(
                new ChangedCsmContext((source) =>
                    source.replace(
                        ...
                        "_invViewProj: Float64Array;",
                    ),
                ),
            ),
        /Pinned CSM storage changed/,
    );
    assert.throws(
        () =>
            pinnedCsmFunctions(
                new ChangedCsmContext((source) =>
                    source.replace(
                        ...
                        "const cache = _getThinCasterAabbCache(), extra = adjustBounds(mesh);",
                    ),
                ),
            ),
        /Unexpected work.*cache prelude/,
    );
});
...
function cppArray(
    ...
    width: "float" | "double" = "double",
): string {
    const lanes = Array.from(value);
    return `std::array<${width}, ${lanes.length}>{${lanes
        .map((lane) =>
            width === "float"
...
test(
    "compiled CSM matches the executed pin across fitting, widths and degenerate boundaries",
    {
        skip: !nativeTools,
    },
    async () => {
        const pin = await importPinnedModuleWithExports<PinCsm>(
            "shadow/csm-shadow-task-hooks.js",
        ...
        const variants = [
            {},
            { count: 1, lambda: 0 },
            { count: 2, lambda: 1, maxZ: 25 },
            { maxZ: 1 },
            { maxZ: 1 - Number.EPSILON },
            { maxZ: 100 },
            { direction: { x: 0, y: -1, z: 0 } },
            { direction: { x: 0, y: 0, z: 0 }, empty: true },
            { stabilize: true },
            { stabilize: true, worldBias: 0.25 },
            { worldBias: 0 },
            { empty: true },
            { parked: true },
        ...
        for (const [index, variant] of variants.entries()) {
            const camera = createFreeCamera(
                { x: 1000002.125, y: 3.375, z: -8.25 },
                { x: 1000000.125, y: 0.25, z: 0.125 },
            );
            camera.nearPlane = 1;
            camera.farPlane = 100;
            if (variant.viewport)
                camera.viewport = { x: 0.1, y: 0, width: 0.75, height: 0.5 };
            const aspect = (1280 / 720) * (variant.viewport ? 1.5 : 1);
            let projection = getViewProjectionMatrix(camera, aspect);
            if (variant.wide) {
                camera._vpCache = Float64Array.from(
                    projection,
                    (lane, cell) => lane + (cell === 12 ? 0.0001220703125 : 0),
                );
            ...
            const config: PinConfig = {
                ...
                _bias: 0.00005,
                _worldSpaceBias: variant.worldBias ?? null,
                _darkness: 0.125,
                _frustumEdgeFalloff: 0.03125,
                _mapSize: 1024,
                _forceRefreshEveryFrame: false,
            };
            ...
            const fitted = expected._transforms
                .map((matrix, cascade) => {
                    const biased = new Float32Array(matrix);
                    const clipBias =
                        config._worldSpaceBias === null
                            ? config._bias * 0.5
                            : pin.csmWorldBiasClipOffset(
                                  config._worldSpaceBias,
                                  expected._near[cascade]!,
                                  expected._far[cascade]!,
                              );
    ...
    ${
        index === 0
            ? `changed::CsmCascadeScratch edited_scratch(4);
    ...
    assert(edited._viewFrustumZ[0] == actual._viewFrustumZ[0] + 0.125);`
        ...
        for (const scale of [
            0,
            1e-10,
            1e-10 - Number.EPSILON * 1e-10,
            1,
            -0.125,
        ]) {
            const input = Float64Array.of(
                scale,
                0,
                0,
                0,
    ...
    same(inverse, ${cppArray(expected, "float")});
        ...
        for (const scale of [0, 1e-12, 1e-9, 1.0000001e-9, 1]) {
            ...
            const expected = pin._castersWorldAabbInto(
                [mesh],
                pin._createCascadeScratch(1),
            );
            cases.push(
                `assert(csm_instance_contributes(${cppArray(instance, "float")}) == ${expected});`,
            );
        }
        const point: [number, number, number] = [
            1000000.0001220703, -3.125, 0.000244140625,
        ];
    ...
    same(point, ${cppArray(transformed)});
        ...
        writeFileSync(
            join(output, "csm_math.hpp"),
            pinnedCsmFunctions(context),
        );
        writeFileSync(
            join(output, "csm_changed.hpp"),
            pinnedCsmFunctions(
                new ChangedCsmContext((source) =>
                    source.replace(
                        "const d = cfg._lambda * (log - uniform) + uniform;",
...
test(
    "CSM full shadow header compiles and publishes pinned receiver bytes",
    {
        skip: !nativeTools,
    },
    async () => {
        const pin = await importPinnedModule<Pick<PinCsm, "_writeCsmUbo">>(
            "shadow/csm-shadow-task-hooks.js",
        );
        const matrices = [
        ...
        pin._writeCsmUbo(
            expected,
            {
                _transforms: matrices,
                _views: [],
                _near: [],
                _far: [],
                _viewFrustumZ: splitZ,
                _frustumLengths: lengths,
            },
            config,
        );
        ...
        writeFileSync(
            join(rendererIncludes, "csm_shadow.hpp"),
...
${matrices
    .map(
        (
            matrix,
            index,
        ) => `    generator.csm_cascades[${index}].transform = ${cppArray(matrix, "float")};
    generator.csm_cascades[${index}].view_frustum_z = ${cppNumber(splitZ[index]!)};
    generator.csm_cascades[${index}].frustum_length = ${cppNumber(lengths[index]!)};`,
    )
    .join("\n")}
