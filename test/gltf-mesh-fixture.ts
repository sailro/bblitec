...
export function readPackedGltfAttribute(
    document: JsonObject,
    binary: Buffer,
    accessor: number,
): number[] {
    ...
    const components =
        value.type === "SCALAR" ? 1 : Number(String(value.type).slice(3));
    ...
    return Array.from({ length: count * components }, (_, index) => {
        const offset =
            ...
            (index % components) * width;
        return value.componentType === 5123
            ? binary.readUInt16LE(offset)
            : value.componentType === 5125
