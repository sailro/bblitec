...
test("Dawn mesh teardown releases bindings before resources and shared layouts", (t) => {
    ...
    writeFileSync(
        ...
        [
            ...
            cppRecord(source, "struct DawnSharedComposedMaterialTextures {"),
            cppRecord(source, "struct DawnSharedMaterialTextures {"),
            "using DawnSharedShaderMaterialTextures = DawnSharedMaterialTextures;",
            ...[
                "DawnMeshBindings",
                "DawnShaderBindings",
                "DawnShaderBindingKey",
                "DawnDrawResources",
            ].map((name) => cppRecord(source, `struct ${name} {`)),
    ...
    writeFileSync(
        join(output, "release-helpers.hpp"),
        [
            "template <typename Shared>\n" +
                cppFunction(shared, "inline void release_shared_user("),
            ...["prune_unused_shared", "release_all_shared"].map(
                (name) =>
    ...
    writeFileSync(
        join(output, "release-methods.hpp"),
        [
            "void release_gpu_resources(DawnDrawResources&",
            "void release_gpu_resources(DawnMeshResources&",
            "void release_meshes(",
