...
const source = (
    body: string,
) => `import { createEngine, createTextRenderer, registerTextRenderer,
...
const others = [
    [
        "renderer:scene",
        "const scene = createSceneContext(engine); scene.camera = createFreeCamera({x:0,y:0,z:-5}, {x:0,y:0,z:0}); const box = createBox(engine); box.material = createStandardMaterial(); addToScene(scene, box); registerScene(scene);",
    ],
    [
        "renderer:sprite",
        "registerSpriteRenderer(createSpriteRenderer(engine, {layers:[]}));",
    ],
    [
        "renderer:frame-graph",
        "registerFrameGraphContext(createFrameGraphContext(engine));",
    ],
    [
        "renderer:effect",
...
test("standalone text refuses mixed context registration in either order", () => {
    for (const [feature, other] of others) {
        ...
        assert(
            compatible.manifest.features.includes(feature),
            `${feature} reaches its ordinary driver`,
        );
        for (const body of [text + other, other + text]) {
            assert.throws(
                () => compileSource(source(body)),
                (error) => {
                    assert(error instanceof Error);
                    assert.match(
                        error.message,
                        /Standalone text rendering cannot be combined/,
                    );
...
test("standalone text admission retains both pinned text scene paths", () => {
    for (const [id, standalone] of [
        ...
        assert.equal(
            result.manifest.features.includes("renderer:scene"),
            !standalone,
        );
    }
});
...
test("unregistered renderer factories do not create mixed rendering contexts", () => {
    for (const [, other] of others) {
        compileSource(
            source(`createTextRenderer(engine, {layers:[]}); ${other}`),
        );
    }
    compileSource(
        source(`${text} const scene = createSceneContext(engine);
