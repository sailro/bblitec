...
test("a native graphics availability guard retains the intended scene compilation",()=>{
    const directory=resolve("artifacts/graphics-availability/scene");mkdirSync(directory,{recursive:true});
    writeFileSync(join(directory,"worker.ts"),"self.close();");
    const scene=readFileSync("examples/primitives.ts","utf8").replace('    const canvas =', '    if (!navigator.gpu) return;\n    const canvas =');
    const result=compileSource('const worker=new Worker(new URL("./worker.ts",import.meta.url),{type:"module"});worker.terminate();'+scene,
...
for(const present of [false,true])test(`graphics capability is ${present?"present":"absent"} on the realm and its workers`,t=>{
    const directory=resolve("artifacts/graphics-availability",present?"present":"absent");mkdirSync(directory,{recursive:true});
    writeFileSync(join(directory,"worker.ts"),`self.addEventListener("message",()=>{const status:{available:boolean;kind:string}={available:!!navigator.gpu,kind:typeof navigator.gpu};self.postMessage(status);});`);
    ...
    if(present){
        const declaration="int main() {", realm="bbl::pal::WorkerRealm realm(loop);";
        assert.ok(program.includes(declaration)&&program.includes(realm));
        program=program.replace(declaration,`struct GraphicsFixtureServices final:bbl::pal::HostServices{const void* graphics_identity()const override{return this;}};\n${declaration}`)
            .replace(realm,'bbl::pal::WorkerRealm realm(loop,"",std::make_shared<GraphicsFixtureServices>());');
    }
    const cpp=join(directory,"check.cpp"),exe=join(directory,"check.exe");writeFileSync(cpp,program);
