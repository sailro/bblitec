...
function fixture() {
                ...
                lights: [
                    {
                        ...
                        irradianceCoefficients: Array.from(
                            { length: 9 },
                            (_, index) => [
                                index + 0.25,
                                index + 0.5,
    ...
    for (let index = 0; index < 12; index++) {
        ...
        for (let pixel = 0; pixel < width * width; pixel++)
            png.data.set([index + 20, index + 40, index + 60, 255], pixel * 4);
    ...
    const prefix = Buffer.alloc(12, 123),
        bin = Buffer.concat([prefix, ...parts]);
    source.document.bufferViews = views;
    source.document.buffers = [{ byteLength: offset }];
    source.document.images = images;
    return {
        document: source.document,
        bin: new DataView(bin.buffer, bin.byteOffset + prefix.length, offset),
        faceBytes,
    };
}
...
function faceData(document: JsonObject, binary: Buffer, view: number): Buffer {
    ...
    return binary.subarray(
        Number(record.byteOffset),
        Number(record.byteOffset) + Number(record.byteLength),
    );
}
...
test("source IBL selection, image slices, compute copies, uniforms and deferred writes are packaged", async () => {
    ...
    assert.deepEqual(
        textures.faces.map((face) =>
            faceData(document, binary, face.bufferView),
        ),
        faceBytes,
    );
    const harmonics = readPackedGltfAttribute(
        document,
        binary,
        textures.harmonics,
    );
    ...
    assert.deepEqual(
        harmonics.filter((_, index) => index % 4 === 3),
        Array(9).fill(0),
    );
    assert.deepEqual(ibl.setup, [
        { kind: "textures", index: 0 },
        { kind: "toneMappingEnabled", value: true },
        { kind: "exposure", value: 0.8 },
        { kind: "contrast", value: 1.2 },
    ]);
});
...
test("source IBL activation and selected-scene guards decide whether resources exist", async () => {
    ...
    assert.deepEqual(
        (await gltfMeshPlan({ ...document, extensionsUsed: [] }, bin)).ibl,
        { textures: [], setup: [] },
    );
    assert.deepEqual((await gltfMeshPlan({ ...document, scene: 1 }, bin)).ibl, {
        textures: [],
        setup: [],
    });
    ...
    const light = asRecords(
        asObject(asObject(without.extensions)?.EXT_lights_image_based)?.lights,
    )[0]!;
    delete light.irradianceCoefficients;
    assert.deepEqual((await gltfMeshPlan(without, bin)).ibl, {
        textures: [],
        setup: [],
    });
});
...
test("source image order and cube destination layers drive native face order", async () => {
    const first = fixture();
    const reversed = await packageGltfMeshPlan(
        first.document,
        first.bin,
        doctoredContext(
            featureModule,
            "light.specularImages.flat()",
            "light.specularImages.map(mip => [...mip].reverse()).flat()",
        ),
    );
    const plan = packagedGltfMeshPlan(first.document).ibl;
    assert.deepEqual(
        plan.textures[0]!.faces.map((face) =>
            faceData(first.document, reversed, face.bufferView),
        ),
        [
            ...first.faceBytes.slice(0, 6).reverse(),
            ...first.faceBytes.slice(6).reverse(),
        ],
    );
    const second = fixture();
    const reordered = await packageGltfMeshPlan(
        second.document,
        second.bin,
        doctoredContext(uploadModule, "z: face", "z: 5 - face"),
    );
    assert.deepEqual(
        packagedGltfMeshPlan(second.document).ibl.textures[0]!.faces.map(
            (face) => faceData(second.document, reordered, face.bufferView),
        ),
        [
            ...second.faceBytes.slice(0, 6).reverse(),
            ...second.faceBytes.slice(6).reverse(),
        ],
    );
});
...
test("source numeric stores and ordered scene assignments replace old lighting defaults", async () => {
    ...
    assert.deepEqual(changed.ibl.setup[1], {
        kind: "toneMappingEnabled",
        value: false,
    });
    const rotated = await gltfMeshPlan(
        document,
        bin,
        doctoredContext(
            featureModule,
            "const envRotationY = light.rotation ? envYawFromQuaternion(light.rotation) : 0;",
            "const envRotationY = 0.75;",
        ),
    );
    assert.deepEqual(rotated.ibl.setup[1], { kind: "rotation", value: 0.75 });
    const reordered = await gltfMeshPlan(
        document,
        bin,
        doctoredContext(
            featureModule,
            "scene.imageProcessing.exposure = 0.8;",
            "scene.imageProcessing.contrast = 2; scene.imageProcessing.exposure = 0.7;",
        ),
    );
    assert.deepEqual(reordered.ibl.setup.slice(2), [
        { kind: "contrast", value: 2 },
        { kind: "exposure", value: 0.7 },
        { kind: "contrast", value: 1.2 },
    ]);
    const baseline = fixture(),
        scaled = fixture();
    const before = await packageGltfMeshPlan(baseline.document, baseline.bin);
    const after = await packageGltfMeshPlan(
        scaled.document,
        scaled.bin,
        doctoredContext(
            featureModule,
    ...
    const lanes = (doc: JsonObject, bytes: Buffer) =>
        readPackedGltfAttribute(
            doc,
            bytes,
            packagedGltfMeshPlan(doc).ibl.textures[0]!.harmonics,
        );
    assert.deepEqual(
        lanes(scaled.document, after),
        lanes(baseline.document, before).map((value) => value * 2),
    );
    const altered = fixture();
    const alteredBytes = await packageGltfMeshPlan(
        altered.document,
        altered.bin,
        doctoredContext(
            assemblyModule,
            "data.set(sh, 40);",
            "data.set(sh.map(value => value * 2), 40);",
        ),
    );
    assert.deepEqual(
        lanes(altered.document, alteredBytes),
        lanes(baseline.document, before).map((value) => value * 2),
    );
});
...
test("unrepresented IBL GPU and runtime-state dependencies refuse", async () => {
    ...
    for (const [module, needle, replacement, error] of [
        ...
        [
            uploadModule,
            "[FLIP_Y_CONSTANT_ID]: 1",
            "[FLIP_Y_CONSTANT_ID]: 0",
            /RGBD decode contract/,
        ],
        [uploadModule, "vec3f(2.2)", "vec3f(2.4)", /compute pipeline/],
        [
            uploadModule,
            "device.queue.submit([encoder.finish()]);",
            "if (face === 5) device.queue.submit([encoder.finish()]);",
            /Incomplete glTF IBL cube faces/,
        ],
        [
            assemblyModule,
            "const size = 256;",
        ...
        [
            assemblyModule,
            "scene._environmentRotation ?? 0",
            "(scene._environmentRotation ?? 0) * 2",
            /arithmetic on the runtime/,
        ],
        [
            featureModule,
            "scene.imageProcessing.exposure = 0.8;",
            "scene.imageProcessing.exposure *= 0.8;",
            /reads existing image processing/,
        ],
        [
            featureModule,
            "scene._envTextures = textures;",
            "scene._envTextures = scene._envTextures || textures;",
            /reads prior environment state/,
        ],
    ] as const)
...
test("native IBL attachment preserves unwritten scene state, source uniform bits and resource identity", async (t) => {
    ...
    const contexts = [
        ...
        doctoredContext(
            featureModule,
            "scene.imageProcessing.exposure = 0.8;",
            "scene.imageProcessing.exposure = 0.7;",
        ),
    ];
    const cases = [];
    for (const context of contexts) {
        ...
        cases.push({
            ibl: plan.ibl,
            harmonics: plan.ibl.textures.map((texture) => ({
                index: texture.harmonics,
                bits: [
                    ...new Uint32Array(
                        Float32Array.from(
                            readPackedGltfAttribute(
    ...
    const directory = resolve("artifacts/test-gltf-ibl-plan");
...
test("packaged IBL storage refuses invalid indices, missing fields and unknown scene writes", async () => {
    ...
    for (const value of [
        undefined,
        { ...ibl, setup: [{ kind: "unknown" }] },
        { ...ibl, setup: [{ kind: "textures", index: 7 }] },
        { ...ibl, textures: [{ ...ibl.textures[0], harmonics: 9999 }] },
        { ...ibl, textures: [{ ...ibl.textures[0], faces: [] }] },
    ])
        assert.throws(
            () =>
                packagedGltfMeshPlan({
                    ...document,
                    [GLTF_MESH_PLAN]: { ...meshPlan, ibl: value },
                }),
            /Invalid|missing/,
        );
});
