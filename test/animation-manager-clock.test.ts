...
test("exact standalone manager source retains both authored modes and live client dimensions", () => {
    ...
    for (const search of ["?seekTime=1", ""]) {
        ...
        assert.equal(
            result.cpp.includes("bbl::start_animation_manager("),
            search === "",
        );
    }
});
...
test("autonomous managers refuse persistent application RAF interleavings in both source orders", () => {
    ...
    const recurring =
        "function tick() { requestAnimationFrame(tick); } requestAnimationFrame(tick);";
...
test("pinned manager distinguishes manual updates, first ticks, fixed steps and cancellation", async () => {
    ...
    try {
        ...
        const manager = pin.createAnimationManager({
            onUpdate: (step) =>
                observations.push({ value: target.value, step }),
        });
        ...
        const group = pin.createPropertyAnimationGroup(manager, target, clip, {
            loop: false,
        });
        ...
        frame(2250);
        assert.deepEqual(
            observations.slice(3).map((row) => row.step),
            [0, 250],
        );
        ...
        const fixed = pin.createAnimationManager({
            fixedDeltaMs: 100,
            onUpdate: (step) =>
                observations.push({ value: target.value, step }),
        });
        ...
        let ordered = 0;
        const observing = pin.createAnimationManager({
            onUpdate: () => assert.equal(ordered, 1),
        });
        ...
        const restarting = pin.createAnimationManager({
            onUpdate: () => {
                if (++restartedCalls === 1) {
...
test(
    "property-only scene registration seeks late managers and freezes their groups",
    { skip: !tools },
    () => {
...
test(
    "autonomous notifications retain block-local objects for inline and named callbacks",
    { skip: !tools },
    () => {
        ...
        const callback =
            "() => { record.value++; if (record.value !== 2) throw new Error('Notification lost its block-local object'); }";
...
test(
    "native manager follows clock, callback order, cancellation and restart contracts",
    { skip: !tools },
    () => {
        ...
        assert.match(
            ...
            /animation-manager-clock-check: ok/,
        );
    },
);
