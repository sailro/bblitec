...
test(
    "iOS system emoji produce visible premultiplied glyphs with stable metrics, cache and ownership",
    {
        skip:
    ...
    () => {
        ...
        assert.ok(
            tools.cmake && tools.cxx && tools.vcpkgToolchain,
            "CMake, Clang and vcpkg are required.",
        );
        ...
        assert.equal(
            device.state,
            "Booted",
            "Boot the selected Simulator before running the font fixture.",
        );
        ...
        const triplet =
            architecture === "arm64"
                ? "arm64-ios-simulator-bblite"
                : "x64-ios-simulator-bblite";
        ...
        const configure = run(tools.cmake, [
            "-S",
            "test/fixtures/ui-color-font",
            "-B",
            output,
            "-G",
            "Ninja",
            ...
            `-DCMAKE_TOOLCHAIN_FILE=${tools.vcpkgToolchain}`,
            `-DVCPKG_TARGET_TRIPLET=${triplet}`,
            `-DVCPKG_OVERLAY_TRIPLETS=${resolve("native/triplets")}`,
            `-DVCPKG_INSTALLED_DIR=${resolve("artifacts/ios-vcpkg")}`,
        ...
        writeFileSync(
            ...
            run(tools.cmake, ["--build", output, "--parallel", "3"]),
        );
        ...
        assert.equal(
            result.match(/^emoji=/gm)?.length,
            12,
            "Each of four menu emoji must render at all three sizes.",
        );
