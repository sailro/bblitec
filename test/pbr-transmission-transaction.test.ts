...
const rebuildModule = "src/scene/scene-rebuild.ts",
    transmissionModule = "src/frame-graph/transmission.ts";
interface Scenario {
    mode: "direct" | "success" | "throw" | "dispose";
    selected: boolean;
    enabled: boolean;
}
...
function sourceOracle(
    ...
    return createJavaScriptFunction(source)() as (
        input: Scenario,
    ) => Promise<unknown>;
}
...
test("transmission transactions match source rebuild success, failure, disposal and receiver fallback", async (t) => {
    ...
    const receiver = original.findNodes(
        original.sourceFile(rebuildModule),
        (node): node is ts.BinaryExpression =>
            ts.isBinaryExpression(node) &&
            original.expressionMatchesShape(node.left, "ctx._p"),
    )[0]!;
    const returned = original
        .functionDeclaration(transmissionModule, "_t")
    ...
    const contexts = [
        ...
        doctoredContext(
            rebuildModule,
            receiver.right.getText(),
            receiver.right.getText().replace("return true", "return false"),
        ),
        doctoredContext(
            transmissionModule,
            tuple.getText(),
    ...
    for (const mode of ["direct", "success", "throw", "dispose"] as const)
        for (const selected of [false, true])
            for (const enabled of [false, true])
                inputs.push({ mode, selected, enabled });
    const cases = [];
    for (const [variant, context] of contexts.entries()) {
        const oracle = sourceOracle(context);
        for (const input of inputs)
            cases.push({ variant, input, expected: await oracle(input) });
    }
    assert.notDeepEqual(
        cases.slice(0, inputs.length).map((row) => row.expected),
        cases
            .slice(inputs.length, inputs.length * 2)
            .map((row) => row.expected),
    );
    assert.notDeepEqual(
        cases.slice(0, inputs.length).map((row) => row.expected),
        cases.slice(inputs.length * 2).map((row) => row.expected),
    );
    const directory = resolve("artifacts/test-pbr-transmission-transaction");
...
test("transmission specialization refuses runtime state outside the precomposed material cache", () => {
    for (const context of [
        ...
        doctoredContext(
            ...
            "() => enableSceneTransmissionTasks(otherScene, engine)",
        ),
    ])
        assert.throws(
            () => lowerPbrSceneHookRegistry(context),
            /specialization|rollback|commit|Expected/,
        );
});
