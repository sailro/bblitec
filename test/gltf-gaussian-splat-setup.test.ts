...
function fixture() {
    return meshPlanFixture({
        ...
        meshes: [
            {
                name: "cloud",
                primitives: [{ mode: 0, extensions: { [extension]: {} } }],
            },
        ],
    });
}
...
test("Gaussian-splat activation and row preparation execute the source registry and feature", async () => {
    const run = async (context?: LoweringContext) => {
        ...
        return {
            document,
            splats: await extractGltfGaussianSplats(
                document,
                bin,
                "fixture",
                context,
    ...
    assert.deepEqual(
        (base.document.meshes as Array<{ primitives: unknown[] }>)[0]!
            .primitives,
        [],
    );
    ...
    const inactive = await run(
        doctoredContext(
            ...
            '["inactive", () => import("./gltf-feature-gaussian-splatting.js")]',
        ),
    );
    assert.equal(inactive.splats, undefined);
    assert.equal(
        (inactive.document.meshes as Array<{ primitives: unknown[] }>)[0]!
            .primitives.length,
        1,
    );
    ...
    await assert.rejects(
        run(doctoredContext(module, "return mesh;", "return {};")),
        /publishes exactly what it attaches/,
    );
});
...
test("native Gaussian setup creates fresh clouds and appends source results on every add", async (t) => {
    ...
    const cases = contexts.map((context) => {
        const { declaration } = context.methodDeclaration(
            module,
            "feature.applyAsset",
        );
        const setup = context.unwrapExpression(
            context.variableInitializer(declaration, "sceneSetup"),
        );
        assert.ok(ts.isArrowFunction(setup));
        const prepared = [
            { name: "first", buffer: new ArrayBuffer(4) },
            { name: "second", buffer: new ArrayBuffer(8) },
        ];
        const ready: unknown[] = [],
            scene = {};
        ...
        const callback = createJavaScriptFunction(
            ...
            (supplied: object, name: string, data: { data: ArrayBuffer }) => {
                assert.equal(supplied, scene);
                assert.equal(
                    data.data,
                    prepared.find((item) => item.name === name)!.buffer,
                );
                const mesh = { name, rotation: { x: 0, y: 0, z: 0 } };
                attached.push(mesh);
                return {
                    then: (resolve: (mesh: object) => unknown) => resolve(mesh),
                };
            },
            prepared,
            ready,
        ) as (scene: object) => void;
        callback(scene);
        const firstCount = ready.length;
        callback(scene);
        assert.deepEqual(ready, attached);
        assert.equal(new Set(ready).size, 4);
        return {
            firstCount,
            names: attached.map((mesh) => mesh.name),
            rotations: attached.map((mesh) => Math.fround(mesh.rotation.z)),
        };
    });
    const directory = resolve("artifacts/test-gltf-gaussian-splat-setup");
