...
test("parameterizes a 64 by 64 box grid while retaining every composition row", () => {
    ...
    assert.equal(
        result.manifest.sceneMeshes.filter((mesh) => mesh.standardMaterial)
            .length,
        4096,
    );
    assert.match(
...
test("an inlined numeric result cannot fold a written parameter to its argument", () => {
    for (const update of [
        "value += 1;",
        "value = 4;",
        "const values = new Float32Array([4]); value = values[0]!;",
    ]) {
        const result = compileSource(
            scene(
                `
...
test(
    "array-assignment and rebind controls execute with JavaScript identities",
    { skip: !nativeTools },
    () => {
...
test(
    "the compact grid executes all 4096 ordered native constructions and live coordinates",
    { skip: !nativeTools },
    () => {
        ...
        writeFileSync(
            join(output, "mesh-material-setter.hpp"),
        ...
        assert.match(
            execFileSync(executable, { encoding: "utf8" }),
            /resource-loop-runtime-check: ok/,
        );
    },
);
...
test("resource effect guards do not reserialize existing literal particle graphs", (t) => {
    ...
    t.mock.method(
        ...
        (...args: Parameters<typeof JSON.stringify>) => {
            ...
            if (
                typeof value === "object" &&
                value !== null &&
                "auditSnapshotMarker" in value &&
                value.auditSnapshotMarker === true
            )
                ++serializedGraphs;
    ...
    assert.equal(
        serializedGraphs,
        1,
        "only final manifest publication needs the graph digest",
    );
});
...
test("native asset discovery does not authorize variable glTF container construction", () => {
    const data = `data:model/gltf+json;base64,${Buffer.from(
        JSON.stringify({
            asset: { version: "2.0" },
            scenes: [{ nodes: [] }],
            nodes: [],
...
test("repeats PBR pairing and receiver facts without duplicating runtime construction", () => {
    ...
    assert.equal(result.cpp.match(/bbl::create_box\(/g)?.length, 1);
    assert.deepEqual(
        result.manifest.shadowReceiverMeshes,
...
test("a caught expansion failure cannot turn a later emission probe into success", () => {
    ...
    const budget = new StaticExpansionBudget((_node, message) => {
    ...
    const composition = new StaticExpansionBudget((_node, message) => {
        throw new Error(message);
    });
    assert.throws(
        () => composition.checkComposition(loop, 65537, 0),
        /Resource-loop composition exceeds/,
    );
    assert.throws(
        () => composition.assertWithinBudget(),
        /Resource-loop composition exceeds/,
    );
});
...
test("materializes large generation-known record members for native instance updates", () => {
    ...
    assert.equal(result.manifest.sceneMeshes.length, 1);
    assert.equal(
        result.cpp.match(/bbl::set_thin_instance_matrix\(/g)?.length,
        1,
    );
    assert.match(result.cpp, /for \(auto&&/);
});
...
test("bound-alias analysis type-checks only initializers rooted in tracked aliases", () => {
    const { checker, sourceFile } = createCompilerProgram(
        ...
        ${Array.from(
            { length: 128 },
            (_, i) => `const ignored${i} = unrelated; const scalar${i} = ${i};`,
        ).join("\n")}
    ...
    // eslint-disable-next-line @typescript-eslint/unbound-method -- Saved for receiver-aware invocation and exact restoration.
