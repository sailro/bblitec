...
test("attribution preserves source, pose, host UI and gates with independent outputs", () => {
    ...
    const { reference, outputDirectory, attribution, ...poseAndGates } =
        twin.parity!;
    const {
        reference: _reference,
        outputDirectory: _output,
        attribution: _attribution,
        ...originalPoseAndGates
    } = scene.parity!;
...
test("diagnostic reference refresh and recapture cannot overwrite the curated reference", (t) => {
    ...
    const scene = {
        ...source,
        id: "attribution-reference-fixture",
        parity: {
            ...source.parity!,
            reference: {
                kind: "source" as const,
                path: join(directory, "curated.png"),
            },
        },
    };
    const twin = attributionScene(scene);
    twin.parity!.reference.path = join(
        directory,
        "diagnostic",
        "reference.png",
    );
    ...
    assert.equal(
        readFileSync(twin.parity!.reference.path, "utf8"),
        "curated bytes",
    );
    writeFileSync(twin.parity!.reference.path, "recaptured diagnostic pose");
    assert.equal(
        readFileSync(scene.parity.reference.path, "utf8"),
        "curated bytes",
    );
    copyAttributionReferences(scene, twin);
    assert.equal(
