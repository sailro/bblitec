...
test(
    "native physics and pinned Havok consume equal time at 60 and 240 fps",
    { skip: !tools },
    async () => {
        ...
        const pinned = await importPinnedModule<{
            createHavokWorld(
                this: void,
                scene: { _beforeRender: Array<(ms: number) => void> },
                hk: typeof hknp,
                gravity: { x: number; y: number; z: number },
            ): World;
            setPhysicsTimestepMs(this: void, world: World, ms: number): void;
            onPhysicsAfterStep(
                this: void,
                world: World,
        ...
        for (const fixed of [true, false])
            for (const fps of [60, 240]) {
                const scene = {
                    _beforeRender: [] as Array<(ms: number) => void>,
                };
                const world = pinned.createHavokWorld(scene, hknp, {
                    x: 0,
                    y: 0,
                    z: 0,
                });
                ...
                try {
                    pinned.setPhysicsTimestepMs(
                        world,
                        fixed ? (1000 / 60 / 8) * 6 : 0,
                    );
                    hknp.HP_Body_SetShape(body, shape);
                    hknp.HP_Body_SetMotionType(body, hknp.MotionType.DYNAMIC);
                    hknp.HP_Body_SetMassProperties(
                        body,
                        hknp.HP_Shape_BuildMassProperties(shape)[1],
                    );
                    ...
                    let calls = 0,
                        seconds = 0;
                    pinned.onPhysicsAfterStep(world, (dt) => {
                        ++calls;
                        seconds += dt;
                    });
                    for (let frame = 0; frame < fps; ++frame) {
                        for (const callback of scene._beforeRender)
                            callback(1000 / (fixed ? 60 : fps));
                    }
        ...
        runNativeFixtureCompiler(tools!, [
            ...
            "/external:W0",
            "test/fixtures/physics-timing-check.cpp",
