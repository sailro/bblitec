...
function fixture(extra: JsonObject = {}) {
    return meshPlanFixture({
        nodes: [
            { mesh: 0, children: [1] },
            { camera: 0, translation: [1, 2, 3], scale: [2, 2, 2] },
        ...
        scenes: [{ nodes: [0] }],
        cameras: [
            { type: "perspective", perspective: { yfov: 0.55, znear: 0.02 } },
            {
                name: "outside",
                type: "perspective",
...
test("source camera construction preserves selected and baked parents and public scalar state", async () => {
    ...
    assert.deepEqual(
        ...
        [
            ["camera0", 0.55, 0.02, 1e6],
            ["outside", 0.8, 0.1, 100],
        ],
    );
    for (const camera of plan.cameras) {
        assert.deepEqual(camera.position, [0, 0, 0]);
        assert.deepEqual(camera.target, [0, 0, -1]);
        assert.equal(camera.speed, 2);
        assert.equal(camera.angularSensitivity, 2000);
        assert.equal(camera.inertia, 0.9);
        assert.equal(camera.yaw, Math.PI);
        assert.equal(camera.pitch, 0);
    }
    assert.deepEqual(
        readPackedGltfAttribute(document, binary, plan.cameras[0]!.parentWorld),
...
test("camera API reach and the actual feature predicate remain isolated between source executions", async () => {
    ...
    assert.equal(
        (await gltfMeshPlan(document, bin, undefined, enabled)).cameras.length,
        2,
    );
    assert.deepEqual((await gltfMeshPlan(document, bin)).cameras, []);
    const disabled = doctoredContext(
        module,
        "!!json.cameras?.length",
        "json.cameras?.length > 2",
    );
    assert.deepEqual(
        (await gltfMeshPlan(document, bin, disabled, enabled)).cameras,
        [],
    );
    assert.equal(
        (await gltfMeshPlan(document, bin, undefined, enabled)).cameras.length,
    ...
    const ignored = doctoredContext(
        "src/loader-gltf/load-gltf.ts",
        "_appendEnabledGltfFeatures(json, features);",
        "features.length = 0;",
    );
    assert.deepEqual(
        (await gltfMeshPlan(document, bin, ignored, enabled)).cameras,
        [],
    );
    const invalid = fixture({ cameras: [{ type: "unknown" }] });
    assert.deepEqual(
        (await gltfMeshPlan(invalid.document, invalid.bin)).cameras,
        [],
    );
...
test("source camera defaults, constructor changes, ordering and aliasing flow into the plan", async () => {
    const { document, bin } = fixture();
    const changed = async (needle: string, replacement: string) =>
        gltfMeshPlan(
            document,
            bin,
    ...
    assert.equal(
        (await changed("p.zfar ?? 1e6", "p.zfar ?? 2500")).cameras[0]!.farPlane,
        2500,
    );
    assert.equal(
        (
            await changed(
                "def.name ?? `camera${camIdx}`",
                "def.name ?? `imported${camIdx}`",
            )
        ).cameras[0]!.name,
        "imported0",
    );
    const repeated = await changed(
        "return cameras.length ? { cameras } : {};",
        "return cameras.length ? { cameras: [...cameras.reverse(), ...cameras] } : {};",
    );
    assert.deepEqual(
        repeated.cameras.map((camera) => camera.name),
        ["outside", "camera0"],
    );
    assert.deepEqual(repeated.containerCameras, [0, 1, 0, 1]);
    const constructor = await gltfMeshPlan(
        document,
        bin,
        doctoredContext(
...
test("source camera scale checks and changing-scale skip run before native packaging", async () => {
    ...
    const accessors = asRecords(document.accessors),
        bufferViews = asRecords(document.bufferViews);
    for (const [data, type, count] of [
        [new Float32Array([0, 1]), "SCALAR", 2],
        [new Float32Array([2, 2, 2, 3, 3, 3]), "VEC3", 2],
    ] as const) {
        bufferViews.push({
            buffer: 0,
    ...
    assert.deepEqual(
        plan.cameras.map((camera) => camera.name),
        ["outside"],
    );
    const ortho = fixture({
        cameras: [
            {
                type: "orthographic",
                orthographic: { xmag: 2, ymag: 1, znear: 0.1, zfar: 10 },
            },
        ],
    });
    await assert.rejects(
        gltfMeshPlan(ortho.document, ortho.bin, undefined, enabled),
        /explicit clip-plane storage/,
    );
});
...
test("camera enabling affects only subsequent loads, including repeated shared calls", () => {
    for (const wrapper of [false, true]) {
        const result =
            compileSource(`import {createEngine, loadGltf, enableGltfCameras} from "@babylonjs/lite";
        ...
        assert.deepEqual(
            result.manifest.assets.map((asset) => [
                asset.source,
                asset.containerCount,
                asset.gltfCameras === true,
            ]),
            [
                ["before.glb", 1, false],
                ["mixed.glb", 2, true],
        ...
        assert.equal(
            result.cpp.match(/bbl::load_gltf\([^\n]+, true\)/g)?.length,
            2,
        );
    ...
    assert.throws(
        () =>
            compileSource(`import {createEngine, enableGltfCameras} from "@babylonjs/lite";
        async function main() { const engine = await createEngine({}); if (Math.random() > .5) enableGltfCameras(); }`),
        /definite setup call/,
    );
});
...
test("native shared loader calls execute the camera mode at each source invocation", (t) => {
    ...
    const directory = resolve("artifacts/test-gltf-camera-order");
    mkdirSync(directory, { recursive: true });
    const headers = resolve(directory, "bblite/upstream");
    mkdirSync(headers, { recursive: true });
    writeFileSync(
        resolve(headers, "camera_math.hpp"),
        new CameraLowerer(new LoweringContext()).lowerArcRotateFactory().header,
    );
...
test("native camera transport preserves source matrices, scalars, aliases and per-load activation", async (t) => {
    ...
    for (const context of [
        ...
        doctoredContext(
            ...
            "-inverseScale, inverseScale * 2, inverseScale",
        ),
        doctoredContext(
            "src/camera/free-camera.ts",
            "speed: 2.0",
        ...
        const plan = packagedGltfMeshPlan(document);
        const matrices = plan.cameras
            .flatMap((camera) => [
                camera.parentWorld,
                ...(camera.binding ? [camera.binding.local] : []),
            ])
            .map((accessor) => ({
                accessor,
                bits: [
                    ...new Uint32Array(
                        Float32Array.from(
                            readPackedGltfAttribute(document, binary, accessor),
                        ).buffer,
                    ),
                ],
            }));
    ...
    const directory = resolve("artifacts/test-gltf-camera-plan");
    mkdirSync(directory, { recursive: true });
    writeFileSync(
