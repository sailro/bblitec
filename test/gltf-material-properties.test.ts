...
import {
    gltfCoreMaterialFields,
    lowerGltfMaterialAssembly,
} from "../src/lowering/gltf/material-assembly.js";
...
function reversedMaterialRegistry(): LoweringContext {
    ...
    const rows = source
        ...
        .filter(
            (line) =>
                line.includes('import("./gltf-ext-diffuse-transmission.js")') ||
                line.includes('import("./gltf-ext-dielectric.js")'),
        );
    ...
    return doctoredContext(
        ...
        source,
        source
            .replace(rows[0]!, "MATERIAL_ROW")
            .replace(rows[1]!, rows[0]!)
            .replace("MATERIAL_ROW", rows[1]!),
    );
}
...
function alteredMaterialSetup(): LoweringContext {
    ...
    return doctoredContext(
        ...
        source,
        source
            .replace(`includes('"texCoord":1')`, `includes('"texCoord":2')`)
            .replace("s.wrapS > 10497", "s.wrapS > 33071")
            .replace("s.magFilter === 9728", "s.magFilter === 9729")
...
function reboundMaterialReceiver(
    ...
    return doctoredContext(
        module,
        declaration.getText(),
        declaration.getText().replace(declaration.body!.getText(), body),
    );
}

/** Execute source bodies with imports bound to the same selected setter declarations. */
function executableFunctions(
    functions: readonly GltfMaterialFunction[],
): Map<string, MaterialFunction> {
    ...
    const boundaries: Record<string, unknown> = {
        _animBaseColorDefs: null,
        _baseColorMod: null,
        _registerPbrExt() {},
        _registerPbrSceneHook() {},
        _setDispersionSampleWgsl() {},
        pbrExt: {},
        stdUvTransformExt: {},
        registerPbrTransmission: {},
        DISPERSION_SAMPLE_WGSL: {},
        getPbrGroupBuilder: () => true,
        cloneTexture2D: (texture: object, fields: object) => ({
            ...texture,
            ...fields,
        }),
        engine: {},
        ...
        generateMipmaps() {},
        getCachedTex() {},
        runMatExts: (...args: unknown[]) =>
            modules["src/loader-gltf/gltf-feature-registry.ts"]!
                .runGltfMaterialFeatures!(...args),
        buildDefaultPbrTexturesExt: textures,
        buildDefaultPbrTextures: textures,
        _ensurePbrExt: () => ({
            ...modules["src/loader-gltf/gltf-pbr-builder-ext.ts"],
            buildDefaultPbrTexturesExt: textures,
        }),
        ctx: {
            _runMatExts: (...args: unknown[]) =>
                modules["src/loader-gltf/gltf-feature-registry.ts"]!
                    .runGltfMaterialFeatures!(...args),
        },
    };
    for (const target of functions) {
                ...
                const visit: ts.Visitor = (node) => {
                    if (
                        ...
                        const module = posix
                            .normalize(
                                posix.join(
                                    posix.dirname(target.module),
                                    path.text,
                                ),
                            )
                            .replace(/\.js$/, ".ts");
                        return context.factory.createElementAccessExpression(
                            context.factory.createIdentifier("runtimeImports"),
        ...
        const imports: Record<string, unknown> = {
            ...boundaries,
            runtimeImports: modules,
            ...Object.fromEntries(target.constants ?? []),
        };
        ...
        for (const candidate of functions.filter(
            (candidate) => candidate.module === target.module,
        )) {
            const executed = result.get(candidate.cpp);
            if (executed) imports[candidate.name] = executed;
        }
        ...
        createJavaScriptFunction(
            "exports",
            ...Object.keys(imports),
            transpileCommonJs(text, target.module),
        )(exported, ...Object.values(imports));
...
for (const [variant, context] of [
    ...
    [
        ...
        doctoredContext(
            ...
            "await applyGltfOptInPbrFeatures(props, mat); props.reflectance = 0.125;",
        ),
    ],
    [
        "variant-construction",
        doctoredContext(
            "src/loader-gltf/gltf-variants.ts",
            "await applyGltfOptInPbrFeatures(props, gltfMat);",
            "await applyGltfOptInPbrFeatures(props, gltfMat); props.reflectance = 0.375;",
        ),
    ],
    [
        "variant-sampler",
    ...
    test(`native material handlers and projection follow ${variant} source`, async (t) => {
        ...
        const upload = context.functionDeclaration(
            "src/loader-gltf/load-gltf.ts",
            "uploadMeshes",
        ).declaration;
        const samplerCall = context.findNodes(
            upload,
        ...
        const samplerDefaults = createJavaScriptFunction(
            `return (${samplerCall.arguments[1]!.getText()});`,
        )() as Record<string, number | string>;
        const variantUpload = context.functionDeclaration(
            "src/loader-gltf/gltf-variants.ts",
            "loadVariantMaterials",
        ).declaration;
        const variantSamplerCall = context.findNodes(
            variantUpload,
            (node): node is ts.CallExpression =>
                ts.isCallExpression(node) &&
                context.expressionMatchesShape(
        ...
        const variantSamplerDefaults = createJavaScriptFunction(
            `return (${variantSamplerCall.arguments[1]!.getText()});`,
        )() as Record<string, number | string>;
        const setupCases = [
            {},
            { materials: null },
            { materials: [] },
            { materials: [{}] },
            ...[0, 1, 2, 10, 1.25, 1e-7, -1, null].map((texCoord) => ({
                materials: [{ normalTexture: { texCoord } }],
            })),
            ...
            ...[
                ...
                [{ magFilter: 9728 }],
                [{ minFilter: null }],
                ...[9728, 9729, 9984, 9985, 9986, 9987].map((minFilter) => [
                    { minFilter },
                ]),
            ...
            [false, true].map((wrap) => {
                const input = {
                    ...document,
                    extensionsUsed: wrap ? ["KHR_texture_transform"] : [],
                };
                return {
                    document: input,
                    wrap,
                    extended: !!execute.get("gltf_pbr_needs_extended")!(
                        input,
                        wrap,
                        false,
                    ),
            ...
            },
            {
                extensions: {
                    KHR_materials_diffuse_transmission: {
                        ...
                        diffuseTransmissionColorFactor: [0.2, 0.4],
                    },
                },
            },
        );
        for (const ior of [1.5, 1.2])
            for (const specular of [undefined, 1, 0.25])
                for (const transmission of [0, 0.5])
                        ...
                        materials.push({
                            extensions: {
                                ...
                                KHR_materials_specular: {
                                    ...
                                    specularColorFactor: [0.4, 0.5, 0.6],
                                    specularTexture: { index: 1 },
                                },
                                KHR_materials_transmission: {
                                    transmissionFactor: transmission,
                                },
                                KHR_materials_dispersion: { dispersion: 2 },
                                KHR_materials_diffuse_transmission: {
        ...
        for (const raw of materials) {
            const mat = JSON.parse(
                JSON.stringify({
                    _rawMatDef: raw,
                    _baseColorFactor: [0.2, 0.3, 0.4, 0.5],
            ...
            const ctx = {
                _texture: (
                    ...
                    info
                        ? execute.get(
                              lowered.functions.find(
                                  (target) => target.name === "wrapTexture",
                              )!.cpp,
                          )!({ _textureIndex: info.index, _srgb: srgb }, info)
                        : undefined,
            };
            for (const target of lowered.functions.filter(
                (target) => target.name === "applyMaterial",
            )) {
            ...
            const propsTarget = lowered.functions.find(
                (target) => target.name === "assemblePbrPropsExt",
            )!;
            const props = execute.get(propsTarget.cpp)!(
                mat,
                tex,
                layers,
            ) as RecordValue;
            const optIn = lowered.functions.find(
                (target) => target.name === "applyGltfOptInPbrFeatures",
            )!;
            const uv = lowered.functions.find(
                (target) => target.name === "applyGltfUvTransform",
            )!;
            await execute.get(uv.cpp)!(props, tex);
            await execute.get(optIn.cpp)!(props, mat);
            expected.props = props;
            for (const used of [Object.keys(raw.extensions ?? {}), []])
                for (const withWrap of [false, true])
                    for (const withSampler of [false, true]) {
                        const document = {
                            materials: [raw],
                            extensionsUsed: [
                                ...used,
                                ...(withWrap ? ["KHR_texture_transform"] : []),
                            ],
                            samplers: [
                                { magFilter: 9729, minFilter: 9729 },
                                ...(withSampler ? [{ magFilter: 9728 }] : []),
                            ],
                        };
                        const wrap = lowered.textureWrapTriggers.some(
                            (target) => execute.get(target.cpp)!(document),
                        );
                        const extended = !!execute.get(
                            "gltf_pbr_needs_extended",
                        )!(document, wrap, false);
                        const sampled = !!execute.get(
                            "gltf_pbr_needs_sampler",
                        )!(document);
                        const selectedContext = {
                            _texture: (
                                ...
                                info
                                    ? wrap
                                        ? ctx._texture(info, srgb)
                                        : {
                                              _textureIndex: info.index,
                                              _srgb: srgb,
                                          }
                                    : undefined,
                        };
                        const selected = lowered.features.flatMap(
                            (feature, index) =>
                                execute.get(feature.trigger.cpp)!(document)
                                    ? [index]
                                    : [],
                        );
                        const run = lowered.functions.find(
                            (target) =>
                                target.name === "runGltfMaterialFeatures",
                        )!;
                        const selectedLayers = await execute.get(run.cpp)!(
                            mat,
                            selected.map((index) => ({
                                applyMaterial: execute.get(
                                    lowered.features[index]!.handler.cpp,
                                )!,
                            })),
                            selectedContext,
                        );
                        const selectedProps = (await execute.get(
                            "gltf_pbr_build_material",
                        )!(
                            mat,
                            selected.map((index) => ({
                                applyMaterial: execute.get(
                            ...
                            selectedContext,
                            extended,
                            sampled ? () => tex : undefined,
                        )) as RecordValue;
                        const variantProps = await execute.get(
                            "gltf_pbr_build_variant",
                        )!(
                            mat,
                            selected.map((index) => ({
                                applyMaterial: execute.get(
        ...
        const directory = resolve(
            `artifacts/gltf-material-properties-${variant}`,
        );
        mkdirSync(directory, { recursive: true });
        writeFileSync(
            join(directory, "cases.json"),
            JSON.stringify(
                { materials: fixtures, setup: setupCases },
                (_key, value: unknown) =>
                    value === undefined ? { __undefined: true } : value,
            ),
        );
        const source = join(directory, "check.cpp"),
            executable = join(directory, "check.exe");
        const named = (name: string) =>
            lowered.functions.find((target) => target.name === name)!.cpp;
        ...
        int main() {
            ...
            const auto unlit_lifetime = [] {
                ...
                auto extensions = GltfPbrValue::object();
                extensions.set("KHR_materials_unlit", GltfPbrValue::object());
                raw.set("extensions", extensions);
                material.set("_rawMatDef", raw);
                material.set("_baseColorFactor", GltfPbrValue::array({GltfPbrValue{0.2}, GltfPbrValue{0.3}, GltfPbrValue{0.4}, GltfPbrValue{1.0}}));
                material.set("_baseColorImage", GltfPbrValue{std::make_shared<GltfMaterialImageSource>(GltfMaterialImageSource{0})});
                const auto result = ${lowered.features.find((feature) => feature.handler.module.endsWith("/gltf-ext-unlit.ts"))!.handler.cpp}(material);
                assert(result.get("_unlit").truthy());
                assert(result.get("_unlitColor").at(0.0).number() == 0.2);
            };
            unlit_lifetime();
            const auto live_allocations = outstanding_allocations;
            for (unsigned iteration = 0; iteration < 1000; ++iteration) unlit_lifetime();
            assert(outstanding_allocations == live_allocations);
            {
            ...
            for (const auto& row : cases.at("materials")) {
                ...
                ${lowered.functions
                    .filter((target) => target.name === "applyMaterial")
                    .map(
                        (target) => `{
                ...
                ${[...gltfCoreMaterialFields]
                    .map(([name, type]) => {
                        ...
                        const expression =
                            type === "GltfMaterialImage"
                                ? `${value}.truthy() ? std::make_shared<GltfMaterialImageSource>(GltfMaterialImageSource{7}) : GltfMaterialImage{}`
                                : type === "const ts::JsonValue*"
                                  ? `${value}.source()`
                                  : type === "std::vector<double>"
                                    ? `double_array(${value}.source())`
                                    : type === "std::string"
                                      ? `${value}.string()`
                                      : type === "bool"
                                        ? `${value}.truthy()`
