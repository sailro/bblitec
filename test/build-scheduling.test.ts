...
test("invalid or incomplete history yields no estimate", () => {
    ...
    for (const contents of [
        ...
        log([valid, "partial"]),
        log(["", valid]),
        log(["-1\t10\t1\tfile.obj\tab"]),
        log(["20\t10\t1\tfile.obj\tab"]),
        log(["0\tNaN\t1\tfile.obj\tab"]),
        log(["0\t1e3\t1\tfile.obj\tab"]),
        log(["0\t9007199254740992\t1\tfile.obj\tab"]),
        log(["0\t10\tmtime\tfile.obj\tab"]),
        log(["0\t10\t1\t\tab"]),
        log(["0\t10\t1\tfile.obj\txyz"]),
        log(["0\t10\t1\tfile.obj\tab\textra"]),
...
test("unknown scenes start early and expensive known scenes precede cheaper work stably", () => {
    ...
    const reads: string[] = [];
    const ordered = orderByHistoricalCost(items, (item) => {
        reads.push(item.id);
        return item.cost;
    });
    assert.deepEqual(
        ordered.map((item) => item.id),
        ["new", "invalid", "large", "small", "same-size", "instant"],
    );
    assert.deepEqual(
        reads,
    ...
    assert.deepEqual(
        orderByHistoricalCost(items, () => undefined),
        items,
    );
    assert.deepEqual(
        orderByHistoricalCost(items, () => -1),
        items,
    );
    assert.deepEqual(
        orderByHistoricalCost(items, () => Infinity),
        items,
    );
    assert.deepEqual(
        orderByHistoricalCost([], () => 1),
        [],
    );
});
