...
test("inverse bind selection, count and initialization execute the pinned source", (t) => {
    ...
    const contexts = [
        ...
        doctoredContext(
            ...
            "if (skin.inverseBindMatrices !== undefined) if (jointCount > 1)",
        ),
    ];
    ...
    const expected = contexts.map((context) => {
        const factory = createJavaScriptFunction(
            "F32",
            "resolveAccessor",
            transpileCommonJs(
                context
                    .functionDeclaration(module, "resolveIBMs")
                    .declaration.getText(),
                module,
            ) + "\nreturn resolveIBMs;",
        );
        return inputs.map((skin) => {
            ...
            const execute = factory(
                Float32Array,
                (_json: object, _binary: object, index: number) => {
                    reads.push(index);
                    return { _data: new Float32Array(backing.buffer, 16, 48) };
                },
            ) as (json: object, binary: object, skin: object) => Float32Array;
            return { values: [...execute({}, {}, skin)], reads };
        });
    });
    const output = resolve("artifacts/gltf-skin-data");
...
test("unsupported skin accessor ownership refuses at the source", () => {
    assert.throws(
        () =>
            lowerGltfInverseBindMatrices(
                doctoredContext(module, "ibmData._data.byteOffset", "0"),
            ),
        /Skin view offset/,
    );
    assert.throws(
        () =>
            lowerGltfInverseBindMatrices(
                doctoredContext(
                    module,
                    "resolveAccessor(json, binChunk, skin.inverseBindMatrices)",
                    "resolveAccessor(json, otherBuffer, skin.inverseBindMatrices)",
                ),
            ),
        /Skin accessor binary/,
    );
});
