...
test("source light constructors retain defaults, selected parents, baked worlds and definition aliases", async () => {
    ...
    assert.deepEqual(
        plan.lights.map((light) => [light.kind, light.node]),
        [
            ["point", 1],
            ["directional", 2],
            ["spot", 3],
    ...
    const worlds = plan.lights.map((light) =>
        readPackedGltfAttribute(document, binary, light.world),
    );
    ...
    assert.deepEqual(plan.lights[2]!.spot, {
        angle: 0.6,
        cosine: Math.fround(Math.cos(0.3)),
        exponent: 1,
    });
...
test("feature and constructor changes flow through the light plan", async () => {
    ...
    assert.equal(
        constructor.lights[2]!.spot!.cosine,
...
test("source light membership, order, repetition and pointer identity remain independent", async () => {
    ...
    const reversed = await gltfMeshPlan(
        ...
        doctoredContext(
            ...
            "return { entities: lights.reverse() };",
        ),
    );
    assert.deepEqual(
        reversed.lights.map((light) => light.node),
        [null, 3, 2, 1],
    );
    ...
    const filtered = await gltfMeshPlan(
        ...
        doctoredContext(
            ...
            "if (lightIdx === undefined || nodeIdx === 1)",
        ),
    );
    assert.deepEqual(
        filtered.lights.map((light) => light.node),
        [2, 3, null],
    );
    assert.deepEqual(filtered.lightTargets, [2, 0, 1, null, null]);
    const registration = await gltfMeshPlan(
        document,
        bin,
        doctoredContext(
            "src/scene/scene-core.ts",
            "ctx.lights.push(entity as LightBase);",
            "if (entity.intensity > 1) ctx.lights.push(entity as LightBase);",
        ),
    );
    assert.deepEqual(registration.sceneLights, [0]);
    assert.deepEqual(
        registration.lights.map((light) => light.node),
        [2, null, 3],
    );
...
test("feature activation and source light budgets control admission without leaking bindings across loads", async () => {
    ...
    assert.deepEqual(
        (await gltfMeshPlan({ ...document, extensionsUsed: [] }, bin)).lights,
        [],
    );
    document.nodes = [
        ...asRecords(document.nodes),
...
test("unrepresented light resources and invalid packaged bindings refuse", async () => {
    ...
    await assert.rejects(
        ...
        /Unsupported glTF light kind/,
    );
    await packageGltfMeshPlan(document, bin);
    const plan = packagedGltfMeshPlan(document);
    const invalid: JsonObject[] = [
        { ...plan, lights: undefined },
        { ...plan, sceneLights: [99] },
        { ...plan, lightTargets: [99] },
        { ...plan, lightTargets: [] },
        ...[
            { ...plan.lights[0], world: 999 },
            { ...plan.lights[0], node: 99 },
            { ...plan.lights[0], diffuse: [1] },
            { ...plan.lights[0], intensity: NaN },
            { ...plan.lights[0], kind: "ambient" },
            { ...plan.lights[2], spot: {} },
        ].map((light) => ({
            ...plan,
            lights: [light, ...plan.lights.slice(1)],
        })),
    ];
