...
for (const sample of cases)
    test(`asynchronous ${sample.name} startup owns suspended and escaped bindings`, (t) => {
        const directory = resolve("artifacts/startup-awaits", sample.name);
        mkdirSync(directory, { recursive: true });
        writeFileSync(join(directory, "worker.ts"), sample.worker);
        const prefix =
            'const worker=new Worker(new URL("./worker.ts",import.meta.url),{type:"module"});';
        const result = compileSource(prefix + sample.source, {
