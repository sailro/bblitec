...
`;
        ...
        for (const order of [
            ...
            for (const [index, scene] of order.entries()) {
                ...
                assert.match(
                    reflection,
                    /^source\.wgsl:4:\d+ warning: code is unreachable/m,
                );
                assert.match(reflection, /\[3\]\[0\]:/);
                assert.match(reflection, /resource_type = UniformBuffer/);
                assert.ok(
                    reflection.indexOf("warning:") <
                        reflection.indexOf("[3][0]:"),
        ...
        for (const artifacts of results.slice(1))
            assert.deepEqual(artifacts, results[0]);
    },
);
...
`;
        ...
        for (const enabledFirst of [false, true]) {
            const root = join(
                directory,
                enabledFirst ? "enabled-first" : "default-first",
            );
            mkdirSync(join(root, "tools"), { recursive: true });
            mkdirSync(join(root, "upstream"));
            copyFileSync(
            ...
            const declare = (mainEnabled: boolean): void =>
                writeFileSync(
                    join(shaders, "composition.json"),
                    JSON.stringify({
                        modules: [
                            {
                                output: "upstream/shaders/text.frag.native.wgsl",
                                entryPoint: "main",
                                pinnedBindings: true,
            ...
            assert.deepEqual(
                read("same.frag"),
                enabled,
                "constant order has no effect on cache identity",
            );
            for (const index of [0, 1])
                assert.notDeepEqual(
                    ordinary[index],
                    enabled[index],
                    `${artifacts[index]} must carry specialized values`,
                );
            assert.match(enabled[0]!.toString(), /0\.75/);
            assert.match(enabled[1]!.toString(), /0\.75/);
            assert.equal(
                readFileSync(join(shaders, "text.frag.native.wgsl"), "utf8"),
                shader,
            ...
            assert.deepEqual(
                read(enabledFirst ? "other.frag" : "text.frag"),
                enabled,
            );
        ...
        assert.deepEqual(
            results[0],
            results[1],
            "first populated specialization cannot contaminate another variant",
        );
    },
);
