...
test("implicit grid styles preserve native layout, alignment cascade and marker suppression",()=>{
    const result=compile('.grid{display:grid;place-items:center;gap:10px}.grid.open{place-items:end start;align-items:center}.items{list-style:none;list-style-type:none}');
    ...
    const inline=compileSource(`import {createEngine} from '@babylonjs/lite';await createEngine({});
...
test("native implicit rows preserve item layout, text, identity and live mutations",t=>runRmlUiFixture(t,'ui-implicit-grid'));
