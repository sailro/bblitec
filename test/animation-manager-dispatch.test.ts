...
function contexts(): LoweringContext[] {
    const base = new LoweringContext(),
        { file, declaration } = base.functionDeclaration(
            module,
            "updateAnimationManager",
        );
    const body = declaration.body!.statements;
    assert.ok(ts.isVariableStatement(body[4]!));
    const earlySnapshot = [body[0], body[1], body[4], body[2], body[3], body[5]]
        .map((statement) => statement!.getText(file))
        .join("\n");
    const changed =
        file.text.slice(0, declaration.body!.getStart(file)) +
        "{\n" +
        earlySnapshot +
        "\n}" +
    ...
    return [
        base,
        doctoredContext(module, file.text, changed),
        doctoredContext(
            module,
            "task._category === handledCategory",
            "task._category !== handledCategory",
        ),
    ];
}
function sourceResult(context: LoweringContext): unknown {
    const source = context
        .functionDeclaration(module, "updateAnimationManager")
        .declaration.getText();
...
test("source manager snapshots tasks after pre-update and category callbacks", () => {
    ...
    for (const context of variants)
        assert.ok(lowerAnimationManagerDispatch(context));
    assert.throws(
        () =>
            lowerAnimationManagerDispatch(
                doctoredContext(
                    module,
...
test("native manager dispatch preserves source callback order and snapshot mutations", (t) => {
    ...
    const variants = contexts(),
        directory = resolve("artifacts/test-animation-manager-dispatch");
