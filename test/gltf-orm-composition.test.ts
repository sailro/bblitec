...
test("ORM pixel output follows the pinned body and changed source stores", async (t) => {
    ...
    const contexts = [
        ...
        doctoredContext(
            module,
            "d1.data[j] = d2.data[j]!;",
            "d1.data[j + 1] = d2.data[j + 2]!;",
        ),
        ...[-4, 0.5, 1.5, 2.5, 254.5, 300].map((value) =>
            doctoredContext(
                module,
                "d1.data[j] = d2.data[j]!;",
                `d1.data[j] = ${value};`,
    ...
    const mr = {
        ...
        data: new Uint8ClampedArray([
            12, 40, 60, 255, 14, 44, 66, 255, 20, 71, 5, 255, 45, 32, 8, 255,
            62, 40, 15, 255, 53, 81, 9, 255,
        ]),
    };
    const occ = {
        width: 3,
        height: 2,
        data: new Uint8ClampedArray([
            20, 19, 5, 255, 35, 23, 8, 255, 87, 34, 80, 255, 144, 64, 4, 255, 9,
            74, 68, 255, 11, 66, 91, 255,
        ]),
    };
    ...
    writeFileSync(
        ...
        int main() {
            using namespace bbl;
            const pal::DecodedImage mr{${mr.width}, ${mr.height}, {${[...mr.data].join(",")}}}, occ{${occ.width}, ${occ.height}, {${[...occ.data].join(",")}}};
            ${expected
                .map(
                    (value, index) => `{
                const auto result = variant${index}::gltf_composite_orm(mr, occ);
                assert(result.width == ${value.width} && result.height == ${value.height});
                assert((result.rgba == std::vector<std::uint8_t>{${[...value.data].join(",")}}));
            }`,
                )
                .join("\n")}
            assert((mr.rgba == std::vector<std::uint8_t>{${[...mr.data].join(",")}}));
            assert((occ.rgba == std::vector<std::uint8_t>{${[...occ.data].join(",")}}));
...
test("unrepresented ORM canvas operations refuse at the pinned source", () => {
    assert.throws(
        () =>
            lowerGltfOrmComposition(
                doctoredContext(
                    module,
                    'c1.getContext("2d")',
                    'c1.getContext("2d", { alpha: false })',
                ),
            ),
        /Unsupported image canvas call/,
    );
    assert.throws(
        () =>
            lowerGltfOrmComposition(
