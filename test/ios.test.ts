...
const inventory = (devices: unknown[]) =>
    JSON.stringify({
        devices: {
            [runtime]: devices,
            "com.apple.CoreSimulator.SimRuntime.tvOS-16-2": [
                { ...first, udid: "tv" },
            ],
        },
    });
...
test("iOS captures select an explicit available simulator without claiming a tvOS or ambiguous booted device", () => {
    ...
    assert.equal(
        selectIosSimulator(inventory([first, second]), second.udid).state,
        "Shutdown",
    );
    assert.throws(
        () => selectIosSimulator(inventory([first]), "tv"),
        /found 0/,
    );
    assert.throws(
        () =>
            selectIosSimulator(
                inventory([first, { ...second, state: "Booted" }]),
                "booted",
            ),
        /found 2/,
    );
    assert.throws(
        () =>
            selectIosSimulator(
                inventory([{ ...first, isAvailable: false }]),
                first.udid,
            ),
        /found 0/,
    );
    for (const value of [
        ...
        "{}",
        '{"devices":[]}',
        '{"devices":{"com.apple.CoreSimulator.SimRuntime.iOS-16-2":null}}',
        inventory([{}]),
    ]) {
    ...
    for (const id of ["scene1", "torus-states", "scene304"]) {
        ...
        for (const backend of ["sdl_gpu", "dawn"] as const) {
            ...
            assert.deepEqual(environment, {
                ...
                BBLITE_GPU_DEBUG: "1",
                BBLITE_TEST_PASS: "1",
                BBLITE_MAX_FRAMES: String(nativeCaptureFrameBudget(original)),
            });
...
test("iOS canvas-only capture changes only UI pixels, not registry timing or renderer selection", () => {
    ...
    for (const backend of ["dawn", "sdl_gpu"] as const) {
        ...
        assert.deepEqual(
            iosCaptureEnvironment(scene, backend, { canvasOnly: true }),
            { ...ordinary, BBLITE_CAPTURE_UI: "0" },
        );
        assert.deepEqual(
            iosCaptureEnvironment(scene, backend, { canvasOnly: false }),
            ordinary,
        );
    }
});
...
test("iOS smoke refuses failed, missing, duplicate and stale native exits", () => {
    ...
    assert.throws(
        () => verifyIosNativeExit("Native exit: 1 run=current\n", "current"),
        /status 1/,
    );
    for (const log of [
        "",
        "Native exit: 0 run=old\n",
        "Native exit: 0 run=current\nNative exit: 0 run=current\n",
    ]) {
...
test("desktop and Simulator measurements share frame, pose and replay precedence", () => {
    ...
    assert.equal(
        environment.BBLITE_FRAME_DELTA_MS,
        scene.parity?.nativeEnvironment?.BBLITE_FRAME_DELTA_MS,
    );
});

const tools = discoverDevelopmentTools();
test(
    "iOS dependency triplets keep static libraries and isolate device and simulator architectures",
    { skip: !tools.cmake },
    (t) => {
        ...
        for (const [triplet, arch, sdk] of [
            ...
            const script = join(directory, "triplet.cmake"),
                output = join(directory, "values.txt");
            writeFileSync(
                script,
...
test(
    "iOS CMake refuses unsupported renderers and admits reached document pickers",
    { skip: !tools.cmake },
    (t) => {
        mkdirSync("artifacts", { recursive: true });
        ...
        const configure = (backend: string, sdk: string, features = "") =>
            spawnSync(
                tools.cmake!,
                [
        ...
        for (const backend of ["SDL_GPU", "BOTH"]) {
            const result = configure(
                backend,
                "Xcode/Platforms/iPhoneSimulator.platform/SDK",
            );
            assert.notEqual(result.status, 0);
            assert.match(
                result.stderr,
                /Simulator requires BBLITE_BACKEND=DAWN/,
            );
        }
...
test(
    "an explicit Dawn install replaces a cached package selection",
    { skip: !tools.cmake },
    (t) => {
        mkdirSync("artifacts", { recursive: true });
        ...
        t.after(() => rmSync(directory, { recursive: true, force: true }));
        const cmake = readFileSync("native/CMakeLists.txt", "utf8");
        const selection = cppSection(
            cmake.replaceAll("\r\n", "\n"),
            "    set(\n        Dawn_DIR",
            "    # The static monolithic Dawn export",
        );
        ...
        for (const name of ["first", "second"]) {
            ...
            writeFileSync(
                join(config, "DawnConfig.cmake"),
                `set(SELECTED_DAWN "${name}")\n`,
            );
        }
        const script = join(directory, "select.cmake");
        writeFileSync(
            script,
            `set(BBLITE_DAWN_DIR "${prefix}/first")
${selection}
...
test(
    "iOS compiler arguments and dependency scripts parse and refuse incompatible targets",
    ...
    (t) => {
        ...
        const helper = cppSection(
            module,
            "function Get-IosCompilerArguments",
            "function Get-BuildParallelArguments",
...
test(
    "mobile dependency reuse requires matching inputs, target identity and complete output",
    { skip: !tools.powershell },
    (t) => {
        mkdirSync("artifacts", { recursive: true });
...
test("iOS SDL entry accepts both generated main signatures and preserves a failing exit status", (t) => {
    ...
    t.after(() => rmSync(directory, { recursive: true, force: true }));
    const entry = join(directory, "entry.cpp"),
        executable = join(directory, "entry.exe");
    for (const body of [
        "int main() { return 7; }",
        "int main(int argc, char**) { return argc + 6; }",
    ]) {
        ...
        const result: SpawnSyncReturns<string> = spawnSync(executable, [], {
            encoding: "utf8",
            windowsHide: true,
            env: {
                ...native.environment,
                BBLITE_RUN_ID: "entry-contract",
...
test("iOS window density preserves native pixels and authored DPR 1 without rounding other caps", (t) => {
    ...
    const fixture = join(directory, "density.cpp"),
        executable = join(directory, "density.exe");
    writeFileSync(
        fixture,
        `
...
test("iOS source setup observes its actual canvas extent before rendering, while realm extents stay owned", (t) => {
    ...
    t.after(() => rmSync(directory, { recursive: true, force: true }));
    const create = cppFunction(
        readFileSync("native/src/pal.cpp", "utf8"),
        "Engine create_engine(",
    );
    const metrics = cppFunction(
        readFileSync("native/src/pal_platform_events.hpp", "utf8"),
