...
test("module namespace records enumerate value exports and retain live bindings", (t) => {
    ...
    writeFileSync(
        ...
        `export * from "./values"; export {A as alias} from "./values";`,
    );
    writeFileSync(
        join(directory, "cache.ts"),
        `
...
test("external project modules and raw files remain generation inputs", (t) => {
    ...
    const inputs = compileSource(source, {
        fileName: entry,
    }).manifest.inputs.map((file) => resolve(file));
...
test("local JavaScript modules supply executable definitions beside declarations", (t) => {
    ...
    writeFileSync(
        module,
        `export const layout = {width: 3, slots: [{x: 1}, {x: 2}]};
        /** @param {number} value */
        export function twice(value) { return value * 2; }`,
    );
    writeFileSync(
        join(directory, "layout.d.mts"),
        `export declare const layout: {readonly width: number; readonly slots: readonly {x: number}[]};
        export declare function twice(value: number): number;`,
    );
    const result = compileSource(
        `import {layout, twice} from "./layout.mjs";
        if (layout.width !== 3 || layout.slots.length !== 2 || twice(4) !== 8) throw new Error("JavaScript implementation");`,
        { fileName: join(directory, "entry.ts") },
    );
    assert.ok(
        result.manifest.inputs.map((file) => resolve(file)).includes(module),
    );
...
test("deployment URLs resolve external public assets and browser base values", (t) => {
    ...
    const options = {
        fileName: join(directory, "entry.ts"),
        publicDir,
        siteUrl: "https://example.invalid/project/",
        search: "mode=preview",
    };
    const result = compileSource(
        `
    ...
    for (const source of [
        ...
        "https://example.invalid/project/settings.json?rev=2",
    ]) {
        assert.equal(
            deploymentAssetSource(source, options),
            join(publicDir, "settings.json"),
        );
    }
    assert.equal(
        deploymentAssetSource(
            "https://elsewhere.invalid/settings.json",
            options,
        ),
        undefined,
    );
    assert.throws(
        () => deploymentAssetSource("%2e%2e%2fsecret.txt", options),
        /escapes/,
    );
    assert.equal(
        deploymentUrl({ siteUrl: "https://example.invalid/project" }).pathname,
        "/project/",
    );
    assert.throws(
        () =>
            deploymentUrl({ siteUrl: "https://example.invalid/?token=hidden" }),
        /base URL/,
    );
});
...
test("explicit build environment preserves strings, empty values and missing defaults", () => {
    ...
    assert.doesNotMatch(
        result.cpp,
        /incorrect build strings|incorrect missing input/,
    );
    assert.match(result.cpp, /label=value/);
    assert.match(result.cpp, /fallback/);
    assert.throws(
        () => deploymentEnvironment({ environment: { DEV: "true" } }),
        /built-in/,
    );
    assert.throws(
        () =>
            deploymentEnvironment({ environment: { "invalid-name": "value" } }),
        /identifier/,
    );
});
...
test("runtime initialization ignores type-only cycles and type-only side effects", (t) => {
    ...
    writeFileSync(
        ...
        `export {result} from "./derived"; export {type Unused} from "./types";`,
    );
    const result = compileSource(
        `
    ...
    assert.ok(
        state >= 0 && derived > state,
        "state storage precedes dependent initialization",
    );
});
