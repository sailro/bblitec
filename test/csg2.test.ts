...
test("CSG2 compiler preserves native material partitions and generation-only solid lifetime", () => {
    ...
    assert.equal(
        result.manifest.sceneMeshes.filter(
            (mesh) => mesh.kind === "from-data" && mesh.standardMaterial,
        ).length,
        2,
    );
    ...
    assert.ok(
        result.manifest.adaptations.some(
            (adaptation) => adaptation.id === "executed-csg2-solid",
        ),
    );
...
test("unchanged Scene 91 retains all six material partitions through nullable try/finally bindings", () => {
    ...
    for (const operation of ["subtract", "intersect", "union"]) {
        for (const material of [0, 1])
            assert.ok(result.cpp.includes(`"csg-${operation}_sub${material}"`));
    }
    assert.equal(result.cpp.match(/bbl::create_mesh_from_data\(/g)?.length, 6);
    assert.ok(
        result.cpp.slice(result.cpp.indexOf("int main()")).length < 100000,
    );
});
...
const box: Csg2SolidPlan = {
    op: "from-mesh",
    source: { factory: "createBox", options: 2 },
    materialSlot: 2,
};
const sphere: Csg2SolidPlan = {
    op: "from-mesh",
    source: {
        factory: "createSphere",
        options: { diameter: 2.5, segments: 8 },
    },
    materialSlot: 0,
};
...
test("pinned CSG2 operations preserve volume and ascending material slots, including sparse slots", () => {
    ...
    for (const op of csg2BooleanNames) {
        ...
        assert.deepEqual(
            parts.map((part) => part.materialSlot),
            [0, 2],
        );
        assert.deepEqual(
        ...
        assert.equal(
            parts.reduce(
                (count, part) => count + part.geometry.indices.length,
                0,
            ),
            merged[0]!.geometry.indices.length,
        );
    ...
    assert.ok(
        Math.abs(
            volumes.get("csg2Subtract")! + volumes.get("csg2Intersect")! - 8,
        ) < 0.00001,
    );
...
test("pinned CSG2 replay preserves exact bytes and refuses missing materials and empty single meshes", () => {
    ...
    assert.deepEqual(
        packBakedCsgMesh(first[0]!.geometry),
        packBakedCsgMesh(second[0]!.geometry),
    );
    assert.throws(
    ...
    assert.throws(() => bakeCsg2Meshes({ plan: empty, name: "empty" }), /#449/);
});
...
test("CSG2 refusal boundary is anchored to the pinned slot reservation and bundled WASM", () => {
    ...
    assert.match(
        pin,
        /materialSlot < 0 \|\| materialSlot >= MATERIAL_ID_RESERVE_COUNT \|\| !Number.isInteger\(materialSlot\)/,
    );
    assert.match(
        pin,
