...
test("the CMake backend arm derives the Dawn twins from the SDL_GPU units featureSources names", () => {
    ...
    const pattern =
        /list\(FILTER BBLITE_SDL_GPU_SOURCES INCLUDE REGEX "([^"]+)"\)/.exec(
            cmake,
        )?.[1];
    assert.ok(
        pattern,
        "CMakeLists.txt does not select the SDL_GPU units by pattern",
    );
    const exclusion =
        /list\(FILTER BBLITE_RUNTIME_SOURCES EXCLUDE REGEX "([^"]+)"\)/.exec(
            cmake,
        )?.[1];
    assert.ok(
        exclusion,
        "CMakeLists.txt does not drop the SDL_GPU units by pattern",
    );
    assert.equal(
        exclusion,
        pattern,
        "selection and exclusion must be one pattern",
    );
    // CMake spells the regex with doubled backslashes inside its quotes.
    const selector = new RegExp(pattern.replaceAll("\\\\", "\\"));
    assert.match(
        cmake,
        /string\(REPLACE "\/src\/pal_sdl_gpu" "\/src\/pal_dawn" bblite_dawn_source/,
    );
    assert.match(
        cmake,
    ...
    for (const source of featureTus) {
        assert.match(
            `/${source}`,
            selector,
            `${source} escapes the SDL_GPU pattern`,
        );
...
test("the SDL scene driver releases run resources before its device", () => {
    ...
    const state = driver.slice(
        ...
        driver.indexOf("} data_;"),
    );
    assert.ok(
        state.indexOf("Resources resources;") <
            state.indexOf("std::optional<PickHookGuard>"),
    );
    assert.ok(
        state.indexOf("Resources resources;") <
            state.indexOf("std::optional<GpuBufferUploadBatch>"),
    );
    const destructor = driver.slice(
        driver.indexOf("~Resources()"),
        driver.indexOf("struct State : FrameSession"),
    );
    assert.equal(destructor.match(/release\(state\);/g)?.length, 1);
    assert.ok(
        destructor.indexOf("destroy_ui_rml_runtime") <
            destructor.indexOf("release(state);"),
    );
    assert.ok(
        destructor.indexOf("release_scene_sprite_pass") <
            destructor.indexOf("release(state);"),
    );
    assert.ok(
        driver.indexOf("} data_;") <
            driver.indexOf("std::optional<Frame> frame_;"),
    );
});
...
test("the run end finishes only its engine's audio session", () => {
    ...
    assert.match(
        text.slice(guard),
        /if \(engine\.audio_session\)\s+engine\.audio_session->finish\(\);/,
    );
...
test("Dawn caches thin-pick bindings and invalidates them with their buffers", () => {
    ...
    assert.match(
        dawn,
        /state\.release_thin_pick_groups\(\);[\s\S]{0,300}wgpuBufferRelease\(state\.pick_mesh_buffer\)/,
    );
    assert.match(
        dawn,
