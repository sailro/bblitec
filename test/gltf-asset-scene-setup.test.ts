...
} from "../src/lowering/gltf/asset-scene-setup.js";
...
const module = "src/loader-gltf/load-gltf.ts",
    registryModule = "src/loader-gltf/gltf-feature-registry.ts";
const featureModules = [
    "./gltf-feature-gaussian-splatting.js",
    "./gltf-ext-lights-image-based.js",
    "./gltf-feature-interactivity.js",
];
const callbackNames = [
    "gaussian_splat_setup",
    "ibl_scene_setup",
    "interactivity_scene_setup",
];
...
interface Scenario {
    ...
    existing: boolean;
    failure: number;
    repeatComposition: boolean;
}
type Fold = (
    container: Container,
    fragments: Array<{ _sceneSetup: Setup | undefined }>,
) => void;
...
async function sourceOrder(context: LoweringContext): Promise<number[]> {
    const redirects = new Map(
        featureModules.map((path, id) => [
            path,
            javascriptModuleUrl(`export default {id:${id}};`),
        ]),
    );
    const imported = (await import(
        pinnedModuleTextUrl(
            "loader-gltf/gltf-feature-registry.js",
            transpileForBrowser(
                context.sourceFile(registryModule).text,
                registryModule,
            ),
            [],
            redirects,
    ...
    return (
        await imported.loadGltfFeatures({
            extensionsUsed: [
...
function sourceResult(fold: Fold, order: number[], input: Scenario): object {
    ...
    for (let i = 0; i < 3; i++) {
        try {
            ...
            if (!(error instanceof Error) || error.message !== "setup")
                throw error;
            failures++;
        }
    }
...
test("native feature composition matches source order, guards, exceptions and shared callback captures", async (t) => {
    ...
    const reversed = registry
        .replace(featureModules[0]!, "__swap__")
        .replace(featureModules[1]!, featureModules[0]!)
        .replace("__swap__", featureModules[1]!);
    const contexts = [
        base,
        doctoredContext(registryModule, registry, reversed),
        doctoredContext(
            module,
        ...
        doctoredContext(
            module,
            "if (_sceneSetup)",
            "if (false && _sceneSetup)",
        ),
    ];
    const scenarios: Scenario[] = Array.from({ length: 16 }, (_, index) => ({
        mask: index % 8,
        existing: index >= 8,
        failure: -1,
        repeatComposition: true,
    }));
    ...
    for (const [variant, context] of contexts.entries()) {
        ...
        assert.deepEqual(
            gltfAssetSceneSetupOrder(context, true, true),
            order.map((id) => callbackNames[id]),
        );
        assert.deepEqual(gltfAssetSceneSetupOrder(context, false, false), [
            "ibl_scene_setup",
        ]);
        cases.push(
            ...scenarios.map((input) => ({
                variant,
                input,
                expected: sourceResult(fold, order, input),
            })),
        );
    }
    const directory = resolve("artifacts/test-gltf-asset-scene-setup");
...
test("feature composition refuses unrepresented resource, target and registry sequencing changes", () => {
    for (const [before, after] of [
        ...
        [
            ...
            "const prev = container.other;",
        ],
        [
            "return container;",
            "container._sceneSetup = undefined; return container;",
        ],
    ])
        assert.throws(
            () =>
            ...
            /Unrepresented|Unsupported|Expected|Cannot|supported|changed|lower|projection/,
        );
    assert.throws(
        () =>
            gltfAssetSceneSetupOrder(
                doctoredContext(
                    ...
                    "return mods.reverse().map((m) => m.default);",
                ),
                true,
                true,
            ),
