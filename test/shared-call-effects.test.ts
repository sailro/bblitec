...
test("PBR and glTF calls retain composition profiles, material order and producer-specific mutations", () => {
    ...
    assert.deepEqual(
        materials.map((material) => material.materialsBefore),
        [1, 3, 5, 7],
    );
    assert.deepEqual(
        materials.map((material) => material.gltfAssetsBefore),
        [0, 0, 1, 2],
    );
    ...
    assert.deepEqual(
        materials.slice(0, 2).map((material) => material.sceneMeshIndices),
        [[0], [1]],
    );
    assert.deepEqual(
...
test("void, inferred and annotated resource helpers preserve distinct composition profiles", () => {
    for (const declaration of [
        ...
        const result = compileSource(
            `${imports} async function main(){ const engine=await createEngine({}); ${declaration} make(); make(); }`,
        );
        assert.equal(
...
test("a declined shared resource return rolls back speculative construction effects", () => {
    ...
    assert.deepEqual(
        result.manifest.scenePbrMaterials.map(
            (material) => material.metallicFactor,
        ),
        [1, 1],
    );
});
...
test("shared material helpers retain tuple and conditional scalar facts", () => {
    ...
    assert.deepEqual(
        result.manifest.scenePbrMaterials.map(
            (material) => material.environmentIntensity,
        ),
        [1, 0.35],
    );
    assert.deepEqual(
        result.manifest.scenePbrMaterials.map((material) => material.unlit),
        [true, true],
    );
});
...
test("shared glTF wrappers preserve contiguous load ordering and its refusal", () => {
    const program = (
        calls: string,
    ) => `${imports} async function main(){ const engine=await createEngine({});
        async function load(url: string) { return await loadGltf(engine,url); } ${calls} }`;
    const result = compileSource(
        program(
            'await load("a.glb"); await load("a.glb"); await load("b.glb"); await load("b.glb");',
        ),
    );
    assert.deepEqual(
        result.manifest.assets.map((asset) => [
    ...
    assert.throws(
        () =>
            compileSource(
                program(
                    'await load("a.glb"); await load("b.glb"); await load("a.glb");',
                ),
            ),
        /must be contiguous/,
    );
...
test(
    "shared resource definitions execute every construction and retain native handle identity",
    { skip: !tools },
    () => {
