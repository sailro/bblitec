...
class ChangedContext extends LoweringContext {
    ...
    public override sourceFile(module: string): ts.SourceFile {
        ...
        assert(
            source.includes(this.before),
            "The mutation must reach the pin.",
        );
        return ts.createSourceFile(
            module,
            source.replace(this.before, this.after),
...
test("query contracts reject tuple, result, collector and no-hit drift", () => {
    for (const [before, after] of [
        [
            ...
            "[r.x, r.y, r.z, r.w], [p.x, p.y, p.z]",
        ],
        [
            "HP_QueryCollector_GetShapeCastResult(collector, 0)[1]",
            "HP_QueryCollector_GetShapeCastResult(collector, 1)[1]",
        ],
        [
            "inputHitPoint: hitVec(hitInputData[3])",
            "inputHitPoint: hitVec(hitShapeData[3])",
        ],
        ["HP_QueryCollector_Create(1)[1]", "HP_QueryCollector_Create(2)[1]"],
        ["query.shouldHitTriggers ?? false", "query.shouldHitTriggers ?? true"],
        [
            "BigInt(query.ignoreBody._hkBody[0])",
            "BigInt(query.ignoreBody._hkBody[1])",
        ],
        [
            "const zero = (): Vec3 => ({ x: 0, y: 0, z: 0 });",
            "const zero = (): Vec3 => ({ x: 1, y: 0, z: 0 });",
        ],
        [
            "const collector = getCollector(world);",
            "const collector = getCollector(world); console.log(collector);",
        ],
        [
            "_ignoreNone: [bigint] | null = null",
            "_ignoreNone: [bigint] | null = [BigInt(1)]",
        ],
    ])
        assert.throws(
            () =>
                lowerPhysicsQueries(
                    new ChangedContext(queries, before!, after!),
                ),
            /changed|exactly/,
            before,
        );
    assert.deepEqual(
        ...
        lowerPhysicsQueries(new LoweringContext(store)),
    );
});
...
test("rotation contracts reject skipped state and lifecycle changes", () => {
    for (const [module, before, after] of [
        ...
        [
            ...
            "const rq = root.rotationQuaternion",
        ],
        [
            rotation,
            "onRotationChanged.notify(out);",
            "onRotationChanged.notify(out); setMeshesMaterial([ring], materials.colored);",
        ],
        [
            rotation,
        ...
        [
            rotation,
            "registerPointerDrag(layer, canvas, drag)",
            "registerPointerDrag(layer, canvas, gizmo)",
        ],
        [
            rotation,
            "gizmo.drag.enabled = node !== null",
            "gizmo.drag.enabled = true",
        ],
        [
            rotation,
            "gizmo.drag.onDragStart.clear();",
            "gizmo.drag.onDragStart.clear(); gizmo.drag.onHoverStart.clear();",
        ],
        ...
        assert.throws(
            () =>
                lowerRotationPointerDrag(
                    new ChangedContext(module!, before!, after!),
                ),
            /changed|exactly/,
            before,
        );
...
test("rotation arithmetic changes remain translated from source", () => {
    const source = lowerRotationPointerDrag(
        new ChangedContext(
            rotation,
            "Math.abs(angle) < 1e-7",
