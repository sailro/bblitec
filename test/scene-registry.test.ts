...
test("registers unique generated scene targets", () => {
    assert.deepEqual(
        scenes
            ...
            .map(({ id }) => id),
        ["primitives", "scene1", "scene3", "scene6", "scene14", "scene24", "scene28", "scene29", "scene31", "scene33", "scene35", "scene231", "scene216", "scene150", "scene178", "scene210", "scene212", "scene243", "scene246", "scene247", "scene252", "scene254", "scene255", "scene258", "scene259", "scene265", "scene2", "scene7", "scene8", "scene5", "scene10", "scene12", "scene13", "scene32", "scene159", "scene160", "scene161", "scene162", "scene165", "scene163", "audit-shader-frame-graph", "regression-runtime-sweep", "regression-sprite-layer-arms", "regression-instanced-ground", "regression-morph-ground", "regression-node-geometry-output", "regression-scene-skeleton", "regression-light-setters", "regression-property-animation-paths", "regression-mesh-flags", "regression-physics-aggregate-options", "regression-physics-mesh-shape", "regression-physics-floating-origin", "regression-material-falloff", "regression-compiler-state", "scene168", "scene176", "scene213", "scene151", "scene154", "scene152", "scene153", "scene157", "scene158", "scene155", "scene156", "scene240", "scene241", "scene250", "scene170", "regression-nav-crowd", "scene171", "scene172", "scene173", "regression-nav-obstacles", "scene174", "scene175", "regression-track-clamp", "scene110", "scene120", "scene121", "scene122", "scene123", "scene124", "scene125", "scene126", "scene127", "scene128", "scene129", "scene113", "scene114", "scene115", "scene116", "scene117", "scene145", "scene146", "scene248", "scene245", "scene249", "scene257", "scene266", "scene267", "scene268", "scene269", "scene270", "scene30", "scene256", "scene260", "scene34", "scene9", "scene242", "scene23", "scene40", "scene179", "scene166", "scene100", "scene101", "scene102", "scene103", "scene106", "scene48", "scene272", "scene273", "scene274", "scene244", "scene37", "scene253", "scene16", "scene44", "scene42", "scene45", "scene43", "scene38", "scene39", "scene21", "scene19", "scene15", "scene50", "scene51", "scene52", "scene53", "scene56", "scene57", "scene92", "scene93", "scene94", "scene95", "scene96", "scene97", "scene54", "scene55", "scene98", "scene99", "scene177", "scene26", "scene27", "scene142", "scene143", "scene261", "scene275", "scene147", "scene11", "scene148", "scene149", "scene58", "scene59", "scene60", "scene64", "scene61", "scene77", "scene78", "scene79", "scene80", "scene82", "scene83", "scene85", "scene86", "scene88", "scene89", "scene63", "scene67", "scene68", "scene69", "scene70", "scene71", "scene84", "scene62", "scene81", "scene87", "scene74", "scene75", "scene76", "scene262", "scene263", "scene264", "scene276", "scene277", "scene280", "scene281", "scene283", "scene284", "scene278", "scene279", "scene300", "scene301", "scene302", "scene73", "scene303", "scene187", "scene305", "scene282", "scene218", "scene219", "scene220", "scene221", "scene222", "scene223", "scene224", "scene225", "scene211", "scene229", "scene25", "scene36", "scene251", "scene18", "scene4", "scene203", "scene205", "scene204", "scene206", "scene207", "scene200", "scene201", "scene202", "scene209", "scene65", "scene141", "scene22", "regression-gltf-sparse", "regression-gltf-uv-sets", "regression-imported-mesh-walk", "regression-gltf-topology", "regression-gltf-step-animation", "regression-shadow-pbr-only", "scene144", "scene217", "scene17", "scene20", "scene66", "scene140", "scene72", "scene271", "scene226", "scene90", "scene91", "scene214", "scene215", "scene118", "scene111", "scene112", "scene167", "regression-timer-callback-cells"]
    );
    assert.deepEqual(
        scenes
            .filter(({ sourceOrigin }) =>
    ...
    const nodeGeometry = getScene("scene149").parity!;
    assert.equal(nodeGeometry.maxFullMad, 0.02);
    assert.equal(nodeGeometry.maxForegroundMad, 0.02);
    assert.deepEqual(nodeGeometry.backgroundColor, [51, 51, 76]);
    assert.equal(nodeGeometry.backgroundThreshold, 8);
    assert.equal(nodeGeometry.referenceSearch, undefined);
    assert.equal(nodeGeometry.referenceTimeSeconds, undefined);
    assert.equal(nodeGeometry.referenceFrame, undefined);
    assert.equal(getScene("scene176").parity?.reference.kind, "source");
    assert.equal(getScene("scene213").parity?.reference.kind, "source");
    assert.equal(getScene("platformer").parity?.referenceFrame, 180);
    assert.equal(
        getScene("platformer").parity?.nativeEnvironment
    ...
    assert.equal(getScene("sandblox").parity?.referenceSearch, "?fresh=1");
    assert.equal(getScene("sandblox").parity?.maxFullMad, 0.2);
    assert.equal(getScene("sandblox").parity?.maxForegroundMad, 0.2);
