...
function sourceResult(context: LoweringContext): unknown {
    const source =
        context
            .functionDeclaration(propertyModule, "createPointerAnimationGroup")
            .declaration.getText() +
        "\n" +
        [
            "playAnimation",
            ...
            .map((name) =>
                context
                    .functionDeclaration(groupModule, name)
                    .declaration.getText(),
            )
            .join("\n");
...
test("property source tick distinguishes paused pose publication from stopped tasks", () => {
    ...
    for (const context of variants)
        assert.ok(lowerPropertyAnimationPlayback(context));
});
test("native property playback follows source clock, stopped writes and publication order", (t) => {
    ...
    const variants = contexts(),
        directory = resolve("artifacts/test-animation-property-playback");
