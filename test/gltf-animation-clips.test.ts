...
function fixture() {
    ...
    const append = (
        ...
        accessors.push({
            ...
            componentType: data instanceof Float32Array ? 5126 : 5120,
            type,
    ...
    const rotation = append(
        new Int8Array([-128, -64, 0, 127, -127, 64, 32, -32]),
        "VEC4",
        4,
        true,
    );
    const document: JsonObject = {
        ...
        meshes: [{ primitives: [{ attributes: { POSITION: position } }] }],
        nodes: [{ mesh: 0 }],
        scenes: [{ nodes: [0] }],
        animations: [
            {
                samplers: [
                    { input: time, output: translation },
                    {
                        input: unusedTime,
                        output: rotation,
                ...
                channels: [
                    ...
                    { sampler: 0, target: { node: 0, path: "unused" } },
                ],
            },
            {
                name: "second",
                samplers: [
                    { input: time, output: rotation, interpolation: "STEP" },
                ],
                channels: [
...
test("packaged clips retain source filtering, order, unused-sampler duration and normalized bits", async () => {
    const cases = [
        ...
        },
        {
            context: doctoredContext(
                module,
                "inputAcc._count, inNorm",
                "inputAcc._count - 1, inNorm",
            ),
            duration: 0,
            name: "",
            divisor: 127,
        ...
        {
            context: doctoredContext(
                "src/loader-gltf/gltf-sampler-denorm.ts",
                "src instanceof I8 ? 127",
                "src instanceof I8 ? 63",
            ),
            duration: 9,
            name: "",
            divisor: 63,
    ...
    for (const row of cases) {
        ...
        assert.equal(animation.clips.length, 2);
        ...
        assert.deepEqual(second.channels, [
            { samplerIdx: 0, nodeIdx: 0, path: constant("PATH_ROTATION") },
        ]);
        assert.equal(
            first.samplers[0]!.interpolation,
            constant(row.step ? "INTERP_STEP" : "INTERP_LINEAR"),
        );
        assert.equal(
            first.samplers[1]!.interpolation,
            constant("INTERP_LINEAR"),
        );
        assert.equal(
            second.samplers[0]!.interpolation,
            constant("INTERP_STEP"),
        );
        assert.deepEqual(
            readPackedGltfAttribute(document, bytes, first.samplers[0]!.input),
            row.input,
        );
        assert.deepEqual(
            readPackedGltfAttribute(document, bytes, first.samplers[0]!.output),
            [1, 2, 3, 4, 5, 6],
        );
        const expected = Float32Array.from(
            [-128, -64, 0, 127, -127, 64, 32, -32],
            (value) => Math.max(value / row.divisor, -1),
        );
        const actual = Float32Array.from(
            readPackedGltfAttribute(
                document,
                bytes,
                second.samplers[0]!.output,
            ),
        );
        assert.deepEqual(
