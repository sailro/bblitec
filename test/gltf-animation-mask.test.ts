...
function contexts(): LoweringContext[] {
    return [
        ...
        doctoredContext(
            maskModule,
            "(mask.names.indexOf(name) !== -1) === (mask.mode === AnimationGroupMaskMode.Include)",
            "(mask.names.indexOf(name) !== -1) !== (mask.mode === AnimationGroupMaskMode.Include)",
        ),
        doctoredContext(
            controllerModule,
        ...
        doctoredContext(
            ...
            "return { mode, names: names.slice(), disabled: true };",
        ),
...
function sourceResult(context: LoweringContext): unknown {
    ...
    const setter = context
        .variableInitializer(controller.declaration, "_setMask")
        .getText();
    const source =
        context
            .findNodes(context.sourceFile(maskModule), ts.isEnumDeclaration)[0]!
            .getText() +
        "\n" +
        [
            ...
            .map((name) =>
                context
                    .functionDeclaration(maskModule, name)
                    .declaration.getText(),
            )
            .join("\n");
    const defaults = [
        ...
        .map(
            (name) =>
                `let ${name}=${context.variableInitializer(controller.declaration, name).getText()};`,
        )
        .join("\n");
...
tick(createAnimationGroupMask(["hip"],AnimationGroupMaskMode.Include));tick(null);return result;`,
            maskModule,
        ),
    );
...
test("source controller masks retain cache identity and resolve source include/exclude membership", () => {
    ...
    for (const output of results.slice(1))
        assert.notDeepEqual(output, results[0]);
    for (const context of variants) assert.ok(lowerGltfAnimationMask(context));
    assert.throws(
        () =>
            lowerGltfAnimationMask(
                doctoredContext(
                    controllerModule,
                    "_maskResolver(mask, nodeNames, maskedNodes, numNodes)",
                    "_maskResolver(mask, nodeNames, maskedNodes, 1)",
                ),
            ),
        /arguments|changed/,
    );
...
test("compiled animation masks refuse mutable names, mode and disabled state", () => {
    for (const mutation of [
        ...
        assert.throws(
            () =>
                compileSource(
                    'import {createAnimationGroupMask} from "@babylonjs/lite"; const mask=createAnimationGroupMask(["hip"]); ' +
                        mutation,
                ),
            /Only property assignments|Unsupported property assignment/,
        );
});
test("native controller mask cache follows actual source and cache/membership mutations", (t) => {
    ...
    const variants = contexts(),
        directory = resolve("artifacts/test-gltf-animation-mask");
