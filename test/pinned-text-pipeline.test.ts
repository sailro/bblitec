...
test("text pipeline composition retains pin shaders, packed instance layout and target-dependent A2C", async () => {
    ...
    for (const sampleCount of [1, 4]) {
        for (const depthWrite of [false, true]) {
            for (const alphaToCoverage of [false, true]) {
                const actual = await composeTextPipeline({
                    format: "bgra8unorm",
                    sampleCount,
                ...
                const effective =
                    sampleCount > 1 && depthWrite && alphaToCoverage;
                ...
                assert.deepEqual(
                    actual.fragmentConstants,
                    effective ? [{ id: 0, value: 1 }] : [],
                );
                assert.deepEqual(
                    actual.quadCorners,
                ...
                assert.deepEqual(
                    descriptor.layout.bindGroupLayouts[0]!.entries,
                    [
                        {
                            binding: 0,
                            visibility: 3,
                ...
                assert.deepEqual(descriptor.primitive, {
                    topology: "triangle-list",
                    cullMode: "none",
                    frontFace: "ccw",
                });
                assert.deepEqual(descriptor.depthStencil, {
    ...
    const noDepth = await composeTextPipeline({
        format: "rgba16float",
        sampleCount: 1,
        depthWrite: false,
...
test("standalone text weights compose the pinned incremental shader and preserve the base descriptor", async () => {
    const plain = await composeStandaloneTextPipelines(false),
        weighted = await composeStandaloneTextPipelines(true);
    assert.equal(plain.length, 1);
    assert.equal(weighted.length, 2);
    assert.deepEqual(weighted[0], plain[0]);
    const { WEIGHT_SHADER_FRAGMENT } = await importPinnedModule<{
        WEIGHT_SHADER_FRAGMENT: unknown;
    }>("text/shaders/weight-shader-fragment.js");
    const { composeSlugShader } = await importPinnedModule<{
        composeSlugShader(
            this: void,
            fragment: unknown,
    ...
    const expected = composeSlugShader(WEIGHT_SHADER_FRAGMENT),
        actual = weighted[1]!;
    assert.equal(actual.descriptor.vertex.module.code, expected._vert);
    assert.equal(actual.descriptor.fragment.module.code, expected._frag);
    assert.equal(actual.weighted, true);
    assert.equal(actual.descriptor.depthStencil, undefined);
    assert.equal(actual.descriptor.multisample.count, 1);
    assert.deepEqual(actual.descriptor.layout, plain[0]!.descriptor.layout);
    assert.match(textPipelineHeader(weighted), /1u, false, false, false, true/);
    assert.deepEqual(
        await composeStandaloneTextPipelines(false),
        plain,
        "opt-in composition restores the previous resolver",
    );
});
