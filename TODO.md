...
- [ ] Inline async collection callbacks need owned invocation rather than inlining their awaits into the caller; map, predicates, reduce and Array.from have incomplete promise results/truthiness (data-lowering.ts, user-functions.ts). M; concurrent loaders and collection transforms.
...
| Audio | setMasterVolume ramps and eleven REFUSED_BY_NAME APIs (audio.ts:56-93); bus.ts is not lowered (output-projection.ts:446); asynchronous AudioEngine results need owned main-bus metadata; context options/statechange, source ended listeners, closed-context node operations and media-stream graphs remain unrepresented; no browser/native offline PCM gate |
