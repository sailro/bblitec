...
- [ ] Dynamic value boundaries need optional-property presence, class instances created before native storage was demanded, and mutations through erased typed records/arrays (data-types.ts, json-record-views.ts, user-functions.ts, js_json.hpp). L; configuration/state helpers.
...
- [ ] Browser GPU adapter requests, constructor/prototype objects and diagnostic hooks need represented native counterparts; realm graphics availability and identity use the existing host service (browser-erasure.ts, pal_window_realm.cpp). M; direct browser GPU API consumers.
- [ ] Dynamic typeof results need conversion into inferred string-literal enum fields; explicitly string-typed fields are supported (data-sinks/structures.ts). S; unannotated structured messages.
...
- [ ] Imported constant initializers reading otherwise unobserved mutable module state need transitive initializer observation; they can currently refuse an unbound scalar (module-initializers.ts). M; eager scalar snapshots.
...
- [ ] Inline style changes need before/after transition state; RmlUi currently starts transitions for stylesheet/class changes only (pal_ui_rml.cpp, ElementStyle.cpp). M; animated direct style writes.
- [ ] Live RmlUi selector matching duplicates the library matcher; compare cached library reuse and preserve authored-tree queries, generated-node filtering and interaction semantics (ui_selector.hpp, ui_selector_match.hpp, pal_ui_rml.cpp). M; private style metadata and generated content.
- [ ] Grid layout needs spanning/named/alternate placement, percentage tracks and item heights, additional intrinsic track functions and baseline/replaced-item alignment (ui-grid.ts, rmlui-zero-track-grid.patch). L; broader application styles.
- [ ] Container queries need named/minimum/block-size/style/scroll-state conditions, container-relative units and additional containment types (ui-projection.ts, rmlui-zz-container-queries.patch); unnamed maximum-width queries and inline-size containment are represented. M; broader responsive components.
- [ ] List markers need types, counters and images; none preserves the native unmarked list layout (ui-projection.ts, pal_ui_defaults.hpp). M; retained lists.
- [ ] Constructed inputs still need checkbox/radio/number/date types and their browser behavior; text/password/range type writes now reach native controls, while transitions away from file inputs refuse (ui-projection.ts, compileUiBrowserFileAttribute). M; programmatic control construction.
- [ ] Custom range styling needs vertical writing, browser tick marks and Firefox control semantics; horizontal Chromium thumb/track styles use native widgets and Gecko-only range rules follow Chromium selector-list rejection (ui-projection.ts, pal_ui_range.hpp). M; broader control themes.
- [ ] Scroll containers need browser edge handoff/bounce/navigation behavior, both-edge/vertical gutters and viewport gutter propagation; native containment axes and single stable gutters are represented (rmlui-overscroll-axes.patch, rmlui-scrollbar-gutter.patch). M; broader scrolling and writing modes.
...
| Audio | setMasterVolume ramps and eleven REFUSED_BY_NAME APIs (audio.ts:56-93); bus.ts is not lowered (output-projection.ts:446); asynchronous AudioEngine results need owned main-bus metadata; context options/statechange, ended event payloads/onended properties/AbortSignal, nullable buffer assignments, closed-context node operations and media-stream graphs remain unrepresented; no browser/native offline PCM gate |
...
- [ ] Retained selectors need additional attribute operators and nth-child(... of selector). Generated content still needs counters/images, typed attribute fallbacks, outlines/gradient-text and layout substitutions requiring authored handles; placeholder styles only represent color/opacity (ui-selector.ts, ui-projection.ts, pal_ui_rml.cpp). L.
- [ ] Box shadows need omitted/currentColor colors and non-pixel lengths; native cached shadow textures are limited to the viewport extent. Ordered inset/outer pixel shadows, color variables and generated-box painting now use the shared RmlUi cache (ui-filters.ts, pal_ui_snapshot.hpp). M.
...
- [ ] Qualify macOS Metal parity against existing browser references, including platform-font UI differences; validate Apple Silicon development and enable its minimal packaging. L; current host coverage is Intel macOS.
