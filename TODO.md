...
| 261 | Camera projection jitter over the source render task's persistent scene UBO and retained history lifetime. Native generation explicitly refuses these missing renderer contracts. Composite output identity, source render-task descriptors, private live blend-factor writes, pinned execute/rebuild ordering and arc-camera setter/version transport have observing controls. TAA refuses untracked camera producers, target/parent replacement, observable-vector aggregate copies and graph record changes after registration; future admission needs their ownership/record epochs. The scene remains unregistered. |
