...
- [ ] Async results need owned packaged-fetch responses; Promise.all still refuses literal spreads, non-array iterables, stored void/value-only arrays and recovery with a different tuple representation (async.ts). M; deferred asset-loading helpers and reusable aggregation.
