...
| 261 | Camera projection jitter over the source render task's persistent scene UBO, exact camera change-key transport and retained history lifetime. Native generation explicitly refuses these missing renderer contracts. Composite output identity, source render-task descriptor transport, private live blend-factor writes and pinned execute/rebuild ordering have observing controls; the scene remains unregistered. |
