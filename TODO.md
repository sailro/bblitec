...
| 261 | Per-frame task execution and camera projection jitter over the source render task's persistent scene UBO, including history lifetime and reset order. Native generation explicitly refuses these missing contracts. Composite output identity, source render-task descriptor transport and the private live blend-factor writer have observing controls; the scene remains unregistered. |
