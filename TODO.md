...
| Splats | Public splatsData/updateData activation, live uploads/versioning in both PALs and per-cloud plugin sets. Retained CPU buffers and byte-safe numeric ArrayBuffer views now preserve aliases; view methods and native contiguous consumers remain explicitly unsupported until adapted. |
...
| 121 | Activate scene-readable splat rows and live `updateData` re-upload in both PALs over the retained CPU buffer and numeric view support; validate unchanged source, uploaded pixels and the complete mutated byte buffer. |
