...
- [ ] Direct async activations with multiple runtime return paths use a synchronous value lambda and reject nullable returns; direct calls/reactions need the stored coroutine control-flow protocol (async.ts, user-functions.ts). M; cancellable concurrent loaders.
