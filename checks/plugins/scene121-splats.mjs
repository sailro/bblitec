...
export async function check(context) {
    ...
    assert.deepEqual(
        idle,
        first,
        "the browser's retained splat data changed while idle",
    );
    assert(
        readFileSync(context.scene.source, "utf8").includes(assetUrl),
        "the source asset URL changed",
    );
    ...
    for (let row = 0; row < translatedRows; row++) {
        expected.writeFloatLE(
            expected.readFloatLE(row * 32 + 4) + translationY,
            row * 32 + 4,
        );
    }
    ...
    assert.equal(
        first.dataSha256,
        expectedSha256,
        "the browser's retained bytes differ from the authored F32 writes",
    );
        ...
        for (const phase of Object.values(context.results[backend])) {
            ...
            for (const field of ["boundMin", "boundMax"]) {
                assert.deepEqual(
                    ...
                    `${where}: ${field} differs from the pin`,
                );
            }
            for (const sample of first.sampleRows) {
                for (let lane = 0; lane < 3; lane++) {
                    assert.equal(
                        ...
                        `${where}: row ${sample.row} lane ${lane}`,
                    );
                }
            }
            details.phases[where] = {
    ...
    context.log(
        `retained bytes ${expectedSha256.slice(0, 12)} identical on every phase and backend`,
    );
    return { details };
}
