...
function paneDifference(actual, expected, right) {
    ...
    let total = 0;
    for (let y = 40; y < actual.height; y++)
        ...
        for (const phase of Object.values(results)) {
            ...
            if (phase.id === "resize") {
                assert.deepEqual(
                    ...
                    [1000, 600],
                    `${where}: the window did not resize`,
                );
                ...
                for (const right of [false, true]) {
                    ...
                    assert(
                        geometryPixels > 1000,
                        `${where}: the resized ${right ? "right" : "left"} canvas lost its geometry`,
                    );
                }
            ...
            if (phase.id === "idle")
                assert(
                    left === 0 && right === 0,
                    `${where}: idle canvases changed`,
                );
            if (phase.id === "left" || phase.id === "crossing") {
                assert(
                    left > 1,
                    `${where}: left camera input did not change its canvas`,
                );
                assert.equal(
                    right,
                    0,
                    `${where}: the left drag changed the right camera`,
                );
            }
            if (phase.id === "right") {
                assert(
                    right > 1,
                    `${where}: right camera input did not change its canvas`,
                );
                assert.equal(
                    left,
                    0,
                    `${where}: the right drag changed the left camera`,
                );
            }
