...
export function check(context) {
    ...
    for (const frame of observations.captureFrames) {
        assert.equal(
            frame.state.steps,
            frame.frame + 1,
            `browser frame ${frame.frame}: Havok stepped ${frame.state.steps} times`,
        );
    }
    ...
    for (const backend of context.backends) {
        for (const phase of Object.values(context.results[backend])) {
            ...
            if (browserFrame !== undefined) {
                ...
                positionError = Math.max(
                    ...bodies.map((body, index) =>
                        maxError(body.position, expected[index].transform[0]),
                    ),
                );
                        ...
                        Math.min(
                            ...[1, -1].map((sign) =>
                                maxError(
                                    ...
                                    expected[index].transform[1].map(
                                        (value) => value * sign,
                                    ),
                                ),
                            ),
            ...
            const pivotErrors = [
                distance(
                    pivot(bodies[0], [-0.5, 0, -0.5]),
                    pivot(bodies[1], [-0.5, 0, 0.5]),
                ),
                distance(
                    pivot(bodies[4], [0, 0, -0.5]),
                    pivot(bodies[5], [0, 0, 0.5]),
                ),
                distance(
                    pivot(bodies[9], [0.5, 0.5, -0.5]),
                    pivot(bodies[10], [-0.5, -0.5, 0.5]),
                ),
            ];
            assert(
            ...
            const radialDistance = distance(
                pivot(bodies[14], [0, -0.5, 0]),
                pivot(bodies[15], [0, 0.5, 0]),
            );
            assert(
