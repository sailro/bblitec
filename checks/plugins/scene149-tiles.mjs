...
export function check(context) {
    ...
    assert(
        Math.abs(orbit.state.camera.alpha - initial.state.camera.alpha) > 0.1,
        "the browser pointer gesture did not turn the camera",
    );
    assert(
        orbit.state.submissions > initial.state.submissions,
        "the browser stopped submitting during the gesture",
    );
    assert(
        failed.errors?.some((error) => error.includes("#84")),
        "the pin's live resize did not throw Babylon error #84",
    );
    assert.equal(
        failed.state.submissions,
        idle.state.submissions,
        "the pin's RAF loop kept submitting after its thrown resize error",
    );
    assert.notDeepEqual(
        failed.state.targets.intermediate,
        failed.state.targets.resolve,
        "the failed resize left matching targets",
    );
    assert.deepEqual(
        startup.extras.startup.viewport,
        ...
        for (const phase of Object.values(context.results[backend])) {
            ...
            if (phase.id === "orbit") {
                ...
                for (const key of ["alpha", "beta", "radius"]) {
                    assert.equal(
                        phase.capture.camera[key],
                        initial.state.camera[key],
                        `${where}: camera ${key}`,
                    );
                }
            }
            details[where] = {
                worstTileMad: Math.max(
            ...
            context.log(
                `${where}: worst tile MAD ${details[where].worstTileMad.toFixed(4)}`,
            );
        }
    }
    return { details };
