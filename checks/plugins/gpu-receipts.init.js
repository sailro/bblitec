...
(() => {
    ...
    const bytesOf = (data, dataOffset, size) => {
        if (ArrayBuffer.isView(data)) {
            ...
            const length =
                size !== undefined
                    ? size * element
                    : data.byteLength - (dataOffset ?? 0) * element;
            return Array.from(new Uint8Array(data.buffer, offset, length));
        }
        const offset = dataOffset ?? 0;
        return Array.from(
            new Uint8Array(data, offset, size ?? data.byteLength - offset),
        );
    };
            ...
            buffer.unmap = function () {
                for (const entry of ranges) {
                    const bytes = Array.from(
                        new Uint8Array(entry.range.slice(0)),
                    );
    ...
    device.createRenderPipeline = function (desc) {
        ...
        receipts.pipelines.push({
            ...
            label: desc.label ?? "",
            vertex: {
                constants: plain(desc.vertex.constants ?? {}),
                buffers: plain(desc.vertex.buffers ?? []),
            },
            fragment: desc.fragment
                ? {
                      constants: plain(desc.fragment.constants ?? {}),
                      targets: plain(desc.fragment.targets ?? []),
                  }
                : null,
    ...
    GPUQueue.prototype.writeTexture = function (
        ...
        receipts.writes.push({
            id: idOf(destination.texture),
            frame: receipts.frame,
            layout: {
                offset: (layout && layout.offset) ?? 0,
                bytesPerRow: layout && layout.bytesPerRow,
                rowsPerImage: layout && layout.rowsPerImage,
            },
            mipLevel: destination.mipLevel ?? 0,
            origin: plain(destination.origin ?? [0, 0, 0]),
            size: plain(size),
            bytes: bytes.length <= 1048576 ? bytes : null,
            byteLength: bytes.length,
        });
    ...
    for (const proto of [
        ...
        proto.setVertexBuffer = function (slot, buffer, offset, size) {
            ...
            return originalSetVertexBuffer.call(
                this,
                slot,
                buffer,
                offset,
        ...
        proto.setIndexBuffer = function (buffer, format, offset, size) {
            ...
            return originalSetIndexBuffer.call(
                this,
                buffer,
                format,
                offset,
        ...
        proto.drawIndexed = function (
            ...
            return originalDrawIndexed.call(
                this,
                indexCount,
                instanceCount,
                firstIndex,
