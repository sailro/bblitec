        ...
        for (const phase of Object.values(context.results[backend])) {
            ...
            for (const draw of state.draws.filter(
                ...
                for (let axis = 0; axis < 3; ++axis) {
                    assert(
                        Math.abs(
                            packet.floats[28 + axis] - mesh.position[axis],
                        ) < 1e-6,
            ...
            for (const [body, overlay] of [
                ...
                assert.deepEqual(
                    state.meshes[body].position,
                    state.meshes[overlay].position,
                    `${where}: overlay ${overlay} detached from body ${body}`,
                );
                assert.deepEqual(
                    state.meshes[body].rotationQuaternion,
                    state.meshes[overlay].rotationQuaternion,
                    `${where}: overlay ${overlay} rotation detached from body ${body}`,
                );
            }
            assert.deepEqual(
                [4, 6, 8].map((index) => state.meshes[index].geometry),
                [0, 0, 0],
                `${where}: clone geometry 0`,
            );
            assert.deepEqual(
                [10, 12, 14].map((index) => state.meshes[index].geometry),
                [1, 1, 1],
