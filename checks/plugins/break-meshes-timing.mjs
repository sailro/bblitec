...
function summarize(samples) {
    ...
    return {
        ...
        seconds,
        wallSeconds,
        simulatedPerWallSecond: seconds / wallSeconds,
    };
}
...
export function check(context) {
    ...
    assert.equal(
        observed.sceneFixed,
        1000 / 60,
        "Do not change the BBL reference's scene clock",
    );
    assert.equal(
        observed.worldFixed,
        12.5,
        "Do not change the BBL reference's physics clock",
    );
    ...
    const browser = {
        ...
        fps,
        ...(fps === null
            ? summarize(browserSamples)
            : controlledTime(browserSamples.slice(1))),
        dynamicBodies: observed.dynamicBodies,
    };
    assert(
        Math.abs(browser.seconds / browser.steps - 0.0125) < 1e-8,
        `browser step length: ${JSON.stringify(browser)}`,
    );
    if (fps !== null)
        assert(
            Math.abs(browser.seconds - fps * 2 * 0.0125) < 1e-5,
            `browser controlled time: ${JSON.stringify(browser)}`,
        );
    ...
    for (const backend of context.backends) {
        ...
        for (const line of phase.log.split(/\r?\n/)) {
            const match = line.match(
                /^\[physics\] step (\d+) dt ([\d.]+) body (\d+) pos (.*)$/,
            );
            if (!match) continue;
            ...
            if (+body === 0)
                samples.push({
                    ...
                    now:
                        started +
                        samples.length *
                            (fps === null ? 1000 / 60 : 1000 / fps),
                });
            if (+step === 0) firstPositions.set(body, position);
            lastPositions.set(body, position);
        }
        assert(
            firstPositions.size > 100,
            `${backend}: missing demo physics bodies`,
        );
        const movedBodies = [...lastPositions].filter(
            ([body, position]) => firstPositions.get(body) !== position,
        ).length;
        assert(
            movedBodies >= 14,
            `${backend}: shatter input did not move the pieces: ${movedBodies}`,
        );
        ...
        const native = {
            ...
            fps,
            ...(fps === null ? summarize(measured) : controlledTime(measured)),
            movedBodies,
        };
        assert(
            Math.abs(
                native.seconds / native.steps - browser.seconds / browser.steps,
            ) < 1e-8,
            `${backend}: step length ${JSON.stringify(native)}`,
        );
        if (fps !== null) {
            assert.equal(
                measured.length,
                fps * 2,
                `${backend}: two seconds of steps`,
            );
            assert(
                Math.abs(native.seconds - browser.seconds) < 1e-5,
                `${backend}: controlled time ${JSON.stringify({ native, browser })}`,
            );
        }
        results.push(native);
        context.log(
            `${backend}: ${native.steps} steps, ${native.seconds.toFixed(4)} simulated seconds, ${movedBodies} bodies moved`,
        );
    }
