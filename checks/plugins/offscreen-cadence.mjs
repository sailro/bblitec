...
export function check(context) {
    ...
    for (const backend of context.backends) {
        ...
        const rows = [
            ...
            time: Number(match[2]),
            canvases: new Map(
                [...match[3].matchAll(/canvas=(\d+):(\d+)@/g)].map((canvas) => [
                    Number(canvas[1]),
                    Number(canvas[2]),
                ]),
            ),
        ...
        native[backend] = [...first.canvases]
            .sort(([left], [right]) => left - right)
            .map(([id, start]) => {
                const framesPerSecond =
                    (last.canvases.get(id) - start) / seconds;
                return {
                    canvas: id,
                    seconds,
                    framesPerSecond,
        ...
        context.log(
            `${backend}: ${native[backend].map((row) => `canvas ${row.canvas} ${row.framesPerSecond.toFixed(1)} fps`).join(", ")}`,
        );
    }
    context.log(
        `browser: main ${browser.main.framesPerSecond.toFixed(1)} fps, worker ${browser.worker.framesPerSecond.toFixed(1)} fps`,
    );
