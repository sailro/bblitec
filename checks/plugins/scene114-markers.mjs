...
export function check(context) {
    ...
    assert.deepEqual(
        firstState.hits,
        {
            ...
            skeletonGpuHit: "scene114-skeleton-target",
            skeletonDetailedHit: "scene114-skeleton-target",
        },
    ...
    assert.deepEqual(
        idle.state,
        first.state,
        "the browser scene moved while idle",
    );
    assert.equal(
        compareImages(
            observedImage(context, first.image),
            observedImage(context, idle.image),
        ).maxDiff,
        0,
    ...
    const referenceMarkers = MARKERS.map((suffix) => {
        ...
        assert(
            marker && marker.position[1] !== -100,
            `Missing reference marker ${suffix}`,
        );
        return marker;
    });
        ...
        for (const phase of Object.values(context.results[backend] ?? {})) {
            ...
            details[`${backend}/${phase.id}`] = referenceMarkers.map(
                (expected) => {
                    ...
                    const scalingError = maxError(
                        actual.scaling,
                        expected.scaling,
                    );
                    assert(
                        positionError < 1e-7,
                        `${backend}/${phase.id}: ${expected.name} point changed (error ${positionError})`,
                    );
                    assert(
                        scalingError < 1e-7,
                        `${backend}/${phase.id}: ${expected.name} barycentric scale changed (error ${scalingError})`,
                    );
