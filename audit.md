...
| CC-14 | L | partial | Native architecture tests still inspect source text | test/compiler-architecture.test.ts | Compiler dependencies use AST checks and renamed-import compilations. Generated shader output and native fixtures cover value, memory, audio, rendering, transforms, buffer, image and file contracts. Remaining source assertions need behavioral coverage. |
...
| Id | Sev | Status | Defect | Where | Fix |
| --- | --- | --- | --- | --- | --- |
| GC-1 | H | partial | Handle-touching function and method bodies expand at their call sites | compiler/user-functions.ts, classes.ts, function-specializations.ts | Native factories, supported class returns, platform reads and collection callbacks share bodies across callback scopes, with local capture environments. Generation-dependent operations retain inline paths. |
