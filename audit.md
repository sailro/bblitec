...
| CC-10 | M | open | 2,033 `!` (1,374 `arguments[N]!`), 197 casts, 333 `?? literal`, one error-swallowing catch | handle-collections.ts:2192, compiler.ts:15008,16606 | all 1,380 `arguments[N]!` go through `argumentAt`; the catch is narrowed to `CompileError`; 37 `as ts.Identifier` behind non-local guards remain |
...
| CC-14 | L | open | `compiler-architecture.test.ts` asserts 417 regexes over the repository's own source with 0 compilations; 10 test files assert nothing | test/compiler-architecture.test.ts | the assertions tripped by this wave were replaced by behaviour tests (globals, loop control); the remaining source-text assertions stay as pins on the wave's own contracts |
...
| LW-2 | H | open | `.babylon` loader 100% hand-written; its lowerer asserts six shapes | templates/babylon-loader-cpp.ts, babylon-lowerer.ts | `bakeLocalMatrix` lowered whole (bytes pinned by test); `parseBabylonCamera` is not translator-shaped (`??` over an optional element chain, guarded member stores) and keeps its asserted partial lowering |
