...
| RDN-35 | low | The Dawn billboard pass keeps one per-frame scene buffer, so billboards inside render tasks draw with the main camera's block where SDL_GPU uses the task's matrices. | A scene block per task, as the pin binds. | open |
...
| DEAD-17 | low | A dead store in Dawn's pinned-background ground arm (`bound_pipeline = arm.pipeline;`), reported by `lint:cpp` on break-meshes. | Delete it. | open |
