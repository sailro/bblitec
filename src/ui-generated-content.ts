...
export interface UiGeneratedContent {
    enabled: boolean;
    parts: UiContentPart[];
}

/** The represented content list: CSS strings and current attribute text. */
export function parseUiGeneratedContent(
    source: string,
): UiGeneratedContent | undefined {
    let rest = source.trim();
    if (/^(none|normal)$/i.test(rest)) return { enabled: false, parts: [] };
    const parts: UiContentPart[] = [];
    while (rest) {
        const attribute = /^attr\(\s*([A-Za-z_][A-Za-z0-9_-]*)\s*\)/i.exec(
            rest,
        );
        if (attribute) {
            parts.push({
...
export function uiGeneratedContentCpp(
    content: UiGeneratedContent | undefined,
    quote: (value: string) => string,
): string {
    if (!content) return "std::nullopt";
    return `bbl::UiGeneratedContent{${content.enabled}, {${content.parts
        .map(
            (part) =>
                `{bbl::UiContentPartKind::${part.kind === "text" ? "Text" : "Attribute"}, ${quote(part.value)}}`,
        )
        .join(", ")}}}`;
}
