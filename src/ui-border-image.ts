...
function fourSides(
    ...
    if (
        ...
        sides.some((side) => {
            ...
            const number = Number.parseFloat(side),
                native = Math.fround(number);
...
export function parseUiBorderImage(
    value: string,
): UiBorderImage | "none" | undefined {
    ...
    // eslint-disable-next-line no-control-regex -- Reject control bytes in CSS image URLs.
    const invalidSource = /[\\\x00-\x1f]/;
    if (
        invalidSource.test(source) ||
        /(?:\.svg(?:[?#]|$)|^data:)/i.test(source)
    )
        return undefined;
    const components = match[4]!
        .replace(/\bstretch(?:\s+stretch)?\s*$/i, "")
        .trim()
        .split("/");
    if (
        components.length > 3 ||
        (components.length > 1 && !components.at(-1)?.trim())
    )
        return undefined;
    const number = "(?:\\d+(?:\\.\\d*)?|\\.\\d+)";
    const slices = fourSides(
        components[0]?.trim() || "100%",
        new RegExp(`^${number}%?$`),
    );
    const widths = fourSides(
        components[1]?.trim() || "1",
        new RegExp(`^(?:${number}(?:px|%)?|auto)$`),
    );
    const outset = components[2];
    if (
        !slices ||
        !widths ||
        (outset !== undefined && !fourSides(outset, /^0(?:\.0*)?(?:px)?$/))
    )
        return undefined;
...
export function renderUiBorderImage(
    image: UiBorderImage,
