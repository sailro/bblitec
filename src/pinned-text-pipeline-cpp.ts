...
} from "./pinned-text-pipeline.js";

/** The default pass can resize between the pin's supported sample counts. */
export async function composeDefaultTextPipelines(): Promise<
    ComposedTextPipeline[]
> {
    ...
    for (const sampleCount of [1, 4])
        for (const depthWrite of [false, true])
            for (const alphaToCoverage of [false, true]) {
                const row = await composeTextPipeline({
                    format: "bgra8unorm",
                    sampleCount,
...
export async function composeStandaloneTextPipelines(
    weighted: boolean,
): Promise<ComposedTextPipeline[]> {
    const rows = [
        await composeTextPipeline({
            format: "bgra8unorm",
            sampleCount: 1,
            depthWrite: false,
...
export function textPipelineStem(
    index: number,
...
export function textPipelineHeader(
    rows: readonly ComposedTextPipeline[],
): string {
    if (!rows.length)
        throw new Error("Text rendering requires composed pinned pipelines.");
    const first = rows[0]!;
    const d = first.descriptor;
    const bindings = variantBindings(
        d.vertex.module.code,
        d.fragment.module.code,
        0,
    );
    const layout = d.layout.bindGroupLayouts;
    if (layout.length !== 1 || bindings.length !== layout[0]!.entries.length)
        throw new Error(
            "Pinned text binding group reflection differs from its descriptor.",
        );
    const bindingRows = layout[0]!.entries.map((entry) => {
        const binding = bindings.find((row) => row.binding === entry.binding);
        const kind =
            "buffer" in entry ? entry.buffer.type : entry.texture.sampleType;
        const expected =
            kind === "uniform"
                ? "uniformBuffer"
        ...
        if (
            ...
            (Number(binding.vertex) | (Number(binding.fragment) << 1)) !==
                entry.visibility
        )
            throw new Error(
                `Pinned text binding ${entry.binding} reflection differs from its descriptor.`,
            );
        return `    {${entry.binding}u, ${entry.visibility}u, ${cppStringLiteral(binding.name)}, ${cppStringLiteral(kind)}},`;
    });
    const shared = (row: ComposedTextPipeline) =>
        JSON.stringify({
            layout: row.descriptor.layout,
            vertex: row.descriptor.vertex.buffers,
            quad: row.quadCorners,
        });
    if (rows.some((row) => shared(row) !== shared(first)))
        throw new Error("Text variants no longer share layout and quad data.");
    ...
    const pipelineRows = rows.map((row, index) => {
        ...
        if (
            target.length !== 1 ||
            target[0]!.format !== "bgra8unorm" ||
            (descriptor.depthStencil &&
                descriptor.depthStencil.format !== "depth24plus-stencil8")
        )
            throw new Error(
                "Text activation requires the represented color/depth target signature.",
            );
        const constants = (["vertex", "fragment"] as const).map((stage) => {
            const values =
                row[
                    stage === "vertex" ? "vertexConstants" : "fragmentConstants"
                ];
            const name = `text_${index}_${stage}_constants`;
            declarations.push(
                `inline constexpr std::array<ShaderStageConstant, ${values.length}> ${name}{{${values.map((v) => `{${v.id}u, ${doubleLiteral(v.value)}}`).join(", ")}}};`,
            );
            return name;
        });
        const blend = target[0]!.blend;
        const factor = (value: string) => {
            if (!["one", "src-alpha", "one-minus-src-alpha"].includes(value))
                throw new Error(`Unsupported text blend factor '${value}'.`);
            return `BlendFactor::${value.replaceAll("-", "_")}`;
        };
        if (
            blend &&
            (blend.color.operation !== "add" || blend.alpha.operation !== "add")
        )
            throw new Error(
                "Text blend operation requires an unrepresented equation.",
            );
        return (
            `    {${descriptor.multisample.count}u, ${!!descriptor.depthStencil}, ${!!descriptor.depthStencil?.depthWriteEnabled}, ${!!descriptor.multisample.alphaToCoverageEnabled}, ${!!row.weighted}, ` +
    ...
    const bufferRows = d.vertex.buffers.map((buffer, index) => {
        declarations.push(
            `inline constexpr std::array<TextVertexAttribute, ${buffer.attributes.length}> text_attributes_${index}{{${buffer.attributes.map((a) => `{${a.shaderLocation}u, ${a.offset}u, ${cppStringLiteral(a.format)}}`).join(", ")}}};`,
        );
