...
import {
    GltfGeometryPacker,
    type GltfMeshGeometry,
    type GltfRecordedGeometry,
} from "./gltf-mesh-geometry.js";
import {
    packageMeshDeformation,
    readMeshDeformation,
    type GltfMeshDeformation,
    type RecordedMeshDeformation,
} from "./gltf-mesh-deformation.js";
import {
    packageMeshSetup,
    readMeshSetup,
    type GltfMeshSetup,
...
import {
    ...
    packagedGltfLights,
    type GltfLightPlan,
} from "./gltf-light-plan.js";
import {
    packageGltfCamera,
    packagedGltfCameras,
    type GltfCameraPlan,
    type SourceCameraMatrices,
} from "./gltf-camera-plan.js";
import {
    gltfIblSourceUrls,
    GltfIblRecording,
    packagedGltfIbl,
    resolveRecordedIblImage,
    type GltfIblPlan,
    type SourceIblImageResolver,
} from "./gltf-ibl-plan.js";
import {
    gltfFlowGraphSourceUrl,
    GltfFlowGraphRecording,
    packagedFlowGraphPrograms,
    type FlowGraphProgram,
    type PathConverterModule,
} from "./pinned-flow-graph.js";
import {
    gltfAnimationBindingsSourceUrl,
...
import {
    ...
    gltfPointerBridgeUrl,
    gltfPointerSourceUrls,
    packageAnimationReceipt,
    readAnimationReceipt,
    type GltfAnimationReceipt,
    type PointerBridge,
...
export interface GltfMeshPlan extends GltfLightPlan, GltfCameraPlan {
    ...
    ibl: GltfIblPlan;
    flowGraphs: FlowGraphProgram[];
    /** Source interactivity mesh ownership, indexed by the upload schedule. */
    flowGraphNodes: Array<number | null>;
    ...
    materials: number[];
    /** Raw definition identities selected by the source animation base-color feature. */
    baseColorDefinitions: number[];
    baseColorModule: boolean;
...
export type GltfConstructedMaterialPlan = Pick<
    GltfMeshPlan,
    "cores" | "materials" | "meshes" | "geometries"
>;
...
interface MeshData {
    _nodeIndex: number;
    _primitive: JsonObject;
    _vertexCount: number;
}
interface Mesh extends RecordedMeshDeformation, RecordedMeshSetup {
    material: Material;
    name: string;
    _gpu: GltfRecordedGeometry;
    _flatNormal?: boolean;
...
interface UploadContext {
    _engine: { _device: object };
    _json: JsonObject;
    _binChunk: DataView;
    _baseUrl: string;
    _matExts: unknown[];
    _wrapTex: unknown;
    recordMaterial(material: CoreMaterial): Promise<Material>;
    ...
    recordFlowAccessor(pointer: string, context: unknown): unknown;
    recordRunFlowGraphs(
        scene: unknown,
        graphs: unknown,
        animations: unknown,
    ): Promise<never[]>;
    recordFlowRejection(message: unknown, error: unknown): never;
    _parentMap: Map<number, number>;
    _worldMatrixCache: Map<number, Float32Array>;
...
interface SourceLoader {
    ...
    __prepareAnimationPointers(
        ...
        | {
              clearMaterialMap(this: void, meshes: object[]): void;
              baseColorDefinitions?: WeakSet<object> | null;
          }
        | undefined
    >;
    __animationFeature: MeshFeature;
    __instanceFeature: MeshFeature;
    __visibilityFeature: MeshFeature;
    __lightFeature: MeshFeature;
    __iblFeature: MeshFeature;
    __flowGraphFeature: MeshFeature;
    __flowPathConverter: PathConverterModule;
    ...
    __applyPreparedAssets(
        ...
        cameras?: object[];
        flowGraphs?: object[];
        flowGraphRuntimes?: unknown;
        _sceneSetup?: (scene: object) => void;
    }>;
    __prepareMeshes(json: JsonObject): Promise<{
        features: MeshFeature[];
        parentMap: Map<number, number>;
        worldMatrixCache: Map<number, Float32Array>;
    }>;
    buildNodeHierarchy(
        json: JsonObject,
        meshes: Mesh[],
        data: MeshData[],
    ): { root: object; nodeMap: readonly (SceneNode | undefined)[] };
    extractAllMeshes(
        json: JsonObject,
        bin: DataView,
        base: string,
        parents: Map<number, number>,
        world: Map<number, Float32Array>,
        decoded: Map<unknown, unknown>,
        recordCore: (
            json: JsonObject,
            bin: DataView,
            index: number,
...
function materialIdentity(buildGroup?: () => never): object {
    return new Proxy(Object.freeze({}), {
        get(_target, key) {
            ...
            throw new Error(
                `glTF scheduling reads opaque material property '${String(key)}'.`,
            );
        },
    });
}

async function recordingLoader(
    context: LoweringContext,
    ...
    const skeleton = sourceModule(
        ...
        new Map([
            ...
            [
                "../skeleton/create-skeleton.js",
                sourceModule("src/skeleton/create-skeleton.ts"),
            ],
        ]),
    );
    const morph = sourceModule(
        ...
        new Map([
            [
                ...
                sourceModule("src/morph/create-morph-targets.ts"),
            ],
        ]),
    );
    const instances = sourceModule(
        ...
        new Map([
            [
                ...
                sourceModule("src/mesh/thin-instance.ts"),
            ],
            [
                "../mesh/enable-thin-instance-world-bounds.js",
                sourceModule("src/mesh/enable-thin-instance-world-bounds.ts"),
            ],
        ]),
    );
    const primitive = sourceModule(
        "src/loader-gltf/gltf-feature-primitive.ts",
        new Map([
            [
                "../material/pbr/pbr-primitive-topology.js",
                sourceModule("src/material/pbr/pbr-primitive-topology.ts"),
            ],
        ]),
    );
    ...
    const lightState = sourceModule(
        "src/loader-gltf/gltf-light-pointer-state.ts",
    );
    const pointers = gltfPointerSourceUrls(
        context,
        animation,
        lightState,
        pointerBridge,
    ...
    const lightMethod = featureMethod(
        lightFile,
        "KHR_lights_punctual",
        "applyAsset",
    );
    const lightContext = lightMethod.parameters[2]?.name;
    const limitCalls = context.findNodes(
        lightMethod,
        (node): node is ts.CallExpression =>
            ts.isCallExpression(node) &&
            context.expressionMatchesShape(node.expression, "setMaxLights"),
    );
    if (
        !lightContext ||
        !ts.isIdentifier(lightContext) ||
        limitCalls.length !== 1 ||
        limitCalls[0]!.arguments.length !== 1
    )
        context.contractError(
            lightMethod,
            "Expected the glTF light-budget configuration boundary.",
        );
    const lightTransform = ts.transform(lightFile, [
        (visitorContext) => (root) => {
            const visit: ts.Visitor = (node) =>
                node === limitCalls[0]
                    ? ts.factory.updateCallExpression(
                          ...
                          undefined,
                          limitCalls[0].arguments,
                      )
                    : ts.visitEachChild(node, visit, visitorContext);
            return ts.visitNode(root, visit, ts.isSourceFile)!;
        },
    ]);
    const lightSource = transpileForBrowser(
        ts.createPrinter().printFile(lightTransform.transformed[0]!),
        lightModule,
    );
    lightTransform.dispose();
    const lights = pinnedModuleTextUrl(
        "loader-gltf/gltf-feature-lights-punctual.js",
        lightSource,
        [],
        new Map([
            ["./gltf-light-pointer-state.js", lightState],
            ["../light/types.js", sourceModule("src/light/types.ts")],
            ...["point", "directional", "spot"].map(
                (type) =>
                    [
                        `../light/${type}-light.js`,
                        sourceModule(`src/light/${type}-light.ts`),
                    ] as const,
            ),
        ]),
    );
    ...
    const registry = sourceModule(
        ...
        new Map([
            ["./gltf-feature-skeleton.js", skeleton],
            ["./gltf-feature-morph.js", morph],
            ["./gltf-feature-gpu-instancing.js", instances],
            ["./gltf-feature-primitive.js", primitive],
            ["./gltf-ext-node-visibility.js", visibility],
            ["./gltf-feature-lights-punctual.js", lights],
            ["./gltf-ext-lights-image-based.js", ibl.feature],
            ["./gltf-feature-interactivity.js", flowGraphs],
            ["./gltf-feature-animations.js", animations],
            ["./gltf-feature-animation-pointer.js", pointers.feature],
            ["./gltf-sampler-denorm.js", pointers.converter],
        ]),
    );
    const module = "src/loader-gltf/load-gltf.ts";
    const { file, call, material, uploadContext } =
        gltfBaseMaterialConstruction(context);
    const extraction = context.functionDeclaration(
        module,
        "extractAllMeshes",
    ).declaration;
    const assemblies = context.findNodes(
        extraction,
        (node): node is ts.CallExpression =>
            ts.isCallExpression(node) &&
            context.expressionMatchesShape(node.expression, "assembleMaterial"),
    );
    if (assemblies.length !== 1 || assemblies[0]!.arguments.length !== 5)
        context.contractError(
            extraction,
            "Expected glTF core material assembly arguments.",
        );
            ...
            const visit: ts.Visitor = (node) => {
                if (node === call)
                    return ts.factory.createCallExpression(
                        ts.factory.createPropertyAccessExpression(
                            ts.factory.createIdentifier(uploadContext),
                            "recordMaterial",
                        ),
                        undefined,
                        [ts.factory.createIdentifier(material)],
                    );
                if (node === assemblies[0])
                    return ts.factory.updateCallExpression(
                        assemblies[0],
                        ts.factory.createIdentifier("__recordCore"),
                        undefined,
                        assemblies[0].arguments,
                    );
                const visited = ts.visitEachChild(node, visit, visitorContext);
                if (node === extraction && ts.isFunctionDeclaration(visited))
    ...
    const source = transpileForBrowser(
    ...
    const binaryStart = statements.findIndex(
        (statement) =>
            ts.isVariableStatement(statement) &&
            statement.declarationList.declarations.some(
                (declaration) =>
                    ts.isIdentifier(declaration.name) &&
                    declaration.name.text === "activeBin",
            ),
    );
    if (binaryStart < 2)
        context.contractError(
            load,
            "Expected the glTF feature-discovery boundary before activeBin.",
        );
    const preparation = transpileForBrowser(
        `async function __prepareMeshes(json) {\n${statements
            .slice(1, binaryStart)
            .map((statement) => statement.getText(file))
            .join("\n")}\nreturn {features, parentMap, worldMatrixCache};\n}`,
        module,
    );
    const binaryLoop = statements[binaryStart + 1];
    if (
        !binaryLoop ||
        !ts.isForOfStatement(binaryLoop) ||
        !context.expressionMatchesShape(binaryLoop.expression, "features")
    )
        context.contractError(
            load,
    ...
    const pointerPreparation = transpileForBrowser(
        ...
        ${[statements[binaryStart]!, binaryLoop].map((statement) => statement.getText(file)).join("\n")}
        if (activeBin !== binChunk) throw new Error("Animation pointer preparation changed binary storage.");
        return {clearMaterialMap: pointer.__clearPointerMaterialMap, baseColorDefinitions: pointer.__baseColorDefinitions()};
    }`,
        module,
    );
    const assetStart = statements.findIndex(
        (statement) =>
            ts.isVariableStatement(statement) &&
            statement.declarationList.declarations.some(
                (declaration) =>
                    ts.isIdentifier(declaration.name) &&
                    declaration.name.text === "assetFragments",
            ),
    );
    if (assetStart < 0)
        context.contractError(load, "Expected the glTF asset-feature phase.");
    const assetPhase = transpileForBrowser(
        `async function __applyPreparedAssets(features, meshes, root, ctx) {\n${statements
            .slice(assetStart)
            .map((statement) => statement.getText(file))
            .join("\n")}\n}`,
        module,
    );
    const scene = sourceModule(
        "src/scene/scene-core.ts",
        new Map([
            [
                "./mesh-scene-registry.js",
                sourceModule("src/scene/mesh-scene-registry.ts"),
            ],
        ]),
    );
    ...
    const loader = (await import(
        pinnedModuleTextUrl(
            ...
            [
                ...
                "__flowGraphFeature",
                "__flowPathConverter",
                "__animationBindingsSource",
                "__animationPointerBridge",
                "__animationControllerSource",
                "__animationFeature",
                ...cameraExports,
                "__getPunctualLight",
            ],
    ...
    if (options.cameras) {
        if (!loader.__enableGltfCameras)
            throw new Error("Missing glTF camera activation module.");
...
export async function recordMeshPlan(
    ...
    options: GltfLoadFeatures,
): Promise<{ plan: GltfMeshPlan; packer: GltfGeometryPacker }> {
    ...
    const loader = await (context
        ...
        : pinnedLoaders.get(enabledCameras)!);
    const { identityTexWrap } = await importPinnedModule<{
        identityTexWrap: unknown;
    ...
    const baseColorDefinitions = asRecords(document.materials).flatMap(
        (definition, index) =>
            pointerPreparation?.baseColorDefinitions?.has(definition)
                ? [index]
                : [],
    );
    const baseColorModule =
        pointerPreparation?.baseColorDefinitions !== undefined;
    const cores: number[] = [],
        coreRecords = new Map<CoreMaterial, number>();
    const sourceMaterials = asRecords(document.materials);
    let imageCache: unknown[] | undefined;
    const data = await loader.extractAllMeshes(
        document,
        bin,
        "",
        parentMap,
        worldMatrixCache,
        new Map(),
        async (json, bytes, index, base, cache) => {
            if (
                json !== document ||
                bytes !== bin ||
                base !== "" ||
                ...
                throw new Error(
                    "glTF core material scheduling changed its resource ownership.",
                );
            imageCache = cache;
            if (
                index !== -1 &&
                (asIndex(index) === undefined || !sourceMaterials[index])
            )
                throw new Error(
                    "glTF mesh planning received an invalid source material.",
                );
    ...
    try {
        ...
        const uploadContext: UploadContext = {
            ...
            _baseUrl: "",
            _matExts: [],
            _wrapTex: identityTexWrap,
            _parentMap: parentMap,
            _worldMatrixCache: worldMatrixCache,
            recordLightLimit(count) {
                throw new Error(
                    `glTF requested MAX_LIGHTS = ${count}; native light capacity is fixed at the pinned limit.`,
                );
            },
            recordIblImage: (json, bytes, index, base) =>
                resolveRecordedIblImage(
                    loader.__resolveIblImage,
                    json,
                    bytes,
                    index,
                    base,
                ),
            recordFlowAccessor(pointer, supplied) {
                if (!flowRecording)
                    throw new Error(
                        "Flow-graph accessors require the constructed hierarchy.",
                    );
                return flowRecording.resolve(pointer, supplied);
            },
            recordRunFlowGraphs(scene, graphs, animations) {
                if (!flowRecording)
                    throw new Error(
                        "Flow-graph setup requires the constructed hierarchy.",
                    );
                return flowRecording.run(scene, graphs, animations);
            },
            recordFlowRejection(message, error) {
                throw new Error(
                    `${String(message)} ${error instanceof Error ? error.message : String(error)}; the pinned parser rejected this graph.`,
                    { cause: error },
                );
            },
            async recordMaterial(material) {
                ...
                if (core === undefined)
                    throw new Error(
                        "glTF mesh planning lost its assembled material identity.",
                    );
                ...
                const result = materialIdentity(() => {
                    throw new Error(
                        "glTF scene planning cannot execute a material GPU builder.",
                    );
                });
        ...
        const meshes = await loader.uploadMeshes(
            data,
            [
                ...features,
                {
                    applyMesh(input, mesh, suppliedContext) {
                        if (
                            !inputs.has(input) ||
                            suppliedContext !== uploadContext ||
                            associations.has(mesh)
                        )
                            throw new Error(
                                "glTF mesh planning changed its mesh data ownership.",
                            );
        ...
        const { root, nodeMap } = loader.buildNodeHierarchy(
            document,
            meshes,
            data,
        );
        uploadContext._nodeMap = nodeMap;
        flowRecording = new GltfFlowGraphRecording(
            loader.__flowPathConverter,
            document,
            new Map(
                nodeMap.flatMap((node, index) =>
                    node ? [[node, index] as const] : [],
                ),
            ),
            new Map(
                [...builtRecords].map(([material, index]) => [
                    material,
                    cores[materials[index]!]!,
                ]),
            ),
        );
        ...
        const prepared = features.filter(
            (feature) =>
                feature === loader.__instanceFeature ||
                feature === loader.__visibilityFeature ||
                feature === loader.__lightFeature ||
                feature === loader.__cameraFeature ||
                feature === loader.__iblFeature ||
                feature === loader.__flowGraphFeature,
        );
        const container = await loader.__applyPreparedAssets(
            prepared,
            meshes,
            root,
            uploadContext,
        ...
        if (
            ...
            !Array.isArray(container.entities) ||
            (container._sceneSetup !== undefined &&
                typeof container._sceneSetup !== "function") ||
            (container.cameras !== undefined &&
                (!Array.isArray(container.cameras) ||
                    container.cameras.some((camera) => !asObject(camera)))) ||
            container.entities[0] !== root ||
            container.entities
                .slice(1)
                .some((entity) => !asObject(entity) || !("lightType" in entity))
        )
            throw new Error("Unrepresented glTF mesh asset fragment.");
        const scene = {
            meshes: [] as Mesh[],
            lights: [] as object[],
            _groups: new Map<object, object>(),
        ...
        loader.__addToScene(
            new Proxy(Object.seal(scene), {
                get(target, key, receiver): unknown {
                    if (!Reflect.has(target, key))
                        throw new Error(
                            `Unrepresented glTF scene registration field '${String(key)}'.`,
                        );
        ...
        const sceneMeshes = scene.meshes.map((mesh) => {
            ...
            if (index === undefined)
                throw new Error(
                    "glTF scene registration returned an unknown loader mesh.",
                );
            return index;
        });
        const nodes = asRecords(document.nodes),
            definitions = asRecords(document.meshes);
        const primitiveIndices = definitions.map(
            (definition) =>
                new Map(
                    asRecords(definition.primitives).map((primitive, index) => [
                        primitive,
                        index,
                    ]),
                ),
        );
        if (meshes.length !== data.length)
            throw new Error(
                "glTF mesh planning changed its extraction cardinality.",
            );
        ...
        const plannedMeshes = meshes.map((mesh) => {
            ...
            if (!input || asIndex(input._nodeIndex) === undefined)
                throw new Error("glTF mesh planning lost its source node.");
            const node = nodes[input._nodeIndex];
            const meshIndex = asIndex(node?.mesh);
            const primitive =
                meshIndex === undefined
                    ? undefined
                    : primitiveIndices[meshIndex]?.get(input._primitive);
            if (primitive === undefined)
                throw new Error(
                    "glTF mesh planning lost its source primitive.",
                );
            ...
            if (
                ...
                (mesh._flatNormal !== undefined &&
                    typeof mesh._flatNormal !== "boolean")
            )
                throw new Error(
                    "glTF mesh planning lost its constructed resource identity.",
                );
            ...
            if (geometry === undefined) {
                ...
                geometryRecords.push(
                    packer.geometry(mesh._gpu, input._vertexCount),
                );
            }
            return {
                node: input._nodeIndex,
                primitive,
                material,
                geometry,
                name: mesh.name,
        ...
        if (nodeMap.length !== nodes.length)
            throw new Error(
                "glTF mesh planning changed its node-map cardinality.",
            );
        const nodeVisibility = Array.from(nodeMap, (node) => {
            if (
                ...
                throw new Error("Invalid constructed glTF node visibility.");
            return node?.visible !== false;
        });
        const lightDefinitions = asObject(
            asObject(document.extensions)?.KHR_lights_punctual,
        )?.lights;
        const targets = Array.isArray(lightDefinitions)
            ? lightDefinitions.map((_, index) =>
                  loader.__getPunctualLight(document, index),
              )
            : [];
        const sourceLights = [
            ...new Set([
                ...scene.lights,
                ...targets.filter(
                    (light): light is object => light !== undefined,
                ),
            ]),
        ];
        const lightIndices = new Map(
            sourceLights.map((light, index) => [light, index]),
        );
        const sourceCameras = [...new Set(container.cameras ?? [])];
        const nodeIndices = new Map<object, number>(
            sourceLights.length || sourceCameras.length
                ? nodeMap.flatMap((node, index) =>
                      node ? [[node, index] as const] : [],
                  )
                : [],
        );
        const lights = sourceLights.map((light) =>
            packageGltfLight(light, nodeIndices, packer),
        );
        const sceneLights = scene.lights.map((light) =>
            lightIndices.get(light)!,
        );
        const lightTargets = targets.map((light) =>
            light === undefined ? null : lightIndices.get(light)!,
        );
        const cameraIndices = new Map(
            sourceCameras.map((camera, index) => [camera, index]),
        );
        if (sourceCameras.length && !loader.__cameraMatrices)
            throw new Error("Unrepresented glTF camera construction.");
        const cameras = sourceCameras.map((camera) =>
            packageGltfCamera(
                camera,
                nodeIndices,
                packer,
                loader.__cameraMatrices!,
            ),
        );
        const containerCameras = (container.cameras ?? []).map((camera) =>
            cameraIndices.get(camera)!,
        );
        const iblPlan = await iblRecording.package(packer);
        const flowGraphs = flowRecording.package(
            container.flowGraphs,
            container.flowGraphRuntimes,
        );
        const flowScope = asObject(container.flowGraphs?.[0])?._assetScope;
        const flowGraphNodes = meshes.map((mesh) => {
            if (!flowScope || mesh._flowGraphAssetScope !== flowScope)
                return null;
            const node = asIndex(mesh._gltfNodeIndex);
            if (node === undefined || node >= nodes.length)
                throw new Error("Invalid source flow-graph mesh node.");
            return node;
        });
        let animation: GltfAnimationReceipt | null = null,
            animationBindings: GltfAnimationBindings | null = null;
        if (features.includes(loader.__animationFeature)) {
            const materialInputs = pointerPreparation
                ? (
                      await (
                          await import("./pinned-material-arms.js")
                      ).materialSubjects(
                          document,
                          {},
                          {
                              ...
                              deferAnimationPointers: true,
                              ...(context ? { context } : {}),
                          },
                      )
                  ).map((subject) => subject.input)
                : [];
            const recording = packageAnimationReceipt(
                loader.__animationBindingsSource,
                loader.__animationPointerBridge,
                loader.__animationControllerSource,
                document,
                bin,
                meshes,
                parentMap,
                ...
                nodeMap,
                materialInputs,
                plannedMeshes.map((mesh) => mesh.material),
                packer,
                pointerPreparation?.clearMaterialMap ?? (() => {}),
            );
            animation = recording.receipt;
            animationBindings = recording.terminal
                ? packSourceAnimationBindings(
                      recording.terminal,
                      meshes,
                      nodeMap,
...
export function packagedGltfMeshPlan(document: JsonObject): GltfMeshPlan {
    ...
    if (
        ...
        !plan.cores.every(
            (index: unknown): index is number =>
                index === -1 ||
                (typeof index === "number" &&
                    asIndex(index) !== undefined &&
                    index < sourceMaterialCount),
        ) ||
        !areGltfIndices(plan.materials, plan.cores.length) ||
        !Array.isArray(plan.meshes) ||
        !Array.isArray(plan.geometries) ||
        !areGltfIndices(plan.sceneMeshes, plan.meshes.length) ||
        !Array.isArray(plan.flowGraphNodes) ||
        plan.flowGraphNodes.length !== plan.meshes.length ||
        !plan.flowGraphNodes.every(
            (index: unknown): index is number | null =>
                index === null ||
                (typeof index === "number" &&
                    asIndex(index) !== undefined &&
                    index < nodes.length),
        ) ||
        !Array.isArray(plan.nodeVisibility) ||
        plan.nodeVisibility.length !== nodes.length ||
        !plan.nodeVisibility.every(
    ...
    const geometries = plan.geometries.map((value) => {
        const geometry = asObject(value),
            attributes = asObject(geometry?.attributes),
            indices = asIndex(geometry?.indices);
        if (
            !attributes ||
            indices === undefined ||
            indices >= accessorCount ||
            !areGltfIndices(Object.values(attributes), accessorCount) ||
            !["POSITION", "NORMAL", "TEXCOORD_0"].every(
                (key) => asIndex(attributes[key]) !== undefined,
            )
        )
            throw new Error("Invalid packaged glTF geometry.");
        const mapped: Record<string, number> = {};
        for (const [key, value] of Object.entries(attributes))
            mapped[key] = asIndex(value)!;
        return { attributes: mapped, indices };
    });
    const definitions = asRecords(document.meshes);
    const primitives = definitions.map((definition) =>
        asRecords(definition.primitives),
    );
    const skinCount = asRecords(document.skins).length;
    const materialIndices = plan.materials,
        meshRecords = plan.meshes;
    const meshes = meshRecords.map((value) => {
        const mesh = asObject(value);
        const node = asIndex(mesh?.node),
            primitive = asIndex(mesh?.primitive),
            material = asIndex(mesh?.material),
            geometry = asIndex(mesh?.geometry);
        const definition =
            node === undefined ? undefined : asIndex(nodes[node]?.mesh);
        if (
            !mesh ||
            node === undefined ||
            primitive === undefined ||
            material === undefined ||
            material >= materialIndices.length ||
            geometry === undefined ||
            geometry >= geometries.length ||
            typeof mesh.name !== "string" ||
            typeof mesh.flatNormal !== "boolean" ||
            definition === undefined ||
            !primitives[definition]?.[primitive]
        )
    ...
    const animation =
        plan.animation === null
            ? null
            : readAnimationReceipt(plan.animation, {
                  nodes: nodes.length,
                  materials: materialIndices.length,
                  accessors: accessorCount,
                  skeletons: animationBindings?.skeletons.length ?? 0,
                  morphs: animationBindings?.morphs.length ?? 0,
              });
    if (animation?.accepted && !animationBindings)
        throw new Error(
            "Accepted glTF animation is missing source target bindings.",
        );
    if (
        !areGltfIndices(
            plan.baseColorDefinitions,
            asRecords(document.materials).length,
        ) ||
        typeof plan.baseColorModule !== "boolean"
    )
        throw new Error(
    ...
    return {
        ...
        baseColorModule: plan.baseColorModule,
        nodeVisibility: plan.nodeVisibility,
        sceneMeshes: plan.sceneMeshes,
        ...packagedGltfLights(document),
        ...packagedGltfCameras(document),
        ibl: packagedGltfIbl(document),
        flowGraphs: packagedFlowGraphPrograms(document),
        flowGraphNodes: plan.flowGraphNodes,
        animationBindings,
        animation,
        meshes,
        geometries,
...
export async function packageGltfMeshPlan(
    document: JsonObject,
    bin: DataView,
    context?: LoweringContext,
    options: GltfLoadFeatures = {},
): Promise<Buffer> {
    if (GLTF_MESH_PLAN in document)
        throw new Error(
            "glTF source already carries compiler mesh scheduling metadata.",
        );
    const { plan, packer } = await recordMeshPlan(
        document,
        bin,
        context,
        options,
