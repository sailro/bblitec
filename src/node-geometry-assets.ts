...
export async function nodeGeometryAssetRefusal(
    document: JsonObject,
): Promise<string | undefined> {
    if (asRecords(document.animations).length > 0)
        return "animated imported geometry";
    const meshes = Array.isArray(document.meshes) ? document.meshes : [];
    const accessors = Array.isArray(document.accessors)
        ? document.accessors
        : [];
    const views = Array.isArray(document.bufferViews)
    ...
    for (const node of asRecords(document.nodes)) {
        ...
        if (asObject(node.extensions)?.EXT_mesh_gpu_instancing !== undefined)
            return "instanced imported geometry";
        ...
        for (const primitive of asRecords(mesh.primitives)) {
            ...
            for (const [name, type] of [
                ...
                if (
                    ...
                    return `non-FLOAT ${type} imported ${name}`;
                if (name === "POSITION") positionCount = accessor.count;
                else if (accessor.count !== positionCount)
                    return `mismatched imported ${name} count`;
                const viewIndex = asIndex(accessor.bufferView);
                const view =
                    viewIndex === undefined
                        ? undefined
                        : asObject(views[viewIndex]);
            ...
            for (const [name, rawIndex] of Object.entries(attributes ?? {})) {
                ...
                if (index === undefined)
                    return `an unresolved imported ${name} accessor`;
                const accessor = asObject(accessors[index]);
                if (
                    !accessor ||
                    ![5121, 5123, 5125, 5126].includes(
                        Number(accessor.componentType),
                    )
                )
                    return `unsupported imported ${name} component format`;
                if (accessorIsStrided(document, index))
                    return `strided imported ${name} bufferView`;
            }
        }
    }
    return undefined;
