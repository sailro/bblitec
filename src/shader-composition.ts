...
function stageDeclaration(
    ...
    if (!/^[A-Za-z_]\w*$/.test(entryPoint))
        throw new Error(`Invalid shader entryPoint '${entryPoint}'.`);
    if (value.constants === undefined || value.constants === null)
        return { entryPoint };
    if (!Array.isArray(value.constants))
        throw new Error("Shader constants must be an array.");
    ...
    for (const item of value.constants) {
        ...
        if (
            ...
            throw new Error(
                "Shader constant id must be an integer in 0..65535.",
            );
        }
        if (typeof amount !== "number" || !Number.isFinite(amount))
            throw new Error(
                "Shader constant value must be a finite JSON number.",
            );
    ...
    return {
        entryPoint,
        constants: constants.sort((left, right) => left.id - right.id),
    };
}
...
export function shaderStageConstants(stage: ShaderStageDeclaration): string {
    return [...(stage.constants ?? [])]
        ...
        .map(({ id, value }) => `${id}=${Object.is(value, -0) ? "-0" : value}`)
        .join(",");
}

/** A module may deploy once and declare multiple compiled stage stems. */
export function readShaderComposition(
    directory: string,
): Map<string, OfflineShaderStage> {
    ...
    const add = (stage: OfflineShaderStage): void => {
        ...
        if (stages.has(stage.stem))
            throw new Error(
                `Duplicate shader stage stem '${stage.stem}' in ${path}.`,
            );
    ...
    for (const item of parsed.modules) {
        ...
        if (typeof module.pinnedBindings !== "boolean")
            throw new Error(
                `Shader module '${output}' must declare pinnedBindings.`,
            );
        const shared = { sourceName, pinnedBindings: module.pinnedBindings };
        add({
            ...stageDeclaration(module),
            ...shared,
            stem: sourceName.slice(0, -".native.wgsl".length),
        });
        if (module.alsoStages !== undefined && module.alsoStages !== null) {
            if (!Array.isArray(module.alsoStages))
                throw new Error(
                    `Shader module '${output}' alsoStages must be an array.`,
                );
            for (const item of module.alsoStages) {
                const extra = object(item, "Additional shader stage");
                add({
                    ...stageDeclaration(extra),
                    ...shared,
                    stem: text(extra.stem, "Shader stage stem"),
                });
            }
        }
    }
