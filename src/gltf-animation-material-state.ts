...
export interface GltfAnimationMaterialState {
    index: number;
    patches: GltfAnimationMaterialPatch[];
}
...
function snapshot(
    ...
    for (const [key, child] of Object.entries(value))
        result.fields!.set(key, snapshot(child, seen));
    return result;
}
function unchanged(
    previous: Snapshot | undefined,
    value: unknown,
    seen = new Set<Snapshot>(),
): boolean {
    ...
    return (
        fields.length === previous.fields.size &&
        fields.every(([key, child]) =>
            unchanged(previous.fields!.get(key), child, seen),
        )
    );
}
function transport(
    ...
    if (typeof value !== "object" || ancestors.has(value))
        throw new Error(
            "Unrepresented source pointer material initialization value.",
        );
    ...
    return Array.isArray(value)
        ? {
              kind: "array",
              values: Array.from(value, (child) => transport(child, next)),
          }
        : {
              kind: "object",
...
export function recordAnimationMaterialState(
    materials: readonly object[],
): () => GltfAnimationMaterialState[] {
        ...
        materials.flatMap((material, index) => {
            ...
            const visit = (
                ...
                if (ancestors.has(value))
                    throw new Error(
                        "Unrepresented cyclic source pointer material mutation.",
                    );
                ...
                if (previous.value !== value)
                    patches.push({ path, operation: "clone" });
                const next = new Set(ancestors);
                next.add(value);
...
export function readAnimationMaterialState(
    value: unknown,
    count: number,
): GltfAnimationMaterialState[] {
    const fail = (): never => {
        throw new Error(
            "Invalid packaged glTF pointer material initialization.",
        );
    };
    ...
    const data = (value: unknown): GltfAnimationMaterialValue => {
        ...
        if (item.kind === "array" && Array.isArray(item.values))
            return { kind: "array", values: item.values.map(data) };
        if (item.kind === "object")
            return {
                kind: "object",
                fields: Object.fromEntries(
                    Object.entries(object(item.fields)).map(([key, child]) => [
                        key,
                        data(child),
                    ]),
                ),
    ...
    return value.map((entry) => {
        ...
        return {
            index,
            patches: state.patches.map((value): GltfAnimationMaterialPatch => {
                ...
                if (
                    ...
                    return fail();
                if (patch.operation === "clone" || patch.operation === "erase")
                    return { path: patch.path, operation: patch.operation };
