...
type Schedule = (
    ...
    build: (material: Material) => Material,
) => Promise<VariantData>;
type Select = (
    container: { materialVariants: VariantData },
    name: string,
) => void;
let pinnedRunners: { schedule: Schedule; select: Select } | undefined;

function variantRunners(context: LoweringContext): {
    schedule: Schedule;
    select: Select;
} {
    const source = gltfVariantMaterialSource(context);
    const schedule = createJavaScriptFunction(
        "json",
        "binChunk",
        "baseUrl",
        "variantNames",
        ...
        "buildMaterial",
        transpileCommonJs(
            `return (async () => { ${source.schedule} })();`,
            "gltf-variant-schedule.ts",
        ),
    ) as Schedule;
...
export async function gltfVariantPlan(
    document: JsonObject,
    context?: LoweringContext,
    constructed?: GltfConstructedMaterialPlan,
): Promise<GltfVariantPlan> {
    ...
    if (!context && !constructed && GLTF_VARIANT_PLAN in document) {
        ...
        if (
            Object.keys(selections).length !== new Set(names).size ||
            names.some((name) => !Object.hasOwn(selections, name)) ||
            Object.values(selections).some(
                (slots) =>
                    !areGltfIndices(slots, baseCount + materials.length) ||
                    slots.length !== meshCount,
            )
        )
            throw new Error(
                "Invalid packaged glTF variant selection metadata.",
            );
        return {
            baseCount,
            materials,
    ...
    const data = await schedule(
        ...
        (json, bytes, index, base, cache) => {
            if (
                ...
                throw new Error(
                    "Variant material scheduling changed its resource ownership.",
                );
            if (asIndex(index) === undefined || !asObject(definitions[index]))
                throw new Error(
                    "Variant mapping references an invalid glTF material.",
                );
            return { index };
        },
        (material) => {
...
export async function packageVariantPlan(document: JsonObject): Promise<void> {
    if (GLTF_VARIANT_PLAN in document)
        throw new Error(
            "glTF source already carries compiler variant material metadata.",
        );
    if (gltfVariantNames(document).length)
        document[GLTF_VARIANT_PLAN] = await gltfVariantPlan(document);
}
