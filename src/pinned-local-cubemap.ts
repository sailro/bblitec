...
import {
    ...
    RecordedBuffer,
    RecordedTexture,
    type RecordedTextureView,
} from "./recording-device.js";

export type LocalCubemapJson =
    | number
    | string
    | boolean
    | null
...
interface LocalCubemapModule {
    ...
    setPbrLocalEnvironment(
        ...
        options: LocalCubemapPlan["options"],
    ): void;
...
interface LocalFragmentModule {
    pbrExt: {
        ...
        frag(context: object): {
            _uboFields: { _name: string; _type: string }[];
        };
        writeUbo(
            data: Float32Array,
            material: object,
...
export function pinnedLocalCubemapLimits(): {
    ...
    return {
        defaultCandidates: context.numericValue(
        ...
        candidateCapacity: context.numericValue(
            context.variableInitializer(
                file,
...
export async function applyPinnedLocalCubemap(
    material: object,
    maxCandidates: number,
): Promise<void> {
    const pin = await importPinnedModule<LocalCubemapModule>(
    ...
    const state = await importPinnedModule<{
        _setPbrLocalEnvironment(
            this: void,
            material: object,
            state: { kind: "environment"; environment: object },
        ): void;
    }>("material/pbr/pbr-local-cubemap-state.js");
    state._setPbrLocalEnvironment(material, {
        kind: "environment",
        environment: {},
    });
}

export async function packLocalCubemap(
    plan: LocalCubemapPlan,
): Promise<LocalCubemapPacket> {
    ...
    const parser = await importPinnedModule<{
        parseEnvFile(
            this: void,
            buffer: ArrayBuffer,
        ): { width: number; mipCount: number; irradianceSH: Float32Array };
    }>("loader-env/env-parse.js");
    const assembler = await importPinnedModule<{
        assembleEnvironmentTextures(
            this: void,
            cube: RecordedTexture,
            brdf: RecordedTexture,
            sh: Float32Array,
            lod: number,
            engine: object,
        ): RecordedEnvironment;
    ...
    const lod = context.callExpression(
        loader.declaration,
        "assembleEnvironmentTextures",
    ).arguments[3];
    if (!lod)
        throw new Error("Pinned .env loader has no LOD-generation scale.");
    ...
    const environments = plan.environments.map((source, index) => {
        ...
        const texture = new RecordedTexture({
            size: [parsed.width, parsed.width, 6],
            mipLevelCount: parsed.mipCount,
            format: "rgba16float",
            usage: flags.TU.TEXTURE_BINDING | flags.TU.COPY_SRC,
        });
        environmentIndices.set(texture, index);
        return assembler.assembleEnvironmentTextures(
            texture,
            texture,
            parsed.irradianceSH,
            lodGenerationScale,
    ...
    if (environments.length === 0)
        throw new Error("Local cubemap has no environment.");
    const material = {};
    let set: RecordedProbeSet | undefined;
    if (plan.kind === "probes") {
        const authored = plan.options["probes"];
        if (!Array.isArray(authored))
            throw new Error("Local probe options require a probe array.");
        const probes = authored.map((probe) => {
            if (!probe || typeof probe !== "object" || Array.isArray(probe))
                throw new Error("Local probe is not an object.");
            const index = probe["environment"];
            if (typeof index !== "number" || !environments[index])
                throw new Error("Local probe environment identity is missing.");
            return { ...probe, environment: environments[index] };
        });
        set = pin.createPbrLocalEnvironmentProbeSet(
            { surface: { engine }, _disposables: [] },
            { ...plan.options, probes },
        );
        if (plan.debug !== undefined)
            pin.setPbrLocalEnvironmentProbeDebug(set, plan.debug);
        pin.setPbrLocalEnvironmentProbeSet(material, set);
    } else if (plan.kind === "single")
        pin.setPbrLocalEnvironment(material, environments[0]!, plan.options);
    else pin.setPbrEnvironment(material, environments[0]!);
    const fragment = (
        await importPinnedModule<LocalFragmentModule>(
            "material/pbr/fragments/local-cubemap-fragment.js",
        )
    ).pbrExt;
    const features = fragment.detect(material);
    const fields = fragment.frag({
        _features: features.f,
        _features2: features.f2,
        _hasSceneIbl: true,
    })._uboFields;
    // Independent padded slots let the pinned writer name every field without depending on any composed layout.
    const offsets = new Map(
        fields.map((field, index) => [field._name, index * 16]),
    );
    ...
    fragment.bind(
        { _engine: engine, _material: material, _env: environments[0] },
        entries,
        0,
    );
    const buffer = (index: number) => {
        ...
        if (
            ...
            !("buffer" in resource) ||
            !(resource.buffer instanceof RecordedBuffer)
        )
            throw new Error(
                "Pinned local cubemap binding is not the recorded buffer.",
            );
        return Array.from(new Uint32Array(resource.buffer.bytes));
    };
    const texture = set?._texture ?? environments[0]!.specularCube;
    const copies: LocalCubemapPacket["copies"] = recorder.textureCopies.map(
        (copy) => ({
            source: environmentIndices.get(copy.source.texture) ?? -1,
            sourceMip: copy.source.mipLevel,
            sourceLayer: copy.source.origin.z,
            mip: copy.destination.mipLevel,
            layer: copy.destination.origin.z,
            size: copy.size.width,
        }),
    );
    if (!set)
        for (let mip = 0; mip < texture.mipLevelCount; mip++)
            for (let layer = 0; layer < 6; layer++)
                copies.push({
                    ...
                    mip,
                    layer,
                    size: Math.max(1, texture.width >> mip),
                });
    return {
        overridesEnvironment: plan.kind !== "probes",
        uniform: buffer(0),
        grid: buffer(1),
        width: texture.width,
...
export function packLocalCubemapSync(plan: LocalCubemapPlan): string {
    const execute = () =>
        Buffer.from(
            runGenerationChild({
                label: "Packing pinned local cubemap resources",
...
process.stdout.write(JSON.stringify(await packLocalCubemap(JSON.parse(readFileSync(0, 'utf8')))));`,
                ...
                env: { BBLITE_LOCAL_CUBEMAP_MODULE: import.meta.url },
            }),
        );
    const modules = moduleClosureBytes(
        [
            "material/pbr/enable-pbr-local-cubemap.js",
            "loader-env/env-parse.js",
            "loader-env/env-helpers.js",
        ...
        ? cachedBakeSync(
              {
                  ...
                  inputs: [
                      ...modules,
                      ...plan.environments.map((source) =>
                          readAssetBytesSync(source, plan.entryFileName),
                      ),
                  ],
              },
              execute,
