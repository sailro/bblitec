...
export function readNodeInputProperty(
    ...
    if (owner.kind === "material" && name === "inputs") {
        ...
        return {
            kind: "data",
            cpp: `bbl::node_material_inputs(${owner.engineCpp}, ${owner.cpp})`,
            dataType: {
                kind: "map",
                key: { kind: "string" },
                value: { kind: "handle", handle: "node-input" },
            },
            freshData: true,
        };
    }
    ...
    if (name === "texture")
        return {
            kind: "data",
            cpp: `bbl::node_input_texture(${owner.cpp})`,
    ...
    if (name === "type")
        return {
            kind: "string",
            cpp: `bbl::node_input_type(${owner.cpp})`,
...
function isInputMap(
    ...
    const type = context.checker
        .getTypeAtLocation(context.unwrap(expression))
        .getStringIndexType();
    return (
        type !== undefined &&
        pinnedHandleKind(context.checker.getNonNullableType(type)) ===
            "node-input"
    );
}

export function compileNodeInputMutation(
    context: NodeInputContext,
    expression: ts.Expression,
): Value | undefined {
    const node = context.unwrap(expression);
    if (
        ...
        if (
            isInput(context, target) ||
            isInputMap(context, target) ||
            isPinnedType(type, ["NodeMaterial"])
        ) {
            context.fail(
                node,
                "Reflective node input mutation is not represented.",
            );
        }
    }
    ...
    if (
        ...
        return undefined;
    if (isInputMap(context, left.expression))
        context.fail(
            left,
            "Node input map replacement is not represented; retain and assign the texture2d handle instead.",
        );
    ...
    context.emit({
        kind: "declaration",
        type: "const bbl::NodeInputHandle",
        name: owner,
        initializer: context.dataLowerer.compileForSink(left.expression, {
            kind: "handle",
            handle: "node-input",
        }),
    });
    ...
    return {
        kind: "data",
        cpp: `bbl::set_node_input_texture(${owner}, ${texture})`,
        dataType: textureType,
        freshData: true,
    };
}
