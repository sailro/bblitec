...
export class DataLowerer {
    ...
    public compileDataMethodCall(
        ...
        const dynamicOwner =
            ...
            ts.isNewExpression(callee.expression) ||
        ...
        if (dataType?.kind === "string") {
            ...
            if (method === "toLowerCase") {
                if (call.arguments.length !== 0) {
                    this.context.fail(call, "String.toLowerCase takes no arguments.");
                }
                return {
                    kind: "data",
            ...
            if (method === "trim") {
                if (call.arguments.length !== 0) {
                    this.context.fail(call, "String.trim takes no arguments.");
                }
                return {
                    kind: "data",
            ...
            if (method === "split") {
                ...
                const separatorValue = this.context.compileValue(
                    call.arguments[0]!,
                );
                ...
                const separator = this.compileForSink(
                    call.arguments[0]!,
                    { kind: "string" },
                );
                return {
            ...
            if (method === "replace") {
                ...
                const pattern = this.context.compileValue(
                    call.arguments[0]!,
                );
            ...
            if (method === "endsWith") {
                if (call.arguments.length !== 1) {
                    this.context.fail(call, "String.endsWith expects one argument.");
                }
                const suffix = this.compileForSink(
                    call.arguments[0]!,
                    { kind: "string" },
                );
                return {
        ...
        if (method === "reduce") {
            ...
            const source =
                this.context.allocateTemporaryCppName("reduce_source");
            const count =
                this.context.allocateTemporaryCppName("reduce_count");
            const index =
                this.context.allocateTemporaryCppName("reduce_index");
            const accumulator =
                this.context.allocateTemporaryCppName("reduce_result");
            this.context.emit(`auto&& ${source} = ${narrowed.cpp};`);
            this.context.emit(
                `const std::size_t ${count} = ${source}.size();`,
            );
            this.context.emit(
                `${this.context.dataTypes.cppType(resultType)} ${accumulator} = ${this.compileForSink(call.arguments[1]!, resultType)};`,
            );
            this.context.emit(
                `for (std::size_t ${index} = 0; ${index} < ${count}; ++${index}) {`,
            );
            ...
            try {
                ...
                this.context.emit(
                    `${accumulator} = ${this.compileKnownValueForSink(reduced, resultType, callback)};`,
                );
            } finally {
                this.context.popScope();
                this.context.decreaseIndent();
            ...
            this.context.emit("}");
            this.registerLocal(accumulator, "owned");
            return this.leafValue(accumulator, resultType);
        }
        if (method === "some") {
            const result =
                this.context.allocateTemporaryCppName(
        ...
        if (method === "map") {
            ...
            if (
                ...
                this.context.emit(`auto&& ${source} = ${narrowed.cpp};`);
                this.context.emit(`bbl::js::Array<double> ${output};`);
                ...
                this.context.emit(`${output}.push_back(${converted});`);
                this.context.decreaseIndent();
                this.context.emit("}");
                this.registerLocal(output, "owned");
                return {
            ...
            this.emitArrayCallbackLoop(
                ...
                (result, callback) => {
                    ...
                    if (
                        ...
                        value = this.compileKnownValueForSink(
                            result,
                            mappedType.element,
                            callback,
                        );
     ...
     * `new Float32Array`, `new Uint8Array`, `new Uint16Array`, `new Int16Array`, `new Uint32Array`, and `new Int32Array` (sized, from a numeric
