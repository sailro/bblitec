...
export function compileWindowServiceCall(
    ...
    if (
        ...
        if (!context.options.workers)
            context.fail(
                call,
                "Location reload requires an asynchronous application realm.",
            );
    ...
    if (!context.options.workers)
        context.fail(
            call,
            "Clipboard promises require an asynchronous application realm.",
        );
    requireWindowHost(context, call);
    context.expectArgumentCount(call, 1, 1);
    return lowerer.leafValue(
        `bbl::pal::window_clipboard_write(${lowerer.compileForSink(call.arguments[0]!, { kind: "string" })})`,
        { kind: "promise" },
    );
}

export function requireWindowHost(
    context: Pick<LoweringServices, "options" | "reachFeature" | "fail">,
    node: ts.Node,
): void {
    if (context.options.workers?.namespace)
        context.fail(node, "This Window API requires an application realm.");
...
export function documentEngine(
    ...
    if (!context.options.workers)
        return context.defaultEngine() === undefined
            ? undefined
            : context.requireDefaultEngine(node);
...
function windowIdentity(): Value {
    ...
    return {
        ...
        truthinessCpp: "true",
        recordProperties: {},
        domEventTargetCpp: "bbl::DomEventTarget::window()",
    };
}

export function compileWindowIdentity(
    ...
    if (global === "document") {
        ...
        return {
            ...
            truthinessCpp: "true",
            recordProperties: {},
            domEventTargetCpp: "bbl::DomEventTarget::document()",
        };
    }
    if (global === "screen") {
        requireWindowHost(context, expression);
        ...
        for (const [name, field] of Object.entries({
            width: "width",
            height: "height",
            availWidth: "available_width",
            availHeight: "available_height",
            colorDepth: "color_depth",
            pixelDepth: "color_depth",
        })) {
            recordProperties[name] = {
                kind: "number",
                cpp: `bbl::pal::window_screen_metrics().${field}`,
                readOnly: true,
            };
        }
        return {
            kind: "record",
            cpp: "",
            objectIdentityCpp: "bbl::pal::window_screen_identity()",
            truthinessCpp: "true",
            recordProperties,
        };
    }
        ...
        recordProperties: {
            type: {
                ...
                cpp: context.cppString(
                    rejection ? "unhandledrejection" : "error",
                ),
                staticString: rejection ? "unhandledrejection" : "error",
            },
            target: windowIdentity(),
            currentTarget: windowIdentity(),
            cancelable: { kind: "boolean", cpp: "true", staticBoolean: true },
            defaultPrevented: {
                kind: "boolean",
                cpp: `${cpp}.default_prevented`,
            },
            ...(rejection
                ? { reason: error }
                : {
