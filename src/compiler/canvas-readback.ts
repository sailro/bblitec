...
type ReadbackArgument =
    | { kind: "engine" }
    | { kind: "value"; value: unknown }
    | { kind: "directory"; index: number };

const dataGlobals = new Set([
    "Object",
    "Number",
    "String",
    "Boolean",
...
function checkClosedInputs(
    context: CanvasReadbackContext,
            ...
            (child) => {
                ...
                if (isDefaultLibraryIdentifier(context.checker, child)) {
                    ...
                    if (
                        ...
                        context.fail(
                            child,
                            "Canvas readback requires deterministic operations over its own canvas and closed data.",
                        );
                    }
                    return;
                }
                ...
                if (
                    ...
                    ts.isSourceFile(target.parent.parent.parent) &&
                    (!(target.parent.flags & ts.NodeFlags.Const) ||
                        !parameterIsReadOnly(context.checker, member, child))
                ) {
                    context.fail(
                        child,
                        "Canvas readback cannot mutate or capture mutable module bindings.",
                    );
                }
            },
...
function dataArgument(
    ...
    const declaration =
        ts.isCallExpression(node) && ts.isIdentifier(node.expression)
            ? tryResolveFunctionDeclaration(context.checker, node.expression)
            : undefined;
    if (
        !declaration ||
        ...
        return context.fail(
            expression,
            "Canvas readback arguments require generation-known data or a closed exported data function.",
        );
    }
    ...
    if (!closure)
        return context.fail(
            expression,
            "Canvas readback data arguments cannot reach foreign functions or pinned APIs.",
        );
    checkClosedInputs(context, closure, dataGlobals);
    try {
        return runModuleJsonSync(
        ...
        if (error instanceof ModuleJsonDeclined)
            return context.fail(expression, error.message);
        throw error;
    }
}

/** Capture one top-level Canvas2D readback from its active, closed invocation. */
export function bakeCanvasReadback(
    context: CanvasReadbackContext,
    call: ts.CallExpression,
): {
    pixels: Uint8Array;
    images: ReadbackImage[];
} {
    ...
    if (
        ...
        return context.fail(
            call,
            "Canvas readback requires a module-level function invocation.",
        );
    }
    const statement = call.parent;
    if (
        !ts.isVariableDeclaration(statement) ||
        !ts.isVariableDeclarationList(statement.parent) ||
        !ts.isVariableStatement(statement.parent.parent) ||
        statement.parent.parent.parent !== owner.body
    ) {
        return context.fail(
            call,
            "Canvas readback must initialize a top-level local in its producer.",
        );
    }
    const invocation = context.userFunctions.invocationFor(owner);
    const closure = sameFileClosure(
        context.checker,
        owner,
        (name) =>
            name === "createTexture2DFromPixels" || name === "loadTexture2D",
    );
    if (
        !invocation ||
        !closure ||
        !closure.some((member) => ownsCanvas(member, context.checker)) ||
        invocation.call.arguments.length !== owner.parameters.length
    ) {
        return context.fail(
            call,
            "Canvas readback requires a closed canvas-producing function with explicit arguments.",
        );
    }
    ...
    for (let index = 0; index < invocation.arguments.length; index++) {
        ...
        if (value.kind === "engine") {
            arguments_.push({ kind: "engine" });
            continue;
        }
        const data = dataArgument(
            context,
            invocation.call.arguments[index]!,
            value,
        );
        const directory =
            typeof data === "string"
                ? resolve(
                      dirname(context.options.fileName),
                      resolveBundledAsset(
                          data,
        ...
        if (
            ...
            if (logical.startsWith(".."))
                return context.fail(
                    call,
                    "Canvas readback asset directories must be within the entry directory.",
                );
            ...
            const visit = (folder: string): void => {
                for (const item of readdirSync(folder, {
                    ...
                    if (imageCodecForFileName(source))
                        images.push({
    ...
    const graph = closureModules(
        source.fileName,
        owner.name.text,
        findRepositoryRoot(dirname(source.fileName)),
    );
    if (!graph)
        return context.fail(
            call,
            "Canvas readback module closure must resolve within the repository.",
        );
    const rewritten =
        source.text.slice(0, call.getStart(source)) +
        `globalThis.__bblReadback(${call.getText(source)})` +
        source.text.slice(call.end) +
        `\nexports.__bblBrowserTextureTarget = ${owner.name.text};\n`;
    graph.modules[graph.entry]!.javascript = transpileCommonJs(
        rewritten,
        source.fileName,
    );
    ...
    const pixels = cachedBakeSync(
        {
            ...
            parameters: {
                module: graph.entry,
                function: owner.name.text,
                readback: call.getStart(source),
            },
    ...
    return new Uint8Array(
        Buffer.from(
            runGenerationChild({
                script,
                input,
                label: "Generation-time Canvas2D readback",
            }),
            "base64",
        ),
    );
