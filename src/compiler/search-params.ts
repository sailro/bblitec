...
export function searchParamsValue(lowerer: DataLowerer, input: string): Value {
    ...
    return {...lowerer.leafValue(`${lowerer.context.dataTypes.cppType(type)}(${input})`, type), impure:true};
}
 ...
 */
export function deploymentSearchParamsValue(lowerer: DataLowerer, search: string): Value {
    ...
    context.emit({ kind: "declaration", type: "static const auto", name, initializer: bag.cpp });
...
export function compileSearchParams(lowerer: DataLowerer, node: ts.NewExpression): Value | undefined {
    ...
    return searchParamsValue(lowerer, lowerer.compileForSink(args[0]!, {kind:"string"}));
}
...
export function compileSearchParamsMethod(lowerer: DataLowerer, call: ts.CallExpression, owner: Value, method: string): Value {
    ...
    const arguments_ = call.arguments.map(argument => {
        ...
        const value = cppIdentifierPattern.test(compiled.cpp) ? compiled : context.pinValueToTemporary(compiled, "query_argument");
