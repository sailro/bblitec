...
export function checkNodeGeometryMutation(
    context: Context,
    ...
    const unprovenMesh = (expression: ts.Expression): boolean => {
        ...
        if (value) {
            if (value.kind === "asset-root" || value.kind === "scene-node")
                return true;
            if (value.kind === "mesh")
                return (
                    value.sceneMeshIndex === undefined &&
                    value.sceneMeshProfileIndex === undefined
                );
            if (value.kind === "transform-node") return false;
        }
        const kind = pinnedHandleKind(
    ...
    const importedVector = (expression: ts.Expression): boolean => {
        ...
        if (value?.sceneNodeVector) {
            ...
            return (
                ...
                (owner.kind === "mesh" &&
                    ...
                    owner.sceneMeshProfileIndex === undefined)
            );
        }
        if (
            ts.isPropertyAccessExpression(node) &&
            transforms.has(node.name.text)
        )
            return unprovenMesh(node.expression);
        ...
        return (
            !value &&
            isPinnedType(context.checker.getTypeAtLocation(node), [
                "ObservableVec3",
        ...
        if (
            ...
            const name = ts.isPropertyAccessExpression(left)
                ...
                : ts.isStringLiteralLike(left.argumentExpression)
                  ...
                  : undefined;
            if (
                importedVector(left.expression) ||
                (name !== undefined &&
                    transforms.has(name) &&
    ...
    if (
        ...
        ["cloneTransformNode", "setParent"].includes(
