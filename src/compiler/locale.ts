...
export function compileLocaleStringMethod(
    ...
    if (
        ...
        if (
            ...
            context.fail(
                optionsNode!,
                "String.localeCompare options require a record of collation options.",
            );
        for (const key of Object.keys(fields))
            if (!collationOptionNames.some((name) => name === key))
                context.fail(
                    optionsNode!,
                    `String.localeCompare option '${key}' is not lowered.`,
                );
    }
    ...
    return lowerer.leafValue(
        `bbl::pal::compare_strings(${source}, ${other}, ${locales}, bbl::pal::CollationOptions{${optionsCpp.join(", ")}})`,
        { kind: "number" },
    );
}
