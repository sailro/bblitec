...
export function compileTextModuleValue(
    context: TextSurfaceContext,
    ...
    if (
        specifier === undefined ||
        !(babylonPackages as readonly string[]).includes(specifier)
    )
        return undefined;
    ...
    return {
        kind: "data",
        cpp: "[](bbl::TextData data, bbl::TextRunRef run, double offset) { bbl::set_font_weight_offset(data, run, offset); }",
        dataType: {
            kind: "function",
            parameters: [
...
export function retainTextValue(
    context: Pick<TextSurfaceContext, "allocateTemporaryCppName" | "emit">,
    value: Value,
): Value {
...
export function readTextProperty(
    ...
    if (["text-data", "text-renderable", "text-vector"].includes(owner.kind))
        context.reachFeature("text:data", site);
    if (owner.kind === "text-data") {
        ...
        context.fail(
            site,
            `Text data property '${name}' is not represented; shaping and internal buffer mutation remain unsupported.`,
        );
    }
    ...
    if (owner.kind === "text-renderable") {
        ...
        context.fail(
            site,
            `Text renderable property '${name}' is not represented.`,
        );
    }
    if (owner.kind === "text-vector") {
        ...
        if (axis < 0)
            context.fail(
                site,
                `Text ${transform} property '${name}' is not represented.`,
            );
        return {
            kind: "number",
            cpp:
...
function possibleTextOwner(
    ...
    return (
        ...
        [...transforms, "positionPx"].includes(node.name.text) &&
        ["text-renderable", "text-layer"].includes(
            pinnedHandleKind(
...
function ownerValue(
    ...
    return context.probeEmission(
        () => {
            ...
            if (textKinds.includes(value.kind))
                context.reachFeature("text:data", expression);
...
export function compileTextMutation(
    context: TextSurfaceContext,
    expression: ts.Expression,
): Value | undefined {
    const node = context.unwrap(expression);
    if (
        ...
        node.arguments[0] &&
        ownerValue(context, node.arguments[0])
    )
        context.fail(
            node,
            "Reflective text property mutation is not represented.",
        );
    if (
        ts.isCallExpression(node) &&
        ts.isPropertyAccessExpression(node.expression) &&
        ...
        if (node.arguments.length !== count)
            context.fail(
                node,
                `Text ${owner.textTransform}.set requires ${count} numeric arguments.`,
            );
    ...
    if (
        ["text-data", "text-run", "text-renderer"].includes(owner.kind) ||
        (owner.kind === "text-vector" && axis < 0) ||
        (owner.kind === "text-layer" &&
            ![
                "rotationRad",
                "scale",
                "order",
        ...
        (owner.kind === "text-renderable" &&
            !["opacity", "order", "ignoreDepth"].includes(name))
    )
        context.fail(
            left,
            `Text property '${name}' is read-only or requires an unsupported replacement.`,
        );
    if (
        owner.kind === "text-renderable" &&
    ...
    if (boolean && operator !== "=")
        context.fail(
            left,
            "Text ignoreDepth supports simple boolean assignment.",
        );
    if (!["=", "+=", "-=", "*=", "/="].includes(operator))
        context.fail(
            left,
            `Text property assignment '${operator}' is not represented.`,
        );
    ...
    if (operator !== "=") {
        ...
        context.emit({
            ...
            type: "const double",
            name: previous,
            initializer: readTextProperty(context, owner, name, left)!.cpp,
        });
    }
    const right = assignment
        ? context.compileValue(assignment.right)
        : { kind: "number" as const, cpp: "1.0" };
    context.expectKind(
        right,
    ...
    context.emit({
        kind: "declaration",
        type: `const ${boolean ? "bool" : "double"}`,
        name: result,
        initializer: previous
            ? `${previous} ${operator[0]} (${right.cpp})`
            : right.cpp,
    });
    context.emit(
    ...
    return {
        ...
        cpp:
            increment && ts.isPostfixUnaryExpression(increment)
                ? previous!
                : result,
