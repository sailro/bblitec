...
/** A scalar union compares its current JavaScript type as well as its value.
...
export function scalarUnionEquality(
    ...
    const leftType = lowerer.dataTypeAt(left), rightType = lowerer.dataTypeAt(right);
    ...
    const snapshot = (node: ts.Expression): Operand => {
        ...
        if (!type || !supported(type)) lowerer.context.fail(node, "Scalar union comparison requires represented scalar operands.");
    ...
    const compare = (a: Operand, b: Operand): string => {
        ...
        return a.type.kind === b.type.kind ? `(${a.cpp} == ${b.cpp})` : "false";
    };
