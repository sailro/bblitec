...
function hasDomInterface(
    ...
    const symbol = context.checker
        .getNonNullableType(context.checker.getTypeAtLocation(expression))
        .getSymbol();
    return (
        symbol?.name === name &&
...
function canvasOwner(
    ...
    if (mapped?.kind !== "handle" || mapped.handle !== "offscreen-canvas")
        return undefined;
    return context.compileValue(expression);
}

export function readMediaQueryProperty(
    context: CanvasContext,
    owner: Value,
    access: ts.PropertyAccessExpression,
): Value | undefined {
    if (
        owner.kind !== "worker-media-query" ||
        !["matches", "media"].includes(access.name.text)
    )
        return undefined;
    requireWindowHost(context, access);
    return {
        kind: access.name.text === "matches" ? "boolean" : "string",
        cpp: `(${owner.cpp})->${access.name.text}()`,
        readOnly: true,
    };
}
...
export function compileCanvasValue(
    ...
    if (
        ...
        const read = (present: Value): Value | undefined =>
            readMediaQueryProperty(context, present, node);
        return node.questionDotToken
            ? (context.dataLowerer.optionalAccess(owner, node, read) ??
                  read(owner))
    ...
    if (ts.isTypeOfExpression(node)) {
        ...
        if (
            ...
            member.name.text === "transferControlToOffscreen" &&
            context.isCanvasElement(member.expression)
        ) {
            return {
                kind: "string",
                cpp: 'std::string("function")',
    ...
    if (
        ...
        if (node.arguments?.length !== 2)
            return context.fail(
                node,
                "OffscreenCanvas requires width and height.",
            );
        const argumentsCpp = node.arguments.map((argument) => {
            const temporary =
                context.allocateTemporaryCppName("canvas_dimension");
            context.emit({
                kind: "declaration",
                type: "const double",
                name: temporary,
        ...
        return {
            ...
            dataType: { kind: "handle", handle: "offscreen-canvas" },
            cpp: `bbl::pal::create_offscreen_canvas(${argumentsCpp.join(", ")}, bbl::pal::WorkerRealm::current().host_services())`,
            impure: true,
        };
    }
    if (
    ...
    if (ts.isCallExpression(node)) {
        ...
        if (
            ...
            if (node.arguments.length !== 1)
                return context.fail(
                    node,
                    "matchMedia requires one media query string.",
                );
            requireWindowHost(context, node);
            return {
                kind: "worker-media-query",
                dataType: { kind: "handle", handle: "worker-media-query" },
                cpp: `bbl::pal::create_media_query(${context.dataLowerer.compileForSink(argumentAt(node, 0), { kind: "string" })})`,
                impure: true,
            };
        }
        if (
        ...
        if (
            ts.isPropertyAccessExpression(callee) &&
            callee.name.text === "transferControlToOffscreen" &&
            context.isCanvasElement(callee.expression)
        ) {
            if (node.arguments.length)
                return context.fail(
                    node,
                    "transferControlToOffscreen accepts no arguments.",
                );
            const owner = context.compileValue(callee.expression);
            if (owner.kind !== "ui-element")
                return context.fail(
                    node,
                    "Canvas transfer requires a retained Window canvas.",
                );
            return {
                kind: "offscreen-canvas",
                dataType: { kind: "handle", handle: "offscreen-canvas" },
                cpp: `bbl::pal::window_canvas(${owner.cpp})->transfer_control_to_offscreen()`,
                impure: true,
            };
        }
        if (
            ...
            if (owner.kind === "worker-resize-observer") {
                ...
                if (callee.name.text === "disconnect" && !argument)
                    return { kind: "void", cpp: `${owner.cpp}->disconnect()` };
                if (!argument || node.arguments.length !== 1)
                    return context.fail(
                        node,
                        "Resize observation requires one element.",
                    );
                const element = context.compileValue(argument);
                if (element.kind !== "ui-element")
                    return context.fail(
                        argument,
                        "Resize observation requires a retained Window element.",
                    );
                return {
                    kind: "void",
                    cpp: `${owner.cpp}->${callee.name.text}(${element.cpp})`,
                };
            }
        }
    }
    if (
        ts.isPropertyAccessExpression(node) &&
        ["clientWidth", "clientHeight"].includes(node.name.text) &&
        context.isCanvasElement(node.expression)
    ) {
        const owner = context.compileValue(node.expression);
        if (owner.kind === "ui-element")
            return {
    ...
    if (
        ts.isPropertyAccessExpression(node) &&
        ["width", "height"].includes(node.name.text)
    ) {
        const owner = canvasOwner(context, node.expression);
        if (owner)
            return {
                kind: "number",
                cpp: `static_cast<double>(${owner.cpp}->${node.name.text}())`,
                dataType: { kind: "number" },
            };
    }
    return undefined;
...
export function emitCanvasAssignment(
    context: CanvasContext,
    ...
    if (
        ...
        !["width", "height"].includes(target.name.text)
    )
        return false;
