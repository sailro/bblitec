...
export function arrayReturnStorage(
    checker: ts.TypeChecker,
    declaration: SupportedFunction,
    isStaticTable?: (root: ts.Identifier) => boolean,
): "fresh" | "static" | undefined {
    ...
    const classify = (
        ...
        if (ts.isIdentifier(node)) {
            ...
            if (
                ...
                return undefined;
            if (ts.findAncestor(binding, (ancestor) => ancestor === body))
                return classify(binding.initializer, seen);
            return undefined;
        }
    ...
    forEachAnalysisNode(
        ...
        (node) => {
            if (ts.isReturnStatement(node))
                returns.push(
                    node.expression ? classify(node.expression) : undefined,
                );
        },
        { functions: "skip" },
    );
    return returns.length > 0 && returns.every((value) => value === returns[0])
        ? returns[0]
        : undefined;
}
