...
export function framePollExecutor(
    ...
    if (
        ...
        !initialCall ||
        !ts.isExpressionStatement(initialCall) ||
        !ts.isCallExpression(initialCall.expression)
    )
        return undefined;
    const poll = declaration.declarationList.declarations[0]!;
    if (!(declaration.declarationList.flags & ts.NodeFlags.Const))
        return undefined;
    if (
        !ts.isIdentifier(poll.name) ||
        !poll.initializer ||
        !ts.isArrowFunction(poll.initializer) ||
        poll.initializer.parameters.length !== 0 ||
        !ts.isBlock(poll.initializer.body) ||
        poll.initializer.body.statements.length !== 2
    )
        return undefined;
    ...
    if (
        ...
        !scheduled ||
        !ts.isExpressionStatement(scheduled) ||
        !ts.isCallExpression(scheduled.expression)
    )
        return undefined;
    const [resolve, done] = guard.thenStatement.statements;
    if (
        !resolve ||
        !ts.isExpressionStatement(resolve) ||
        !ts.isCallExpression(resolve.expression) ||
        !ts.isIdentifier(resolve.expression.expression) ||
        resolve.expression.expression.text !== resolveParameter.text ||
        resolve.expression.arguments.length !== 0 ||
        !done ||
        !ts.isReturnStatement(done) ||
        done.expression
    )
        return undefined;
    ...
    if (
        ...
        initialCall.expression.expression.text !== poll.name.text ||
        initialCall.expression.arguments.length !== 0
    )
        return undefined;
    ...
    if (
        ...
        !sameSymbol(initialCall.expression.expression, poll.name) ||
