...
export function emitFrozenParticleSheetAssignment(
    ...
    const owner = {
        set: target.nodeParticleSetIndex!,
        system: target.nodeParticleSystemIndex!,
    };
    requireParticleBakeWritable(context, owner, expression);
    if (expression.operatorToken.kind !== ts.SyntaxKind.EqualsToken) {
        context.fail(
            expression.operatorToken,
            "A frozen particle sprite sheet takes a plain assignment.",
        );
    }
    ...
    if (!ts.isObjectLiteralExpression(object)) {
        context.fail(
            object,
            "A frozen particle sprite sheet is supplied as an object literal; replacing fields through a sheet-object alias is not lowered.",
        );
    }
    ...
    for (const property of object.properties) {
        if (
            ...
            context.fail(
                property,
                "A frozen particle sprite sheet requires named, non-computed fields.",
            );
        }
        const name = context.propertyName(property.name);
        if (
            !name ||
            !["cellWidth", "cellHeight", "cellIndex", "update"].includes(name)
        ) {
            context.fail(
                property,
        ...
        if (fields.has(name)) {
            context.fail(
                property,
                `A frozen particle sprite sheet cannot repeat field '${name}'.`,
            );
        }
        fields.set(
            name,
    ...
    const field = (name: string): ts.Expression => {
        ...
        if (!value)
            context.fail(
                object,
                `A frozen particle sprite sheet requires '${name}'.`,
            );
        return value;
    };
    const width = compileStaticNumber(
        context,
        field("cellWidth"),
        "Sprite-sheet cell width",
    );
    const height = compileStaticNumber(
        context,
        field("cellHeight"),
    ...
    if (
        (!ts.isArrowFunction(update) && !ts.isFunctionExpression(update)) ||
        update.parameters.some(
            (parameter) =>
                parameter.initializer || !ts.isIdentifier(parameter.name),
        ) ||
        update.modifiers?.some(
        ...
        context.fail(
            update,
            "A frozen particle sprite sheet requires a no-op update callback.",
        );
    }
    ...
    if (indices.dataType?.kind !== "u16array") {
        context.fail(
            field("cellIndex"),
            "Particle sprite-sheet cellIndex requires shared Uint16Array storage.",
        );
    }
