...
export function captureArrayReceiver(
    lowerer: DataLowerer,
    owner: Value,
): string {
...
export function compileArrayValueMethod(
    ...
    method: string,
    owner: Value,
    type: DataType & { kind: "vector" | "span" },
): Value | undefined {
    if (
        ![
            "at",
    ...
    if (
        type.kind === "span" &&
        (method === "copyWithin" || method === "fill" || method === "splice")
    ) {
        lowerer.context.fail(
            call,
            `Array.${method} requires owned mutable array storage.`,
        );
    }
    ...
    if (method === "at") {
        if (call.arguments.length > 1)
            lowerer.context.fail(call, "Array.at expects at most one index.");
        const resultType: DataType =
            lowerer.dataTypeAt(call) ??
            (type.element.kind === "optional"
                ? type.element
                : { kind: "optional", inner: type.element });
        return lowerer.leafValue(
            `bbl::js::array_relative_at<${lowerer.context.dataTypes.cppType(resultType)}>(${source}, ${numericArgument(0, "0.0")})`,
            resultType,
        );
    }
    ...
    if (method === "concat") {
        ...
        const arguments_ = call.arguments.map((argument) => {
            ...
            const name =
                lowerer.context.allocateTemporaryCppName("concat_argument");
            lowerer.context.emit({
                kind: "declaration",
                type: "const auto",
                name: name,
        ...
        const result =
            lowerer.context.allocateTemporaryCppName("concat_result");
        lowerer.context.emit(
    ...
    if (method === "copyWithin") {
        if (call.arguments.length < 2 || call.arguments.length > 3)
            lowerer.context.fail(
                call,
                "Array.copyWithin expects two or three arguments.",
            );
        const target = numericArgument(0, "0.0");
        const start = numericArgument(1, "0.0");
        const end = numericArgument(
            2,
        ...
        return {
            kind: "data",
            cpp: `bbl::js::array_copy_within(${source}, ${target}, ${start}, ${end})`,
            dataType: type,
        };
    }
    if (method === "fill") {
        if (call.arguments.length < 1 || call.arguments.length > 3)
            lowerer.context.fail(
                call,
                "Array.fill expects one to three arguments.",
            );
        ...
        return {
            kind: "data",
            cpp: `bbl::js::array_fill_range(${source}, ${item}, ${start}, ${end})`,
            dataType: type,
        };
    }
    ...
    return {
        kind: "data",
        cpp: `bbl::js::array_splice(${source}, ${start}, ${count}, {${inserted.join(", ")}})`,
        dataType: type,
    };
}
