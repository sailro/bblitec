...
export function usesWorkers(frontend: CompilerProgram): boolean {
    ...
    return frontend.program
        .getSourceFiles()
        .some((file) => !file.isDeclarationFile && visit(file));
}
...
export function compileWorkerApplication(
    ...
    const configuration = (
        ...
        register(node) {
            ...
            if (node.arguments?.length !== 2)
                return fail(
                    node,
                    "Native Worker requires a local module URL and { type: 'module' }.",
                );
            ...
            if (
                ...
                return fail(
                    node,
                    "Worker scripts must resolve through new URL(relativePath, import.meta.url).",
                );
            }
            ...
            if (
                ...
                return fail(
                    url,
                    "Worker module URLs must be local literals relative to import.meta.url.",
                );
            }
            if (
                !options ||
                ...
                return fail(
                    node,
                    "Native Worker currently supports module workers; classic worker scripts are not admitted.",
                );
            }
            ...
            if (!module) {
                const identity = relative(
                    dirname(frontend.sourceFile.fileName),
                    fileName,
                ).replaceAll("\\", "/");
                module = {
                    fileName,
                    namespace: `bblworker_${createHash("sha256").update(identity).digest("hex").slice(0, 16)}`,
                };
        ...
        declarations: () =>
            [...modules.values()]
                .map(
                    (module) =>
                        `namespace ${module.namespace} { void initialize(bbl::pal::WorkerRealm&); }`,
                )
                .join("\n"),
    });
    ...
    for (const module of modules.values()) {
        let source: string;
        try {
            source = readFileSync(module.fileName, "utf8");
        } catch {
            return fail(
                frontend.sourceFile,
                `Unable to read worker module '${module.fileName}'.`,
            );
        }
    ...
    const results = [
        application,
        ...[...modules.values()].map((module) => module.result!),
    ];
    const features = featureOrder.filter((feature) =>
        results.some((result) => result.manifest.features.includes(feature)),
    );
    ...
    const graphics = results.filter((result) =>
        result.manifest.features.includes("backend:sdl"),
    );
    ...
    for (const result of graphics.slice(1)) {
        if (renderingProduct(result.manifest) !== product) {
            fail(
                frontend.sourceFile,
                `Worker application reaches incompatible generated rendering products in '${result.manifest.source}'; distinct product domains are not yet admitted.`,
            );
        }
    }
    ...
    return {
        ...
        manifest: {
            ...
            inputs: [
                ...new EmissionSet(
                    results.flatMap((result) => result.manifest.inputs),
                ),
            ].sort(),
            features,
            runtimeSources,
