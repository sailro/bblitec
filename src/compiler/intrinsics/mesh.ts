...
function writableVec3Lanes(
    ...
    if (value.kind === "record") {
        ...
        if (
            ...
            return lanes.map((lane) => lane!.cpp) as [string, string, string];
        }
    }
    if (value.kind === "data" && value.dataType?.kind === "struct") {
        const dataType = value.dataType;
        const separator = context.dataTypes.isReferenceStruct(dataType.name)
            ? "->"
            : ".";
...
function vec3RecordCpp(
    ...
    if (
        element.kind !== "record" &&
        !(element.kind === "data" && element.dataType?.kind === "struct")
    ) {
        context.fail(
            node,
...
function compileVec3PathArray(
    ...
    if (rows) {
        return `std::vector<std::vector<bbl::Vec3d>>{${rows
            .map((row) => {
                const points =
                    row.tupleElements ??
                    ...
                    row.staticElements;
                return points
                    ? vec3PointsCpp(context, points, expression)
                    : context.fail(
                          expression,
...
function compileVec3Path(
    ...
    const bound = context.handleCollections.tupleElements(expression);
    if (bound) {
    ...
    return `std::vector<bbl::Vec3d>{${context
        .expectStaticArrayLiteral(context.resolveStaticExpression(expression))
        .elements.map((element) => context.compileVec3(element, "double"))
        .join(", ")}}`;
}
...
function csgOptionBag<Name extends string>(
    ...
    for (const name of names) {
        ...
        const value = staticNumberValue(context, context.unwrap(property));
...
function csgSourceFromCall(
    ...
    if (factory === "createBox") {
        ...
        if (!ts.isObjectLiteralExpression(unwrapped)) {
            ...
            return size === undefined ? undefined : { factory, options: size };
        }
    ...
    if (factory === "createSphere") {
        ...
        const bag = csgOptionBag(context, unwrapped, SPHERE_OPTION_NAMES);
        return bag && { factory, options: bag };
    }
    return undefined;
}
...
function requireCsg2Solid(
    ...
    if (!value.csg2Solid || value.csg2Solid.disposed) {
        context.fail(
            argument,
            "CSG2 requires a live generation-known solid; this solid is absent or disposed.",
        );
    }
...
export function compileMeshIntrinsic(
    ...
    if (
        csg2Intrinsics.has(importedName) &&
        context.isRuntimeResourceConstruction()
    ) {
        context.fail(
            call,
            "CSG2 modelling and lifetime require unconditional generation-known execution; runtime branches, loops and callbacks are unsupported.",
        );
    }
...
function compileCreateCsg2FromMesh(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    ...
    if (!initializedCsg2Contexts.has(context))
        context.fail(
            call,
            "CSG2 requires initializeCsg2Async before creating a solid.",
        );
    ...
    if (!source)
        context.fail(
            argumentAt(call, 0),
            "createCsg2FromMesh requires an unchanged identity-transform createBox/createSphere with generation-known options; only preceding material assignments are permitted.",
        );
    ...
    if (
        ...
        materialSlot < 0 ||
        materialSlot >= slotCount
    ) {
        context.fail(
            call.arguments[1] ?? call,
            `A CSG2 material slot must be a generation-known integer in [0, ${slotCount - 1}].`,
        );
    }
    context.reachFeature("mesh:csg2", call);
    return {
        ...
        cpp: "",
        csg2Solid: {
            plan: { op: "from-mesh", source, materialSlot },
            disposed: false,
        },
    };
}

function compileCsg2Subtract(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "csg2Subtract" | "csg2Intersect" | "csg2Add",
): Value | undefined {
    ...
    return {
        kind: "csg2-solid",
        cpp: "",
        csg2Solid: { plan: { op: importedName, left, right }, disposed: false },
    };
}

function compileDisposeCsg2(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
function compileCreateMeshFromCsg2(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "createMeshFromCsg2" | "createMeshesFromCsg2",
): Value | undefined {
    ...
    const materials = partitioned
        ? context.handleCollections.staticHandleList(argumentAt(call, 2))
        : undefined;
    if (partitioned && !materials)
        context.fail(
            argumentAt(call, 2),
            "CSG2 material partitioning requires a generation-known material list.",
        );
    for (const material of materials ?? []) {
        ...
        if (!material.value.standardMaterial)
            context.fail(
                material.node,
                "CSG2 material partitioning currently requires Standard materials.",
            );
    }
    const nameArgument = call.arguments[partitioned ? 3 : 2];
    const name = nameArgument
        ? context.compileValue(nameArgument).staticString
        : "csg2";
    if (name === undefined)
        context.fail(
            nameArgument ?? call,
            "A CSG2 output name must be generation-known.",
        );
    const baked = (() => {
        try {
            return bakeCsg2Meshes({
                plan,
                name,
                ...(materials ? { materialCount: materials.length } : {}),
            });
        } catch (error) {
    ...
    for (const output of baked) {
        ...
        context.emit({
            ...
            type: "auto",
            name: cpp,
            initializer: `bbl::create_mesh_from_data(${engine.cpp}, ${context.cppString(output.name)}, ${geometry.positions}, ${geometry.normals}, ${geometry.indices}, ${geometry.uvs}, {}, {}, {})`,
        });
        const material =
            output.materialSlot === undefined
                ? undefined
                : materials?.[output.materialSlot]?.value;
    ...
    return partitioned
        ? { kind: "tuple", cpp: "", tupleElements: meshes }
        : meshes[0]!;
}

function compileQuatFromLookDirectionRH(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
...
function compileAddVec3(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "addVec3" | "subtractVec3" | "crossVec3",
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
...
function compileAddVec3ToRef(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "addVec3ToRef" | "subtractVec3ToRef" | "crossVec3ToRef",
): Value | undefined {
...
function compileScaleVec3ToRef(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 3, 3);
...
function compileSetMeshVisible(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
...
function compileNormalizeVec3Object(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 1, 1);
...
function compileNormalizeVec3ToRef(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
function compileMat4Translation(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 3, 3);
    ...
    return {
        kind: "data",
        cpp:
            `bbl::upstream::create_translation_mat4(` +
            call.arguments
                .map((argument) => context.compileNumber(argument, "double"))
                .join(", ") +
            `)`,
...
function compileSetParent(
    ...
    if (child.kind === "asset-root" && parent.kind !== "transform-node") {
        context.fail(
            call,
...
function compileCloneTransformNode(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 1, 1);
    ...
    if (source.kind === "mesh") {
        ...
        return {
            kind: "mesh",
            sceneMeshIndex,
            cpp: `bbl::clone_mesh_node(${engine}, ` + `${source.cpp})`,
            engineCpp: engine,
        };
    }
    ...
    return {
        ...source,
        cpp: `bbl::clone_asset_root(${engine}, ` + `${source.cpp})`,
        assetRootClone: true,
...
function compileAddHierarchyInstance(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "addHierarchyInstance" | "setHierarchyInstanceMatrix",
): Value | undefined {
...
function compileSetHierarchyInstanceCount(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "setHierarchyInstanceCount" | "removeHierarchyInstance",
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
    const pool = context.compileValue(argumentAt(call, 0));
    context.expectKind(pool, "hierarchy-instance-pool", argumentAt(call, 0));
...
function compileSetThinInstances(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 3, 3);
...
function compileSetThinInstanceColors(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
    const mesh = context.compileValue(argumentAt(call, 0));
    context.expectKind(mesh, "mesh", argumentAt(call, 0));
    const colors = context.compileTypedArrayArgument(
        argumentAt(call, 1),
        "f32array",
    );
...
function compileSetThinInstanceColor(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value {
    ...
    return {
        kind: "void",
        cpp: `bbl::set_thin_instance_color(${context.requireEngine(mesh, call)}, ${mesh.cpp}, ${args.join(", ")})`,
    };
}

function compileSetThinInstanceCullBoundsPad(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value {
    context.expectArgumentCount(call, 2, 2);
    ...
    return {
        kind: "void",
        cpp: `bbl::set_thin_instance_cull_bounds_pad(${context.requireEngine(mesh, call)}, ${mesh.cpp}, ${pad})`,
    };
}

function compileSetThinInstanceCount(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
...
function compileSetThinInstanceMatrix(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 3, 3);
...
function compileFlushThinInstances(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 1, 1);
...
function compileAddThinInstance(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
function compileRemoveThinInstance(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
...
function compileEnableThinInstanceGpuCulling(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
function compileCreateTransformNode(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
function compileCreateGroundFromHeightMap(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    const sceneMeshIndex = context.sceneManifest.recordSceneMesh("ground");
...
function compileCreateCsgFromMesh(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 1, 2);
...
function compileCsgUnion(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "csgUnion" | "csgSubtract" | "csgIntersect",
): Value | undefined {
    context.expectArgumentCount(call, 2, 2);
...
function compileCreateMeshFromCsg(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 2, 3);
...
function compileCreateMeshesFromCsg(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
function compileCreateBoxData(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
    importedName: "createBoxData" | "createSphereData",
): Value | undefined {
    ...
    for (const field of ["positions", "normals", "uvs", "indices"] as const) {
        ...
        context.emit({
            kind: "declaration",
            type: `bbl::js::${indices ? "U32Array" : "F32Array"}`,
            name: cpp,
            initializer: `std::move(${temporary}.${field})`,
        });
        recordProperties[field] = {
            kind: "data",
            cpp,
...
function compileCreateMorphTargets(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 4, 4);
    ...
    for (const property of target.properties) {
        const name =
            ...
            (ts.isIdentifier(property.name) ||
                ...
                ? property.name.text
                : undefined;
        if (name !== "positions" && name !== "normals") {
            context.fail(
                property,
    ...
    return {
        ...
        engineCpp: engine.engineCpp ?? engine.cpp,
        morphTarget: {
            positionsCpp: context.compileTypedArrayArgument(
                positions,
                "f32array",
            ),
...
function compileSetMorphTargetWeights(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    context.expectArgumentCount(call, 3, 3);
    ...
    if (morph.engineCpp !== (engine.engineCpp ?? engine.cpp)) {
        context.fail(
            call,
    ...
    const weights = context.compileTypedArrayArgument(
        argumentAt(call, 2),
        "f32array",
    );
    context.reachFeature("mesh:morph-targets", call);
...
function compileCreateExtrudeShape(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
function compileCreatePolyhedron(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    ...
    if (call.arguments[1]) {
        ...
        if (typeExpression) {
            ...
            type = named < 0 || named >= pinnedPolyhedronCount() ? 0 : named;
        }
    ...
    const rows = (table: readonly (readonly number[])[]): string =>
        `{${table
            .map(
                (row) =>
...
function compileCreateCylinder(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
    ...
    if (call.arguments[1]) {
        ...
        if (top) {
            topIsZero = `(${context.compileNumber(top, "double")} == 0.0)`;
        }
    }
...
function compileCreateTorusKnot(
    context: MeshIntrinsicContext,
    call: ts.CallExpression,
): Value | undefined {
...
const meshIntrinsicHandlers = new EmissionMap<
    string,
    (
        context: MeshIntrinsicContext,
        call: ts.CallExpression,
...
>([
    ...
    [
        "csg2Subtract",
        (context, call) => compileCsg2Subtract(context, call, "csg2Subtract"),
    ],
    [
        "csg2Intersect",
        (context, call) => compileCsg2Subtract(context, call, "csg2Intersect"),
    ],
    [
        "csg2Add",
        (context, call) => compileCsg2Subtract(context, call, "csg2Add"),
    ],
    ["disposeCsg2", compileDisposeCsg2],
    [
        "createMeshFromCsg2",
        (context, call) =>
            compileCreateMeshFromCsg2(context, call, "createMeshFromCsg2"),
    ],
    [
        "createMeshesFromCsg2",
        (context, call) =>
            compileCreateMeshFromCsg2(context, call, "createMeshesFromCsg2"),
    ],
    ["createQuatFromLookDirectionRH", compileQuatFromLookDirectionRH],
    ["addVec3", (context, call) => compileAddVec3(context, call, "addVec3")],
    [
        "subtractVec3",
        (context, call) => compileAddVec3(context, call, "subtractVec3"),
    ],
    [
        "crossVec3",
        (context, call) => compileAddVec3(context, call, "crossVec3"),
    ],
    [
        "addVec3ToRef",
        (context, call) => compileAddVec3ToRef(context, call, "addVec3ToRef"),
    ],
    [
        "subtractVec3ToRef",
        (context, call) =>
            compileAddVec3ToRef(context, call, "subtractVec3ToRef"),
    ],
    [
        "crossVec3ToRef",
        (context, call) => compileAddVec3ToRef(context, call, "crossVec3ToRef"),
    ],
    ...
    [
        ...
        (context, call) =>
            compileAddHierarchyInstance(context, call, "addHierarchyInstance"),
    ],
    [
        "setHierarchyInstanceMatrix",
        (context, call) =>
            compileAddHierarchyInstance(
                context,
                call,
                "setHierarchyInstanceMatrix",
            ),
    ...
    [
        ...
        (context, call) =>
            compileSetHierarchyInstanceCount(
                context,
                call,
                "setHierarchyInstanceCount",
            ),
    ...
    [
        "csgSubtract",
        (context, call) => compileCsgUnion(context, call, "csgSubtract"),
    ],
    [
        "csgIntersect",
        (context, call) => compileCsgUnion(context, call, "csgIntersect"),
    ],
    ["createMeshFromCsg", compileCreateMeshFromCsg],
    ["createMeshesFromCsg", compileCreateMeshesFromCsg],
    [
        "createBoxData",
        (context, call) => compileCreateBoxData(context, call, "createBoxData"),
    ],
    [
        "createSphereData",
        (context, call) =>
            compileCreateBoxData(context, call, "createSphereData"),
    ],
