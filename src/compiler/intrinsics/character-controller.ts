...
function options(
    context: CharacterOptionsContext,
    ...
    validateObjectProperties(
        ...
        "Character options must describe the capsule height and radius.",
    );
...
export function compileCharacterIntrinsic(
    ...
    if (name === "createPhysicsCharacterController") {
        ...
        return {
            kind: "physics-character-controller",
            cpp: `bbl::character::create_physics_character_controller(${world.cpp}, ${context.compileVec3(argumentAt(call, 1), "double")}, ${options(context, argumentAt(call, 2))})`,
            dataType: {
                kind: "handle",
                handle: "physics-character-controller",
            },
    ...
    if (name === "getPhysicsCharacterControllerBody") {
        ...
        return {
            kind: "physics-body",
            cpp: `${controller.cpp}->getBody()->value`,
            ...(controller.engineCpp
                ? { engineCpp: controller.engineCpp }
                : {}),
        };
    }
}

export function compileCharacterMethod(
    context: CharacterIntrinsicContext,
    call: ts.CallExpression,
    owner: Value,
    name: string,
): Value | undefined {
    if (owner.kind === "physics-character-observable") {
        if (name !== "add")
            context.fail(
                call,
                `Character observable method '${name}' is not represented.`,
            );
        context.expectArgumentCount(call, 1, 1);
        return {
            kind: "data",
    ...
    if (["moveWithCollisions", "setPosition", "setVelocity"].includes(name)) {
        ...
        return {
            kind: "void",
            cpp: `${owner.cpp}->${name}(bbl::character::vector(${context.compileVec3(argumentAt(call, 0), "double")}))`,
        };
    }
    ...
    if (name === "setShapeOptions") {
        ...
        return {
            kind: "void",
            cpp: `${owner.cpp}->setShapeOptions(${options(context, argumentAt(call, 0))}${call.arguments[1] ? `, ${context.compileBoolean(call.arguments[1])}` : ""})`,
        };
    }
    if (name === "dispose") {
        context.expectArgumentCount(call, 0, 0);
    ...
    context.fail(
        call,
        `Character method '${name}' has no represented scene value projection.`,
    );
}

export function readCharacterProperty(
    context: PhysicsIntrinsicContext,
    owner: Value,
    name: string,
): Value | undefined {
    if (owner.kind !== "physics-character-controller") return;
    if (name === "onTriggerCollisionObservable")
        return {
            kind: "physics-character-observable",
            cpp: owner.cpp,
            ...(owner.engineCpp ? { engineCpp: owner.engineCpp } : {}),
        };
    if (
    ...
    if (name === "up")
        return retainCharacterVector(context, `${owner.cpp}->up`);
}
