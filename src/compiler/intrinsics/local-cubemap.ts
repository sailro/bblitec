...
function optionsJson(
    context: LocalCubemapIntrinsicContext,
    value: Value,
    node: ts.Node,
    environments: Value[],
): LocalCubemapJson {
    ...
    return context.fail(
        node,
        `Local cubemap geometry and options must be static; environments must be retained loadEnvironment results (${value.kind}: ${value.cpp}).`,
    );
}

function packetExpression(
    context: LocalCubemapIntrinsicContext,
    state: NonNullable<Value["localCubemap"]>,
    node: ts.Node,
): string {
    let payload: string;
    try {
        payload = packLocalCubemapSync(state.plan);
    } catch (error) {
...
export function compileLocalCubemapIntrinsic(
    context: LocalCubemapIntrinsicContext,
    name: string,
    ...
    if (
        ...
        context.fail(
            call,
            "Local cubemap configuration currently requires static calls before scene registration; live rebuilds are not represented.",
        );
    if (name === "enablePbrLocalCubemap") {
        ...
        if (
            ...
            typeof value !== "object" ||
            Array.isArray(value) ||
            Object.keys(value).some((key) => key !== "maxCandidates")
        )
            context.fail(
                call,
                "enablePbrLocalCubemap accepts only static maxCandidates.",
            );
        const limits = pinnedLocalCubemapLimits();
        const maxCandidates =
            value["maxCandidates"] ?? limits.defaultCandidates;
        if (
            typeof maxCandidates !== "number" ||
            !Number.isInteger(maxCandidates) ||
            maxCandidates < 1 ||
            maxCandidates > limits.candidateCapacity
        )
            context.fail(
                call,
                `Local cubemap maxCandidates must be an integer from 1 to ${limits.candidateCapacity}.`,
            );
        if (
            context.localCubemapState.maxCandidates !== undefined &&
            context.localCubemapState.maxCandidates !== maxCandidates
        )
            context.fail(
                call,
                "Local cubemap maxCandidates cannot change after initialization.",
            );
        context.localCubemapState.maxCandidates = maxCandidates;
    ...
    if (maxCandidates === undefined)
        context.fail(
            call,
            "Call enablePbrLocalCubemap before configuring local environments.",
        );
    const argumentCount =
        name === "setPbrLocalEnvironment"
            ? 3
            : name === "clearPbrLocalEnvironment"
              ? 1
              : 2;
    ...
    if (name === "setPbrLocalEnvironmentProbeDebug") {
        ...
        if ((enabled !== "true" && enabled !== "false") || !first.localCubemap)
            context.fail(
                call,
                "Local probe debug must be a static boolean on a retained probe set.",
            );
    ...
    if (name === "createPbrLocalEnvironmentProbeSet")
        context.expectKind(first, "scene", argumentAt(call, 0));
    else {
        context.expectKind(first, "material", argumentAt(call, 0));
        if (first.scenePbrMaterialIndex === undefined)
            context.fail(
                call,
    ...
    const optionArgument =
        name === "createPbrLocalEnvironmentProbeSet"
            ? call.arguments[1]
            : name === "setPbrLocalEnvironment"
              ? call.arguments[2]
              : undefined;
    if (name !== "createPbrLocalEnvironmentProbeSet") {
        ...
        if (!environment.environmentAsset)
            context.fail(
                call,
                "Local environments require retained loadEnvironment results.",
            );
    ...
    if (optionArgument) {
        ...
        if (!value || typeof value !== "object" || Array.isArray(value))
            context.fail(
                optionArgument,
                "Local environment options require a static object.",
            );
        options = value;
        const allowed =
            name === "createPbrLocalEnvironmentProbeSet"
                ? ["probes", "voxelGrid", "parallaxCorrection"]
                : [
                      "shape",
                      "projectionPosition",
                      "projectionSize",
        ...
        for (const key of Object.keys(options))
            if (!allowed.includes(key))
                context.fail(
                    optionArgument,
                    `Unsupported local environment option '${key}'.`,
                );
    }
    for (const environment of environments)
        context.expectSameEngine(first, environment, call);
    const plan: LocalCubemapPlan = {
        ...
        environments: environments.map(
            (environment) => environment.environmentAsset!.source,
        ),
        options,
    };
    const state = { plan, environments };
    const cpp = packetExpression(context, state, call);
    return name === "createPbrLocalEnvironmentProbeSet"
        ? {
              kind: "pbr-local-probe-set",
              cpp,
              engineCpp: first.engineCpp!,
              localCubemap: state,
          }
        : {
              kind: "void",
