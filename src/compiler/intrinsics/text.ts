...
export function compileTextIntrinsic(
    ...
    if (name === "setFontWeightOffset") {
        ...
        return {
            kind: "void",
            cpp: `bbl::set_font_weight_offset(${owner.cpp},${run},${offset})`,
        };
    }
    ...
    if (name === "createTextLayer") {
        ...
        for (const [field, value] of textOptionEntries(
            ...
            if (field === "positionPx") {
                ...
                if (
                    ...
                    new EmissionSet(components.map(([name]) => name)).size !== 2
                )
                    context.fail(
                        value,
                ...
                context.fail(
                    value,
                    `Text layer option '${field}' is not represented.`,
                );
        }
        ...
        return {
            kind: "text-layer",
            cpp: `bbl::create_text_layer(${owner.cpp}, ${options})`,
            dataType: { kind: "handle", handle: "text-layer" },
        };
    }
    if (name === "createTextRenderer") {
        ...
        const options = context.allocateTemporaryCppName(
        ...
        for (const [field, value] of textOptionEntries(
            ...
            if (field === "layers") {
                ...
                context.fail(
                    value,
                    `Text renderer option '${field}' is not represented.`,
                );
        }
        if (!hasLayers)
            context.fail(call, "Text renderer requires a layer array.");
        context.reachFeature("renderer:text", call);
        return {
            kind: "text-renderer",
    ...
    if (name === "registerTextRenderer") {
        ...
        return {
            kind: "void",
            cpp: `bbl::register_text_renderer(${value.cpp})`,
        };
    }
    ...
    if (
        ["disposeScene", "unregisterScene", "rebuildSceneRenderables"].includes(
            name,
        )
    )
    ...
    if (
        ...
        call.arguments[0] &&
        pinnedHandleKind(
            context.checker.getTypeAtLocation(call.arguments[0]),
        ) === "text-renderable"
    ) {
        context.expectArgumentCount(
            call,
            name === "setAlphaToCoverage" ? 2 : 1,
            name === "setAlphaToCoverage" ? 2 : 1,
        );
    ...
    if (name === "createTextRenderable") {
        ...
        return {
            kind: "text-renderable",
            cpp: `bbl::create_text_renderable(${data.cpp}, ${options})`,
    ...
    if (name === "addTextRenderable") {
        ...
        const scene = {
            ...originalScene,
            cpp: context.allocateTemporaryCppName("text_scene"),
        };
        context.emit({
            kind: "declaration",
            type: "auto",
            name: scene.cpp,
            initializer: originalScene.cpp,
        });
        ...
        return {
            kind: "void",
            cpp: `bbl::add_text_renderable(${scene.cpp}, ${renderable.cpp})`,
        };
    }
    const dispose = {
        disposeTextRenderable: ["text-renderable", "dispose_text_renderable"],
        disposeTextData: ["text-data", "dispose_text_data"],
        disposeDefaultTextData: ["text-data", "dispose_default_text_data"],
    } as const;
    if (name in dispose) {
    ...
    if (name !== "loadFont" && name !== "createDefaultTextData")
        return undefined;
    if (context.isRuntimeResourceConstruction()) {
        context.fail(
            call,
            `${name} requires definite initialization; dynamic font/text layouts are not materialized.`,
        );
    }
    if (name === "loadFont") {
        context.expectArgumentCount(call, 1, 1);
        ...
        try {
            materializePinnedText(bytes);
        } catch (error) {
            context.fail(
                call,
                `Pinned font materialization failed: ${String(error)}`,
            );
        }
        return {
            kind: "text-font",
    ...
    if (color && !omitted(context, color)) {
        ...
        if (retained) {
            ...
            if (
                ...
                elements.some(
                    (value) =>
                        ...
                        Object.is(value.staticNumber, -0),
                )
            ) {
                context.fail(
                    color,
                    "Static text color requires four known numeric components; mutated or dynamic arrays are not materialized.",
                );
            }
            layout.color = elements.map((value) => value.staticNumber!);
        } else if (ts.isCallExpression(context.unwrap(color))) {
            liveColor = context.compileForDataSink(color, {
                kind: "tuple",
                arity: 4,
            });
            layout.live = true;
            ...
            if (array.elements.length !== 4)
                context.fail(
                    color,
                    "Static text color requires four numeric components.",
                );
            layout.color = array.elements.map((component) =>
                finiteNumber(context, component, "Text color"),
            );
        }
    }
    ...
    if (options && !omitted(context, options)) {
        ...
        for (const property of object.properties) {
            if (
                ...
                context.fail(
                    property,
                    "Text layout options require static named properties.",
                );
            }
            ...
            if (key === "align") {
                ...
                if (value !== "left" && value !== "center" && value !== "right")
                    context.fail(property, "Unsupported text alignment.");
                ...
                layout.options[key] = finiteNumber(
                    context,
                ...
                context.fail(
                    property,
                    `Text layout option '${key}' is not materialized.`,
                );
        }
    }
    ...
    return {
        kind: "text-data",
        cpp: layout.live
            ? `bbl::create_live_text_data(${row.id}, ${textValue.cpp}${liveColor ? `, ${liveColor}` : ""})`
            : `bbl::create_compiled_text_data(${row.id})`,
...
function textOptionEntries(
    context: TextIntrinsicContext,
    expression?: ts.Expression,
): Array<readonly [string, ts.Expression]> {
    if (!expression || omitted(context, expression)) return [];
    const object = context.unwrap(expression);
    if (!ts.isObjectLiteralExpression(object))
        context.fail(
            expression,
            "Text options require a direct object literal.",
        );
    return object.properties.map((property) => {
        if (
            ...
            context.fail(
                property,
                "Text options require named property assignments.",
            );
...
function compileRenderableOptions(
    context: TextIntrinsicContext,
    expression?: ts.Expression,
): string {
    if (!expression || omitted(context, expression)) return "{}";
    ...
    if (!ts.isObjectLiteralExpression(object))
        context.fail(
            expression,
            "Text renderable options require a direct object literal.",
        );
    ...
    for (const property of object.properties) {
        if (
            ...
            context.fail(
                property,
                "Text renderable options require named property assignments.",
            );
        ...
        if (
            ...
            if (!ts.isObjectLiteralExpression(vector))
                context.fail(
                    vector,
                    "Text transform options require direct component literals; retained vector options are not represented.",
                );
            const lanes =
                name === "rotationQuaternion"
                    ? ["x", "y", "z", "w"]
            ...
            for (const component of vector.properties) {
                if (
                    ...
                    (!ts.isIdentifier(component.name) &&
                        !ts.isStringLiteral(component.name)) ||
                    !lanes.includes(component.name.text)
                )
                    context.fail(
                        component,
                        "Text transform options require named numeric components.",
                    );
            ...
            context.fail(
                property,
                `Text renderable option '${name}' is not represented.`,
            );
    }
    return options;
}
...
function finiteNumber(
    ...
    if (!Number.isFinite(number) || Object.is(number, -0))
        context.fail(
            expression,
            `${label} must be finite and not negative zero for static materialization.`,
        );
    return number;
}
