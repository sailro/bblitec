...
> = {
    promise: {
        cpp: (type, context) =>
            `bbl::js::Promise<${type.result ? context.cppType(type.result) : "bbl::js::PromiseVoid"}>`,
        key: (type, key) =>
            `promise(${type.result ? key(type.result) : "void"})`,
    ...
    product: {
        cpp: (type, context) =>
            `bbl::js::Product<${type.elements.map((element) => context.cppType(element)).join(", ")}>`,
    ...
    union: {
        cpp: (type, context) =>
            `std::variant<${type.members.map((member) => context.cppType(member)).join(", ")}>`,
        key: (type, key) => `union(${type.members.map(key).join(",")})`,
        equal: (left, right, equal) =>
            left.members.length === right.members.length &&
            left.members.every((member, index) =>
                equal(member, right.members[index]!),
            ),
    ...
    optional: {
        cpp: (type, context) =>
            ...
            context.isReferenceStruct(type.inner.name)
                ? context.cppType(type.inner)
                : `bbl::js::Nullable<${context.cppType(type.inner)}>`,
        key: (type, key) =>
            `${type.undefinedOnly ? "u" : "o"}(${key(type.inner)})`,
        equal: (left, right, equal) =>
    ...
    span: {
        cpp: (type, context) =>
            `bbl::js::Span<const ${context.cppType(type.element)}>`,
    ...
    table: {
        cpp: (type, context) =>
            `const ${context.tableCppType(type.dimensions)}&`,
