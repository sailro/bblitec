...
const featureSources: Record<Feature, string> = {
    ...
    "light:hemispheric": "src/light_hemispheric.cpp",
...
class Compiler {
    ...
    public compile(): CompileResult {
        ...
        const runtimeSources = features.map((feature) => featureSources[feature]);
        return {
            cpp: this.renderCpp(),
            cmake: this.renderCmake(features, runtimeSources),
            manifest: {
...
`;
    ...
    private renderCmake(features: Feature[], runtimeSources: string[]): string {
