...
function stageConstants(stage: TextShaderStage): ShaderStageConstant[] {
    return Object.entries(stage.constants ?? {})
        .map(([key, value]) => {
            const id = Number(key);
            if (
                !Number.isInteger(id) ||
                ...
                throw new Error(
                    `Pinned text pipeline constant '${key}' is not a finite numeric override ID/value.`,
                );
            }
            return { id, value };
        })
...
export async function composeTextPipeline(
    ...
    const module = await importPinnedModule<{
        getOrCreateTextPipeline(
            ...
            _pipeline: TextPipelineDescriptor;
            _variantPipeline: TextPipelineDescriptor;
        };
        clearTextPipelineCache(this: void, engine: unknown): void;
        _textVariantResolver: ((device: unknown) => unknown) | null;
        _installTextVariantResolver(
            this: void,
            resolver: ((device: unknown) => unknown) | null,
        ): void;
    }>("text/_gpu/text-pipeline.js");
    const { setAlphaToCoverage } = await importPinnedModule<{
        setAlphaToCoverage(this: void, owner: object, enabled: boolean): void;
    ...
    try {
        if (options.weighted) {
            ...
            const { composeSlugShader } = await importPinnedModule<{
                composeSlugShader(
                    this: void,
                    fragment: unknown,
                ): { _key: string; _vert: string; _frag: string };
            }>("text/shaders/slug-shader.js");
            const composed = composeSlugShader(WEIGHT_SHADER_FRAGMENT);
            module._installTextVariantResolver(() => ({
                _id: composed._key,
                _vertModule: device.createShaderModule({
                    code: composed._vert,
                }),
                _fragModule: device.createShaderModule({
                    code: composed._frag,
                }),
            }));
        }
        const result = module.getOrCreateTextPipeline(
            engine,
            options.format,
            options.sampleCount,
        ...
        if (
            (!options.weighted &&
                result._pipeline !== result._variantPipeline) ||
            recorder.buffers.length !== 1 ||
            !quad
        ) {
            throw new Error(
                "Pinned base text pipeline requires one quad buffer and no installed style variant.",
            );
        }
        if (!quad.mappedAtCreation)
            throw new Error(
                "Pinned text quad is no longer mapped at creation.",
            );
        const descriptor = options.weighted
            ? result._variantPipeline
            : result._pipeline;
        return {
            descriptor,
