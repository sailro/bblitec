...
function storageTextureRegisters(source: string): Set<string> {
    return new Set(
        [
            ...source.matchAll(
                /(Texture\w*)<\s*([^>]+)>\s+\w+\s*:\s*register\(t(\d+)(?:, space(\d+))?/g,
            ),
        ]
            .filter(
                (match) =>
                    /MS(?:Array)?$/.test(match[1]!) ||
                    /^(?:uint|int)\d?$/.test(match[2]!.trim()),
            )
            .map((match) =>
                registerKey({
                    kind: "t",
                    index: Number(match[3]),
                    space: Number(match[4] ?? 0),
                }),
            ),
    );
}
...
function compactRegisters(source: string, vertex?: boolean): string {
    ...
    for (const match of source.matchAll(pattern)) {
        const register = {
            ...
            space: Number(match[3] ?? 0),
        };
        registers.set(registerKey(register), register);
    }
    const storage = new Set<string>();
    for (const match of source.matchAll(
        /(?:RW)?(?:ByteAddress|Structured)Buffer(?:<[^>]+>)?\s+\w+\s*:\s*register\(t(\d+)(?:, space(\d+))?\)/g,
    )) {
        storage.add(
            registerKey({
                kind: "t",
    ...
    const resourceOrder = (register: Register): number =>
        storage.has(registerKey(register))
            ? 2
            : storageTextures.has(registerKey(register))
              ? 1
              : 0;
    for (const kind of ["b", "t", "s", "u"]) {
        const ordered = [...registers.values()].filter(
            (register) => register.kind === kind,
        );
        ...
        for (const register of ordered) {
            if (vertex === undefined && register.space !== previousSpace)
                index = 0;
    ...
    return source.replace(
        pattern,
        (_match, kind: string, index: string, space: string | undefined) => {
            const original = {
                kind,
                index: Number(index),
            ...
            if (mapped === undefined)
                throw new Error(
                    `Unmapped HLSL register ${registerKey(original)}.`,
                );
            const target =
                vertex === undefined
                    ? original.space
...
function mapSampledTextureUses(
    ...
    source = source.replace(
        /(\b(\w+)\.(?:Sample\w*|Gather\w*|CalculateLevelOfDetail\w*)\(\s*)(\w+)/g,
        pair,
    );
    ...
    if (samplers.length)
        source = source.replace(
            new RegExp(`(\\b(\\w+)\\s*,\\s*)(${samplers.join("|")})\\b`, "g"),
            pair,
        );
    return source;
}
...
export function sdlSpirvSource(source: string, vertex: boolean): string {
    ...
    const samplers = new Map(
        [
            ...source.matchAll(
                /(Sampler\w*State)\s+(\w+)\s*:\s*register\(s\d+, space\d+\)/g,
            ),
        ].map((match) => [match[2]!, match[1]!]),
    );
    const textures = new Map<
        string,
        { index: number; space: number; paired: string; used: Set<string> }
    >();
    ...
    for (const match of source.matchAll(
        /Texture\w*(?:<[^>]+>)?\s+(\w+)\s*:\s*register\(t(\d+), space(\d+)\)/g,
    )) {
        const texture = match[1]!,
            index = Number(match[2]),
            space = Number(match[3]);
        if (storageTextures.has(registerKey({ kind: "t", index, space })))
            continue;
        textures.set(texture, {
            index,
            space,
            paired: `bblite_spirv_sampler_${space}_${index}`,
            used: new Set(),
        });
    }
    const pair = (
    ...
    for (const [texture, { index, space, paired, used }] of textures) {
        if (used.size > 1)
            throw new Error(
                `SDL Vulkan cannot bind multiple samplers for texture ${texture}.`,
            );
        const sampler = [...used][0];
        if (sampler !== undefined && !samplers.has(sampler))
            throw new Error(
                `Unknown HLSL sampler ${sampler} for texture ${texture}.`,
            );
        pairedSamplers.push(
            `${sampler === undefined ? "SamplerState" : samplers.get(sampler)} ${paired} : register(s${index}, space${space});`,
        );
    }
    source =
        pairedSamplers.join("\n") +
        "\n" +
        source.replace(
            /Sampler\w*State\s+\w+\s*:\s*register\(s\d+, space\d+\)\s*;/g,
            "",
        );
    return source.replace(
...
export function normalizeTintHlslBindings(source: string): string {
    ...
    const normalized = compactRegisters(source)
        .replace(
            ...
            (_match, name: string, body: string) => {
                ...
                const position = (line: string): boolean =>
                    /:\s*SV_Position/.test(line);
...
export function shaderStageSlots(hlsl: string): ShaderSlot[] {
    ...
    for (const match of hlsl.matchAll(
        ...
        if (
            !storageTextures.has(
                registerKey({ kind: "t", index: Number(match[1]), space }),
            )
        ) {
    ...
    for (const match of hlsl.matchAll(
        ...
        const storageTexture =
            match[4] === "t" &&
            storageTextures.has(
                registerKey({ kind: "t", index: Number(match[5]), space }),
            );
        slots.push({
            ...
            index:
                Number(match[5]) -
                (storage
                    ? (texturesBySpace.get(space) ?? 0)
                    : storageTexture
                      ? (sampledBySpace.get(space) ?? 0)
                      : 0),
...
export function sdlMslSource(
    msl: string,
    hlsl: string,
    slots: readonly ShaderSlot[],
): string {
        ...
        (call, _prefix, texture, sampler) => {
            ...
            if (slot?.kind === "t" && byName.get(sampler)?.kind === "s") {
                samplerSlots.set(
                    sampler,
                    Math.min(
                        samplerSlots.get(sampler) ?? slot.index,
                        slot.index,
                    ),
                );
            }
    ...
    if (/\btint_storage_buffer_sizes\s*\[\[buffer\(30\)\]\]/.test(msl)) {
        ...
        let lengths = 0;
        msl = msl.replace(
            /(\(\*\w+\.tint_storage_buffer_sizes\))\[(\d+)u\]\.([xyzw])/g,
            (_load, pointer: string, vector: string, lane: string) => {
                const buffer =
                    storage[Number(vector) * 4 + "xyzw".indexOf(lane)];
                if (!buffer)
                    throw new Error(
    ...
    return msl.replace(
        /\b(\w+)\s*\[\[(buffer|texture|sampler)\(\d+\)\]\]/g,
        (_declaration, name: string, kind: string) => {
            if (name === "tint_storage_buffer_sizes" && kind === "buffer")
                return `${name} [[buffer(30)]]`;
            ...
            if (kind !== expectedKind)
                throw new Error(
                    `Metal resource ${name} has unexpected kind ${kind}.`,
                );
            const index =
                slot.kind === "s"
                    ? samplerSlots.get(name)
                    : slot.index +
                      (slot.kind === "r"
                          ? uniforms
                          : slot.kind === "i"
...
export function hlslUniformBufferNames(hlsl: string): string[] {
    return [...hlsl.matchAll(/cbuffer\s+(\w+)\s*:\s*register\(b/g)].map(
        (match) => match[1]!,
    );
}
...
export function assertUniformBufferCap(hlsl: string, stage: string): void {
    ...
    if (names.length > 4) {
        throw new Error(
            `${stage} binds ${names.length} uniform buffers (${names.join(", ")}); SDL_GPU caps a stage at 4.`,
        );
    }
}
...
export function sdlUniformSource(
    ...
    if (hlslUniformBufferNames(hlsl).length <= 4 && !plan?.required)
        return undefined;
