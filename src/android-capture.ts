...
export function androidCaptureSettings(
    scene: SceneDefinition | undefined,
    {
        canvasOnly = false,
        backend = "sdl_gpu",
    }: { canvasOnly?: boolean; backend?: NativeBackend } = {},
    ...
    const captureEnvironment = measuredRunEnvironment({
        ...
        extra: {
            ...
            ...(canvasOnly ? { BBLITE_CAPTURE_UI: "0" } : {}),
        },
    });
...
export function verifyAndroidNativeRun(
    log: string,
    runId: string,
    ...
    for (const match of log.matchAll(
        /\b(?:GPU backend: (\S+)|Native exit: (-?\d+)) run=(\S+)/g,
    )) {
    ...
    if (selections !== 1 || selected !== backend) {
        throw new Error(
            `The Android app did not select ${backend} for this run; inspect logcat.txt.`,
        );
    }
    if (exits !== 1) {
        throw new Error(
            "The Android app did not report one native exit within 90 seconds; inspect logcat.txt and ensure the device is unlocked.",
        );
    }
    if (exitStatus !== "0")
        throw new Error(
            `The Android app exited with status ${exitStatus}; inspect logcat.txt.`,
        );
}
