...
export function parseImageCodecs(manifest: unknown): ImageCodec[] {
    ...
    if (!features)
        throw new Error("Image codec manifest must declare features.");
    ...
    for (const [codec, feature] of Object.entries(features)) {
        ...
        const refuse = (): never => {
            throw new Error(
                `Invalid image codec metadata for vcpkg feature '${codec}'.`,
            );
        };
    ...
    if (codecs.length === 0)
        throw new Error("Image codec manifest declares no image features.");
    return codecs;
}

export const imageCodecs = parseImageCodecs(
    JSON.parse(
        readFileSync(
            resolve(
...
export function imageCodecForFileName(
    reference: string,
    ...
    return imageCodecs.find(({ extensions }) =>
