...
export function lowerBabylonSubmeshIndices(context: LoweringContext): string {
    ...
    const slice = lowerPinnedBody(
        file,
        [
            ts.factory.createReturnStatement(
                context.variableInitializer(declaration, "subIndices"),
            ),
        ],
        {
            bindings,
                ...
                [
                    ...
                    (receiver, args) => {
                        if (args.length !== 2)
                            context.contractError(
                                declaration,
                                "Expected the submesh's index slice boundaries.",
                            );
    ...
    const loops = context.findNodes(
        ...
        (node): node is ts.ForOfStatement =>
            ...
            context.expressionMatchesShape(node.expression, "subMeshes"),
    );
    const loop = loops[0];
    const guard =
        loop && ts.isBlock(loop.statement) && loop.statement.statements[0];
    if (loops.length !== 1 || !guard || !ts.isIfStatement(guard))
        context.contractError(declaration, "Expected the empty-submesh guard.");
    const keep = lowerPinnedBody(file, [guard], {
        bindings,
        calls: new Map(),
        statement(statement, _lowerer, indent) {
            return ts.isContinueStatement(statement) && !statement.label
                ? [`${indent}return false;`]
                : undefined;
        },
    });
