...
export class TaaPostProcessLowerer {
    constructor(
        ...
        private readonly composite: ComposedComposite,
    ) {
        if (composite.intrinsic !== FACTORY)
            throw new Error("TAA lifecycle requires the pinned TAA composite.");
    }
    ...
    private method(name: string) {
        return this.context.propertyFunction(MODULE, FACTORY, name, {
            unique: true,
        });
    }
    ...
    private children(): { name: string; index: number }[] {
        ...
        return [
            ["_blend", "-blend"],
            ["_present", "-present"],
            ["_historyUpdate", "-history-update"],
        ].map(([property, suffix]) => {
            const child = this.context.propertyInitializer(task, property!);
            if (!ts.isIdentifier(child))
                this.context.contractError(
                    child,
                    "Expected a retained TAA child task.",
                );
            const index = this.composite.passes.findIndex(
                (pass) => passSuffix(pass.name) === suffix,
            );
            if (index < 0)
                this.context.contractError(
                    child,
                    `TAA child '${property}' was not observed.`,
                );
    ...
    private initialize(): string {
        ...
        const lowerer = new PinnedNumericLowerer(file, {
            bindings: new Map([
                ...
                [
                    "config.disableOnCameraMove",
                    { cpp: "disable_on_camera_move", type: "bool" },
                ],
            ]),
            calls: new Map(),
        });
        return `inline TaaPostProcessState create_taa_post_process_state(double factor, bool disable_on_camera_move) {
    return TaaPostProcessState{${stateFields
        .map(([field]) =>
            lowerer.expression(this.context.propertyInitializer(task, field)),
        )
        .join(", ")}};
}`;
    }
    ...
    private record(): string {
        ...
        const prefixFile = ts.createSourceFile(
            `${file.fileName}-record-resources`,
            `const record_resources = () => {${prefix.map((statement) => statement.getText(file)).join("\n")}};`,
            ts.ScriptTarget.Latest,
            true,
        );
        const shape = this.context.variableInitializer(
            prefixFile,
            "record_resources",
        );
        if (
            ...
            this.context.contractError(
                declaration,
                "TAA record resource prefix changed; its PAL callback must be re-read.",
            );
        }

        const body = lowerPinnedBody(
            file,
            declaration.body.statements.slice(prefix.length),
            { bindings: this.bindings(), calls: new Map() },
        );
...
${body}
    ...
    private execute(): string {
        ...
        const calls = new Map<string, (args: readonly string[]) => string>([
            ...
            [
                `${children[0]!.name}.updateUniforms`,
                (args) => {
                    if (args.length !== 0)
                        this.context.contractError(
                            declaration,
        ...
        const lowerer: PinnedNumericLowerer = new PinnedNumericLowerer(file, {
            ...
            returnValue: (expression) =>
                expression
                    ...
                    : this.context.contractError(
                          declaration,
                          "TAA execute must return its draw count.",
                      ),
        });
        for (const child of children) {
            const call = `${child.name}.execute?.()`;
            const sites = this.context.findNodes(
                declaration,
            ...
            if (sites.length !== 1)
                this.context.contractError(
                    declaration,
                    `Expected one optional '${call}' result.`,
                );
...
${body}
    ...
    private leafDrawCount(): string {
        ...
        if (!last || !ts.isReturnStatement(last) || !last.expression)
            this.context.contractError(
                declaration,
                "Post-process leaf needs a draw count.",
            );
        const count = this.context.numericValue(last.expression, file);
        const parsed = ts.createSourceFile(
            "post-process-leaf.ts",
            `const body = () => ${declaration.body.getText(file)}`,
            ts.ScriptTarget.Latest,
            true,
        );
        this.context.assertExpressionShape(
            this.context.variableInitializer(parsed, "body"),
            `() => {
