...
export function lowerPhysicsHeightfield(context: LoweringContext): {
    header: string;
    source: string;
} {
    const { file, declaration } = context.functionDeclaration(
        physicsHeightfieldModule,
        "optionsFromGroundMesh",
    );
    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        statement: (statement, numeric, indent) => {
            if (
                ...
                context.assertStatementShapes(
                    statement,
                    [statement],
                    `if (!localPositions || localPositions.length === 0) { throw new Error("createHeightFieldShape ground mesh has no vertex positions."); }`,
                    "heightfield absent positions",
                );
                const condition = statement.expression as ts.BinaryExpression;
                const buffer = numeric.expression(
                    (condition.left as ts.PrefixUnaryExpression).operand,
                );
                return [
                    `${indent}if (${buffer}.empty()) throw std::runtime_error("createHeightFieldShape ground mesh has no vertex positions.");`,
                ];
            }
            if (
                ...
                if (assignment.operatorToken.kind !== ts.SyntaxKind.EqualsToken)
                    context.contractError(
                        assignment,
                        "Heightfield matrix writes require direct numeric assignment.",
                    );
                const slot = assignment.left as ts.ElementAccessExpression;
                return [
                    `${indent}const double matrix_index = ${numeric.expression(slot.argumentExpression)};`,
            ...
            if (
                ...
                ts.isTemplateExpression(statement.expression.arguments[0])
            ) {
                ...
                context.assertExpressionShape(
                    ...
                    "heightfield grid diagnostic",
                );
                return [
                    `${indent}throw std::runtime_error("createHeightFieldShape requires a regular square grid; got " + bbl::js::number_to_string(${numeric.expression(message.templateSpans[0]!.expression)}) + " vertices.");`,
                ];
            }
            return undefined;
        },
        returnValue: (expression, lowerer) => {
            if (!expression || !ts.isObjectLiteralExpression(expression))
                context.contractError(
                    declaration,
                    "Expected resolved heightfield inputs.",
                );
            context.assertExpressionShape(
                expression,
                "{ numX: samples, numZ: samples, sizeX: extendX * 2, sizeZ: extendZ * 2, data: matrix }",
                "resolved ground heightfield fields",
            );
    ...
    const remap = lowerPinnedBody(file, factory.body!.statements.slice(6, 9), {
        bindings: new Map<string, PinnedBinding>([
            ...["numX", "numZ", "sizeX", "sizeZ"].map(
                (name) =>
                    [
                        name,
                        { cpp: `resolved.${name}`, type: "scalar" as const },
                    ] as const,
            ),
