...
export const babylonTextureProperties = new Map<
    string,
    { field: string; type: "float" | "std::uint32_t" | "bool" }
>([
    [
        "diffuseCoordIndex",
        { field: "diffuse_coord_index", type: "std::uint32_t" },
    ],
    [
        "specularCoordIndex",
        { field: "specular_coord_index", type: "std::uint32_t" },
    ],
    [
        "ambientCoordIndex",
        { field: "ambient_coord_index", type: "std::uint32_t" },
    ],
...
export function lowerBabylonTextureSlots(context: LoweringContext): string {
    ...
    const table = context.unwrapExpression(
        context.variableInitializer(file, "TEX_SLOTS"),
    );
    if (!ts.isArrayLiteralExpression(table))
        context.contractError(
            table,
            "Expected the Babylon texture slot table.",
        );
    const loops = context.findNodes(
        declaration,
        (node): node is ts.ForOfStatement =>
            ts.isForOfStatement(node) &&
            context.expressionMatchesShape(node.expression, "TEX_SLOTS"),
    );
    const loop = loops[0];
    if (loops.length !== 1 || !loop || !ts.isBlock(loop.statement))
        context.contractError(
            declaration,
            "Expected one Babylon texture slot loop.",
        );
    if (
        !ts.isVariableDeclarationList(loop.initializer) ||
        loop.initializer.declarations.length !== 1 ||
        !ts.isIdentifier(loop.initializer.declarations[0]!.name) ||
        loop.initializer.declarations[0]!.name.text !== "slot"
    )
        context.contractError(
            loop.initializer,
            "Expected the Babylon slot binding.",
        );
    const fields = new Map([
        ["diffuseTexture", "base_color_texture"],
        ["_bumpTexture", "bump_texture"],
        ["_specularTexture", "specular_texture"],
        ["_ambientTexture", "ambient_texture"],
        ["_lightmapTexture", "lightmap_texture"],
        ["_opacityTexture", "opacity_texture"],
    ...
    const string = (expression: ts.Expression): string => {
        if (!ts.isStringLiteral(expression))
            context.contractError(
                expression,
                "Expected a texture slot property name.",
            );
    ...
    const write = (
        ...
        if (!property)
            context.contractError(
                node,
                `Unsupported Babylon texture property '${name}'.`,
            );
        return [
            `${indent}material.${property.field} = static_cast<${property.type}>(${value});`,
        ];
    };
    const lowerJson = (
        ...
        const propertyName = (value: ts.Expression): string | undefined => {
            ...
            return ts.isPropertyAccessExpression(property) &&
                ...
                property.expression.text === "t"
                ? property.name.text
                : undefined;
        };
        const name = ts.isBinaryExpression(node)
    ...
    const callbacks = (
        ...
        if (!ts.isArrowFunction(arrow))
            context.contractError(arrow, "Expected a texture slot callback.");
        const expected = kind === "skipIf" ? ["t"] : ["t", "m"];
        if (
            arrow.parameters.length !== expected.length ||
            ...
            context.contractError(
                arrow,
                "Unexpected texture callback parameters.",
            );
        return arrow;
    };
    const output: string[] = [];
    for (const entry of table.elements) {
        if (!ts.isObjectLiteralExpression(entry))
            context.contractError(entry, "Expected a texture slot descriptor.");
        for (const property of entry.properties) {
            if (
                ...
                context.contractError(
                    property,
                    "Unsupported Babylon texture slot descriptor.",
                );
        }
        const source = string(context.propertyInitializer(entry, "src"));
        const setter = context.unwrapExpression(
            context.propertyInitializer(entry, "set"),
        );
        if (!ts.isArrowFunction(setter))
            context.contractError(
                setter,
                "Expected the texture setter callback.",
            );
        let target: string;
        if (ts.isBlock(setter.body)) {
            context.assertExpressionShape(
                setter,
                "(m, tex) => { m.diffuseTexture = tex; }",
                "Babylon direct texture setter",
            );
            target = "diffuseTexture";
        } else {
            const imports = context.findNodes(
                setter,
            ...
            if (
                ...
                context.contractError(
                    setter,
                    "Expected one imported Standard texture setter.",
                );
            const path = string(imported.arguments[0]!);
            const name = called.expression.text;
            context.assertExpressionShape(
                setter,
                `(m, tex) => import(${JSON.stringify(path)}).then(({ ${name} }) => ${name}(m, tex))`,
                "Babylon imported texture setter",
            );
            const definition = context.functionDeclaration(
                `src/${path.slice(3).replace(/\.js$/, ".ts")}`,
                name,
            ).declaration;
            ...
            if (
                ...
                context.contractError(
                    definition,
                    "Expected the Standard texture field assignment.",
                );
            target = assignment.expression.left.name.text;
            context.assertExpressionShape(
                assignment.expression,
                `mat.${target} = texture`,
                "Standard texture projection",
            );
            if (
                statements.length !== 2 ||
                !context.hasCall(statements[1]!, "_registerStdExt")
            )
                context.contractError(
                    definition,
                    "Expected only texture assignment and extension registration.",
                );
        }
        ...
        if (!nativeField)
            context.contractError(
                setter,
                `Unsupported Babylon texture field '${target}'.`,
            );
        const level = optionalProperty(entry, "level"),
            coord = optionalProperty(entry, "coordIndex");
        if (coord && !ts.isObjectLiteralExpression(coord))
            context.contractError(coord, "Expected a coordinate selector.");
        const destination = coord
            ? string(context.propertyInitializer(coord, "dst"))
            : undefined;
        const onlyOne = coord ? optionalProperty(coord, "only1") : undefined;
        if (
            onlyOne &&
            onlyOne.kind !== ts.SyntaxKind.TrueKeyword &&
            onlyOne.kind !== ts.SyntaxKind.FalseKeyword
        )
            context.contractError(
                onlyOne,
                "Expected a boolean coordinate selector.",
            );
        const skip = optionalProperty(entry, "skipIf"),
            extra = optionalProperty(entry, "extra");
        const bindings = new Map<string, PinnedBinding>([
            [
                "slot.level",
                { cpp: String(!!level), type: "bool", staticBoolean: !!level },
            ],
            [
                "slot.coordIndex",
                { cpp: String(!!coord), type: "bool", staticBoolean: !!coord },
            ],
            [
                "slot.coordIndex.only1",
                {
                    cpp: String(onlyOne?.kind === ts.SyntaxKind.TrueKeyword),
                    type: "bool",
                    staticBoolean: onlyOne?.kind === ts.SyntaxKind.TrueKeyword,
                },
            ],
            [
                "t",
                ...
                statement(statement, lowerer, indent) {
                    if (ts.isVariableStatement(statement)) {
                        ...
                        if (
                            ...
                            if (
                                ...
                                return [
                                    `${indent}const auto texture = source.value(${JSON.stringify(source)}, Json{});`,
                                ];
                            if (
                                variable.name.text === "texUrl" &&
                                variable.initializer
                            ) {
                                context.assertExpressionShape(
                                    variable.initializer,
                                    "baseUrl + t.name",
                                    "Babylon texture URL join",
                                );
                                return [
                                    `${indent}const std::string tex_url = pal::join_path(base_path, ${lowerer.expression((context.unwrapExpression(variable.initializer) as ts.BinaryExpression).right)});`,
                                ];
                            }
                        }
                    }
                    ...
                    if (
                        ...
                        if (!ts.isBlock(arrow.body))
                            context.contractError(
                                arrow,
                                "Expected texture side-effect statements.",
                            );
                            ...
                            arrow.body.statements,
                            {
                                ...
                                statement(statement, inner, spacing) {
                                    if (!ts.isExpressionStatement(statement))
                                        return undefined;
                                    const assignment = context.unwrapExpression(
                                        statement.expression,
                                    );
                                    if (
                                        ...
                                        assignment.left.expression.text !== "m"
                                    )
                                        return undefined;
                                    if (
                                        assignment.left.name.text === "uvScale"
                                    ) {
                                        const array = context.unwrapExpression(
                                            assignment.right,
                                        );
                                        if (
                                            ...
                                            context.contractError(
                                                array,
                                                "Expected two texture scale channels.",
                                            );
                                        return array.elements.map(
                                            (element, index) =>
                                                `${spacing}material.diffuse_${index === 0 ? "u" : "v"}_scale = static_cast<float>(${inner.expression(element)});`,
                                        );
                                    }
                                    return write(
                                        assignment.left.name.text,
                                        inner.expression(assignment.right),
                                        assignment.left,
                                        spacing,
                                    );
                                },
                    ...
                    if (
                        ...
                        if (
                            ...
                            const name = context.expressionMatchesShape(
                            ...
                            if (!name)
                                context.contractError(
                                    left,
                                    "Unknown texture slot assignment.",
                                );
                            return write(
                                name,
                    ...
                    if (
                        ...
                        return [
                            `${indent}const auto loaded_texture = load_texture(${JSON.stringify(source)}, tex_url);`,
                            `${indent}material.${nativeField} = loaded_texture.data;`,
                            ...(target === "diffuseTexture"
                                ? [
                                      `${indent}material.source_albedo_texture = loaded_texture;`,
                                  ]
                                : []),
                        ];
                    return undefined;
    ...
    if (
        ...
        context.contractError(
            loop,
            "Expected the texture-loading guard around the slot loop.",
        );
    const body = lowerPinnedBody(file, [guard], {
        bindings: new Map([
            ["opts.loadTextures", { cpp: "load_textures", type: "bool" }],
        ]),
        calls: new Map(),
