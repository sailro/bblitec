...
export function pinnedMat4CreateHeader(context: LoweringContext): string {
    ...
    const allocate = context.functionDeclaration(
        allocator,
        "allocateMat4",
    ).declaration;
    context.assertStatementShapes(
        allocate,
        allocate.body!.statements,
        "return (_allocate ?? _defaultAllocate)();",
        "Matrix allocator dispatch",
    );
    const defaultAllocate = context.functionDeclaration(
        allocator,
        "_defaultAllocate",
    ).declaration;
    context.assertStatementShapes(
        defaultAllocate,
        defaultAllocate.body!.statements,
        "return new F32(16);",
        "Default matrix storage",
    );
    const names = ["tx", "ty", "tz", "qx", "qy", "qz", "qw", "sx", "sy", "sz"];
    const numeric = (parameters: readonly string[]) =>
        parameters.map((pinned) => ({
            pinned,
            cpp: pinned,
            kind: "number" as const,
        }));
    const writer = lowerPinnedFunction(
        context,
        ...
        [
            ...
            { pinned: "off", cpp: "off", kind: "number" },
            ...numeric(names),
        ],
    ...
    const returned = (name: string) => ({
        type: "js::F32Array",
        value: (
            _lowerer: PinnedNumericLowerer,
            expression: ts.Expression | undefined,
        ): string => {
            if (!expression)
                return context.contractError(
                    allocate,
                    "Expected a matrix result.",
                );
            ...
            localStorage: [
                {
                    ...
                    binding: { cpp: "m", type: "f32" },
                    declaration: "js::F32Array m(16);",
                },
            ],
        },
    );
    const compose = lowerPinnedFunction(
        ...
        numeric(names),
        {
            cppName: "compose_mat4",
            inline: true,
            returns: returned("out"),
            localStorage: [
                {
                    pinned: "out",
                    initializer: "allocateMat4()",
                    binding: { cpp: "out", type: "f32" },
                    declaration: "js::F32Array out(16);",
                },
            ],
            calls: new Map([
                [
                    "composeMat4IntoBuffer",
                    (args) => `compose_mat4_into_buffer(${args.join(", ")})`,
                ],
            ]),
        },
    );
    const translation = lowerPinnedFunction(
        context,
        "src/math/create-translation-mat4.ts",
        "createTranslationMat4",
        numeric(["x", "y", "z"]),
        {
            cppName: "create_translation_mat4",
            inline: true,
            returns: returned("out"),
            localStorage: [
                {
                    pinned: "out",
                    initializer: "createIdentityMat4()",
                    binding: { cpp: "out", type: "f32" },
                    declaration: "js::F32Array out = create_identity_mat4();",
                },
                {
                    pinned: "s",
                    initializer: "out",
    ...
    return pinnedHeader(
        ["<bblite/js_data.hpp>"],
        [writer, identity, compose, translation].join("\n\n"),
    );
}
