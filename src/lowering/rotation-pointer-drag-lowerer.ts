...
export function lowerRotationPointerDrag(context: LoweringContext): string {
    ...
    for (const [source, native] of [
        ...
        ["signedAngleAroundNormal", "drag_signed_angle"],
        ["quatMul", "quat_mul"],
        ["quatNormalize", "quat_normalize"],
        ["quatFromAxisAngle", "quat_from_axis_angle"],
    ...
    const angle = lowerPinnedFunction(
        context,
        MATH,
        "signedAngleAroundNormal",
        parameters(["a", "b", "normal"]),
        {
            cppName: "drag_signed_angle",
            calls,
            callShapes: new Map([["crossVec3", "vec3"]]),
            returns: "double",
        },
    );
    const tupleCalls = new Map(
        [
            "quatMul",
            "quatNormalize",
            "quatFromAxisAngle",
    ...
    const worldLocal = context.functionDeclaration(
        MATH,
        "worldRotationToLocal",
    );
    const localStatements = [...worldLocal.declaration.body!.statements];
    const parentQuaternion = localStatements.findIndex(
        (statement) =>
    ...
    if (parentQuaternion < 0)
        context.contractError(
            worldLocal.declaration,
            "Expected the parent quaternion conjugation.",
        );
    context.assertStatementInventory(
        worldLocal.declaration,
        localStatements,
        "worldRotationToLocal",
        "the parent guard is a native seam and conjugation translates",
        [
            "variable statement",
            "if statement",
            "variable statement",
    ...
    context.assertStatementShapes(
        worldLocal.declaration,
        localStatements.slice(0, parentQuaternion),
        `
    ...
    const callback = assertRotationPointerContract(
        context,
        factory.declaration,
    );
    const body = [...callback.body.statements];
    const begin = body.findIndex(
        (statement) =>
            ts.isVariableStatement(statement) &&
            statement.declarationList.declarations[0]?.name.getText(
                factory.file,
    ...
    const end = body.findIndex(
        (statement) =>
            ...
            statement.expression.expression.getText(factory.file) === "rq.set",
    );
    if (begin < 0 || end <= begin)
        context.contractError(
            callback,
            "Missing plane rotation quaternion update.",
        );
    calls.set(
        "worldRotationToLocal",
        (args) => `drag_local_rotation(engine, ${args.join(", ")})`,
    );
    calls.set(
        "rq.set",
        (args) =>
                ...
                ...["x", "y", "z", "w"].map(
                    (lane) =>
                        [
                            `rq.${lane}`,
                            {
                                cpp: `static_cast<double>(node.rotation_quaternion.${lane})`,
                                type: "scalar",
                            },
                        ] as [string, PinnedBinding],
                ),
    ...
    const localBody = localStatements
        .slice(parentQuaternion)
        .flatMap((statement) => {
            if (
                ts.isVariableStatement(statement) &&
                statement.declarationList.declarations[0]?.name.getText(
                ...
                if (
                    ...
                    expression.elements.length !== 4
                )
                    context.contractError(
                        expression,
                        "Expected the inverse parent quaternion tuple.",
                    );
                return [
                    `    const std::array<double, 4> invPq{${expression.elements.map((item) => localLowerer.expression(item)).join(", ")}};`,
                ];
            }
