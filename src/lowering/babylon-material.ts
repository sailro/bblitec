...
export function lowerBabylonMaterialProperties(
    ...
    if (
        ...
        context.contractError(
            declaration,
            "Expected one Babylon material construction loop.",
        );
    }
    ...
    if (
        ...
        context.contractError(
            loop,
            "Expected the material factory before its property assignments.",
        );
    }
    const created = first.declarationList.declarations[0]!;
    if (
        !ts.isIdentifier(created.name) ||
        !created.initializer ||
        !context.expressionMatchesShape(
            created.initializer,
    ...
    const textures = statements.findIndex((statement) =>
        context.hasCall(statement, "loadTexture2D"),
    );
    if (textures < 1)
        context.contractError(
            loop,
            "Expected texture loading after material properties.",
        );
    const colors = new Map([
        ["diffuseColor", "source_diffuse_color"],
    ...
    const initializers = [...colors].map(([name, field]) => {
        const lanes = context.numericTuple(
            context.propertyInitializer(defaults, name),
            factory.file,
        );
    ...
    initializers.unshift(
        `    material.source_group_builder = ${materialGroupIdentity(context, "standard")};`,
    );
    for (const [name, field] of scalars) {
        initializers.push(
            `    material.${field} = ${context.floatLiteral(context.numericValue(context.propertyInitializer(defaults, name), factory.file))};`,
        );
    }
    const culling = context.propertyInitializer(defaults, "backFaceCulling");
    if (
        culling.kind !== ts.SyntaxKind.TrueKeyword &&
        culling.kind !== ts.SyntaxKind.FalseKeyword
    )
        context.contractError(
            culling,
            "Standard material culling default must be boolean.",
        );
    initializers.push(
        `    material.double_sided = ${culling.kind === ts.SyntaxKind.FalseKeyword};`,
    );
    for (const [name, property] of babylonTextureProperties) {
        ...
        if (
            ...
            context.contractError(
                value,
                "Expected a boolean Standard texture default.",
            );
        const literal =
            property.type === "bool"
                ? String(value.kind === ts.SyntaxKind.TrueKeyword)
                : context.doubleLiteral(
                      context.numericValue(value, factory.file),
                  );
        initializers.push(
            `    material.${property.field} = static_cast<${property.type}>(${literal});`,
        );
    }
    const scale = context.unwrapExpression(
        context.propertyInitializer(defaults, "uvScale"),
    );
    if (!ts.isArrayLiteralExpression(scale) || scale.elements.length !== 2)
        context.contractError(
            scale,
            "Expected two Standard texture scale defaults.",
        );
    scale.elements.forEach((value, index) =>
        initializers.push(
    ...
    const body = lowerPinnedBody(file, statements.slice(1, textures), {
        ...
        statement(statement, lowerer, indent) {
            ...
            if (
                ...
                target.expression.text !== material
            )
                return undefined;
            ...
            if (color) {
                ...
                if (
                    ...
                    context.contractError(
                        value,
                        "A material RGB assignment requires three channels.",
                    );
                }
                const lanes = value.elements.map((element) =>
            ...
            if (scalar)
                return [
                    `${indent}material.${scalar} = static_cast<float>(${lowerer.expression(assignment.right)});`,
                ];
            if (name === "backFaceCulling")
                return [
                    `${indent}material.double_sided = !(${lowerer.expression(assignment.right)});`,
                ];
            context.contractError(
                target,
                `Unsupported Babylon material property '${name}'.`,
            );
        },
    });
