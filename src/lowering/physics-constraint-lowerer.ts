...
export function lowerPhysicsConstraints(context: LoweringContext): {
    header: string;
    source: string;
} {
    ...
    context.assertExpressionShape(
        factory.declaration.parameters[4]!.initializer!,
        "{}",
        "constraint options default",
    );
    context.assertExpressionShape(
        factory.declaration.parameters[5]!.initializer!,
        "[]",
        "constraint limits default",
    );
    ...
    for (const [name, expression] of [
        ...
        context.assertExpressionShape(
            context.variableInitializer(factory.file, name!),
            expression!,
    ...
    context.assertStatementShapes(
        ...
        "return [v.x,v.y,v.z];",
        "constraint anchor array order",
    );
    const configure = context.functionDeclaration(
        module,
        "configureConstraintAxes",
    ).declaration;
    ...
    if (statements.length !== 5 || !select || !ts.isSwitchStatement(select))
        context.contractError(
            configure,
            "Expected constraint configuration to end in its type switch.",
        );
    context.assertExpressionShape(
        select.expression,
        "type",
    ...
    const hinge = select.caseBlock.clauses.find(
        (clause) =>
            ts.isCaseClause(clause) &&
            ts.isPropertyAccessExpression(clause.expression) &&
            clause.expression.name.text === "HINGE",
    );
    if (!hinge || !ts.isCaseClause(hinge))
        context.contractError(
            select,
            "Expected the HINGE constraint configuration.",
        );
    context.assertExpressionShape(
        hinge.expression,
        "PhysicsConstraintType.HINGE",
        "hinge constraint selection",
    );
    context.assertStatementShapes(
        hinge,
        hinge.statements,
    ...
    const enumMembers = (name: string) =>
        context
            .objectInitializer(factory.file, name)
            .properties.map((property) => {
                if (
                    ...
                    context.contractError(
                        property,
                        "Constraint enumerators must be named numeric properties.",
                    );
                return [
                    property.name.text,
                    context.numericValue(property.initializer, factory.file),
                ] as const;
            });
    ...
    const bindings = () =>
        new Map<string, PinnedBinding>([
            ...
            ...axisMembers.map(
                ([name, value]) =>
                    [
                        `PhysicsConstraintAxis.${name}`,
                        { ...scalar(`${value}.0`), staticNumber: value },
                    ] as const,
            ),
            ...axisNames.map(
                (name, index) =>
                    [`axis.${name}`, scalar(`${index}.0`)] as const,
            ),
        ]);
    const axisMapping = context.functionDeclaration(
        module,
        "constraintAxisToNative",
    );

    const nativeAxis = `double native_constraint_axis(double value) {\n${lowerPinnedBody(
        axisMapping.file,
        axisMapping.declaration.body!.statements,
        {
            calls: new Map(),
            bindings: new Map([...bindings(), ["value", scalar("value")]]),
            returnValue: (value, axisLowerer) =>
                value
                    ? axisLowerer.expression(value)
                    : context.contractError(
                          axisMapping.declaration,
                          "A constraint axis mapping must return a value.",
                      ),
        },
    )}\n    throw std::runtime_error("Unknown constraint axis.");\n}`;
    const configurations = select.caseBlock.clauses
        .map((clause) => {
            if (
                !ts.isCaseClause(clause) ||
                !ts.isPropertyAccessExpression(clause.expression)
                ...
                context.contractError(
                    clause,
                    "Constraint configuration requires named type cases.",
                );
            ...
            if (value === undefined)
                context.contractError(
                    clause,
                    "Unknown constraint type configuration.",
                );
            context.assertExpressionShape(
                clause.expression,
                `PhysicsConstraintType.${name}`,
            ...
            scopeBindings.set("options.maxDistance", {
                ...scalar("*options.max_distance"),
                absentCpp: "!options.max_distance",
            });
            scopeBindings.set("l.minLimit", {
                ...scalar("l.minimum"),
                absentCpp: "false",
            });
            scopeBindings.set("l.maxLimit", {
                ...scalar("l.maximum"),
                absentCpp: "false",
            });
            for (const field of ["minLimit", "maxLimit"])
                scopeBindings.set(`l.${field} !== undefined`, {
                    cpp: "true",
                    type: "bool",
                    staticBoolean: true,
                });
            scopeBindings.set("l.stiffness", {
                ...scalar("0.0"),
                staticallyAbsent: true,
            });
            scopeBindings.set("l.damping", {
                ...scalar("0.0"),
                staticallyAbsent: true,
            });
            scopeBindings.set("l.axis", scalar("l.axis"));
            const element = (axis: string) =>
            ...
            const lowerer = new PinnedNumericLowerer(factory.file, {
                ...
                expression: (node, lowerer) => {
                    ...
                    for (const field of [
                        ...
                        if (
                            ...
                            return field === "minLimit" || field === "maxLimit"
                                ? "true"
                                : "false";
                    }
                    return undefined;
                ...
                statement: (node) => {
                    ...
                    for (const [field, setter] of [
                        ...
                        context.assertStatementShapes(
                            node,
                            [node],
                            `if (l.${field} !== undefined) { hknp.HP_Constraint_SetAxis${setter}(joint, nativeAxis, l.${field}); }`,
                            "Absent constraint spring option",
                        );
                        return [];
                    }
                    return undefined;
                ...
                calls: new Map([
                    [
                        "lock",
                        (args) =>
                            `${element(args[0]!)}.mode = pal::PhysicsConstraintAxisMode::locked`,
                    ],
                    [
                        "limit",
                        (args) =>
                            `${element(args[0]!)} = {pal::PhysicsConstraintAxisMode::limited, ${args[1]}, ${args[2]}}`,
                    ],
                    [
                        "constraintAxisToNative",
                        (args) => `native_constraint_axis(${args[1]})`,
                    ],
                    [
                        "hknp.HP_Constraint_SetAxisMode",
                        (args) => `${element(args[1]!)}.mode = ${args[2]}`,
                    ],
                    [
                        "hknp.HP_Constraint_SetAxisMinLimit",
                        (args) => `${element(args[1]!)}.minimum = ${args[2]}`,
                    ],
                    [
                        "hknp.HP_Constraint_SetAxisMaxLimit",
                        (args) => `${element(args[1]!)}.maximum = ${args[2]}`,
                    ],
                ]),
                forOf: (range, name) =>
                    range === "limits" && name === "l"
            ...
            scopeBindings.set("mode.LIMITED", {
                cpp: "pal::PhysicsConstraintAxisMode::limited",
                type: "opaque",
            });
            const body =
            ...
            if (!last || !ts.isBreakStatement(last))
                context.contractError(
                    clause,
                    "Constraint type configuration must terminate with break.",
                );
            return `    if (type == ${value}.0) {\n${lowerer.statements(body.slice(0, -1), "        ").join("\n")}\n        return axes;\n    }`;
        })
        .join("\n");
    const normalFunctions = ["normalizeVec3", "normalTo"]
        .map((name) => {
                ...
                declaration.body!.statements,
                {
                    ...
                    returnValue: (expression, lowerer) => {
                        if (!expression)
                            context.contractError(
                                declaration,
                                "A constraint normal helper must return a vector.",
                            );
