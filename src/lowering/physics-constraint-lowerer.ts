...
// ${context.provenance(module, "createPhysicsConstraint", "HINGE anchor defaults and axis locks; PAL joint solver")}
void create_physics_hinge(PhysicsWorldHandle world, PhysicsBody body_a, PhysicsBody body_b, const PhysicsConstraintOptions& options) {
    ...
    pal::physics_world_create_hinge(physics_world_record(world).handle, body_a.handle, body_b.handle,
        {{pivot_a.x, pivot_a.y, pivot_a.z}, {axis_a.x, axis_a.y, axis_a.z}, {perpendicular_a.x, perpendicular_a.y, perpendicular_a.z}},
        {{pivot_b.x, pivot_b.y, pivot_b.z}, {axis_b.x, axis_b.y, axis_b.z}, {perpendicular_b.x, perpendicular_b.y, perpendicular_b.z}}, options.collision);
}
`;
