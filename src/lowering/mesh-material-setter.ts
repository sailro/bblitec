...
export function lowerMeshMaterialSetter(context: LoweringContext): string {
    ...
    context.assertExpressionShape(
        context.variableInitializer(source, "_meshScenes"),
        "null",
        "Initial mesh owner registry",
    );
    ...
    if (
        ...
        setter.parameters[0]!.name.getText() !== "v"
    )
        context.contractError(
            install.declaration,
            "Expected the installed mesh material setter.",
        );
    const lookup =
        ...
        lowerPinnedBody(declaration.getSourceFile(), statements, {
            ...
            expression(node, lowerer) {
                if (
                    ...
                    context.expressionMatchesShape(node, "new Set()")
                )
                    return "std::make_shared<MeshMaterialSceneOwners>()";
                if (
                    ts.isBinaryExpression(node) &&
                    node.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
                    context.expressionMatchesShape(node.left, "scenes")
                )
                ...
                if (
                    context.expressionMatchesShape(node, "scenes.delete(scene)")
                )
            ...
            statement(statement, lowerer, indent) {
                if (
                    ...
                    if (variable.name.text === "map") {
                        context.assertExpressionShape(
                            variable.initializer,
                            "(_meshScenes ??= new WeakMap())",
                            "Mesh owner map allocation",
                        );
                        return [];
                    }
                ...
                if (ts.isForOfStatement(statement)) {
                    ...
                    if (
                        ...
                        context.contractError(
                            statement,
                            "Expected a scene subscriber binding.",
                        );
                    const statements = ts.isBlock(statement.statement)
                        ? statement.statement.statements
                        : [statement.statement];
                    return [
                        `${indent}for (const auto& owner : *scenes) {`,
                        `${indent}    const auto state = owner.lock();`,
                        `${indent}    if (!state) continue;`,
                        `${indent}    auto scene = Scene::from_state(state);`,
                        ...lowerer.statements(statements, indent + "    "),
                        `${indent}}`,
                    ];
                }
                if (
                    ...
                    if (
                        ...
                        return [
                            `${indent}if (scene.state->enqueue_material_group) scene.state->enqueue_material_group(scene, mesh);`,
                        ];
                }
                return undefined;
            },
    ...
    if (!guard || !ts.isIfStatement(guard))
        context.contractError(
            register,
            "Expected disposed mesh validation before subscription.",
        );
    context.assertExpressionShape(
        guard.expression,
        "mesh._disposed",
        "Mesh registration disposal guard",
    );
    const unregister = context.functionDeclaration(
        module,
        "unregisterMeshScene",
    ).declaration;
