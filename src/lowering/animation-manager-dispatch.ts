...
export function lowerAnimationManagerDispatch(
    ...
    const prefix = statements[0],
        guard = statements[1];
    if (
        !prefix ||
        !ts.isVariableStatement(prefix) ||
        prefix.declarationList.declarations.length !== 1 ||
        !ts.isIdentifier(prefix.declarationList.declarations[0]!.name) ||
        prefix.declarationList.declarations[0]!.name.text !== "step" ||
        !guard ||
        !ts.isIfStatement(guard)
    )
        context.contractError(
            declaration,
            "Expected source manager clock prefix before task traversal.",
        );
    ...
    context.expectShapeCount(
        task.declaration,
        "task.active = true",
        "Attached animation task activity",
    );
    const taskModule = "src/animation/animation-group-task.ts";
    const groupTask = context.functionDeclaration(
        taskModule,
        "addAnimationGroup",
    );
    context.expectShapeCount(
        groupTask.declaration,
        "category: ANIMATION_GROUP_TASK_CATEGORY",
        "Attached animation task category",
    );
    const bindings = new Map<string, PinnedBinding>([
        ["step", { cpp: "step", type: "scalar" }],
        [
            "handledCategory",
            { cpp: "handled", type: "bool", absentCpp: "!handled" },
        ],
        [
            "manager._taskCategory",
            { cpp: "true", type: "bool", staticBoolean: true },
        ],
        [
            "tasks.length",
            { cpp: "static_cast<std::int64_t>(tasks.size())", type: "scalar" },
        ],
    ...
    const body = lowerPinnedBody(file, statements.slice(2), {
        ...
        expression(node) {
            ...
            if (
                context.expressionMatchesShape(
                    node,
                    "manager._taskCategoryHandler?.(manager, step) ? manager._taskCategory : undefined",
                )
            )
            ...
            if (
                context.expressionMatchesShape(
                    node,
                    "manager.animations.slice()",
                )
            )
                return "manager.ordered_groups";
            if (context.expressionMatchesShape(node, "tasks[index]!"))
                return "tasks.at(static_cast<std::size_t>(index))";
            if (
                context.expressionMatchesShape(
                    node,
        ...
        statement(statement, lowerer, indent) {
            ...
            if (!ts.isIdentifier(variable.name) || !variable.initializer)
                return undefined;
            const names = new Map([
                ["handledCategory", "const bool handled"],
                ["tasks", "const auto tasks"],
                ["task", "const auto& task"],
            ]);
            const cpp = names.get(variable.name.text);
            return cpp
                ? [
                      `${indent}${cpp} = ${lowerer.expression(variable.initializer)};`,
                  ]
                : undefined;
        },
    });
