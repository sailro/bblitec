...
import {
    cppPrimary,
    renderPinnedArithmetic,
    type PinnedExpressionSpelling,
    type RenderedCpp,
...
> = new Map([
    ...
    [
        "vec3-list",
        { storage: cppVector(CPP_RECORD.vec3.storage), element: "vec3" },
    ],
    [
        "vec3-list-2d",
        {
            storage: cppVector(cppVector(CPP_RECORD.vec3.storage)),
            element: "vec3-list",
        },
    ],
]);
...
function recordBindings(
    ...
    const bindings = new Map<string, PinnedBinding>([
        [name, { cpp, type: "opaque", record: shape }],
    ]);
    for (const member of shape.members) {
        for (const [path, binding] of member.read(cpp)) {
            bindings.set(`${name}.${member.name}${path}`, binding);
        }
    }
    return bindings;
}
...
export interface PinnedBinding {
    ...
    type:
        ...
        | "opaque"
        /**
         * A growable list of the pin's object records
         * (`_ClusteredActiveLight[]`): indexed, iterated, truncated,
     ...
     * pushed onto the list is built member by member from it.
     */
...
/**
 * A native struct one of the pin's object records lands on: the caller owns
 * the struct, so it names how each member reads and stores.
 */
export interface PinnedRecordShape {
    /** The native struct. */
    readonly cpp: string;
    /** The pin's members, in the struct's own declaration order. */
    readonly members: readonly PinnedRecordMember[];
}

export interface PinnedRecordMember {
    readonly name: string;
    /**
     * What a read of this member off an element spelled `owner` binds,
     * keyed by the path under the member: `""` is the member itself, and
     * `".position"` one of its own members the body reads through it.
     */
    readonly read: (owner: string) => ReadonlyMap<string, PinnedBinding>;
    /** A literal's lowered value for this member, as its field initializer. */
    readonly store: (value: string) => string;
    /** The initializer an optional member a literal leaves out takes. */
...
export interface PinnedNumericScope {
     ...
     * produce.
     */
    arrayCopy?: (receiver: string, source: string, offset: string) => string;
    /**
...
export class PinnedNumericLowerer {
    ...
    public statement(statement: ts.Statement, indent: string): string[] {
        ...
        if (adapted !== undefined) {
            this.translationActivity?.request(
                "statement",
                statement,
                this.file,
            );
            ...
            if (
                ...
                if (
                    ...
                    if (
                        ...
                        const arrayStores = this.chainedArrayStores(
                            expression,
                            indent,
                        );
                    ...
                    return [
                        ...this.statement(
                            ts.factory.createExpressionStatement(right),
                            indent,
                        ),
                        ...this.statement(
        ...
        if (ts.isIfStatement(statement)) {
            ...
            if (known !== undefined) {
                if (known)
                    return this.statement(statement.thenStatement, indent);
    ...
    protected forOfIteration(
        ...
        for (const [name, value] of resolved.bindings)
            this.scope.bindings.set(name, value);
     ...
     */
    public statements(list: readonly ts.Statement[], indent: string): string[] {
        const lines: string[] = [];
    ...
    private declarations(
        ...
        for (const declaration of list.declarations) {
            ...
            if (ts.isObjectBindingPattern(declaration.name)) {
                ...
                const temporary = this.temporaryName(declaration, lines.length);
                // `const { light, depth } = activeLights[i]!` -- members of
                // one element of a record list, read in place through the
                // struct the list's elements are.
                const record = this.recordElement(declaration.initializer);
                if (record) {
                    lines.push(
                        `${indent}const auto& ${temporary} = ${record.cpp};`,
                    );
                    for (const element of declaration.name.elements) {
                        const member = record.shape.members.find(
                            (candidate) =>
                                ts.isIdentifier(element.name) &&
            ...
            const source: ts.Expression =
                specialized ?? declaration.initializer;
            ...
            if (ts.isConditionalExpression(rowSource)) {
                const chosen = [rowSource.whenTrue, rowSource.whenFalse].map(
                    (branch) => {
            ...
            if (
                ts.isCallExpression(initializer) &&
                this.scope.nullableMatrixCalls?.has(
                ...
                lines.push(
                    `${indent}${isConst ? "const " : ""}auto ${cpp} = ${this.expression(source)};`,
                );
                continue;
            }
     ...
     */
    private allocation(initializer: ts.Expression):
        | {
              type: PinnedBinding["type"];
              bytesCpp?: string;
              mutable?: true;
        ...
        const constructor =
            TYPED_ARRAY_CONVERSIONS.get(
                sourceConstructor,
            )?.type.toUpperCase() ?? sourceConstructor;
        ...
        if (constructor === "F32" && ts.isArrayLiteralExpression(argument)) {
            const values = argument.elements.map(
                (element) => `static_cast<float>(${this.expression(element)})`,
            );
            return {
                type: "f32",
                declare: (name) =>
        ...
        if (source && isListShape(source.type)) {
            ...
            return conversion === undefined || width === undefined
                ...
                : {
                      type: width,
                      declare: (name) => `auto ${name} = ${conversion};`,
                  };
        }
        ...
        const literal = ts.isNumericLiteral(unwrapExpression(argument))
            ...
            : undefined;
        const fixed =
            literal !== undefined && Number.isInteger(literal) && literal > 0
                ? literal
                : undefined;
    ...
    private expressionStatement(expression: ts.Expression): string {
        ...
        if (ts.isBinaryExpression(expression)) {
            ...
            if (operator) {
                ...
                const owner = ts.isElementAccessExpression(target)
                    ? this.elementOwner(target)
                    : undefined;
                if (
                    owner?.indexedStore &&
                    ...
                    return owner.indexedStore(
                        owner.cpp,
                        this.expression(target.argumentExpression),
    ...
    private inPlaceSelfStore(
        ...
        if (
            ...
            !this.scope.receiverReturningMethods.has(call.expression.name.text)
        ) {
            return undefined;
        }
        const receiver = unwrapExpression(call.expression.expression).getText(
            this.file,
        );
    ...
    /**
     * `list.length = 0`: JavaScript's own truncation of a growable list,
     * which empties it in place. Any other length store would either
     * truncate to a run-time count or extend the list with holes, neither
     * of which a pinned body reached so far writes, so it refuses.
     */
    private lengthStore(expression: ts.BinaryExpression): string | undefined {
        const target = unwrapExpression(expression.left);
        if (
            expression.operatorToken.kind !== ts.SyntaxKind.EqualsToken ||
        ...
        if (this.staticNumberOf(expression.right) !== 0) {
            this.fail(expression, "list length store other than 0");
        }
        return `${list.cpp}.clear()`;
    }

    /**
     * `a ||= b`, `a &&= b` and the bitwise compound stores (`flags |= 1`).
     *
     * The logical forms assign only where the left side does not already
     * decide, and evaluate the right side only then -- which C++'s own
     * `a = a || b` does too, over a boolean local. A numeric local's
     * `||=` selects a value rather than a truth, so it refuses. The
     * bitwise forms go through the same `bbl::js` helpers the plain
     * operators do, so the ToInt32 coercion is JavaScript's.
     */
    private logicalAssignment(
        expression: ts.BinaryExpression,
    ): string | undefined {
        ...
        if (!logical && bitwise === undefined) return undefined;
        const target = unwrapExpression(expression.left);
        if (!ts.isIdentifier(target)) {
            return this.fail(expression, "compound assignment target");
        }
        ...
        if (logical) {
            ...
            return `${cpp} = ${cpp} ${logical} (${this.expression(expression.right)})`;
        }
        ...
        return `${cpp} = static_cast<double>(${jsBitwiseCall(bitwise!, cpp, this.expression(expression.right))})`;
    }
    ...
    private recordListForOf(
        ...
        return {
            range: list.cpp,
            bindings: recordBindings(element, element, list.record),
        };
    }

    /** One element of a record list, or a bound record, and its struct. */
    private recordElement(
        expression: ts.Expression,
    ): { cpp: string; shape: PinnedRecordShape } | undefined {
    ...
    /**
     * `list.push({...})` and `list.sort((a, b) => ...)` on a record list.
     *
     * Taken before the call's arguments lower, because an object literal is
     * not a value on its own: it is the list's struct, member by member.
     * `Array.prototype.sort` has been stable since ES2019, so the order
     * ties keep is the order they were pushed in, and the comparator's
     * `< 0` is the "a before b" its spec reads.
     */
    private recordListCall(node: ts.CallExpression): string | undefined {
        ...
        if (callee.name.text === "push") {
            ...
            const pushes = node.arguments.map(
                (argument) =>
                    `${list.cpp}.push_back(${this.recordValueOf(argument, shape)})`,
            );
            return pushes.length === 1 ? pushes[0]! : `(${pushes.join(", ")})`;
        }
        if (callee.name.text === "sort") {
            ...
            if (
                ...
                return this.fail(
                    node,
                    "record list sort (one two-parameter expression comparator, as a statement)",
                );
            }
    ...
    /**
     * One record value of `shape`: an object literal built member by
     * member in the struct's order -- a member it leaves out takes its
     * absent spelling, and one the struct does not have refuses -- or a
     * record of the same struct, copied.
     */
    private recordValueOf(
        ...
        for (const property of node.properties) {
            ...
            if (
                !name ||
                !shape.members.some((member) => member.name === name)
            ) {
                this.fail(property, `${shape.cpp} member`);
            }
            values.set(
                name,
                ts.isShorthandPropertyAssignment(property)
                    ...
                    : ts.isPropertyAssignment(property)
                      ...
                      : this.fail(property, `${shape.cpp} member value`),
            );
        }
        const fields = shape.members.map((member) => {
            const value = values.get(member.name);
            if (value) return member.store(this.expression(value));
            return (
                member.absent ??
                this.fail(node, `${shape.cpp} literal without '${member.name}'`)
            );
        });
        return `${shape.cpp}{${fields.join(", ")}}`;
    }
    ...
    private specializedDeclaration(
        ...
        if (ts.isCallExpression(initializer)) {
            const shape = callShapeOf(
                this.scope.callShapes,
                initializer,
                this.file,
            );
     ...
     */
    private storedValue(target: ts.Expression, value: ts.Expression): string {
        const unwrapped = unwrapExpression(target);
        ...
        if (binding?.type === "bool" && !this.booleanValue(value)) {
            this.fail(value, "non-boolean store into a boolean local");
        }
    ...
    private listConversion(node: ts.NewExpression): string | undefined {
        ...
        const spelling = TYPED_ARRAY_CONVERSIONS.get(node.expression.text);
    ...
    private inlinedHelperCall(
        ...
        const header = `${indent}// ${callee.text}(${node.arguments
    ...
    private truthyValue(
        ...
        if (kind === "nullable") return `static_cast<bool>(${cpp})`;
        return `bbl::js::number_truthy(${cpp})`;
    }
    ...
    private logicalValue(node: ts.BinaryExpression): string {
        ...
        if (kind === "boolean")
            return `(${this.condition(node.left)} ${or ? "||" : "&&"} ${this.condition(node.right)})`;
    ...
    private elementBinding(
        ...
        // One record of a record list is the struct itself, read in place.
        if (owner?.type === "record-list" && owner.record) {
            return {
                cpp: this.elementAccess(access),
                type: "opaque",
                record: owner.record,
            };
        }
    ...
    private chainedArrayStores(
        ...
        const effectFree = (node: ts.Node): boolean => {
            ...
            return !ts.forEachChild(node, (child) =>
                effectFree(child) ? undefined : true,
            );
        };
        if (targets.some((target) => !effectFree(target))) return undefined;
        const references = targets.map((target, index) => ({
            target,
            name: this.temporaryName(target, index),
        }));
        return [
            ...references.map(
                ({ target, name }) =>
                    `${indent}auto& ${name} = ${this.assignmentTarget(target)};`,
            ),
            ...references
                .reverse()
                .map(
                    ({ target, name }) =>
                        `${indent}${name} = ${this.storedValue(target, value)};`,
                ),
        ];
    }
    ...
    private assignmentTarget(expression: ts.Expression): string {
        ...
        if (ts.isElementAccessExpression(unwrapped)) {
            ...
            if (target?.indexedStore)
                this.fail(unwrapped, "reference to an adapted indexed store");
            if (
                target &&
            ...
            if (owner && isListShape(owner.type)) {
                const index = this.expression(unwrapped.argumentExpression);
                return (
    ...
    private elementAccess(expression: ts.ElementAccessExpression): string {
    ...
    public renderExpression(expression: ts.Expression): RenderedCpp {
        ...
        if (adapted !== undefined)
            this.translationActivity?.request("expression", node, this.file);
        const domain = adapted ?? this.expressionDomain(node);
        if (domain !== undefined)
            return typeof domain === "string" ? cppPrimary(domain) : domain;
        return (
            renderPinnedArithmetic(
                node,
                (child) => this.renderExpression(child),
                this.scope.expressionSpelling,
            ) ??
            this.fail(
                node,
                ts.isBinaryExpression(node) ? "binary operator" : "expression",
            )
        );
    }
    ...
    private absentOptionalCall(expression: ts.Expression): boolean {
        ...
        while (
            ts.isCallChain(chain) ||
            ts.isPropertyAccessChain(chain) ||
            ts.isElementAccessChain(chain)
        ) {
            if (
        ...
        if (known !== undefined) {
            let selected = clauses.findIndex((clause) => {
                ...
                if (value === undefined) {
                    this.fail(
                        clause.expression,
                        "switch case over a static discriminant",
                    );
                }
                return value === known;
            });
            if (selected < 0) {
                selected = clauses.findIndex((clause) =>
    ...
    private valueKind(
        expression: ts.Expression,
    ): "boolean" | "number" | "string" | "object" | "nullable" {
        const node = unwrapExpression(expression);
        const binding = this.scope.bindings.get(node.getText(this.file));
        if (binding) {
    ...
    private propertyAccess(
        ...
        if (optional) {
            ...
            return (
                `(${optional.present} ? ${optional.member.cpp} : ` +
                `${absent})`
            );
        }
    ...
    private call(node: ts.CallExpression): string {
        const recordListCall = this.recordListCall(node);
        if (recordListCall !== undefined) return recordListCall;
        const callee = node.expression;
        ...
        if (
            ...
            const declaration = pinnedNamesOf(this.file).declarationOf(
                callee.expression,
            );
            const global = declaration
                ? declarationInDefaultLibrary(declaration)
                : !moduleSymbols(this.file).getSymbolAtLocation(
                      callee.expression,
                  );
            if (global) {
    ...
    private binary(node: ts.BinaryExpression): string | undefined {
        ...
        if (
            ...
            const leftAbsent = this.absenceTest(left),
                rightAbsent = this.absenceTest(right);
            if (leftAbsent !== undefined || rightAbsent !== undefined) {
                const lhs = leftAbsent ?? "false",
