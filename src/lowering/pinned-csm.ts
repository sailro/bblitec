...
class CsmNumericAdapter extends PinnedNumericLowerer {
    ...
    private value(expression: ts.Expression): Value | undefined {
        ...
        if (ts.isElementAccessExpression(node)) {
            ...
            if (
                !owner ||
                (owner.shape.kind !== "array" && owner.shape.kind !== "buffer")
            )
                return undefined;
            this.bindNumeric(node.expression, owner);
            return {
                shape:
                    owner.shape.kind === "array" ? owner.shape.element : number,
    ...
    protected override expressionDomain(
        ...
        if (ts.isBinaryExpression(node)) {
            ...
            if (left?.shape.kind === "optional") {
                ...
                if (
                    this.context.unwrapExpression(node.right).kind ===
        ...
        if (
            ...
            const owner = this.value(node.expression);
            if (
                owner?.shape.kind === "array" ||
                owner?.shape.kind === "buffer"
            ) {
        ...
        if (value) {
            ...
            if (value.shape.kind === "number")
                return `static_cast<double>(${value.cpp})`;
            return value.cpp;
        }
        try {
    ...
    public override statement(
            ...
            return statement.declarationList.declarations.flatMap(
                (declaration) => {
                    const value = declaration.initializer
                        ? this.value(declaration.initializer)
                        : undefined;
                    if (
                        value &&
                        value.shape.kind !== "number" &&
                        value.shape.kind !== "boolean"
                    ) {
                        if (
                            ...
                            return this.context.contractError(
                                declaration,
                                "Expected a constant pinned CSM object alias.",
                            );
                        }
                        const cpp = `csm_ref_${declaration.getStart(this.source)}`;
                        this.bind(declaration.name, {
                            shape: value.shape,
                            cpp,
                        });
                        return [
                            `${indent}auto${value.temporary || value.shape.kind === "optional" ? "" : "&"} ${cpp} = ${value.cpp};`,
                        ];
                    }
                    const one = ts.factory.updateVariableStatement(
                        statement,
                        statement.modifiers,
        ...
        if (ts.isForOfStatement(statement)) {
            ...
            if (
                ...
                return this.context.contractError(
                    statement,
                    "Unsupported pinned CSM object iteration.",
                );
            }
...
function bindParameters(
    ...
    if (declaration.parameters.length !== parameters.length) {
        context.contractError(
            declaration,
            "Pinned CSM parameter count changed.",
        );
    }
    declaration.parameters.forEach((parameter, index) => {
        ...
        if (
            ...
            parameter.name.text !== expected.name ||
            parameter.type?.getText() !== expected.annotation
        ) {
            context.contractError(
                parameter,
                `Expected pinned CSM '${expected.name}: ${expected.annotation}'.`,
            );
        }
        lowerer.bind(parameter.name, {
...
function constant(
    ...
    const declaration = context
        .findNodes(file, ts.isVariableDeclaration)
...
function lowerTransformCoordinate(context: LoweringContext): string {
    return lowerPinnedFunction(
        ...
        [
            {
                ...
                mutableRecord: true,
                binding: { cpp: "out", type: "f64-buffer" },
            },
            { pinned: "m", kind: "numberArray", cpp: "m", cppType: "Matrix" },
            ...["x", "y", "z"].map((pinned): PinnedFunctionParameter => ({
                pinned,
                kind: "number",
                cpp: pinned,
            })),
        ...
        {
            cppName: "csm_transform_coord_into",
            inline: true,
            templateParameters: ["typename Matrix"],
            returns: "void",
        },
    );
}
...
function lowerCsmMatrixHelpers(context: LoweringContext): string {
    const scalar = (pinned: string): PinnedFunctionParameter => ({
        pinned,
        kind: "number",
        cpp: pinned,
    });
    return [
        ...
        lowerPinnedFunction(
            context,
            "src/math/invert-mat4-to-ref-or-identity.ts",
            "invertMat4ToRefOrIdentity",
            [
                {
                    pinned: "input",
                    kind: "mat4Const",
        ...
        lowerPinnedFunction(
            ...
            {
                cppName: "csm_bias_view_projection",
                inline: true,
                returns: "void",
            },
        ),
...
function assertStorageContracts(context: LoweringContext): void {
    for (const [name, fields] of [
        ...
        for (const member of declaration.members) {
            if (
                ...
                member.type?.getText() !== expected.get(member.name.text)
            ) {
                context.contractError(
                    member,
                    `Pinned CSM storage changed in ${name}.`,
                );
            }
        ...
        if (expected.size)
            context.contractError(
                declaration,
                `Missing CSM storage members in ${name}.`,
            );
    }
    ...
    for (const field of ["_view", "_invViewProj"]) {
        const initializer = context.unwrapExpression(
            context.propertyInitializer(result, field),
        );
        if (
            !ts.isNewExpression(initializer) ||
            !ts.isIdentifier(initializer.expression) ||
            initializer.expression.text !== "Float32Array" ||
            ...
            context.contractError(
                initializer,
                `Expected 16 Float32Array lanes for ${field}.`,
            );
        }
    }
    ...
    const { declaration: aspect } = context.functionDeclaration(
        modulePath,
        "csmCameraAspect",
    );
    context.assertExpressionShape(
        context.variableInitializer(aspect, "rt"),
        "scene.surface.scRT",
        "CSM aspect target",
    );
    context.expectShapeCount(
        aspect,
        "getEffectiveAspectRatio(camera, rt._width, rt._height)",
        "CSM camera aspect",
    );
}
...
function lowerThinBounds(context: LoweringContext): string {
    ...
    if (
        ...
        !hit ||
        !allocate ||
        !ts.isVariableStatement(cache) ||
        !ts.isVariableStatement(worldVersion) ||
        !ts.isVariableStatement(lookup) ||
        !ts.isIfStatement(hit) ||
        !ts.isIfStatement(allocate)
    ) {
        return context.contractError(
            declaration,
            "Expected the pinned thin-caster cache prelude.",
        );
    }
    for (const statement of [cache, worldVersion, lookup]) {
        if (statement.declarationList.declarations.length !== 1) {
            context.contractError(
                statement,
                "Unexpected work in the pinned CSM cache prelude.",
            );
        }
    }
    ...
    context.assertExpressionShape(
        cache.declarationList.declarations[0]!.initializer!,
        "_getThinCasterAabbCache()",
        "CSM cache owner",
    );
    context.assertExpressionShape(
        worldVersion.declarationList.declarations[0]!.initializer!,
        "mesh.worldMatrixVersion",
        "CSM cached world version",
    );
    context.assertExpressionShape(
        lookup.declarationList.declarations[0]!.initializer!,
        "cache.get(mesh)",
        "CSM cache lookup",
    );
    context.assertExpressionShape(
        hit.expression,
        "entry && entry._version === ti._version && entry._worldVersion === worldVersion",
        "CSM cache hit",
    );
    context.assertStatementInventory(
        hit,
        ts.isBlock(hit.thenStatement)
            ? hit.thenStatement.statements
            : [hit.thenStatement],
        "CSM cache hit",
        "only cache hits are omitted",
        ["return statement"],
    );
    context.expectShapeCount(
        hit,
        "Number.isFinite(entry._bounds[0]) ? entry : null",
        "CSM cached result",
    );
    context.assertExpressionShape(
        allocate.expression,
        "!entry",
        "CSM cache allocation",
    );
    if (
        !ts.isBlock(allocate.thenStatement) ||
        allocate.thenStatement.statements.length !== 2
    ) {
        return context.contractError(
            allocate,
            "Expected the CSM cache entry and registration.",
        );
    }
    ...
    if (
        ...
        assignment.expression.operatorToken.kind !==
        ...
        return context.contractError(
            allocate,
            "Expected the pinned CSM cache entry literal.",
        );
    }
    context.expectShapeCount(
        allocate.thenStatement.statements[1]!,
        "cache.set(mesh, entry)",
        "CSM cache registration",
    );
    const initialBounds = context.unwrapExpression(
        context.propertyInitializer(assignment.expression.right, "_bounds"),
    );
    if (
        !ts.isArrayLiteralExpression(initialBounds) ||
        initialBounds.elements.length !== 6
    ) {
        return context.contractError(
            initialBounds,
            "Expected six pinned CSM bound lanes.",
        );
    }
    const kernel = statements.slice(5).filter((statement) => {
        if (
            ...
            !["_version", "_worldVersion"].includes(
        ...
        context.assertExpressionShape(
            ...
            statement.expression.left.name.text === "_version"
    ...
    bindParameters(context, lowerer, declaration, [
        ...
        {
            name: "ti",
            annotation: 'NonNullable<Mesh["thinInstances"]>',
            cpp: "ti",
            shape: thinShape,
        },
    ]);
    lowerer.bind(assignment.expression.left, {
        shape: boundsShape,
        cpp: "entry",
    });
    scope.returnValue = (expression) =>
        expression
            ...
            : context.contractError(
                  declaration,
                  "Expected a CSM bounds result.",
              );
    const initial = initialBounds.elements
        .map((lane) => lowerer.expression(lane))
        .join(", ");
...
function lowerCasterBounds(context: LoweringContext): string {
    const { file, declaration } = context.functionDeclaration(
        modulePath,
        "_castersWorldAabbInto",
    );
    const scope = numericScope();
    const lowerer = new CsmNumericAdapter(
        context,
        file,
        scope,
        new Map([
            [
                "_thinInstanceWorldAabb",
                {
                    ...
                    cpp: (args: readonly string[]) =>
                        `csm_thin_instance_world_aabb(${args.join(", ")})`,
                },
            ],
        ]),
    );
    bindParameters(context, lowerer, declaration, [
        {
            ...
            cpp: "casters",
            shape: { kind: "array", element: casterShape },
        },
        {
            name: "scratch",
            annotation: "CsmCascadeScratch",
            cpp: "scratch",
            shape: scratchShape,
        },
    ]);
    scope.returnValue = (expression) =>
        expression
            ...
            : context.contractError(
                  declaration,
                  "Expected a CSM bounds validity result.",
              );
...
function lowerInstancePredicate(context: LoweringContext): string {
    ...
    if (
        !ts.isVariableDeclaration(magnitude) ||
        !ts.isIdentifier(magnitude.name) ||
        !ts.isVariableDeclarationList(magnitude.parent) ||
        !ts.isVariableStatement(magnitude.parent.parent)
    ) {
        return context.contractError(
            magnitude,
            "Expected the pinned instance linear-magnitude declaration.",
        );
    }
    ...
    if (!ts.isBlock(statement.parent))
        return context.contractError(
            statement,
            "Expected the pinned instance loop.",
        );
    const guard =
    ...
    if (
        ...
        return context.contractError(
            statement,
            "Expected the pinned parked-instance continue guard.",
        );
    }
    const scope = numericScope();
...
function lowerCascades(context: LoweringContext): string {
    ...
    calls.set("csmCameraAspect", (args) => {
        if (args.length !== 2)
            context.contractError(
                declaration,
                "Expected the pinned CSM aspect arguments.",
            );
        return args[0]!;
    });
    ...
    bindParameters(context, lowerer, declaration, [
        {
            name: "scene",
            annotation: "SceneContext",
            cpp: "effective_aspect",
            shape: number,
        },
        {
            name: "camera",
    ...
    if (!ts.isIdentifier(ndc.declaration.name))
        return context.contractError(
            ndc.declaration,
            "Expected the NDC binding.",
        );
    lowerer.bind(ndc.declaration.name, {
        shape: cornersShape,
        cpp: "csm_frustum_ndc",
    });
    scope.returnValue = (expression) =>
        expression
            ...
            : context.contractError(
                  declaration,
                  "Expected the CSM cascade result.",
              );
...
function lowerCasterClipBias(context: LoweringContext): string {
    ...
    scope.calls = new Map([
        ...
        [
            ...
            (args: readonly string[]) =>
                `csm_world_bias_clip_offset(${args.join(", ")})`,
        ],
    ]);
    const lowerer = new CsmNumericAdapter(context, file, scope);
    const cfg = declaration.parameters.find(
        (parameter) =>
            ts.isIdentifier(parameter.name) && parameter.name.text === "cfg",
    );
    const cascades = context
        .findNodes(declaration, ts.isVariableDeclaration)
        .find(
            (node) =>
                ts.isIdentifier(node.name) && node.name.text === "cascades",
        );
    ...
    if (
        !cfg ||
        !ts.isIdentifier(cfg.name) ||
        !cascades ||
        !ts.isIdentifier(cascades.name) ||
        !index ||
        !ts.isIdentifier(index.name)
    ) {
        return context.contractError(
            expression,
            "Expected the pinned CSM caster bias inputs.",
        );
    }
    ...
    scope.methods = new Map([
        [
            ...
            (receiver: string, args: readonly string[]) =>
                `std::fill(${receiver}.begin(), ${receiver}.end(), static_cast<float>(${args[0]}))`,
        ],
        [
            "set",
            (receiver: string, args: readonly string[]) =>
                `std::copy(${args[0]}.begin(), ${args[0]}.end(), ${receiver}.begin() + static_cast<std::size_t>(${args[1]}))`,
        ],
    ]);
    const lowerer = new CsmNumericAdapter(context, file, scope);
    bindParameters(context, lowerer, declaration, [
