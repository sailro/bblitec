...
export function lowerAnimationManagerClock(context: LoweringContext): {
    ...
    const { file, declaration: update } = context.functionDeclaration(
        module,
    ...
    const { declaration: stop } = context.functionDeclaration(
        module,
    ...
    if (!ts.isArrowFunction(tick) || !ts.isBlock(tick.body)) {
        context.contractError(
            tick,
            "Expected the manager's autonomous tick callback.",
        );
    }
    ...
    if (!created || !ts.isReturnStatement(created) || !created.expression) {
        context.contractError(
            create,
            "Expected the manager's initial state record.",
        );
    }
    ...
    context.assertStatementInventory(
        ...
        "startAnimationManager",
        "the host replaces RAF availability and registration only",
        [
            "if statement",
            "if statement",
    ...
    context.assertStatementInventory(
        ...
        "clock arithmetic, update, notification and requeue retain source order",
        [
            "if statement",
            "variable statement",
            "expression statement",
    ...
    context.assertStatementInventory(
        stop,
        stop.body!.statements,
        "stopAnimationManager",
        "cancellation precedes resetting the clock and running state",
        [
            "if statement",
            "expression statement",
            "expression statement",
    ...
    const tickExpressions = tick.body.statements.filter(
    ...
    for (const [index, shape] of [
        ...
        context.assertExpressionShape(
            tickExpressions[index]!.expression,
            shape,
            "Manager tick operation order",
        );
    }
    ...
    const scope: CppExpressionScope = {
        ...
        propertyRead: (expression) => {
            ...
            if (
                ...
                expression.expression.text !== "manager" ||
                !field
            ) {
                context.contractError(
                    expression,
                    "Animation manager clock reads an unmapped field.",
                );
            }
            return {
                text: `owner.${field}`,
        ...
        callRead: (expression) => {
            if (
                expression.expression.getText(file) !== "Number.isFinite" ||
                expression.arguments.length !== 1
            ) {
                context.contractError(
                    expression,
                    "Animation manager clock calls an unmapped operation.",
                );
            }
            return {
    ...
    const body = (
        function_: ts.FunctionDeclaration | ts.ArrowFunction,
    ): ts.Block => {
        if (!function_.body || !ts.isBlock(function_.body)) {
            context.contractError(
                function_,
                "Animation manager operation needs a block body.",
            );
        }
    ...
    const firstGuard = (
        ...
        if (
            ...
            context.contractError(
                function_,
                "Expected the animation manager's early-return guard.",
            );
        }
    ...
    const writes = (
        ...
        for (const statement of body(function_).statements) {
            ...
            if (
                ...
                left.expression.text !== "manager"
            ) {
                context.contractError(
                    left,
                    "Animation manager lifecycle writes an unmapped target.",
                );
            }
            if (left.name.text === "_rafId") {
                ...
                context.assertExpressionShape(
                    ...
                    "Animation manager RAF registration",
                );
                continue;
            }
    ...
    context.expectShapeCount(
        start,
        "updateAnimationManager(manager, deltaMs)",
        "Autonomous manager update",
    );
    context.expectShapeCount(
        start,
        "manager.onUpdate?.(step)",
        "Autonomous manager notification",
    );
    context.expectShapeCount(
        stop,
