...
export function lowerBabylonSceneData(
    ...
    const scope = (): PinnedBodyScope => {
        const bindings = new Map<string, PinnedBinding>([
            ...
            [
                "data.activeCameraID",
                {
                    cpp: 'string_or(document, "activeCameraID")',
                    type: "opaque",
                    absentCpp: 'string_or(document, "activeCameraID").empty()',
                },
            ],
            [
                "camData",
        ...
        for (const [root, cpp, fields] of [
            [
                "data",
                "document",
                ["clearColor", "ambientColor", "cameras", "lights"],
            ],
            [
                "ld",
                "ld",
                ["position", "diffuse", "specular", "intensity", "range"],
            ],
        ] as const)
            for (const field of fields) {
                bindings.set(`${root}.${field}`, {
                    cpp: `${cpp}.at(${JSON.stringify(field)})`,
                    type: "opaque",
                    absentCpp: `!babylon_json_truthy(${cpp}, ${JSON.stringify(field)})`,
                });
                bindings.set(`${root}.${field}?.length`, {
                    cpp: `babylon_json_length(${cpp}, ${JSON.stringify(field)})`,
                    type: "scalar",
                });
            }
        return {
                    ...
                    (args) => {
                        if (args.length !== 2)
                            context.contractError(
                                declaration,
                                "Expected the point-light position and intensity.",
                            );
            ...
            expression(node, lowerer) {
                ...
                if (ts.isArrayLiteralExpression(node))
                    return `std::array<double, ${node.elements.length}>{${node.elements.map((value) => lowerer.expression(value)).join(", ")}}`;
                if (ts.isObjectLiteralExpression(node)) {
                    ...
                    for (const property of node.properties) {
                        if (!ts.isPropertyAssignment(property))
                            context.contractError(
                                property,
                                "Expected clear-color channels.",
                            );
                        const name = context.propertyName(property.name);
                        if (!name || !["r", "g", "b", "a"].includes(name))
                            context.contractError(
                                property,
                                "Unrepresented clear-color channel.",
                            );
                        values.set(
                            name,
                    ...
                    return `Color4{${["r", "g", "b", "a"]
                        .map((name) => {
                            ...
                            if (!value)
                                context.contractError(
                                    node,
                                    `Missing clear-color channel '${name}'.`,
                                );
                ...
                if (ts.isCallExpression(node)) {
                    if (
                        ...
                        const callback = context.unwrapExpression(
                            node.arguments[0]!,
                        );
                        const parameter =
                    ...
                    if (
                        ...
                        callee.name.text === "parseBabylonCamera"
                    ) {
                        context.assertExpressionShape(
                            callee.expression,
                            '(await import("./parse-camera.js"))',
                            "Camera module boundary",
                        );
                        if (node.arguments.length !== 1)
                            context.contractError(
                                node,
                                "Expected camera source data.",
                            );
            ...
            statement(statement, lowerer, indent) {
                if (
                    ...
                    if (ts.isObjectBindingPattern(variable.name)) {
                        if (
                            ...
                            context.expressionMatchesShape(
                                variable.initializer,
                                'await import("../light/point-light.js")',
                            )
                        ) {
                            const imported = variable.name.elements[0];
                            if (
                                variable.name.elements.length !== 1 ||
                                !imported ||
                                !ts.isIdentifier(imported.name) ||
                                imported.name.text !== "createPointLight" ||
                                imported.propertyName ||
                                imported.dotDotDotToken
                            )
                                context.contractError(
                                    variable,
                                    "Expected the point-light factory import.",
                                );
                            return [];
                        }
                        if (
                            ...
                            return undefined;
                        return variable.name.elements
                            .map((element) => {
                                if (!ts.isIdentifier(element.name))
                                    context.contractError(
                                        element,
                                        "Expected a light mesh-ID name.",
                                    );
                    ...
                    if (
                        variable.initializer &&
                        ["pl", "camData", "camera"].includes(name)
                    ) {
                        const type =
                            name === "pl"
                                ? "LightHandle"
                                : name === "camData"
                                  ? "const Json*"
                                  : "std::optional<CameraHandle>";
                        return [
                            `${indent}${type} ${name} = ${lowerer.expression(variable.initializer)};`,
                        ];
                    }
                }
                ...
                if (
                    ...
                    return [
                        `${indent}asset.lights.push_back(${lowerer.expression(expression.arguments[0]!)});`,
                    ];
                if (
    ...
    const variableStatement = (name: string): ts.VariableStatement => {
        const variable = context.findNodes(
            declaration,
    ...
    const clearIf =
        declaration.body!.statements[
            declaration.body!.statements.indexOf(clear) + 1
        ];
    if (!clearIf || !ts.isIfStatement(clearIf))
        context.contractError(clear, "Expected the clear-color guard.");
    const clearBody = lowerPinnedBody(file, [clear, clearIf], scope());
    const ambient = lowerPinnedBody(
        file,
        [
            ts.factory.createReturnStatement(
                context.variableInitializer(declaration, "sceneAmbient"),
            ),
        ],
        {
            ...scope(),
    ...
    const lightStatement = declaration.body!.statements.find(
        (statement) =>
            ts.isIfStatement(statement) &&
            context.hasCall(statement, "createPointLight"),
    );
    if (!lightStatement)
        context.contractError(
            declaration,
