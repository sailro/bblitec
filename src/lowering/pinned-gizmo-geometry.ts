...
function objectValues(
    ...
    if (
        ...
        context.contractError(
            expression,
            `Expected pinned geometry record { ${members.join(", ")} }.`,
        );
    }
...
function numericParameters(
    ...
    if (
        declaration.parameters.length !== names.length ||
        ...
        context.contractError(
            declaration,
            `Expected pinned geometry parameters (${names.join(", ")}).`,
        );
    }
}
...
class GeometryNumericLowerer extends PinnedNumericLowerer {
    private readonly lists = new WeakMap<
        PinnedBinding,
        "record" | "edge" | "pair"
    >();
            ...
            if (shape && binding) {
                if (
                    ...
                    this.context.contractError(
                        node,
                        "Expected a pinned geometry append statement.",
                    );
                }
                const values = node.arguments.map((argument) => {
                    ...
                    if (
                        ...
                        value.expression.text === "buildFrustumEdge"
                    ) {
                        return this.expression(value);
                    }
                    if (shape === "pair") {
                    ...
                    return this.context.contractError(
                        argument,
                        `Expected a pinned '${shape}' geometry list element.`,
                    );
                });
                const calls = values.map(
                    (value) => `${binding.cpp}.push_back(${value})`,
                );
    ...
    public listResult(
        ...
        if (!binding || this.lists.get(binding) !== shape) {
            this.context.contractError(
                returned ?? at,
                `Expected the pinned '${shape}' geometry list result.`,
            );
        }
        return binding.cpp;
    }
    ...
    public override statement(
        ...
        if (
            ...
            if (
                ...
                if (
                    element &&
                    ts.isTupleTypeNode(element) &&
                    element.elements.length === 2 &&
                    element.elements.every(
                        (lane) => lane.kind === ts.SyntaxKind.NumberKeyword,
                    )
                ) {
                    const rows = value.elements.map(
                        (row) =>
                ...
                if (
                    ...
                    value.elements.every((entry) =>
                        ts.isObjectLiteralExpression(
                            this.context.unwrapExpression(entry),
                        ),
                    )
                ) {
                    const rows = value.elements.map(
                    ...
                    return [
                        `${indent}std::vector<Vec3d> ${name}{${rows.join(", ")}};`,
                    ];
                }
                this.context.contractError(
                    declared,
                    "Unsupported pinned geometry list carrier.",
                );
            }
        }
        if (
            ...
            if (ts.isArrayBindingPattern(declared.name)) {
                ...
                if (!rangeBinding || this.lists.get(rangeBinding) !== "pair") {
                    this.context.contractError(
                        statement,
                        "Expected a pinned index-pair range.",
                    );
                }
            ...
            returns: {
                ...
                value: (lowerer, expression) => {
                    ...
                    if (
                        ...
                        returned.expression.text !== "createMeshFromData" ||
                        returned.arguments.length !== 6 ||
                        !ts.isIdentifier(returned.arguments[0]!) ||
                        returned.arguments[0].text !== "engine" ||
                        !ts.isStringLiteral(returned.arguments[1]!)
                    ) {
                        context.contractError(
                            declaration,
                            "Expected the pinned hemisphere to return its mesh-data factory call.",
                        );
                    }
                    return `GizmoHemisphereGeometry{${JSON.stringify(returned.arguments[1].text)}, ${returned.arguments
                        .slice(2)
...
function lowerLineDefinitions(context: LoweringContext): string {
    ...
    const { declaration: shape } = context.interfaceDeclaration(
        LIGHT,
        "LineDef",
    );
    if (
        shape.members.length !== LINE_MEMBERS.length ||
        shape.members.some(
            (member, index) =>
        ...
        context.contractError(
            shape,
            "Expected the pinned numeric LineDef record.",
        );
    }
    ...
    scope.returnValue = (expression) =>
        lowerer.listResult(expression, "record", declaration);
...
function lowerFrustumEdge(context: LoweringContext): string {
    ...
    if (
        ...
        context.contractError(
            declaration,
            "Expected the pinned frustum-edge parameter order.",
        );
    }
    if (
        declaration.parameters[4]!.type?.kind !== ts.SyntaxKind.NumberKeyword ||
        declaration.parameters.slice(5).some((parameter) => {
            const type = parameter.type;
            return (
                !type ||
                !ts.isTypeLiteralNode(type) ||
                type.members.length !== 3 ||
                type.members.some(
                    (member, index) =>
                        !ts.isPropertySignature(member) ||
                        !member.name ||
        ...
        context.contractError(
            declaration,
            "Expected a numeric thickness and two pinned point records.",
        );
    }
    const scope = numericScope([
        ...
        ["a", { cpp: "a", type: "vec3" }],
        ["b", { cpp: "b", type: "vec3" }],
        ...["position", "scaling", "rotationQuaternion"].map(
            (channel): [string, PinnedBinding] => [
                `mesh.${channel}`,
                { cpp: channel, type: "scalar" },
            ],
        ),
    ]);
        ...
        [
            ...
            (receiver: string, args: readonly string[]): string => {
                ...
                if (
                    ...
                    context.contractError(
                        declaration,
                        "Unsupported pinned frustum transform write.",
                    );
                }
    ...
    const record = (name: string, at: ts.Node): void => {
        if (seen.has(name))
            context.contractError(
                at,
                `Duplicate pinned frustum '${name}' boundary.`,
            );
        seen.add(name);
    };
    const body: string[] = [];
    for (const statement of declaration.body!.statements) {
        if (
            ...
            if (ts.isIdentifier(mesh.name) && mesh.name.text === "mesh") {
                const call = mesh.initializer
                    ? context.unwrapExpression(mesh.initializer)
                    : undefined;
                if (
                    !call ||
                    !ts.isCallExpression(call) ||
                    !ts.isIdentifier(call.expression) ||
                    call.expression.text !== "createCylinder" ||
                    call.arguments.length !== 2 ||
                    !ts.isIdentifier(call.arguments[0]!) ||
                    call.arguments[0].text !== "engine"
                ) {
                    context.contractError(
                        mesh,
                        "Expected a pinned frustum cylinder factory.",
                    );
                }
                const options = objectValues(
                    context,
                    lowerer,
                    call.arguments[1]!,
                    CYLINDER_MEMBERS,
                ...
                body.push(
                    ...CYLINDER_MEMBERS.map(
                        (name, index) =>
                            `    edge.${name} = ${options[index]};`,
                    ),
                );
        ...
        if (ts.isExpressionStatement(statement)) {
            ...
            if (
                ...
                expression.left.expression.text === "mesh"
            ) {
                ...
                context.contractError(
                    expression,
                    "Unsupported pinned frustum mesh attachment.",
                );
            }
            if (
                ...
                if (
                    ...
                    expression.arguments.some(
                        (argument, index) =>
                            ...
                            argument.text !== ["utilityScene", "mesh"][index],
                    )
                ) {
                    context.contractError(
                        expression,
                        "Expected pinned frustum registration in its utility scene.",
                    );
                }
        ...
        if (ts.isReturnStatement(statement)) {
            if (
                ...
                statement.expression.text !== "mesh"
            ) {
                context.contractError(
                    statement,
                    "Expected the pinned frustum edge mesh result.",
                );
            }
    ...
    if (seen.size !== 6) {
        context.contractError(
            declaration,
            "Expected the pinned frustum cylinder, attachment, registration and return.",
        );
    }
...
function lowerFrustum(context: LoweringContext): string {
    ...
    if (
        ...
        context.contractError(
            declaration,
            "Expected the pinned frustum geometry parameter order.",
        );
    }
    const scope = numericScope(
        parameters.map((name): [string, PinnedBinding] => [
            name,
    ...
    scope.calls = new Map([
        ...
        [
            ...
            (args: readonly string[]): string => {
                if (
                    args.length !== 7 ||
                    resources.some((name, index) => args[index] !== name)
                ) {
                    context.contractError(
                        declaration,
                        "Expected the pinned frustum to attach each edge to the same resources.",
                    );
                }
    ...
    const lowerer = new GeometryNumericLowerer(context, file, scope);
    scope.returnValue = (expression) =>
        lowerer.listResult(expression, "edge", declaration);
...
function block(context: LoweringContext, node: ts.Node): ts.Block {
    if (!ts.isBlock(node))
        context.contractError(node, "Expected a pinned geometry block.");
    return node;
}
...
function containingIf(
    ...
    const call = context.findNodes(
        ...
        (node): node is ts.CallExpression =>
            ts.isCallExpression(node) &&
            context.propertyPath(node.expression)?.join(".") === callee,
    )[0];
    if (!call) context.contractError(scope, `Expected pinned '${callee}'.`);
    let parent: ts.Node | undefined = call.parent;
    while (parent && parent !== scope && !ts.isIfStatement(parent))
        parent = parent.parent;
    if (!parent || !ts.isIfStatement(parent)) {
        context.contractError(
            call,
            "Expected a guarded pinned geometry write.",
        );
    }
    return parent;
}
...
function scaleBody(
    ...
    const worldLoads = statements.flatMap((statement) =>
        context.findNodes(
            statement,
    ...
    if (
        worldLoads.length !== 1 ||
        !worldLoads[0]!.initializer ||
        context
            .propertyPath(context.unwrapExpression(worldLoads[0]!.initializer))
            ?.join(".") !== worldPath.join(".")
    ) {
        context.contractError(
            statements[0]!,
            "Expected the pinned follow to read its utility camera's world matrix.",
        );
    }
    const scope = numericScope([
        ...bindings,
    ...
    scope.methods = new Map([
        [
            "set",
            (target: string, args: readonly string[]): string => {
                if (target !== "result" || args.length !== 3) {
                    context.contractError(
                        statements[0]!,
                        "Unsupported pinned gizmo scaling write.",
                    );
                }
...
function cameraGizmoFollow(context: LoweringContext): string {
    ...
    const afterFollow = callback(
        context,
        context.callExpression(camera, "attachFollowTarget"),
        4,
    );
    const bodyOuter = containingIf(
        context,
        afterFollow,
        "gizmo._bodyOuter.scaling.set",
    );
    const cameraBody = scaleBody(
        context,
        CAMERA,
        block(context, bodyOuter.thenStatement).statements,
        [
            ["cam", { cpp: "has_camera", type: "bool" }],
            ["wm", { cpp: "wm", type: "f32" }],
        ],
        "gizmo._bodyOuter.scaling",
        ["cam", "worldMatrix"],
    );
    if (
        context
            .propertyPath(
                context.unwrapExpression(
                    context.variableInitializer(bodyOuter, "cam"),
                ),
            )
            ?.join(".") !== "utilityScene.camera"
    ) {
        context.contractError(
            bodyOuter,
...
function lightGizmoFollow(context: LoweringContext): string {
    ...
    const lightFollow = callback(
        context,
        context.callExpression(light, "onBeforeRender"),
        1,
    );
    const cameraScale = containingIf(context, lightFollow, "root.scaling.set");
    const lightBody = scaleBody(
        context,
        LIGHT,
        [cameraScale],
        [
    ...
    if (
        context
            .propertyPath(
                context.unwrapExpression(
                    context.variableInitializer(lightFollow, "camera"),
                ),
            )
            ?.join(".") !== "utilityScene.camera"
    ) {
...
function boundsValue(
    ...
    if (
        !ts.isObjectLiteralExpression(literal) ||
        literal.properties.length !== 4
    ) {
        context.contractError(
            literal,
            "Expected the pinned AabbBounds record.",
        );
    }
...
export function pinnedGizmoBoundsGeometry(context: LoweringContext): string {
    ...
    for (const statement of initialStatements) {
        if (!ts.isVariableStatement(statement)) {
            context.contractError(
                statement,
                "Expected pinned min/max bound initializers.",
            );
        }
        for (const declared of statement.declarationList.declarations) {
            const target = ts.isIdentifier(declared.name)
                ? bindings.get(declared.name.text)
                : undefined;
            if (!target || !declared.initializer) {
                context.contractError(
                    declared,
                    "Expected a pinned scalar bound initializer.",
                );
            }
            initialization.push(
                `    ${target.cpp} = ${initial.expression(declared.initializer)};`,
            );
        }
    }
    if (initialization.length !== bindings.size) {
        context.contractError(
            declaration,
            "Expected all six pinned bounds to be initialized.",
        );
    }
    const visitor = context.unwrapExpression(
        context.variableInitializer(declaration, "visit"),
    );
    if (!ts.isArrowFunction(visitor))
        context.contractError(visitor, "Expected the pinned bounds visitor.");
    const finite = context.callExpression(visitor, "isFinite");
    if (
        context.propertyPath(finite.expression)?.join(".") !== "Number.isFinite"
    ) {
        context.contractError(
            finite,
            "Expected the pinned Number.isFinite bounds guard.",
        );
    }
    ...
    if (!ts.isIfStatement(guardedFold) || guardedFold.expression !== finite) {
        context.contractError(
            finite,
            "Expected the pinned finite-AABB fold guard.",
        );
    }
    ...
    foldScope.calls = new Map([
        [
            "Number.isFinite",
            (args: readonly string[]) => `std::isfinite(${args.join(", ")})`,
        ],
    ]);
    ...
    finishScope.returnValue = (expression) =>
        expression
            ? boundsValue(context, finish, expression)
            : context.contractError(
                  declaration,
                  "Expected a pinned bounds result.",
              );
    const candidates = declaration.body!.statements.filter(
    ...
    if (candidates.length !== 1) {
        context.contractError(
            declaration,
            "Expected the pinned supplemental candidate traversal.",
        );
    }
    ...
    if (
        suffix.length === 0 ||
        !ts.isReturnStatement(suffix[suffix.length - 1]!)
    ) {
        context.contractError(
            declaration,
            "Expected the pinned bounds result after traversal.",
        );
    }
    const loweredRegions = new Set<ts.Node>([
        ...initialStatements,
        guardedFold,
        ...suffix,
    ]);
    const outsideProjection = context.findNodes(
        declaration,
    ...
    if (outsideProjection.length > 0) {
        context.contractError(
            outsideProjection[0]!,
            "Pinned bounds arithmetic escaped the numeric projection.",
        );
    }
