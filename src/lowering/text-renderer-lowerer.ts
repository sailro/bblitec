...
${lowerPinnedBody(file, declaration.body!.statements, {
    bindings: new Map([
        ...this.layerBindings(),
        ["targetW", scalar("width")],
        ["targetH", scalar("height")],
        ["out", { cpp: "out", type: "f32", mutable: true }],
    ]),
    calls: new Map([
        ...pinnedNumericMathCalls(),
        ["out.fill", (args) => `out.fill(static_cast<float>(${args[0]}))`],
    ]),
})}
}`;
    }
                ...
                (name) =>
                    [
                        ...
                        scalar(
                            native +
                                {
                                    instanceCap: "instance_capacity",
                                    uploadedDataVersion:
                                        "uploaded_data_version",
                                    uploadedStyleVersion:
                                        "uploaded_style_version",
                                    uploadedViewportW: "uploaded_viewport_w",
                                    uploadedViewportH: "uploaded_viewport_h",
                                    bundleLayoutVersion:
                                        "bundle_layout_version",
                                    bundleDrawCalls: "bundle_draw_calls",
                                }[name],
                        ),
                    ] as const,
            ),
            [`${root}._pipeline`, opaque(`${native}pipeline`)],
            [`${root}._variantPipeline`, opaque(`${native}variant_pipeline`)],
            [
                `${root}._renderBundle`,
                {
                    ...opaque(`${native}render_bundle`),
                    absentCpp: `!${native}render_bundle`,
                },
            ],
        ]);
    }
    ...
    private factories(): string {
        ...
        c.assertFunctionBodyShape(
            ...
            return rr;}`,
            "Text renderer surface and layer ownership",
        );
        c.assertFunctionBodyShape(
            c.functionDeclaration(module, "registerTextRenderer").declaration,
...
inline std::shared_ptr<TextLayerGpuState> find_text_layer_gpu(const TextRendererState& renderer,const TextLayer& layer) {
    ...
    private ensureGpu(): string {
        ...
        const constants = new Map<string, PinnedBinding>([
            [
                ...
                scalar(
                    String(
                        c.numericValue(
                            c.variableInitializer(file, "TEXT_UBO_BYTES"),
                            file,
                        ),
                    ),
                ),
        ...
        const fields: Record<string, string> = {
            _uploadedStyleVersion: "uploaded_style_version",
            _instanceCap: "instance_capacity",
            _uploadedDataVersion: "uploaded_data_version",
            _uploadedViewportW: "uploaded_viewport_w",
            _uploadedViewportH: "uploaded_viewport_h",
            _mvpUploaded: "mvp_uploaded",
            _bundleLayoutVersion: "bundle_layout_version",
            _bundleDrawCalls: "bundle_draw_calls",
        };
        const bindings = new Map<string, PinnedBinding>([
            ...constants,
            ...this.gpuBindings("lg", "lg->"),
            ["lg", { ...opaque("lg"), absentCpp: "!lg" }],
            ["layer", opaque("layer")],
            [
                "layer.data._instanceCount",
                scalar("static_cast<double>(layer->data->instance_count)"),
            ],
        ]);
...
${lowerPinnedBody(file, declaration.body!.statements, {
    ...
    calls: new Map([
        ...pinnedNumericMathCalls(),
        ["rr._layerGpu.set", () => "rr.layer_gpu[layer]=lg"],
    ]),
    ...
    statement: (node, lowerer, indent) => {
        if (
            ...
            const aliases: Record<string, readonly [string, string]> = {
                lg: [
                    "rr._layerGpu.get(layer)",
                    "auto lg=find_text_layer_gpu(rr,layer);",
                ],
                engine: ["rr._surface.engine", ""],
        ...
        if (
            ...
            c.contractError(
                object,
                "Text layer GPU factory assignment changed.",
            );
        const expectedFields = [
            ...
            "_instanceCap",
            "_styleBuf",
            "_uploadedStyleVersion",
            "_pipeline",
            "_variantPipeline",
            "_bindGroupCache",
            "_uploadedDataVersion",
            "_uploadedViewportW",
            "_uploadedViewportH",
            "_lastMvpInputs",
            "_mvpUploaded",
            "_renderBundle",
            "_bundleLayoutVersion",
            "_bundleDrawCalls",
        ];
        if (
            object.properties
                .map((property) => property.name?.getText())
                .join() !== expectedFields.join()
        )
            c.contractError(
                object,
                "Text layer GPU fields or initialization order changed.",
            );
        const lines = [
            `${indent}lg=std::make_shared<TextLayerGpuState>();`,
            `${indent}lg->device_identity=device_identity;`,
        ];
        for (const property of object.properties) {
            if (!ts.isPropertyAssignment(property))
                c.contractError(
                    property,
                    "Text layer GPU descriptor property changed.",
                );
            const name = property.name.getText(),
                value = property.initializer;
            if (fields[name]) {
                lines.push(
                    `${indent}lg->${fields[name]}=${lowerer.expression(value)};`,
                );
                continue;
            }
            if (name === "_layer") {
                c.assertExpressionShape(value, "layer", "Text GPU layer owner");
                lines.push(`${indent}lg->layer=layer;`);
                continue;
            }
            if (
                ["_pipeline", "_variantPipeline", "_renderBundle"].includes(
                    name,
                )
            ) {
                c.assertExpressionShape(
                    value,
                    "null",
                    "Text layer initial GPU cache",
                );
                continue;
            }
            if (name === "_bindGroupCache") {
                c.assertExpressionShape(
                    value,
                    "[]",
                    "Text layer initial group cache",
                );
                continue;
            }
            if (name === "_lastMvpInputs") {
                c.assertExpressionShape(
                    value,
                    "new Float32Array(6)",
                    "Text layer MVP cache fields",
                );
                continue;
            }
            if (name === "_textU") {
                c.assertExpressionShape(
                    value,
                    'createEmptyUniformBuffer(engine,TEXT_UBO_BYTES,"text-layer-ubo")',
                    "Text layer uniform allocation",
                );
                lines.push(
                    `${indent}ops.create_renderable_buffer(*lg,TextBufferKind::uniform,text_resource_size(${constants.get("TEXT_UBO_BYTES")!.cpp}));`,
                );
                continue;
            }
            if (name === "_styleBuf") {
                c.assertExpressionShape(
                    value,
                    "createStyleBuffer(device,1)",
                    "Text layer initial style palette",
                );
                lines.push(`${indent}create_text_style_buffer(*lg,1,ops);`);
                continue;
            }
            if (name === "_instanceBuf") {
                c.assertExpressionShape(
                    value,
                    'device.createBuffer({label:"text-layer-instances",size:cap*TEXT_INSTANCE_BYTES,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST})',
                    "Text layer initial instance buffer",
                );
                if (
                    ...
                    c.contractError(
                        value,
                        "Text layer buffer descriptor changed.",
                    );
                lines.push(
                    `${indent}ops.create_renderable_buffer(*lg,TextBufferKind::instances,text_resource_size(${lowerer.expression(c.propertyInitializer(value.arguments[0], "size"))}));`,
                );
                continue;
            }
...
${lowerPinnedBody(file, declaration.body!.statements, {
    ...
    statement: (node, lowerer, indent) => {
        ...
        if (c.propertyPath(assignment.left)?.join(".") === "lg._renderBundle") {
            c.assertExpressionShape(
                ...
                "Text layer capacity bundle invalidation",
            );
        ...
        if (c.propertyPath(assignment.left)?.join(".") === "lg._instanceBuf") {
            c.assertExpressionShape(
                assignment.right,
                `device.createBuffer({label:"text-layer-instances",size:cap*TEXT_INSTANCE_BYTES,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST})`,
                "Text layer instance allocation boundary",
            );
            const descriptor = assignment.right;
            if (
                !ts.isCallExpression(descriptor) ||
                !descriptor.arguments[0] ||
                !ts.isObjectLiteralExpression(descriptor.arguments[0])
            )
                c.contractError(
                    descriptor,
                    "Text layer instance descriptor changed.",
                );
            const object = descriptor.arguments[0];
            return [
                `${indent}ops.create_renderable_buffer(gpu,TextBufferKind::instances,text_resource_size(${lowerer.expression(c.propertyInitializer(object, "size"))}));`,
            ];
        }
        return undefined;
    },
    ...
    private uniforms(): string {
        ...
        const boundary = statements.findIndex(
            (s) =>
                ts.isVariableStatement(s) &&
                s.declarationList.declarations[0]?.name.getText() === "W",
        );
        if (boundary < 0)
            c.contractError(
                declaration,
                "Standalone text uniform boundary changed.",
            );
        const bindings = new Map<string, PinnedBinding>([
            ...this.layerBindings(),
            ["layer", opaque("layer")],
            ["rr._targetWidth", scalar("width")],
            ["rr._targetHeight", scalar("height")],
            [
                "lg._lastMvpInputs",
                { cpp: "gpu.last_mvp_inputs", type: "f32", mutable: true },
            ],
            ["lg._mvpUploaded", { cpp: "gpu.mvp_uploaded", type: "bool" }],
            ["lg._uploadedViewportW", scalar("gpu.uploaded_viewport_w")],
            ["lg._uploadedViewportH", scalar("gpu.uploaded_viewport_h")],
        ...
        const statement: NonNullable<PinnedNumericScope["statement"]> = (
            ...
            if (
                ...
                if (
                    ...
                    variable.initializer.expression.text === "Float32Array"
                ) {
                    ...
                    if (
                        ...
                        c.contractError(
                            variable,
                            "Standalone text uniform typed-array literal changed.",
                        );
                    ...
                    if (values.length !== 4)
                        c.contractError(
                            variable,
                            "Text viewport/color uniforms require the pinned four-float span.",
                        );
                    bindings.set(name, { cpp: name, type: "f32" });
                    return [
                        `${indent}const std::array<float,${values.length}> ${name}{${values.map((value) => `static_cast<float>(${lowerer.expression(value)})`).join(",")}};`,
                    ];
                }
            }
            if (
                ...
                if (
                    c.propertyPath(call.expression)?.join(".") ===
                    "device.queue.writeBuffer"
                ) {
                    if (call.arguments.length !== 5)
                        c.contractError(
                            call,
                            "Standalone text uniform upload arity changed.",
                        );
                    c.assertExpressionShape(
                        call.arguments[0]!,
                        "lg._textU",
                        "Standalone text uniform owner",
                    );
                    ...
                    if (
                        ...
                        c.contractError(
                            source,
                            "Standalone text uniform source array changed.",
                        );
                    const name = source.expression.text;
                    if (!["_mvpScratch", "vp", "col"].includes(name))
                        c.contractError(
                            source,
                            "Unknown standalone text uniform array.",
                        );
                    c.assertExpressionShape(
                        call.arguments[3]!,
                        `${name}.byteOffset`,
                        "Standalone text uniform byte offset",
                    );
                    c.assertExpressionShape(
                        call.arguments[4]!,
                        name === "_mvpScratch" ? "64" : "16",
                        "Standalone text uniform byte count",
                    );
                    return [
                        `${indent}write(text_resource_size(${lowerer.expression(call.arguments[1]!)}), text_layer_bytes(${name === "_mvpScratch" ? "mvp" : name}));`,
                    ];
                }
            }
            return undefined;
...
${lowerPinnedBody(file, statements.slice(boundary), {
    bindings,
    statement,
    booleanAnd: true,
    booleanOr: true,
    calls: new Map([
        ...pinnedNumericMathCalls(),
        ["buildLayerMvp", (args) => `build_text_layer_mvp(${args.join(",")})`],
    ]),
})}
}`;
    }
    ...
    private resources(): string {
        ...
        if (boundary < 0)
            c.contractError(
                declaration,
                "Standalone text resource/uniform boundary changed.",
            );
        const dataFile = c.sourceFile("src/text/text-data.ts");
        const bindings = new Map<string, PinnedBinding>([
            ...this.gpuBindings(),
            ["lg", opaque("gpu")],
            ["data", opaque("data")],
            ["device", opaque("gpu.device_identity")],
            ["bindGroupLayout", opaque("layout")],
            ["rebuilt", { cpp: "atlas_result.rebuilt", type: "bool" }],
            ...
            [
                "data._instanceCount",
                scalar("static_cast<double>(data.instance_count)"),
            ],
            ["data._version", scalar("data.version")],
            [
                "data._dirtyStart",
                scalar("static_cast<double>(data.dirty_start)"),
            ],
            ["data._dirtyEnd", scalar("static_cast<double>(data.dirty_end)")],
            [
                "TEXT_INSTANCE_BYTES",
                scalar(
                    String(
...
${lowerPinnedBody(file, statements.slice(0, boundary), {
    bindings,
    booleanAnd: true,
    booleanOr: true,
    calls: new Map([
        ...
        [
            ...
            (args: readonly string[]) =>
                `ensure_text_layer_capacity(gpu,${args[2]},ops)`,
        ],
    ]),
    statement: (node, lowerer, indent) => {
        if (
            ...
            if (ts.isObjectBindingPattern(variable.name)) {
                c.assertExpressionShape(
                    variable.initializer!,
                    "ensureSharedAtlasGpu(device,g._curveSet._atlas)",
                    "Standalone text atlas owner",
                );
                if (
                    variable.name.elements
                        .map(
                            (e) =>
                                `${e.propertyName?.getText()}:${e.name.getText()}`,
                        )
                        .join() !== "_rebuilt:rebuilt,_gpu:atlasGpu"
                )
                    c.contractError(
                        variable,
            ...
            const aliases: Record<string, readonly [string, string]> = {
                device: ["rr._surface.engine._device", ""],
                layer: ["lg._layer", "const auto& layer=*gpu.layer;"],
                data: ["layer.data", "auto& data=*layer.data;"],
                g: [
                    "data._groups[i]!",
                    "const auto& g=data.groups.at(static_cast<std::size_t>(i));",
                ],
                cached: [
                    "lg._bindGroupCache[i]",
                    "const auto cached=static_cast<std::size_t>(i)<gpu.bind_group_cache.size()?gpu.bind_group_cache[static_cast<std::size_t>(i)]:nullptr;",
                ],
            };
            const alias = aliases[name];
            if (alias) {
            ...
            if (name === "styleRecreated")
                c.assertExpressionShape(
                    initial,
                    "ensureStyleGpu(device,data,lg)",
                    "Standalone text style synchronization",
                );
            if (name === "view") {
                if (
                    ...
                    c.contractError(
                        initial,
                        "Standalone text instance view changed.",
                    );
                c.assertExpressionShape(
                    initial.expression,
                    "data._instances.subarray",
                    "Standalone text instance view owner",
                );
        ...
        if (ts.isBinaryExpression(expression)) {
            ...
            if (path === "lg._renderBundle") {
                c.assertExpressionShape(
                    expression,
                    "lg._renderBundle=null",
                    "Standalone text bundle invalidation",
                );
                return [`${indent}gpu.render_bundle.reset();`];
            }
            if (path === "lg._bindGroupCache.length") {
                c.assertExpressionShape(
                    ...
                    "Standalone text group cache truncation",
                );
                return [
                    `${indent}gpu.bind_group_cache.resize(data.groups.size());`,
                ];
            }
            if (path === "data._dirtyStart" || path === "data._dirtyEnd") {
                c.assertExpressionShape(
                    expression,
                    `${path}=0`,
                    "Standalone text dirty reset",
                );
                return [
                    `${indent}data.${path === "data._dirtyStart" ? "dirty_start" : "dirty_end"}=0;`,
                ];
            }
            if (
                ts.isElementAccessExpression(expression.left) &&
                c.propertyPath(expression.left.expression)?.join(".") ===
                    "lg._bindGroupCache"
            ) {
                c.assertExpressionShape(
                    expression,
                    `lg._bindGroupCache[i]={_bindGroup:device.createBindGroup({label:"text-renderer-bg0-"+g._curveSetId,layout:bindGroupLayout,entries:[
        ...
        if (
            ...
            c.propertyPath(expression.expression)?.join(".") ===
                "device.queue.writeBuffer"
        ) {
            if (expression.arguments.length !== 5)
                c.contractError(
                    expression,
                    "Standalone text instance upload arity changed.",
                );
            for (const [index, shape] of [
                [0, "lg._instanceBuf"],
                [2, "view.buffer as ArrayBuffer"],
                [3, "view.byteOffset"],
                [4, "view.byteLength"],
            ] as const)
                c.assertExpressionShape(
                    ...
                    shape,
                    "Standalone text instance upload range",
                );
            return [
                `${indent}ops.write_renderable_buffer(gpu,TextBufferKind::instances,text_resource_size(${lowerer.expression(expression.arguments[1]!)}),view);`,
            ];
        }
        return undefined;
    },
...
${lowerPinnedBody(compare.file, compare.declaration.body!.statements, {
    bindings: new Map([
        ["a.order", scalar("a->order")],
        ["b.order", scalar("b->order")],
    ]),
    calls: new Map(),
    returnValue: (e, comparison) => (e ? comparison.expression(e) : ""),
})}
}
...
${lowerPinnedBody(file, declaration.body!.statements, {
    bindings: new Map<string, PinnedBinding>([
        ...
        ["rr._targetWidth", scalar("rr.target_width")],
        ["rr._targetHeight", scalar("rr.target_height")],
        ["size.width", scalar("width")],
        ["size.height", scalar("height")],
        ["rr._layers.length", scalar("static_cast<double>(rr.layers.size())")],
        ...this.gpuBindings("lg", "lg->"),
        ["pipeline", opaque("pipeline")],
        ["variantPipeline", opaque("variantPipeline")],
        ["rr", opaque("rr")],
        ["lg", opaque("lg")],
        ["cache._bindGroupLayout", opaque("layout")],
    ...
    calls: new Map([
        [
            "rr._layers.sort",
            () =>
                "std::stable_sort(rr.layers.begin(),rr.layers.end(),[](const auto& a,const auto& b){return compare_text_layers(a,b)<0;})",
        ],
        [
            "uploadLayer",
            () =>
                "upload_text_layer_resources(*lg,layout,ops); upload_text_layer_uniforms(*layer,*lg,rr.target_width,rr.target_height,[&](std::size_t offset,std::span<const std::uint8_t> bytes){ops.write_renderable_buffer(*lg,TextBufferKind::uniform,offset,bytes);})",
        ],
    ]),
    forOf: (iterated, element) =>
        iterated === "rr._layers" && element === "layer"
            ? {
                  range: "rr.layers",
                  bindings: new Map([
                      ...this.layerBindings("layer", "layer->"),
                      ["layer", opaque("layer")],
                  ]),
              }
            : undefined,
    statement: (node, _lowerer, indent) => {
        if (
            ...
            if (ts.isObjectBindingPattern(variable.name)) {
                c.assertExpressionShape(
                    variable.initializer!,
                    "getOrCreateTextPipeline(rr._surface.engine,rr._surface.format,1,null,false)",
                    "Standalone text pipeline signature",
                );
                if (
                    variable.name.elements
                        .map(
                        ...
                        .join() !==
                    "_pipeline:pipeline,_variantPipeline:variantPipeline,_cache:cache"
                )
                    c.contractError(
                        variable,
                        "Standalone text pipeline result changed.",
                    );
                return [
                    `${indent}const auto resolved=ops.resolve_text_renderer_pipeline();`,
                    `${indent}const auto& pipeline=resolved.pipeline;`,
                    `${indent}const auto& variantPipeline=resolved.variant_pipeline;`,
                    `${indent}const auto& layout=resolved.layout;`,
                ];
            }
            const name = variable.name.getText();
            if (name === "size") {
                c.assertExpressionShape(
                    variable.initializer!,
                    "rr._surface.canvas",
                    "Standalone text target dimensions",
                );
                return [];
            }
            if (name === "lg") {
                c.assertExpressionShape(
                    variable.initializer!,
                    "ensureLayerGpu(rr,layer)",
                    "Standalone text layer GPU owner",
                );
                return [
                    `${indent}const auto lg=ensure_text_layer_gpu(rr,layer,device_identity,ops);`,
                ];
            }
        }
        if (ts.isExpressionStatement(node)) {
            ...
            if (
                ...
                c.assertExpressionShape(
                    ...
                    "Standalone text stable ordering",
                );
            if (
                ts.isCallExpression(expression) &&
                c.propertyPath(expression.expression)?.join(".") ===
            ...
            if (ts.isBinaryExpression(expression)) {
                ...
                if (path === "lg._bindGroupCache.length") {
                    c.assertExpressionShape(
                        ...
                        "Text pipeline group-cache invalidation",
                    );
                    return [`${indent}lg->bind_group_cache.clear();`];
                }
                if (path === "lg._renderBundle") {
                    c.assertExpressionShape(
                        expression,
                        "lg._renderBundle=null",
                        "Text pipeline bundle invalidation",
                    );
                    return [`${indent}lg->render_bundle.reset();`];
                }
            }
        }
        return undefined;
    ...
    private record(): string {
        ...
        const bindings = new Map<string, PinnedBinding>([
            ...
            [
                "lg._pipeline",
                { ...opaque("lg->pipeline"), absentCpp: "!lg->pipeline" },
            ],
            [
                "data._instanceCount",
                scalar("static_cast<double>(data.instance_count)"),
            ...
            ["bound", opaque("bound")],
            ["p", opaque("p")],
            ["quadVertex", opaque("quadVertex")],
            ["bg", { ...opaque("bg"), absentCpp: "!bg" }],
            [
                "visibleBundles.length",
                scalar("static_cast<double>(rr.visible_bundles.size())"),
            ],
...
${lowerPinnedBody(file, declaration.body!.statements, {
    bindings,
    booleanOr: true,
    returnValue: (e, lowerer) => (e ? lowerer.expression(e) : ""),
    expression: (node) => {
        if (
            ts.isBinaryExpression(node) &&
            node.operatorToken.kind === ts.SyntaxKind.EqualsEqualsToken &&
            c.propertyPath(node.left)?.join(".") === "lg._renderBundle"
        ) {
            c.assertExpressionShape(
                node,
                "lg._renderBundle==null",
                "Text bundle absence check",
            );
            return "!lg->render_bundle";
        }
        return undefined;
    },
    calls: new Map([
        [
            "visibleBundles.push",
            (args: readonly string[]) =>
                `rr.visible_bundles.push_back(${args[0]})`,
        ],
        [
            "pass.executeBundles",
            () => "replay_text_bundles(rr.visible_bundles,ops)",
        ],
        ["pass.end", () => "ops.end_text_renderer_pass()"],
    ]),
    forOf: (iterated, element) =>
        iterated === "rr._layers" && element === "layer"
            ? {
                  range: "rr.layers",
                  bindings: this.layerBindings("layer", "layer->"),
              }
            : undefined,
    statement: (node, lowerer, indent) => {
        if (
            ...
            const name = variable.name.getText();
            const aliases: Record<string, readonly [string, string]> = {
                eng: ["rr._surface.engine", ""],
                device: ["eng._device", ""],
                encoder: ["eng._currentEncoder", ""],
                swapView: ["rr._surface.scRT._colorView!", ""],
                format: ["rr._surface.format", ""],
                quadVertex: [
                    "getTextPipelineCache(eng)._quadVertexBuffer",
                    "const auto quadVertex=ops.text_renderer_quad();",
                ],
                visibleBundles: ["rr._visibleBundles", ""],
                lg: [
                    "rr._layerGpu.get(layer)",
                    "const auto lg=find_text_layer_gpu(rr,layer);",
                ],
                data: ["layer.data", "const auto& data=*layer->data;"],
                g: [
                    "data._groups[i]!",
                    "const auto& g=data.groups.at(static_cast<std::size_t>(i));",
                ],
                bg: [
                    "lg._bindGroupCache[i]?._bindGroup",
                    "const auto bg=static_cast<std::size_t>(i)<lg->bind_group_cache.size()&&lg->bind_group_cache[static_cast<std::size_t>(i)]?lg->bind_group_cache[static_cast<std::size_t>(i)]->group:nullptr;",
                ],
                be: [
                    "device.createRenderBundleEncoder({colorFormats:[format],sampleCount:1})",
                    "auto be=std::make_shared<TextCommandBundle>();",
                ],
                pass: [
                    'encoder.beginRenderPass({colorAttachments:[{view:swapView,clearValue:rr.clearColor,loadOp:rr._clear?"clear":"load",storeOp:"store"}]})',
                    "ops.begin_text_renderer_pass(rr);",
                ],
            };
            const alias = aliases[name];
            if (alias) {
                c.assertExpressionShape(
                    variable.initializer!,
                    alias[0],
                    `Standalone text record ${name} boundary`,
                );
                return alias[1] ? [`${indent}${alias[1]}`] : [];
            }
            if (["base", "bound", "p"].includes(name))
                return [
        ...
        if (ts.isBinaryExpression(expression)) {
            const path = c.propertyPath(expression.left)?.join(".");
            if (path === "visibleBundles.length") {
                c.assertExpressionShape(
                    expression,
                    "visibleBundles.length=0",
                    "Text visible bundle reset",
                );
                return [`${indent}rr.visible_bundles.clear();`];
            }
            if (path === "lg._renderBundle") {
                c.assertExpressionShape(
                    expression,
                    "lg._renderBundle=be.finish()",
                    "Text bundle retention",
                );
                return [`${indent}lg->render_bundle=be;`];
            }
        }
        ...
        if (path === "be.setPipeline") {
            if (expression.arguments.length !== 1)
                c.contractError(
                    expression,
                    "Text bundle pipeline arity changed.",
                );
            return [
                `${indent}be->commands.push_back({TextBundleOp::pipeline,${lowerer.expression(expression.arguments[0]!)}});`,
            ];
        }
        if (path === "be.setVertexBuffer") {
            const slot = c.numericValue(expression.arguments[0]!, file);
            if (slot !== 0 && slot !== 1)
                c.contractError(expression, "Text bundle vertex slot changed.");
            c.assertExpressionShape(
                expression,
                slot === 0
                    ? "be.setVertexBuffer(0,quadVertex)"
                    : "be.setVertexBuffer(1,lg._instanceBuf)",
                "Text bundle vertex owner",
            );
            return [
                `${indent}be->commands.push_back({TextBundleOp::${slot === 0 ? "quad" : "instances"},${slot === 0 ? "quadVertex" : "ops.retain_instance_buffer(*lg)"}});`,
            ];
        }
        if (path === "be.setBindGroup") {
            c.assertExpressionShape(
                ...
                "Text bundle group slot",
            );
            return [
                `${indent}be->commands.push_back({TextBundleOp::group,bg});`,
            ];
        }
        if (path === "be.draw") {
            if (expression.arguments.length !== 4)
                c.contractError(expression, "Text bundle draw arity changed.");
            return [
                `${indent}be->commands.push_back({TextBundleOp::draw,{}, {${expression.arguments.map((arg) => `text_resource_size(${lowerer.expression(arg)})`).join(",")}}});`,
            ];
        }
        if (path === "pass.executeBundles")
            c.assertExpressionShape(
                expression,
                "pass.executeBundles(visibleBundles)",
                "Text bundle execution list",
            );
        if (path === "pass.end")
            c.assertExpressionShape(
                expression,
                "pass.end()",
                "Text render pass end",
            );
        return undefined;
    },
})}
