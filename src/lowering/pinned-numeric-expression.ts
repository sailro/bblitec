...
export function renderPinnedArithmetic(
    ...
    const operand = (value: RenderedCpp, minimum: number): string =>
        style === "minimal" && value.precedence < minimum
            ? `(${value.text})`
            : value.text;
    const result = (
        text: string,
    ...
    if (ts.isNumericLiteral(node))
        return cppPrimary(
            spelling.numeric?.(node) ?? doubleLiteral(Number(node.text)),
        );
    ...
    if (ts.isPrefixUnaryExpression(node)) {
        ...
        const minimum =
            (operator === "+" || operator === "-") &&
            value.text.startsWith(operator)
                ? cppPrecedence.primary
                : cppPrecedence.unary;
        return result(operator + operand(value, minimum), cppPrecedence.unary);
    }
    if (ts.isConditionalExpression(node)) {
        return result(
            `${operand(render(node.condition), cppPrecedence.logicalOr)} ? ` +
                `${render(node.whenTrue).text} : ${render(node.whenFalse).text}`,
            cppPrecedence.conditional,
            true,
        );
    }
    ...
    if (
        ...
        spelling.remainder !== "integral"
    ) {
        return cppPrimary(
            pinnedRemainderCall(
                render(node.left).text,
                render(node.right).text,
            ),
        );
    }
    if (kind === ts.SyntaxKind.AsteriskAsteriskToken) {
        return cppPrimary(
            `std::pow(${render(node.left).text}, ${render(node.right).text})`,
        );
    }
    const operator =
        kind === ts.SyntaxKind.PercentToken
            ? "%"
            : PINNED_BOOLEAN_OPERATORS.get(kind);
    const level = precedence.get(kind);
    if (operator === undefined || level === undefined) return undefined;
    return result(
        `${operand(render(node.left), level)} ${operator} ${operand(render(node.right), level + 1)}`,
        level,
    );
}
