    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        statement(statement) {
            ...
            context.assertExpressionShape(
                ...
                "Gamma-albedo composition registration",
            );
            return [];
        },
    });
...
export function lowerPbrTransmissionSelection(
    ...
    const activation = statements.findIndex(
        (statement) =>
            ts.isExpressionStatement(statement) &&
            context.findNodes(
                statement,
    ...
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "mat?._transmissive",
            { cpp: "(mat && mat->source_transmissive)", type: "bool" },
        ],
    ]);
    const body = lowerPinnedBody(file, statements.slice(0, activation), {
        ...
        expression(node, lowerer) {
            if (
                ...
                context.expressionMatchesShape(
                    node.left,
                    "mat._subsurface?.refraction?.intensity",
                )
            )
        ...
        statement(statement, lowerer, indent) {
            if (ts.isReturnStatement(statement)) {
                if (statement.expression)
                    context.contractError(
                        statement,
                        "Expected a void transmission early return.",
                    );
            ...
            if (
                ...
                read.name.text !== "material"
            )
                context.contractError(
                    read,
                    "Expected a transmission mesh material read.",
                );
            const mesh = context.unwrapExpression(read.expression);
            if (
                !ts.isElementAccessExpression(mesh) ||
                !context.expressionMatchesShape(mesh.expression, "meshes") ||
                !mesh.argumentExpression
            )
                context.contractError(
                    mesh,
...
export function lowerPbrSceneHookRegistry(context: LoweringContext): string {
    ...
    context.assertExpressionShape(
        context.variableInitializer(source, "_pbrSceneHooks"),
        "null",
        "Initial PBR hook registry",
    );
    const register = context.functionDeclaration(
        module,
    ...
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "_pbrSceneHooks",
            {
                cpp: "pbr_scene_hooks",
                type: "opaque",
                absentCpp: "!pbr_scene_hooks",
            },
        ],
    ]);
    const registerBody = lowerPinnedBody(
        register.file,
        register.declaration.body!.statements,
        {
            bindings,
            calls: new Map(),
            expression(node, lowerer) {
        ...
        get.declaration.body!.statements,
        {
            ...
            returnValue: (expression, lowerer) => {
                if (!expression)
                    context.contractError(
                        get.declaration,
                        "Expected the PBR hook iterable return.",
                    );
    ...
    const builder = context.functionDeclaration(
        builderModule,
        "buildPbrRenderables",
    );
    const loop = builder.declaration.body!.statements.find(
        (statement) =>
    ...
    if (!loop || !ts.isForOfStatement(loop))
        context.contractError(
            builder.declaration,
            "Expected PBR scene hook dispatch.",
        );
    const dispatch = lowerPinnedBody(builder.file, [loop], {
        bindings: new Map([["hook", { cpp: "hook", type: "opaque" }]]),
        calls: new Map(),
        statement(statement, lowerer, indent) {
            if (ts.isForOfStatement(statement)) {
                context.assertStatementShapes(
                    statement,
                    [statement],
                    "for (const hook of _getPbrSceneHooks()) { await hook(scene as SceneContext, engine, meshes); }",
                    "PBR scene hook iteration",
                );
                const body = ts.isBlock(statement.statement)
                    ? statement.statement.statements
                    : [statement.statement];
                return [
                    `${indent}for (const auto& hook : get_pbr_scene_hooks()) {`,
    ...
    const hook = context.functionDeclaration(
        "src/material/pbr/pbr-transmission-ext.ts",
        "registerPbrTransmission",
    );
    const activation = lowerPinnedBody(
        hook.file,
        hook.declaration.body!.statements.slice(-2),
        {
            bindings: new Map([
            ...
            statement(statement, numeric, indent) {
                if (
                    ...
                    return [
                        `${indent}static_cast<void>(${numeric.expression(statement.expression)});`,
                    ];
                if (
                    !ts.isExpressionStatement(statement) ||
                    !context.expressionMatchesShape(
