...
export function lowerBabylonMaterialMaps(context: LoweringContext): string {
    ...
    const loops = ["data.materials", "data.multiMaterials"].map(
        (expression) => {
            const matches = context.findNodes(
                declaration,
                (node): node is ts.ForOfStatement =>
                    ts.isForOfStatement(node) &&
                    context.expressionMatchesShape(node.expression, expression),
            );
            ...
            if (
                ...
                !ts.isIfStatement(loop.parent.parent)
            )
                context.contractError(
                    declaration,
                    `Expected the '${expression}' map construction branch.`,
                );
            return loop;
        },
    );
    const materialLoop = loops[0]!;
    if (!ts.isBlock(materialLoop.statement))
        context.contractError(
            materialLoop,
            "Expected the material hydration block.",
        );
    const materialStatements = materialLoop.statement.statements;
    const first = materialStatements[0],
        last = materialStatements.at(-1);
    if (
        !first ||
        ...
        context.contractError(
            materialLoop,
            "Expected material creation, hydration and map publication.",
        );
    ...
    if (
        ...
        context.contractError(
            created,
            "Expected the Standard material factory.",
        );
    const bindings = new Map<string, PinnedBinding>([
        [
            "data.materials",
            {
                cpp: 'document.at("materials")',
                type: "opaque",
                absentCpp: '!babylon_json_truthy(document, "materials")',
            },
        ],
        [
            "data.multiMaterials",
            {
                cpp: 'document.at("multiMaterials")',
                type: "opaque",
                absentCpp: '!babylon_json_truthy(document, "multiMaterials")',
            },
        ],
        ...
        [
            "mm.materials",
            { cpp: 'babylon_material_ids(mm.at("materials"))', type: "opaque" },
        ],
        [created.name.text, { cpp: created.name.text, type: "opaque" }],
    ]);
    const body = lowerPinnedBody(
        file,
        loops.map((loop) => loop.parent.parent as ts.IfStatement),
        {
            bindings,
            calls: new Map(),
            ...
            statement(statement, lowerer, indent) {
                if (statement === materialLoop)
                    return lowerer.statement(
                        ts.factory.updateForOfStatement(
                            materialLoop,
                            materialLoop.awaitModifier,
                            materialLoop.initializer,
                            materialLoop.expression,
                            ts.factory.createBlock([first, last]),
                        ),
                        indent,
                    );
                if (statement === first)
                ...
                const target = context.expressionMatchesShape(
                    call.expression,
                    "materialMap.set",
                )
                    ? "materials"
                ...
                return [
                    `${indent}${target}[${lowerer.expression(call.arguments[0]!)}] = ${lowerer.expression(call.arguments[1]!)};`,
                ];
            },
        },
    );
