...
export function lowerBabylonMeshConstruction(context: LoweringContext): string {
    ...
    const loops = context.findNodes(
        ...
        (node): node is ts.ForOfStatement =>
            ...
            context.expressionMatchesShape(node.expression, "data.meshes") &&
            context.hasCall(node, "initMeshTransform"),
    );
    const loop = loops[0];
    if (loops.length !== 1 || !loop || !ts.isBlock(loop.parent))
        context.contractError(
            declaration,
            "Expected the mesh construction pass.",
        );
    ...
    const transformArguments = (
        ...
        const value = transform.expression?.(node, lowerer);
        if (value === undefined)
            context.contractError(
                node,
                "Unrepresented Babylon mesh transform arguments.",
            );
        return typeof value === "string" ? value : value.text;
    };
    const material = babylonSubmeshMaterialScope(
        context,
        "md",
        'sub.at("materialIndex").get<double>()',
    );
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "md.isVisible",
            { cpp: 'md.value("isVisible", Json{})', type: "opaque" },
        ],
        [
            "md.indices.length",
            {
                cpp: 'static_cast<double>(md.at("indices").size())',
                type: "scalar",
            },
        ],
        [
            "subMeshes.length",
            { cpp: "static_cast<double>(subMeshes.size())", type: "scalar" },
        ],
        [
            "sub.indexCount",
            { cpp: 'sub.at("indexCount").get<double>()', type: "scalar" },
        ],
        [
            "sub.indexStart",
            { cpp: 'sub.at("indexStart").get<double>()', type: "scalar" },
        ],
        ...
        [
            "firstMesh",
            {
                cpp: "firstMesh",
                type: "opaque",
                absentCpp: "firstMesh == invalid_handle",
            },
        ],
    ]);
    for (const field of [
        ...
        bindings.set(`md.${field}`, {
            cpp: `md.at(${JSON.stringify(field)})`,
            type: "opaque",
    ...
    const cpuAttributes = new Map([
        ["_cpuPositions", upload.arguments[1]!],
        ["_cpuNormals", upload.arguments[2]!],
        ["_cpuIndices", upload.arguments[3]!],
        ["_cpuUvs", upload.arguments[4]!],
    ]);
    const body = lowerPinnedBody(
        file,
        loop.parent.statements.slice(
            0,
            loop.parent.statements.indexOf(loop) + 1,
        ),
        {
            bindings,
            ...
            calls: new Map([
                ...
                [
                    "uploadMeshToGPU",
                    (args) => `upload_babylon_mesh(${args.join(", ")})`,
                ],
                [
                    "bakeLocalMatrix",
                    (args) => {
                        if (args.length !== 3)
                            context.contractError(
                                loop,
                                "Expected the pivot bake buffers and matrix.",
                            );
            ...
            expression(node, lowerer) {
                if (
                    ...
                    context.expressionMatchesShape(node.left, "opts.maxMeshes")
                )
                ...
                if (
                    ts.isBinaryExpression(node) &&
                    context.expressionMatchesShape(node.left, "firstMesh") &&
                    ...
                    const operator =
                        ...
                        ts.SyntaxKind.EqualsEqualsEqualsToken
                            ...
                            : node.operatorToken.kind ===
                              ...
                              : undefined;
                    if (operator)
                        return `(firstMesh ${operator} invalid_handle)`;
                }
                ...
                if (ts.isTemplateExpression(node))
                    return `(std::string{${JSON.stringify(node.head.text)}}${node.templateSpans
                        .map(
                            (span) =>
                                ` + js::number_to_string(${lowerer.expression(span.expression)}) + ${JSON.stringify(span.literal.text)}`,
                        )
                        .join("")})`;
                if (
                ...
                if (ts.isCallExpression(node)) {
                    if (
                        ...
                        const record = context.unwrapExpression(
                            node.arguments[0]!,
                        );
                        if (!ts.isObjectLiteralExpression(record))
                            context.contractError(
                                record,
                                "Expected the mesh factory record.",
                            );
                        ...
                        for (const property of record.properties) {
                            if (!ts.isPropertyAssignment(property))
                                context.contractError(
                                    property,
                                    "Unsupported mesh record property.",
                                );
                            ...
                            if (
                                ...
                                context.contractError(
                                    property,
                                    `Unrepresented mesh record property '${name}'.`,
                                );
                            values.set(
                                name,
                        ...
                        const args = [
                            ...
                            if (!value)
                                context.contractError(
                                    record,
                                    `Missing mesh record property '${name}'.`,
                                );
                            return value;
                        });
                        return `create_babylon_mesh(engine, nodes, ${args.join(", ")}, ${transformArguments(node, lowerer)})`;
                    }
                    if (
                        context.expressionMatchesShape(
                            node.expression,
                        ...
                        return `create_babylon_container(nodes, ${transformArguments(node, lowerer)})`;
                    if (
                        context.expressionMatchesShape(
                            node.expression,
                            "meshesByNodeId.has",
            ...
            statement(statement, lowerer, indent) {
                const fromMaterial = material.statement?.(
                    statement,
                    lowerer,
                    indent,
                );
                ...
                if (
                    ...
                    if (name === "firstMesh") {
                        if (
                            ...
                            context.contractError(
                                variable,
                                "Expected an absent first mesh.",
                            );
                        return [
                            `${indent}std::size_t firstMesh = invalid_handle;`,
                        ];
                    }
                    if (name === "subMeshes") {
                        lowerer.bindPorts(
                ...
                if (
                    ...
                    const attribute = cpuAttributes.get(
                        expression.left.name.text,
                    );
                    if (!attribute) return undefined;
                    context.assertExpressionShape(
                        expression.right,
                        attribute.getText(file),
                        "Uploaded CPU attribute alias",
                    );
                    return [];
                }
                if (!ts.isCallExpression(expression)) return undefined;
                const args = (): string[] =>
                    expression.arguments.map((arg) => lowerer.expression(arg));
                if (
                    context.expressionMatchesShape(
                    ...
                    return [
                        `${indent}set_babylon_node(node_map, ${args().join(", ")});`,
                    ];
                if (
                    context.expressionMatchesShape(
                        expression.expression,
                ...
                if (
                    ...
                    return [
                        `${indent}meshes_by_id[${lowerer.expression(expression.arguments[0]!)}] = {};`,
                    ];
                }
                if (
                    ...
                    const receiver = context.unwrapExpression(
                    ...
                    if (
                        ...
                        return [
                            `${indent}meshes_by_id.at(${lowerer.expression(receiver.arguments[0]!)}).push_back(${args()[0]});`,
                        ];
                }
                return undefined;
            },
