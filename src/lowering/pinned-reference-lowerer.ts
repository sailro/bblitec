...
export const referenceTuple = (types: readonly string[]): string =>
    `tuple:${JSON.stringify(types)}`;
function referenceTypeList(type: string, prefix: string): string[] | undefined {
    ...
    if (
        ...
        throw new Error(
            `Pinned reference types must be a string array: ${type}.`,
        );
    }
    return types;
}
...
export class PinnedReferenceLowerer {
    ...
    public constructor(
        ...
        private readonly schema: ReferenceSchema,
    ) {}

    public type(node: ts.TypeNode): string {
        const alias = this.schema.typeAliases?.get(
        ...
        if (ts.isUnionTypeNode(node)) {
            const defined = node.types.filter(
                (type) => type.kind !== ts.SyntaxKind.UndefinedKeyword,
            );
            if (defined.length === 1 && defined.length < node.types.length)
                return `optional:${this.type(defined[0]!)}`;
        ...
        if (ts.isTypeReferenceNode(node) && ts.isIdentifier(node.typeName)) {
            ...
            if (name === "ArrayLike" && node.typeArguments?.length === 1)
                return `${this.type(node.typeArguments[0]!)}[]`;
            if (name === "Map" && node.typeArguments?.length === 2)
                return `map:${JSON.stringify(node.typeArguments.map((type) => this.type(type)))}`;
            if (
                name === "WeakMap" &&
                node.typeArguments?.length === 2 &&
                this.type(node.typeArguments[0]!) === "object"
            )
                return `weakmap:${this.type(node.typeArguments[1]!)}`;
        }
        return this.context.contractError(
            node,
            "Pinned reference type has no native representation.",
        );
    }
    ...
    public storage(type: string): string {
        ...
        if (type.startsWith("weakmap:"))
            return `js::WeakMap<${this.storage(type.slice(8))}>`;
        if (type.startsWith("optional:"))
            return `std::optional<${this.storage(type.slice(9))}>`;
        const tuple = tupleTypes(type);
        if (tuple)
            return `std::tuple<${tuple.map((type) => this.storage(type)).join(", ")}>`;
        if (type.startsWith("map:"))
            return `js::Map<${mapTypes(type)!
                .map((type) => this.storage(type))
                .join(", ")}>`;
        if (type.endsWith("[]"))
            return `js::Array<${this.storage(type.slice(0, -2))}>`;
        if (this.schema.records.has(type)) return `js::Ref<${type}>`;
        throw new Error(
            `Pinned reference storage is not represented: ${type}.`,
        );
    }
    ...
    private scoped<T>(body: () => T): T {
        ...
        try {
            ...
            for (const [key, value] of saved)
                this.schema.bindings.set(key, value);
        }
    }
    ...
    public statements(
        ...
        return statements
            .map((statement) => this.statement(statement, indent))
            .join("\n");
    }
    ...
    private branch(statement: ts.Statement, indent: string): string {
        return this.scoped(() =>
            ts.isBlock(statement)
                ? this.statements(statement.statements, indent)
                : this.statement(statement, indent),
        );
    }
    ...
    private declaration(declaration: ts.VariableDeclaration): string {
        if (!declaration.initializer)
            return this.context.contractError(
                declaration,
                "Pinned reference local requires a named initializer.",
            );
    ...
    public statement(statement: ts.Statement, indent: string): string {
        ...
        if (ts.isForStatement(statement))
            return this.scoped(() => {
                if (
                    ...
                    return this.context.contractError(
                        statement,
                        "Pinned reference loop requires initializer, guard and increment.",
                    );
                const initial = this.declaration(
        ...
        if (ts.isSwitchStatement(statement)) {
            const clauses = statement.caseBlock.clauses
                .map(
                    (clause) =>
                        `${indent}${ts.isCaseClause(clause) ? `case ${this.context.numericValue(clause.expression, clause.getSourceFile())}` : "default"}:\n${this.scoped(() => this.statements(clause.statements, indent + "    "))}`,
                )
                .join("\n");
    ...
    public expression(input: ts.Expression, expected?: string): ReferenceValue {
        ...
        const exact = this.schema.bindings.get(
            node.getText(node.getSourceFile()),
        );
        if (exact) {
        ...
        if (ts.isNumericLiteral(node))
            return { cpp: doubleLiteral(Number(node.text)), type: "number" };
        ...
        if (
            ...
            return {
                cpp: node.kind === ts.SyntaxKind.TrueKeyword ? "true" : "false",
                type: "boolean",
            };
        if (
        ...
        if (ts.isIdentifier(node))
            return this.context.contractError(
                node,
                "Unbound pinned reference identifier.",
            );
        if (ts.isPropertyAccessExpression(node)) {
            ...
            const type = this.schema.records
                .get(owner.type)
                ?.get(node.name.text);
            if (!type)
                return this.context.contractError(
        ...
        if (ts.isObjectLiteralExpression(node)) {
            ...
            const type =
                expected ??
                [...this.schema.records].find(
                    ([, fields]) =>
                        fields.size === names.length &&
                        names.every((name) => fields.has(name)),
                )?.[0];
            const fields = type && this.schema.records.get(type);
            if (!type || !fields || fields.size !== names.length)
                return this.context.contractError(
                    node,
        ...
        if (ts.isArrayLiteralExpression(node)) {
            ...
            const values = node.elements.map((value, i) =>
                this.expression(
                    value,
                    expected?.endsWith("[]")
                        ? expected.slice(0, -2)
            ...
            const type =
                ...
                values.every((value) => value.type === values[0]!.type)
                    ? `${values[0]!.type}[]`
                    : referenceTuple(values.map((value) => value.type)));
            if (!type.endsWith("[]") && !tupleTypes(type))
                return this.context.contractError(
                    node,
        ...
        if (ts.isNewExpression(node) && ts.isIdentifier(node.expression)) {
            if (
                ...
                const type = node.typeArguments?.[0]
                    ? `${this.type(node.typeArguments[0])}[]`
                    : expected!;
                return {
                    cpp: `${this.storage(type)}(js::array_index(${this.expression(node.arguments[0]!).cpp}))`,
                    type,
                };
            }
            if (
            ...
            if (node.expression.text === "WeakMap" && !node.arguments?.length) {
                const type =
                    node.typeArguments?.length === 2 &&
                    this.type(node.typeArguments[0]!) === "object"
                        ? `weakmap:${this.type(node.typeArguments[1]!)}`
                        : expected;
                if (type?.startsWith("weakmap:"))
                    return { cpp: `${this.storage(type)}{}`, type };
            }
        ...
        if (
            ...
            if (!operator)
                return this.context.contractError(
                    node,
                    "Pinned reference unary operator is not represented.",
                );
            return {
                cpp: `(${ts.isPrefixUnaryExpression(node) ? operator + value.cpp : value.cpp + operator})`,
                type: operator === "!" ? "boolean" : value.type,
            };
        }
        if (ts.isBinaryExpression(node)) {
            if (
                ...
                if (array.type.endsWith("[]"))
                    return {
                        cpp: `js::array_truncate(${array.cpp}, ${this.expression(node.right).cpp})`,
                        type: "void",
                    };
            }
            ...
            if (
                ...
                this.schema.records.has(left.type)
            ) {
            ...
            if (node.operatorToken.kind === ts.SyntaxKind.PercentToken)
                return {
                    cpp: pinnedRemainderCall(left.cpp, right.cpp),
                    type: "number",
                };
            const operator =
                PINNED_ASSIGNMENT_OPERATORS.get(node.operatorToken.kind) ??
                PINNED_BOOLEAN_OPERATORS.get(node.operatorToken.kind);
            if (!operator)
                return this.context.contractError(
                    node,
                    "Pinned reference binary operator is not represented.",
                );
            return {
                cpp: `(${left.cpp} ${operator} ${right.cpp})`,
                type: ["==", "!=", "<", ">", "<=", ">=", "&&", "||"].includes(
                    operator,
                )
                    ? "boolean"
        ...
        if (ts.isConditionalExpression(node)) {
            ...
            return {
                cpp: `(${this.expression(node.condition).cpp} ? ${yes.cpp} : ${no.cpp})`,
                type: yes.type,
            };
        }
        ...
        if (
            ...
            return {
                cpp: `[&]() { return ${this.expression(node.body, type).cpp}; }`,
                type: `function:${type}`,
            };
        }
        return this.context.contractError(
            node,
            "Pinned reference expression has no native representation.",
        );
    }
    ...
    private call(node: ts.CallExpression): ReferenceValue {
        ...
        if (math)
            return {
                cpp: math(
                    node.arguments.map(
                        (argument) => this.expression(argument).cpp,
                    ),
                ),
                type: "number",
            };
        ...
        if (ts.isPropertyAccessExpression(node.expression)) {
            ...
            if (types) {
                ...
                if (method === "set" && node.arguments.length === 2)
                    return {
                        cpp: `${owner.cpp}.set(${this.expression(node.arguments[0]!, key).cpp}, ${this.expression(node.arguments[1]!, value).cpp})`,
                        type: owner.type,
                    };
            }
            if (
                ...
                return {
                    cpp: `js::array_splice_one(${owner.cpp}, ${this.expression(node.arguments[0]!).cpp})`,
                    type: "void",
                };
            if (
                owner.type.endsWith("[]") &&
                ["filter", "findIndex"].includes(method) &&
                node.arguments.length === 1 &&
