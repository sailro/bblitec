...
const extent = (cpp: string): PinnedBinding =>
    scalar(`static_cast<double>(${cpp})`);
...
`;
    ...
    private constants(): Map<string, PinnedBinding> {
        return new Map([
            ...
            [
                "TEXT_UBO_BYTES",
                scalar(String(this.numeric(RENDERABLE, "TEXT_UBO_BYTES"))),
            ],
            [
                "GLYPH_METADATA_BYTES",
                scalar(String(this.numeric(TEXTURES, "GLYPH_METADATA_BYTES"))),
            ],
            ["TEX_WIDTH", scalar(String(this.numeric(TEXTURES, "TEX_WIDTH")))],
            [
                "BYTES_PER_ROW",
                scalar(String(this.numeric(TEXTURES, "BYTES_PER_ROW"))),
            ],
    ...
    private body(
        ...
        returnValue?: PinnedNumericScope["returnValue"],
        ...
        const scope: PinnedNumericScope = {
            bindings: new Map([...this.constants(), ...bindings]),
            calls: new Map([...pinnedNumericMathCalls(), ...calls]),
            booleanAnd: true,
            booleanOr: true,
            forOf: (range, element) => {
                if (!["data._groups", "r._data._groups"].includes(range))
                    return undefined;
                      ...
                      statement: (
                          ...
                          if (
                              ...
                              this.context.contractError(
                                  statement,
                                  "Text resource declaration gained additional bindings.",
                              );
                          return result;
                      },
                  }
        ...
        return (
            `    // ${this.context.provenance(module, name)}\n` +
    ...
    private targetKey(): string {
        ...
        if (
            ...
            this.context.contractError(
                declaration,
                "Text target signature body changed.",
            );
        const value = (node: ts.Expression): string => {
            ...
            return this.context.contractError(
                node,
                "Unrepresented text target signature component.",
            );
        };
    ...
    private rows(): string {
        return ["nextPow2Rows", "rowsForTexels"]
            .map((name) => {
...
${body}\n}`;
    ...
    private bufferDescriptor(
        ...
        if (
            ...
            this.context.contractError(
                call,
                "Text buffer creation descriptor changed.",
            );
        this.context.assertExpressionShape(
            call.expression,
            "device.createBuffer",
            "Text buffer device boundary",
        );
        const object = call.arguments[0];
        if (
            object.properties
                .map((property) => property.name?.getText())
                .join() !== "label,size,usage"
        )
            this.context.contractError(
                object,
                "Text buffer descriptor fields changed.",
            );
        this.context.assertExpressionShape(
            this.context.propertyInitializer(object, "label"),
            JSON.stringify(label),
            "Text buffer label",
        );
        this.context.assertExpressionShape(
            this.context.propertyInitializer(object, "usage"),
            usage,
            "Text buffer usage",
        );
        return lowerer.expression(
            this.context.propertyInitializer(object, "size"),
        );
    }
    ...
    private styleFactory(): string {
        ...
        if (
            ...
            this.context.contractError(
                declaration,
                "Text style buffer factory changed.",
            );
        ...
        const bytes = this.bufferDescriptor(
            statement.expression,
            "text-styles",
    ...
    private gpuBindings(
        ...
        return new Map([
            ...
            [
                `${root}._uploadedDataVersion`,
                scalar(`${cpp}uploaded_data_version`),
            ],
            [
                `${root}._uploadedStyleVersion`,
                scalar(`${cpp}uploaded_style_version`),
            ],
        ...
        const body = this.body(
            ...
            (statement, lowerer, indent) => {
                ...
                if (
                    ...
                    c.propertyPath(expression.left)?.join(".") ===
                        "gpu._styleBuf"
                ) {
                    c.assertExpressionShape(
                        expression,
                        "gpu._styleBuf = createStyleBuffer(device, data._styles.byteLength / TEXT_STYLE_BYTES)",
                        "Text style capacity creation",
                    );
                    const call = expression.right as ts.CallExpression;
                    return [
                        `${indent}create_text_style_buffer(gpu, ${lowerer.expression(call.arguments[1]!)}, ops);`,
                    ];
                }
                if (
    ...
    private instanceCapacity(): string {
        ...
        const body = this.body(
            ...
            new Map([...this.gpuBindings(), ["needed", scalar("needed")]]),
            new Map([
                [
            ...
            (statement, lowerer, indent) => {
                if (
                    ts.isExpressionStatement(statement) &&
                    ts.isBinaryExpression(statement.expression) &&
                    c.propertyPath(statement.expression.left)?.join(".") ===
                        "gpu._instanceBuf"
                ) {
                    const bytes = this.bufferDescriptor(
                    ...
                    return [
                        `${indent}ops.create_renderable_buffer(gpu, TextBufferKind::instances, text_resource_size(${bytes}));`,
                    ];
                }
                return undefined;
            },
    ...
    private uploadAtlas(): string {
        const body = this.body(
            TEXTURES,
            "uploadAll",
            new Map([["texelsUsed", scalar("used")]]),
            new Map([
                [
                    "rowsForTexels",
                    (args) => `text_rows_for_texels(${args.join(", ")})`,
                ],
            ]),
            (statement, lowerer, indent) => {
                if (
                ...
                this.context.assertExpressionShape(
                    statement.expression,
                    `device.queue.writeTexture({texture:tex},cpuData.buffer as ArrayBuffer,
    ...
    private atlasFactories(): string {
        ...
        if (
            texture.declaration.body!.statements.length !== 1 ||
            !returned ||
            ...
            c.contractError(
                texture.declaration,
                "Text atlas texture factory changed.",
            );
        c.assertExpressionShape(
            returned.expression,
            `device.createTexture({label,format:"rgba32float",
            size:{width:TEX_WIDTH,height:rows,depthOrArrayLayers:1},usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.COPY_SRC})`,
            "Text atlas texture descriptor",
        );
        const metadata = c.functionDeclaration(TEXTURES, "createMetaBuffer");
        const result = metadata.declaration.body!.statements[0];
        if (
            metadata.declaration.body!.statements.length !== 1 ||
            !result ||
            ...
            c.contractError(
                ...
                "Text atlas metadata factory changed.",
            );
        const lowerer = new PinnedNumericLowerer(metadata.file, {
            bindings: new Map([
                ...this.constants(),
                ["slots", scalar("slots")],
            ]),
...
template<class Ops> void create_text_atlas_metadata(TextAtlasGpuState& gpu, double slots, Ops& ops) {
    ...
    private allocationObject(
        ...
        const fields = atlas
            ? new Map([
                  ...
                  ["_curveTex", "curves"],
                  ["_bandTex", "bands"],
                  ["_curveTexRows", "curve_rows"],
                  ["_bandTexRows", "band_rows"],
                  ["_metaBuf", "metadata"],
                  ["_metaCap", "metadata_capacity"],
                  ["_uploadedVersion", "uploaded_version"],
              ])
            : new Map([
                  ["_device", "device_identity"],
                  ["_textU", "uniform"],
                  ["_instanceBuf", "instances"],
                  ["_instanceCap", "instance_capacity"],
                  ["_styleBuf", "styles"],
                  ["_uploadedStyleVersion", "uploaded_style_version"],
                  ["_pipeline", "pipeline"],
                  ["_variantPipeline", "variant_pipeline"],
                  ...
                  ["_targetKey", "target_key"],
              ]);
        if (
            object.properties
                .map((property) => property.name?.getText())
                .join() !== [...fields.keys()].join()
        )
            c.contractError(
                object,
                "Text GPU state inventory changed; update the native resource owner.",
            );
        const lines = [
            `${indent}{`,
            `${indent}    auto created = std::make_shared<${atlas ? "TextAtlasGpuState" : "TextGpuState"}>();`,
        ];
        for (const [source, target] of fields) {
            ...
            if (source === "_curveTex" || source === "_bandTex") {
                ...
                c.assertExpressionShape(
                    expression,
                    `createAtlasTexture(device, ${name}, "text-slug-${target}")`,
                    "Text atlas allocation",
                );
                lines.push(
                    `${at}create_text_atlas_texture(*created, TextAtlasTextureKind::${target}, ${lowerer.expression((expression as ts.CallExpression).arguments[1]!)}, ops);`,
                );
            } else if (source === "_metaBuf") {
                c.assertExpressionShape(
                    expression,
                    "createMetaBuffer(device, metaCap)",
                    "Text metadata allocation",
                );
                lines.push(
                    `${at}create_text_atlas_metadata(*created, ${lowerer.expression((expression as ts.CallExpression).arguments[1]!)}, ops);`,
                );
            } else if (source === "_textU") {
                c.assertExpressionShape(
                    expression,
                    'createEmptyUniformBuffer(engine, TEXT_UBO_BYTES, "text-renderable-ubo")',
                    "Text uniform allocation",
                );
                lines.push(
                    `${at}ops.create_renderable_buffer(*created, TextBufferKind::uniform, text_resource_size(${lowerer.expression((expression as ts.CallExpression).arguments[1]!)}));`,
                );
                ...
                lines.push(
                    `${at}ops.create_renderable_buffer(*created, TextBufferKind::instances, text_resource_size(${bytes}));`,
                );
            } else if (source === "_styleBuf") {
                c.assertExpressionShape(
                    expression,
                    "createStyleBuffer(device, 1)",
                    "Text initial style allocation",
                );
                lines.push(
                    `${at}create_text_style_buffer(*created, ${lowerer.expression((expression as ts.CallExpression).arguments[1]!)}, ops);`,
                );
            } else
                lines.push(
                    `${at}created->${target} = ${lowerer.expression(expression)};`,
                );
        }
    ...
    private atlas(): string {
        ...
        const calls = new Map<string, (args: readonly string[]) => string>([
            ...
            [
                "uploadAll",
                (args) =>
                    `upload_text_atlas_texture(*gpu, ${args.slice(1).join(", ")}, ops)`,
            ],
            ...[
                ["_curveTex", "curves"],
                ["_bandTex", "bands"],
                ...
                ([field, role]): [
                    ...
                    `gpu.${field}.destroy`,
                    () => `if (gpu->destroy_${role}) gpu->destroy_${role}()`,
                ],
            ),
        ]);
        const body = this.body(
            ...
            (statement, lowerer, indent) => {
                ...
                if (ts.isBinaryExpression(expression)) {
                    ...
                    for (const [source, kind, rows] of [
                        ["gpu._curveTex", "curves", "gpu._curveTexRows"],
                        ["gpu._bandTex", "bands", "gpu._bandTexRows"],
                    ]) {
                        if (path === source) {
                            c.assertExpressionShape(
                                expression.right,
                                `createAtlasTexture(device, ${rows}, "text-slug-${kind}")`,
                                "Text atlas growth",
                            );
                            return [
                                `${indent}create_text_atlas_texture(*gpu, TextAtlasTextureKind::${kind}, ${lowerer.expression((expression.right as ts.CallExpression).arguments[1]!)}, ops);`,
                            ];
                        }
                    }
                    if (path === "gpu._metaBuf") {
                        c.assertExpressionShape(
                            expression.right,
                            "createMetaBuffer(device, gpu._metaCap)",
                            "Text metadata growth",
                        );
                        return [
                            `${indent}create_text_atlas_metadata(*gpu, ${lowerer.expression((expression.right as ts.CallExpression).arguments[1]!)}, ops);`,
                        ];
                    }
                }
                if (
    ...
    private renderable(): string {
        const c: LoweringContext = this.context;
        const bindings = new Map([
            ...this.gpuBindings("gpu", "gpu->"),
            ["gpu", { ...opaque("gpu"), absentCpp: "!gpu" }],
            ["device", opaque("device_identity")],
            ["key", opaque("key")],
            ["pipeline", opaque("pipelines.pipeline")],
            ["variantPipeline", opaque("pipelines.variant_pipeline")],
            ["r._gpu", opaque("r.gpu")],
            ["r._data._instanceCount", extent("r.data->instance_count")],
        ]);
        const calls = new Map<string, (args: readonly string[]) => string>(
            [
                ["_textU", "uniform"],
                ...
                if (ts.isVariableStatement(statement)) {
                    ...
                    if (ts.isObjectBindingPattern(declaration.name)) {
                        ...
                        if (
                            declaration.name.elements
                                ...
                                .join() !==
                            "_pipeline:pipeline,_variantPipeline:variantPipeline"
                        )
                            c.contractError(
                                declaration,
                                "Text pipeline destructuring changed.",
                            );
                        return [];
                    }
                    ...
                    if (declaration.name.getText() === "key") {
                        c.assertExpressionShape(
                            declaration.initializer!,
                            "targetSig(target)",
                            "Text target cache key",
                        );
                        return [
                            `${indent}const std::string key = text_target_key(target);`,
                        ];
                    }
                    if (declaration.name.getText() === "gpu") {
                        c.assertExpressionShape(
                            declaration.initializer!,
                            "r._gpu",
                            "Text retained renderable GPU alias",
                        );
...
template<class Ops> std::shared_ptr<TextGpuState> ensure_text_gpu(TextRenderableState& r,
    ...
    private resources(): string {
        const c: LoweringContext = this.context;
        const { declaration } = c.functionDeclaration(
            RENDERABLE,
        ...
        if (boundary < 0)
            c.contractError(
                declaration,
                "Text resource/uniform update boundary changed.",
            );
        const bindings = new Map([
            ...this.gpuBindings(),
            ...this.atlasBindings("atlasGpu", "atlasGpu."),
            ["gpu", opaque("gpu")],
            ["data", opaque("data")],
            ["device", opaque("gpu.device_identity")],
            ["bindGroupLayout", opaque("layout")],
            ["rebuilt", { cpp: "atlas_result.rebuilt", type: "bool" as const }],
            ["data._instanceCount", extent("data.instance_count")],
            ["data._version", scalar("data.version")],
            ["data._dirtyStart", extent("data.dirty_start")],
            ["data._dirtyEnd", extent("data.dirty_end")],
            ["view.buffer", opaque("view")],
            ["view.byteOffset", scalar("0")],
            ["view.byteLength", scalar("view.size()")],
        ]);
        const body = this.body(
            RENDERABLE,
            "updateTextRenderable",
            ...
            new Map([
                [
                    "ensureStyleGpu",
                    () => "ensure_text_style_gpu(data, gpu, ops)",
                ],
                [
                    "ensureInstanceCapacity",
                    (args: readonly string[]) =>
                        `ensure_text_instance_capacity(gpu, ${args[2]}, ops)`,
                ],
            ]),
            (statement, lowerer, indent) => {
                if (ts.isVariableStatement(statement)) {
                    ...
                    if (ts.isObjectBindingPattern(variable.name)) {
                        c.assertExpressionShape(
                            ...
                            "Text group atlas synchronization",
                        );
                        if (
                    ...
                    if (name === "styleRecreated")
                        c.assertExpressionShape(
                            ...
                            "Text style synchronization inputs",
                        );
                    if (name === "device") {
                        c.assertExpressionShape(
                            variable.initializer!,
                    ...
                    if (name === "view") {
                        ...
                        if (
                            ...
                            c.contractError(
                                variable,
                                "Text instance upload view changed.",
                            );
                        c.assertExpressionShape(
                            call.expression,
                            "data._instances.subarray",
                            "Text instance upload source",
                        );
                ...
                if (ts.isBinaryExpression(expression)) {
                    const path = c.propertyPath(expression.left)?.join(".");
                    if (
                        path === "data._dirtyStart" ||
                        path === "data._dirtyEnd"
                    ) {
                        c.assertExpressionShape(
                            expression,
                            `${path}=0`,
                            "Text dirty range reset after upload",
                        );
                        return [
                ...
                if (
                    ...
                    c.assertExpressionShape(
                        expression,
                        `g._bindGroup=device.createBindGroup({label:"text-bg0-"+g._curveSetId,layout:bindGroupLayout,entries:[
                ...
                if (ts.isCallExpression(expression)) {
                    if (
                        ...
                        if (expression.arguments.length !== 5)
                            c.contractError(
                                expression,
                                "Text instance upload gained arguments.",
                            );
    ...
    private draw(): string {
        ...
        const body = this.body(
            ...
            new Map([
                [
                    "pass.setPipeline",
                    (args) => `ops.set_pipeline(${args.join(", ")})`,
                ],
                [
                    "pass.draw",
                    (args) =>
                        `ops.draw(${args.map((arg) => `text_resource_size(${arg})`).join(", ")})`,
                ],
            ]),
            (statement, lowerer, indent) => {
                if (ts.isVariableStatement(statement)) {
                    ...
                    if (name === "base" || name === "bound" || name === "p") {
                        if (!variable.initializer)
                            c.contractError(
                                variable,
                                "Text pipeline alias has no initializer.",
                            );
                        if (name === "base")
                            c.assertExpressionShape(
                                variable.initializer,
                                "gpu._pipeline",
                                "Text base pipeline snapshot",
                            );
                        if (name === "bound")
                            c.assertExpressionShape(
                                variable.initializer,
                                "base",
                                "Text bound pipeline snapshot",
                            );
                        if (name === "p")
                            c.assertExpressionShape(
                                variable.initializer,
                                "g._groupKey === g._curveSetId ? base : gpu._variantPipeline",
                                "Text group pipeline selection",
                            );
                        return [
                            `${indent}${name === "bound" ? "auto" : "const auto"} ${name} = ${lowerer.expression(variable.initializer)};`,
                        ];
                    }
                }
                if (
                    ...
                    if (
                        ...
                        if (expression.arguments[0]?.getText() === "0") {
                            c.assertExpressionShape(
                                ...
                                "Text quad vertex binding",
                            );
                            return [
                                `${indent}ops.set_quad_vertex_buffer(quad);`,
                            ];
                        }
                        c.assertExpressionShape(
                            expression,
                            "pass.setVertexBuffer(1, gpu._instanceBuf)",
                            "Text instance vertex binding",
                        );
                        return [
                            `${indent}ops.set_instance_vertex_buffer(gpu);`,
                        ];
                    }
                    if (
                        c.propertyPath(expression.expression)?.join(".") ===
