...
export function lowerAssetSceneAttachment(context: LoweringContext): string {
    ...
    if (
        ...
        context.contractError(
            declaration,
            "Expected the source AssetContainer attachment arm.",
        );
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "result.clearColor",
            {
                cpp: "record.clear_color",
                type: "opaque",
                absentCpp: "!record.has_clear_color",
            },
        ],
        ...
        [
            "ctx.camera",
            {
                cpp: "scene.camera",
                type: "opaque",
                absentCpp: "scene.camera.value == invalid_handle",
            },
        ],
        [
            "result.animationGroups",
            { cpp: "record.animation_groups", type: "opaque" },
        ],
        [
            "result.animationGroups?.length",
            { cpp: "record.animation_groups.size()", type: "scalar" },
        ],
        [
            "result._beforeRenderHook",
            { cpp: "record.before_render_hook", type: "opaque" },
        ],
        ...
        branch.thenStatement.statements,
        {
            ...
            returnValue: (expression) => {
                if (expression)
                    context.contractError(
                        expression,
                        "Expected void asset attachment.",
                    );
                return "";
            },
            statement(statement, lowerer, indent) {
                if (
                    ...
                    if (
                        ...
                        context.contractError(
                            statement,
                            "Expected one container entity binding.",
                        );
                    const entity = statement.initializer.declarations[0]!;
                    if (
                        !ts.isIdentifier(entity.name) ||
                        entity.name.text !== "e" ||
                        entity.initializer
                    )
                        context.contractError(
                            entity,
                            "Unrepresented container entity binding.",
                        );
                    const calls = ts.isBlock(statement.statement)
                        ? statement.statement.statements
                        : [statement.statement];
                    if (
                        calls.length !== 1 ||
                        !ts.isExpressionStatement(calls[0]!)
                    )
                        context.contractError(
                            statement,
                            "Unrepresented container entity traversal.",
                        );
                    context.assertExpressionShape(
                        calls[0].expression,
                        "addToScene(scene, e)",
                        "Container entity attachment",
                    );
                    return [
                        `${indent}add_asset_meshes(scene, record);`,
                ...
                if (
                    ...
                    if (name === "hook") {
                        ...
                        return [
                            `${indent}js::Callback<void(float)> hook = [tick = record.animation_tick](float delta_ms) { if (tick) tick(delta_ms); };`,
                        ];
                    }
                }
                ...
                if (
                    ...
                    if (
                        ...
                        context.contractError(
                            expression,
                            "Expected a container animation-group spread.",
                        );
                    const groups = lowerer.expression(spread.expression);
                    return [
                        `${indent}scene.animation_groups.insert(scene.animation_groups.end(), ${groups}.begin(), ${groups}.end());`,
                    ];
                }
                if (
                    context.expressionMatchesShape(
                        expression.expression,
                        "ctx._beforeRender.push",
                    )
                ) {
                    if (expression.arguments.length !== 1)
                        context.contractError(
                            expression,
                            "Expected one before-render hook.",
                        );
                    return [
                        `${indent}scene.before_render.push_back(${lowerer.expression(expression.arguments[0]!)});`,
                    ];
                }
                if (
                    context.expressionMatchesShape(
                    ...
                    context.assertExpressionShape(
                        ...
                        "Container scene setup call",
                    );
                    return [
                        `${indent}if (record.scene_setup) record.scene_setup(scene);`,
                    ];
                }
                return undefined;
            },
