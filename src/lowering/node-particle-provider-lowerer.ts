...
export function lowerNodeParticleProviderShared(
    context: LoweringContext,
): string {
    const copy = lowerPinnedFunction(
        context,
        module,
        "copyMatrix",
        [
        ...
        {
            cppName: "npe_copy_matrix",
            returns: "void",
            templateParameters: ["typename Source"],
        },
    );
    ...
    context.assertExpressionShape(
        context.variableInitializer(declaration, "provided"),
        ...
        [
            {
                ...
                cpp: "provider",
                kind: "record",
                annotation: "NodeParticleEmitterProvider",
                cppType: "bbl::js::Callback<bbl::js::F32Array()>",
                binding: { cpp: "provider", type: "opaque" },
            },
        ],
        {
            ...
            returns: {
                ...
                value: (lowerer, expression) => lowerer.expression(expression!),
            },
            memberBindings: new Map([
                [
                    "provided",
                    { cpp: "provided", type: "f32", staticBoolean: true },
                ],
            ]),
            calls,
        },
 ...
 */
export function lowerNodeParticleProviderState(
    context: LoweringContext,
): string {
    ...
    const property = context
        .findNodes(declaration, ts.isPropertyAssignment)
        .find((node) => node.name.getText(file) === "_setupEmitter");
    if (
        !property ||
        ...
        context.contractError(
            declaration,
            "Expected the provider setup closure.",
        );
    }
    ...
    const initializers = new Map<string, string | undefined>([
        ...
        ["emitterWorldMatrix", "allocateMat4()"],
        ["emitterInverseWorldMatrices", "[]"],
        ["nextMatrix", "allocateMat4()"],
        ["inverseScratch", undefined],
        ["prepareFrame", "system._prepareFrame"],
    ]);
    const statements: ts.Statement[] = [];
    let frame: ts.ArrowFunction | undefined;
    const links = new Set([
        "state.emitterWorldMatrix",
        "state.emitterInverseWorldMatrices",
    ]);
    for (const statement of setup.statements) {
        if (ts.isVariableStatement(statement)) {
            for (const variable of statement.declarationList.declarations) {
                ...
                if (!initializers.has(name))
                    context.contractError(
                        variable,
                        "Unexpected provider setup storage.",
                    );
                ...
                if (expected === undefined) {
                    if (variable.initializer)
                        context.contractError(
                            variable,
                            "Provider inverse scratch is no longer lazy.",
                        );
                    ...
                    context.contractError(
                        variable,
                        `Missing provider ${name} initialization.`,
                    );
        ...
        if (
            ...
            if (links.delete(target)) {
                context.assertExpressionShape(
                    expression.right,
                    target.slice("state.".length),
                    "provider stable reference",
                );
                continue;
            }
            if (
                target === "system._prepareFrame" &&
            ...
            context.contractError(
                expression,
                "Unexpected provider setup assignment.",
            );
        }
        statements.push(statement);
    }
    if (
        ...
        links.size ||
        !frame ||
        !ts.isBlock(frame.body) ||
        frame.parameters.length
    ) {
        context.contractError(
            setup,
            "Provider setup storage or frame-hook contract changed.",
        );
    }
    const bindings = (): Map<string, PinnedBinding> =>
        new Map([
            ...
            [
                "emitterWorldMatrix",
                { cpp: "state.emitter_world_matrix", type: "f32" },
            ],
            ["nextMatrix", { cpp: "state.next_emitter_matrix", type: "f32" }],
            ["initialMatrix", { cpp: "snapshot", type: "f32" }],
            [
                "emitterInverseWorldMatrices",
                { cpp: "/* empty inverse list */", type: "opaque" },
            ],
            [
                "emitterInverseWorldMatrices.length",
                { cpp: "0.0", type: "scalar", staticNumber: 0 },
            ],
            ["inverseScratch", absentBinding()],
            ["prepareFrame", absentBinding()],
    ...
    const calls = new Map<string, (args: readonly string[]) => string>([
        ...
        [
            "mat4GetTranslationToRef",
            (args) => `npe_provider_translation(${args.join(", ")})`,
        ],
    ]);
    const lower = (body: readonly ts.Statement[]): string =>
