import { outlineLoweredBody } from "../../compiler/lowered-body.js";
...
${lowered.iblLoading}${
    ...
    const GltfMatrixReader read_matrix{buffer, container, views};
${
    animationPointer
...
${nodeTransforms ? gltfNodeHierarchyCpp() : ""}`,
        {
            captures: [
                { name: "engine", type: "Engine" },
                { name: "planned_meshes", type: "const JsonArray" },
                ...
                {
                    name: "container",
                    type: "const upstream::ParsedGlbContainer",
                },
                { name: "views", type: "std::vector<BufferViewInfo>" },
                { name: "read_matrix", type: "const GltfMatrixReader" },
                { name: "document", type: "const JsonObject" },
                { name: "materials", type: "std::vector<MaterialHandle>" },
                { name: "asset", type: "AssetRecord" },
                {
                    name: "animation_mesh_indices",
                    type: "std::vector<std::size_t>",
                },
                { name: "mesh_plan", type: "const JsonObject" },
                ...(nodeTransforms
                    ? [
                          {
                              name: "node_mesh_children",
                              type: "std::vector<std::vector<MeshHandle>>",
                          },
                      ]
                    : []),
            ],
            code: `    for (std::size_t gltf_mesh_counter = 0; gltf_mesh_counter < planned_meshes.size(); ++gltf_mesh_counter) {
...
struct GltfMatrixReader {
    const ts::ArrayBuffer& buffer;
    const upstream::ParsedGlbContainer& container;
    const std::vector<BufferViewInfo>& views;
    Matrix operator()(const AccessorInfo& value, std::size_t index) const {
        Matrix matrix{};
