...
export function lowerBabylonHierarchy(context: LoweringContext): string {
    ...
    const loop = loops[0];
    if (loops.length !== 1 || !loop)
        context.contractError(
            declaration,
            "Expected the Babylon hierarchy pass.",
        );
    const rootDeclaration = context.variableInitializer(
        declaration,
        "rootMeshes",
    ).parent.parent.parent;
    if (
        !ts.isVariableStatement(rootDeclaration) ||
        rootDeclaration.parent !== declaration.body
    )
        context.contractError(
            declaration,
            "Expected root filtering after hierarchy construction.",
        );
    const rootStatements = declaration.body.statements.slice(
        declaration.body.statements.indexOf(rootDeclaration),
    );
    const bindings = new Map<string, PinnedBinding>([
        [
            "md.parentId",
            {
                cpp: 'string_or(md, "parentId")',
                type: "opaque",
                absentCpp: 'string_or(md, "parentId").empty()',
            },
        ],
        [
            "parent",
            {
                cpp: "parent",
                type: "opaque",
                absentCpp: "parent == invalid_handle",
            },
        ],
        [
            "childNode",
            {
                cpp: "child_node",
                type: "opaque",
                absentCpp: "child_node == invalid_handle",
            },
        ],
    ...
    const expression = (
        ...
        if (context.expressionMatchesShape(node, "md.id"))
            return 'string_or(md, "id")';
        if (context.expressionMatchesShape(node, "m.id"))
            return "nodes.at(m).id";
        if (ts.isCallExpression(node) && node.arguments.length === 1) {
            ...
            if (
                ...
                return `child_node_ids.contains(${key()})`;
            if (
                context.expressionMatchesShape(
                    node.expression,
                    "meshesByNodeId.has",
                ...
                return `meshes_by_id.contains(${key()})`;
            if (context.expressionMatchesShape(node.expression, "nodeMap.get"))
                return `find_babylon_node(node_map, ${key()})`;
        }
        return undefined;
    };
    const body = lowerPinnedBody(file, [loop, ...rootStatements], {
        ...
        forOf(iterated, element) {
            const range =
                iterated === "data.meshes"
                    ? "mesh_sources"
                    : iterated === "childMeshes"
                      ? "child_meshes"
                      : undefined;
            return range
                ? {
        ...
        statement(statement, lowerer, indent) {
            if (
                ...
                if (name === "childMeshes") {
                    context.assertExpressionShape(
                        ...
                        "meshesByNodeId.get(md.id) ?? []",
                        "Babylon child mesh lookup",
                    );
                    return [
                        `${indent}const auto found = meshes_by_id.find(string_or(md, "id"));`,
                        `${indent}const auto child_meshes = found != meshes_by_id.end() ? found->second : std::vector<std::size_t>{};`,
                    ];
                }
                if (name === "rootMeshes") {
                    const filter = context.unwrapExpression(
                        variable.initializer,
                    );
                    if (
                        ...
                        ts.isBlock(filter.arguments[0].body)
                    )
                        context.contractError(
                            filter,
                            "Expected the mesh root filter.",
                        );
                    const predicate = filter.arguments[0];
                    if (
                        predicate.parameters.length !== 1 ||
                        !ts.isIdentifier(predicate.parameters[0]!.name) ||
                        predicate.parameters[0]!.name.text !== "m"
                    )
                        context.contractError(
                            predicate,
                            "Expected the mesh root filter parameter.",
                        );
                    return [
                        `${indent}std::vector<std::size_t> root_meshes;`,
                ...
                if (name === "rootTransformNodes") {
                    context.assertExpressionShape(
                        variable.initializer,
                        "[]",
                        "Babylon container root list",
                    );
                    return [
                        `${indent}std::vector<std::size_t> root_containers;`,
                    ];
                }
            }
            if (
                ts.isForOfStatement(statement) &&
                context.expressionMatchesShape(statement.expression, "nodeMap")
            ) {
                if (
                ...
                if (
                    ...
                    context.contractError(
                        statement,
                        "Expected the node map's ID and node bindings.",
                    );
                ...
                return [
                    `${indent}for (const auto& [${names.join(", ")}] : node_map) {`,
                    ...lowerer.statements(
            ...
            if (ts.isReturnStatement(statement)) {
                if (
                    ...
                    context.contractError(
                        statement,
                        "Expected the Babylon asset result.",
                    );
                const entities = context.unwrapExpression(
                    context.propertyInitializer(
                        statement.expression,
                        "entities",
                    ),
                );
                if (!ts.isArrayLiteralExpression(entities))
                    context.contractError(
                        entities,
                        "Expected Babylon entity ordering.",
                    );
                const ranges = entities.elements.flatMap((element) => {
                    ...
                    const range =
                        name === "rootMeshes"
                            ? "root_meshes"
                            : name === "rootTransformNodes"
                              ? "root_containers"
                              : undefined;
                    if (!range)
                        context.contractError(
                            element,
                            "Unknown Babylon root entity collection.",
                        );
                    return [
                        `${indent}roots.insert(roots.end(), ${range}.begin(), ${range}.end());`,
                    ];
                });
                return [
                    `${indent}std::vector<std::size_t> roots;`,
                    ...ranges,
                    `${indent}return roots;`,
                ];
            }
            ...
            if (
                ...
                node.left.name.text === "parent"
            )
                return [
                    `${indent}nodes.at(${lowerer.expression(node.left.expression)}).parent = ${lowerer.expression(node.right)};`,
                ];
            if (ts.isCallExpression(node) && node.arguments.length === 1) {
                const argument = (): string =>
                    lowerer.expression(node.arguments[0]!);
                if (
                    context.expressionMatchesShape(
                        node.expression,
                        "childNodeIds.add",
                    ...
                    return [`${indent}child_node_ids.insert(${argument()});`];
                if (
                    context.expressionMatchesShape(
                        node.expression,
                    ...
                    return [
                        `${indent}nodes.at(parent).children.push_back(${argument()});`,
                    ];
                if (
                    context.expressionMatchesShape(
                        node.expression,
                        "rootTransformNodes.push",
                    )
                )
                    return [
                        `${indent}root_containers.push_back(${argument()});`,
                    ];
            }
            return undefined;
        },
