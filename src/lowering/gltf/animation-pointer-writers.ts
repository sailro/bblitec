...
export function lowerGltfAnimationPointerWriters(context: LoweringContext): {
    source: string;
    ...
    const helper = (module: string, name: string): string | undefined => {
        ...
        for (const call of context.findNodes(
            declaration.body!,
            (node): node is ts.CallExpression => ts.isCallExpression(node),
        ))
            if (ts.isIdentifier(call.expression))
                helper(module, call.expression.text);
        return cpp;
    };
    const bodies: string[] = [];
    for (const module of modules)
        for (const closure of gltfPointerClosures(context, module)) {
            const parameters = closure.declaration.parameters.map(
                (parameter) => {
                    if (!ts.isIdentifier(parameter.name))
                        context.contractError(
                            parameter,
                            "Expected a named source pointer writer parameter.",
                        );
                    return parameter.name.text;
                },
            );
            if (parameters.length !== (closure.kind === "lookup" ? 0 : 2))
                context.contractError(
                    closure.declaration,
                    "Unrepresented source pointer writer arity.",
                );
            const calls = context.findNodes(
                closure.declaration.body,
                (node): node is ts.CallExpression => ts.isCallExpression(node),
            );
            const effects = calls.some(
                (call) =>
                    ...
                    (ts.isPropertyAccessExpression(call.expression) &&
                        call.expression.name.text === "_bumpLightVersion") ||
                    (ts.isIdentifier(call.expression) &&
                        closure.captures.includes(call.expression.text)),
            );
            const cpp = `gltf_animation_pointer_${writers.length}`;
            const names = [
                ...(effects ? ["__pointerEffects"] : []),
                ...closure.captures,
                ...parameters,
            ];
            if (new Set(names).size !== names.length)
                context.contractError(
                    closure.declaration,
                    "Ambiguous source pointer writer capture names.",
                );
            const body = closure.declaration.body;
            const text =
                `function ${cpp}(${names.join(", ")}): ${closure.kind === "lookup" ? "unknown" : "void"} ` +
                (ts.isBlock(body)
                    ? body.getText()
                    : `{ return ${body.getText()}; }`);
            ...
            if (!declaration || !ts.isFunctionDeclaration(declaration))
                context.contractError(
                    body,
                    "Expected a source pointer writer function.",
                );
            const target: GltfMaterialFunction = {
                module,
                name: cpp,
                cpp,
                declaration,
                sourceSymbol: `writer@${closure.declaration.getStart()}`,
                ...(effects
                    ? {
                          contextParameter: "__pointerEffects",
                          contextType: "GltfAnimationPointerEffects",
                      }
                    : {}),
            };
            bodies.push(
                    ...
                    (call, lowerer) => {
                        ...
                        if (
                            ...
                            if (call.arguments.length !== 2)
                                context.contractError(
                                    call,
                                    "Expected source visibility target and value.",
                                );
                        ...
                        if (
                            ...
                            if (call.arguments.length !== 2)
                                context.contractError(
                                    call,
                                    "Expected source light lookup owner and index.",
                                );
                        ...
                        if (
                            ...
                            if (
                                ...
                                context.contractError(
                                    call,
                                    "Expected the source optional light-version callback.",
                                );
                        ...
                        if (
                            ...
                            if (call.arguments.length)
                                context.contractError(
                                    call,
                                    "Unrepresented source pointer helper invocation.",
                                );
    ...
    const helperBodies = [...helpers.values()]
        .reverse()
        .map((target) =>
            lowerGltfMaterialObjectFunction(context, target, (name) =>
                helper(target.module, name),
            ),
        );
    return {
        writers,
        source: `struct GltfAnimationPointerEffects {
...
${writers
    .map(
        (
            writer,
        ) => `    if (site == ${JSON.stringify(writer.site)}) return +[]([[maybe_unused]] const GltfAnimationPointerEffects& effects,
        [[maybe_unused]] const GltfPbrValue& captures, [[maybe_unused]] GltfPbrValue output, [[maybe_unused]] GltfPbrValue offset) { return ${writer.cpp}(${[
            ...
            ...writer.captures.map(
                (name) => `captures.get(${JSON.stringify(name)})`,
            ),
