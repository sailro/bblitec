...
function remainderStore(
    ...
    return [
        `${indent}${target} = ${pinnedRemainderCall(target, lowerer.expression(statement.expression.right))};`,
    ];
}

/** Playback state/control flow; the caller supplies the source pose/writeback boundary. */
export function lowerGltfAnimationPlayback(
    context: LoweringContext,
    weighted = false,
): string {
    const controller = context.functionDeclaration(
        controllerModule,
        "createAnimationController",
    );
    const value = context.unwrapExpression(
        context.variableInitializer(controller.declaration, "ctrl"),
    );
    if (!ts.isObjectLiteralExpression(value))
        context.contractError(value, "Expected animation controller state.");
    const fields = new Map(
        value.properties
            .filter(ts.isPropertyAssignment)
            .map((property) => [context.propertyName(property.name), property]),
    );
    const selectedTick = fields.get("tick")?.initializer;
    if (
        !selectedTick ||
        !ts.isConditionalExpression(selectedTick) ||
        !ts.isArrowFunction(selectedTick.whenFalse) ||
        !ts.isBlock(selectedTick.whenFalse.body)
    )
        context.contractError(
            value,
            "Expected source animation controller tick selection.",
        );
    context.assertExpressionShape(
        selectedTick.whenTrue,
        "noopAnimationTick",
        "Zero-duration controller tick",
    );
    const noop = context.functionDeclaration(
        controllerModule,
        "noopAnimationTick",
    );
    context.assertFunctionBodyShape(
        noop.declaration,
        "{}",
        "Zero-duration controller specialization",
    );
    context.assertExpressionShape(
        context.variableInitializer(controller.declaration, "uploadGpu"),
        "true",
        "Initial animation upload state",
    );
    const cache = context.findNodes(
        controller.declaration,
        (node): node is ts.VariableDeclaration =>
            ts.isVariableDeclaration(node) &&
            ts.isIdentifier(node.name) &&
            node.name.text === "cachedEngine",
    );
    if (cache.length !== 1 || cache[0]!.initializer)
        context.contractError(
            controller.declaration,
            "Expected initially absent animation engine cache.",
        );
    ...
    if (end < 0)
        context.contractError(
            selectedTick,
            "Expected controller pose-time boundary.",
        );
    const bindings = (): Map<string, PinnedBinding> =>
        new Map([
            ...
            [
                "group._ctrl",
                { cpp: "ctrl", type: "opaque", staticBoolean: true },
            ],
            ...
            [
                "cachedEngine",
                {
                    cpp: "ctrl.cached_engine",
                    type: "bool",
                    absentCpp: "!ctrl.cached_engine",
                },
            ],
    ...
    const controllerScope = {
        ...
        expression(node: ts.Expression, lowerer: PinnedNumericLowerer) {
            if (
                ...
                context.assertExpressionShape(
                    node.left,
                    "engine",
                    "Optional animation engine identity",
                );
        ...
        statement(
            ...
            if (!ts.isIdentifier(variable.name) || !variable.initializer)
                return undefined;
            if (variable.name.text === "activeEngine")
                return [
                    `${indent}const bool active_engine = ${lowerer.expression(variable.initializer)};`,
                ];
            if (variable.name.text === "device") {
                context.assertExpressionShape(
                    variable.initializer,
                    "requiresEngine && uploadGpu ? activeEngine!._device : null",
                    "Animation device transport",
                );
                return [];
            }
            return undefined;
    ...
    const skip = lowerPinnedBody(
        controller.file,
        [
            ts.factory.createIfStatement(
                selectedTick.condition,
                ts.factory.createReturnStatement(),
            ),
        ],
        controllerScope,
    );
    const advance = lowerPinnedBody(
        controller.file,
        statements.slice(0, end + 1),
        controllerScope,
    );
    const state = [
        ["time", "double", "time"],
        ["playing", "bool", "playing"],
        ["speedRatio", "double", "speed_ratio"],
        ["loop", "bool", "loop"],
    ]
        .map(([field, type, cpp]) => {
            ...
            if (!initializer)
                context.contractError(
                    value,
                    `Expected controller ${field} initializer.`,
                );
            const initial =
                type === "bool"
            ...
            if (initial === undefined)
                context.contractError(
                    initializer,
                    "Expected a static controller state initializer.",
                );
        ...
        return lowerPinnedBody(file, declaration.body!.statements, {
            ...
            expression(node, lowerer) {
                if (
                    ...
                    context.assertExpressionShape(
                        ...
                        "Playback controller synchronization identity",
                    );
                }
                if (
                    ts.isCallExpression(node) &&
                    ...
                    if (node.arguments.length < 2 || node.arguments.length > 3)
                        context.contractError(
                            node,
                            "Changed playback core arguments.",
                        );
                    context.assertExpressionShape(
                        node.arguments[0]!,
                        "group",
                        "Playback core group identity",
                    );
                }
                if (
                    ts.isBinaryExpression(node) &&
            ...
            calls: new Map([
                [
                    "syncControllerFromGroup",
                    () =>
                        "gltf_sync_animation_controller(group, ctrl, speed_ratio, sync_mask)",
                ],
                [
                    "group._ctrl.tick",
                    (args: readonly string[]) =>
                        `gltf_tick_animation_controller(group, ctrl, ${args[0]}, ${args[1] ?? "false"}, requires_engine, upload_gpu, pose)`,
                ],
                [
                    "ctrl.tick",
                    (args: readonly string[]) =>
                ...
                [
                    "tickAnimationCore",
                    (args: readonly string[]) =>
                        `gltf_tick_animation_core(group, ${args[1]}, speed_ratio, ${args[2] ?? "false"}, requires_engine, upload_gpu, sync_mask, pose)`,
                ],
            ]),
            statement(statement, _lowerer, indent) {
                if (
                    ...
                    if (
                        ts.isIdentifier(variable.name) &&
                        variable.name.text === "ctrl" &&
                        variable.initializer
                    ) {
                        context.assertExpressionShape(
                            variable.initializer,
                            "group._ctrl",
                            "Animation controller identity",
                        );
                        return [];
                    }
                }
                if (
                    ...
                    context.assertExpressionShape(
                        ...
                        "Animation mask synchronization boundary",
                    );
    ...
    const groupValue = context.unwrapExpression(
        context.variableInitializer(factory.declaration, "group"),
    );
    if (!ts.isObjectLiteralExpression(groupValue))
        context.contractError(
            groupValue,
            "Expected source glTF animation group.",
        );
    const ctrlProperty = groupValue.properties.find(
        (property) =>
            ts.isPropertyAssignment(property) &&
            context.propertyName(property.name) === "_ctrl",
    );
    if (!ctrlProperty || !ts.isPropertyAssignment(ctrlProperty))
        context.contractError(
            groupValue,
            "Expected the glTF group controller.",
        );
    context.assertExpressionShape(
        ctrlProperty.initializer,
        "ctrl",
        "Factory controller identity",
    );
    const mixerAssignment = context.findNodes(
        factory.declaration,
        (node): node is ts.BinaryExpression =>
            ts.isBinaryExpression(node) &&
            context.expressionMatchesShape(node.left, "group._gltfMixer"),
    );
    if (mixerAssignment.length !== 1)
        context.contractError(
            factory.declaration,
            "Expected source glTF group mixer publication.",
        );
    context.assertExpressionShape(
        mixerAssignment[0]!.right,
        "[clip, nodes, skeletons]",
        "Factory mixer identity",
    );
              ...
              return lowerPinnedBody(file, declaration.body!.statements, {
                  ...
                  statement(statement, lowerer, indent) {
                      ...
                      context.assertExpressionShape(
                          variable.initializer,
                          "mixer[GLTF_CLIP]",
                          "Weighted animation clip identity",
                      );
                      return [];
                  },
...
${
    weightedBody === undefined
        ? ""
        : `// ${context.provenance(mixerModule, "advanceGroupTime")}
