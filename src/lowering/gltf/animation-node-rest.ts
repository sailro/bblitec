...
export function lowerGltfAnimationNodeRest(context: LoweringContext): string {
    ...
    if (start < 0 || end <= start)
        context.contractError(
            declaration,
            "Expected the complete animation node-rest section.",
        );
    ...
    const body = lowerPinnedBody(file, statements.slice(start, end), {
        ...
        expression(node, lowerer) {
            ...
            if (
                ...
                context.expressionMatchesShape(node.expression, "findParent")
            ) {
                if (node.arguments.length !== 2)
                    context.contractError(
                        node,
                        "Expected animation parent lookup arguments.",
                    );
        ...
        statement(statement, lowerer, indent) {
            if (
                ...
                if (
                    ...
                    variable.type?.getText(file) === "NodeRest[]"
                ) {
                ...
                if (
                    ...
                    const value = lowerer.expression(
                ...
                if (
                    ...
                    if (!ts.isArrayLiteralExpression(fallback))
                        context.contractError(
                            fallback,
                            "Expected an animation TRS array default.",
                        );
            ...
            if (ts.isExpressionStatement(statement)) {
                ...
                if (
                    ...
                    context.contractError(
                        call,
                        "Expected a node-rest record append.",
                    );
                ...
                for (const property of value.properties) {
                    if (!ts.isPropertyAssignment(property))
                        context.contractError(
                            property,
                            "Expected a named node-rest field.",
                        );
                    const key = context.propertyName(property.name);
                    if (!key || !fields.includes(key) || entries.has(key))
                        context.contractError(
                            property,
                ...
                if (entries.size !== fields.length)
                    context.contractError(
                        value,
                        "Expected all node-rest fields.",
                    );
                // Preserve initializer evaluation order independently of native record layout.
                return [
                    `${indent}{`,
                    `${indent}    GltfNodeRest rest;`,
                    ...[...entries].map(
                        ([key, expression]) =>
                            `${indent}    rest.${key === "_matrix" ? "matrix" : key} = ${lowerer.expression(expression)};`,
                    ),
                    `${indent}    nodes.push_back(rest);`,
                    `${indent}}`,
                ];
            }
            return undefined;
