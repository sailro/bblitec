...
export function lowerGltfDefaultSampler(
    ...
    if (
        ...
        context.contractError(
            declaration,
            "Expected the loader's shared sampler creation.",
        );
    const descriptor = call.arguments[1]!;
    if (!ts.isObjectLiteralExpression(descriptor))
        context.contractError(
            descriptor,
            "Expected a shared sampler descriptor.",
        );
    for (const property of descriptor.properties) {
        ...
        if (!gltfSamplerFields.some(([field]) => field === name))
            context.contractError(
                property,
                "Unrepresented shared sampler field.",
            );
    }
    context.assertExpressionShape(
        call.arguments[0]!,
        "engine",
        "Shared sampler engine",
    );
    const file = ts.createSourceFile(
        module,
        `function ${prefix}_descriptor() { return ${descriptor.getText()}; }`,
    ...
    if (!selected || !ts.isFunctionDeclaration(selected))
        context.contractError(
            descriptor,
            "Expected a sampler descriptor function.",
        );
    return (
        lowerGltfMaterialObjectFunction(
            ...
            {
                module,
                name: `${prefix}_descriptor`,
                cpp: `${prefix}_descriptor`,
