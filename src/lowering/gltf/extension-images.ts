...
export function lowerGltfExtensionImages(
        ...
        (statement) =>
            ...
            statement.declarationList.declarations.some(
                (variable) =>
                    ...
                    variable.name.text ===
                        (variant ? "imageCache" : "extImageCache"),
            ),
    );
    if (first < 0)
        context.contractError(
            declaration,
            "Expected extension image cache initialization.",
        );
    const bindings = new Map<string, PinnedBinding>([
        ["matExts.length", { cpp: "extension_count", type: "scalar" }],
    ]);
        ...
        statement(statement, lowerer, indent) {
            ...
            if (variant) {
                if (name === "imageCache") {
                    if (
                        ...
                        context.contractError(
                            initializer,
                            "Expected an empty variant image cache.",
                        );
                    return [];
                }
                if (name !== fetchName)
                    context.contractError(
                        variable,
                        "Expected variant image fetch state.",
                    );
                context.assertExpressionShape(
                    initializer,
                    "makeImageFetcher(json, binChunk, baseUrl, imageCache)",
                    "Variant image environment",
                );
                return [
                    `${indent}return [&json, &imageCache, resolve_image](const ts::JsonValue* info) {
            ...
            if (name === "extImageCache") {
                if (
                    ...
                    initializer.whenTrue.elements.length
                )
                    context.contractError(
                        initializer,
                        "Expected an empty extension image cache.",
                    );
                return [
                    `${indent}auto ${name} = ${condition} ? std::make_shared<GltfMaterialImageCache>() : nullptr;`,
                ];
            }
            if (name !== "extFetchImg")
                context.contractError(
                    variable,
                    "Unrepresented extension image binding.",
                );
            context.assertExpressionShape(
                initializer.whenTrue,
                "makeImageFetcher(json, binChunk, baseUrl, extImageCache)",
                "Extension image environment",
            );
            return [
                `${indent}if (!(${condition})) return {};
${indent}return [&json, cache = std::move(extImageCache), resolve_image](const ts::JsonValue* info) {
    ...
    const variables = context.findNodes(
        ...
        (node): node is ts.VariableDeclaration =>
            ...
            node.name.text === "extCtx",
    );
    const object =
        variables[0]?.initializer &&
        context.unwrapExpression(variables[0].initializer);
    if (
        variables.length !== 1 ||
        !object ||
        ...
        context.contractError(
            declaration,
            "Expected one material extension context.",
        );
    const methods = object.properties.filter(
        (property): property is ts.MethodDeclaration =>
            ts.isMethodDeclaration(property) &&
            ts.isIdentifier(property.name) &&
            property.name.text === "_texture",
    );
    const method = methods[0];
    if (methods.length !== 1 || !method?.body || method.parameters.length !== 2)
        context.contractError(object, "Expected the extension texture method.");
    const textureBindings = new Map<string, PinnedBinding>([
        [
            fetchName,
            { cpp: "fetch_image", type: "opaque", absentCpp: "!fetch_image" },
        ],
    ]);
    method.parameters.forEach((parameter, index) => {
        if (!ts.isIdentifier(parameter.name))
            context.contractError(
                parameter,
                "Expected a named extension texture parameter.",
            );
        textureBindings.set(
            parameter.name.text,
            index
                ? { cpp: "srgb", type: "bool" }
                : { cpp: "info", type: "opaque", absentCpp: "!info.truthy()" },
        );
    });
    const body = lowerPinnedBody(file, method.body.statements, {
        ...
        expression(node, lowerer) {
            ...
            if (ts.isAwaitExpression(node))
                return `(${lowerer.expression(node.expression)}).get()`;
            if (
                ts.isCallExpression(node) &&
                context.expressionMatchesShape(node.expression, fetchName) &&
            ...
            if (
                ...
                if (node.arguments.length !== 5)
                    context.contractError(
                        node,
                        "Expected variant image upload arguments.",
                    );
                for (const [index, shape] of [
                    ...
                    context.assertExpressionShape(
                        ...
                        shape,
                        "Variant image upload environment",
                    );
        ...
        statement(statement, lowerer, indent) {
            ...
            return statement.declarationList.declarations.map((variable) => {
                if (!ts.isIdentifier(variable.name) || !variable.initializer)
                    context.contractError(
                        variable,
                        "Expected an extension image result.",
                    );
                const name = variable.name.text,
    ...
    const uploads = object.properties.filter(
        (property): property is ts.MethodDeclaration =>
            ts.isMethodDeclaration(property) &&
            ts.isIdentifier(property.name) &&
            property.name.text === "_uploadImage",
    );
    const upload = uploads[0];
    if (uploads.length !== 1 || !upload?.body || upload.parameters.length !== 2)
        context.contractError(
            object,
            "Expected the extension bitmap upload method.",
        );
    ...
    upload.parameters.forEach((parameter, index) => {
        if (!ts.isIdentifier(parameter.name))
            context.contractError(
                parameter,
                "Expected a named bitmap upload parameter.",
            );
        uploadBindings.set(
            parameter.name.text,
            index
    ...
    const uploadBody = lowerPinnedBody(file, upload.body.statements, {
        ...
        expression(node, lowerer) {
            ...
            if (node.arguments.length !== 5)
                context.contractError(
                    node,
                    "Expected the bitmap upload boundary.",
                );
            for (const [index, shape] of [
                [0, "engine"],
                [3, "sampler"],
                [4, variant ? "generateMipmaps" : "_generateMipmaps!"],
            ] as const)
                context.assertExpressionShape(
                    node.arguments[index]!,
                    shape,
                    "Bitmap upload environment",
                );
