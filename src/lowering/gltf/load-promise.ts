...
public:
    ...
    explicit GltfLoadPromise(T value) : result_(std::in_place, std::in_place_index<0>, std::move(value)) {}
    explicit operator bool() const { return bool(result_); }
    const T& get() const {
        if (!result_) throw std::runtime_error("Cannot await an absent glTF promise.");
        if (const auto* error = std::get_if<std::exception_ptr>(&*result_)) std::rethrow_exception(*error);
    ...
    template<class Resolve> static GltfLoadPromise settle(Resolve resolve) {
        try { return GltfLoadPromise{resolve()}; }
            ...
            promise.result_.emplace(std::in_place_index<1>, std::current_exception());
            return promise;
        }
    }
};`;
