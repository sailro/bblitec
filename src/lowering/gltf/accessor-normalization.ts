...
export function lowerAccessorNormalizationCpp(file: ts.SourceFile): string {
    ...
    if (parameters.length !== 4)
        refuseNode(
            ...
            file,
            declaration,
            "requires a DataView, offset, component type and normalization flag",
        );
        ...
        methods: new Map(
            Object.entries(accessorReadsByGetter).map(([getter, read]) => [
                ...
                (receiver, args) => {
                    if (
                        receiver !== "view" ||
                        args.length !== (read.littleEndian ? 2 : 1) ||
                        (read.littleEndian && args[1] !== "true")
                    )
                        refuseNode(
                            "readComponent",
                            file,
                            declaration,
                            `requires the native little-endian '${getter}' read`,
                        );
        ...
        statement(statement, lowerer, indent) {
            ...
            if (
                ...
                refuseNode(
                    ...
                    file,
                    statement,
                    "throws an unrepresented error",
                );
            return [
                `${indent}throw std::runtime_error(${lowerer.expression(value.arguments[0]!)});`,
            ];
        },
