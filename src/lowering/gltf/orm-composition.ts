...
export function lowerGltfOrmComposition(context: LoweringContext): string {
    ...
    const methods = new Map<
        string,
        {
            receiver: "canvas" | "context";
            cpp: string;
            count: number;
            result?: "bitmap" | "context";
        }
    >([
        [
            "getContext",
            { receiver: "canvas", cpp: "context", count: 1, result: "context" },
        ],
        ...
        [
            "putImageData",
            { receiver: "context", cpp: "put_image_data", count: 3 },
        ],
    ]);
        ...
        expression(node, lowerer) {
            if (
                ...
                if (node.arguments?.length !== 2)
                    context.contractError(
                        node,
                        "Expected image canvas dimensions.",
                    );
            ...
            if (context.expressionMatchesShape(callee, "createImageBitmap")) {
                ...
                if (
                    ...
                    shapes.get(source.text) !== "canvas"
                )
                    context.contractError(
                        node,
                        "Expected a canvas bitmap snapshot.",
                    );
            ...
            if (
                shapes.get(callee.expression.text) !== method.receiver ||
                node.arguments.length !== method.count
            )
                ...
                if (
                    ...
                    if (target?.type === "u8") {
                        if (
                            ...
                            context.contractError(
                                expression,
                                "Unsupported image data update.",
                            );
                        return [
                            `${indent}pal::image_data_store(${target.cpp}, static_cast<double>(${lowerer.expression(expression.left.argumentExpression)}), ${lowerer.expression(expression.right)});`,
                        ];
                    }
                }
            }
    ...
    for (const update of context.findNodes(
        declaration,
        ...
        if (
            ...
            bindings.get(operand.expression.getText(file))?.type === "u8"
        )
