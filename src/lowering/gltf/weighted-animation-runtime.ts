...
export function lowerGltfWeightedAnimationRuntime(
    ...
    const quaternionNames = [
        "normalizeQuaternionAt",
        "quatSlerpInto",
        "quatRefInverseTimesSample",
        "applyAdditiveQuaternion",
    ];
    const quaternionCpp = new Map(
        quaternionNames.map((name) => [name, `gltf_weighted_${name}`]),
    );
    const quaternionBodies = quaternionNames.map((name) => {
        ...
        const parameters = declaration.parameters.map((parameter) => {
            if (!ts.isIdentifier(parameter.name) || !parameter.type)
                context.contractError(
                    parameter,
                    "Expected typed quaternion helper parameters.",
                );
            const array =
                ts.isTypeReferenceNode(parameter.type) &&
                ts.isIdentifier(parameter.type.typeName) &&
                parameter.type.typeName.text === "Float32Array";
            if (!array && parameter.type.kind !== ts.SyntaxKind.NumberKeyword)
                context.contractError(
                    parameter,
                    "Unsupported weighted quaternion transport.",
                );
...
${lowerPinnedBody(file, declaration.body!.statements, {
    ...
    const bindingScope = (): Map<string, PinnedBinding> => {
        ...
        for (const [name, cpp] of [
            ["trs", "target.currentTRS"],
            ["target.trs", "target.currentTRS"],
            ["localMat", "target.localMat"],
            ["worldMat", "target.worldMat"],
            ["_boneTmp", "target._boneTmp"],
            ["RH_TO_LH", "target.RH_TO_LH"],
            ["scratch.sample", "scratch.sample"],
            ["scratch.reference", "scratch.reference"],
            ["scratch.delta", "scratch.delta"],
            ["target.tWeight", "target.tWeight"],
            ["target.rWeight", "target.rWeight"],
            ["target.sWeight", "target.sWeight"],
            ["boneData", "boneData"],
            ["skel.invMeshWorld", "skel.invMeshWorld"],
            ["skel.inverseBindMatrices", "skel.inverseBindMatrices"],
        ])
            bind(name!, cpp!, "f32");
        for (const name of ["baseRot", "target.baseRot"])
            result.set(name, {
                cpp: "(*target.baseRot)",
                type: "f32",
                absentCpp: "!target.baseRot",
            });
        for (const name of ["target.nodes.length", "nodes.length"])
            bind(name, "static_cast<double>(target.nodes.size())");
        bind(
            "target.skeletons.length",
            "static_cast<double>(target.skeletons.size())",
        );
        bind(
            "clip.channels.length",
            "static_cast<double>(clip.channels.size())",
        );
        ...
        for (const member of ["nodeIdx", "samplerIdx", "path"])
            bind(`ch.${member}`, `ch.${member}`);
    ...
    const lower = (name: string): string => {
        ...
        const arrays = new Map([
            ...
            ["target.topoOrder", "target.topo_order"],
            ["clip.channels", "clip.channels"],
            ["clip.samplers", "clip.samplers"],
            ["target.skeletons", "target.skeletons"],
        ...
        return lowerPinnedBody(file, declaration.body!.statements, {
            ...
            methods: new Map([
                [
                    ...
                    (receiver, args, binding) => {
                        if (args.length !== 1 || binding.type !== "f32")
                            context.contractError(
                                declaration,
                                "Unsupported weighted scratch fill.",
                            );
            ...
            calls: new Map([
                ...
                [
                    "evaluateSampler",
                    (args: readonly string[]) =>
                        `evaluate_sampler(${args.join(", ")})`,
                ],
                [
                    "composeMat4IntoBuffer",
                    (args: readonly string[]) =>
                        `compose_matrix(${args.join(", ")})`,
                ],
                [
                    "multiplyMat4IntoBuffer",
                    (args: readonly string[]) =>
                        `multiply_matrix(${args.join(", ")})`,
                ],
            ]),
            expression(node, lowerer) {
                if (
                    ts.isConditionalExpression(node) &&
                    context.expressionMatchesShape(node.condition, "baseRot")
                )
                    return `(target.baseRot.has_value() ? ${lowerer.expression(node.whenTrue)} : ${lowerer.expression(node.whenFalse)})`;
                if (ts.isElementAccessExpression(node)) {
                    const array = [...arrays].find(([source]) =>
                    ...
                    if (array)
                        return `${array}.at(static_cast<std::size_t>(${lowerer.expression(node.argumentExpression)}))`;
                }
                if (
                    context.expressionMatchesShape(
                        node,
                ...
                if (context.expressionMatchesShape(node, "skel.boneMatrices"))
                    return "(*skel.boneMatrices)";
                if (!ts.isCallExpression(node)) return undefined;
                if (
                    context.expressionMatchesShape(node.expression, "getTarget")
                ) {
                    context.assertExpressionShape(
                        node,
                        "getTarget(scratch, mixer)",
                        "Weighted target storage identity",
                    );
                    return "get_target()";
                }
                if (
                    ...
                    if (node.arguments.length !== 3)
                        context.contractError(
                            node,
                            "Expected weighted group advancement arguments.",
                        );
                    context.assertExpressionShape(
                        node.arguments[0]!,
                        "group",
                        "Weighted group identity",
                    );
                    context.assertExpressionShape(
                        node.arguments[1]!,
                        "mixer",
                        "Weighted clip identity",
                    );
                ...
                if (
                    ...
                    if (node.arguments.length !== 3)
                        context.contractError(
                            node,
                            "Expected weighted mask lookup arguments.",
                        );
                    context.assertExpressionShape(
                        node.arguments[0]!,
                        "group",
                        "Weighted mask group identity",
                    );
            ...
            statement(statement, lowerer, indent) {
                if (ts.isSwitchStatement(statement) && !loweringSwitch) {
                    if (
                        context.findNodes(statement, ts.isContinueStatement)
                            .length
                    )
                        context.contractError(
                            statement,
                            "Weighted switch continuation needs its outer-loop identity preserved.",
                        );
                    loweringSwitch = true;
                    try {
                        return [
                            `${indent}do {`,
                            ...lowerer.statement(statement, indent + "    "),
                            `${indent}} while (false);`,
                        ];
                    } finally {
                        loweringSwitch = false;
                    }
                }
                if (
                    ...
                    if (ts.isObjectBindingPattern(variable.name)) {
                        if (!variable.initializer)
                            context.contractError(
                                variable,
                                "Expected weighted target destructuring.",
                            );
                        context.assertExpressionShape(
                            variable.initializer,
                            "target",
                            "Weighted target storage owner",
                        );
                        ...
                        for (const element of variable.name.elements)
                            if (
                                ...
                                !allowed.has(element.name.text)
                            )
                                context.contractError(
                                    element,
                                    "Unsupported weighted target destructuring.",
                                );
                        return [];
                    }
                    if (
                        ...
                        if (name === "device") {
                            context.assertExpressionShape(
                                variable.initializer,
                                "manager.engine._device",
                                "Weighted device transport",
                            );
                            return [];
                        }
                    }
                ...
                if (
                    ...
                    if (call.arguments.length < 3 || call.arguments.length > 4)
                        context.contractError(
                            call,
                            "Expected weighted bone override phases.",
                        );
                    context.assertExpressionShape(
                        call.arguments[0]!,
                        "overrides as ReadonlyMap<number, BoneOverride>",
                        "Weighted override identity",
                    );
                    return [
                        `${indent}apply_bone_overrides(${call.arguments
                            .slice(1)
                            .map((argument) => lowerer.expression(argument))
                            .join(
                                ", ",
                            )}${call.arguments.length === 3 ? ", false" : ""});`,
                    ];
                }
                if (
                    context.expressionMatchesShape(
                    ...
                    context.assertExpressionShape(
                        ...
                        "Weighted bone GPU upload transport",
                    );
                ...
                if (
                    ...
                    if (call.arguments.length !== 2)
                        context.contractError(
                            call,
                            "Expected authored matrix storage copy.",
                        );
                    context.assertExpressionShape(
                        call.arguments[0]!,
                        "node._matrix",
                        "Weighted authored matrix identity",
                    );
                    return [
                        `${indent}std::copy(node.matrix->begin(), node.matrix->end(), target.localMat.begin() + static_cast<std::size_t>(${lowerer.expression(call.arguments[1]!)}));`,
                    ];
                }
                return undefined;
            },
...
function lowerWeightedMask(context: LoweringContext): string {
    ...
    const bindings = new Map<string, PinnedBinding>([
        [
            "group.mask",
            { cpp: "group.mask", type: "opaque", absentCpp: "!group.mask" },
        ],
        ["mask", { cpp: "mask", type: "opaque", absentCpp: "!mask" }],
        ["mask.disabled", { cpp: "mask->disabled", type: "bool" }],
        ["mask.mode", { cpp: "mask->mode", type: "scalar" }],
        ["nodeIdx", { cpp: "node_index", type: "scalar" }],
        ["channelIndex", { cpp: "channel_index", type: "scalar" }],
    ]);
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        expression(node) {
            if (
                context.expressionMatchesShape(
                    node,
                    'group.targetedAnimations[channelIndex]!.targetName ?? ""',
                )
            )
                return 'group.target_names.at(static_cast<std::size_t>(channel_index)).value_or("")';
            if (
                context.expressionMatchesShape(node, "mask.names.indexOf(name)")
            )
        ...
        statement(statement, lowerer, indent) {
            ...
            if (!ts.isIdentifier(variable.name) || !variable.initializer)
                return undefined;
            if (variable.name.text === "mask")
                return [
                    `${indent}const auto mask = ${lowerer.expression(variable.initializer)};`,
                ];
            if (variable.name.text === "name")
                return [
                    `${indent}const auto name = ${lowerer.expression(variable.initializer)};`,
                ];
            return undefined;
        },
