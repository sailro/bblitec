...
export function lowerGltfAnimationBoneOverrides(
    ...
    for (const field of hidden === undefined
        ? ["mask", "tx", "ty", "tz", "rx", "ry", "rz", "rw", "sx", "sy", "sz"]
        : ["mask"])
    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        statement(statement, lowerer, indent) {
            if (
                ...
                context.expressionMatchesShape(statement.expression.left, "m")
            ) {
                const bit = context.numericValue(
                    statement.expression.right,
                    file,
                );
                if ((bit & hidden) === 0)
                    return statement.elseStatement
                        ? lowerer.statement(statement.elseStatement, indent)
                        : [];
            }
            ...
            if (
                ...
                context.contractError(
                    statement,
                    "Expected indexed bone override entries.",
                );
            const names = statement.initializer.declarations[0]!.name;
            if (
                !ts.isArrayBindingPattern(names) ||
                names.elements.length !== 2 ||
                !ts.isBindingElement(names.elements[0]!) ||
                names.elements[0].name.getText(file) !== "ni" ||
                !ts.isBindingElement(names.elements[1]!) ||
                names.elements[1].name.getText(file) !== "o"
            )
                context.contractError(
                    statement,
...
function visibilityMask(context: LoweringContext): number {
    ...
    const writes = context.findNodes(
        visible.declaration,
        (node): node is ts.BinaryExpression =>
            ts.isBinaryExpression(node) &&
            node.operatorToken.kind === ts.SyntaxKind.BarEqualsToken &&
            context.expressionMatchesShape(node.left, "o.mask"),
    );
    if (writes.length !== 1)
        context.contractError(
            visible.declaration,
            "Expected one admitted bone visibility bit writer.",
        );
    ...
    if (
        ...
        context.contractError(
            writes[0]!,
            "Expected one bone visibility mask bit.",
        );
    context.assertFunctionBodyShape(
        visible.declaration,
        `{
