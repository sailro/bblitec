...
export function lowerGltfAssetSceneSetup(context: LoweringContext): string {
    ...
    context.assertExpressionShape(
        context.variableInitializer(declaration, "assetFragments"),
    ...
    if (!loop || !ts.isForOfStatement(loop))
        context.contractError(
            declaration,
            "Expected the glTF asset fragment fold.",
        );
    context.assertStatementShapes(
        declaration,
        declaration.body!.statements.slice(
            declaration.body!.statements.indexOf(loop) + 1,
        ),
        "return container;",
        "glTF completed fragment projection",
    );
    const bindings = new Map<string, PinnedBinding>([
        [
            "container._sceneSetup",
            {
                cpp: "container.scene_setup",
                type: "opaque",
                absentCpp: "!container.scene_setup",
            },
        ],
        [
            "_sceneSetup",
            { cpp: "_sceneSetup", type: "opaque", absentCpp: "!_sceneSetup" },
        ],
    ]);
        ...
        statement(statement, lowerer, indent) {
            ...
            if (
                ...
                context.assertStatementShapes(
                    ...
                    "glTF fragment entity projection",
                );
                if (statement.elseStatement)
                    context.contractError(
                        statement,
                        "Unrepresented glTF fragment entity alternative.",
                    );
                return [];
            }
            if (
                ...
                if (ts.isObjectBindingPattern(variable.name)) {
                    context.assertStatementShapes(
                        ...
                        "glTF fragment field projection",
                    );
                    return [`${indent}const auto _sceneSetup = frag;`];
                }
                if (
                    ts.isIdentifier(variable.name) &&
                    variable.name.text === "prev" &&
            ...
            if (
                context.expressionMatchesShape(expression, "void _ignored") ||
                context.expressionMatchesShape(
                    expression,
            ...
            if (
                ...
                context.assertExpressionShape(
                    expression,
                    `${name}${expression.questionDotToken ? "?." : ""}(scene, target)`,
                    "glTF setup scene and target forwarding",
                );
                return [
                    `${indent}${expression.questionDotToken ? `if (${name}) ` : ""}${name}(scene);`,
                ];
            }
            if (
                ...
                if (
                    ...
                    context.contractError(
                        callback,
                        "Expected the glTF scene setup callback boundary.",
                    );
                return [
                    `${indent}container.scene_setup = [prev, _sceneSetup](Scene& scene) {`,
                    ...lowerer.statements(
...
export function gltfAssetSceneSetupOrder(
    ...
    context.assertFunctionBodyShape(
        context.functionDeclaration(registryModule, "loadGltfFeatures")
            .declaration,
        `{
    ...
    if (!ts.isArrayLiteralExpression(registry))
        context.contractError(registry, "Expected the glTF feature registry.");
    const adapters = new Map([
        [
            "./gltf-feature-gaussian-splatting.js",
            gaussianSplats ? "gaussian_splat_setup" : undefined,
        ],
        ["./gltf-ext-lights-image-based.js", "ibl_scene_setup"],
        [
            "./gltf-feature-interactivity.js",
            interactivity ? "interactivity_scene_setup" : undefined,
        ],
    ]);
    ...
    for (const row of registry.elements) {
        if (!ts.isArrayLiteralExpression(row) || row.elements.length !== 2)
            context.contractError(
                row,
                "Expected a glTF trigger and feature loader.",
            );
        ...
        for (const call of imports) {
            ...
            if (
                ...
                !adapters.has(path.text)
            )
                continue;
            if (seen.has(path.text))
                context.contractError(
                    row,
                    "Repeated glTF setup feature needs distinct native fragment resources.",
                );
            context.assertExpressionShape(
                row.elements[1]!,
                `() => import(${JSON.stringify(path.text)})`,
                "glTF setup feature loader",
            );
            seen.add(path.text);
    ...
    if (seen.size !== adapters.size)
        context.contractError(
            registry,
            "Missing glTF scene setup feature in the registry.",
        );
    return ordered;
}
