...
export function lowerGltfInverseBindMatrices(context: LoweringContext): string {
    ...
    if (names.length !== 3)
        context.contractError(
            declaration,
            "Expected document, binary and skin parameters.",
        );
    ...
    const bindings = new Map<string, PinnedBinding>([
        [
            `${skin}.joints.length`,
            {
                cpp: 'static_cast<double>(required(skin, "joints").as_array().size())',
                type: "scalar",
            },
        ],
        [
            ...
            {
                cpp: 'gltf_json_number(optional(skin, "inverseBindMatrices"))',
                type: "scalar",
        ...
        expression(node, lowerer) {
            if (
                ...
                if (node.arguments.length !== 3)
                    context.contractError(
                        node,
                        "Expected skin accessor arguments.",
                    );
                context.assertExpressionShape(
                    node.arguments[0]!,
                    json!,
                    "Skin accessor document",
                );
                context.assertExpressionShape(
                    node.arguments[1]!,
                    binary!,
            ...
            if (
                ...
                context.expressionMatchesShape(node.expression, "F32") &&
                ...
                const buffer =
                    ...
                    source.name.text === "buffer"
                        ? context.unwrapExpression(source.expression)
                        : undefined;
                const owner =
                    buffer &&
                    ts.isPropertyAccessExpression(buffer) &&
                    buffer.name.text === "_data"
                        ? buffer.expression
                        : undefined;
                if (
                    !owner ||
                    ...
                    context.contractError(
                        node,
                        "Expected a resolved skin accessor view.",
                    );
                context.assertExpressionShape(
                    node.arguments[1]!,
                    `${owner.text}._data.byteOffset`,
                    "Skin view offset",
                );
