...
export function lowerGltfAnimationGroupFactory(
    ...
    if (!ts.isObjectLiteralExpression(value))
        context.contractError(
            value,
            "Expected the source animation group object.",
        );
    const properties = new Map(
        value.properties.filter(ts.isPropertyAssignment).map((property) => {
            const name = context.propertyName(property.name);
            if (name === undefined)
                context.contractError(
                    property,
                    "Expected a named animation group field.",
                );
    ...
    for (const key of properties.keys()) {
        if (!names.has(key) && key !== "_ctrl" && key !== "targetedAnimations")
            context.contractError(
                properties.get(key)!,
                `Unbound animation group factory field ${key}.`,
            );
    }
    ...
    const text = (node: ts.Expression): string => {
        ...
        if (context.expressionMatchesShape(node, "clip.name"))
            return "clip_name";
        if (
        ...
        if (ts.isTemplateExpression(node)) {
            ...
            for (const span of node.templateSpans) {
                context.assertExpressionShape(
                    span.expression,
                    "clipIndex",
                    "Animation fallback name index",
                );
                parts.push(
                    "std::to_string(static_cast<std::size_t>(clip_index))",
        ...
        context.contractError(
            node,
            "Unsupported source animation group name expression.",
        );
    };
    const started = lowerer.expression(
        context.variableInitializer(declaration, "started"),
    );
    const stores = [...names]
        .map(([source, field]) => {
            ...
            if (!initializer)
                context.contractError(
                    value,
                    `Missing animation group field ${source}.`,
                );
