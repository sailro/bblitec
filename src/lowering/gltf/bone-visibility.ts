    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        statement(node, numeric, indent) {
            if (ts.isVariableStatement(node)) {
                ...
                if (
                    context.expressionMatchesShape(
                        variable.initializer,
                        "ensureOverride(skeleton, bone)",
                    )
                )
                    return [`${indent}auto* o = &entries.at(node);`];
                context.assertExpressionShape(
                    variable.initializer,
                    "skeleton._overrides.get(bone._nodeIndex)",
                    "Bone override lookup",
                );
                return [
                    `${indent}auto* o = entries.at(node).mask ? &entries.at(node) : nullptr;`,
                ];
            }
            if (
            ...
            context.assertExpressionShape(
                expression.left,
                "o.mask",
                "Bone visibility mask store",
            );
            const operation = ts.setTextRange(
            ...
            return [
                `${indent}o->mask = static_cast<std::uint32_t>(${numeric.expression(operation)});`,
            ];
        },
    });
