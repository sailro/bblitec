...
export function lowerGltfMaterialTextures(context: LoweringContext): string {
    ...
    for (const [symbol, signature, parameters] of [
        [
            ...
            "gltf_base_factor_texture(const std::vector<double>& factor)",
            [["factor", { cpp: "factor", type: "f64-buffer" }]],
        ],
        [
            "uploadOrmFactorTexture",
            "gltf_orm_factor_texture(double roughness, double metallic)",
            [
                ["roughness", { cpp: "roughness", type: "scalar" }],
                ["metallic", { cpp: "metallic", type: "scalar" }],
            ],
        ],
        ...
        const body = lowerPinnedBody(file, declaration.body!.statements, {
            ...
            expression(node, lowerer) {
                if (
                    ...
                    return undefined;
                const [engine, bitmap, srgb, sampler, mipmaps, fallback] =
                    node.arguments;
                if (
                    ...
                    !mipmaps ||
                    !fallback ||
                    bitmap.kind !== ts.SyntaxKind.NullKeyword
                )
                    context.contractError(
                        node,
                        "Expected a factor texture upload.",
                    );
                context.assertExpressionShape(
                    engine,
                    "engine",
                    "Factor upload engine",
                );
                context.assertExpressionShape(
                    sampler,
                    "sampler",
                    "Factor upload sampler",
                );
                context.assertExpressionShape(
                    mipmaps,
                    "generateMipmaps",
                    "Factor upload mipmaps",
                );
                const bytes = context.unwrapExpression(fallback);
                if (
                    !ts.isNewExpression(bytes) ||
                    !context.expressionMatchesShape(bytes.expression, "U8") ||
                    bytes.arguments?.length !== 1 ||
                    !ts.isArrayLiteralExpression(bytes.arguments[0]!) ||
                    bytes.arguments[0].elements.length !== 4
                )
                    context.contractError(
                        bytes,
                        "Expected four factor texture bytes.",
                    );
                return `GltfMaterialTexture{nullptr, ${lowerer.expression(srgb)}, std::array<std::uint8_t, 4>{${bytes.arguments[0].elements
                    .map(
                        (lane) =>
                            `bbl::js::to_uint8(${lowerer.expression(lane)})`,
                    )
                    .join(", ")}}, nullptr}`;
            },
            statement(statement, lowerer, indent) {
                ...
                const arrow = variable.initializer,
                    parameter = arrow.parameters[0]?.name;
                if (
                ...
                return [
                    `${indent}const auto ${variable.name.text} = [](double ${parameter.text}) { return ${expression}; };`,
                ];
            },
            returnValue: (node, lowerer) => lowerer.expression(node!),
        });
        functions.push(
            `// ${context.provenance(builderModule, symbol)}\nGltfMaterialTexture ${signature} {\n${body}\n}`,
        );
    }
    for (const [module, symbol, signature] of [
        [
            extModule,
            "occlusionNeedsSplit",
            "bool gltf_occlusion_needs_split(const JsonObject& raw)",
        ],
        [
            builderModule,
            "buildDefaultPbrTextures",
            "GltfPbrTextures gltf_default_pbr_textures(const GltfCoreMaterial& mat, GltfTextureCache* cache = nullptr, GltfSamplerContext* sampler_context = nullptr)",
        ],
        [
            extModule,
            "buildDefaultPbrTexturesExt",
            "GltfPbrTextures gltf_default_pbr_textures_ext(const GltfCoreMaterial& mat, bool sampled = false, GltfTextureCache* cache = nullptr, GltfSamplerContext* sampler_context = nullptr, const std::function<GltfMaterialTexture(GltfMaterialImage, bool)>& upload = {})",
        ],
        [
            "src/loader-gltf/gltf-sampler-desc.ts",
            "buildSampledPbrTextures",
            "GltfPbrTextures gltf_sampled_pbr_textures(const GltfCoreMaterial& mat, GltfTextureCache* cache = nullptr, GltfSamplerContext* sampler_context = nullptr)",
        ],
    ] as const) {
        const { file, declaration } = context.functionDeclaration(
            module,
        ...
        const objects = new Set<string>(
            symbol === "occlusionNeedsSplit" ? ["raw"] : [],
        );
        ...
        bindings.set("mat._baseColorFactor", {
            cpp: "mat._baseColorFactor",
            type: "f64-buffer",
        });
        const jsonPath = (expression: ts.Expression): string | undefined => {
            ...
            const root = objects.has(node.text)
                ? node.text
                : pointers.has(node.text)
                  ? `gltf_material_object(${node.text})`
                  : undefined;
            return root
                ? `gltf_json_path(${root}, {${keys.map((key) => JSON.stringify(key)).join(", ")}})`
                : undefined;
        };
        const nativeType = (expression: ts.Expression): string => {
            const node = context.unwrapExpression(expression);
            if (ts.isIdentifier(node) && types.has(node.text))
                return types.get(node.text)!;
            if (jsonPath(node)) return "const ts::JsonValue*";
            if (
                ts.isPropertyAccessExpression(node) &&
                node.name.text.endsWith("Image")
            )
                return "GltfMaterialImage";
            if (ts.isConditionalExpression(node))
                return nativeType(node.whenTrue);
            if (
                ts.isBinaryExpression(node) &&
                node.operatorToken.kind === ts.SyntaxKind.QuestionQuestionToken
            )
                return nativeType(node.left);
            ...
            for (const [index, name] of resourceBindings.entries()) {
                ...
                if (
                    ...
                    context.contractError(
                        ...
                        "Expected the texture upload and wrapper boundary.",
                    );
            }
        ...
        const body = lowerPinnedBody(file, statements, {
            ...
            expression(node, lowerer) {
                ...
                if (
                    ...
                    if (left)
                        return `(${left} ${node.operatorToken.kind === ts.SyntaxKind.ExclamationEqualsToken ? "!=" : "=="} nullptr)`;
                }
                if (
                    ts.isCallExpression(node) &&
                    ...
                    const argument = (index: number) => {
                        ...
                        if (!value)
                            context.contractError(
                                node,
                                "Missing texture builder argument.",
                            );
                        return lowerer.expression(value);
                    };
                    if (name === "getCachedTex" && args.length === 2)
                        return `gltf_cached_material_texture(tex_cache, ${argument(0)}, ${argument(1)}, sampler_context ? sampler_context->default_sampler : nullptr)`;
                    if (name === "pickTex" && args.length === 3)
                        return `pick_texture(${argument(0)}, ${argument(1)}, ${argument(2)})`;
                    if (
                        name === "cached" &&
                        symbol === "buildSampledPbrTextures" &&
                        args.length === 3
                    )
                        return `gltf_sampled_material_texture(tex_cache, ${argument(0)}, ${argument(1)}, ${argument(2)}, *sampler_context)`;
                    if (name === "wrap" && args.length === 2)
                        return `gltf_wrap_material_texture(${argument(0)}, ${argument(1)})`;
                    if (
                        name === "uploadBaseColorFactorTexture" &&
                        args.length === 4
                    )
                        return `gltf_base_factor_texture(${argument(1)})`;
                    if (name === "uploadOrmFactorTexture" && args.length === 5)
                        return `gltf_orm_factor_texture(${argument(1)}, ${argument(2)})`;
                    if (name === "occlusionNeedsSplit" && args.length === 1)
                        return `gltf_occlusion_needs_split(${argument(0)})`;
                }
                const path = jsonPath(node);
                return path &&
                    ts.isPropertyAccessExpression(node) &&
                    node.name.text === "index"
                    ? `gltf_json_number(${path})`
                    : path;
            },
            statement(statement, lowerer, indent) {
                if (
                ...
                if (
                    ...
                    init.operatorToken.kind ===
                        ts.SyntaxKind.QuestionQuestionToken &&
                    ts.isObjectLiteralExpression(init.right) &&
                    init.right.properties.length === 0
                ) {
                    const pointer = context.expressionMatchesShape(
                        init.left,
                        "mat._rawMatDef",
                    )
                        ? "mat._rawMatDef"
            ...
            returnValue(node, lowerer) {
                ...
                const assignments = returned.properties.map((property) => {
                    if (
                        ...
                        context.contractError(
                            property,
                            "Unsupported PBR texture slot.",
                        );
                    if (
                        !ts.isIdentifier(property.name) ||
                        !textureFields.includes(property.name.text) ||
                        seen.has(property.name.text)
                    )
                        context.contractError(
                            property,
                            "Unrepresented PBR texture slot.",
                        );
                ...
                if (
                    seen.size !==
                    (symbol === "buildDefaultPbrTexturesExt" ? 5 : 4)
                )
                    context.contractError(
                        returned,
                        "Incomplete PBR texture slots.",
                    );
                return `[&]() { GltfPbrTextures result; ${assignments.join(" ")} return result; }()`;
            },
        });
        const cache =
            symbol === "occlusionNeedsSplit"
                ? ""
                : (symbol === "buildSampledPbrTextures"
                      ? '    if (!sampler_context) throw std::runtime_error("Missing glTF sampler context.");\n'
                      : symbol === "buildDefaultPbrTexturesExt"
                        ? '    if (sampled && !sampler_context) throw std::runtime_error("Missing glTF sampler context.");\n'
                        : "") +
        ...
        const picker =
            symbol !== "buildDefaultPbrTexturesExt"
                ? ""
                : `    std::function<GltfMaterialSampler(const ts::JsonValue*)> sampler_for;
...
`;
        functions.push(
            `// ${context.provenance(module, symbol)}\n${signature} {\n${cache}${picker}${body}\n}`,
        );
    }
