...
import { collectNodes, refuseModule, topLevelFunction, unwrapPin } from "./shared.js";
...
export function lowerGltfMaterialColorPresence(builderFile: ts.SourceFile, propsName = "assemblePbrProps"): string {
    ...
    const factorSpreads = collectNodes(props.body, (node): node is ts.SpreadAssignment => {
        if (!ts.isSpreadAssignment(node)) return false;
        const expression = unwrapPin(node.expression);
        if (!ts.isConditionalExpression(expression)) return false;
        const present = unwrapPin(expression.whenTrue);
    ...
    const factorSpread = unwrapPin(factorSpreads[0]!.expression) as ts.ConditionalExpression;
    const factorFields = unwrapPin(factorSpread.whenTrue) as ts.ObjectLiteralExpression;
