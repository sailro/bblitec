...
export function lowerGltfMaterialSetup(
    ...
    const { declaration, body: constructionBody } =
        ...
        (statement) =>
            ...
            statement.declarationList.declarations.some(
                (variable) =>
                    ...
                    variable.name.text === "_needsPbrExt",
            ),
    );
    ...
    if (start < 0 || !guard || !ts.isIfStatement(guard))
        context.contractError(
            declaration,
            "Expected material path initialization and guard.",
        );
    const samplerGate = statements.find(
        (statement) =>
            ...
            context.findNodes(
                ...
                (node): node is ts.CallExpression =>
                    ...
                    node.arguments[0].text === "./gltf-sampler-desc.js",
            ).length > 0,
    );
    if (!samplerGate || !ts.isIfStatement(samplerGate))
        context.contractError(
            declaration,
            "Expected sampled material path selection.",
        );
    const functions = [
        [
            "gltf_pbr_needs_extended",
            "json, wrapTex, identityTexWrap",
            `${statements[start]!.getText()}\n${guard.getText()}\nreturn _needsPbrExt;`,
        ],
        [
            "gltf_pbr_needs_sampler",
            "json",
            `return ${samplerGate.expression.getText()};`,
        ],
        [
            "gltf_pbr_build_material",
            "mat, matExts, extCtx, _needsPbrExt, buildSampledPbrTextures",
            constructionBody.statements
                .map((statement) => statement.getText())
                .join("\n"),
        ],
    ] satisfies [string, string, string][];
    const targets = functions.map(
        ([name, parameters, body]): GltfMaterialFunction => {
            ...
            if (!selected || !ts.isFunctionDeclaration(selected))
                context.contractError(
                    declaration,
                    "Expected material setup body.",
                );
            return {
                module,
                name,
                ...
                sourceSymbol: "uploadMeshes",
                ...(name === "gltf_pbr_build_material"
                        ...
                        (call, lowerer) => {
                            if (
                                ...
                                if (call.arguments.length !== 5)
                                    context.contractError(
                                        call,
                                        "Expected variant texture upload arguments.",
                                    );
                                for (const [index, shape] of [
                                    ...
                                    context.assertExpressionShape(
                                        ...
                                        shape,
                                        "Variant texture upload environment",
                                    );
                            ...
                            if (
                                ...
                                const run = resolveCall(
                                    "runGltfMaterialFeatures",
                                );
                                if (!run || call.arguments.length !== 3)
                                    context.contractError(
                                        call,
                                        "Expected the material extension runner.",
                                    );
                                return `${run}(${call.arguments.map((argument) => lowerer.expression(argument)).join(", ")})`;
                            }
                            const builders = [
                                [
                                    "extMod.buildDefaultPbrTexturesExt",
                                    "extended_textures",
                                    [
                                        "engine",
                                        "mat",
                            ...
                            for (const [name, member, parameters] of builders) {
                                ...
                                if (call.arguments.length !== parameters.length)
                                    context.contractError(
                                        call,
                                        "Expected material texture upload arguments.",
                                    );
                                call.arguments.forEach((argument, index) =>
                                    context.assertExpressionShape(
                                        argument,
                                        parameters[index]!,
                                        "Material texture boundary",
                                    ),
                                );
