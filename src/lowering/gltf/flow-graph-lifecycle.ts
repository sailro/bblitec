...
export function lowerGltfFlowGraphLifecycle(context: LoweringContext): string {
    const cleanup = context.functionDeclaration(
        cleanupModule,
    ...
    const cleanupBindings = new Map<string, PinnedBinding>([
        [
            "previous",
            { cpp: "previous", type: "opaque", absentCpp: "!previous" },
        ],
        ["cleanup", { cpp: "cleanup", type: "opaque" }],
        ["combined", { cpp: "combined", type: "opaque" }],
    ]);
    const cleanupBody = lowerPinnedBody(
        cleanup.file,
        cleanup.declaration.body!.statements,
        {
            ...
            statement(statement, lowerer, indent) {
                if (
                    ...
                    if (variable.name.text === "cleanups") {
                        context.assertExpressionShape(
                            variable.initializer,
                            "container._sceneCleanups ??= new WeakMap()",
                            "Asset scene cleanup storage",
                        );
                        return [
                            `${indent}auto& cleanups = container.scene_cleanups;`,
                        ];
                    }
                    if (variable.name.text === "previous") {
                        context.assertExpressionShape(
                            variable.initializer,
                            "cleanups.get(scene)",
                            "Asset scene cleanup lookup",
                        );
                        return [
                            `${indent}const auto found = cleanups.find(scene.state);`,
                    ...
                    if (variable.name.text === "combined") {
                        ...
                        if (
                            ...
                            !ts.isArrowFunction(value.whenTrue) ||
                            !ts.isBlock(value.whenTrue.body) ||
                            value.whenTrue.parameters.length
                        )
                            context.contractError(
                                value,
                                "Expected combined asset cleanup callbacks.",
                            );
                        return [
                            `${indent}const js::Callback<void()> combined = ${lowerer.expression(value.condition)} ? js::Callback<void()>{[previous, cleanup] {`,
                            ...lowerer.statements(
                                value.whenTrue.body.statements,
                                indent + "    ",
                            ),
                ...
                if (
                    ...
                    context.assertExpressionShape(
                        expression.left,
                        "scene._disposables[scene._disposables.indexOf(previous)]",
                        "Asset cleanup replacement slot",
                    );
                    return [
                        `${indent}const auto slot = std::find(scene.disposables.begin(), scene.disposables.end(), previous);`,
                ...
                if (
                    ...
                    if (expression.arguments.length !== 1)
                        context.contractError(
                            expression,
                            "Expected one asset cleanup callback.",
                        );
                    return [
                        `${indent}scene.disposables.push_back(${lowerer.expression(expression.arguments[0]!)});`,
                    ];
                }
                if (
                    context.expressionMatchesShape(
    ...
    const runBindings = new Map<string, PinnedBinding>([
        ["runtimes", { cpp: "runtimes", type: "opaque" }],
        [
            "runtimes.length",
            { cpp: "static_cast<double>(runtimes.size())", type: "scalar" },
        ],
        ["scene", { cpp: "scene", type: "opaque" }],
        ["rt", { cpp: "rt", type: "opaque" }],
    ]);
    const runBody = lowerPinnedBody(
        run.file,
        run.declaration.body!.statements,
        {
            bindings: runBindings,
            calls: new Map([
                [
                    "attachFlowGraph",
                    (args) => `attach_flow_graph(${args.join(", ")})`,
                ],
                [
                    "detachFlowGraph",
                    (args) => `detach_flow_graph(${args.join(", ")})`,
                ],
            ]),
            forOf(iterated, element) {
                return iterated === "loaded" && element === "lg"
                    ? {
                          range: "loaded",
                          bindings: new Map([
            ...
            expression(node, lowerer) {
                if (
                    ...
                    context.expressionMatchesShape(node.expression, "runtimes")
                )
            ...
            statement(statement, lowerer, indent) {
                if (ts.isTryStatement(statement)) {
                    if (!statement.catchClause || statement.finallyBlock)
                        context.contractError(
                            statement,
                            "Expected flow runtime rollback catch.",
                        );
                    return [
                        `${indent}try {`,
                        ...lowerer.statements(
                ...
                if (
                    ...
                    if (name === "runtimes") {
                        context.assertExpressionShape(
                            variable.initializer,
                            "[]",
                            "Initial attached flow runtime list",
                        );
                    ...
                    if (name === "resolveAccessor") {
                        context.assertExpressionShape(
                            variable.initializer,
                            "lg.resolveAccessor ? (pointer: string) => lg.resolveAccessor!(pointer, scene, animations) : undefined",
                            "Static flow accessor factory",
                        );
                        return [];
                    }
                    if (name === "rt") {
                        ...
                        return [
                            `${indent}const auto rt = lg(*scene.engine, asset);`,
                        ];
                    }
                }
                if (
                    ...
                    if (statement.expression.arguments.length !== 1)
                        context.contractError(
                            statement,
                            "Expected one attached flow runtime.",
                        );
                    return [
                        `${indent}runtimes.push_back(${lowerer.expression(statement.expression.arguments[0]!)});`,
                    ];
                }
                return undefined;
            },
    ...
    const feature = context.methodDeclaration(
        featureModule,
        "feature.applyAsset",
    );
    const setups = context.findNodes(
        feature.declaration,
        (node): node is ts.MethodDeclaration =>
            ts.isMethodDeclaration(node) &&
            context.propertyName(node.name) === "_sceneSetup",
    );
    if (setups.length !== 1 || !setups[0]!.body)
        context.contractError(
            feature.declaration,
            "Expected one flow-graph asset setup.",
        );
    const setupBindings = new Map<string, PinnedBinding>([
        ["removed", { cpp: "state->removed", type: "bool" }],
        ["active", { cpp: "state->active", type: "opaque" }],
    ...
    const forEach = (
        ...
        if (
            ...
            callback.parameters[0]!.name.getText(feature.file) !== "runtime"
        )
            context.contractError(
                call,
        ...
        return [
            `${indent}for (const auto& runtime : ${lowerer.expression(call.expression.expression)}) {`,
            ...lowerer.statements(body, indent + "    "),
            `${indent}}`,
        ];
    };
    const setupBody = lowerPinnedBody(
        feature.file,
        setups[0]!.body.statements,
        {
            ...
            statement(statement, lowerer, indent) {
                if (
                    ...
                    if (
                        ...
                        return undefined;
                    if (variable.name.text === "removed")
                        return [
                            `${indent}state->removed = ${lowerer.expression(variable.initializer)};`,
                        ];
                    if (variable.name.text === "active") {
                        context.assertExpressionShape(
                            variable.initializer,
                            "[]",
                            "Initial flow attachment ownership",
                        );
                    ...
                    if (variable.name.text === "runtimes") {
                        context.assertExpressionShape(
                            ...
                            "Flow attachment construction boundary",
                        );
                        ...
                        return [
                            `${indent}const auto runtimes = [&] { return run_flow_graphs(scene, asset, factories); };`,
                        ];
                    }
                }
                if (!ts.isExpressionStatement(statement)) return undefined;
                ...
                if (
                    ts.isBinaryExpression(expression) &&
                    context.expressionMatchesShape(expression.left, "active") &&
                    context.expressionMatchesShape(expression.right, "[]")
                )
                    return [`${indent}state->active.clear();`];
                const call = ts.isVoidExpression(expression)
                    ? context.unwrapExpression(expression.expression)
                    : expression;
                ...
                if (
                    ...
                    if (
                        ...
                        context.contractError(
                            call,
                            "Expected flow attachment cleanup callback.",
                        );
                    context.assertExpressionShape(
                        call.arguments[0]!,
                        "container",
                ...
                if (
                    ...
                    const fulfilled = call.arguments[0],
                        rejected = call.arguments[1];
                    if (
                        call.arguments.length !== 2 ||
                        !fulfilled ||
                        !ts.isArrowFunction(fulfilled) ||
                        fulfilled.parameters.length !== 1 ||
                        fulfilled.parameters[0]!.name.getText(feature.file) !==
                            "loaded" ||
                        !ts.isBlock(fulfilled.body) ||
                        !rejected
                    )
                        context.contractError(
                            call,
                            "Expected resolved flow runtime continuation.",
                        );
                    context.assertExpressionShape(
                        rejected,
                        "() => undefined",
                    ...
                    return [
                        `${indent}{`,
                        `${indent}    const auto loaded = runtimes();`,
                        `${indent}    if (published) *published = loaded;`,
                        ...lowerer.statements(
                            fulfilled.body.statements,
                            indent + "    ",
                        ),
                        `${indent}}`,
                    ];
...
export function lowerFlowGraphMembership(context: LoweringContext): string {
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "scene._flowGraphTick",
            {
                cpp: "scene.state->flow_graph_tick",
                type: "opaque",
                absentCpp: "!scene.state->flow_graph_tick",
            },
        ],
        [
            "scene._flowGraphDispose",
            {
                cpp: "scene.state->flow_graph_dispose",
                type: "opaque",
                absentCpp: "!scene.state->flow_graph_dispose",
            },
        ],
        [
            "scene._flowGraphBus",
    ...
    const lower = (name: string) => {
        ...
        return lowerPinnedBody(file, declaration.body!.statements, {
            ...
            calls: new Map([
                ...
                [
                    "disposeFlowGraph",
                    (args: readonly string[]) => `${args[0]}->dispose()`,
                ],
            ]),
            expression(node) {
                if (
                    node.kind === ts.SyntaxKind.Identifier &&
                    node.getText(file) === "undefined"
                )
                    return "{}";
                return undefined;
            },
            statement(statement, lowerer, indent) {
                if (
                    ...
                    if (variable.name.text === "list") {
                        context.assertExpressionShape(
                            variable.initializer,
                            "scene._flowGraphs",
                            "Scene flow runtime membership",
                        );
                        return [
                            `${indent}auto& list = scene.state->flow_graphs;`,
                        ];
                    }
                    if (
                        variable.name.text === "tick" ||
                        variable.name.text === "dispose"
                    )
                        return [
                            `${indent}const auto ${variable.name.text} = ${lowerer.expression(variable.initializer)};`,
                        ];
                    const value = context.unwrapExpression(
                        variable.initializer,
                    );
                    if (
                        ...
                        if (value.arguments.length !== 1)
                            context.contractError(
                                value,
                                "Expected one flow registration lookup.",
                            );
                        const target =
                            value.expression.expression.getText(file);
                        const collection =
                            target === "list"
                                ? "list"
                                : target === "scene._beforeRender"
                                  ? "scene.before_render"
                                  : target === "scene._disposables"
                                    ? "scene.disposables"
                                    : undefined;
                        if (!collection)
                            context.contractError(
                                value,
                                "Unrepresented flow registration collection.",
                            );
                        const name = variable.name.text;
                ...
                if (
                    ...
                    return [
                        `${indent}if (scene.state->flow_graph_pointer_refresh) refresh_flow_graph_pointer_picking(scene);`,
                    ];
                if (
                    !ts.isCallExpression(expression) ||
                    ...
                    return undefined;
                const target = expression.expression.expression.getText(file),
                ...
                if (
                    ...
                    return [
                        `${indent}list.push_back(${lowerer.expression(expression.arguments[0]!)});`,
                    ];
                if (method === "splice" && expression.arguments.length === 2) {
                    ...
                    context.assertExpressionShape(
                        ...
                        "1",
                        "Flow registration removal length",
                    );
                    const index = lowerer.expression(expression.arguments[0]!);
                    return [
                        `${indent}${collection}.erase(${collection}.begin() + ${index}, ${collection}.begin() + ${index} + 1);`,
                    ];
                }
                return undefined;
            },
...
export function lowerFlowGraphDisposal(
    context: LoweringContext,
    slots: readonly string[],
): string {
    const { file, declaration } = context.functionDeclaration(
        "src/flow-graph/runtime.ts",
        "disposeFlowGraph",
    );
    return lowerPinnedBody(file, declaration.body!.statements, {
        ...
        statement(statement, _lowerer, indent) {
            if (ts.isVariableStatement(statement)) {
                ...
                if (ts.isObjectBindingPattern(variable.name)) {
                    context.assertExpressionShape(
                        variable.initializer,
                        "rt",
                        "Flow disposal context",
                    );
                    if (
                        variable.name.getText(file).replace(/\s/g, "") !==
                        "{context:ctx,env}"
                    )
                        context.contractError(
                            variable,
                            "Unrepresented flow disposal context binding.",
                        );
                    return [];
                }
                if (
                    ts.isIdentifier(variable.name) &&
                    variable.name.text === "visited"
                ) {
                    context.assertExpressionShape(
                        variable.initializer,
                        "new Set<string>()",
                        "Empty pending-task disposal",
                    );
                    return [];
                }
            }
            if (ts.isForOfStatement(statement)) {
                if (
                    ...
                    context.expressionMatchesShape(
                        statement.expression,
                        "Object.keys(ctx.executionVariables)",
                    )
                )
                    return [];
                if (
                    context.expressionMatchesShape(
                        statement.expression,
                        "Object.keys(ctx.connectionValues)",
                    )
                ) {
                    context.assertStatementShapes(
                        ...
                        "Static flow connection cleanup",
                    );
                    return slots.map(
                        (member) => `${indent}state.${member} = {};`,
                    );
                }
            }
            if (
