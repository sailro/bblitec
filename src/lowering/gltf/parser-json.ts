...
export function lowerGltfParserJson(context: LoweringContext): string {
    ...
    if (
        ...
        context.contractError(
            declaration,
            "Expected the glTF texture parameter.",
        );
    const path = (expression: ts.Expression): string | undefined => {
        ...
        return keys.length &&
            ...
            ? `gltf_json_path(texture, {${keys.map((key) => JSON.stringify(key)).join(", ")}})`
            : undefined;
    };
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        expression(node, lowerer) {
            if (
                ...
                if (left)
                    return `(${left} ? ${lowerer.expression(node.left)} : ${lowerer.expression(node.right)})`;
            }
            const property = path(node);
            return property ? `gltf_json_number(${property})` : undefined;
        },
        returnValue: (value, lowerer) =>
            `gltf_checked_index(${lowerer.expression(value!)})`,
    });
