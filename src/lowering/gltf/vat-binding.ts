...
export function lowerGltfVatBinding(context: LoweringContext): string {
    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        calls: new Map(),
        returnValue(node, numeric) {
            return node ? numeric.expression(node) : "false";
        },
        expression(node) {
            ...
            if (ts.isCallExpression(node)) {
                context.assertExpressionShape(
                    node,
                    "bindings?.find((binding) => binding.runtimeSkeleton === skeleton || binding.boneTexture === skeleton.boneTexture)",
        ...
        statement(node, _numeric, indent) {
            ...
            if (
                ...
                context.contractError(
                    node,
                    "Expected VAT binding storage alias.",
                );
            ...
            context.assertExpressionShape(
                variable.initializer,
                "group._gltfMixer?.[2]",
                "VAT all-bindings tuple",
            );
            if (variable.name.text !== "bindings")
                context.contractError(node, "Expected VAT bindings alias.");
            return [`${indent}const bool bindings = bindings_present;`];
        },
    });
    const prepare = context.functionDeclaration(module, "prepareVatMany");
    const skeletonMap = context.unwrapExpression(
        context.variableInitializer(prepare.declaration, "states"),
    );
    if (
        !ts.isCallExpression(skeletonMap) ||
        !ts.isArrowFunction(skeletonMap.arguments[0]!) ||
        !ts.isBlock(skeletonMap.arguments[0].body)
    )
        context.contractError(
            skeletonMap,
            "Expected VAT target preparation map.",
        );
    const statements = skeletonMap.arguments[0].body.statements;
    const bindingMap = context.unwrapExpression(
        context.variableInitializer(skeletonMap.arguments[0], "bindings"),
    );
    if (
        !ts.isCallExpression(bindingMap) ||
        !ts.isArrowFunction(bindingMap.arguments[0]!) ||
        !ts.isBlock(bindingMap.arguments[0].body)
    )
        context.contractError(
            bindingMap,
            "Expected VAT per-group binding map.",
        );
    const guard = (statements: readonly ts.Statement[]) =>
        lowerPinnedBody(file, statements, {
            bindings: new Map<string, PinnedBinding>([
                ...
                [
                    "binding.boneCount",
                    { cpp: "binding_bone_count", type: "scalar" },
                ],
                [
                    "skeleton.boneCount",
                    { cpp: "skeleton_bone_count", type: "scalar" },
                ],
            ]),
            calls: new Map(),
            statement(node, _numeric, indent) {
                ...
                if (
                    ...
                    context.contractError(
                        node,
                        "Expected a VAT preparation error template.",
                    );
                ...
                return [
                    `${indent}throw std::runtime_error(${parts.join(" + ")});`,
                ];
            },
        });
    const bindingIndex = statements.findIndex(
        (statement) =>
            ts.isVariableStatement(statement) &&
            statement.declarationList.declarations.some(
                (variable) =>
                    ts.isIdentifier(variable.name) &&
                    variable.name.text === "bindings",
            ),
    );
    const skeletonGuards = statements
        .slice(0, bindingIndex)
        .filter(ts.isIfStatement);
    const bindingGuards = bindingMap.arguments[0].body.statements.filter(
        ts.isIfStatement,
    );
    if (skeletonGuards.length !== 1 || bindingGuards.length !== 2)
        context.contractError(
            prepare.declaration,
            "Expected source skeleton and binding preparation guards.",
        );
