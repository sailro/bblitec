...
import {
    lowerGltfMaterialObjectFunction,
    type GltfMaterialFunction,
} from "./material-object-lowerer.js";
...
const setters = [
    ["set-clearcoat", "setPbrClearCoat"],
    ["set-sheen", "setPbrSheen"],
    ["set-iridescence", "setPbrIridescence"],
    ["set-unlit", "setPbrUnlit"],
    ["set-emissive", "setPbrEmissive"],
    ["set-alpha-cutoff", "setPbrAlphaCutoff"],
    ["set-transmission", "setPbrTransmission"],
    ["set-dispersion", "setPbrDispersion"],
    ["set-metallic-reflectance", "setPbrMetallicReflectance"],
    ["enable-material-uv-transform", "enableMaterialUvTransform"],
    ["set-anisotropy", "setPbrAnisotropy"],
    ["set-subsurface", "setPbrSubsurface"],
] as const;
...
export function lowerGltfMaterialProperties(context: LoweringContext): {
    ...
    features: readonly {
        handler: GltfMaterialFunction;
        trigger: GltfMaterialFunction;
    }[];
    ...
    const add = (module: string, name: string, method = false) => {
        ...
        if (!declaration || (method && candidates.length !== 1))
            context.contractError(
                context.sourceFile(module),
                "Expected one material method.",
            );
        const contextName =
            method && name === "applyMaterial"
                ? declaration.parameters[1]?.name
                : undefined;
        if (contextName && !ts.isIdentifier(contextName))
            context.contractError(
                contextName,
                "Expected a material texture context.",
            );
        ...
        functions.push({
            ...
            cpp,
            declaration,
            ...(contextName ? { contextParameter: contextName.text } : {}),
        });
    };
    for (const [file, name] of setters)
        add(`src/material/pbr/${file}.ts`, name);
    ...
    for (const file of [
        "gltf-ext-clearcoat",
        "gltf-ext-iridescence",
        "gltf-ext-emissive-strength",
        "gltf-ext-sheen",
        "gltf-ext-anisotropy",
        "gltf-ext-diffuse-transmission",
        "gltf-ext-unlit",
        "gltf-ext-spec-gloss",
        "gltf-ext-dielectric",
        "gltf-ext-orm",
    ...
    for (const target of functions.slice(-2)) {
        ...
        const file = ts.createSourceFile(
            ...
            `function ${target.name}(${[
                ...
                "pointerContext",
            ].join(", ")}) ${declaration.body!.getText()}`,
            ts.ScriptTarget.Latest,
            true,
        );
        const lowered = file.statements[0];
        if (!lowered || !ts.isFunctionDeclaration(lowered))
            context.contractError(
                declaration,
                "Expected a source pointer material function.",
            );
        ...
        if (ts.isVariableStatement(statement))
            for (const variable of statement.declarationList.declarations)
                if (
                    ...
                    ts.isStringLiteralLike(variable.initializer)
                )
                    constants.set(
                        variable.name.text,
                        variable.initializer.text,
                    );
    const table = context.findNodes(
        registry,
        (node): node is ts.VariableDeclaration =>
            ts.isVariableDeclaration(node) &&
            ts.isIdentifier(node.name) &&
            node.name.text === "_features",
    )[0];
    if (!table?.initializer || !ts.isArrayLiteralExpression(table.initializer))
        context.contractError(registry, "Expected a glTF feature registry.");
    const features: {
        handler: GltfMaterialFunction;
        trigger: GltfMaterialFunction;
    }[] = [];
    ...
    for (const row of table.initializer.elements) {
        if (!ts.isArrayLiteralExpression(row) || row.elements.length !== 2)
            context.contractError(row, "Expected a glTF feature row.");
        ...
        if (
            imports.length !== 1 ||
            imports[0]!.arguments.length !== 1 ||
            !ts.isStringLiteralLike(imports[0]!.arguments[0]!)
        )
        ...
        const handler = functions.find(
            (target) =>
                target.module === module && target.name === "applyMaterial",
        );
        const textureWrap = functions.some(
            (target) =>
                target.module === module && target.name === "wrapTexture",
        );
        if (!handler && !textureWrap) continue;
        const expression = row.elements[0]!;
        const name = handler
            ? `gltf_pbr_feature_${features.length}`
            : `gltf_pbr_texture_wrap_${textureWrapTriggers.length}`;
        const parameter = ts.isArrowFunction(expression)
        ...
        const body = ts.isArrowFunction(expression)
            ...
            : predicate
              ? `{ return ${expression.getText()}(document); }`
              : `{ return (document.extensionsUsed ?? []).includes(${expression.getText()}); }`;
        const file = ts.createSourceFile(
            registryModule,
            `function ${name}(${parameter?.getText() ?? "document"}) ${body}`,
            ts.ScriptTarget.Latest,
            true,
        );
        ...
        if (!declaration || !ts.isFunctionDeclaration(declaration))
            context.contractError(
                row,
                "Expected a material feature predicate.",
            );
        const trigger: GltfMaterialFunction = {
    ...
    if (
        features.length !==
        functions.filter((target) => target.name === "applyMaterial").length
    )
        context.contractError(
            registry,
            "A lowered material handler is missing from the feature registry.",
        );
    add(registryModule, "runGltfMaterialFeatures");
    functions[functions.length - 1]!.contextParameter = "ctx";
    const bodies = functions.map((target) =>
        lowerGltfMaterialObjectFunction(
            context,
            target,
            (name) =>
                (
                    functions.find(
                        (candidate) =>
                            ...
                            candidate.name === name,
                    ) ?? functions.find((candidate) => candidate.name === name)
                )?.cpp,
            (call, lowerer) => {
                if (
                    target.name === "whiteFallback" &&
                    context.expressionMatchesShape(
                        call.expression,
                        "_animBaseColorDefs?.has",
                    )
                ) {
                    if (call.arguments.length !== 1)
                        context.contractError(
                            call,
                            "Expected source base-color definition membership.",
                        );
                    context.assertExpressionShape(
                        call.arguments[0]!,
                        "mat._rawMatDef",
                        "Source animation base-color owner",
                    );
                ...
                if (
                    ...
                    context.expressionMatchesShape(
                        call.expression,
                        "_baseColorMod?.whiteFallback",
                    )
                ) {
                    if (call.arguments.length !== 1)
                        context.contractError(
                            call,
                            "Expected the source base-color material feature call.",
                        );
                    const handler = functions.find(
                        (target) => target.name === "whiteFallback",
                    )!;
                    return `(pointerContext.base_color_module ? ${handler.cpp}(${lowerer.expression(call.arguments[0]!)}, pointerContext) : GltfPbrValue{})`;
                }
                if (
                    target.module !== "src/loader-gltf/gltf-ext-orm.ts" ||
                    !context.expressionMatchesShape(
                        call.expression,
                        "compositeOrm",
                    )
                    ...
                    return undefined;
                if (call.arguments.length !== 2 || !target.contextParameter)
                    context.contractError(
                        call,
                        "Expected two ORM bitmaps and their image context.",
                    );
                return `gltf_pbr_composite_orm(${target.contextParameter}, ${call.arguments.map((argument) => lowerer.expression(argument)).join(", ")})`;
            },
        ),
    );
    const setup = lowerGltfMaterialSetup(
        context,
        (name) => functions.find((target) => target.name === name)?.cpp,
    );
    return {
        functions: [...functions, ...setup.functions],
        features,
        textureWrapTriggers,
        source: `${lowerPbrSceneHookRegistry(context)}
                  ...
                  : type === "bool"
                    ...
                    : type === "std::string"
                      ...
                      : type === "GltfMaterialImage"
                        ? `${access}.nullish() ? GltfMaterialImage{} : ${access}.image()`
                        : `${access}.source()`;
