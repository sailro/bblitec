...
export function lowerGltfAnimationRootFlip(
    context: LoweringContext,
    module = modulePath,
): string {
    const file = context.sourceFile(module);
    const expression = context.unwrapExpression(
        context.variableInitializer(file, "RH_TO_LH"),
    );
    if (
        !ts.isNewExpression(expression) ||
        !ts.isIdentifier(expression.expression) ||
        expression.expression.text !== "F32" ||
        expression.arguments?.length !== 1 ||
        !ts.isArrayLiteralExpression(expression.arguments[0]!)
    )
        context.contractError(
            expression,
            "Expected the source controller's Float32 root conversion.",
        );
    const values = expression.arguments[0].elements;
    if (values.length !== 16)
        context.contractError(
            expression,
            "Expected a sixteen-lane controller root matrix.",
        );
...
export function lowerGltfAnimationPose(context: LoweringContext): string {
    ...
    if (!ts.isObjectLiteralExpression(value))
        context.contractError(value, "Expected the animation controller.");
    const tick = value.properties.find(
        (property) =>
    ...
    if (
        !tick ||
        !ts.isPropertyAssignment(tick) ||
        !ts.isConditionalExpression(tick.initializer) ||
        !ts.isArrowFunction(tick.initializer.whenFalse) ||
        !ts.isBlock(tick.initializer.whenFalse.body)
    )
    ...
    const clock = statements.findIndex(
        (statement) =>
    ...
    if (clock < 0)
        context.contractError(tick, "Expected the source pose-time boundary.");
    ...
    for (const name of ["pointerScratch", "morphUploadF32"])
        bind(name, `(*state.${name})`, "f32");
    ...
    bind(
        "nodeTrsBindings.length",
        "static_cast<double>(state.node_trs_bindings.size())",
    );
    ...
    const arrays = new Map([
        ["nodes", "state.nodes"],
        ["clip.channels", "state.channels"],
        ["clip.samplers", "state.samplers"],
        ["nodeTrsBindings", "state.node_trs_bindings"],
        ["topoOrder", "state.topo_order"],
        ["clipSkeletons", "state.skeletons"],
        ["boneScratch", "state.bone_scratch"],
        ["skel.jointNodes", "skel.jointNodes"],
        ["bindings", "(*bindings)"],
        ["maskedNodes", "state.masked_nodes"],
    ]);
    const arrayRead = (
        node: ts.Expression,
        ...
        const source = [...arrays].find(([name]) =>
            context.expressionMatchesShape(receiver, name),
        )?.[1];
        return source
            ? `${source}.at(static_cast<std::size_t>(${lowerer.expression(node.argumentExpression)}))`
            : undefined;
    };
    const calls = new Map<string, (args: readonly string[]) => string>([
        ...
        [
            "morphUploadF32.fill",
            (args) =>
                `std::fill(state.morphUploadF32->begin(), state.morphUploadF32->end(), static_cast<float>(${args[0]}))`,
        ],
    ]);
        ...
        expression(node, lowerer) {
            ...
            if (
                context.expressionMatchesShape(
                    node,
                    "boneOverrides !== undefined",
                )
            )
                return "state.has_bone_overrides";
            if (context.expressionMatchesShape(node, "_maskResolver !== null"))
                return "state.mask_resolver";
            if (context.expressionMatchesShape(node, "node._matrix"))
                return "node.matrix.has_value()";
            if (
                context.expressionMatchesShape(node, "bindings[0]!.targetCount")
            )
                return "bindings->at(0).targetCount";
            if (
                context.expressionMatchesShape(
                    node,
                    "morphBindingsByNode[ch.nodeIdx]",
                )
            )
            ...
            if (
                context.expressionMatchesShape(
                    node,
                    "mb.runtimeMorphTargets?._disposed",
                )
            )
                return "mb.disposed";
            if (
        ...
        statement(statement, lowerer, indent) {
            if (ts.isSwitchStatement(statement)) {
                ...
                return statement.caseBlock.clauses.flatMap((clause, index) => {
                    if (!ts.isCaseClause(clause))
                        context.contractError(
                            clause,
                            "Expected a named source animation path.",
                        );
                    const arm =
                        clause.statements.length === 1 &&
                        ts.isBlock(clause.statements[0]!)
                            ? clause.statements[0].statements
                            : clause.statements;
                    if (!arm.length || !ts.isBreakStatement(arm.at(-1)!))
                        context.contractError(
                            clause,
                            "Expected a terminating animation path.",
                        );
                    for (const child of arm.slice(0, -1))
                        if (
                            context.hasNode(
                                child,
                                (node) =>
                            ...
                            context.contractError(
                                child,
                                "Nested animation-path control transfer needs an explicit switch boundary.",
                            );
                    return [
                        `${indent}${index ? "else " : ""}if (${selected} == ${lowerer.expression(clause.expression)}) {`,
                        ...lowerer.statements(
                            arm.slice(0, -1),
                            indent + "    ",
                        ),
            ...
            if (
                ...
                if (
                    ...
                    aliases.has(variable.name.text)
                ) {
                    const name = variable.name.text;
                    return [
                        `${indent}${name === "bindings" ? "auto*" : "auto&"} ${name} = ${lowerer.expression(variable.initializer)};`,
                    ];
                }
            }
            ...
            if (
                ...
                if (
                    ...
                    expression.right.expression.text !== "F32" ||
                    expression.right.arguments?.length !== 1
                )
                    context.contractError(
                        expression,
                        "Expected source morph scratch allocation.",
                    );
                return [
                    `${indent}state.morphUploadF32 = std::make_shared<std::vector<float>>(static_cast<std::size_t>(${lowerer.expression(expression.right.arguments[0]!)}), 0.0f);`,
                ];
            }
            ...
            if (context.expressionMatchesShape(callee, "_boneApplier")) {
                if (
                    ...
                    context.contractError(
                        expression,
                        "Expected the bone override hook.",
                    );
                context.assertExpressionShape(
                    expression.arguments[0]!,
                    "boneOverrides as ReadonlyMap<number, BoneOverride>",
                    "Bone override identity",
                );
                return [
                    `${indent}apply_bone_overrides(${expression.arguments
                        .slice(1)
                        .map((argument) => lowerer.expression(argument))
                        .join(
                            ", ",
                        )}${expression.arguments.length === 3 ? ", false" : ""});`,
                ];
            }
            if (context.expressionMatchesShape(callee, "mb.weights.set")) {
                context.assertExpressionShape(
                    ...
                    "Morph weight storage copy",
                );
                return [
                    `${indent}std::copy_n(state.morphUploadF32->begin(), static_cast<std::size_t>(tc), mb.weights.begin());`,
                ];
            }
            if (
                context.expressionMatchesShape(
                ...
                context.assertExpressionShape(
                    ...
                    "Morph GPU upload transport",
                );
                return [
                    `${indent}upload_morph(mb, *state.morphUploadF32, tc);`,
                ];
            }
            if (
                context.expressionMatchesShape(
                ...
                context.assertExpressionShape(
                    expression,
                    "device!.queue.writeTexture({ texture: skel.runtimeSkeleton?.boneTexture ?? skel.boneTexture }, boneData.buffer, { bytesPerRow: texWidth * 16 }, { width: texWidth, height: 1 })",
                    "Bone GPU upload transport",
                );
                return [`${indent}upload_bones(skel, boneData, texWidth);`];
            }
            if (context.expressionMatchesShape(callee, "ch.pointerWriter")) {
                return [
                    `${indent}write_pointer(ch, ${expression.arguments.map((argument) => lowerer.expression(argument)).join(", ")});`,
                ];
            }
            for (const [source, target] of [
                ...
                if (
                    ...
                    return [
                        `${indent}write_${target}(b.target, ${expression.arguments.map((argument) => lowerer.expression(argument)).join(", ")});`,
                    ];
            }
            if (context.expressionMatchesShape(callee, "localMat.set")) {
                context.assertExpressionShape(
                    expression.arguments[0]!,
                    "node._matrix",
                    "Authored local matrix storage",
                );
                return [
                    `${indent}std::copy(node.matrix->begin(), node.matrix->end(), state.localMat.begin() + static_cast<std::size_t>(${lowerer.expression(expression.arguments[1]!)}));`,
                ];
            }
            return undefined;
        },
