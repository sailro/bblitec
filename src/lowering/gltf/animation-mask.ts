...
export function lowerGltfAnimationMask(context: LoweringContext): string {
    const module = "src/animation/animation-group-mask.ts",
        controllerModule = "src/skeleton/skeleton-updater.ts";
    const controller = context.functionDeclaration(
        controllerModule,
        "createAnimationController",
    );
    const value = context.unwrapExpression(
        context.variableInitializer(controller.declaration, "_setMask"),
    );
    if (!ts.isArrowFunction(value) || !ts.isBlock(value.body))
        context.contractError(value, "Expected source controller mask setter.");
    const initial = (name: string) =>
        context.variableInitializer(controller.declaration, name);
    for (const name of ["maskedNodes", "cMask", "cNames"])
        context.assertExpressionShape(
            initial(name),
            "null",
            "Initial controller mask identity",
        );
    for (const name of ["maskActive", "cDisabled"])
        context.assertExpressionShape(
            initial(name),
            "false",
            "Initial controller mask flags",
        );
    const bindings = new Map<string, PinnedBinding>([
        ...
        ["mask.names", { cpp: "mask->names", type: "opaque" }],
        ["name", { cpp: "name", type: "opaque" }],
        ["names", { cpp: "names", type: "opaque" }],
        [
            "names.length",
            { cpp: "static_cast<double>(names.size())", type: "scalar" },
        ],
        [
            "nodeNames",
            { cpp: "node_names", type: "opaque", staticBoolean: true },
        ],
        [
            "numNodes",
            { cpp: "static_cast<double>(pose.nodes.size())", type: "scalar" },
        ],
        ["out", { cpp: "out", type: "u8" }],
        [
            "maskedNodes",
            {
                cpp: "pose.masked_nodes",
                type: "u8",
                absentCpp: "!cache.allocated",
            },
        ],
        ["maskActive", { cpp: "pose.mask_active", type: "bool" }],
        ["_maskResolver", { cpp: "pose.mask_resolver", type: "bool" }],
        [
            "cMask",
            { cpp: "cache.mask", type: "opaque", absentCpp: "!cache.mask" },
        ],
        [
            "cNames",
            { cpp: "cache.names", type: "opaque", absentCpp: "!cache.names" },
        ],
        ["cLen", { cpp: "cache.length", type: "scalar" }],
    ...
    const sharedExpression = (
        node: ts.Expression,
        lowerer: { expression(node: ts.Expression): string },
    ): string | undefined => {
        if (ts.isStringLiteral(node))
            return `std::string(${JSON.stringify(node.text)})`;
        if (context.expressionMatchesShape(node, "names === cNames"))
            return "std::addressof(names) == cache.names";
        if (
            ...
            if (node.arguments.length !== 1)
                context.contractError(
                    node,
                    "Expected source mask name membership argument.",
                );
        ...
        if (
            ...
            context.expressionMatchesShape(node.expression, "nodeNames")
        )
            return `node_names.at(static_cast<std::size_t>(${lowerer.expression(node.argumentExpression)}))`;
        if (
            ts.isBinaryExpression(node) &&
            node.operatorToken.kind === ts.SyntaxKind.QuestionQuestionToken &&
            context.expressionMatchesShape(node.left, "nodeNames[i]")
        )
            return `node_names.at(static_cast<std::size_t>(i)).value_or(${lowerer.expression(node.right)})`;
        return undefined;
    };
    const retains = context.functionDeclaration(
        module,
        "animationGroupMaskRetainsTarget",
    );
    const retainsBody = lowerPinnedBody(
        retains.file,
        retains.declaration.body!.statements,
        {
            bindings,
            calls: new Map(),
    ...
    const resolver = context.functionDeclaration(
        module,
        "resolveAnimationMask",
    );
    const resolveBindings = new Map(bindings);
    resolveBindings.set("numNodes", { cpp: "node_count", type: "scalar" });
    const resolveBody = lowerPinnedBody(
        resolver.file,
        resolver.declaration.body!.statements,
        {
            bindings: resolveBindings,
            calls: new Map([
                [
                    "animationGroupMaskRetainsTarget",
                    (args) => `gltf_animation_mask_retains(${args.join(", ")})`,
                ],
            ]),
            expression: sharedExpression,
        },
        ...
        statement(statement, lowerer, indent) {
            if (
                ...
                if (
                    ...
                    variable.name.text === "names" &&
                    variable.initializer
                ) {
                    context.assertExpressionShape(
                        variable.initializer,
                        "mask.names",
                        "Controller mask names identity",
                    );
                    return [`${indent}const auto& names = mask->names;`];
                }
            }
            ...
            if (
                ...
                if (context.expressionMatchesShape(expression.left, "cNames")) {
                    context.assertExpressionShape(
                        expression.right,
                        "names",
                        "Controller cached names identity",
                    );
                    return [`${indent}cache.names = std::addressof(names);`];
                }
                if (
                    context.expressionMatchesShape(
                        expression.left,
                    ...
                    if (
                        ...
                        allocation.expression.text !== "U8" ||
                        allocation.arguments?.length !== 1
                    )
                        context.contractError(
                            allocation,
                            "Expected lazy source node-mask allocation.",
                        );
                    return [
                        `${indent}pose.masked_nodes = std::vector<std::uint8_t>(static_cast<std::size_t>(${lowerer.expression(allocation.arguments[0]!)}));`,
                        `${indent}cache.allocated = true;`,
                    ];
                }
            }
            if (
                ...
                context.assertExpressionShape(
                    ...
                    "Controller mask resolver arguments",
                );
                return [
                    `${indent}gltf_resolve_animation_mask(mask,node_names,pose.masked_nodes,static_cast<double>(pose.nodes.size()));`,
                ];
            }
            return undefined;
        },
            ...
            statement(statement, lowerer, indent) {
                if (
                    ...
                    context.assertExpressionShape(
                        ...
                        "Mask factory resolver installation",
                    );
                    return [];
                }
                if (
                ...
                const seen = new Set<string>(),
                    lines = [`${indent}auto mask = std::make_shared<Mask>();`];
                for (const property of statement.expression.properties) {
                    if (
                        ...
                        context.contractError(
                            property,
                            "Unsupported mask factory field.",
                        );
                    if (
                        !ts.isIdentifier(property.name) ||
                        !["mode", "names", "disabled"].includes(
                            property.name.text,
                        ) ||
                        ...
                        context.contractError(
                            property,
                            "Changed mask factory fields.",
                        );
                    const name = property.name.text;
                    seen.add(name);
                    const initializer = ts.isShorthandPropertyAssignment(
                        property,
                    )
                        ? property.name
                    ...
                    if (name === "names") {
                        context.assertExpressionShape(
                            initializer,
                            "names.slice()",
                            "Source mask names copy",
                        );
                        lines.push(`${indent}mask->names = names;`);
                    } else
                        lines.push(
                            `${indent}mask->${name} = ${lowerer.expression(initializer)};`,
                        );
                }
                if (seen.size !== 3)
                    context.contractError(
                        statement,
                        "Missing mask factory fields.",
                    );
