...
export function lowerGltfWeightedAnimationPasses(
    ...
    const bindings = new Map<string, PinnedBinding>([
        ...
        ["groups", { cpp: "groups", type: "opaque" }],
        [
            "groups.length",
            { cpp: "static_cast<std::int64_t>(groups.size())", type: "scalar" },
        ],
        ...
        [
            "group._gltfMixer",
            {
                cpp: "transport.mixer(group)",
                type: "opaque",
                absentCpp: "!transport.mixer(group)",
            },
        ],
        ["mixer", { cpp: "mixer", type: "opaque", absentCpp: "!mixer" }],
        [
            "mixer[GLTF_NODES]",
            { cpp: "transport.nodes_key(mixer)", type: "opaque" },
        ],
        [
            "getTarget(scratch, mixer).active",
            { cpp: "transport.get_target(mixer).active", type: "bool" },
        ],
        ["target", { cpp: "target", type: "opaque" }],
        ["target.active", { cpp: "target.active", type: "bool" }],
        ["key", { cpp: "key", type: "opaque" }],
        ["manager", { cpp: "transport", type: "opaque" }],
        ["manager.engine", { cpp: "transport.engine()", type: "opaque" }],
    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        expression(node, lowerer) {
            ...
            const expected = (
                count: number,
                owners: ReadonlyArray<readonly [number, string]>,
            ) => {
                if (args.length !== count)
                    context.contractError(
                        node,
                        "Changed weighted animation transport arity.",
                    );
                for (const [index, shape] of owners)
                    context.assertExpressionShape(
                        args[index]!,
                        shape,
                        "Weighted animation source identity",
                    );
            };
            if (callee === "keys.clear") {
                expected(0, []);
                return "keys.clear()";
            }
            ...
            if (callee === "getTarget") {
                ...
                return "transport.get_target(mixer)";
            }
            if (callee === "advanceGroupTime") {
                ...
                return `transport.advance_group_time(group, mixer, ${lowerer.expression(args[2]!)})`;
            }
            ...
            if (callee === "accumulateAdditiveGroup") {
                ...
                return "transport.accumulate_additive_group(group, mixer)";
            }
            if (callee === "tickAnimationCore") {
                ...
                return `transport.tick_animation_core(group, ${lowerer.expression(args[1]!)})`;
            }
            if (callee === "uploadTarget") {
                ...
                return "transport.upload_target(target)";
            }
            return undefined;
        },
        statement(statement, lowerer, indent) {
            if (
                ...
                const adapters = new Map([
                    [
                        "scratch",
                        ["getScratch(manager)", "auto& scratch = transport;"],
                    ],
                    ["keys", ["scratch.keys", "auto& keys = scratch.keys;"]],
                    [
                        "groups",
                        [
                            "getAnimationGroups(manager)",
                            "const auto& groups = transport.groups;",
                        ],
                    ],
                    [
                        "group",
                        [
                            "groups[groupIndex]!",
                            "const auto group = groups.at(static_cast<std::size_t>(groupIndex));",
                        ],
                    ],
                    [
                        "mixer",
                        [
                            "group._gltfMixer",
                            "const auto mixer = transport.mixer(group);",
                        ],
                    ],
                ]);
            ...
            if (
                ...
                !context.expressionMatchesShape(
                    ...
                    "scratch.targets.forEach",
                )
            )
                return undefined;
            const call = statement.expression,
                callback = call.arguments[0];
            if (call.arguments.length !== 1 || !callback)
                context.contractError(
                    call,
                    "Expected weighted target traversal.",
                );
            if (ts.isIdentifier(callback)) {
                context.assertExpressionShape(
                    callback,
                    "resetWeightedGltfTarget",
                    "Weighted target reset callback",
                );
                return [
                    `${indent}for (auto& entry : scratch.targets) transport.reset_target(entry.second);`,
                ];
            }
            if (
                !ts.isArrowFunction(callback) ||
                callback.parameters.length !== 2 ||
                callback.parameters[0]!.name.getText(file) !== "target" ||
                callback.parameters[1]!.name.getText(file) !== "key" ||
