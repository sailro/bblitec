...
export const gltfCoreMaterialFields = new Map([
    ["_baseColorFactor", "std::vector<double>"],
    ["_metallicFactor", "double"],
    ["_roughnessFactor", "double"],
    ["_emissiveFactor", "std::vector<double>"],
    ["_baseColorImage", "GltfMaterialImage"],
    ["_metallicRoughnessImage", "GltfMaterialImage"],
    ["_normalImage", "GltfMaterialImage"],
    ["_normalScale", "double"],
    ["_occlusionTexCoord", "double"],
    ["_occlusionImage", "GltfMaterialImage"],
    ["_emissiveImage", "GltfMaterialImage"],
    ["_doubleSided", "bool"],
    ["_alphaMode", "std::string"],
    ["_alphaCutoff", "double"],
    ["_rawMatDef", "const ts::JsonValue*"],
]);
...
export function lowerGltfMaterialAssembly(context: LoweringContext): string {
    ...
    const parameters = declaration.parameters.map((parameter) => {
        if (!ts.isIdentifier(parameter.name))
            context.contractError(parameter, "Expected named material inputs.");
        return parameter.name.text;
    });
    if (parameters.length !== 5)
        context.contractError(
            declaration,
            "Expected the five core material inputs.",
        );
    ...
    const value = (
        ...
        if (
            ...
            if (left)
                return `(${left} ? ${value(node.left, type, lowerer)} : ${value(node.right, type, lowerer)})`;
        }
        if (ts.isConditionalExpression(node))
            return `(${lowerer.expression(node.condition)} ? ${value(node.whenTrue, type, lowerer)} : ${value(node.whenFalse, type, lowerer)})`;
        const property = jsonPath(node);
        if (property) {
            if (type === "std::vector<double>")
                return `double_array(${property})`;
        ...
        if (ts.isArrayLiteralExpression(node)) {
            if (type !== "std::vector<double>")
                context.contractError(
                    node,
                    "Unexpected material array storage.",
                );
    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        ...
        expression(node, lowerer) {
            ...
            if (
                ...
                ts.isTypeOfExpression(context.unwrapExpression(node.left)) &&
                ...
                if (property)
                    return `(${property} && ${property}->is_number())`;
            }
            if (
            ...
            if (
                ...
                imageFetchers.has(node.expression.text)
            ) {
                if (node.arguments.length !== 1)
                    context.contractError(
                        node,
                        "Expected one material texture info.",
                    );
                const property = jsonPath(node.arguments[0]!);
                if (!property)
                    context.contractError(
                        node,
                        "Expected a material texture property.",
                    );
        ...
        statement(statement, lowerer, indent) {
            ...
            if (ts.isArrayBindingPattern(variable.name)) {
                ...
                if (
                    ...
                    context.contractError(
                        variable,
                        "Expected concurrent core material image reads.",
                    );
                const reads = awaited.arguments[0].elements;
                if (reads.length !== variable.name.elements.length)
                    context.contractError(
                        variable,
                        "Material image arity changed.",
                    );
                const collected = "material_image_promises";
                return [
                    `${indent}const std::array ${collected}{${reads.map((read) => lowerer.expression(read)).join(", ")}};`,
                    ...variable.name.elements.map((element, index) => {
                        if (
            ...
            if (
                ...
                initializer.right.properties.length === 0
            ) {
                const left =
            ...
            if (
                ...
                context.assertExpressionShape(
                    initializer,
                    `makeImageFetcher(${parameters[0]}, ${parameters[1]}, ${parameters[3]}, ${parameters[4]})`,
                    "Material image environment",
                );
                imageFetchers.add(name);
                return [
                    `${indent}const auto ${name} = make_gltf_image_fetcher(json, image_cache, resolve_image);`,
                ];
            }
            return undefined;
        },
        returnValue(expression, lowerer) {
            ...
            if (!returned || !ts.isObjectLiteralExpression(returned))
                context.contractError(
                    declaration,
                    "Expected the core material record.",
                );
            ...
            const stores = returned.properties.map((property) => {
                if (
                    ...
                    context.contractError(
                        property,
                        "Unsupported core material member.",
                    );
                const name = property.name.text,
                    type = fields.get(name);
                if (!type || seen.has(name))
                    context.contractError(
                        property,
                        `Unrepresented core material member '${name}'.`,
                    );
                seen.add(name);
                return `result.${name} = ${value(property.initializer, type, lowerer)};`;
            });
            if (seen.size !== fields.size)
                context.contractError(
                    returned,
                    "The core material record is incomplete.",
                );
            return `[&]() { GltfCoreMaterial result; ${stores.join(" ")} return result; }()`;
        },
    });
    const { declaration: fetcher } = context.functionDeclaration(
        module,
        "makeImageFetcher",
    );
    const returnedFetcher = fetcher.body?.statements[0];
    if (
        fetcher.body?.statements.length !== 1 ||
        !returnedFetcher ||
        !ts.isReturnStatement(returnedFetcher) ||
        !returnedFetcher.expression ||
        !ts.isArrowFunction(returnedFetcher.expression) ||
        !ts.isBlock(returnedFetcher.expression.body)
    )
        context.contractError(
            fetcher,
            "Expected the per-load material image fetcher.",
        );
    const arrow = returnedFetcher.expression;
    if (!ts.isBlock(arrow.body))
        context.contractError(arrow, "Expected a material image fetch body.");
    const texture = arrow.parameters[0]?.name;
    if (arrow.parameters.length !== 1 || !texture || !ts.isIdentifier(texture))
        context.contractError(arrow, "Expected a texture info input.");
    const fetchBindings = new Map<string, PinnedBinding>([
        [
            texture.text,
            {
                cpp: "texture_info",
                type: "opaque",
                absentCpp: "texture_info == nullptr",
            },
        ],
    ]);
    const fetchBody = lowerPinnedBody(file, arrow.body.statements, {
        ...
        expression(node, lowerer) {
            ...
            if (
                ...
                ts.isIdentifier(node.expression) &&
                node.expression.text === texture.text
            )
            ...
            if (
                ...
                context.assertExpressionShape(
                    node,
                    `getTextureImageIndex(json.textures[${texture.text}.index])`,
                    "Material image source selection",
                );
            ...
            if (
                ...
                context.assertExpressionShape(
                    node.right,
                    `resolveImage(json, binChunk, ${node.left.argumentExpression.getText(file)}, baseUrl)`,
