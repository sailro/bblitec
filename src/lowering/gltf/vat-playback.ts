...
export function lowerGltfVatPlayback(context: LoweringContext): string {
    ...
    if (cpu.length !== 1)
        context.contractError(
            controller.declaration,
            "Expected one CPU animation controller entry.",
        );
    context.assertStatementShapes(
        cpu[0]!,
        cpu[0]!.body!.statements,
        `
    ...
    const body = lowerPinnedBody(file, declaration.body!.statements, {
        bindings: new Map([
            ...
            [
                "ctrl.speedRatio",
                { cpp: "group.controller.speed_ratio", type: "scalar" },
            ],
            ["ctrl.loop", { cpp: "group.controller.loop", type: "bool" }],
        ]),
        calls: new Map([
            ["ctrl._tickCpu", (args) => `tick_cpu(${args.join(", ")})`],
        ]),
        ...
        statement(node, _numeric, indent) {
            ...
            if (
                ...
                context.assertExpressionShape(
                    ...
                    "VAT controller mask synchronization",
                );
