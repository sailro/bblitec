...
export function lowerGltfTextureCache(context: LoweringContext): string {
    ...
    const variables = context.findNodes(
        declaration,
        (node): node is ts.VariableDeclaration =>
            ts.isVariableDeclaration(node),
    );
    const cache = variables.find(
        (node) => ts.isIdentifier(node.name) && node.name.text === "texCache",
    );
    const selected = variables.find(
        (node) =>
            ts.isIdentifier(node.name) && node.name.text === "getCachedTexture",
    );
    const arrow =
        selected?.initializer && context.unwrapExpression(selected.initializer);
    if (
        !cache?.initializer ||
        !ts.isNewExpression(cache.initializer) ||
        !context.expressionMatchesShape(cache.initializer.expression, "Map") ||
        cache.initializer.arguments?.length ||
        !arrow ||
        !ts.isArrowFunction(arrow) ||
        !ts.isBlock(arrow.body) ||
        arrow.parameters.length !== 2
    )
        context.contractError(
            declaration,
            "Expected the per-load texture Map and upload closure.",
        );
    const parameters = arrow.parameters.map((parameter) => {
        if (!ts.isIdentifier(parameter.name))
            context.contractError(
                parameter,
                "Expected a named texture cache input.",
            );
    ...
    const bindings = new Map<string, PinnedBinding>([
        [
            parameters[0]!,
            { cpp: "bitmap", type: "opaque", absentCpp: "!bitmap" },
        ],
    ...
    const body = lowerPinnedBody(file, arrow.body.statements, {
        ...
        expression(node, lowerer) {
            ...
            if (ts.isCallExpression(node)) {
                ...
                if (
                    context.expressionMatchesShape(node.expression, "uploadTex")
                ) {
                    const [engine, image, srgb, sampler, mipmaps] =
                        node.arguments;
                    if (
                        ...
                        context.contractError(
                            node,
                            "Expected the shared image upload boundary.",
                        );
                    context.assertExpressionShape(
                        engine,
                        "engine",
                        "Texture upload engine",
                    );
                    context.assertExpressionShape(
                        sampler,
                        "sampler",
                        "Texture upload sampler",
                    );
                    context.assertExpressionShape(
                        mipmaps,
                        "_generateMipmaps!",
                        "Texture upload mipmaps",
                    );
            ...
            if (
                ...
                arrays.has(node.expression.text)
            )
        ...
        statement(statement, lowerer, indent) {
            if (ts.isExpressionStatement(statement))
                return [
                    `${indent}${lowerer.expression(statement.expression)};`,
                ];
            ...
            return statement.declarationList.declarations.map((variable) => {
                if (!ts.isIdentifier(variable.name) || !variable.initializer)
                    context.contractError(
                        variable,
                        "Expected an initialized texture cache binding.",
                    );
                const name = variable.name.text,
                    initializer = context.unwrapExpression(
...
export function lowerGltfSampledTexture(context: LoweringContext): string {
    ...
    const arrow =
        selected[0]?.initializer &&
        context.unwrapExpression(selected[0].initializer);
    if (
        selected.length !== 1 ||
        !arrow ||
        !ts.isArrowFunction(arrow) ||
        !ts.isBlock(arrow.body) ||
        arrow.parameters.length !== 3
    )
        context.contractError(
            declaration,
            "Expected the sampled texture cache closure.",
        );
    const bindings = new Map<string, PinnedBinding>([
        ["defaultSampler", { cpp: "default_sampler", type: "opaque" }],
    ]);
    arrow.parameters.forEach((parameter, index) => {
        if (!ts.isIdentifier(parameter.name))
            context.contractError(
                parameter,
                "Expected a named sampled texture input.",
            );
        bindings.set(parameter.name.text, {
            cpp: ["bitmap", "srgb", "info"][index]!,
            type: index === 1 ? "bool" : "opaque",
        });
    });
    const body = lowerPinnedBody(file, arrow.body.statements, {
        bindings,
        ...
        expression(node, lowerer) {
            ...
            if (ts.isObjectLiteralExpression(node)) {
                ...
                if (
                    ...
                    !sampler ||
                    !ts.isPropertyAssignment(sampler) ||
                    !ts.isIdentifier(sampler.name) ||
                    sampler.name.text !== "sampler"
                )
                    context.contractError(
                        node,
                        "Expected a texture spread with sampler replacement.",
                    );
        ...
        statement(statement, lowerer, indent) {
            ...
            return statement.declarationList.declarations.map((variable) => {
                if (!ts.isIdentifier(variable.name) || !variable.initializer)
                    context.contractError(
                        variable,
                        "Expected an initialized texture wrapper binding.",
                    );
...
export function lowerGltfExtendedTexturePicker(
    ...
    const declarations = declaration
        ...
        .map((statement) => {
            if (
                ...
                context.contractError(
                    statement,
                    "Expected extension texture cache bindings.",
                );
    ...
    declarations.forEach((variable, index) => {
        if (
            ...
            variable.name.text !== expected[index] ||
            !variable.initializer
        )
            context.contractError(
                variable,
                "Expected extension texture cache state.",
            );
    });
    ...
    const bindings = new Map<string, PinnedBinding>([
        [
            "samplerFor",
            { cpp: "sampler_for", type: "opaque", absentCpp: "!sampler_for" },
        ],
    ...
    arrow.parameters.forEach((parameter, index) => {
        if (!ts.isIdentifier(parameter.name))
            context.contractError(
                parameter,
                "Expected a named texture picker parameter.",
            );
        bindings.set(parameter.name.text, {
            cpp: ["image", "srgb", "info"][index]!,
            type: index === 1 ? "bool" : "opaque",
        });
    });
    const nullable = new Set<string>();
    const maps = new Map([
        ["_localCache", "GltfLocalTextureCache"],
        ["_ids", "GltfTextureImageIds"],
    ]);
    const mapReceiver = (node: ts.Expression) => {
        const receiver = context.unwrapExpression(node);
        return ts.isIdentifier(receiver) && maps.has(receiver.text)
            ? receiver.text
            : undefined;
    };
    const body = lowerPinnedBody(file, arrow.body.statements, {
        ...
        expression(node, lowerer) {
            if (
                ...
                context.expressionMatchesShape(node.expression, "Map") &&
            ...
            if (ts.isCallExpression(node)) {
                if (ts.isPropertyAccessExpression(node.expression)) {
                    const receiver = mapReceiver(node.expression.expression),
                    ...
                    if (
                        ...
                        const key = lowerer.expression(node.arguments[0]!),
                            value = lowerer.expression(node.arguments[1]!);
                ...
                if (
                    ...
                    if (node.arguments.length !== 5)
                        context.contractError(
                            node,
                            "Expected the extension texture upload.",
                        );
                    context.assertExpressionShape(
                        node.arguments[0]!,
                        "engine",
                        "Extension upload engine",
                    );
                    context.assertExpressionShape(
                        node.arguments[4]!,
                        "generateMipmaps",
                        "Extension upload mipmaps",
                    );
                    return `upload(${node.arguments
                        .slice(1, 4)
        ...
        statement(statement, lowerer, indent) {
            ...
            return statement.declarationList.declarations.map((variable) => {
                if (!ts.isIdentifier(variable.name) || !variable.initializer)
                    context.contractError(
                        variable,
                        "Expected an initialized extension cache binding.",
                    );
                const name = variable.name.text,
                    initializer = context.unwrapExpression(
                        variable.initializer,
                ...
                const receiver =
                    ...
                    mapReceiver(initializer.expression.expression);
                if (receiver) nullable.add(name);
                if (receiver === "_localCache")
    ...
    const initial = lowerPinnedBody(
        file,
        declaration.body!.statements.slice(1, 4),
        {
            bindings: new Map([
                [
                    "samplerFor",
                ...
                return statement.declarationList.declarations.map(
                    (variable) => {
                        if (
                            ...
                            context.contractError(
                                variable,
                                "Expected cache allocation state.",
                            );
                        const name = variable.name.text,
                            initializer = context.unwrapExpression(
                                variable.initializer,
                        ...
                        if (type) {
                            if (
                                ...
                                initializer.whenFalse.kind !==
                                    ts.SyntaxKind.NullKeyword ||
                                !ts.isNewExpression(initializer.whenTrue) ||
                                !context.expressionMatchesShape(
                                    initializer.whenTrue.expression,
                                    "Map",
                                ) ||
                                initializer.whenTrue.arguments?.length
                            )
                                context.contractError(
                                    initializer,
                                    "Expected a conditional texture Map allocation.",
                                );
