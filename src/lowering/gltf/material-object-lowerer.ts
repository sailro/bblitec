...
export function lowerGltfMaterialObjectFunction(
    ...
    if (!declaration.body)
        context.contractError(
            declaration,
            "Expected a material function body.",
        );
    ...
    for (const node of context.findNodes(
        ...
        (
            ...
            isAssignmentExpression(node) || isUpdateExpression(node),
    )) {
        const targets = isAssignmentExpression(node)
            ? assignmentTargets(node.left)
            : assignmentTargets(node.operand);
        for (const target of targets) {
            const name = context.unwrapExpression(target);
            if (ts.isIdentifier(name)) reassigned.add(name.text);
        }
    }
    const snapshotType = (
        ...
        return ts.isIdentifier(node) && !reassigned.has(node.text)
            ? "const auto&"
            : "auto";
    };
    const bindings = new Map<string, PinnedBinding>();
    for (const [name, value] of target.constants ?? [])
        bindings.set(name, {
            cpp: `GltfPbrValue{${JSON.stringify(value)}}`,
            type: "opaque",
        });
    const parameters = declaration.parameters.map((parameter) => {
        if (!ts.isIdentifier(parameter.name))
            context.contractError(
                parameter,
                "Expected named material parameters.",
            );
        const sourceName = parameter.name.text;
        bindings.set(sourceName, {
            cpp: sourceName,
            type: "opaque",
            absentCpp: `!${sourceName}.truthy()`,
        });
        return sourceName === contextParameter
            ? `const ${target.contextType ?? "GltfPbrContext"}& ${sourceName}`
    ...
    const member = (
        expression: ts.Expression,
        lowerer: PinnedNumericLowerer,
    ): { receiver: string; key: string; owner: ts.Expression } | undefined => {
        const node = context.unwrapExpression(expression);
        if (ts.isPropertyAccessExpression(node))
            return {
                receiver: value(node.expression, lowerer),
                key: JSON.stringify(node.name.text),
        ...
        if (
            ...
            return {
                receiver: value(node.expression, lowerer),
                key: JSON.stringify(node.argumentExpression.text),
                owner: node.expression,
            };
        return undefined;
    };
    const object = (
        ...
        const statements = node.properties.map((property) => {
            if (ts.isSpreadAssignment(property))
                return `${result}.merge(${lowerer.expression(property.expression)});`;
            if (
                ...
                context.contractError(
                    property,
                    "Unsupported material object member.",
                );
            if (
                ...
                context.contractError(
                    property.name,
                    "Expected a named material object member.",
                );
    ...
    const body = lowerPinnedBody(file, declaration.body.statements, {
        ...
        expression(node, lowerer) {
            ...
            if (node.kind === ts.SyntaxKind.NullKeyword)
                return "GltfPbrValue{nullptr}";
            if (node.kind === ts.SyntaxKind.TrueKeyword)
                return "GltfPbrValue{true}";
            if (node.kind === ts.SyntaxKind.FalseKeyword)
                return "GltfPbrValue{false}";
            if (ts.isNumericLiteral(node))
                return `GltfPbrValue{${context.doubleLiteral(Number(node.text))}}`;
            if (ts.isStringLiteralLike(node))
                return `GltfPbrValue{${JSON.stringify(node.text)}}`;
            if (ts.isTemplateExpression(node)) {
                ...
                for (const span of node.templateSpans)
                    parts.push(
                        `${value(span.expression, lowerer)}.text()`,
                        JSON.stringify(span.literal.text),
                    );
                return `GltfPbrValue{${parts.join(" + ")}}`;
            }
            if (ts.isArrayLiteralExpression(node))
                return `GltfPbrValue::array({${node.elements.map((element) => lowerer.expression(element)).join(", ")}})`;
            if (ts.isObjectLiteralExpression(node))
                return object(node, lowerer);
            ...
            if (ts.isPrefixUnaryExpression(node)) {
                ...
                if (node.operator === ts.SyntaxKind.ExclamationToken)
                    return `GltfPbrValue{!${operand}.truthy()}`;
                if (node.operator === ts.SyntaxKind.MinusToken)
                    return `GltfPbrValue{-${operand}.number()}`;
            ...
            if (
                ...
                if (!location)
                    context.contractError(
                        node,
                        "Expected a material field increment.",
                    );
                const receiver = `material_increment_${temporary++}`,
                    previous = `material_previous_${temporary++}`;
                const sign =
                    node.operator === ts.SyntaxKind.PlusPlusToken ? "+" : "-";
                return (
                    `[&]() { ${snapshotType(location.owner)} ${receiver} = ${location.receiver}; const auto ${previous} = ${receiver}.get(${location.key}); ` +
            ...
            if (ts.isBinaryExpression(node)) {
                ...
                if (
                    ...
                    return `[&]() -> GltfPbrValue { ${snapshotType(node.left)} ${left} = ${value(node.left, lowerer)}; if (${condition}) return ${value(node.right, lowerer)}; return ${left}; }()`;
                }
                if (
                    ...
                    if (!store)
                        context.contractError(
                            node.left,
                            "Unsupported material assignment target.",
                        );
                    if (operator === ts.SyntaxKind.EqualsToken) return store;
                    const receiver = `material_target_${temporary++}`;
                    if (target)
                        return `[&]() { ${snapshotType(target.owner)} ${receiver} = ${target.receiver}; const auto previous = ${receiver}.get(${target.key}); return previous.nullish() ? ${receiver}.set(${target.key}, ${value(node.right, lowerer)}) : previous; }()`;
                    return `(${value(node.left, lowerer)}.nullish() ? ${store} : ${value(node.left, lowerer)})`;
                }
                if (operator === ts.SyntaxKind.PlusToken)
                    return `${value(node.left, lowerer)}.add(${value(node.right, lowerer)})`;
                if (operator === ts.SyntaxKind.PercentToken)
                    return `GltfPbrValue{std::fmod(${value(node.left, lowerer)}.number(), ${value(node.right, lowerer)}.number())}`;
                const token = numericOperators.get(operator);
                if (token)
                    return `GltfPbrValue{${value(node.left, lowerer)}.number() ${token} ${value(node.right, lowerer)}.number()}`;
                if (operator === ts.SyntaxKind.AsteriskAsteriskToken)
                    return `GltfPbrValue{std::pow(${value(node.left, lowerer)}.number(), ${value(node.right, lowerer)}.number())}`;
                if (operator === ts.SyntaxKind.BarToken)
                    return `GltfPbrValue{double(bbl::js::bitwise_or(${value(node.left, lowerer)}.number(), ${value(node.right, lowerer)}.number()))}`;
            }
            if (ts.isCallExpression(node)) {
                ...
                const argument = (index: number) => {
                    ...
                    if (!value)
                        context.contractError(
                            node,
                            "Missing material call argument.",
                        );
                ...
                if (
                    ...
                    return `[&]() { ${snapshotType(node.arguments[0]!)} ${result} = ${argument(0)}; ${result}.merge(${argument(1)}); return ${result}; }()`;
                }
                if (
                    ts.isPropertyAccessExpression(callee) &&
                    ["map", "some", "includes"].includes(callee.name.text) &&
                    node.arguments.length === 1
                ) {
                    ...
                    return `[&]() { ${snapshotType(callee.expression)} ${receiver} = ${value(callee.expression, lowerer)}; return ${receiver}.nullish() ? GltfPbrValue{} : gltf_pbr_${callee.name.text}(${receiver}, ${argument(0)}); }()`;
                }
                if (
                    ts.isPropertyAccessExpression(callee) &&
                    callee.name.text === "applyMaterial" &&
                    node.arguments.length === 2
                )
                ...
                if (node.expression.kind === ts.SyntaxKind.ImportKeyword) {
                    if (
                        ...
                        !/^\.\.\/material\/pbr\/(set-(transmission|dispersion|metallic-reflectance|emissive|alpha-cutoff)|enable-material-uv-transform)\.js$/.test(
                            node.arguments[0].text,
                        )
                    )
                        context.contractError(
                            node,
                            "Unsupported material feature module.",
                        );
                    return "GltfPbrValue{true}";
                }
                const callName = ts.isIdentifier(node.expression)
                    ...
                    : ts.isPropertyAccessExpression(node.expression)
                      ...
                      : undefined;
                if (
                    callName === "getPbrGroupBuilder" &&
                    node.arguments.length === 0
                )
                    return "GltfPbrValue{true}";
                if (
                    callName === "cloneTexture2D" &&
                    node.arguments.length === 2
                ) {
                ...
                if (functionName)
                    return `${functionName}(${node.arguments.map((argument) => value(argument, lowerer)).join(", ")})`;
            }
            return undefined;
        },
        statement(statement, lowerer, indent) {
            if (ts.isReturnStatement(statement) && !statement.expression)
                return [`${indent}return GltfPbrValue{};`];
            if (ts.isExpressionStatement(statement)) {
                ...
                if (
                    ...
                    context.assertExpressionShape(
                        ...
                        "PBR transmission hook registration",
                    );
                    return [
                        `${indent}register_pbr_scene_hook(register_pbr_transmission);`,
                    ];
                }
                if (
                    ts.isCallExpression(expression) &&
                    ts.isIdentifier(expression.expression) &&
                    ["_registerPbrExt", "_setDispersionSampleWgsl"].includes(
                    ...
                    if (
                        ...
                        context.contractError(
                            expression,
                            "Unsupported material composition registration.",
                        );
                    return [];
                }
                if (ts.isDeleteExpression(expression)) {
                    ...
                    if (!target)
                        context.contractError(
                            expression,
                            "Unsupported material property deletion.",
                        );
                    return [
                        `${indent}${target.receiver}.erase(${target.key});`,
                    ];
                }
            ...
            return statement.declarationList.declarations.flatMap(
                (variable) => {
                    ...
                    if (ts.isObjectBindingPattern(variable.name)) {
                        const imported = ts.isAwaitExpression(initializer)
                            ? context.unwrapExpression(initializer.expression)
                            : initializer;
                        if (
                            !ts.isCallExpression(imported) ||
                            imported.expression.kind !==
                                ts.SyntaxKind.ImportKeyword
                        )
                            context.contractError(
                                variable,
                                "Expected a material feature import.",
                            );
                        ...
                        for (const element of variable.name.elements) {
                            ...
                            if (
                                ...
                                context.contractError(
                                    element,
                                    "Unsupported material import binding.",
                                );
                            const cpp = resolveCall(importedName.text);
                            if (!cpp)
                                context.contractError(
                                    element,
                                    "Unknown material feature import.",
                                );
                            calls.set(
                                element.name.text,
                                (args) => `${cpp}(${args.join(", ")})`,
                    ...
                    if (ts.isArrayBindingPattern(variable.name)) {
                        ...
                        const lines = [
                            `${indent}const auto ${result} = ${value(initializer, lowerer)};`,
                        ];
                        variable.name.elements.forEach((element, index) => {
                            ...
                            lines.push(
                                `${indent}GltfPbrValue ${name} = ${result}.at(${context.doubleLiteral(index)});`,
                            );
                        });
                        return lines;
                    }
                    if (!ts.isIdentifier(variable.name))
                        context.contractError(
                            variable,
                            "Expected a material state binding.",
                        );
                    const name = variable.name.text;
        ...
        returnValue: (expression, lowerer) =>
            expression ? value(expression, lowerer) : "GltfPbrValue{}",
    });
...
${
    ...
    (declaration.type &&
        ...
        declaration.type.typeArguments?.[0]?.kind === ts.SyntaxKind.VoidKeyword)
        ? "    return {};\n"
        : ""
}}`;
}
