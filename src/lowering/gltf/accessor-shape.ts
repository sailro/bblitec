...
export function lowerGltfAccessorShape(context: LoweringContext): string {
    ...
    if (
        !sizes ||
        !ts.isObjectLiteralExpression(context.unwrapExpression(sizes))
    )
        context.contractError(
            declaration,
            "Expected the accessor component-count table.",
        );
    ...
    const entries = table.properties.map((property) => {
        if (!ts.isPropertyAssignment(property))
            context.contractError(
                property,
                "Expected a component-count entry.",
            );
        const key = context.propertyName(property.name),
            size = context.numericValue(property.initializer, file);
        if (!key || !Number.isSafeInteger(size) || size <= 0)
            context.contractError(
                property,
                "Expected a positive component count.",
            );
        return `        {${JSON.stringify(key)}, ${size}},`;
    });
    const componentCount = context.variableInitializer(
        declaration,
        "componentCount",
    );
    const count = lowerPinnedBody(
        file,
        [ts.factory.createReturnStatement(componentCount)],
        {
            bindings: new Map(),
            calls: new Map(),
    ...
    const switches = context.findNodes(
        declaration,
        (node): node is ts.SwitchStatement => ts.isSwitchStatement(node),
    );
    const dispatch = switches[0];
    if (switches.length !== 1 || !dispatch)
        context.contractError(
            declaration,
            "Expected accessor constructor selection.",
        );
    const nativeReads: Readonly<Record<number, string>> = {
        5120: "I8",
        5121: "U8",
        5122: "I16",
        5123: "U16",
    ...
    for (const clause of dispatch.caseBlock.clauses) {
        ...
        const assignments = context.findNodes(
            ...
            (node): node is ts.BinaryExpression =>
                ...
                context.expressionMatchesShape(node.left, "Ctor"),
        );
        if (
            assignments.length !== 1 ||
            !context.expressionMatchesShape(
                assignments[0]!.right,
                nativeReads[context.numericValue(clause.expression, file)] ??
                    "undefined",
            )
        )
            context.contractError(
                clause,
                "Accessor constructor does not match its native binary read.",
            );
    }
    ...
    const width = lowerPinnedBody(file, [dispatch], {
        ...
        statement(statement, _lowerer, indent) {
            ...
            if (
                ...
                !context.expressionMatchesShape(value.expression, "Error")
            )
                context.contractError(
                    statement,
                    "Expected an unsupported accessor type error.",
                );
            return [
                `${indent}throw std::runtime_error("Unsupported glTF accessor component type.");`,
            ];
        },
    });
