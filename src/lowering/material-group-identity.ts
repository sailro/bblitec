...
export function materialGroupIdentity(
    context: LoweringContext,
    family: "standard" | "shader" | "node",
): string {
    if (family === "node") {
        ...
        if (
            ...
            initializer.parent.parent.parent.parent !== declaration.body
        )
            context.contractError(
                initializer,
                "Expected a fresh node material builder closure.",
            );
        const families = declaration.body.statements.filter(
            (node) =>
                ts.isExpressionStatement(node) &&
                ts.isBinaryExpression(node.expression) &&
                context.expressionMatchesShape(
                    node.expression.left,
                    "_buildGroup._materialFamily",
                ),
        );
        context.assertStatementShapes(
            ...
            "Node builder family identity",
        );
        context.assertExpressionShape(
            context.propertyInitializer(
                context.objectInitializer(declaration, "material"),
                "_buildGroup",
            ),
            "_buildGroup",
            "Node material builder identity",
        );
    ...
    const factory = context.functionDeclaration(
        factoryModule,
        `create${title}Material`,
    ).declaration;
    const properties = context.findNodes(
        factory,
        (node): node is ts.PropertyAssignment =>
            ts.isPropertyAssignment(node) &&
            ts.isIdentifier(node.name) &&
            node.name.text === "_buildGroup",
    );
    if (properties.length !== 1)
        context.contractError(
            factory,
            "Expected one material group builder identity.",
        );
    context.assertExpressionShape(
        properties[0]!.initializer,
        `get${title}GroupBuilder()`,
        "Material builder singleton selection",
    );
    const getter = context.functionDeclaration(
        `src/material/${family}/${family}-group-builder.ts`,
        `get${title}GroupBuilder`,
    ).declaration;
    const statements = getter.body!.statements;
    context.assertStatementShapes(
        getter,
        [statements[0]!, ...statements.slice(-2)],
        `
        if (_${family}GroupBuilder) { return _${family}GroupBuilder; }
        builder._materialFamily = "${family}";
        return (_${family}GroupBuilder = builder);`,
        "Material builder singleton lifetime",
    );
