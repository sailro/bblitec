...
export class TextLayoutLowerer {
    ...
    public header(): string {
        ...
        const bindings = new Map<string, PinnedBinding>([
            ["fontSizePx", scalar("font_size")],
            ["text", opaque("text")],
            ["rawFont", opaque("font")],
            ["rawFont.unitsPerEm", scalar("font.units_per_em")],
            ["input", opaque("input")],
        ...
        const recordLiteral = (
            ...
            const names = expression.properties.map((property) =>
                property.name?.getText(file),
            );
            const record = [...records].find(
                ([, members]) => members.join() === names.join(),
            );
            if (!record)
                c.contractError(
                    expression,
                    "Text layout record fields changed.",
                );
            return `${record[0]}{${expression.properties
                .map((property) => {
                    ...
                    if (!ts.isPropertyAssignment(property))
                        c.contractError(
                            property,
                            "Text layout requires ordinary record fields.",
                        );
                    return lowerer.expression(property.initializer);
                })
                .join(", ")}}`;
        };
        const expr: NonNullable<PinnedNumericScope["expression"]> = (
            node,
            lowerer,
        ) => {
            ...
            if (ts.isPropertyAccessExpression(node)) {
                ...
                if (
                    ...
                    containers.has(owner.text)
                )
                    return `static_cast<double>(${lowerer.expression(owner)}.size())`;
                if (fields[name])
                    return `(${lowerer.expression(owner)})${ts.isIdentifier(owner) && owner.text === "last" ? "->" : "."}${fields[name]}`;
            }
            if (ts.isElementAccessExpression(node)) {
                const owner = c.unwrapExpression(node.expression);
                if (ts.isIdentifier(owner) && containers.has(owner.text))
                    return `${lowerer.expression(owner)}.at(${index(lowerer.expression(node.argumentExpression))})`;
            }
            if (ts.isObjectLiteralExpression(node))
                return recordLiteral(node, lowerer);
            if (
                ...
                if (name === "trim" && node.arguments.length === 0)
                    return `text_trim(${lowerer.expression(owner)})`;
                if (
                    name === "charCodeAt" &&
                    ts.isIdentifier(owner) &&
        ...
        const calls = new Map([
            ...
            [
                "rawFont.scaleForSize",
                (args: readonly string[]) => `(${args[0]} / font.units_per_em)`,
            ],
            [
                "rawFont.glyphId",
                (args: readonly string[]) => {
                    if (args[0] !== "32.0")
                        throw new Error(
                            "Text layout requested a new nominal glyph query.",
                        );
                    return "font.space_glyph";
                },
            ],
            ...
            [
                ...
                (args: readonly string[]) => {
                    if (args.length !== 2)
                        throw new Error("Text shaping append arity changed.");
                    return `input.append(${args[0]?.startsWith('"') ? "U" : ""}${args[0]})`;
                },
            ],
        ...
        const scope: PinnedNumericScope = {
            ...
            forOf: (range, name) => {
                if (range === "line")
                    return {
                        range: "line",
                        bindings: new Map([[name, opaque(name)]]),
                    };
                if (range === "lineWidths")
                    return {
                        range: "lineWidths",
                        bindings: new Map([[name, scalar(name)]]),
                    };
                return undefined;
            },
            statement: (statement, lowerer, indent) => {
                if (
                    ...
                    const name = decl.name.text,
                        init = c.unwrapExpression(decl.initializer);
                    const emit = (
                        cpp: string,
                        storage: PinnedBinding = opaque(name),
                    ): string[] => {
                        bindings.set(name, storage);
                        return [`${indent}${cpp}`];
                    };
                    if (name === "rawFont") {
                    ...
                    if (name === "collapsed") {
                        c.assertExpressionShape(
                            init,
                            'text.replace(/\\t/g, " ".repeat(tabSize)).replace(/ +/g, " ")',
                            "Text whitespace preprocessing",
                        );
                        return emit(
                            `const auto collapsed = text_collapse(text, ${lowerer.expression(ts.factory.createIdentifier("tabSize"))});`,
                        );
                    }
                    if (name === "paragraphs") {
                        c.assertExpressionShape(
                            init,
                            'collapsed.split("\\n")',
                            "Text paragraph delimiter",
                        );
                        return emit(
                            "const auto paragraphs = text_paragraphs(collapsed);",
                        );
                    }
                    if (name === "input") {
                        c.assertExpressionShape(
                            init,
                            "scratchInput ??= new UnicodeBuffer()",
                            "Text shaping input scratch",
                        );
                        return emit("thread_local std::u32string input;");
                    }
                    if (name === "output") {
                        c.assertExpressionShape(
                            init,
                            "scratchOutput ??= new GlyphBuffer()",
                            "Text shaping output scratch",
                        );
                        return emit("thread_local TextShapeOutput output;");
                    }
                    if (name === "ends") {
                        c.assertExpressionShape(
                            init,
                            "scratchEnds ??= new Int32Array(BATCH_PARAGRAPHS)",
                            "Text paragraph boundary scratch",
                        );
                        return emit(
                            `thread_local std::array<double, ${c.numericValue(c.variableInitializer(file, "BATCH_PARAGRAPHS"), file)}> ends{};`,
                            { cpp: "ends", type: "f64-buffer" },
                        );
                    }
                    if (
                        name === "lines" ||
                        ...
                        if (
                            ...
                            c.contractError(
                                init,
                                "Text layout collection must start empty.",
                            );
                        const type =
                            name === "lines"
                    ...
                    if (name === "infos" || name === "positions") {
                        c.assertExpressionShape(
                            init,
                            `output.${name}`,
                            "Shaping output view",
                        );
                        return emit(`const auto& ${name} = output.${name};`);
                    }
                    if (name === "textAlign")
                        return emit(
                            `const auto textAlign = ${lowerer.expression(init)};`,
                        );
                    if (name === "pos" || name === "line")
                        return emit(
                            `const auto& ${name} = ${lowerer.expression(init)};`,
                        );
                    if (name === "last") {
                        c.assertExpressionShape(
                            init,
                        ...
                        return emit(
                            "const auto* last = currentLine.empty() ? nullptr : &currentLine.back();",
                            {
                                cpp: "last",
                                type: "opaque",
                                absentCpp: "last == nullptr",
                            },
                        );
                    }
                }
                if (ts.isExpressionStatement(statement)) {
                    ...
                    if (
                        ...
                        const owner = value.expression.expression.getText(file),
                            name = value.expression.name.text;
                        if (
                            name === "push" &&
                            ["lines", "currentLine", "placed"].includes(
                                owner,
                            ) &&
                            ...
                            return [
                                `${indent}${owner}.push_back(${ts.isArrayLiteralExpression(arg) && arg.elements.length === 0 ? "{}" : lowerer.expression(arg)});`,
                            ];
                        }
                        if (owner === "input" && name === "addStr")
                            c.assertExpressionShape(
                                value.arguments[1]!,
                                "input.length",
                                "Shaper append cluster origin",
                            );
                    }
                    if (
                        ts.isBinaryExpression(value) &&
                        ts.isIdentifier(value.left) &&
                        value.left.text === "currentLine" &&
                        value.operatorToken.kind ===
                            ts.SyntaxKind.EqualsToken &&
                        ts.isArrayLiteralExpression(value.right) &&
                        !value.right.elements.length
                    )
            ...
            returnValue: (expression) => {
                if (!expression || !ts.isObjectLiteralExpression(expression))
                    c.contractError(
                        declaration,
                        "Text layout return record changed.",
                    );
                const names = [
                    "_glyphs",
                    "_pixelsPerFontUnit",
                ...
                if (
                    ...
                    c.contractError(
                        expression,
                        "Text layout result fields changed.",
                    );
