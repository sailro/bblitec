...
export function lowerPbrMaterialGroups(context: LoweringContext): string {
    ...
    if (!ts.isVariableStatement(statement) || !ts.isBlock(statement.parent))
        context.contractError(
            initializer,
            "Expected the mesh material-group selection.",
        );
    const next =
        statement.parent.statements[
            statement.parent.statements.indexOf(statement) + 1
        ];
    if (!next || !ts.isIfStatement(next))
        context.contractError(
            statement,
            "Expected the material-group attachment guard.",
        );
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "mesh.material._buildGroup",
            { cpp: "source_material_group_key(*material)", type: "scalar" },
        ],
        ...
        [
            "ctx._built",
            { cpp: "scene.state->material_groups_built", type: "bool" },
        ],
    ]);
    const deferred = (call: ts.CallExpression, indent: string): string[] => {
        ...
        if (
            ...
            context.contractError(
                call,
                "Expected the deferred material-group builder.",
            );
                ...
                statement(node, _numeric, depth) {
                    if (ts.isVariableStatement(node)) {
                        context.assertStatementShapes(
                            ...
                            "Deferred PBR group invocation",
                        );
                        return [
                            `${depth}const auto outputs = capture_material_outputs(scene, group->meshes);`,
                            `${depth}if (build == 1) build_pbr_material_group(scene, group->meshes);`,
                        ];
                    }
                    if (ts.isIfStatement(node)) {
                        context.assertStatementShapes(
                            node,
                            [node],
                            "if (result.updater) { ctx._uniformUpdaters.push(result.updater); }",
                            "Native PBR updater transport",
                        );
                        return [];
                    }
                    ...
                    if (
                        ...
                        return [
                            `${depth}append_material_outputs(scene, outputs);`,
                        ];
                    if (
                        context.expressionMatchesShape(
                            node.expression,
                        ...
                        return [`${depth}group->outputs = outputs;`];
                    context.assertExpressionShape(
                        node.expression,
                        "group!.r = result.rebuildSingle",
                        "Completed PBR group publication",
                    );
        ...
        return [
            `${indent}if (build < 4) scene.deferred_builders.emplace_back([owner = std::weak_ptr<SceneState>(scene.state), group, build] {`,
            `${indent}    const auto state = owner.lock();`,
            `${indent}    if (!state || state->disposed) return;`,
            `${indent}    auto scene = Scene::from_state(state);`,
            body,
    ...
    const body = lowerPinnedBody(file, [statement, next], {
        ...
        expression(node) {
            ...
            if (
                ...
                return "source_material_group_key(*material)";
            if (
                node.kind === ts.SyntaxKind.UndefinedKeyword ||
                (ts.isIdentifier(node) && node.text === "undefined")
            )
                return "std::uint64_t{0}";
            if (context.expressionMatchesShape(node, "ctx._groups.get(build)"))
                return "scene_material_group(scene, build)";
            return undefined;
        },
        statement(node, lowerer, indent) {
            if (
                ...
                if (
                    ...
                    ["build", "group"].includes(variable.name.text)
                )
                    return [
                        `${indent}auto ${variable.name.text} = ${lowerer.expression(variable.initializer)};`,
                    ];
            }
            if (!ts.isExpressionStatement(node)) return undefined;
            const expression = context.unwrapExpression(node.expression);
            if (context.expressionMatchesShape(expression, "group = []"))
                return [
                    `${indent}group = std::make_shared<SourceMaterialGroupState>();`,
                ];
            if (
                context.expressionMatchesShape(
                    expression,
                ...
                return [
                    `${indent}store_scene_material_group(scene, build, group);`,
                ];
            ...
            if (
                ...
                return deferred(expression, indent);
            if (
                context.expressionMatchesShape(
                    expression.expression,
                    "group.push",
                ...
                if (expression.arguments.length !== 1)
                    context.contractError(
                        expression,
                        "Expected one PBR group member.",
                    );
                return [
                    `${indent}group->meshes.push_back(${lowerer.expression(expression.arguments[0]!)});`,
                ];
            }
            if (
                context.expressionMatchesShape(
    ...
    const registry = context.functionDeclaration(
        "src/scene/mesh-scene-registry.ts",
        "enqueueMaterialSwap",
    );
    const enqueue = lowerPinnedBody(
        registry.file,
        registry.declaration.body!.statements,
        {
            bindings: new Map([["mesh", { cpp: "mesh", type: "opaque" }]]),
            calls: new Map([
                [
                    "scene._materialSwapQueue.includes",
                    (args) =>
                        `pbr_mesh_list_contains(scene.state->pbr_material_swap_queue, ${args.join(", ")})`,
                ],
                [
                    "scene._materialSwapQueue.push",
                    (args) =>
                        `scene.state->pbr_material_swap_queue.push_back(${args.join(", ")})`,
                ],
            ]),
            returnValue: (expression) => {
                if (expression)
                    context.contractError(
                        expression,
                        "Expected a void material queue return.",
                    );
                return "";
            },
        },
    ...
    context.assertStatementShapes(
        buildScene.declaration,
        [buildScene.declaration.body!.statements.at(-1)!],
        "await ctx._rebuildHook?.(ctx);",
        "Scene build rearmed group hook",
    );
    const clear = buildScene.declaration.body!.statements.find(
        ts.isIfStatement,
    );
    if (!clear)
        context.contractError(
            buildScene.declaration,
            "Expected the initial material swap queue reset.",
        );
    const prepare = lowerPinnedBody(buildScene.file, [clear], {
        ...
        statement(node, _lowerer, indent) {
            ...
            context.assertExpressionShape(
                ...
                "Initial material swap queue reset",
            );
            return [`${indent}scene.state->pbr_material_swap_queue.clear();`];
        },
    });
    const builtStore = context.findNodes(
        buildScene.declaration,
    ...
    if (builtStore.length !== 1)
        context.contractError(
            buildScene.declaration,
            "Expected one completed scene build publication.",
        );
    const finish = lowerPinnedBody(
        buildScene.file,
        [ts.factory.createExpressionStatement(builtStore[0]!)],
        {
            bindings: new Map([
                [
                    "ctx._built",
...
function lowerPbrGroupBuild(context: LoweringContext): string {
    ...
    const gamma = context.variableInitializer(declaration, "hasGammaAlbedo")
        .parent.parent.parent;
    const scan = declaration.body!.statements.find(
        (statement) =>
            ...
            context.hasNode(
                ...
                (node) =>
                    ts.isBinaryExpression(node) &&
                    context.expressionMatchesShape(node.left, "hasGammaAlbedo"),
            ),
    );
    const guard = declaration.body!.statements.find(
        (statement) =>
            ...
            context.expressionMatchesShape(
                ...
                "!hasGammaAlbedo || !group.r",
            ),
    );
    if (
        !ts.isVariableStatement(gamma) ||
        !scan ||
        !ts.isForStatement(scan) ||
        !ts.isBlock(scan.statement) ||
        !guard
    )
        context.contractError(
            declaration,
            "Expected the PBR group gamma-albedo invalidator.",
        );
    const statements = scan.statement.statements.filter(
        (statement) =>
    ...
    const projectedScan = ts.factory.updateForStatement(
        scan,
        scan.initializer,
        scan.condition,
        scan.incrementor,
    ...
    context.assertExpressionShape(
        groupRead,
        "scene._groups.get(meshes[0]!.material!._buildGroup)!",
        "PBR builder group identity",
    );
    const bindings = new Map<string, PinnedBinding>([
        [
            "meshes.length",
            { cpp: "static_cast<double>(meshes.size())", type: "scalar" },
        ],
    ...
    const body = lowerPinnedBody(
        ...
        [
            gamma,
            projectedScan,
            groupRead.parent.parent.parent as ts.VariableStatement,
            guard,
        ],
        {
            bindings,
            ...
            statement(statement, lowerer, indent) {
                if (
                    ...
                    if (!operator)
                        context.contractError(
                            expression,
                            "Expected the PBR gamma-albedo fold.",
                        );
                    return [
                        `${indent}hasGammaAlbedo = hasGammaAlbedo ${operator} ${lowerer.expression(expression.right)};`,
                    ];
                }
                if (
                    !ts.isVariableStatement(statement) ||
                ...
                if (variable.name.text === "m") {
                    ...
                    if (
                        ...
                        context.contractError(
                            read,
                            "Expected the gamma-albedo group mesh read.",
                        );
                    return [
                        `${indent}const auto m = meshes.at(static_cast<std::size_t>(${lowerer.expression(read.argumentExpression)}));`,
                    ];
                }
            ...
            expression(node) {
                ...
                if (ts.isArrowFunction(node)) {
                    context.assertExpressionShape(
                        node,
                        "(mesh) => (mesh.material as PbrMaterialProps | null)?._gammaAlbedo",
                        "PBR gamma-albedo invalidation predicate",
                    );
                    return "true";
                }
                return undefined;
    ...
    if (
        ...
        !result.expression ||
        !ts.isObjectLiteralExpression(result.expression)
    )
        context.contractError(declaration, "Expected the PBR group result.");
    context.assertExpressionShape(
        context.propertyInitializer(result.expression, "rebuildSingle"),
        "rebuildSingle",
        "PBR single-mesh rebuild result",
    );
    const single = context.variableInitializer(declaration, "rebuildSingle");
    if (!ts.isArrowFunction(single))
        context.contractError(
            single,
            "Expected the PBR single-mesh builder closure.",
        );
...
function lowerPbrGroupUpdates(context: LoweringContext): string {
    const swap = context.functionDeclaration(
    ...
    if (
        ...
        returned.expression.expression.name.text !== "then"
    )
        context.contractError(
            swap.declaration,
            "Expected the asynchronous first-build import continuation.",
        );
    context.assertExpressionShape(
        returned.expression.expression.expression,
        'import("./scene-runtime-mesh-build.js")',
        "First-build module boundary",
    );
    const continuation = returned.expression.arguments[0];
    if (!continuation)
        context.contractError(
            returned,
            "Expected the first-build continuation.",
        );
    context.assertExpressionShape(
        continuation,
        "({ C }) => C(scene, builds, pending)",
        "First-build continuation dispatch",
    );
    const loop = swap.declaration.body!.statements.find(ts.isForOfStatement);
    if (!loop || !ts.isBlock(loop.statement))
        context.contractError(
            swap.declaration,
            "Expected the material swap mesh loop.",
        );
    const old = context.variableInitializer(loop, "old").parent.parent.parent;
    if (!ts.isVariableStatement(old))
        context.contractError(loop, "Expected the material swap GPU boundary.");
    const directBuild = context.variableInitializer(loop, "built").parent.parent
        .parent;
    const prefix = [
        ...loop.statement.statements.slice(
            0,
            loop.statement.statements.indexOf(old),
        ),
        directBuild as ts.Statement,
    ];
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "firstBuilds",
            {
                cpp: "first_builds",
                type: "opaque",
                absentCpp: "first_builds.empty()",
            },
        ],
    ]);
    const queueDeclaration = swap.declaration.body!.statements[0]!;
    const queueGuard = swap.declaration.body!.statements[1]!;
    const queueClear = swap.declaration.body!.statements.find(
        (node) =>
            ts.isExpressionStatement(node) &&
            context.expressionMatchesShape(node.expression, "q.length = 0"),
    );
    const noBuilds = swap.declaration.body!.statements.find(
        (node) =>
            ts.isIfStatement(node) &&
            context.expressionMatchesShape(node.expression, "!firstBuilds"),
    );
    if (!queueClear || !noBuilds)
        context.contractError(
            swap.declaration,
            "Expected material swap queue completion.",
        );
    const projectedLoop = ts.factory.updateForOfStatement(
        loop,
        loop.awaitModifier,
        loop.initializer,
        loop.expression,
    ...
    const select = lowerPinnedBody(
        swap.file,
        [queueDeclaration, queueGuard, projectedLoop, queueClear, noBuilds],
        {
            bindings,
            calls: new Map(),
            ...
            returnValue: (expression) => {
                if (expression)
                    context.assertExpressionShape(
                        expression,
                        "pending",
                        "Native swap completion",
                    );
                return expression
                    ? "dispatch_pbr_runtime_builds(scene, runtime_builds, {})"
                    : "";
            },
            statement(statement, _lowerer, indent) {
                if (
                    ...
                    if (
                        ...
                        return undefined;
                    if (statement === directBuild)
                        return [
                            `${indent}publish_single_material_output(scene, mesh, group);`,
                        ];
                    const named = new Map([
                        [
                            "q",
                            [
                                "scene._materialSwapQueue",
                        ...
                        [
                            ...
                            [
                                "mesh.material",
                                "const auto* mat = pbr_mesh_material(scene, mesh);",
                            ],
                        ],
                        [
                            "runtimeBuild",
                        ...
                        [
                            "group",
                            [
                                "scene._groups.get(mat._buildGroup)",
                                "const auto group = scene_material_group(scene, source_material_group_key(*mat));",
                            ],
                        ],
                        [
                            "rebuild",
                            [
                                "group?.r",
                                "const bool rebuild = group && group->rebuild_ready;",
                            ],
                        ],
                    ]);
                ...
                if (ts.isExpressionStatement(statement)) {
                    ...
                    if (
                        ...
                        return [
                            `${indent}first_builds.push_back({mesh, std::nullopt});`,
                        ];
                    if (
                        context.expressionMatchesShape(
                            statement.expression,
    ...
    const rebuild = context.functionDeclaration(
        "src/scene/scene-rebuild.ts",
        "rebuildSceneGroups",
    );
    ...
    if (!guard)
        context.contractError(
            rebuild.declaration,
            "Expected the scene group rebuild guard.",
        );
    const rebuildGuard = lowerPinnedBody(rebuild.file, [guard], {
        ...
        returnValue: (expression) => {
            if (expression)
                context.contractError(
                    expression,
                    "Expected a void group rebuild guard.",
                );
            return "";
        },
    });
...
function lowerPbrGroupReconciliation(context: LoweringContext): string {
    ...
    const functions = context.findNodes(
        declaration,
        (node): node is ts.FunctionDeclaration =>
            ts.isFunctionDeclaration(node) &&
            node.name?.text === "reconcileGroups",
    );
    const reconcile = functions[0];
    if (functions.length !== 1 || !reconcile?.body)
        context.contractError(declaration, "Expected group reconciliation.");
    const bindings = new Map<string, PinnedBinding>([
        ["build", { cpp: "build", type: "scalar" }],
                ...
                statement(node, _numeric, depth) {
                    if (ts.isVariableStatement(node)) {
                        ...
                        const declarations = new Map([
                            [
                                "wanted",
                                [
                                    "new Map<MeshGroupBuilder, Mesh[]>()",
                                    "SourceMaterialGroups wanted;",
                                ],
                            ],
                            [
                                "build",
                            ...
                            [
                                ...
                                [
                                    "wanted.get(build)",
                                    "const auto list = material_group_map_get(wanted, build);",
                                ],
                            ],
                            [
                                "next",
                                [
                                    "wanted.get(build) ?? []",
                                    "const auto wanted_group = material_group_map_get(wanted, build);\nconst auto next = wanted_group ? wanted_group->meshes : std::vector<MeshHandle>{};",
                                ],
                            ],
                            [
                                "group",
                                [
                                    "next as SceneMeshGroup",
                        ...
                        context.assertExpressionShape(
                            variable.initializer,
                            value[0]!,
                            "PBR group reconciliation carrier",
                        );
                        return value[1]!
                            .split("\n")
                            .map((line) => depth + line);
                    ...
                    if (
                        ...
                        if (
                            ...
                            return undefined;
                        const names = variable.name.elements.map((element) =>
                        ...
                        if (
                            ...
                            names.join(",") === "build,group"
                        )
                            return [
                                `${depth}if (scene.state->source_material_groups) for (const auto& [build, group] : *scene.state->source_material_groups) {`,
                                lower(
                        ...
                        if (
                            ...
                            names.join(",") === "build,next"
                        )
                            return [
                                `${depth}for (const auto& [build, next] : wanted) {`,
                                lower(
                                    node.statement.statements,
                                    depth + "    ",
                                ),
                        ...
                        context.contractError(
                            node,
                            "Expected the source builder reconciliation map traversal.",
                        );
                    }
                    if (ts.isExpressionStatement(node)) {
                        const operations = new Map([
                            ...
                            [
                                ...
                                "auto created = std::make_shared<SourceMaterialGroupState>(); created->meshes.push_back(mesh); wanted.set(build, created);",
                            ],
                            ["group.length = 0", "group->meshes.clear();"],
                            [
                                "group.push(...next)",
                                "group->meshes.insert(group->meshes.end(), next.begin(), next.end());",
                            ],
                            [
                                "wanted.delete(build)",
                                "(void)wanted.erase(build);",
                            ],
                            [
                                "scene._groups.set(build, group)",
                                "store_scene_material_group(scene, build, group);",
                            ],
                        ]);
                        for (const [shape, cpp] of operations)
                            if (
                                context.expressionMatchesShape(
                                    node.expression,
                                    shape,
...
function lowerPbrGroupRebuildBody(context: LoweringContext): string {
    ...
    if (!ts.isVariableStatement(variable) || !ts.isBlock(variable.parent))
        context.contractError(initializer, "Expected the live rebuild group.");
    const empty =
        variable.parent.statements[
            variable.parent.statements.indexOf(variable) + 1
        ];
    const gammaGuard = context.findNodes(
        declaration,
        (node): node is ts.IfStatement =>
            ts.isIfStatement(node) &&
            context.expressionMatchesShape(
                node.expression,
                'builder._materialFamily === "pbr" && result._G',
            ),
    )[0];
    const emptyGroup = context.findNodes(
        declaration,
        (node): node is ts.IfStatement =>
            ts.isIfStatement(node) &&
            context.expressionMatchesShape(
    ...
    if (!empty || !gammaGuard || !emptyGroup)
        context.contractError(
            declaration,
            "Expected the group rebuild publication.",
        );
    const dropFunctions = context.findNodes(
        declaration,
        (node): node is ts.FunctionDeclaration =>
            ts.isFunctionDeclaration(node) &&
            node.name?.text === "dropGroupOutput",
    );
    const drop = dropFunctions[0];
    if (dropFunctions.length !== 1 || !drop?.body)
        context.contractError(
            declaration,
            "Expected the group output teardown.",
        );
    const retainedDrop = drop.body.statements.filter((node) => {
        ...
        return ["meshes.r", "ctx._rebuildHook", "rearmed"].some((shape) =>
            context.expressionMatchesShape(expression.left, shape),
        );
    });
    if (retainedDrop.length !== 3)
        context.contractError(
            drop,
            "Expected group readiness teardown and rebuild rearming.",
        );
    const dropSource = (indent: string) =>
        `${indent}drop_material_group_outputs(scene, group);\n` +
        lowerPinnedBody(
            file,
            retainedDrop,
            {
                bindings: new Map([
                    ...
                    [
                        "ctx._rebuildHook",
                        {
                            cpp: "scene.state->material_group_rebuild_pending",
                            type: "bool",
                        },
                    ],
    ...
    const bindings = new Map<string, PinnedBinding>([
        ...
        [
            "mesh.material?._buildGroup",
            {
                cpp: "(pbr_mesh_material(scene, mesh) ? source_material_group_key(*pbr_mesh_material(scene, mesh)) : std::uint64_t{0})",
                type: "scalar",
            },
        ],
        ...
        [
            "meshes.length",
            {
                cpp: "static_cast<double>(group->meshes.size())",
                type: "scalar",
            },
        ],
    ...
    const scope = {
        ...
        statement(
            ...
            if (node === variable) {
                ...
                return [
                    ...
                    `${indent}    if (${numeric.expression(call.arguments[0].body)}) group_meshes.push_back(mesh);`,
                    `${indent}}`,
                ];
            }
            if (ts.isExpressionStatement(node)) {
                ...
                if (
                    ...
                    return [`${indent}group->gamma_invalidates = false;`];
            }
            return undefined;
        },
        returnValue: (expression: ts.Expression | undefined) => {
            if (expression)
                context.contractError(
                    expression,
                    "Expected an empty group rebuild return.",
                );
            return "";
        },
    };
...
function lowerPbrRuntimeDispatch(context: LoweringContext): string {
    ...
    const a = context.functionDeclaration(module, "A"),
        b = context.functionDeclaration(module, "B"),
        c = context.functionDeclaration(module, "C");
    const move = context.functionDeclaration(module, "moveRuntimeMeshToGroup");
    const thin = context.functionDeclaration(
        "src/mesh/thin-instance.ts",
        "buildRuntimeThinMesh",
    );
    context.assertStatementShapes(
        thin.declaration,
        thin.declaration.body!.statements,
    ...
    const bindings = new Map<string, PinnedBinding>([
        [
            "material",
            {
                cpp: "material.value",
                type: "scalar",
                absentCpp: "material.value >= scene.engine->materials.size()",
            },
        ],
        ["mesh", { cpp: "mesh", type: "opaque" }],
        [
        ...
        lowerPinnedBody(file, statements, {
            ...
            calls: new Map([
                ...
                [
                    "target.includes",
                    (args) =>
                        `pbr_mesh_list_contains(target->meshes, ${args.join(", ")})`,
                ],
                [
                    "target.push",
                    (args) => `target->meshes.push_back(${args.join(", ")})`,
                ],
                [
                    "meshes.indexOf",
                    (args) =>
            ...
            statement(node, _numeric, indent): string[] | undefined {
                if (ts.isForOfStatement(node)) {
                    if (
                        ...
                        context.contractError(
                            node,
                            "Expected runtime group migration body.",
                        );
                    ...
                    if (
                        ...
                        binding.elements
                            ...
                            .join(",") !== "groupBuilder,meshes"
                    )
                        context.contractError(
                            node,
                            "Expected runtime builder and mesh-list identities.",
                        );
                    context.assertExpressionShape(
                        node.expression,
                        "scene._groups",
                        "Runtime source group traversal",
                    );
                    return [
                        `${indent}if (scene.state->source_material_groups) for (const auto& [groupBuilder, meshes] : *scene.state->source_material_groups) {`,
                        lower(file, node.statement.statements),
                        `${indent}}`,
                    ];
                }
                if (ts.isVariableStatement(node)) {
                    ...
                    context.assertStatementShapes(
                        ...
                        "Runtime PBR group target",
                    );
                    return [
                        `${indent}auto target = scene_material_group(scene, builder);`,
                    ];
                }
                if (ts.isExpressionStatement(node)) {
                    if (
                        ...
                        return [
                            `${indent}target = std::make_shared<SourceMaterialGroupState>();`,
                        ];
                    if (
                        context.expressionMatchesShape(
                            node.expression,
                        ...
                        return [
                            `${indent}store_scene_material_group(scene, builder, target);`,
                        ];
                    if (
                        context.expressionMatchesShape(
                            node.expression,
                        ...
                        return [
                            `${indent}meshes->meshes.erase(meshes->meshes.begin() + static_cast<std::ptrdiff_t>(index));`,
                        ];
                }
                return undefined;
            },
    ...
    if (!loop || !ts.isBlock(loop.statement))
        context.contractError(
            c.declaration,
            "Expected runtime build request traversal.",
        );
            ...
            if (ts.isVariableStatement(node)) {
                ...
                if (variable.name.text === "pair") {
                    context.assertExpressionShape(
                        variable.initializer,
                        "Array.isArray(entry)",
                        "Runtime captured material request",
                    );
                    return [
                        `${indent}const bool pair = entry.material.has_value();`,
                    ];
                }
                context.assertStatementShapes(
                    node,
        ...
        b.declaration.body!.statements.slice(2),
        {
            ...
            returnValue(expression) {
                ...
                context.assertExpressionShape(
                    ...
                    "Runtime single-mesh group build",
                );
