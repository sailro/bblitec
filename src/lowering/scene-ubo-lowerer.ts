...
export class SceneUboLowerer {
    ...
    private packMatrix(): string {
        return lowerPinnedFunction(
            this.context,
            "src/math/pack-mat4-into-f32.ts",
            "packMat4IntoF32",
            [
                {
                    pinned: "view",
                ...
                },
                {
                    pinned: "mat",
                    kind: "mat4Const",
                    annotation: "Mat4 | Float32Array | Float64Array",
                    cpp: "matrix",
                    cppType: "Matrix",
    ...
    private packScene(): string {
        return lowerPinnedFunction(
            ...
            ],
            {
                ...
                leadingParameters: [
                    "ViewProjection&& get_view_projection",
                    "View&& get_view",
                    "World&& camera_world",
                ],
                calls: new Map([
                    [
                        ...
                        (args) => `get_view_projection(${args.join(", ")})`,
                    ],
                    ["getViewMatrix", (args) => `get_view(${args.join(", ")})`],
                    [
                        "packMat4IntoF32",
                        (args) => `pack_scene_matrix(${args.join(", ")})`,
                    ],
                ]),
                callShapes: new Map([
                    ["getViewProjectionMatrix", "f64-buffer"],
                    ["getViewMatrix", "f64-buffer"],
                ]),
                methods: new Map([
                    [
                        "fill",
                ...
                memberBindings: new Map<string, PinnedBinding>([
                    [
                        "camera.worldMatrix",
                        {
                            cpp: "camera_world(camera)",
                            type: "f64-buffer",
                            materializeAlias: true,
                        },
                    ],
                    [
                        "eng.useFloatingOrigin",
                        { cpp: "engine.use_floating_origin", type: "bool" },
                    ],
                    [
                        "eng.canvas.width",
                        { cpp: "engine.width", type: "scalar" },
                    ],
                    [
                        "eng.canvas.height",
                        { cpp: "engine.height", type: "scalar" },
                    ],
                    [
                        "scene._envTextures",
                        {
                            cpp: "scene.environment",
                            type: "opaque",
                            optional: {
                                ...
                                members: new Map([
                                    [
                                        "lodGenerationScale",
                                        {
                                            cpp: "scene.environment->lod_generation_scale",
                                        },
                                    ],
                                ]),
                            },
                    ...
                    [
                        ...
                        { cpp: "scene.exposure", type: "scalar" },
                    ],
                    [
                        "scene.imageProcessing.contrast",
                        { cpp: "scene.contrast", type: "scalar" },
                    ],
                    [
                        "scene.imageProcessing.toneMappingEnabled",
                        { cpp: "scene.tone_mapping_enabled", type: "bool" },
                    ],
                ]),
            },
        );
    ...
    private contributor(
        symbol: "writeFogUbo" | "writeClipPlaneUbo" | "writeEnvUbo",
        cpp: string,
    ): string {
        return lowerPinnedFunction(
            this.context,
            ...
            ],
            {
                cppName: cpp,
                returns: "void",
                templateParameters: ["class Scene"],
                arrayCopy: copyFloats,
                memberBindings: new Map<string, PinnedBinding>([
                    [
                        "scene.fog",
                        {
                            cpp: "scene.fog",
                            type: "opaque",
                            absentCpp: "!scene.fog.has_value()",
                        },
                    ],
                    ...["mode", "start", "end", "density"].map(
                        (field): [string, PinnedBinding] => [
                            `scene.fog.${field}`,
                            { cpp: `scene.fog->${field}`, type: "scalar" },
                        ],
                    ),
                    [
                        "scene.fog.color",
                        { cpp: "scene.fog->color", type: "f64-buffer" },
                    ],
                    [
                        "scene.clipPlane",
                        {
                            cpp: "(*scene.clip_plane)",
                            type: "f64-buffer",
                            absentCpp: "!scene.clip_plane.has_value()",
                        },
                    ],
                    [
                        "scene",
                        {
                            ...
                            optional: {
                                present:
                                    "scene.environment_rotation.has_value()",
                                members: new Map([
                                    [
                                        "_environmentRotation",
                                        {
                                            cpp: "(*scene.environment_rotation)",
                                        },
                                    ],
                                ]),
                            },
                    ...
                    [
                        ...
                        {
                            cpp: "scene.environment->harmonics",
                            type: "f32",
                            absentCpp:
                                "!scene.environment.has_value() || !scene.environment->has_harmonics",
                        },
                    ],
                ]),
            },
...
${this.contributor("writeEnvUbo", "write_environment_scene_uniforms")}`,
    ...
    private writeScene(): string {
        ...
        const tailIndex = declaration.body!.statements.findIndex(
            (statement) =>
                ts.isVariableStatement(statement) &&
                statement.declarationList.declarations.some(
                    (item) =>
                        ts.isIdentifier(item.name) && item.name.text === "data",
                ),
        );
        if (tailIndex < 0)
            this.context.contractError(
                declaration,
                "Scene UBO materialization boundary changed.",
            );
        const tail = ts.createSourceFile(
            `${file.fileName}-materialize`,
            `const materialize = () => {${declaration
                .body!.statements.slice(tailIndex)
        ...
        if (
            !this.context.expressionMatchesShape(
                this.context.variableInitializer(tail, "materialize"),
                `() => {
            ...
            this.context.contractError(
                declaration,
                "Scene UBO pack/contributor/upload tail changed; re-read its PAL boundary.",
            );
        const bindings = new Map<string, PinnedBinding>([
            ...
            [
                ...
                { cpp: "camera->viewport->width", type: "scalar" },
            ],
            [
                "camera.viewport.height",
                { cpp: "camera->viewport->height", type: "scalar" },
            ],
            ["task._config.rt", { cpp: "source", type: "opaque" }],
            ["task._config.rt._width", { cpp: "source.width", type: "scalar" }],
            [
                "task._config.rt._height",
                { cpp: "source.height", type: "scalar" },
            ],
            ...
            [
                "scene._envTextures",
                { cpp: "scene.environment_identity", type: "opaque" },
            ],
            ...
            ...[
                ...
                "contrast",
                "environment",
            ].map((field, index): [string, PinnedBinding] => [
                `s[${index}]`,
                {
                    cpp: `source.cache.${field}`,
                    type:
...
${lowerPinnedBody(file, declaration.body!.statements.slice(0, tailIndex), {
            ...
            [{ pinned: "samples", kind: "number", cpp: "samples" }],
            {
                ...
                returns: {
                    ...
                    value: (lowerer, value) => {
                        if (!value)
                            throw new Error(
                                "Pinned Halton generation must return its sequence.",
                            );
                        return lowerer.expression(value);
                    },
                },
                calls: new Map([
                    ["halton", (args) => `taa_halton(${args.join(", ")})`],
                ]),
            },
        );
    }
    ...
    private advance(): string {
        ...
        const bindings = new Map<string, PinnedBinding>([
            ...
            [
                "task._sourceRenderTask._suData",
                { cpp: "source.clean", type: "f32" },
            ],
            [
                "task._sourceRenderTask._sceneUBO",
                { cpp: "source", type: "opaque" },
            ],
            ...
            [
                "task._haltonIndex",
                { cpp: "state.halton_index", type: "scalar" },
            ],
            [
                "task._jitterScratch",
                { cpp: "state.jitter_scratch", type: "f32" },
            ],
        ]);
...
`;
    ...
    public storageHeader(): string {
        ...
        const cache = this.context.propertyInitializer(
            task,
            "_sceneUboCacheKey",
        );
        if (
            !ts.isNewExpression(clean) ||
            !ts.isIdentifier(clean.expression) ||
            clean.expression.text !== "F32" ||
            clean.arguments?.length !== 1 ||
            !ts.isArrayLiteralExpression(cache) ||
            cache.elements.length !== 0
        ) {
            this.context.contractError(
                task,
                "Source scene UBO scratch/cache initialization changed.",
            );
        }
        const length = this.context.numericValue(clean.arguments[0]!, file);
        const { declaration: taaFactory } = this.context.functionDeclaration(
            TAA_MODULE,
        ...
        if (
            !this.context.expressionMatchesShape(
                this.context.propertyInitializer(taa, "_halton"),
                "generateHalton(samples)",
            ) ||
            !this.context.expressionMatchesShape(
                this.context.propertyInitializer(taa, "_jitterScratch"),
                "new F32(16)",
            )
        ) {
            this.context.contractError(
