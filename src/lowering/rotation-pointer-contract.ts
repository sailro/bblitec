...
export function assertRotationPointerContract(
    context: LoweringContext,
    factory: ts.FunctionDeclaration,
): ts.ArrowFunction & { body: ts.Block } {
    const statements = [...factory.body!.statements];
    const key = (statement: ts.Statement): string => {
        if (ts.isVariableStatement(statement))
            return `var ${statement.declarationList.declarations.map((item) => (ts.isIdentifier(item.name) ? item.name.text : "binding")).join(",")}`;
        if (ts.isIfStatement(statement)) return "if";
        if (ts.isReturnStatement(statement)) return "return";
        if (ts.isExpressionStatement(statement)) {
            const expression = statement.expression;
            if (ts.isCallExpression(expression))
                return `call ${context.propertyPath(expression.expression)?.join(".")}`;
    ...
    context.assertStatementInventory(
        ...
        "factory state, geometry and registration retain order",
        [
            "var color",
            "var tessellation",
            "var thickness",
            ...
            "call addToScene",
            "var q",
            "call root.rotationQuaternion.set",
            "call root.scaling.set",
            "var ring",
            "write ring.material",
            "call ring.rotation.set",
            "write ring.parent",
            "call addToScene",
            "var collider",
            "write collider.material",
            "call collider.rotation.set",
            "write collider.visible",
            "write collider.parent",
            "call addToScene",
            "var rotationColor",
            "var rotationDisplayMaterial",
            "var rotationDisplayPlane",
            "write rotationDisplayPlane.material",
            "call rotationDisplayPlane.rotation.set",
            "write rotationDisplayPlane.visible",
            "write rotationDisplayPlane.pickable",
            "write rotationDisplayPlane.parent",
            "call addToScene",
            "var drag",
            "write drag._colliders",
            "var onRotationChanged",
            "var planeNormal",
            "var initialNormal",
            "write planeNormal.x",
            "write planeNormal.y",
            ...
            "var anglesUniform",
            "var gizmo",
            "call drag.onDragStart.add",
            "call drag.onDragEnd.add",
            "call drag.onHoverStart.add",
            "call drag.onHoverEnd.add",
            "call drag.onDrag.add",
            "var canvas",
            "if",
            "write gizmo._disposeFollow",
            "return",
        ],
        key,
    );
    const index = (name: string): number =>
        statements.findIndex((statement) => key(statement) === `var ${name}`);
    const slice = (from: string, to: string): ts.Statement[] =>
        statements.slice(index(from), index(to));
    context.assertStatementShapes(
        factory,
        slice("drag", "canvas").slice(0, 13),
        `
    ...
    context.assertStatementShapes(
        factory,
        statements.slice(index("canvas"), index("canvas") + 2),
        `
    ...
    const registrations = context.findNodes(
        factory,
    ...
    const callback = (name: string): ts.ArrowFunction => {
        ...
        if (
            found.length !== 1 ||
            found[0]!.arguments.length !== 1 ||
            !candidate ||
            !ts.isArrowFunction(candidate)
        ) {
            context.contractError(
                factory,
                `Expected exactly one rotation ${name} callback.`,
            );
        }
        return candidate;
    };
    const callbackBody = (name: string, expected: string): void => {
        ...
        if (!ts.isBlock(arrow.body))
            context.contractError(arrow, `Expected ${name} block.`);
        context.assertStatementShapes(
            arrow,
            arrow.body.statements,
            expected,
            `rotation ${name} state and explicit visual adaptation`,
        );
    };
    callbackBody(
        "onDragStart",
    ...
    callbackBody(
        "onDragEnd",
        `lastDragPoint = null; setMeshesMaterial([ring], materials.colored); rotationDisplayPlane.visible = false;`,
    );
    const hover = callback("onHoverStart");
    if (ts.isBlock(hover.body))
        context.contractError(hover, "Expected hover material expression.");
    context.assertExpressionShape(
        hover.body,
        "setMeshesMaterial([ring], materials.hover)",
        "rotation hover material",
    );
    callbackBody(
        "onHoverEnd",
        `if (!drag.dragging) { setMeshesMaterial([ring], materials.colored); }`,
    );
    const drag = callback("onDrag");
    if (
        !ts.isBlock(drag.body) ||
        drag.parameters.length !== 1 ||
        !ts.isIdentifier(drag.parameters[0]!.name) ||
        drag.parameters[0]!.name.text !== "event"
    ) {
        context.contractError(
            drag,
            "Expected the rotation drag event parameter and block.",
        );
    }
    context.assertStatementInventory(
        drag,
        drag.body.statements,
        "rotation onDrag",
        "all arithmetic translates and other statements are explicit native seams",
        [
            "var node",
            "if",
            "var wm",
            ...
            "var out",
            "call rq.set",
            "call onRotationChanged.notify",
            "write lastDragPoint",
            "write cumulativeAngle",
            "write anglesUniform[]",
            "call setShaderUniform",
        ],
        key,
    );
    context.assertStatementShapes(
        drag,
        drag.body.statements.slice(0, 3),
        `
    ...
    context.assertStatementShapes(
        drag,
        drag.body.statements.slice(10, 11),
        "const rq = node.rotationQuaternion;",
        "rotation quaternion target",
    );
    context.assertStatementShapes(
        drag,
        drag.body.statements.slice(13),
        `
    ...
    const createDrag = context.functionDeclaration(
        "src/gizmo/pointer-drag.ts",
        "createPointerDrag",
    ).declaration;
    context.assertStatementShapes(
        createDrag,
        createDrag.body!.statements,
        `return {
    ...
    for (const [name, expected] of [
        [
            ...
            "gizmo.attachedNode = node; gizmo.drag.enabled = node !== null;",
        ],
        [
