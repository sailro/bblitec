...
export function lowerPbrTransmissionTransaction(
    ...
    context.assertStatementShapes(
        mark.declaration,
        mark.declaration.body!.statements,
        ...
        statement(statement, _lowerer, indent) {
            ...
            if (ts.isExpressionStatement(statement)) {
                context.assertExpressionShape(
                    ...
                    "Precomposed linear transmission flags",
                );
                return [];
            }
...
function lowerRebuildTransaction(context: LoweringContext): string {
    const module = "src/scene/scene-rebuild.ts",
        file = context.sourceFile(module);
    const installs = context.findNodes(
        file,
        (node): node is ts.BinaryExpression =>
            ts.isBinaryExpression(node) &&
            node.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
            context.expressionMatchesShape(node.left, "ctx._p"),
    );
    ...
    if (
        ...
        !ts.isBlock(install.parent.parent)
    )
        context.contractError(
            file,
            "Expected one group-build transmission transaction receiver.",
        );
    const statements = install.parent.parent.statements;
    const index = statements.indexOf(install.parent),
        attempt = statements[index + 1];
    if (
        !attempt ||
        ...
        context.contractError(
            install,
            "Expected group-build transaction recovery.",
        );
    const cancelled = statements[index + 2],
        commit = statements[index + 3];
    if (
        !cancelled ||
        !ts.isIfStatement(cancelled) ||
        !ts.isBlock(cancelled.thenStatement) ||
        !commit
    )
        context.contractError(
            install,
            "Expected group-build transaction publication.",
        );
    const receiver = context.unwrapExpression(install.right);
    if (
        !ts.isArrowFunction(receiver) ||
        receiver.parameters.length !== 1 ||
        receiver.parameters[0]!.name.getText(file) !== "value" ||
        !ts.isBlock(receiver.body)
    )
        context.contractError(
            install,
            "Expected a transmission transaction capture.",
        );
    const bindings = new Map<string, PinnedBinding>([
        [
            "transmission",
            { cpp: "transmission", type: "opaque", absentCpp: "!transmission" },
        ],
        ...
        statement(
            ...
            if (ts.isDeleteExpression(expression)) {
                context.assertExpressionShape(
                    expression,
                    "delete ctx._p",
                    "Transmission receiver teardown",
                );
                return [
                    `${indent}scene.state->pbr_transmission_transaction = {};`,
                ];
            }
            if (
                ...
                if (
                    ...
                    context.contractError(
                        expression,
                        "Expected an optional transmission callback.",
                    );
                return [
                    `${indent}if (transmission) (*transmission).at(static_cast<std::size_t>(${numeric.expression(member.argumentExpression)}))();`,
                ];
            }
            if (
                ts.isBinaryExpression(expression) &&
                context.expressionMatchesShape(expression.left, "result")
            ) {
                context.assertExpressionShape(
                    expression.right,
                    "await builder(ctx, groupMeshes)",
                    "Native material group build boundary",
                );
    ...
    const capture = lowerPinnedBody(
        file,
        receiver.body.statements,
        scope,
        "            ",
    );
    ...
    if (!rollback)
        context.contractError(
            attempt,
            "Expected transmission rollback before GPU cleanup.",
        );
    ...
    if (
        !cancelledRollback ||
        !cancelledReturn ||
        !ts.isReturnStatement(cancelledReturn)
    )
        context.contractError(
            cancelled,
            "Expected cancelled transmission build rollback and return.",
        );
    const cancelledProjection = ts.factory.updateIfStatement(
        cancelled,
        cancelled.expression,
        ts.factory.updateBlock(cancelled.thenStatement, [
            cancelledRollback,
            cancelledReturn,
        ]),
        undefined,
