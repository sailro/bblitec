...
export function lowerBabylonCubeTexture(context: LoweringContext): string {
    ...
    const branches = context.findNodes(
        declaration,
        (node): node is ts.IfStatement =>
            ts.isIfStatement(node) &&
            context.hasCall(node, "loadCubeTexture") &&
            !context.hasNode(
                node.thenStatement,
                (child) =>
                    ts.isIfStatement(child) &&
                    context.hasCall(child, "loadCubeTexture"),
            ),
    );
    ...
    if (branches.length !== 1 || !branch)
        context.contractError(
            declaration,
            "Expected one cube texture construction branch.",
        );
    const prefix = "md.reflectionTexture";
    const present =
        '(source.contains("reflectionTexture") && !source.at("reflectionTexture").is_null())';
    ...
    const body = lowerPinnedBody(file, [branch], {
        ...
        expression(node) {
            ...
            if (context.expressionMatchesShape(node, `${prefix}.level`))
                return `${texture}.at("level").get<double>()`;
            if (context.expressionMatchesShape(node, `${prefix}.name`))
                return `${texture}.at("name").get<std::string>()`;
            return undefined;
        },
        statement(statement, lowerer, indent) {
            if (ts.isExpressionStatement(statement)) {
                ...
                if (
                    ...
                    return [
                        `${indent}material.reflection_level = static_cast<float>(${lowerer.expression(assignment.right)});`,
                    ];
            }
            if (ts.isVariableStatement(statement)) {
                ...
                if (
                    ...
                    variable.name.text === "cubeName" &&
                    variable.initializer
                )
                    return [
                        `${indent}const std::string cube_name = ${lowerer.expression(variable.initializer)};`,
                    ];
            }
            if (
                !ts.isExpressionStatement(statement) ||
                !context.hasCall(statement, "loadCubeTexture")
            )
                return undefined;
            ...
            return [
                `${indent}material.reflection_cube = load_cube(cube_name);`,
            ];
        },
    });
