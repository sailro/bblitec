...
export function lowerBabylonCamera(context: LoweringContext): string {
    ...
    if (
        ...
        context.contractError(
            declaration,
            "Expected one Babylon camera data parameter and a body.",
        );
    }
    ...
    for (const key of members.keys()) {
        bindings.set(`${root}.${key}`, {
            cpp: `cd.at("${key}").get<double>()`,
            type: "scalar",
        });
    }
    ...
    calls.set("createFreeCamera", (args) => {
        if (args.length !== 2)
            context.contractError(
                declaration,
                "Expected a FreeCamera position and target.",
            );
    ...
    const body = lowerPinnedBody(file, declaration.body.statements, {
        ...
        statement(statement, lowerer, indent) {
            ...
            if (
                ...
                initializer.expression.text !== "createFreeCamera"
            )
                return undefined;
            if (!ts.isIdentifier(local.name))
                context.contractError(
                    local,
        ...
        expression(expression, lowerer) {
            ...
            if (
                ...
                left.expression.expression.text === root &&
            ...
            if (
                ...
                left.expression.text === root &&
                members.has(left.name.text)
            ) {
        ...
        returnValue(expression, lowerer) {
            if (!expression)
                context.contractError(
                    declaration,
                    "A camera parser must return its camera handle.",
                );
