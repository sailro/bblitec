...
export class TextWeightLowerer {
    ...
    header(): string {
        ...
        c.assertFunctionBodyShape(
            c.functionDeclaration(module, "runOffset").declaration,
            "{return _offsets?.get(run)??0;}",
            "Text weight nullable offset map",
        );
        c.assertFunctionBodyShape(
            c.functionDeclaration(module, "runGroupKey").declaration,
            `{
        ...
        c.assertFunctionBodyShape(
            c.functionDeclaration(module, "variantForDevice").declaration,
            `{
        ...
        c.assertFunctionBodyShape(
            c.functionDeclaration("src/text/text-data.ts", "_resolveRunRef")
                .declaration,
            `{
        ...
        const bindings = new Map<string, PinnedBinding>([
            ...
            ["data", opaque("data")],
            ["run", opaque("run")],
            ["variantForDevice", opaque("variant_for_device")],
            [
                "MAX_WEIGHT_OFFSET",
                scalar(
                    String(
                        c.numericValue(
                            c.variableInitializer(file, "MAX_WEIGHT_OFFSET"),
                            file,
                        ),
                    ),
                ),
...
${lowerPinnedBody(file, declaration.body!.statements, {
    ...
    calls: new Map([
        ...
        [
            "Number.isFinite",
            (args: readonly string[]) => `std::isfinite(${args[0]})`,
        ],
        ...
        [
            "offsets.set",
            (args: readonly string[]) => `${args[0]}->weight=${args[1]}`,
        ],
    ...
    expression: (node) =>
        c.expressionMatchesShape(node, "_offsets===null")
            ? "!text_weight_installed"
            : undefined,
    statement: (node, lowerer, indent) => {
        if (
            ...
            if (variable.name.getText() === "target") {
                c.assertExpressionShape(
                    ...
                    '_resolveRunRef(data,run,"setFontWeightOffset")',
                    "Text weight run resolution",
                );
                return [
                    `${indent}const auto target=resolve_text_weight_run(data,run);`,
                ];
            }
            if (variable.name.getText() === "offsets") {
                c.assertExpressionShape(
                    variable.initializer!,
                    "_offsets",
                    "Text weight map alias",
                );
                return [];
            }
        }
        if (ts.isTryStatement(node)) {
            if (
                !node.catchClause ||
                node.finallyBlock ||
                node.catchClause.variableDeclaration?.name.getText() !== "err"
            )
                c.contractError(node, "Text weight rollback boundary changed.");
            return [
                `${indent}try {`,
                ...lowerer.statements(
        ...
        if (c.expressionMatchesShape(expression, "_offsets=new WeakMap()"))
            return [`${indent}text_weight_installed=true;`];
        ...
        if (path === "updateTextData") {
            c.assertExpressionShape(
                ...
                "Text weight repack owner",
            );
            return [`${indent}reset_weighted_text_data(*data);`];
        }
        if (path === "console.error") {
            c.assertExpressionShape(
                expression,
                'console.error("setFontWeightOffset: offset must be finite, got",offset)',
                "Text weight finite diagnostic",
            );
            return [
                `${indent}std::cerr<<"setFontWeightOffset: offset must be finite, got "<<offset<<'\\n';`,
            ];
        }
        if (path === "console.warn") {
            c.assertExpressionShape(
                ...
                "Text weight clamp diagnostic",
            );
            return [
                `${indent}std::cerr<<"setFontWeightOffset: offset "<<offset<<" clamped to "<<clamped<<'\\n';`,
            ];
        }
        return undefined;
    },
