...
export function gltfFlowGraphSourceUrl(context: LoweringContext): string {
    ...
    const parameter = context.methodDeclaration(module, "feature.applyAsset")
        .declaration.parameters[2]?.name;
    if (!parameter || !ts.isIdentifier(parameter))
        context.contractError(file, "Expected the interactivity load context.");
    const transform = ts.transform(file, [
        (visitorContext) => (root) => {
            const visit: ts.Visitor = (node) => {
                if (ts.isCallExpression(node)) {
                    const method = context.expressionMatchesShape(
                        node.expression,
                        "resolvePointerAccessor",
                    )
                        ? "recordFlowAccessor"
                    ...
                    if (method)
                        return ts.factory.updateCallExpression(
                            ...
                            ts.factory.createPropertyAccessExpression(
                                ts.factory.createIdentifier(parameter.text),
                                method,
                            ),
                            undefined,
                            node.arguments,
    ...
    try {
        return pinnedModuleTextUrl(
            "loader-gltf/gltf-feature-interactivity.js",
            transpileForBrowser(
...
export class GltfFlowGraphRecording {
    private readonly accessors = new WeakMap<
        object,
        FlowGraphPointerAccessor
    >();
    ...
    public constructor(
        private readonly converter: PathConverterModule,
        private readonly document: JsonRecord,
        private readonly nodes: ReadonlyMap<object, number>,
        private readonly materials: ReadonlyMap<object, number>,
    ) {}
    ...
    public resolve(pointer: string, context: unknown): PinnedAccessor | null {
        ...
        if (
            !supplied ||
            supplied.json !== this.document ||
            !Array.isArray(supplied.nodeMap) ||
            !Array.isArray(supplied.materials) ||
            Object.keys(supplied).some(
                (key) => !["json", "nodeMap", "materials"].includes(key),
            )
        )
        ...
        const map = (
            ...
            values.map((value) => {
                ...
                const object = asObject(value),
                    index = object && indices.get(object);
                if (index === undefined)
                    throw new Error(
                        `Unrepresented glTF flow-graph ${kind} identity.`,
                    );
                const root =
                    kind === "node"
                        ? new RecordingRoot({ kind, index })
                        : materialStandIn(this.document, index);
                roots.push(root);
                return root.proxy;
            });
        const accessor = this.converter.resolvePointerAccessor(pointer, {
            nodeMap: map(supplied.nodeMap, this.nodes, "node"),
            materials: map(supplied.materials, this.materials, "material"),
            json: this.document,
        });
        if (!accessor) {
            this.unresolvedPointers.add(pointer);
            return null;
        }
        ...
        const target = roots.find(
            (root) => root.proxy === accessor.target,
        )?.root;
        this.accessors.set(accessor, {
            pointer,
            type: accessor.type,
            ...(target ? { target } : {}),
    ...
    public package(
        value: unknown,
        publishedRuntimes: unknown,
    ): FlowGraphProgram[] {
        if (publishedRuntimes !== this.runtimes)
            throw new Error(
                "Unrepresented glTF flow-runtime promise publication.",
            );
        if (value === undefined) {
            if (this.started !== undefined)
                throw new Error("Unpublished glTF flow-graph setup.");
            return [];
        }
        if (!Array.isArray(value) || this.started !== value)
            throw new Error("Unrepresented glTF flow-graph publication.");
        const scope = asObject(value[0])?._assetScope;
        return value.map((value, graphIndex) => {
            const loaded = asObject(value),
                graph = asObject(loaded?.graph),
                accessors = asObject(loaded?.accessors);
            if (
                !graph ||
                !accessors ||
                loaded?.rightHanded !== true ||
                typeof loaded.resolveAccessor !== "function" ||
                !asObject(loaded._assetScope) ||
                loaded._assetScope !== scope
            )
                throw new Error("Unrepresented loaded glTF flow graph.");
            ...
            const observed: FlowGraphProgram["accessors"] = Object.fromEntries(
                [...this.unresolvedPointers].map((pointer) => [pointer, null]),
            );
            for (const [pointer, value] of Object.entries(accessors)) {
                const accessor = asObject(value),
                    recording = accessor && this.accessors.get(accessor);
                if (!recording || recording.pointer !== pointer)
                    throw new Error(
                        "Unrepresented glTF flow-graph accessor publication.",
                    );
...
function plainValue(value: unknown, path: string): unknown {
    if (typeof value === "number" && !Number.isFinite(value))
        throw new Error(
            `KHR_interactivity: ${path} is not a finite packaged number.`,
        );
    if (
        value === null ||
        value === undefined ||
    ...
    if (ArrayBuffer.isView(value)) {
        throw new Error(
            `KHR_interactivity: ${path} is a matrix, which this port does not carry.`,
        );
    }
    if (Array.isArray(value)) {
        return value.map((entry, index) =>
            plainValue(entry, `${path}[${index}]`),
        );
    }
    ...
    throw new Error(
        `KHR_interactivity: ${path} is a ${typeof value}, which this port does not carry.`,
    );
}

function records(value: unknown, label: string): JsonRecord[] {
    if (!Array.isArray(value))
        throw new Error(`Invalid glTF flow-graph ${label}.`);
...
function string(value: unknown): string {
    if (typeof value !== "string")
        throw new Error("Invalid glTF flow-graph string.");
    return value;
}
function scale(value: unknown): number {
    if (typeof value !== "number" || !Number.isFinite(value))
        throw new Error("Invalid glTF flow-graph scale.");
    return value;
}
...
function graphProgram(
    graphIndex: number,
    graph: JsonRecord,
    accessors: FlowGraphProgram["accessors"],
): FlowGraphProgram {
    ...
    return {
        ...
        variables: Object.fromEntries(
            Object.entries(variables).map(([name, value]) => {
                ...
                if (!variable)
                    throw new Error("Invalid glTF flow-graph variable.");
                return [
                    name,
                    {
        ...
        blocks: records(graph.blocks, "blocks").map((block) => ({
            ...
            signalIn: records(block.signalIn, "signal inputs").map((socket) =>
                string(socket.name),
            ),
            signalOut: records(block.signalOut, "signal outputs").map(
                (socket) => ({
                    name: string(socket.name),
                    targets: records(socket.targets, "signal targets").map(
                        (target) => ({
                            blockId: string(target.blockId),
                            socket: string(target.socket),
                        }),
                    ),
                }),
            ),
...
export function packagedFlowGraphPrograms(
    document: JsonRecord,
    assetName = "glTF",
): FlowGraphProgram[] {
    const plan = asObject(document[GLTF_MESH_PLAN]);
    if (!plan || !Array.isArray(plan.flowGraphs))
        throw new Error(
            `${assetName}: missing source flow-graph construction schedule.`,
        );
    return plan.flowGraphs.map((value, index) => {
        const program = asObject(value),
            accessors = asObject(program?.accessors);
        if (!program || program.graphIndex !== index || !accessors)
            throw new Error("Invalid packaged glTF flow graph.");
        ...
        for (const [pointer, value] of Object.entries(accessors)) {
            ...
            if (
                ...
                typeof accessor.writable !== "boolean"
            )
            ...
            observed[pointer] = {
                ...
                touches: records(accessor.touches, "accessor touches").map(
                    (touch) => {
                        if (typeof touch.write !== "boolean")
                            throw new Error(
                                "Invalid packaged glTF flow-graph touch.",
                            );
                        return {
                            ...root(touch),
        ...
        const blocks = records(program.blocks, "blocks").map((block) => {
            if (!Array.isArray(block.signalIn))
                throw new Error(
                    "Invalid packaged glTF flow-graph signal inputs.",
                );
            return {
                ...block,
                signalIn: block.signalIn.map((name) => ({
                    name: string(name),
                })),
            };
        });
