...
import { isAssignmentExpression, isUpdateExpression, propertyNameText } from "./compiler/syntax.js";
...
export interface ApiUse {
    ...
    line: number;
    column: number;
    operation: "call" | "construct" | "read" | "write" | "read-write" | "provide" | "reference";
    shape: string;
}
...
export function scanApiUsage(program: ts.Program, surface: ApiSurface, root: string,
    reached?: ReadonlySet<ts.Node>): ApiUsage {
    ...
    const ids = (symbol: ts.Symbol | undefined): string[] => {
        ...
        return [...new Set((target.declarations ?? []).flatMap(declaration =>
            surface.byDeclaration.get(declarationKey(declaration)) ?? []))];
    };
    let kinds = surfaceKinds.get(surface);
    if (!kinds) {
        kinds = new Map(surface.snapshot.items.map(item => [item.id, item.kind]));
        surfaceKinds.set(surface, kinds);
    }
    const add = (targets: readonly string[], node: ts.Node, operation: ApiUse["operation"], shape: () => string): void => {
        const selected = targets.filter(id => !(operation === "read" && kinds!.get(id) === "set") &&
            !(operation === "write" && kinds!.get(id) === "get"));
        if (!selected.length) return;
        const { file, line, character } = sourceLocation(node);
        const description = shape();
        for (const id of selected) {
            const use: ApiUse = { id, file: repositoryRelativePath(root, file.fileName), line,
                column: character, operation, shape: description };
            uses.set(JSON.stringify(use), use);
        }
    };
    const properties = (type: ts.Type, name: string): string[] =>
        [...new Set((type.isUnionOrIntersection() ? type.types : [type]).flatMap(part =>
            ids(checker.getPropertyOfType(part, name))))];
    ...
    const shape = (node: ts.Expression): string => checker.typeToString(checker.getTypeAtLocation(node), node,
    ...
    const provided = (actual: ts.Type, expected: ts.Type, seen = new Map<ts.Type, Set<ts.Type>>()): void => {
        ...
        for (const part of actual.isUnion() ? actual.types : [actual]) for (const member of checker.getPropertiesOfType(part)) {
            for (const declaration of member.declarations ?? []) {
                if ((!ts.isPropertyAssignment(declaration) && !ts.isShorthandPropertyAssignment(declaration)) ||
                    !localFiles.has(declaration.getSourceFile())) continue;
                const value = ts.isPropertyAssignment(declaration) ? declaration.initializer : declaration.name;
                if (!reached || reached.has(declaration)) add(properties(expected, member.name), declaration, "provide", () => shape(value));
                for (const contract of expected.isUnionOrIntersection() ? expected.types : [expected]) {
                    const target = checker.getPropertyOfType(contract, member.name);
                    if (target) provided(checker.getTypeOfSymbolAtLocation(member, declaration),
                        checker.getTypeOfSymbolAtLocation(target, declaration), seen);
                }
            }
        }
    };
    ...
    const visit = (node: ts.Node): void => {
        ...
        if (ts.isCallExpression(node) || ts.isNewExpression(node)) {
            ...
            if (declaration) add(surface.byDeclaration.get(declarationKey(declaration)) ?? [], node,
                ts.isNewExpression(node) ? "construct" : "call", () => `(${(node.arguments ?? []).map(shape).join(", ")})`);
            for (const [index, argument] of (node.arguments ?? []).entries()) {
                const parameter = signature?.parameters[index];
                if (parameter) provided(checker.getTypeAtLocation(argument), checker.getTypeOfSymbolAtLocation(parameter, node));
            }
        } else if (ts.isPropertyAccessExpression(node) || ts.isElementAccessExpression(node)) {
            if (!((ts.isCallExpression(node.parent) || ts.isNewExpression(node.parent)) && node.parent.expression === node)) {
                const name = ts.isPropertyAccessExpression(node) ? node.name : node.argumentExpression;
                add(ids(checker.getSymbolAtLocation(name)), node, operation(node), () => shape(node));
            }
            ...
            if (context && name !== undefined) {
                add(properties(context, name), node, "provide", () => ts.isPropertyAssignment(node) ? shape(node.initializer) : name);
            }
        } else if (ts.isBindingElement(node) && ts.isObjectBindingPattern(node.parent)) {
            const name = node.propertyName ?? node.name;
            if (ts.isIdentifier(name) || ts.isStringLiteralLike(name)) {
                add(properties(checker.getTypeAtLocation(node.parent), name.text), node, "read", () => name.text);
            }
        } else if (ts.isIdentifier(node)) {
            const targets = ids(checker.getSymbolAtLocation(node));
            const constants = targets.filter(id => kinds!.get(id) === "variable" || kinds!.get(id) === "enum-member");
            if (constants.length) add(constants, node, "read", () => shape(node));
    ...
    if (reached) {
        for (const node of reached) if (localFiles.has(node.getSourceFile())) visit(node);
    } else files.forEach(visit);
    const diagnostics = reached ? [] : files.flatMap(file => [...program.getSyntacticDiagnostics(file), ...program.getSemanticDiagnostics(file)]
        .map(diagnostic => ({ file: repositoryRelativePath(root, file.fileName),
            line: file.getLineAndCharacterOfPosition(diagnostic.start ?? 0).line + 1,
            message: ts.flattenDiagnosticMessageText(diagnostic.messageText, " ") })));
    return {
        scope: reached ? "Sites visited during successful lowering; evidence is limited to these source forms." :
