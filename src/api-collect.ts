...
if (output) {
    ...
    const suite = (): string =>
        process.env.BBLITE_API_SCENE ??
        repositoryRelativePath(root, process.argv[1] ?? "unknown");
    const translations = new Set<string>();
    observePinnedTranslation((trace) => {
        const store = sharedUpstreamStore();
        const modulePath = trace.file.fileName.replaceAll("\\", "/");
        if (
            !store.hasSource(modulePath) ||
            store.getSource(modulePath) !== trace.file.text
        )
            return;
        const metadata = {
            kind: "translation",
        ...
        const record = JSON.stringify({
            ...metadata,
            source: apiHash(trace.file.text),
        });
