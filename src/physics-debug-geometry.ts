...
function checked<T>(
    ...
    if (result[0] !== havok.Result.RESULT_OK)
        throw new Error(`Physics debug geometry: ${operation} failed.`);
    return result[1];
}
    ...
    const fields = (names: string[]) => {
        for (const name of Object.keys(shape))
            if (!names.includes(name)) {
                throw new Error(
                    `Physics debug geometry: unrepresented ${shape.type} field '${name}'.`,
                );
            }
    };
    const finite = (
        ...
        if (
            ...
            throw new Error(
                `Physics debug geometry: ${label} requires ${length ?? "only"} finite values.`,
            );
        }
        return Array.from(values);
    };
    const vector = (value: Vector3): [number, number, number] =>
        finite(value, "vector", 3) as [number, number, number];
    const rotation = (value: Quaternion): [number, number, number, number] =>
        finite(value, "quaternion", 4) as [number, number, number, number];
    const radius = (value: number): number => {
        if (!Number.isFinite(value) || value <= 0)
            throw new Error(
                "Physics debug geometry: radius must be positive and finite.",
            );
        return value;
    };
    switch (shape.type) {
        ...
        case "CYLINDER":
            ...
            return {
                type: shape.type,
                pointA: vector(shape.pointA),
                pointB: vector(shape.pointB),
                radius: radius(shape.radius),
            };
        case "CONVEX_HULL":
        case "MESH": {
            ...
            if (
                ...
                positions.length < (shape.type === "MESH" ? 9 : 12)
            ) {
                throw new Error(
                    "Physics debug geometry: incomplete vertex triples.",
                );
            }
            if (shape.type === "CONVEX_HULL")
                return { type: shape.type, positions };
            ...
            if (
                ...
                throw new Error(
                    "Physics debug geometry: triangle indices must address complete vertices.",
                );
            }
        ...
        case "HEIGHTFIELD": {
            ...
            if (
                ![shape.samplesX, shape.samplesZ].every(
                    (value) => Number.isInteger(value) && value >= 2,
                )
            ) {
                throw new Error(
                    "Physics debug geometry: heightfield sample dimensions must be integers of at least two.",
                );
            }
            if (shape.samplesX !== shape.samplesZ)
                throw new Error(
                    "Physics debug geometry: only square heightfield grids are materialized.",
                );
            return {
                type: shape.type,
                samplesX: shape.samplesX,
                samplesZ: shape.samplesZ,
                scale: vector(shape.scale),
                heights: finite(
                    shape.heights,
                    "heights",
                    shape.samplesX * shape.samplesZ,
                ),
            };
        }
        case "CONTAINER":
                ...
                children: shape.children.map((child) => {
                    for (const name of Object.keys(child))
                        if (
                            ...
                            throw new Error(
                                `Physics debug geometry: unrepresented child field '${name}'.`,
                            );
                        }
                    return {
                        shape: canonicalShape(child.shape),
                        position: vector(child.position),
        ...
        default:
            throw new Error(
                "Physics debug geometry: unrepresented shape type.",
            );
    }
}
...
function createShape(
    ...
    const buffer = (data: Float32Array | Uint32Array): number => {
        ...
        if (!address)
            throw new Error(
                "Physics debug geometry: WASM buffer allocation failed.",
            );
    ...
    try {
        switch (shape.type) {
            ...
            case "CONVEX_HULL":
                return checked(
                    havok,
                    havok.HP_Shape_CreateConvexHull(
                        buffer(new Float32Array(shape.positions)),
            ...
            case "MESH":
                return checked(
                    havok,
                    havok.HP_Shape_CreateMesh(
                        buffer(new Float32Array(shape.positions)),
                        shape.positions.length / 3,
                        buffer(new Uint32Array(shape.indices)),
            ...
            case "HEIGHTFIELD":
                return checked(
                    havok,
                    havok.HP_Shape_CreateHeightField(
                        shape.samplesX,
                        shape.samplesZ,
                        [...shape.scale],
                        buffer(new Float32Array(shape.heights)),
                    ),
                    shape.type,
                );
            case "CONTAINER": {
                const container = checked(
                    havok,
                    havok.HP_Shape_CreateContainer(),
                    shape.type,
                );
                try {
                    for (const child of shape.children) {
                        ...
                        try {
                            const result = havok.HP_Shape_AddChild(
                                container,
                                childShape,
                                [
                                    [...child.position],
                            ...
                            if (result !== havok.Result.RESULT_OK)
                                throw new Error(
                                    "Physics debug geometry: AddChild failed.",
                                );
                        } finally {
                            havok.HP_Shape_Release(childShape);
                        }
                    }
                    return container;
                } catch (error) {
                    havok.HP_Shape_Release(container);
                    throw error;
                }
            }
        }
        ...
        for (const address of allocated) havok._free(address);
    }
}
...
export async function materializePhysicsDebugGeometry(
    input: PhysicsDebugShape,
): Promise<PhysicsDebugGeometry> {
    const shape = canonicalShape(input);
    const wasm = readFileSync(wasmPath);
    const wrapper = readFileSync(
        fileURLToPath(import.meta.resolve("@babylonjs/havok")),
    );
    return cachedJsonBake(
        {
            kind: "physics-debug-geometry",
            version: "1",
        ...
        async () => {
            ...
            try {
                const geometry = checked(
                    havok,
                    havok.HP_Shape_CreateDebugDisplayGeometry(handle),
                    "CreateDebugDisplayGeometry",
                );
                try {
                    const [
                        vertexAddress,
                        ...
                        havok,
                        havok.HP_DebugGeometry_GetInfo(geometry),
                        "GetInfo",
                    );
                    return {
                    ...
                    havok.HP_DebugGeometry_Release(geometry);
                }
            } finally {
                havok.HP_Shape_Release(handle);
            }
        },
    );
}
...
export function physicsDebugShapeIdentity(input: PhysicsDebugShape): string {
    return createHash("sha256")
        .update(JSON.stringify(canonicalShape(input)))
        .digest("hex");
}
