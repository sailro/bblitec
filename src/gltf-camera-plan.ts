...
function scalar(value: unknown): number {
    if (typeof value !== "number" || !Number.isFinite(value))
        throw new Error("Invalid glTF camera scalar.");
    return value;
}
...
function matrix(value: unknown): Float32Array {
    if (
        ...
        value.length !== 16 ||
        value.some((value) => !Number.isFinite(value))
    )
...
function fields(
    camera: JsonObject,
): Omit<GltfCamera, "parentWorld" | "binding"> {
    if (typeof camera.name !== "string")
        throw new Error("Invalid glTF camera name.");
    const lanes = (value: unknown) => {
        if (!Array.isArray(value) || value.length !== 3)
            throw new Error("Invalid glTF camera vector lanes.");
        return value.map(scalar);
    };
    return {
        name: camera.name,
        position: lanes(camera.position),
        target: lanes(camera.target),
        fov: scalar(camera.fov),
        nearPlane: scalar(camera.nearPlane),
        farPlane: scalar(camera.farPlane),
        speed: scalar(camera.speed),
        angularSensitivity: scalar(camera.angularSensitivity),
        inertia: scalar(camera.inertia),
        yaw: scalar(camera.yaw),
        pitch: scalar(camera.pitch),
    };
}

/** Observe the constructed FreeCamera and its actual fixup parent. */
export function packageGltfCamera(
    value: object,
    nodes: ReadonlyMap<object, number>,
    packer: GltfGeometryPacker,
    source: SourceCameraMatrices,
): GltfCamera {
    const camera = asObject(value),
        parent = asObject(camera?.parent);
    if (!camera || !parent)
        throw new Error("Unrepresented glTF camera parent.");
    if (camera.ortho !== undefined)
        throw new Error(
            "glTF orthographic cameras require native explicit clip-plane storage.",
        );
    const position = vector(camera.position),
        target = vector(camera.target);
    const node =
        typeof parent.parent === "object" && parent.parent !== null
            ? nodes.get(parent.parent)
            : undefined;
    ...
    if (node !== undefined) {
        ...
        const local =
            parent._localMatrix ??
            source.composeTrsLocalMatrix(
                vector(parent.position),
                { ...vector(rotation), w: scalar(rotation?.w) },
                vector(parent.scaling),
            );
        binding = { node, local: packer.float32(matrix(local), 4) };
    } else if (!asObject(parent.parent)?._localMatrix)
        throw new Error("Unrepresented glTF camera ancestor.");
    return {
        ...fields({
            name: camera.name,
            ...
            speed: camera.speed,
            angularSensitivity: camera.angularSensitivity,
            inertia: camera.inertia,
            yaw: camera._yaw,
            pitch: camera._pitch,
        }),
        parentWorld: packer.float32(matrix(parent.worldMatrix), 4),
        binding,
    };
}
...
export function packagedGltfCameras(document: JsonObject): GltfCameraPlan {
    ...
    if (
        ...
        !areGltfIndices(plan.containerCameras, plan.cameras.length)
    )
    ...
    const accessors = asRecords(document.accessors),
        nodeCount = asRecords(document.nodes).length;
    const accessor = (value: unknown): number => {
        const index = asIndex(value),
            record = index === undefined ? undefined : accessors[index];
        if (
            index === undefined ||
            record?.type !== "VEC4" ||
            record.count !== 4 ||
    ...
    return {
        cameras: plan.cameras.map((value) => {
            const camera = asObject(value),
                binding = asObject(camera?.binding);
            const node = asIndex(binding?.node);
            if (
                !camera ||
                (camera.binding !== null &&
                    (!binding || node === undefined || node >= nodeCount))
            )
                throw new Error("Invalid packaged glTF camera binding.");
            return {
                ...fields(camera),
                parentWorld: accessor(camera.parentWorld),
                binding: binding
                    ? { node: node!, local: accessor(binding.local) }
                    : null,
            };
        }),
