...
export const transmissionRegistrationMarker =
    "__bblitecTransmissionRegistration";
...
export function recordingTransmissionSetter(
    ...
    const name =
        kind === "transmission" ? "setPbrTransmission" : "setPbrDispersion";
    const { file, declaration } = context.functionDeclaration(module, name);
    const material = declaration.parameters[0]?.name;
    if (!material || !ts.isIdentifier(material))
        context.contractError(
            declaration,
            "Expected the PBR setter material parameter.",
        );
    ...
    for (const call of calls)
        context.assertExpressionShape(
            ...
            "Transmission hook registration",
        );
    const transformed = ts.transform(file, [
        (visitorContext) => (root) => {
            const visit: ts.Visitor = (node) =>
                ts.isCallExpression(node) && calls.includes(node)
    ...
    const text = transpileForBrowser(
...
export function pinnedPbrTransmissionSelection(
    ...
    let enabled = false;
    const hook = createJavaScriptFunction(
        ...
        scene: object,
        engine: object,
        meshes: readonly { material: object }[],
    ) => void;
