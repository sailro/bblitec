...
export type CompiledMeshWalk =
    | { kind: "preorder" }
    | { kind: "source" | "owner-map"; parameter: string; body: string };
...
function records(value: unknown, field: string): JsonObject[] {
    if (!Array.isArray(value) || value.some((entry) => !asObject(entry))) {
        throw new Error(
            `Source mesh walks require an object array at glTF ${field}.`,
        );
    }
...
function indices(value: unknown, limit: number): number[] {
    if (
        !Array.isArray(value) ||
        value.some((entry) => asIndex(entry) === undefined || entry >= limit)
    ) {
        throw new Error(
            "Source mesh walks require valid glTF hierarchy node indices.",
        );
    }
...
export async function gltfMeshWalks(
    document: JsonObject,
    walks: readonly CompiledMeshWalk[],
): Promise<number[][]> {
    if (
        asStrings(document.extensionsUsed).some(
            (name) =>
                name === "EXT_mesh_gpu_instancing" ||
                name === "KHR_gaussian_splatting",
        )
    ) {
        throw new Error(
            "Source mesh walks do not yet represent instancing or splat loader producers.",
        );
    }
    ...
    while (pending.length) {
        ...
        if (seen.has(node))
            throw new Error(
                "Source mesh walks require a non-repeated glTF hierarchy.",
            );
        seen.add(node);
    ...
    for (const [index, node] of nodes.entries()) {
        if (asObject(node.extensions)?.EXT_mesh_gpu_instancing !== undefined) {
            throw new Error(
                "Source mesh walks do not represent instancing loader producers.",
            );
        }
        const mesh = asIndex(node.mesh);
        if (mesh === undefined) continue;
        for (const primitive of records(
            definitions[mesh]?.primitives,
            "mesh primitives",
        )) {
            if (isGaussianSplatPrimitive(primitive))
                throw new Error(
                    "Source mesh walks do not represent splat primitives.",
                );
    ...
    return walks.map((walk) => {
        const collect =
            walk.kind === "preorder"
                ...
                : (createJavaScriptFunction(
                      walk.parameter,
                      transpileCommonJs(walk.body, "source-mesh-walk.ts"),
                  ) as (container: {
                      entities: Node[];
        ...
        if (walk.kind === "owner-map") {
            if (!(result instanceof Map))
                throw new Error("Source owner walk must return a Map.");
            collected = [];
            for (const [name, entries] of result) {
                if (typeof name !== "string" || !Array.isArray(entries))
                    throw new Error(
                        "Source owner walk requires string keys and mesh arrays.",
                    );
                for (const mesh of entries) {
                    const nativeName = ownerName?.(mesh);
                    if (name !== nativeName)
                        throw new Error(
                            "Source owner walk key differs from its native node-wrapper identity.",
                        );
                    collected.push(mesh);
                }
            }
            ...
            if (!Array.isArray(result))
                throw new Error("Source mesh walk must return a mesh array.");
            collected = result;
        }
        const indices = collected.map((mesh) => {
            ...
            if (index === undefined)
                throw new Error(
                    "Source mesh walk returned an unknown loader mesh.",
                );
            return index;
        });
        ...
        if (
            indices.length !== meshes.length ||
            new Set(indices).size !== meshes.length
        ) {
            throw new Error(
                "Source mesh walks require one visit to every imported mesh; partial or repeated hierarchies are not represented.",
            );
        }
        return indices;
    });
...
export async function packageMeshWalks(
    document: JsonObject,
    walks: readonly (CompiledMeshWalk | undefined)[],
): Promise<void> {
    const demanded = walks.filter(
        (walk): walk is CompiledMeshWalk => walk !== undefined,
    );
    if (!demanded.length) return;
    if (GLTF_MESH_WALKS in document)
        throw new Error(
            "glTF source already carries compiler mesh-walk metadata.",
        );
    const observed = await gltfMeshWalks(document, demanded);
    let cursor = 0;
    document[GLTF_MESH_WALKS] = walks.map((walk) =>
        walk ? observed[cursor++]! : [],
    );
}
