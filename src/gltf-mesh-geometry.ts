...
export class GltfGeometryPacker {
    ...
    private readonly buffers = new Map<
        object,
        { offset: number; views: Map<number, number> }
    >();
    ...
    public accessor(
        ...
        if (
            !(buffer instanceof RecordedBuffer) ||
            buffer.destroyed ||
            asIndex(count) === undefined ||
            asIndex(stride) === undefined ||
            asIndex(offset) === undefined ||
            stride < components * bytes ||
            offset % bytes !== 0 ||
            stride % bytes !== 0 ||
            (layout &&
                (layout._count !== count ||
                    layout._componentType !== componentType ||
                    layout._componentCount !== components)) ||
            offset + (count ? (count - 1) * stride + components * bytes : 0) >
                buffer.size
        )
            throw new Error(
                "Unsupported recorded glTF vertex/index buffer layout.",
            );
        const view = this.bufferView(
            buffer,
            new Uint8Array(buffer.bytes),
            layout ? stride : 0,
        );
    ...
    public float32(data: Float32Array, components: 1 | 2 | 3 | 4): number {
        if (data.length % components !== 0)
            throw new Error("Incomplete glTF construction elements.");
        const view = this.bufferView(
            data,
    ...
    public geometry(
        ...
        const attribute = (
            ...
            components: number,
            layout: VertexLayout | undefined,
            required = false,
        ): void => {
            if (buffer === null && !required) return;
            if (!(buffer instanceof RecordedBuffer))
                throw new Error(`Missing recorded glTF ${name} buffer.`);
            attributes[name] = this.accessor(
                buffer,
                vertexCount,
                components,
                5126,
        ...
        return {
            ...
            indices: this.accessor(
                ...
                gpu.indexCount,
                1,
                gpu.indexFormat === "uint16" ? 5123 : 5125,
            ),
        };
    }
    ...
    public bytes(identity: object, bytes: Uint8Array): number {
        return this.bufferView(identity, bytes);
    }
}
