...
export async function packageBabylonMeshWalks(
    document: JsonObject,
    ...
    if (GLTF_MESH_WALKS in document)
        throw new Error(
            "Babylon source already carries compiler mesh-walk metadata.",
        );
    ...
    try {
        const container = await imported.module.loadBabylon(
            {},
            "https://bblite.invalid/meshes.babylon",
            { loadTextures: false, loadCamera: false },
        );
        const { getContainerMeshes } = await importPinnedModule<{
            getContainerMeshes(this: void, container: Container): Meshes;
        }>("asset-container.js");
        const observed = await evaluateMeshWalks(
            container,
