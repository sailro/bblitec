...
function scalar(value: unknown): number {
    if (typeof value !== "number" || !Number.isFinite(value))
        throw new Error("Invalid glTF light scalar.");
    return value;
}

function color(value: unknown): number[] {
    if (!Array.isArray(value) || value.length !== 3)
        throw new Error("Invalid glTF light color.");
    return value.map(scalar);
}

function kind(value: unknown): GltfLight["kind"] {
    if (value !== "point" && value !== "directional" && value !== "spot")
        throw new Error("Unsupported glTF light kind.");
    return value;
}

/** Observe an actual source light after its source scene parents are attached. */
export function packageGltfLight(
    value: object,
    ...
    const type = kind(light.lightType),
        world = light.worldMatrix;
    if (
        !(world instanceof Float32Array) ||
        world.length !== 16 ||
        world.some((value) => !Number.isFinite(value))
    )
    ...
    if (
        light._bumpLightVersion != null &&
        typeof light._bumpLightVersion !== "function"
    )
        throw new Error("Unrepresented glTF light version callback.");
    const result: GltfLight = {
        kind: type,
        world: packer.float32(world, 4),
        node,
        diffuse: color(light.diffuse),
        specular: color(light.specular),
        intensity: scalar(light.intensity),
...
function readGltfLight(
    value: unknown,
    accessorCount: number,
    nodeCount: number,
): GltfLight {
    const light = asObject(value),
        world = asIndex(light?.world),
        node = light?.node === null ? null : asIndex(light?.node);
    if (
        !light ||
        world === undefined ||
        world >= accessorCount ||
    ...
    if (typeof light.bumpVersion !== "boolean")
        throw new Error("Missing packaged glTF light callback state.");
    const result: GltfLight = {
        kind: kind(light.kind),
        world,
        node,
        diffuse: color(light.diffuse),
        specular: color(light.specular),
    ...
    if (result.kind === "spot") {
        ...
        result.spot = {
            angle: scalar(spot.angle),
            cosine: scalar(spot.cosine),
            exponent: scalar(spot.exponent),
        };
    } else if (light.spot !== undefined)
        throw new Error("Unexpected packaged glTF spot light state.");
    return result;
}
...
export function packagedGltfLights(document: JsonObject): GltfLightPlan {
    ...
    if (
        ...
        !areGltfIndices(plan.sceneLights, count) ||
        !Array.isArray(plan.lightTargets) ||
        plan.lightTargets.length !==
            (Array.isArray(definitions) ? definitions.length : 0) ||
        !plan.lightTargets.every(
    ...
    return {
        lights: plan.lights.map((light) =>
            readGltfLight(light, accessorCount, nodeCount),
        ),
        sceneLights: plan.sceneLights,
        lightTargets: plan.lightTargets,
    };
}
