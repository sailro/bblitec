 ...
 */
export function inspectApiBindings(
    ...
    return Object.entries(snapshot.exports)
        .filter(
            ([, owner]) =>
                functions.has(owner) &&
                (owners === undefined || owners.has(owner)),
        )
        .map(([name, owner]) => {
            let route: IntrinsicRoute | undefined;
            let failure: unknown;
            const restore = observeIntrinsicRouting((value) => {
                if (value.name === name) route = value;
            });
            try {
                compileSource(
                    `import { ${JSON.stringify(name)} as entry } from "@babylonjs/lite"; entry();`,
                );
            } catch (error) {
                failure = error;
            } finally {
            ...
            return {
                ...
                owner,
                status:
                    route?.outcome === "missing"
                        ? "no-route-observed"
                        : route
                          ? "route-found"
                          : "probe-error",
                ...(route?.lowerer ? { lowerer: route.lowerer } : {}),
