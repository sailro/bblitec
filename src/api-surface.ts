...
function declarationName(node: ts.Node): ts.DeclarationName | undefined {
    if (ts.isFunctionDeclaration(node) || ts.isInterfaceDeclaration(node) || ts.isTypeAliasDeclaration(node) ||
        ts.isClassDeclaration(node) || ts.isEnumDeclaration(node) || ts.isEnumMember(node) ||
        ts.isVariableDeclaration(node) || ts.isModuleDeclaration(node) || ts.isMethodSignature(node) ||
        ts.isMethodDeclaration(node) || ts.isPropertySignature(node) || ts.isPropertyDeclaration(node) ||
        ts.isGetAccessorDeclaration(node) || ts.isSetAccessorDeclaration(node) || ts.isParameter(node)) return node.name;
    return undefined;
}
...
function kindOf(node: ts.Node): string | undefined {
    ...
    if (ts.isConstructSignatureDeclaration(node) || ts.isConstructorTypeNode(node)) return "construct";
    if (ts.isCallSignatureDeclaration(node) || ts.isFunctionTypeNode(node)) return "call";
    if (ts.isIndexSignatureDeclaration(node)) return "index";
    return undefined;
}

const signatures = new Set(["function", "method", "constructor", "construct", "call", "index"]);
...
export function extractApi(program: ts.Program, entry: ts.SourceFile, pin: UpstreamPin): ApiSurface {
    const checker = program.getTypeChecker();
    const module = checker.getSymbolAtLocation(entry);
    if (!module) throw new Error(`API declaration entry is not a module: ${entry.fileName}`);
    const unalias = (symbol: ts.Symbol): ts.Symbol => symbol.flags & ts.SymbolFlags.Alias
    ...
    for (const statement of entry.statements) {
        ...
        for (const node of declarations) {
            ...
            const mark = (child: ts.Node): void => { rootOf.set(child, name); ts.forEachChild(child, mark); };
            mark(node);
        }
    }
    ...
    for (const symbol of checker.getExportsOfModule(module)) {
        ...
        const owner = target.declarations?.map(node => rootOf.get(node)).find(name => name !== undefined);
        if (!owner) throw new Error(`Public export ${symbol.name} is outside the rolled declaration entry.`);
    ...
    const references = (node: ts.Node): string[] => {
        ...
        const visit = (child: ts.Node): void => {
            ...
            if (ts.isTypeReferenceNode(child) || ts.isExpressionWithTypeArguments(child) || ts.isTypeQueryNode(child)) {
                const name = ts.isTypeReferenceNode(child) ? child.typeName :
                    ts.isTypeQueryNode(child) ? child.exprName : child.expression;
                const symbol = checker.getSymbolAtLocation(name);
                if (symbol) for (const declaration of unalias(symbol).declarations ?? []) {
    ...
    const edges = new Map([...roots].map(([name, nodes]) => [name, [...new Set(nodes.flatMap(references))]]));
    ...
    const rootHashes = new Map([...roots].map(([name, nodes]) =>
        [name, apiHash(nodes.map(declarationText).sort().join("\n"))]));
    const fingerprint = (declaration: string, dependencies: readonly string[]): string =>
        apiHash(JSON.stringify([declaration, dependencies.map(name => [name, rootHashes.get(name)])]));
    ...
    for (const owner of closure(Object.values(exports))) {
        const visit = (node: ts.Node, path: string, top = false): void => {
            ...
            if (kind) {
                const name = memberName(node);
                const staticPrefix = ts.canHaveModifiers(node) && ts.getModifiers(node)?.some(m =>
                    m.kind === ts.SyntaxKind.StaticKeyword) ? "static." : "";
                const label = !top && name ? `${path}.${staticPrefix}${name}` : path;
                ...
                if (previous && previous.declaration !== declaration) {
                    ...
                    previous.dependencies = [...new Set([...previous.dependencies, ...dependencies])].sort();
                    previous.fingerprint = fingerprint(previous.declaration, previous.dependencies);
                } else if (!previous) items.set(id, { id, owner, kind, declaration, fingerprint: fingerprint(declaration, dependencies), dependencies });
                const key = declarationKey(node);
                byDeclaration.set(key, [...(byDeclaration.get(key) ?? []), id]);
                nested = id;
            } else if (ts.isParameter(node)) nested = `${path}.parameter.${memberName(node)}`;
            else if (ts.isUnionTypeNode(node) || ts.isIntersectionTypeNode(node)) {
                for (const type of node.types) visit(type, `${path}.variant.${apiHash(declarationText(type)).slice(0, 16)}`);
                return;
            }
...
export function loadApiSurface(): ApiSurface {
    ...
    const program = ts.createProgram([path], { strict: true, noEmit: true, skipLibCheck: true, types: [] });
...
export function readApiSnapshot(path: string): ApiSnapshot {
    ...
    if (!isRecord(value) || value.schemaVersion !== 1 || !Array.isArray(value.items) ||
        !isRecord(value.pin) || typeof value.pin.package !== "string" || typeof value.pin.version !== "string" ||
        typeof value.pin.sourceVersion !== "string" || !isRecord(value.exports) ||
        typeof value.typescript !== "string" || typeof value.declarationsSha256 !== "string") {
        throw new Error(`Invalid API snapshot: ${path}`);
    }
    const items = value.items.map((item: unknown): ApiItem => {
        if (!isRecord(item) || typeof item.id !== "string" || typeof item.owner !== "string" || typeof item.kind !== "string" ||
            typeof item.fingerprint !== "string" || typeof item.declaration !== "string" ||
            !Array.isArray(item.dependencies) || !item.dependencies.every((name: unknown) => typeof name === "string")) {
            throw new Error(`Invalid API item in ${path}`);
        }
        return { id: item.id, owner: item.owner, kind: item.kind, declaration: item.declaration,
            fingerprint: item.fingerprint, dependencies: item.dependencies };
    });
    if (new Set(items.map(item => item.id)).size !== items.length) throw new Error(`Duplicate API items in ${path}`);
    const exports: Record<string, string> = {};
    for (const [name, owner] of Object.entries(value.exports)) {
        if (typeof owner !== "string") throw new Error(`Invalid API export in ${path}`);
        exports[name] = owner;
    }
    return { schemaVersion: 1, pin: { package: value.pin.package, version: value.pin.version, sourceVersion: value.pin.sourceVersion },
        typescript: value.typescript, declarationsSha256: value.declarationsSha256, exports, items };
}
...
export function diffApi(before: ApiSnapshot, after: ApiSnapshot): {
    added: string[]; removed: string[]; changed: string[]; exportsChanged: boolean;
} {
    const old = new Map(before.items.map(item => [item.id, item]));
    const current = new Map(after.items.map(item => [item.id, item]));
    return {
        added: after.items.filter(item => !old.has(item.id)).map(item => item.id),
        removed: before.items.filter(item => !current.has(item.id)).map(item => item.id),
        changed: after.items.filter(item => old.has(item.id) && old.get(item.id)?.fingerprint !== item.fingerprint).map(item => item.id),
        exportsChanged: JSON.stringify(before.exports) !== JSON.stringify(after.exports),
    };
}
