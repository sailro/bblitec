...
const phases = [
    { name: "animating", frame: 5, replay: [] },
    { name: "frozen", frame: 40, replay: [] },
...
for (const backend of ["sdl_gpu", "dawn"]) {
    ...
    assert.deepEqual(early.meshes, frozen.meshes, "Texture animation changed geometry");
    assert.notDeepEqual(early.pinnedMaterialBlocks, frozen.pinnedMaterialBlocks, "Animation did not update material uniforms");
    const animationMad = mad(early.image, frozen.image);
    assert(animationMad > 0.05, `Texture animation did not change rendered materials: ${animationMad}`);
