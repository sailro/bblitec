...
if (!entry || !output || process.argv.length !== 4) {
    throw new Error(
        "Usage: node tools/project-requirements.mjs <entry.ts> <report.json>",
    );
}
...
function add(id, node, detail) {
    if (!requirements.has(id))
        requirements.set(id, { id, assessment: "unassessed", sites: [] });
    requirements
        .get(id)
        .sites.push({ ...location(node), ...(detail ? { detail } : {}) });
}
function libraryMember(symbol) {
    ...
    const name =
        ts.isInterfaceDeclaration(owner) || ts.isClassDeclaration(owner)
            ? `${owner.name?.text ?? "anonymous"}.${symbol.name}`
            : symbol.name;
    ...
    function visit(node) {
        ...
        if (ts.isCallExpression(node) || ts.isNewExpression(node)) {
            ...
            if (
                ...
                libraryMember(checker.getSymbolAtLocation(node.expression)) ===
                    "standard:Worker"
            ) {
                details.workers.push({
                    ...location(node),
                    argument: node.arguments?.[0]?.getText(file),
                });
            }
            if (
                ...
                if (checker.isTupleType(receiver))
                    add(`tuple:method:${node.expression.name.text}`, node);
            }
        }
        if (
            ...
            if (!symbol && ts.isPropertyAccessExpression(node)) {
                add("unresolved:property", node, {
                    member: node.name.text,
                    receiverType: checker.typeToString(receiver),
                    operation:
            ...
            if (id) {
                const call =
                    ...
                    node.parent.expression === node
                        ...
                        : undefined;
                const detail = call
                    ? { arguments: call.arguments.map(argumentShape) }
                    : { operation: "access" };
                if (
                    call &&
                    ts.isPropertyAccessExpression(node) &&
            ...
            if (id)
                add(id, node, {
                    arguments: (node.parent.arguments ?? []).map(argumentShape),
                });
        }
        if (ts.isBindingElement(node) && node.dotDotDotToken)
            add("syntax:rest-binding", node);
        if (ts.isParameter(node) && node.dotDotDotToken)
            add("syntax:rest-parameter", node);
        ...
        if (ts.isClassStaticBlockDeclaration(node))
            add("syntax:class-static-block", node);
        if (
        ...
        if (ts.isAwaitExpression(node)) {
            ...
            for (
                ...
                if (ts.isCatchClause(parent))
                    add("syntax:await-in-catch", node);
                if (
                    ts.isBlock(parent) &&
                    ts.isTryStatement(parent.parent) &&
                    parent.parent.finallyBlock === parent
                )
                    add("syntax:await-in-finally", node);
            }
        }
        if (ts.isAsExpression(node) || ts.isTypeAssertionExpression(node)) {
            if (node.type.kind === ts.SyntaxKind.AnyKeyword)
                add("syntax:any-assertion", node);
        }
...
const report = {
    ...
    totals: {
        ...
        calls: callCount,
        unresolvedCalls,
        requirementGroups: requirements.size,
    },
    files: local
        .map((file) => ({
            path: pathOf(file),
            sha256: createHash("sha256").update(file.text).digest("hex"),
            ...moduleDetails.get(file),
        }))
...
console.log(
    `Wrote ${resolve(output)}. Source inventory only; native compilation and execution require separate evidence.`,
);
