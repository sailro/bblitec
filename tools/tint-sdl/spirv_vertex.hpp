...
compact_spirv_vertex_inputs(std::span<std::uint32_t> words) {
    ...
    walk([&](std::size_t i, std::uint32_t opcode, std::uint32_t count) {
        ...
        if (opcode >= 19 && opcode <= 32 && count >= 2)
            type_opcodes.emplace(words[i + 1], opcode);
        if (opcode == 32 && count == 4)
            pointees.emplace(words[i + 1], words[i + 3]);       // OpTypePointer
        if (opcode == 71 && count == 4 && words[i + 2] == 30) { // OpDecorate Location
            locations.emplace(words[i + 1], words[i + 3]);
            location_offsets.emplace(words[i + 1], i + 3);
        }
    });
    ...
    std::uint32_t next = 0;
    for (auto& [location, compact] : mapping) {
        (void)location;
        compact = next++;
    }
    for (const auto& [id, location] : locations)
