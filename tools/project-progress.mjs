...
if (!ledgerPath || process.argv.length > 4) {
    throw new Error("Usage: node tools/project-progress.mjs <ledger.json> [report.md]");
}
...
for (const item of ledger.packages) {
    if (!item.id || packages.has(item.id)) fail(`duplicate/missing package id ${item.id}`);
    if (!["open", "complete"].includes(item.status)) fail(`${item.id}: invalid status`);
    if (!Array.isArray(item.acceptance) || !item.acceptance.length ||
        item.acceptance.some(check => typeof check.text !== "string" || !check.text.trim())) fail(`${item.id}: no acceptance criteria`);
    if (item.status === "complete") {
        if (item.blockers?.length) fail(`${item.id}: completed with open blockers`);
...
if (!ledger.baselinePackageIds?.length || !ledger.integrationGateIds?.length) fail("baseline and integration gates are required");
if (new Set(ledger.baselinePackageIds).size !== ledger.baselinePackageIds.length) fail("duplicate baseline ids");
for (const gate of ["generate", "build", "run", "validate"]) {
    if (!ledger.endToEnd?.[gate] || !ledger.integrationGateIds.includes(ledger.endToEnd[gate])) fail(`missing ${gate} integration gate`);
}
if (new Set(["generate", "build", "run", "validate"].map(gate => ledger.endToEnd[gate])).size !== 4) fail("integration stages require distinct gates");
...
const additions = new Set((ledger.scopeChanges ?? []).flatMap(change => {
    if (!change.reason || !change.date || !Array.isArray(change.addedPackageIds)) fail("scope additions require a dated reason");
    return change.addedPackageIds;
}));
for (const item of packages.values()) {
    if (!ledger.baselinePackageIds.includes(item.id) && !additions.has(item.id)) fail(`undocumented scope addition: ${item.id}`);
    for (const id of item.dependencies ?? []) {
        if (!packages.has(id)) fail(`${item.id}: missing dependency ${id}`);
        if (item.status === "complete" && packages.get(id).status !== "complete") fail(`${item.id}: dependency ${id} remains open`);
    }
}
if (ledger.inventory) {
...
function visit(id) {
    ...
    visiting.add(id);
    for (const dependency of packages.get(id).dependencies ?? []) visit(dependency);
    visiting.delete(id); visited.add(id);
}
for (const id of packages.keys()) visit(id);
const complete = ledger.packages.filter(item => item.status === "complete").length;
...
const summary = {
    ...
    complete, total, remaining: total - complete,
    integrationGates: ledger.integrationGateIds.map(id => ({ id, status: packages.get(id).status })),
};
console.log(JSON.stringify(summary, null, 2));
if (markdownPath) {
    const safe = value => String(value).replaceAll("|", "\\|").replaceAll("\n", " ");
    writeFileSync(markdownPath, `# Integration acceptance progress\n\n` +
        ...
        ledger.packages.map(item => `| ${item.id} ${safe(item.title)} | ${item.status} | ${safe(item.acceptance.map(check=>check.text).join("; "))} |`).join("\n") + "\n");
}
