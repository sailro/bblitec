...
} from "../dist/src/build-scheduling.js";
...
if (
    ...
    !workersText ||
    rest.length ||
    !/^[1-9]\d*$/.test(workersText)
) {
    throw new Error(
        "Usage: node tools/model-build-scheduling.mjs <workspace> <concurrent-scenes>",
    );
}
const workers = Number(workersText);
if (!Number.isSafeInteger(workers))
    throw new Error("Concurrent scenes must be a positive safe integer.");
...
const measurements = scenes.map((scene) => {
    ...
    if (readCacheConfiguration(directory)?.CMAKE_GENERATOR !== "Ninja") {
        throw new Error(
            `${scene.id}: the model requires a Ninja history for every selected scene.`,
        );
    }
    ...
    if (costMs === undefined)
        throw new Error(
            `${scene.id}: missing or invalid Ninja timing samples.`,
        );
    ...
    const last = new Map(
        lines.map((line, index) => [line.split("\t")[3], index]),
    );
    const older = lines.filter(
        (line, index) => last.get(line.split("\t")[3]) !== index,
    );
...
console.log(
    JSON.stringify(
        {
            kind: "historical-command-cost-model",
            workspace: root,
            workers,
            assumptions:
                "One job per scene; command durations stay fixed when launch order changes. Includes retained commands that may be clean next time; excludes configure/process overhead.",
            sceneCount: measurements.length,
            logBytes: measurements.reduce((sum, row) => sum + row.logBytes, 0),
            readHistoryMs,
            olderSampleCount: measurements.filter(
                (row) => row.olderCostMs !== undefined,
            ).length,
            registry: model(measurements),
            historical: model(
                orderByHistoricalCost(measurements, (row) => row.costMs),
            ),
            olderHistorical: model(
                orderByHistoricalCost(measurements, (row) => row.olderCostMs),
            ),
            measurements,
        },
        null,
