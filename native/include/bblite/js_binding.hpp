...
template <typename T> class LexicalBinding {
public:
    void initialize(T value) {
        if (value_)
            throw std::logic_error("A lexical binding was initialized twice.");
        value_.emplace(std::move(value));
    }
    T& get() {
        if (!value_)
            throw std::runtime_error("Cannot access a lexical binding before initialization.");
        return *value_;
    }
    const T& get() const {
        if (!value_)
