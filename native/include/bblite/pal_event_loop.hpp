...
public:
    ...
    public:
        ...
        bool post(std::unique_ptr<ExternalEvent> event) {
            if (!event)
                throw std::invalid_argument("Cannot post an empty event.");
            {
                std::lock_guard lock(mutex_);
                if (closed_ || terminated_.load(std::memory_order_relaxed))
                    return false;
        ...
        bool terminated() const noexcept { return terminated_.load(std::memory_order_relaxed); }
    ...
    explicit EventLoop(std::shared_ptr<Inbox> inbox = std::make_shared<Inbox>(),
        ...
        if (!inbox_)
            throw std::invalid_argument("An event loop requires an inbox.");
        std::lock_guard lock(inbox_->mutex_);
        if (inbox_->attached_ || inbox_->closed_)
            throw std::logic_error("Inbox already has a realm owner.");
    ...
    static EventLoop& current() {
        if (!current_)
            throw std::logic_error("No active realm event loop.");
        return *current_;
    }

    void checkpoint() const {
        if (inbox_->terminated() && !closing_)
            throw WorkerTerminated{};
    }
    bool aborting() const { return discarding_ || (inbox_->terminated() && !closing_); }

    using ContinuationId = std::uint64_t;
    ContinuationId own_continuation(std::coroutine_handle<> continuation,
    ...
    void on_unhandled_rejection(ErrorHandler handler) {
        require_owner();
    ...
    void report_unhandled_rejection(std::exception_ptr error) {
        ...
        if (rejection_handler_)
            rejection_handler_(error);
        else if (error_handler_)
            error_handler_(error);
        else
            std::rethrow_exception(error);
    }
    void after_microtasks(Task task) {
        require_owner();
        rejection_checks_.push_back(std::move(task));
    }
    void defer_cleanup(Task cleanup) {
        require_owner();
    ...
    std::uint64_t register_completion(EventHandler handler, Task cancel = {}) {
        ...
        if (!id)
            throw std::overflow_error("Native completion identifiers exhausted.");
    ...
    void post(Task task) {
        require_owner();
        if (!task)
            throw std::invalid_argument("Cannot queue an empty task.");
        std::lock_guard lock(inbox_->mutex_);
        if (!inbox_->closed_ && !inbox_->terminated())
            inbox_->tasks_.emplace_back(std::move(task));
    }

    void queue_microtask(Task task) {
        require_owner();
        if (!task)
            throw std::invalid_argument("Cannot queue an empty microtask.");
        checkpoint();
    ...
    TimerId set_timer(Task task, Clock::duration delay, bool repeat = false) {
        require_owner();
        if (!task)
            throw std::invalid_argument("Cannot schedule an empty timer.");
        if (closed())
            return 0;
        delay = std::max(delay, Clock::duration::zero());
        const auto nesting = timer_nesting_ + 1;
        if (timer_nesting_ > 5)
            delay = std::max(delay, Clock::duration(std::chrono::milliseconds(4)));
    ...
    TimerId set_timeout(Task task, double milliseconds, bool repeat = false) {
        ...
        double converted =
            std::isfinite(milliseconds) ? std::fmod(std::trunc(milliseconds), 4294967296.0) : 0.0;
        if (converted < 0.0)
            converted += 4294967296.0;
        if (converted >= 2147483648.0)
            converted -= 4294967296.0;
        return set_timer(
            std::move(task),
            std::chrono::milliseconds(static_cast<std::int64_t>(std::max(0.0, converted))), repeat);
    }
    ...
    AnimationFrameId request_animation_frame(AnimationCallback callback) {
        require_owner();
        if (!callback)
            throw std::invalid_argument("Cannot request an empty animation callback.");
        if (closed())
            return 0;
        const auto id = next_animation_frame_++;
        if (!id)
            throw std::overflow_error("Animation frame identifiers exhausted.");
        animation_callbacks_.emplace(id, std::move(callback));
        std::lock_guard lock(inbox_->mutex_);
        inbox_->animation_requested_ = true;
        return id;
    }
    void cancel_animation_frame(AnimationFrameId id) {
        require_owner();
...
private:
    ...
    struct ContinuationScope {
        std::shared_ptr<ContinuationContext> context;
        explicit ContinuationScope(std::shared_ptr<ContinuationContext> value)
    ...
    struct Activation {
        explicit Activation(EventLoop& loop) {
            if (current_)
                throw std::logic_error("Reentrant realm dispatch is unsupported.");
            current_ = &loop;
        }
    ...
    void turn(Task task) {
        ...
        if (draining_microtasks_)
            return;
        draining_microtasks_ = true;
        struct Reset {
            bool& value;
            ~Reset() { value = false; }
        } reset{draining_microtasks_};
    ...
    void fire_timer(TimerId id) {
        ...
        struct Reset {
            ...
            unsigned value;
            ~Reset() { target = value; }
        } reset{timer_nesting_, previous_nesting};
    ...
    bool dispatch_one(bool wait) {
        ...
        Inbox::QueuedTask task;
        {
            ...
            while (inbox_->tasks_.empty() && !inbox_->closed_ && !inbox_->terminated()) {
                if (!wait)
                    return false;
                const auto ready = [&] {
                    return !inbox_->tasks_.empty() || inbox_->closed_ || inbox_->terminated();
                };
                if (deadlines_.empty())
                    inbox_->wake_.wait(lock, ready);
                else
                    inbox_->wake_.wait_until(lock, deadlines_.top().due, ready);
                lock.unlock();
                queue_due_timers();
                lock.lock();
            }
        ...
        if (auto* callback = std::get_if<Task>(&task)) {
            ...
            turn([&] {
                ...
                if (!event_handler_)
                    throw std::logic_error("No receiver for external event.");
...
void dispatch_platform_event(EventLoop& loop, Listeners& listeners, Event& event) {
    listeners.dispatch_with(
        [&](auto& listener, auto& value) {
            loop.dispatch_callback([callback = listener, &value] { callback(value); });
        },
        event);
}
