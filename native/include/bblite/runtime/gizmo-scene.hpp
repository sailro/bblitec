...
inline bool pointer_drag_state(const std::shared_ptr<PointerDragDispatcher>& state,
                               unsigned int query) {
    if (!state)
        return false;
    if (query == 1u)
        return state->pick_pending;
    return state->active.value != invalid_handle ||
           (query == 2u && state->hovered.value != invalid_handle);
}

std::shared_ptr<PointerDragDispatcher> create_pointer_drag_dispatcher(Engine&, UtilityLayerHandle,
                                                                      bool host_canvas);
js::Callback<void()> register_pointer_drag(const std::shared_ptr<PointerDragDispatcher>&,
                                           PointerDragHandle);
js::Callback<void(js::BorrowedEvent)>
pointer_drag_listener(const std::shared_ptr<PointerDragDispatcher>&, unsigned int event);
void set_pointer_drag_cleanup(const std::shared_ptr<PointerDragDispatcher>&, js::Callback<void()>);
