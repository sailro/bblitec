...
struct RmlUiSelectorTree {
    ...
    std::size_t child_count(Node node) const {
        return static_cast<std::size_t>(node->GetNumChildren());
    }
    Node child(Node node, std::size_t index) const {
        return node->GetChild(static_cast<int>(index));
    }
    ...
    bool empty(Node node) const {
        for (int index = 0; index < node->GetNumChildren(); ++index) {
            ...
            if (candidate->GetPseudoElement() != Rml::Element::PseudoElement::None)
                continue;
    ...
    bool state(Node node, UiSelectorTestKind kind) const {
        switch (kind) {
        case UiSelectorTestKind::Hover:
            return node->IsPseudoClassSet("hover");
        case UiSelectorTestKind::Active:
            return node->IsPseudoClassSet("active");
        case UiSelectorTestKind::Focus:
            return node->IsPseudoClassSet("focus");
        case UiSelectorTestKind::FocusVisible:
            return node->IsPseudoClassSet("focus-visible");
        case UiSelectorTestKind::FocusWithin:
            return node->IsPseudoClassSet("focus-within");
        case UiSelectorTestKind::Disabled:
            return node->IsPseudoClassSet("disabled");
        case UiSelectorTestKind::Checked:
            return node->IsPseudoClassSet("checked");
        default:
            throw std::logic_error("Invalid compiled UI selector state.");
        }
    }
};

/** Match compiled terms on RmlUi's live tree without parsing during layout. */
inline bool ui_selector_sequence_matches(Rml::Element* element,
                                         const std::vector<UiSelectorStep>& steps) {
