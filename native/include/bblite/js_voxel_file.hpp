...
[[nodiscard]] inline bool save_voxel_world(Engine& engine, Ref<SaveData> data) {
    ...
    std::string text =
        concat("{\"v\":1,\"seed\":", number(data->seed), ",\"time\":", number(data->time),
               ",\"player\":{\"x\":", number(data->player->x), ",\"y\":", number(data->player->y),
               ",\"z\":", number(data->player->z), ",\"yaw\":", number(data->player->yaw),
               ",\"pitch\":", number(data->player->pitch), "},\"edits\":[");
...
public:
    explicit VoxelSaveJsonReader(std::string text) : text_(std::move(text)) {}
...
private:
    void skip_whitespace() {
        while (position_ < text_.size() && is_ascii_whitespace(text_[position_])) {
            ++position_;
        }
    }
...
template <typename SaveData> [[nodiscard]] inline SaveData load_voxel_world(Engine& engine) {
    ...
    if (!reader.consume("{\"v\":") || !reader.number(version) || version != 1.0 ||
        ...
        !reader.consume(",\"pitch\":") || !reader.number(data->player->pitch) ||
