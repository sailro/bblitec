...
inline void validate_temporal_source(const Engine& engine, const FrameTaskRecord& task,
                                     ...
                                     const upstream::RenderDrawLists& draws) {
    if (!task.source_scene || task.render.scene_stages ||
        task.render.shadow_generator.value != invalid_handle) {
        throw std::runtime_error(
            "Temporal source requires an explicit color pass in its owning scene.");
    }
    if (!camera || camera->kind != CameraKind::arc_rotate || camera->orthographic) {
        throw std::runtime_error(
            "Temporal source camera requires tracked ArcRotate perspective transport.");
    }
    if (task.source_scene->clustered_lights.value != invalid_handle ||
        task.source_scene->transmission_enabled) {
        throw std::runtime_error(
    ...
    for (const auto* list : {&draws.opaque, &draws.transparent}) {
        for (const auto& draw : list->commands) {
            if (draw.item.material_kind != upstream::RenderMaterialKind::standard) {
                throw std::runtime_error(
                    "Temporal source requires a prepared Standard material draw adapter.");
            }
        }
    }
}
...
void prepare_temporal_scene_uniforms(FrameTaskRecord& task, CameraRecord* camera,
                                     double target_width, double target_height, double canvas_width,
                                     double canvas_height, Upload&& upload) {
    if (!task.source_scene)
        throw std::runtime_error("Temporal source has no retained scene.");
    if (!task.scene_uniforms)
        task.scene_uniforms = upstream::create_persistent_scene_uniforms();
    ...
    if (storage.clean.size() * sizeof(float) != sizeof(upstream::SceneUniforms) ||
        ...
        throw std::runtime_error(
            "Temporal source scene UBO does not match its pinned block layout.");
    }
    ...
    upstream::write_pass_scene_ubo(
        source, dimensions, inputs, camera,
        [](CameraRecord* value) { return upstream::scene_camera_change_key(*value); },
        [&](SourceInput&, double aspect) {
            struct PackEngine {
                bool use_floating_origin;
                double width, height;
            } pack_engine{BBLITE_FLOATING_ORIGIN != 0, canvas_width, canvas_height};
            struct Fog {
                double mode, start, end, density;
            ...
            } pack_scene{scene.environment.exposure, scene.environment.contrast,
                         scene.environment.tone_mapping_enabled};
            if (scene.state->fog_identity)
                pack_scene.fog = Fog{scene.fog_mode,
                                     scene.fog_start,
                                     scene.fog_end,
                                     scene.fog_density,
            ...
            pack_scene.clip_plane = {scene.clip_plane.x, scene.clip_plane.y, scene.clip_plane.z,
                                     scene.clip_plane.w};
            if (scene.state->environment_identity) {
                pack_scene.environment = Environment{scene.environment.lod_generation_scale,
                                                     scene.environment.has_irradiance};
                pack_scene.environment_rotation = scene.environment.rotation_y;
                for (std::size_t band = 0; band < scene.environment.spherical_harmonics.size();
                     ++band) {
            ...
            upstream::pack_scene_uniforms(
                [](CameraRecord& value, double ratio) {
                    return upstream::build_view_projection(value, ratio);
                },
                [](CameraRecord& value) {
                    return upstream::build_view_matrix(upstream::camera_world_matrix(value));
                },
            ...
            if (pack_scene.environment)
                upstream::write_environment_scene_uniforms(storage.clean, pack_scene);
...
inline upstream::SceneUniforms temporal_clean_scene_block(const PersistentSceneUniforms& source) {
    ...
    if (source.clean.size() * sizeof(float) != sizeof(block)) {
        throw std::runtime_error(
            "Temporal clean scene UBO does not match its pinned block layout.");
    }
...
void advance_temporal_jitter(TaaPostProcessState& state, PersistentSceneUniforms& source,
    ...
    upstream::advance_taa_jitter(
        ...
        [&](PersistentSceneUniforms& target, double offset, const auto& values) {
            ...
            std::memcpy(reinterpret_cast<std::uint8_t*>(target.drawn.data()) + byte_offset,
                        values.data(), bytes);
        });
}
