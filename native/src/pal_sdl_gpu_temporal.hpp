...
prepare_sdl_gpu_uniforms(const std::vector<std::string>& names, Resolve&& resolve,
    ...
    for (std::size_t slot = 0; slot < names.size(); ++slot) {
        ...
        if (!block.data)
            throw std::runtime_error("Temporal draw declares an unmapped uniform block '" +
                                     names[slot] + "'.");
        PreparedSdlUniform uniform;
        if (names[slot] == "scene") {
            if (!source || source->drawn.size() * sizeof(float) != block.bytes) {
                throw std::runtime_error(
                    "Temporal draw scene block does not match its retained source UBO.");
            }
...
inline void encode_sdl_gpu_prepared_draw(SDL_GPUCommandBuffer* command, SDL_GPURenderPass* pass,
    ...
    if (!draw.vertex_textures.empty())
        SDL_BindGPUVertexSamplers(pass, 0, draw.vertex_textures.data(),
                                  static_cast<Uint32>(draw.vertex_textures.size()));
    if (!draw.fragment_textures.empty())
        SDL_BindGPUFragmentSamplers(pass, 0, draw.fragment_textures.data(),
                                    static_cast<Uint32>(draw.fragment_textures.size()));
    if (!draw.vertex_storage.empty())
        SDL_BindGPUVertexStorageBuffers(pass, 0, draw.vertex_storage.data(),
                                        static_cast<Uint32>(draw.vertex_storage.size()));
    if (!draw.fragment_storage.empty())
        SDL_BindGPUFragmentStorageBuffers(pass, 0, draw.fragment_storage.data(),
                                          static_cast<Uint32>(draw.fragment_storage.size()));
    SDL_BindGPUVertexBuffers(pass, 0, draw.vertex_buffers.data(),
                             static_cast<Uint32>(draw.vertex_buffers.size()));
...
inline void encode_sdl_gpu_prepared_scene(SDL_GPUCommandBuffer* command,
    ...
    if (scene.viewport)
        SDL_SetGPUViewport(pass, &*scene.viewport);
    if (scene.scissor)
