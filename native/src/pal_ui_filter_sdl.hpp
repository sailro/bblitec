...
struct UiFilterSdlResources {
    struct Texture {
        ...
        SDL_GPUTexture* ensure(SDL_GPUDevice* device, std::uint32_t w, std::uint32_t h,
                               SDL_GPUTextureFormat f) {
    ...
    void finish_frame(SDL_GPUDevice* device, std::size_t count) {
        for (auto& layer : layers)
            if (!layer.used)
                layer.texture.release(device);
        for (std::size_t i = count; i < workspaces.size(); ++i)
            workspaces[i].release([&](Texture& texture) { texture.release(device); });
    ...
    void release(SDL_GPUDevice* device) {
        ...
        for (auto& workspace : workspaces)
            workspace.release([&](Texture& texture) { texture.release(device); });
        if (pipeline)
            SDL_ReleaseGPUGraphicsPipeline(device, pipeline);
        *this = {};
    }
    SDL_GPUTexture* target(SDL_GPUDevice* device, SDL_GPUCommandBuffer* command,
        ...
        if (!layer.used) {
            ...
            if (!pass)
                gpu_error("SDL_BeginGPURenderPass UI filter clear");
            pass.end();
            layer.used = true;
        }
        return result;
    ...
    void ensure_pipeline(SDL_GPUDevice* device) {
        if (pipeline)
            return;
        auto vertex =
            load_shader(device, "ui-filter.vert", SDL_GPU_SHADERSTAGE_VERTEX, 0, 0, "mainVertex");
        auto fragment = load_shader(device, "ui-filter.frag", SDL_GPU_SHADERSTAGE_FRAGMENT, 2, 1,
                                    "mainFragment");
...
inline void render_ui_composite_sdl(SDL_GPUDevice* device, SDL_GPUCommandBuffer* command,
    ...
    auto* destination =
        resources.target(device, command, root, format, frame, composite.destination);
    if (resources.workspaces.size() <= index)
        resources.workspaces.resize(index + 1);
    auto& textures = resources.workspaces[index];
    textures.begin();
    auto* snapshot = textures.get(UiFilterSurface::Snapshot)
                         .ensure(device, composite.width, composite.height, format);
    SdlCopyPass copy{SDL_BeginGPUCopyPass(command)};
    const SDL_GPUTextureLocation from{
        source, 0, 0, static_cast<Uint32>(composite.left), static_cast<Uint32>(composite.top), 0};
    ...
    for (const auto& draw : plan.draws) {
        ...
        SdlRenderPass pass{SDL_BeginGPURenderPass(command, &attachment, 1, nullptr)};
        if (!pass)
            gpu_error("SDL_BeginGPURenderPass UI filter");
    ...
    SdlRenderPass pass{SDL_BeginGPURenderPass(command, &attachment, 1, nullptr)};
    if (!pass)
        gpu_error("SDL_BeginGPURenderPass UI filter composite");
