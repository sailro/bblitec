...
public:
    ...
    ~DawnWindowPresenter() override {
        try {
            for (auto& flight : in_flight_)
                wait_for(state_.instance, flight.future);
    ...
    bool present(std::span<const WindowCanvasFrame> frames, const UiRenderFrame& ui,
        ...
        int w = 0, h = 0;
        if (!SDL_GetWindowSizeInPixels(state_.window, &w, &h))
            dawn_error(SDL_GetError());
        if (w <= 0 || h <= 0)
            return false;
        ...
        if (!acquire_dawn_surface_texture(state_, target))
            return false;
        ...
        const auto external_texture = [&](std::uint64_t id) -> const SpriteUiDawnTexture* {
            ...
            if (!binding)
                binding = std::make_unique<SamplingBinding>(state_, ui_, found->frame.image);
            return &binding->texture;
        };
        render_sprite_ui_dawn_frame(state_, encoder, target.texture, view, ui_, ui,
                                    external_texture);
        DawnSurfaceCapture snapshot;
        if (!capture.empty())
            snapshot =
                begin_dawn_surface_capture(state_.device, encoder, target.texture, width, height);
        ...
        if (!capture.empty()) {
            ...
            callback.callback = [](WGPUQueueWorkDoneStatus status, WGPUStringView message,
                ...
                if (status != WGPUQueueWorkDoneStatus_Success)
                    static_cast<SharedDawnErrors*>(errors)->report("Window submit: " +
                                                                   view_text(message));
            };
            callback.userdata1 = &errors_;
            in_flight_.push_back({wgpuQueueOnSubmittedWorkDone(state_.queue, callback),
    ...
    void retire_frames() {
        while (!in_flight_.empty()) {
            ...
            if (status != WGPUWaitStatus_Success)
                dawn_error("Window fence poll failed.");
            in_flight_.pop_front();
        }
    }
    struct SamplingBinding {
        SamplingBinding(DawnDevice& state, SpriteUiDawnResources& ui,
                        const std::shared_ptr<OffscreenImage>& source)
            : image(source) {
            const auto* native = dynamic_cast<DawnOffscreenImage*>(source.get());
            if (!native)
                dawn_error("Window received an incompatible canvas GPU image.");
            try {
                texture.view = create_dawn_texture_view(native->texture, nullptr);
                texture.group =
                    create_sprite_ui_dawn_texture_group(state, ui, texture.view, ui.sampler);
                texture.nearest_group = create_sprite_ui_dawn_texture_group(state, ui, texture.view,
                                                                            ui.nearest_sampler);
            } catch (...) {
                texture.release();
                throw;
...
std::shared_ptr<WindowPresenter> create_window_dawn_presenter(SDL_Window* window) {
    return std::make_shared<DawnWindowPresenter>(window);
}
} // namespace bbl::pal
