...
std::optional<SystemFontFace> find_platform_font(std::string_view family, int weight) {
    ...
    BOOL exists = false;
    if (FAILED(collection->FindFamilyName(wide_family->c_str(), &family_index, &exists)) ||
        !exists) {
        return std::nullopt;
    }
    ...
    if (FAILED(owned_files.front()->GetReferenceKey(&reference_key, &reference_key_size))) {
        return std::nullopt;
    }
    ...
    if (FAILED(local_loader->GetFilePathLengthFromKey(reference_key, reference_key_size,
                                                      &path_length))) {
        return std::nullopt;
    }
    std::wstring path(path_length + 1, L'\0');
    if (FAILED(local_loader->GetFilePathFromKey(reference_key, reference_key_size, path.data(),
                                                static_cast<std::uint32_t>(path.size())))) {
        return std::nullopt;
    }
    path.resize(path_length);
    return SystemFontFace{std::filesystem::path(std::move(path)), std::string(family),
...
std::optional<SystemFontFace> android_font_face(const AFont* font, int weight) {
    ...
    if (FT_New_Face(library, path, collection_index, &raw_face))
        return std::nullopt;
    ...
    if (!FT_Get_MM_Var(face.get(), &variations)) {
        ...
        for (FT_UInt axis = 0; axis < variations->num_axis; ++axis) {
            ...
            for (FT_UInt style = 0; style < variations->num_namedstyles; ++style) {
                const int candidate =
                    static_cast<int>(variations->namedstyle[style].coords[axis] / 65536);
...
std::optional<SystemFontFace> find_platform_font(std::string_view family, int weight) {
    ...
    static const auto fonts = [] {
        ...
        while (AFont* next = ASystemFontIterator_next(iterator.get())) {
            ...
            FontOwner<FT_Face, &FT_Done_Face> face(raw_face, FT_Done_Face);
            if (face->family_name)
                result.push_back({std::move(font), face->family_name});
        }
        return result;
    }();
...
std::string cf_string(CFStringRef value) {
    ...
    if (!CFStringGetCString(value, result.data(), static_cast<CFIndex>(result.size()),
...
std::optional<SystemFontFace> find_platform_font(std::string_view family, int weight) {
    ...
    const float normalized_weight =
        std::clamp((static_cast<float>(weight) - 400.0f) / 500.0f, -1.0f, 1.0f);
    CfRef<CFNumberRef> weight_value(
    ...
    if (!attributes.get())
        return std::nullopt;
    CfRef<CTFontDescriptorRef> descriptor(CTFontDescriptorCreateWithAttributes(attributes.get()));
    if (!descriptor.get())
        return std::nullopt;
    CfRef<CTFontRef> font(CTFontCreateWithFontDescriptor(descriptor.get(), 16.0, nullptr));
    if (!font.get())
        return std::nullopt;
    ...
    if (!resolved.get())
        return std::nullopt;
    CfRef<CFURLRef> url(
        static_cast<CFURLRef>(CTFontDescriptorCopyAttribute(resolved.get(), kCTFontURLAttribute)));
    if (!url.get())
        return std::nullopt;
    CfRef<CFStringRef> path(CFURLCopyFileSystemPath(url.get(), kCFURLPOSIXPathStyle));
    CfRef<CFStringRef> resolved_family(CTFontCopyFamilyName(font.get()));
    if (!path.get() || !resolved_family.get())
        return std::nullopt;
    ...
    if (FT_Init_FreeType(&raw_library))
        return std::nullopt;
    const std::unique_ptr<std::remove_pointer_t<FT_Library>, decltype(&FT_Done_FreeType)> library(
        raw_library, FT_Done_FreeType);
    FT_Long face_count = 1;
    for (FT_Long face_index = 0; face_index < face_count; ++face_index) {
        FT_Face raw_face = nullptr;
        if (FT_New_Face(library.get(), filename.c_str(), face_index, &raw_face))
            return std::nullopt;
        const std::unique_ptr<std::remove_pointer_t<FT_Face>, decltype(&FT_Done_Face)> face(
            raw_face, FT_Done_Face);
        face_count =
            std::min(face->num_faces, static_cast<FT_Long>(std::numeric_limits<int>::max()));
        const char* name = FT_Get_Postscript_Name(face.get());
        if (name && postscript == name) {
            return SystemFontFace{std::filesystem::path(filename), cf_string(resolved_family.get()),
...
std::optional<SystemFontFace> find_platform_font(std::string_view family, int weight) {
    ...
    int face_index = 0;
    const bool valid = FcPatternGetString(match, FC_FILE, 0, &file) == FcResultMatch &&
                       FcPatternGetString(match, FC_FAMILY, 0, &resolved_family) == FcResultMatch;
    FcPatternGetInteger(match, FC_INDEX, 0, &face_index);
    std::optional<SystemFontFace> font;
    if (valid) {
        font = SystemFontFace{reinterpret_cast<const char*>(file),
                              reinterpret_cast<const char*>(resolved_family), face_index};
    }
