...
inline std::string_view audio_container_extension(std::span<const std::uint8_t> bytes) {
    ...
    if (header.starts_with("RIFF") && header.size() >= 12 && header.substr(8, 4) == "WAVE")
        return "wav";
    if (header.starts_with("wvpk"))
        return "wv";
    if (header.starts_with("MPCK"))
        return "mpc";
    if (header.starts_with("fLaC"))
        return "flac";
    ...
    if (header.starts_with("OggS")) {
        ...
        if (page.find("OpusHead") != std::string_view::npos)
            return "opus";
        if (page.find("vorbis") != std::string_view::npos)
            return "ogg";
    }
    return {};
}
...
inline std::unique_ptr<lab::AudioBus> decode_audio_bus(const std::vector<std::uint8_t>& bytes,
...
#endif
    if (!decoded || pcm.channelCount <= 0 || pcm.sampleRate <= 0 || pcm.samples.empty())
        return {};
    const auto channels = static_cast<std::size_t>(pcm.channelCount);
    const auto frames = pcm.samples.size() / channels;
    if (frames == 0 || frames > static_cast<std::size_t>(std::numeric_limits<int>::max()))
        return {};
