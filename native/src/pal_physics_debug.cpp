...
PhysicsDebugExtractionScope::PhysicsDebugExtractionScope() {
    if (extracting_constructor_inputs)
        throw std::runtime_error("Nested constructor extraction is not supported.");
...
bool collect_physics_debug_descriptor(const PhysicsDebugShapeDescriptor& descriptor) {
    if (!extracting_constructor_inputs)
        return false;
    if (std::find(collected_descriptors.begin(), collected_descriptors.end(), descriptor) ==
        collected_descriptors.end()) {
