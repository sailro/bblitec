...
void sync_clustered_payloads(ClusteredLightContainer& container, std::uint64_t& uploaded_version,
                             ...
                             double far_plane, Params&& params, Texture&& texture) {
    upstream::refresh_clustered_lights(container, view, projection, near_plane, far_plane);
    if (uploaded_version == container.upload_version)
        return;
    ...
    const auto upload = [&](ClusteredTexture slot, const auto& bytes, std::uint32_t texel_bytes,
        ...
        texture(slot, bytes.data(), bytes.size() * sizeof(bytes[0]), texel_bytes, region.width,
                region.height);
    };
    upload(ClusteredTexture::lights, container.light_data, 16u, container.light_texels,
           container.light_rows);
    upload(ClusteredTexture::cells, container.slice_data, 16u, container.slice_count,
           container.slice_rows);
    upload(ClusteredTexture::indices, container.mask_data, 4u, container.mask_texels,
           container.mask_rows);
