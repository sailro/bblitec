...
void release_target_resources(SDL_GPUDevice* device, TargetResources& target) noexcept {
    if (target.sampled && target.sampled != target.color)
        SDL_ReleaseGPUTexture(device, target.sampled);
    if (target.color)
...
void build_graph(State& state, const Engine& engine, std::uint32_t width, std::uint32_t height) {
    ...
    const auto target_plans =
        plan_render_targets(engine, width, height, state.gpu.swapchain_format,
    ...
    for (std::size_t index = 0; index < engine.render_targets.size(); ++index) {
        ...
        if (record.swapchain)
            continue;
        if (!record.has_color) {
            throw std::runtime_error("Standalone frame graphs currently require color targets.");
        }
        const SDL_GPUSampleCount samples = target_samples(state, record.samples);
        target.color = create_texture(
            state, target.format, samples, target.width, target.height,
            SDL_GPU_TEXTUREUSAGE_COLOR_TARGET |
                (samples == SDL_GPU_SAMPLECOUNT_1 ? SDL_GPU_TEXTUREUSAGE_SAMPLER : 0));
        target.sampled =
            samples == SDL_GPU_SAMPLECOUNT_1
                ? target.color
                : create_texture(state, target.format, SDL_GPU_SAMPLECOUNT_1, target.width,
                                 target.height,
...
#if BBLITE_HAS_POST_PROCESS
SDL_GPUTexture* source_texture(State& state, const Engine& engine, const RenderTextureRef& source,
...
std::size_t post_process_program(State& state, std::uint32_t module, SDL_GPUTextureFormat format,
                                 SDL_GPUSampleCount samples, std::uint32_t alpha_mode) {
...
void record_post_process(State& state, Engine& engine, TaskHandle task_handle,
                         std::size_t pass_index, SDL_GPUCommandBuffer* command,
    ...
    if (presents && !state.present_copy) {
        state.present_copy =
            create_texture(state, state.gpu.swapchain_format, SDL_GPU_SAMPLECOUNT_1, width, height,
                           SDL_GPU_TEXTUREUSAGE_COLOR_TARGET | SDL_GPU_TEXTUREUSAGE_SAMPLER);
    }
    if (gpu.program == npos) {
        ...
        for (const std::string& name : program.fragment_slots.textures) {
            if (name == "sourceTextureSampler") {
                ...
                throw std::runtime_error(
                    "Post-process stage declares a texture the pass does not carry: " + name);
            }
        }
    }
    const PostProcessProgram& program = state.programs[gpu.program];
    ...
    target.texture = presents ? state.present_copy
                              : target_texture(state, engine, pass.output_target, swapchain, false);
    ...
    if (pass.has_viewport) {
        ...
        const SDL_GPUViewport viewport{static_cast<float>(rect.x),
    ...
    SDL_GPUSampler* sampler = pass.sampling == PostProcessSampling::nearest ? state.nearest_sampler
    ...
    for (std::size_t slot = 0; slot < gpu.texture_sources.size(); ++slot) {
        ...
        bindings[slot] = SDL_GPUTextureSamplerBinding{
            source_texture(state, engine,
                           source < 0 ? pass.source
                                      : pass.extra_textures.at(static_cast<std::size_t>(source)),
                           swapchain),
            sampler};
    }
    if (!gpu.texture_sources.empty()) {
        SDL_BindGPUFragmentSamplers(render_pass, 0, bindings.data(),
    ...
    if (presents) {
        ...
        blit.source = SDL_GPUBlitRegion{state.present_copy, 0, 0, 0, 0, width, height};
...
public:
    ...
    void setup() {
        reject_unsupported_frame_options(frame_options, "SDL_GPU frame graph", true, false);
    ...
    bool acquire() {
        command = SdlGpuCommand{SDL_AcquireGPUCommandBuffer(state.gpu.device)};
        if (!command)
            gpu_error("SDL_AcquireGPUCommandBuffer frame graph");
        swapchain = nullptr;
        width = 0;
        height = 0;
        if (!command.acquire_swapchain(state.gpu.window, &swapchain, &width, &height)) {
    ...
    void encode() {
        ...
        for (const TaskHandle handle : context->tasks) {
            ...
            if (task.kind == FrameTaskKind::effect) {
                ...
                SDL_GPUColorTargetInfo target{};
                target.texture =
                    target_texture(state, engine, task.effect.target, swapchain, false);
                target.clear_color =
    ...
    void present() {
        ...
        if (capture_frame) {
            ...
            save_texture_png(state.gpu.device, command, capture_texture, state.gpu.swapchain_format,
                             width, height, frame_options.screenshot_path);
