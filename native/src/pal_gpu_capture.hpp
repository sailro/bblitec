...
public:
    ...
    void write(std::uint64_t id, std::size_t offset, std::span<const std::uint8_t> bytes) {
        ...
        if (offset > resource.allocation_bytes ||
            bytes.size() > resource.allocation_bytes - offset) {
        ...
        if (bytes.empty())
            return;
        resource.uploaded_bytes.resize(
            std::max(resource.uploaded_bytes.size(), offset + bytes.size()));
        std::copy(bytes.begin(), bytes.end(),
                  resource.uploaded_bytes.begin() + static_cast<std::ptrdiff_t>(offset));
        ...
        for (const auto range : ranges) {
            if (!count || ranges[count - 1].offset + ranges[count - 1].bytes < range.offset) {
                ...
                previous.bytes =
                    std::max(previous.offset + previous.bytes, range.offset + range.bytes) -
                    previous.offset;
            }
        }
        ranges.resize(count);
...
private:
    GpuResourceCapture& find(std::uint64_t id) {
        if (!id || id > resources_.size())
            throw std::runtime_error("Unknown GPU capture resource.");
