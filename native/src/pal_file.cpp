...
void release_pointer_lock_for_dialog(Engine& engine) {
    if (!engine.pointer_locked && !engine.pointer_lock_requested)
        return;
...
void SDLCALL receive_dialog_result(void* userdata, const char* const* files, int) noexcept {
    auto& result = *static_cast<DialogResult*>(userdata);
    try {
        std::lock_guard lock(result.mutex);
        if (!files) {
            throw std::runtime_error("SDL file dialog failed: " + std::string(SDL_GetError()) +
                                     ".");
        }
        if (files[0])
            result.path = files[0];
...
void set_dialog_property(bool accepted, SDL_PropertiesID properties, std::string_view property) {
    if (!accepted) {
        SDL_DestroyProperties(properties);
        throw std::runtime_error("Unable to set SDL file-dialog property '" +
                                 std::string(property) + "': " + SDL_GetError() + ".");
    }
}

[[nodiscard]] std::optional<std::string> choose_sdl_file(const FileDialogOptions& options,
                                                         bool save) {
    ...
    if (properties == 0) {
        throw std::runtime_error(
            "Unable to create SDL file-dialog properties: " + std::string(SDL_GetError()) + ".");
    }
    set_dialog_property(
        SDL_SetPointerProperty(properties, SDL_PROP_FILE_DIALOG_FILTERS_POINTER, &filter),
        properties, SDL_PROP_FILE_DIALOG_FILTERS_POINTER);
    set_dialog_property(SDL_SetNumberProperty(properties, SDL_PROP_FILE_DIALOG_NFILTERS_NUMBER, 1),
                        properties, SDL_PROP_FILE_DIALOG_NFILTERS_NUMBER);
    SDL_Window* window = SDL_GetKeyboardFocus();
    if (!window)
        window = SDL_GetMouseFocus();
    if (window) {
    ...
    if (!options.title.empty()) {
        set_dialog_property(SDL_SetStringProperty(properties, SDL_PROP_FILE_DIALOG_TITLE_STRING,
                                                  options.title.c_str()),
                            properties, SDL_PROP_FILE_DIALOG_TITLE_STRING);
    }
    if (save && !options.suggested_name.empty()) {
        set_dialog_property(SDL_SetStringProperty(properties, SDL_PROP_FILE_DIALOG_LOCATION_STRING,
                                                  options.suggested_name.c_str()),
                            properties, SDL_PROP_FILE_DIALOG_LOCATION_STRING);
    }

    DialogResult result;
    SDL_ShowFileDialogWithProperties(save ? SDL_FILEDIALOG_SAVEFILE : SDL_FILEDIALOG_OPENFILE,
                                     receive_dialog_result, &result, properties);
...
std::optional<SelectedFileSnapshot> choose_open_file(Engine& engine,
    ...
    if (!override_path.empty())
        return detail::selected_file_snapshot(detail::utf8_file_path(override_path));
    release_pointer_lock_for_dialog(engine);
#if defined(SDL_PLATFORM_IOS)
    return choose_ios_open_file(options);
#else
    const auto path = choose_sdl_file(options, false);
    return path ? std::optional(detail::selected_file_snapshot(detail::utf8_file_path(*path)))
                : std::nullopt;
#endif
}
