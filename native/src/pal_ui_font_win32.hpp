...
class Win32UiFontEngine final : public Rml::FontEngineInterface {
    ...
    static void check(HRESULT result, const char* operation) {
        if (FAILED(result))
            throw std::runtime_error(std::string("DirectWrite UI font: ") + operation);
    }
    struct Table {
        ...
        Table(IDWriteFontFace* face, UINT32 tag) : face(face) {
            BOOL exists = FALSE;
            check(face->TryGetFontTable(tag, reinterpret_cast<const void**>(&bytes), &length,
                                        &context, &exists),
                  "font table lookup");
            if (!exists)
                length = 0;
    ...
    struct ScriptAnalysis final : IDWriteTextAnalysisSource, IDWriteTextAnalysisSink {
        ...
        ScriptAnalysis(const std::wstring& text, std::wstring locale,
                       DWRITE_READING_DIRECTION direction)
            : text(text), locale(std::move(locale)), direction(direction), scripts(text.size()),
              levels(text.size()) {}
        HRESULT STDMETHODCALLTYPE QueryInterface(REFIID id, void** out) override {
            *out = nullptr;
            if (id == __uuidof(IUnknown) || id == __uuidof(IDWriteTextAnalysisSource))
                *out = static_cast<IDWriteTextAnalysisSource*>(this);
            else if (id == __uuidof(IDWriteTextAnalysisSink))
                *out = static_cast<IDWriteTextAnalysisSink*>(this);
            return *out ? S_OK : E_NOINTERFACE;
        }
        ULONG STDMETHODCALLTYPE AddRef() override { return 1; }
        ULONG STDMETHODCALLTYPE Release() override { return 1; }
        HRESULT STDMETHODCALLTYPE GetTextAtPosition(UINT32 position, const WCHAR** value,
                                                    UINT32* length) noexcept override {
            *value = position < text.size() ? text.data() + position : nullptr;
            *length = position < text.size() ? static_cast<UINT32>(text.size()) - position : 0;
            return S_OK;
        }
        HRESULT STDMETHODCALLTYPE GetTextBeforePosition(UINT32 position, const WCHAR** value,
                                                        UINT32* length) noexcept override {
            *value = position && position <= text.size() ? text.data() : nullptr;
            *length = *value ? position : 0;
            return S_OK;
        }
        ...
        HRESULT STDMETHODCALLTYPE GetLocaleName(UINT32 position, UINT32* length,
                                                const WCHAR** value) noexcept override {
            *length = static_cast<UINT32>(text.size()) - position;
            *value = locale.c_str();
            return S_OK;
        }
        HRESULT STDMETHODCALLTYPE GetNumberSubstitution(
            UINT32 position, UINT32* length, IDWriteNumberSubstitution** value) noexcept override {
            *length = static_cast<UINT32>(text.size()) - position;
            *value = nullptr;
            return S_OK;
        }
        HRESULT STDMETHODCALLTYPE SetScriptAnalysis(
            UINT32 position, UINT32 length, const DWRITE_SCRIPT_ANALYSIS* value) noexcept override {
            std::fill_n(scripts.begin() + position, length, *value);
            return S_OK;
        }
        HRESULT STDMETHODCALLTYPE
        SetLineBreakpoints(UINT32, UINT32, const DWRITE_LINE_BREAKPOINT*) noexcept override {
            return S_OK;
        }
        HRESULT STDMETHODCALLTYPE SetBidiLevel(UINT32 position, UINT32 length, UINT8,
                                               UINT8 resolved) noexcept override {
            std::fill_n(levels.begin() + position, length, resolved);
            return S_OK;
        }
        HRESULT STDMETHODCALLTYPE
        SetNumberSubstitution(UINT32, UINT32, IDWriteNumberSubstitution*) noexcept override {
            return S_OK;
        }
    };
    ...
    static DWRITE_RENDERING_MODE rendering_mode(IDWriteFontFace* face, int size) {
        ...
        if (gasp.length >= 4 && gasp.u16(0) <= 1 && gasp.u16(2) <= 1024 &&
            4u + 4u * gasp.u16(2) <= gasp.length) {
            int previous = -1;
            for (unsigned i = 0; i < gasp.u16(2); ++i) {
                const int upper = gasp.u16(4 + i * 4);
                if (previous < size && size <= upper) {
                    version = gasp.u16(0);
                    flags = gasp.u16(6 + i * 4);
                    if (flags == 1) {
                        minimum = previous + 1;
                        maximum = upper;
                    }
        ...
        if (eblc.length >= 8 && eblc.u32(0) == 0x20000 && eblc.u32(4) <= 1024 &&
            8u + 48u * eblc.u32(4) <= eblc.length) {
            for (unsigned i = 0; i < eblc.u32(4); ++i) {
                const unsigned p = 8 + 48 * i;
                const int ppem = eblc.bytes[p + 44];
                if (ppem == eblc.bytes[p + 45] && minimum <= ppem && ppem <= maximum &&
                    eblc.u16(p + 42) >= eblc.u16(p + 40) + 3)
                    return DWRITE_RENDERING_MODE_GDI_CLASSIC;
            }
        }
        Table ebsc(face, DWRITE_MAKE_OPENTYPE_TAG('E', 'B', 'S', 'C'));
        if (ebsc.length >= 8 && ebsc.u32(0) == 0x20000 && ebsc.u32(4) <= 1024 &&
            8u + 28u * ebsc.u32(4) <= ebsc.length) {
            for (unsigned i = 0; i < ebsc.u32(4); ++i) {
                const unsigned p = 8 + 28 * i;
                const int ppem = ebsc.bytes[p + 24];
                if (ppem == ebsc.bytes[p + 25] && minimum <= ppem && ppem <= maximum)
                    return DWRITE_RENDERING_MODE_GDI_CLASSIC;
            }
        }
        if (version >= 1)
            return flags & 8 ? DWRITE_RENDERING_MODE_NATURAL_SYMMETRIC
                             : DWRITE_RENDERING_MODE_NATURAL;
        Table maxp(face, DWRITE_MAKE_OPENTYPE_TAG('m', 'a', 'x', 'p'));
        const bool hinted = maxp.length >= 32 && maxp.u32(0) == 0x10000 && maxp.u16(26) != 0;
        return size > 20 || !hinted ? DWRITE_RENDERING_MODE_NATURAL_SYMMETRIC
                                    : DWRITE_RENDERING_MODE_NATURAL;
    }
    ...
    static std::array<std::uint8_t, 256> make_coverage_table(unsigned green) {
        ...
        const float source =
            static_cast<float>((bucket << 5) | (bucket << 2) | (bucket >> 1)) / 255.f;
        const auto linear = [](float value) {
            return value <= .04045f ? value / 12.92f : std::pow((value + .055f) / 1.055f, 2.4f);
        };
        const auto encoded = [](float value) {
            return value <= .0031308f ? value * 12.92f
                                      : 1.055f * std::pow(value, 1.f / 2.4f) - .055f;
        };
        const float destination = 1.f - source, source_linear = linear(source),
                    destination_linear = linear(destination);
        ...
        for (unsigned i = 0; i < result.size(); ++i) {
            ...
            const float corrected =
                std::abs(source - destination) < 1.f / 256.f
                    ? alpha
                    : (encoded(source_linear * alpha + destination_linear * (1.f - alpha)) -
                       destination) /
                          (source - destination);
            result[i] =
                static_cast<std::uint8_t>(std::clamp(std::lround(corrected * 255.f), 0l, 255l));
        }
        return result;
    }
    ...
    Glyph& glyph(Face& face, UINT16 index, int quarter, unsigned green) {
        ...
        const DWRITE_GLYPH_RUN run{face.source->face.Get(),
                                   static_cast<float>(face.metrics.size),
                                   1,
                                   &index,
                                   &advance,
                                   &offset,
        ...
        Glyph value{{static_cast<float>(rect.left), static_cast<float>(rect.top)},
                    {static_cast<float>(width), static_cast<float>(height)},
                    {}};
        if (width && height) {
            std::vector<std::uint8_t> mask(static_cast<std::size_t>(width) * height * 3);
            check(analysis->CreateAlphaTexture(DWRITE_TEXTURE_CLEARTYPE_3x1, &rect, mask.data(),
            ...
            for (std::size_t p = 0; p < mask.size() / 3; ++p) {
                const auto alpha =
                    table[(static_cast<unsigned>(mask[p * 3]) + mask[p * 3 + 1] + mask[p * 3 + 2]) /
                          3];
                for (unsigned c = 0; c < 4; ++c)
                    pixels[p * 4 + c] = alpha;
            }
            value.texture = std::make_unique<Rml::CallbackTextureSource>(
                [pixels = std::move(pixels), width,
    ...
    bool indices(Face& face, Rml::StringView text, std::vector<UINT16>& output) const {
        ...
        for (auto it = Rml::StringIteratorU8(text); it; ++it)
            if (static_cast<UINT32>(*it) >= 32)
                points.push_back(static_cast<UINT32>(*it));
        output.resize(points.size());
        if (points.empty())
            return true;
        check(face.source->face->GetGlyphIndices(points.data(), static_cast<UINT32>(points.size()),
                                                 output.data()),
              "glyph mapping");
    ...
    float shape(Face& face, Rml::StringView text, const Rml::TextShapingContext& context,
        ...
        wide->erase(std::remove_if(wide->begin(), wide->end(), [](wchar_t c) { return c < 32; }),
                    wide->end());
        glyphs.clear();
        positions.clear();
        if (wide->empty())
        ...
        for (UINT32 start = 0; start < count;) {
            UINT32 end = start + 1;
            while (end < count && analysis.scripts[end].script == analysis.scripts[start].script &&
                   analysis.levels[end] == analysis.levels[start])
                ++end;
            ...
            DWRITE_FONT_FEATURE kern{DWRITE_FONT_FEATURE_TAG_KERNING,
                                     context.font_kerning == Rml::Style::FontKerning::None ? 0u
                                                                                           : 1u};
            const DWRITE_TYPOGRAPHIC_FEATURES features{&kern, 1};
            const DWRITE_TYPOGRAPHIC_FEATURES* feature_pointer = &features;
            UINT32 actual = 0;
            check(analyzer->GetGlyphs(wide->data() + start, length, face.source->face.Get(), FALSE,
                                      rtl, &analysis.scripts[start], analysis.locale.c_str(),
                                      nullptr, &feature_pointer, &length, 1, capacity,
                                      clusters.data(), text_properties.data(), mapped.data(),
                                      glyph_properties.data(), &actual),
                  "OpenType glyph mapping");
            std::vector<FLOAT> advances(actual);
            std::vector<DWRITE_GLYPH_OFFSET> offsets(actual);
            check(analyzer->GetGlyphPlacements(
                      wide->data() + start, clusters.data(), text_properties.data(), length,
                      mapped.data(), glyph_properties.data(), actual, face.source->face.Get(),
                      static_cast<float>(face.metrics.size), FALSE, rtl, &analysis.scripts[start],
                      analysis.locale.c_str(), &feature_pointer, &length, 1, advances.data(),
                      offsets.data()),
                  "OpenType glyph placement");
            for (UINT32 ordered = 0; ordered < actual; ++ordered) {
                const UINT32 i = rtl ? actual - ordered - 1 : ordered;
                glyphs.push_back(mapped[i]);
                positions.push_back({width + offsets[i].advanceOffset, -offsets[i].ascenderOffset});
...
public:
    ...
    bool LoadFontFace(const Rml::String& path, int index, bool is_fallback,
                      Rml::Style::FontWeight weight) override {
        return fallback.LoadFontFace(path, index, is_fallback, weight);
    }
    ...
    bool LoadFontFace(const Rml::String& path, int index, const Rml::String& family,
                      Rml::Style::FontStyle style, Rml::Style::FontWeight weight,
                      bool is_fallback) override {
        if (!fallback.LoadFontFace(path, index, family, style, weight, is_fallback))
            return false;
        const auto wide = utf8_to_wide(path);
        if (!wide)
            throw std::runtime_error("DirectWrite font path is not UTF-8.");
        ComPtr<IDWriteFontFile> file;
        check(factory->CreateFontFileReference(wide->c_str(), nullptr, &file),
              "font file reference");
        BOOL supported;
        ...
        if (!supported || index < 0 || static_cast<UINT32>(index) >= count)
            throw std::runtime_error("DirectWrite cannot represent the registered font face.");
        auto source = std::make_unique<SourceFace>();
        source->family = lower(family);
        source->style = style;
        source->weight = weight;
        auto* raw = file.Get();
        check(factory->CreateFontFace(face_type, 1, &raw, static_cast<UINT32>(index),
                                      DWRITE_FONT_SIMULATIONS_NONE, &source->face),
              "font face creation");
    ...
    Rml::FontFaceHandle GetFontFaceHandle(const Rml::String& family, Rml::Style::FontStyle style,
        ...
        if (!handle || faces.contains(handle))
            return handle;
        SourceFace* source = nullptr;
        for (auto& candidate : sources)
            if (candidate->family == lower(family) && candidate->style == style &&
                (!source ||
                 std::abs(static_cast<int>(candidate->weight) - static_cast<int>(weight)) <
                     std::abs(static_cast<int>(source->weight) - static_cast<int>(weight))))
        ...
        metrics.ascent = std::round(original.ascent * scale);
        metrics.descent = std::round(original.descent * scale);
    ...
    Rml::FontEffectsHandle PrepareFontEffects(Rml::FontFaceHandle handle,
        ...
        return fallback.PrepareFontEffects(handle, effects);
    }
    const Rml::FontMetrics& GetFontMetrics(Rml::FontFaceHandle handle) override {
        const auto found = faces.find(handle);
        return found == faces.end() ? fallback.GetFontMetrics(handle) : found->second.metrics;
    }
    ...
    int GenerateString(Rml::RenderManager& manager, Rml::FontFaceHandle handle,
        ...
        for (std::size_t i = 0; i < mapped.size(); ++i) {
            ...
            if (group == groups.end()) {
                ...
                meshes.emplace_back();
                meshes.back().texture = raster.texture->GetTexture(manager);
                group = groups.emplace(&raster, index).first;
            }
