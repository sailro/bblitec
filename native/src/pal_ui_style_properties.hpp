...
inline void register_ui_style_properties() {
    ...
    Rml::StyleSheetSpecification::RegisterShorthand("-webkit-appearance", "appearance", Rml::ShorthandType::Replicate);
    ...
    for (const Rml::String family : {"padding", "margin"}) {
        const auto shorthand = [&](const char* suffix, const Rml::String& properties) {
            Rml::StyleSheetSpecification::RegisterShorthand(family + suffix, properties, Rml::ShorthandType::Replicate);
        };
