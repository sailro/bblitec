...
NSString* native_string(std::string_view value) {
    ...
    if (!result)
        throw std::runtime_error("iOS file dialog requires valid UTF-8.");
    return result;
}
...
void on_main_thread(const std::function<void()>& work) {
    ...
    if (!SDL_RunOnMainThread(
            [](void* data) {
                ...
                try {
                    @try {
                        ...
                        throw std::runtime_error("iOS document picker: " +
            ...
            &invocation, true)) {
        throw std::runtime_error("Unable to dispatch iOS document picker: " +
...
UIViewController* presenter() {
    for (UIScene* scene in UIApplication.sharedApplication.connectedScenes) {
        ...
        for (UIWindow* window in ((UIWindowScene*)scene).windows) {
            ...
            if (!controller.view.window || controller.isBeingDismissed)
                continue;
            return controller;
        }
    }
...
void read_selection(const std::shared_ptr<DocumentDialog>& state, NSURL* url) {
    ...
    @try {
                              ...
                              try {
                                  ...
                                  if (!path)
                                      throw std::runtime_error(
                                          "The selected iOS file has no filesystem representation.");
                                  state->selected =
                                      detail::selected_file_snapshot(detail::utf8_file_path(path));
                                  const char* name = url.lastPathComponent.UTF8String;
                                  if (!name || !*name)
                                      throw std::runtime_error(
                                          "The selected iOS file has no display name.");
        ...
        if (error)
            state->failure =
                std::make_exception_ptr(file_error("Unable to read selected iOS document", error));
        if (!state->selected && !state->failure) {
            state->failure = std::make_exception_ptr(
                std::runtime_error("The file provider did not supply the selected document."));
        }
    } @finally {
        if (scoped)
...
@interface BbliteDocumentPickerDelegate
    ...
    try {
        @try {
            ...
            if (state->exporting)
                ...
                bbl::pal::read_selection(state, urls.firstObject);
        } @catch (NSException* error) {
...
std::shared_ptr<DocumentDialog> run_document_picker(const FileDialogOptions& options,
                                                    NSURL* export_url) {
    ...
    try {
        on_main_thread([state, options, export_url] {
            if (picker_active)
                throw std::runtime_error("An iOS document picker is already active.");
            UIViewController* host = presenter();
            if (export_url) {
                state->picker =
                    [[UIDocumentPickerViewController alloc] initForExportingURLs:@[ export_url ]
                                                                          asCopy:YES];
            } else {
                NSMutableArray<UTType*>* types = [NSMutableArray new];
                for (const auto& extension :
                     detail::file_dialog_extensions(options.filter_pattern)) {
                    UTType* type = [UTType typeWithFilenameExtension:native_string(extension)];
                    if (!type)
                        throw std::runtime_error(
                            "iOS cannot represent the requested file extension: " + extension);
            ...
            if (!state->picker)
                throw std::runtime_error("Unable to create the iOS document picker.");
            NSURL* documents = [NSFileManager.defaultManager URLsForDirectory:NSDocumentDirectory
                                                                    inDomains:NSUserDomainMask]
                                   .firstObject;
            if (!documents)
                throw std::runtime_error("iOS document storage is unavailable.");
            ...
            if (![NSFileManager.defaultManager createDirectoryAtURL:files
            ...
            if (!state->delegate)
                throw std::runtime_error("Unable to create the iOS document picker delegate.");
        ...
        try {
            ...
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "iOS document picker cleanup: %s",
                         cleanup.what());
        }
...
public:
    ExportStaging() {
        ...
        if (!temporary)
            throw std::runtime_error("iOS temporary storage is unavailable.");
        directory_ =
            detail::utf8_file_path(temporary) / ("bblite-export-" + detail::random_staging_token());
    ...
    ~ExportStaging() {
        ...
        if (error)
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "iOS export staging cleanup: %s",
                         error.message().c_str());
    }
...
bool export_ios_file(const FileDialogOptions& options, std::span<const std::uint8_t> bytes) {
    @autoreleasepool {
        ...
        if (name.empty() || name == "." || name == ".." ||
            ...
            throw std::runtime_error(
                "iOS export requires a filename without directory components.");
        }
        ExportStaging staging;
        const auto file = staging.directory() / detail::utf8_file_path(name);
        detail::write_file_atomically(file, bytes, detail::maximum_selected_file_bytes,
                                      "exported file");
