...
struct UiFilterDawnResources {
    struct Texture {
        ...
        void ensure(WGPUDevice device, WGPUBindGroupLayout layout, WGPUSampler sampler,
            ...
            if (!value.texture || width != w || height != h || format != f) {
                release();
                value = create_ui_backdrop_dawn_texture(device, layout, sampler, w, h, f);
                width = w;
                height = h;
                format = f;
            }
    ...
    struct Pass {
        ...
        void release() {
            if (uniform_group)
                wgpuBindGroupRelease(uniform_group);
            if (texture_group)
                wgpuBindGroupRelease(texture_group);
            if (buffer)
    ...
    void reset_layer(std::uint32_t id) {
        if (id > 0 && id <= layers.size())
            layers[id - 1].used = false;
    }
    void finish_frame(std::size_t count) {
        for (auto& layer : layers)
            if (!layer.used)
                layer.texture.release();
        for (std::size_t i = count; i < workspaces.size(); ++i)
            workspaces[i].release();
        if (workspaces.size() > count)
            workspaces.resize(count);
    }
    void release() {
        for (auto& layer : layers)
            layer.texture.release();
    ...
    void ensure_pipeline(WGPUDevice device) {
        ...
        stage.module = fragment;
        stage.entryPoint = string_view("mainFragment");
        stage.targetCount = 1;
        stage.targets = &target;
        descriptor.fragment = &stage;
        pipeline = require_dawn_resource(wgpuDeviceCreateRenderPipeline(device, &descriptor),
                                         "UI filter pipeline");
    ...
    UiDawnTexture target(WGPUDevice device, WGPUCommandEncoder encoder, UiDawnTexture root,
                         WGPUTextureFormat format, WGPUBindGroupLayout layout, WGPUSampler sampler,
                         const UiRenderFrame& frame, std::uint32_t id) {
        if (id == 0)
            return root;
        if (id > frame.layer_count)
            throw std::runtime_error("Invalid retained UI layer.");
        if (layers.size() < id)
            layers.resize(id);
        ...
        if (!layer.used) {
            ...
            attachment.view = layer.texture.value.view;
...
inline void render_ui_composite_dawn(WGPUDevice device, WGPUQueue queue, WGPUCommandEncoder encoder,
                                     UiDawnTexture root, WGPUTextureFormat format,
                                     WGPUBuffer vertices, WGPUBuffer indices, WGPUSampler sampler,
                                     WGPUBindGroup screen_group,
                                     WGPUBindGroupLayout ui_texture_layout,
                                     WGPURenderPipeline composite_pipeline,
                                     UiFilterDawnResources& resources, const UiRenderFrame& frame,
                                     std::size_t index) {
    const auto& composite = frame.composites[index];
    if (!composite.index_count)
        return;
    const auto source = resources.target(device, encoder, root, format, ui_texture_layout, sampler,
                                         frame, composite.source);
    const auto destination = resources.target(device, encoder, root, format, ui_texture_layout,
                                              sampler, frame, composite.destination);
    if (resources.workspaces.size() <= index)
        resources.workspaces.resize(index + 1);
    auto& workspace = resources.workspaces[index];
    auto& textures = workspace.textures;
    textures.begin();
    textures.get(UiFilterSurface::Snapshot)
        .ensure(device, ui_texture_layout, sampler, composite.width, composite.height, format);
    WGPUTexelCopyTextureInfo from{};
    from.texture = source.texture;
    from.aspect = WGPUTextureAspect_All;
    from.origin = {static_cast<std::uint32_t>(composite.left),
                   static_cast<std::uint32_t>(composite.top), 0};
    WGPUTexelCopyTextureInfo to{};
    to.texture = textures.get(UiFilterSurface::Snapshot).value.texture;
    ...
    if (!plan.draws.empty())
        resources.ensure_pipeline(device);
    for (std::size_t i = plan.draws.size(); i < workspace.passes.size(); ++i)
        workspace.passes[i].release();
    ...
    for (std::size_t i = 0; i < plan.draws.size(); ++i) {
        ...
        output.ensure(device, ui_texture_layout, sampler, draw.width, draw.height,
                      WGPUTextureFormat_RGBA16Float);
        auto& resources_pass = workspace.passes[i];
        if (!resources_pass.buffer) {
            WGPUBufferDescriptor descriptor = WGPU_BUFFER_DESCRIPTOR_INIT;
            descriptor.size = sizeof(draw.uniforms);
            descriptor.usage = WGPUBufferUsage_Uniform | WGPUBufferUsage_CopyDst;
            resources_pass.buffer = require_dawn_resource(
                wgpuDeviceCreateBuffer(device, &descriptor), "UI filter uniforms");
            WGPUBindGroupEntry uniform_entry = WGPU_BIND_GROUP_ENTRY_INIT;
            uniform_entry.binding = 0;
            uniform_entry.buffer = resources_pass.buffer;
            uniform_entry.size = sizeof(draw.uniforms);
            WGPUBindGroupDescriptor group = WGPU_BIND_GROUP_DESCRIPTOR_INIT;
            group.layout = resources.uniform_layout;
            group.entryCount = 1;
            group.entries = &uniform_entry;
        ...
        if (!resources_pass.texture_group || resources_pass.source != source_view ||
            resources_pass.shadow != shadow_view || resources_pass.sampler != sampler) {
            if (resources_pass.texture_group)
                wgpuBindGroupRelease(resources_pass.texture_group);
            ...
            entries[2].textureView = shadow_view;
            entries[3].sampler = sampler;
            WGPUBindGroupDescriptor descriptor = WGPU_BIND_GROUP_DESCRIPTOR_INIT;
            descriptor.layout = resources.texture_layout;
            descriptor.entryCount = entries.size();
            descriptor.entries = entries.data();
            resources_pass.texture_group = require_dawn_resource(
                wgpuDeviceCreateBindGroup(device, &descriptor), "UI filter texture group");
            resources_pass.source = source_view;
            resources_pass.shadow = shadow_view;
            resources_pass.sampler = sampler;
        }
        WGPURenderPassColorAttachment attachment = WGPU_RENDER_PASS_COLOR_ATTACHMENT_INIT;
        attachment.view = output.value.view;
        attachment.loadOp = WGPULoadOp_Clear;
        attachment.storeOp = WGPUStoreOp_Store;
    ...
    WGPURenderPassColorAttachment attachment = WGPU_RENDER_PASS_COLOR_ATTACHMENT_INIT;
    attachment.view = destination.view;
