...
void validate_request(const std::string& url, const HttpRequest& request) {
    ...
    if (request.method.empty() ||
        !std::all_of(request.method.begin(), request.method.end(), token_character))
    ...
    if (request.body && request.body->size() > maximum_body)
...
std::wstring wide(const std::string& value) {
    ...
    if (!result)
        throw std::runtime_error("Invalid UTF-8 in HTTP request.");
    return std::move(*result);
}

std::string narrow(const wchar_t* value, int length) {
    const auto count = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, value, length, nullptr, 0,
                                           nullptr, nullptr);
    if (!count)
        throw std::runtime_error("HTTP URL conversion failed.");
    std::string result(static_cast<std::size_t>(count), '\0');
    if (!WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, value, length, result.data(), count,
                             nullptr, nullptr))
        throw std::runtime_error("HTTP URL conversion failed.");
    return result;
}
...
void require(BOOL success) {
    if (!success)
        throw std::runtime_error("HTTP transport failed (Windows error " +
...
HttpResponseData perform_http_request(const std::string& url, HttpRequest request,
    ...
    if (normalized == "DELETE" || normalized == "GET" || normalized == "HEAD" ||
        normalized == "OPTIONS" || normalized == "POST" || normalized == "PUT")
        request.method = normalized;
    if (normalized == "CONNECT" || normalized == "TRACE" || normalized == "TRACK")
        throw std::runtime_error("This HTTP method is forbidden by fetch.");
    validate_request(url, request);
    const bool content_type =
    ...
    if (const auto fragment = path.find(L'#'); fragment != std::wstring::npos)
        path.resize(fragment);
    if (path.empty())
        path = L"/";
    InternetHandle session(WinHttpOpen(L"bblitec/native", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,
    ...
    InternetHandle message(
        WinHttpOpenRequest(connection.value, wide(request.method).c_str(), path.c_str(), nullptr,
                           WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES,
                           parts.nScheme == INTERNET_SCHEME_HTTPS ? WINHTTP_FLAG_SECURE : 0));
    DWORD disabled = WINHTTP_DISABLE_COOKIES | WINHTTP_DISABLE_AUTHENTICATION;
    require(WinHttpSetOption(message.value, WINHTTP_OPTION_DISABLE_FEATURE, &disabled,
                             sizeof(disabled)));
    std::wstring headers;
    for (const auto& [name, value] : request.headers) {
    ...
    require(WinHttpSendRequest(message.value, headers.c_str(), static_cast<DWORD>(headers.size()),
                               request.body ? const_cast<char*>(request.body->data()) : nullptr,
                               length, length, 0));
    ...
    require(WinHttpQueryHeaders(
        message.value, WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER,
    ...
    if (!url_length)
        throw std::runtime_error("HTTP response has no URL.");
    ...
    for (;;) {
        ...
        DWORD read = 0;
        require(
            WinHttpReadData(message.value, chunk.data(), static_cast<DWORD>(chunk.size()), &read));
        if (!read)
            break;
        if (read > maximum_body - response.body.size())
            throw std::runtime_error("HTTP response body exceeds 32 MiB.");
    ...
    const std::unique_ptr<CURL, decltype(&curl_easy_cleanup)> client(curl_easy_init(),
                                                                     curl_easy_cleanup);
    if (!client)
        throw std::runtime_error("HTTP client allocation failed.");
    auto option = [&](CURLoption name, auto value) {
        if (curl_easy_setopt(client.get(), name, value) != CURLE_OK)
            throw std::runtime_error("HTTP client option failed.");
    };
    ...
    for (const auto& [name, value] : request.headers) {
        auto* appended =
            curl_slist_append(raw_headers, (name + (value.empty() ? ";" : ": " + value)).c_str());
        if (!appended)
            throw std::bad_alloc();
    ...
    if (request.method == "HEAD")
        option(CURLOPT_NOBODY, 1L);
    option(CURLOPT_HTTPHEADER, raw_headers);
    option(
        CURLOPT_WRITEFUNCTION,
        +[](char* data, std::size_t size, std::size_t count, void* target) -> std::size_t {
            auto& body = *static_cast<std::vector<std::uint8_t>*>(target);
            if (count && size > (maximum_body - body.size()) / count)
                return 0;
    ...
    if (performed != CURLE_OK)
        throw std::runtime_error(std::string("HTTP transport failed: ") +
                                 curl_easy_strerror(performed));
    long status = 0;
    char* final_url = nullptr;
    if (curl_easy_getinfo(client.get(), CURLINFO_RESPONSE_CODE, &status) != CURLE_OK ||
        curl_easy_getinfo(client.get(), CURLINFO_EFFECTIVE_URL, &final_url) != CURLE_OK ||
        !final_url)
        throw std::runtime_error("HTTP response metadata unavailable.");
    response.status = static_cast<double>(status);
