 ...
 * unavailable; failures after acquisition propagate through take_latest(). */
class WindowFrameClock {
public:
    using Clock = std::chrono::steady_clock;

    explicit WindowFrameClock(bool cpu_profile = false) : cpu_profile_(cpu_profile) {
#ifdef _WIN32
        api_ = acquire_api();
        if (api_)
...
private:
    ...
    void run() noexcept {
        try {
            ...
            constexpr DWORD display_occluded = 0xC01E0006UL;
            ...
            for (;;) {
                ...
                if (status == WAIT_TIMEOUT)
                    continue;
                if (status == display_occluded) {
                    // Retry availability without generating frames or busy-spinning.
                    const auto retry = WaitForSingleObject(api_->stop, timeout_ms);
