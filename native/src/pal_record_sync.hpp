...
template <class Buffer> struct VersionedGpuBuffer {
...
void reconcile_ordered_records(const std::vector<Handle>& handles, std::vector<Resource>& resources,
    ...
    try {
        for (std::size_t index = 0; index < handles.size(); ++index) {
            ...
            if (previous[index] == missing)
                added[index].emplace(create(handles[index]));
        }
    } catch (...) {
        for (auto& resource : added)
            if (resource)
                release(*resource);
        throw;
    }
    for (std::size_t index = 0; index < handles.size(); ++index) {
        next.push_back(previous[index] == missing ? std::move(*added[index])
                                                  : std::move(resources[previous[index]]));
    }
    for (std::size_t index = 0; index < resources.size(); ++index) {
        if (!used[index])
            release(resources[index]);
    }
...
void sync_retained_textures(const std::vector<Source>& sources, Exists&& exists,
    ...
    if (std::any_of(sources.begin(), sources.end(),
                    [](const Source& source) { return source.disposed; })) {
        validate_retired();
    }
    for (std::size_t index = 0; index < sources.size(); ++index) {
        if (sources[index].disposed)
            release(index);
        else if (!exists(index))
            create(index, sources[index]);
    }
}
