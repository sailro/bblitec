...
MediaQueryList::MediaQueryList(std::string query, double (*read_pixel_ratio)(),
    ...
    if (std::regex_match(query, result, resolution)) {
        ...
        throw std::invalid_argument(
            "Only resolution and reduced-motion media queries are admitted by the Window realm.");
    }
    matches_ = matches();
}

bool MediaQueryList::matches() const {
    return feature_ == Feature::Resolution ? resolution_ == read_pixel_ratio_()
                                           : reduce_ == read_motion_preference_();
}
