...
public:
    ...
    void performDiscreteCollisionDetection() override {
        measure(times.collision,
                [&] { btDiscreteDynamicsWorld::performDiscreteCollisionDetection(); });
    }
    void updateAabbs() override {
        measure(times.aabbs, [&] { btDiscreteDynamicsWorld::updateAabbs(); });
    }
    void computeOverlappingPairs() override {
        measure(times.broadphase, [&] { btDiscreteDynamicsWorld::computeOverlappingPairs(); });
    }
    void solveConstraints(btContactSolverInfo& info) override {
        measure(times.constraints, [&] { btDiscreteDynamicsWorld::solveConstraints(info); });
    }
    void calculateSimulationIslands() override {
        measure(times.islands, [&] { btDiscreteDynamicsWorld::calculateSimulationIslands(); });
    }
    void predictUnconstraintMotion(btScalar delta) override {
        measure(times.predict, [&] { btDiscreteDynamicsWorld::predictUnconstraintMotion(delta); });
    }
    void integrateTransforms(btScalar delta) override {
        measure(times.integrate, [&] { btDiscreteDynamicsWorld::integrateTransforms(delta); });
    }
    void updateActivationState(btScalar delta) override {
        measure(times.activate, [&] { btDiscreteDynamicsWorld::updateActivationState(delta); });
    }
    void synchronizeMotionStates() override {
        measure(times.synchronize, [&] { btDiscreteDynamicsWorld::synchronizeMotionStates(); });
    }
};
