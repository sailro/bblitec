...
// Match the standalone sprite/text WebGPU clear conversion. Metal fast clears
// can truncate fractional UNORM values on large targets; use the nearest byte.
...
inline void generate_texture_mipmaps(
    ...
    // Metal's generateMipmapsForTexture averages encoded sRGB values.
    // Match the pin's texture/generate-mipmaps.ts: bilinear sample and
    // render into each next level so sRGB decode/encode surrounds filtering.
